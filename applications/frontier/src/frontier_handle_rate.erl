...
is_device_defaults(JObj) ->
    ...
    frontier_utils:is_device(JObj) andalso
        IsDefaultRates.
...
-spec lookup_rate_limit_records(kz_term:ne_binary(), boolean(), kz_term:ne_binaries()) ->
    kz_json:object().
...
build_list_of_querynames(Entity, IncludeRealm) ->
    ...
    QueryName =
        case Realm =/= Entity of
            'true' -> Username;
            'false' -> Realm
        end,
    case
        IncludeRealm andalso
            Realm =/= Entity
    of
        'true' -> [Realm, Username];
        'false' -> [QueryName]
...
-spec run_rate_limits_query(
    kz_term:ne_binary(), kz_term:ne_binary(), boolean(), kz_term:ne_binaries()
) -> kz_json:objects().
run_rate_limits_query(Entity, AccountDB, IncludeRealm, MethodList) ->
...
fold_responses(Record, Acc) ->
    ...
    S1 =
        case
            RPM =/= 'undefined' andalso
                kz_json:get_value([<<"Minute">>, JsonMethod], Section)
        of
    ...
    S2 =
        case
            RPS =/= 'undefined' andalso
                kz_json:get_value([<<"Second">>, JsonMethod], Section)
        of
            'undefined' -> kz_json:set_value([<<"Second">>, JsonMethod], RPS, S1);
            _ -> S1
        end,
    S3 =
        case Entity =/= ?DEVICE_DEFAULT_RATES of
...
construct_records(Method, Entity, RPM, RPS) ->
    ...
    RPMObject = kz_json:from_list([
        {<<"type">>, Type},
        {?MINUTE, RPM}
    ]),
    RPSObject = kz_json:from_list([
        {<<"type">>, Type},
        {?SECOND, RPS}
    ]),
...
-spec fetch_rates_from_sys_config(
...
-spec fetch_rates(kz_term:ne_binaries(), boolean(), kz_term:ne_binaries(), kz_term:ne_binary()) ->
    kz_json:objects().
fetch_rates(_, _, [], _) ->
...
fetch_rates(EntityList, IncludeRealm, MethodList, AccountDB) ->
    ...
    ViewOpts = [{'keys', Keys}],
    Results =
        case kz_datamgr:get_results(AccountDB, ?RATES_CROSSBAR_LISTING, ViewOpts) of
            {'ok', JObjs} ->
                lager:info("Got ~p records for entities ~p from db document", [
                    length(JObjs), EntityList
                ]),
                JObjs;
            _ ->
...
-spec fetch_from_parents(kz_term:ne_binary(), kz_term:ne_binaries(), kz_term:ne_binary()) ->
    kz_json:objects().
...
-spec check_fallbacks(kz_term:ne_binaries(), kz_term:ne_binaries(), kz_term:ne_binary()) ->
    kz_json:objects().
check_fallbacks(Tree, MethodList, Realm) ->
    Result = lists:foldl(
        fun(X, Acc) -> check_fallback(X, Acc, MethodList, Realm) end, 'empty', Tree
    ),
    case Result of
        'empty' -> [];
        _ -> Result
...
-spec check_fallback(
    kz_term:ne_binary(), atom() | kz_json:objects(), kz_term:ne_binaries(), kz_term:ne_binary()
) ->
...
-spec handle_db_response(kz_json:objects(), kz_json:objects(), kz_json:objects(), boolean()) ->
    rates_ret().
