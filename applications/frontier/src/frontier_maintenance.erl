...
lookup_rate_limits(Entity) ->
    ...
    lists:foreach(
        fun(S) -> print_limits(S, kz_json:get_value(S, Limits)) end,
        [<<"Device">>, <<"Realm">>]
    ).
...
print_limits(Section, Rates) ->
    ...
    Keys = [
        frontier_handle_rate:name_to_method(N)
     || N <- frontier_handle_rate:names()
    ],
    lists:foreach(
        fun(Key) ->
            io:format(
                "~-15s: ~7.10B/m ~7.10B/s~n",
...
update_system_default(Path, Value) ->
    ...
    {'ok', _} = kapps_config:set(
        ?APP_NAME, <<"rate_limits">>, kz_json:set_value(Keys, NewRate, Rates)
    ),
    'ok'.
