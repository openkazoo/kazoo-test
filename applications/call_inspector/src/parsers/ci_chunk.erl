...
-record(ci_chunk, {
    ...
    %% Parsing Kamailio logs: this can be undefined (DON'T parse them)
    c_seq :: kz_term:api_ne_binary()
}).
-type chunk() :: #ci_chunk{}.
...
-spec src(chunk() | kz_term:ne_binary()) ->
    kz_term:ne_binary() | {kz_term:ne_binary(), pos_integer()}.
...
-spec dst(chunk() | kz_term:ne_binary()) ->
    kz_term:ne_binary() | {kz_term:ne_binary(), pos_integer()}.
...
find_previous_packet(
    ...
    try
        lists:last(PreviousPackets)
    catch
        'error':'function_clause' -> 'no_previous'
    end.
