...
-spec make_and_store_chunk(
    atom(), kz_term:ne_binary(), pos_integer(), kz_term:ne_binary(), pos_integer(), data()
) ->
    pos_integer().
...
cleanse_data_and_get_timestamp(Data0) ->
    lists:foldl(
        fun cleanse_data_fold/2,
        {[], 'undefined'},
        Data0
    ).

-spec cleanse_data_fold(
    datum(),
    cleanse_acc()
) -> cleanse_acc().
...
from([<<"log|source ", From/binary>> | _Data], Default) -> get_ip(From, Default);
from([_Line | Lines], Default) -> from(Lines, Default).
...
to([], Default) -> Default;
to([<<"start|received internal reply", _/binary>> | Data], Default) -> to(Data, Default);
to([<<"start|", _/binary>> | _Data], Default) -> Default;
to([<<"pass|", To/binary>> | _Data], Default) -> get_ip(To, Default);
to([_Datum | Data], Default) -> to(Data, Default).
...
from_port([<<"log|source ", From/binary>> | _Data], Default) -> get_port(From, Default);
from_port([_Line | Lines], Default) -> from_port(Lines, Default).
...
to_port([<<"start|received internal reply", _/binary>> | Data], Default) -> to_port(Data, Default);
to_port([<<"start|", _/binary>> | _Data], Default) -> Default;
to_port([<<"pass|", To/binary>> | _Data], Default) -> get_port(To, Default);
to_port([_Datum | Data], Default) -> to(Data, Default).
