...
-export([
    set_threshold/3, set_threshold/4,
...
hangups_summary() ->
    Hangups = [
        {Name, hangups_query_listener:meter_resp(Name)}
...
print_stats([]) ->
    io:format("No data found for request\n");
print_stats(Stats) ->
    io:format(
        ?STAT_SUMMARY_FORMAT,
...
print_stat({Name, Stats}) ->
    AccountId =
        case hangups_util:meter_account_id(Name) of
            'undefined' -> <<>>;
            ID -> ID
        end,
...
set_threshold(HangupCause, Metric, LoadAvg) ->
    case is_valid_request(HangupCause, Metric, LoadAvg) of
        'false' -> 'ok';
        'true' -> save_threshold(HangupCause, Metric, LoadAvg, fun kapps_config:set_default/3)
    end.
...
-spec save_threshold(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), save_fun()) ->
    'ok'.
save_threshold(HangupCause, Metric, LoadAvg, SaveFun) ->
    ConfigName = hangups_util:meter_name(HangupCause),
...
is_valid_hangup_cause(HangupCause) ->
    ...
    case lists:member(HangupCause, HangupCauses) of
        ...
        'false' ->
            io:format(
                "  '~s' not currently monitored, adding to ~s",
                [
                    HangupCause,
...
set_monitor_threshold(HangupCause, TOM) ->
    ...
    update_monitor_thresholds(HangupCause, ThresholdOnMinute) andalso
...
update_monitor_thresholds(HangupCause, ThresholdOnMinute) ->
    ...
    lists:foldl(
        fun({ThresholdName, MinutesPer}, Acc) ->
...
set_monitor_threshold(HangupCause, ThresholdName, T) ->
    ...
    set_monitor_threshold(
        kz_term:to_upper_binary(HangupCause),
        ThresholdName,
        Threshold,
...
set_monitor_threshold(HangupCause, ThresholdName, Threshold, 'true') ->
    ...
    case kapps_config:get_float(ConfigName, ThresholdName) of
        ...
        _OldValue ->
            kapps_config:set_default(ConfigName, ThresholdName, Threshold),
            io:format("updating ~s for ~s to ~p from ~p~n", [
                ThresholdName, ConfigName, Threshold, _OldValue
            ])
    end,
    'true'.

-spec is_valid_threshold_name(kz_term:ne_binary()) -> boolean().
is_valid_threshold_name(<<"one">>) ->
    'true';
is_valid_threshold_name(<<"five">>) ->
    'true';
is_valid_threshold_name(<<"fifteen">>) ->
    'true';
is_valid_threshold_name(<<"day">>) ->
    'true';
is_valid_threshold_name(<<"mean">>) ->
    'true';
