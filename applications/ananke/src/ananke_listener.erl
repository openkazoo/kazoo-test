...
-spec schedule({kz_term:ne_binary(), time(), amqp_cron_task:execargs()}) ->
    {'ok', pid()} | {'error', any()}.
schedule({Name, Time, Action}) ->
    lager:info("scheduling ~p", [Name]),
    amqp_cron:schedule_task(Name, Time, Action).

-spec get_time_token_value(kz_json:object()) ->
    fun(({kz_term:ne_binary(), any()}) -> time_token_value()).
get_time_token_value(JObj) ->
    fun({TokenName, Default}) ->
        parse_time_token(TokenName, JObj, Default)
    end.

-spec parse_time_token(kz_term:ne_binary(), kz_json:object(), time_token_value()) ->
    time_token_value().
