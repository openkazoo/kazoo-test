...
handle_req(JObj, Props) ->
    'true' =
        props:get_value(<<"skip_verification">>, Props, 'false') orelse
            kapi_notifications:voicemail_saved_v(JObj),
    ...
    case kzd_voicemail_box:owner_id(VMBoxJObj) of
        ...
        UserId ->
            ...
            Number = get_first_defined([
                {<<"number">>, VMBoxNotifyJObj},
                {<<"number">>, UserNotifyJObj}
            ]),
            IsDisabled = kz_term:is_true(
                get_first_defined([
                    {<<"disabled">>, VMBoxNotifyJObj},
...
get_voicemail_number(AccountDb, Mailbox) ->
    {'ok', Callflows} = kz_datamgr:get_results(
        AccountDb,
...
is_voicemail_cf(JObj) ->
    ...
    IsFlow =
        kz_json:is_json_object(FlowJObj) andalso
            not kz_json:is_empty(FlowJObj),
    case
        {IsFlow,
            IsFlow andalso
                kz_json:get_value(<<"module">>, FlowJObj) =:= <<"voicemail">> andalso
                kz_json:get_value([<<"data">>, <<"action">>], FlowJObj) =:= <<"check">>}
    of
        {'false', _} -> 'false';
        {'true', 'true'} -> 'true';
...
build_originate_req(#args{
    ...
    CustomChannelVars = kz_json:from_list([
        {<<"Account-ID">>, AccountId},
        {<<"Owner-ID">>, UserId},
        {<<"AutoAnswer">>, 'true'},
    ...
    ApplicationData = kz_json:from_list([
        {<<"Route">>, VMNumber},
        {<<"Custom-Channel-Vars">>, CustomChannelVars}
    ]),
...
get_schedule_from_attempts_interval(Attempts, Interval) when
    is_integer(Attempts), is_integer(Interval), Interval > 0
->
...
get_callback_timeout(VMBoxJObj, UserJObj, AccountJObj) ->
    ...
    of
        ...
        CallTimeout ->
            kz_term:to_integer(CallTimeout)
    end.
