...
-define(RESPONDERS, [
    {{'omnip_subscriptions', 'handle_kamailio_subscribe'}, [{<<"presence">>, <<"subscription">>}]},
    {{'omnip_subscriptions', 'handle_kamailio_notify'}, [{<<"presence">>, <<"notify">>}]},
    {{'omnip_subscriptions', 'handle_mwi_update'}, [{<<"presence">>, <<"mwi_update">>}]},
    {{'omnip_subscriptions', 'handle_dialog_update'}, [{<<"presence">>, <<"dialog_update">>}]}
]).
...
-define(SUBSCRIPTIONS_SYNC_ENABLED,
    kapps_config:get_is_true(?CONFIG_CAT, <<"subscriptions_sync_enabled">>, 'false')
).
...
handle_cast({'gen_listener', {'federators_consuming', _IsConsuming}}, State) ->
    {'noreply', State};
handle_cast(
    'send_sync', #state{subs_pid = Pid, queue = Queue, consuming = IsConsuming} = State
) when
    Pid =:= 'undefined' orelse
        Queue =:= 'undefined' orelse
        IsConsuming =:= 'false'
->
    {'noreply', State};
...
handle_cast('send_sync', #state{consuming = 'false'} = State) ->
    {'noreply', State};
handle_cast(
    'send_sync', #state{subs_pid = Pid, queue = Queue, consuming = 'true', sync = 'false'} = State
) ->
...
maybe_sync_subscriptions('false', _) ->
    'ok';
