...
-export([
    current_subscriptions/0, current_subscriptions/1, current_subscriptions/2,
...
print_subscription(JObj, Now) ->
    ExpiresIn =
        kz_json:get_integer_value(<<"expires">>, JObj) -
...
subscribe(Realm, User) ->
    Prop = [
        {<<"User">>, <<"sip:", User/binary, "@", Realm/binary>>},
        {<<"Expires">>, 1},
        {<<"Queue">>, <<>>}
        | kz_api:default_headers(?APP_NAME, ?APP_VERSION)
    ],
    case
        kz_amqp_worker:call_custom(
            Prop,
    ...
    of
        {'ok', UpJObj} ->
            io:format(
                "Sent subscription for ~s@~s, recv'd update of state ~s~n",
...
-spec send_mwi_update(
    kz_term:ne_binary(), kz_term:ne_binary() | integer(), kz_term:ne_binary() | integer()
) -> 'ok'.
