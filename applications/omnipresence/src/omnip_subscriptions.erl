...
-define(EXPIRE_SUBSCRIPTIONS,
    kapps_config:get_integer(?CONFIG_CAT, <<"expire_check_ms">>, ?MILLISECONDS_IN_SECOND)
).
...
-define(VM_NUMBER(A),
    kapps_account_config:get_global(A, ?CONFIG_CAT, ?VM_NUMBER_KEY, ?DEFAULT_VM_NUMBER)
).
...
handle_info(
    {'timeout', Ref, ?EXPIRE_MESSAGE} = _R, #state{expire_ref = Ref, ready = 'true'} = State
) ->
    ...
    {'noreply', State#state{
        expire_ref = start_expire_ref(),
        other_nodes_count = kz_nodes:whapp_count(?APP_NAME)
    }};
handle_info(
    {'timeout', Ref, ?EXPIRE_MESSAGE} = _R, #state{expire_ref = Ref, ready = 'false'} = State
) ->
    {'noreply', State#state{
        expire_ref = start_expire_ref(),
        other_nodes_count = kz_nodes:whapp_count(?APP_NAME)
...
subscribe_to_record(JObj) ->
    ...
    Version =
        case kz_json:get_value(<<"Subscription-ID">>, JObj) of
            'undefined' -> 1;
            _Else -> 2
        end,
    #omnip_subscription{
        ...
        stalker = kz_json:get_first_defined(
            [
                <<"Subscription-ID">>,
                <<"Server-ID">>,
                <<"Queue">>
...
subscription_to_json(#omnip_subscription{
    user = User,
    from = From,
    stalker = Stalker,
    expires = Expires,
...
-spec find_subscriptions(ets:match_spec()) ->
    {'ok', subscriptions()}
    | {'error', 'not_found'}.
find_subscriptions(MatchSpec) ->
    try ets:select(table_id(), MatchSpec) of
        ...
        _E:_M ->
            lager:error("error fetching subscriptions : ~p : ~p", [_E, _M]),
            {'error', 'not_found'}
    end.
...
-spec search_for_subscriptions(
    kz_term:ne_binary() | '_', kz_term:ne_binary(), kz_term:ne_binary() | '_'
) -> subscriptions().
...
subscribe(
    ...
    case find_subscription(CallId) of
        {'ok', #omnip_subscription{
            timestamp = _T,
            expires = _E2
        }} ->
            lager:debug(
                "re-subscribe ~s/~s/~s expires in ~ps(prior remaining ~ps)",
...
notify(JObj) ->
    ...
    case find_subscription(CallId) of
        {'ok', #omnip_subscription{
            ...
            lager:debug("received notify reply ~B for ~s subscription (~s) of ~s to ~s", [
                Reply, Event, CallId, From, User
            ]),
            ets:update_element(
                table_id(),
                CallId,
...
maybe_probe(_, {<<"dialog">>, <<"*", _/binary>> = Username, Realm, _}) ->
    case kapps_util:get_account_by_realm(Realm) of
        {'ok', Account} ->
            ...
            S = size(VM),
            case Username of
                <<VM:S/binary, New/binary>> ->
                    omnip_util:request_probe(<<"message-summary">>, New, Realm);
                _ ->
                    'ok'
            end;
        _ ->
