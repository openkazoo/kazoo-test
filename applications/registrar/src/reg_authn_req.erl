...
send_auth_resp(
    ...
    lager:info("sending SIP authentication reply, with credentials for user ~s@~s", [
        Username, Realm
    ]),
...
maybe_get_presence_id(#auth_user{
...
get_presence_id(AccountDb, DeviceId) -> get_device_presence_id(AccountDb, DeviceId).
    ...
    create_custom_sip_headers(
        props:filter_undefined(
            [
                ...
                {<<"P-Associated-URI">>,
                    <<"<sip:", Username/binary, "@", AccountRealm/binary, ">">>},
                {<<"P-Kazoo-Primary-Number">>, Number}
            ]
        )
    );
create_custom_sip_headers(?ANY_AUTH_METHOD, _) ->
    'undefined'.
...
get_auth_user_in_agg(Username, Realm) ->
    ...
    case
        kapps_config:get_is_true(?CONFIG_CAT, <<"use_aggregate">>, 'true') andalso
            kz_datamgr:get_results(?KZ_SIP_DB, <<"credentials/lookup">>, ViewOptions)
    of
        'false' ->
...
-spec jobj_to_auth_user(
    kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()
) ->
    {'ok', auth_user()}
    | {'error', any()}.
jobj_to_auth_user(JObj, Username, Realm, Req) ->
    ...
    AuthUser = #auth_user{
        ...
        suppress_unregister_notifications = kz_json:is_true(
            <<"suppress_unregister_notifications">>, AuthDoc
        ),
...
add_account_name(#auth_user{account_id = AccountId} = AuthUser) ->
    case kzd_accounts:fetch(AccountId) of
        ...
        {'ok', Account} ->
            ...
            AuthUser#auth_user{
                account_name = kzd_accounts:name(Account),
                account_realm = Realm,
...
maybe_msisdn(AuthUser) ->
    AuthUser.

-spec maybe_msisdn_from_callflows(auth_user(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    auth_user().
maybe_msisdn_from_callflows(
    #auth_user{account_db = AccountDB} = AuthUser,
    Type,
    Id
) ->
    ViewOptions = [
    ...
    case kz_datamgr:get_results(AccountDB, <<"callflows/msisdn">>, ViewOptions) of
        ...
        {'ok', [User | _]} ->
            ...
            lager:debug(
                "found msisdn ~s for ~s@~s in account db: ~s",
...
-spec encryption_method_map(kz_term:proplist(), kz_term:api_binaries() | kz_json:object()) ->
    kz_term:proplist().
...
-spec maybe_enforce_security({auth_user(), kz_term:proplist()}) ->
    {auth_user(), kz_term:proplist()}.
...
-spec maybe_set_encryption_flags({auth_user(), kz_term:proplist()}) ->
    {auth_user(), kz_term:proplist()}.
