...
-define(TO_EMAILS(Emails, Type),
    kz_json:from_list([
        {<<"to">>, kz_json:from_list([{<<"email_addresses">>, Emails}, {<<"type">>, Type}])}
    ])
).
...
find_addresses_test_() ->
    [
        {"find_addresses for type " ++ kz_term:to_list(Type), generate_find_addresses_test(Type)}
     || %%, 'undefined', ?EMAIL_ADMINS]
        Type <- [?EMAIL_SPECIFIED, ?EMAIL_ORIGINAL]
    ...
    [
        {"check defined in template with path 'to.email_addresses' even if emails are defined in datajobj",
            ?_assertEqual(
                ...
                props:get_value(
                    <<"to">>,
                    teletype_util:find_addresses(
                        ?EMAILS_IN_DATAJOBJ_1(Type),
                        ?EMAILS_IN_TEMPLATE_1(Type),
                        <<"some_notification">>
                    )
                )
            )},
        ...
        {"fallback to datajobj with path 'to.email_addresses' if not found in template",
            ?_assertEqual(
                [<<"another_address@test.com">>],
                props:get_value(
                    <<"to">>,
                    teletype_util:find_addresses(
                        ?EMAILS_IN_DATAJOBJ_1(Type), kz_json:new(), <<"some_notification">>
                    )
                )
            )},
        ...
        {"email is undefined",
            ?_assertEqual(
                ...
                props:get_value(
                    ...
                    teletype_util:find_addresses(
                        ?TO_EMAILS('undefined'),
                        ?EMAILS_IN_TEMPLATE_1(?EMAIL_ORIGINAL),
                        <<"some_notification">>
                    )
                )
            )},
        ...
        {"email is a JSON",
            ?_assertEqual(
                ...
                props:get_value(
                    <<"to">>,
                    teletype_util:find_addresses(
                        ?TO_EMAILS([<<"a_json@test.com">>], ?EMAIL_ORIGINAL),
                        ?EMAILS_IN_TEMPLATE_1(?EMAIL_ORIGINAL),
                        <<"some_notification">>
                    )
                )
