...
-export([
    voicemail_to_email/1, voicemail_to_email/2,
...
-spec voicemail_to_email(kz_term:ne_binary()) ->
    'ok'
    | {'ok', kz_json:objects()}
    | {'error', any()}.
...
fax_inbound_to_email(AccountId) ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"faxes/crossbar_listing">>, ['include_docs']) of
        ...
        {'error', _E} -> lager:debug("failed to find faxes: ~p", [_E])
    end.
...
-spec fax_inbound_to_email(kz_term:ne_binary(), kz_term:ne_binary()) ->
