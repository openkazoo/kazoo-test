    ...
    kz_json:from_list(
        [
            ...
            ?MACRO_VALUE(
                ...
                <<"invoice_card_type">>,
                <<"Credit Card Type">>,
                <<"Default payment method card type">>
            ),
            ?MACRO_VALUE(
                <<"invoice.last_four">>,
                <<"invoice_last_four">>,
            ...
            ?MACRO_VALUE(
                <<"invoice.due_date.local">>,
                <<"invoice_due_date_local">>,
                <<"Due Date (Local)">>,
                <<"Invoice due date (Local)">>
            ),
            ?MACRO_VALUE(
                <<"invoice.due_date.timezone">>,
                <<"invoice_due_date_timezone">>,
                <<"Due Date Local Timezone">>,
                <<"Invoice due date (Local Timezone)">>
            ),
            ?MACRO_VALUE(
                <<"invoice.due_date.utc">>,
            ...
            ?MACRO_VALUE(
                ...
                <<"invoice_due_date_timestamp">>,
                <<"Due Date Timestamp (Gregorian)">>,
                <<"Due Date Timestamp (Gregorian)">>
            ),
            ?MACRO_VALUE(
                ...
                <<"Category name that the item belongs to">>
            ),
            ?MACRO_VALUE(
            ...
            ?MACRO_VALUE(
                <<"timestamp.timezone">>, <<"date_called_timezone">>, <<"Timezone">>, <<"Timezone">>
            ),
            ?MACRO_VALUE(
                <<"timestamp.timestamp">>,
...
fake_invoice(Now) ->
    ...
    DueDate = calendar:datetime_to_gregorian_seconds({
        kz_date:normalize({Year, Month + 1, 1}), {1, 0, 0}
    }),
                ...
                [
                    {<<"due_date">>,
                        teletype_util:fix_timestamp(
                            kz_json:get_integer_value(<<"due_date">>, DataJObj, kz_time:now_s())
                        )},
                    {<<"items">>, Items},
                    {<<"payment_method">>,
                        get_payment_token(
                            kz_json:values(
                                kz_json:get_json_value(<<"payment_token">>, DataJObj, kz_json:new())
                            )
                        )},
...
get_payment_token(Bookkeeper, Token) ->
    props:filter_undefined(
        [
            ...
            {<<"card_type">>,
                kz_json:get_ne_binary_value([<<"metadata">>, <<"card_type">>], Token)},
    ...
    case Quantity =:= 0 of
        ...
        'false' ->
            {TotalDiscounts + Discounts, Total + TotalAcc, [
                {Name, props:filter_undefined(ItemProps)} | CategoryAcc
            ]}
    end.
