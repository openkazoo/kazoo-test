...
process_req(DataJObj) ->
    ...
    Emails = maybe_add_user_email(
        BoxEmails, kzd_user:email(UserJObj), kzd_user:voicemail_notification_enabled(UserJObj)
    ),
    Values = [
        {<<"vmbox_doc">>, VMBoxJObj},
        {<<"user">>, UserJObj},
    ...
    case teletype_util:is_preview(DataJObj) of
        ...
        'true' -> maybe_process_req(kz_json:merge_jobjs(DataJObj, ReqData))
    end.
...
-spec maybe_add_user_email(kz_term:ne_binaries(), kz_term:api_binary(), boolean()) ->
    kz_term:ne_binaries().
...
maybe_process_req(DataJObj, false) ->
    Msg = io_lib:format(
        "requestor or box ~s has no emails or owner doesn't want emails",
...
do_process_req(DataJObj) ->
    ...
    case
        teletype_util:send_email(
            Emails, Subject, RenderedTemplates, props:get_value(<<"attachments">>, Macros0)
        )
    of
...
maybe_email_attachments(DataJObj, Macros) ->
    case
        kzd_vmboxes:include_message_on_notify(
            kz_json:get_json_value(<<"vmbox_doc">>, DataJObj)
        ) andalso
            (not teletype_util:is_preview(DataJObj))
    of
        'true' ->
...
email_attachments(DataJObj, Macros) ->
    ...
    VMJObj =
        case kvm_message:fetch(AccountId, VMId) of
            {'ok', JObj} -> JObj;
            {'error', _} -> throw({'error', 'no_attachment'})
        end,

    maybe_fetch_attachments(
        DataJObj, Db, VMId, get_file_name(VMJObj, Macros), kz_doc:attachments(VMJObj)
    ).

-spec maybe_fetch_attachments(
    kz_json:object(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
get_file_name(MediaJObj, Macros) ->
    ...
    CallerID =
        case
            {
                props:get_value([<<"caller_id">>, <<"name">>], Macros),
                props:get_value([<<"caller_id">>, <<"number">>], Macros)
            }
        of
            {'undefined', 'undefined'} ->
                props:get_value([<<"from">>, <<"user">>], Macros, <<"Unknown">>);
            {'undefined', Num} ->
                Num;
            {Name, _} ->
                Name
    ...
    FileName = iolist_to_binary([
        CallerID, $_, kz_time:pretty_print_datetime(LocalDateTime), $., Extension
    ]),
    ...
    props:filter_undefined(
        [
            ...
            {<<"vmbox_include_message_on_notify">>,
                kz_json:is_true(
                    [<<"vmbox_doc">>, <<"include_message_on_notify">>], DataJObj, 'true'
                )},
            {<<"msg_id">>, kz_json:get_value(<<"voicemail_id">>, DataJObj)},
            %% backward compatibility
            {<<"name">>, kz_json:get_value(<<"voicemail_id">>, DataJObj)},
...
maybe_get_transcription(DataJObj) ->
    case
        kzd_vmboxes:include_transcription_on_notify(
            kz_json:get_json_value(<<"vmbox_doc">>, DataJObj)
        )
    of
        'true' ->
