...
-spec add_attachments(kz_json:object(), kz_term:proplist(), boolean()) ->
    {kz_term:proplist(), attachments()}.
add_attachments(DataJObj, Macros, ShouldTerminate) ->
    IsPreview = teletype_util:is_preview(DataJObj),
    FaxDoc = kz_json:get_value(<<"fax_doc">>, DataJObj),
    case
        kz_json:is_json_object(FaxDoc) andalso
            not kz_json:is_empty(FaxDoc) andalso
            maybe_fetch_attachments(DataJObj, FaxDoc, Macros, IsPreview)
    of
...
-spec maybe_terminate(kz_term:proplist(), boolean(), boolean()) ->
    {kz_term:proplist(), attachments()}.
...
-spec to_email_addresses(
    kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary() | kz_term:api_binaries()
) -> kz_term:api_binaries().
to_email_addresses(_, _, ?NE_BINARY = Email) ->
    [Email];
to_email_addresses(_, _, Emails) when
...
get_faxbox_doc(DataJObj, FaxDoc) ->
    ...
    case
        kz_term:is_ne_binary(BoxId) andalso
            teletype_util:open_doc(<<"faxbox">>, BoxId, DataJObj)
    of
        'false' -> kz_json:new();
        {'ok', J} -> J;
        {'error', _} -> kz_json:new()
...
-spec get_fax_doc(kz_term:ne_binary(), kz_term:api_binary()) ->
...
-spec find_timezone(kz_term:api_ne_binary() | kz_json:object(), kz_json:object()) ->
    kz_term:ne_binary().
...
fax_db(DataJObj, FaxId) ->
    case kapi_notifications:account_db(DataJObj, 'true') of
        'undefined' ->
            maybe_get_fax_db_from_id(
                kz_json:get_ne_binary_value(<<"account_id">>, DataJObj), FaxId
            );
        Db ->
            maybe_get_fax_db_from_id(Db, FaxId)
    end.

-spec maybe_get_fax_db_from_id(kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
    kz_term:ne_binary().
maybe_get_fax_db_from_id('undefined', _) ->
    ?KZ_FAXES_DB;
maybe_get_fax_db_from_id(?MATCH_MODB_SUFFIX_ENCODED(_, _, _) = Db, _) ->
    Db;
maybe_get_fax_db_from_id(Db, ?MATCH_MODB_PREFIX(Year, Month, _)) ->
    kazoo_modb:get_modb(kz_util:format_account_id(Db), Year, Month);
maybe_get_fax_db_from_id(Db, _) ->
    Db.

-spec maybe_fetch_attachments(kz_json:object(), kz_json:object(), kz_term:proplist(), boolean()) ->
    attachments().
...
get_file_name(Macros, Ext) ->
    ...
    CallerID =
        case
            {
                props:get_value(<<"name">>, CallerIdMacros),
    ...
    LocalDateTime = props:get_value(
        [<<"date_called">>, <<"local">>], Macros, <<"0000-00-00_00-00-00">>
    ),
