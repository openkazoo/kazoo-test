...
port_request_data(DataJObj, TemplateId) ->
    PortReqId = kz_json:get_first_defined(
        [
            <<"port_request_id">>,
            [<<"port">>, <<"port_id">>]
        ],
    ...
    case teletype_util:open_doc(<<"port_request">>, PortReqId, DataJObj) of
        ...
        {'error', _Reason} ->
            lager:debug(
                "failed to open port request ~s doc, throwing no_attachment here, don't panic: ~p",
                [PortReqId, _Reason]
            ),
...
port_request_data(DataJObj, TemplateId, PortReqJObj) ->
    ...
    JObj = lists:foldl(
        fun(F, AccJObj) -> F(DataJObj, TemplateId, AccJObj) end,
        PortReqJObj,
        Routines
    ),
...
-spec fix_port_authority(kz_json:object(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
...
fix_numbers(_DataJObj, _TemplateId, PortReqJObj) ->
    Numbers = kz_json:foldl(
        fun(Number, _Value, Acc) -> [Number | Acc] end,
        [],
...
fix_billing(_DataJObj, _TemplateId, PortReqJObj) ->
    kz_json:foldl(
        fun(Key, Value, Acc) -> kz_json:set_value(<<"bill_", Key/binary>>, Value, Acc) end,
...
-spec fix_bill_object(kz_json:object(), [
...
fix_bill_object(PortReqJObj, [{Category, KeyMaps} | Rest]) ->
    NewPort = lists:foldl(
        fun(KeyMap, Acc) -> fix_bill_object(Acc, Category, KeyMap) end,
        PortReqJObj,
        KeyMaps
    ),
    fix_bill_object(NewPort, Rest).

-spec fix_bill_object(
    kz_json:object(),
    kz_term:ne_binary(),
...
-spec fix_reference_number(kz_json:object(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
fix_reference_number(_DataJObj, _TemplateId, PortReqJObj) ->
    kzd_port_requests:set_reference_number(
        PortReqJObj,
    ...
    case get_the_comment(DataJObj, PortReqJObj, IsPreview) of
        ...
        Comment ->
            ...
            Props = [
                ...
                {<<"timestamp">>, kz_json:get_value(<<"local">>, Date)}
...
fix_dates(DataJObj, _TemplateId, PortReqJObj) ->
    IsPreview = teletype_util:is_preview(DataJObj),
    lists:foldl(
        fun(Key, Value) -> fix_date_fold(Key, Value, IsPreview) end,
...
-spec fix_notifications(kz_json:object(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
...
fix_carrier(_DataJObj, _TemplateId, PortReqJObj) ->
    ...
    kz_json:set_values(
        ...
        %% not deleting the key for backward compatibility
        PortReqJObj
    ).

-spec fix_transfer_date(kz_json:object(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
fix_transfer_date(_DataJObj, _TemplateId, PortReqJObj) ->
    kz_json:set_values(
        [
            {<<"requested_port_date">>, kz_json:get_value(<<"transfer_date">>, PortReqJObj)},
            %% backward compatibility
            {<<"transfer_date">>,
                kz_json:get_value([<<"transfer_date">>, <<"local">>], PortReqJObj)}
        ],
        PortReqJObj
    ).

-spec fix_scheduled_date(kz_json:object(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
fix_scheduled_date(_DataJObj, _TemplateId, PortReqJObj) ->
    kz_json:set_values(
        [
            {<<"port_scheduled_date">>, kz_json:get_value(<<"scheduled_date">>, PortReqJObj)},
            {<<"scheduled_date">>,
                kz_json:get_value([<<"scheduled_date">>, <<"local">>], PortReqJObj)}
        ],
        PortReqJObj
    ).
...
maybe_add_reason(DataJObj, _TemplateId, PortReqJObj) ->
    ...
    case get_the_reason(DataJObj, PortReqJObj, IsPreview) of
        ...
        Reason ->
            ...
            Date = kz_json:from_list(teletype_util:fix_timestamp(Timestamp, DataJObj)),
            Props = [
                {<<"content">>, kzd_comment:content(Reason)},
                {<<"date">>, Date},
...
-spec maybe_add_user_data(kz_json:object(), kz_term:api_ne_binary(), boolean()) ->
    kz_term:proplist().
...
maybe_add_user_data(DataJObj, Author, 'false') ->
    ...
    UserId = kz_json:get_first_defined(
        [
            [<<"comment">>, <<"user_id">>],
            [<<"reason">>, <<"user_id">>]
        ],
...
maybe_add_attachments(DataJObj, TemplateId) ->
    maybe_add_attachments(
        DataJObj, is_attachable_template(TemplateId), teletype_util:is_preview(DataJObj)
    ).
...
get_attachment_fold(Name, Acc, PortReqId, Doc) ->
    case kz_datamgr:fetch_attachment(?KZ_PORT_REQUESTS_DB, PortReqId, Name) of
        ...
        {'error', _Reason} ->
            lager:debug("failed to get port request ~s attchament ~s: ~p", [
                PortReqId, Name, _Reason
            ]),
            throw({'error', 'no_attachment'})
    end.
...
-define(AUTHORITY_TEMPLATES, [
    teletype_port_cancel:id(),
...
maybe_fix_emails(DataJObj, TemplateId, 'false') ->
    ...
    AuthorityEmails = maybe_get_port_authority(
        DataJObj, TemplateId, lists:member(TemplateId, ?AUTHORITY_TEMPLATES)
    ),

    maybe_set_from_email(
        kz_json:set_values(
            [
                {<<"to">>, SubmitterEmails},
...
-spec maybe_get_submitter(kz_json:object(), kz_term:ne_binary(), boolean()) ->
...
is_comment_private(DataJObj, TemplateId) ->
    teletype_port_comment:id() =:= TemplateId andalso
        kzd_comment:is_private_legacy(
            kz_json:get_json_value(<<"comment">>, DataJObj, kz_json:new())
        ).
...
-spec maybe_get_port_authority(kz_json:object(), kz_term:ne_binary(), boolean()) ->
    kz_term:api_binaries().
...
maybe_use_port_support(PortAuthority) ->
    case kzd_whitelabel:fetch(PortAuthority) of
        ...
        {'error', _} ->
            lager:debug(
                "can't open port authority whitelabel doc, maybe using port authority admins"
            ),
            maybe_use_port_authority_admins(PortAuthority)
    end.
...
-spec maybe_use_master_admins(kz_term:ne_binary(), kz_term:api_ne_binary()) ->
    kz_term:api_binaries().
maybe_use_master_admins(TemplateId, PortAuthority) ->
    case kapps_util:get_master_account_id() of
        ...
        {'error', _} ->
            lager:debug(
                "failed to find port authority, master account is undefined, maybe using system template"
            ),
            maybe_use_system_emails(TemplateId)
    end.

-spec maybe_use_master_admins(kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:ne_binary()) ->
    kz_term:api_binaries().
...
maybe_use_master_admins(TemplateId, _, MasterAccountId) ->
    case teletype_util:find_account_admin_email(MasterAccountId) of
        'undefined' ->
            lager:debug(
                "master doesn't have any admins, maybe using default_to from system template"
            ),
            maybe_use_system_emails(TemplateId);
        [_ | _] = Admins ->
