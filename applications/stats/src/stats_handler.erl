...
-spec handle_req(kz_json:object(), kz_term:proplist()) ->
    'no_node_name'
    | 'no_tables'
    | 'no_items'
    | 'ok'.
...
collect_items([{Domain, Items} | Rest], Db) ->
    ...
    NewData =
        [{Key, Fun(Key, Value)} || {Key, Value} <- Items] ++
            [
                {Key, Value}
             || {Key, Value} <- DomainData,
...
get_next(Table, Row, Col) when
    is_list(Col), is_list(Row), is_binary(Table)
->
