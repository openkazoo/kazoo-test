...
maybe_ignore_app(JObj) ->
    ...
    {lists:member(AppName, ?IGNORED_APP), <<"ignoring cdr request from ", AppName/binary>>}.
...
prepare_and_save(AccountId, Timestamp, JObj) ->
    ...
    Routines = [
        ...
        %% due to interaction-timestamp MUST be called LAST
        fun update_pvt_parameters/3,
        fun save_cdr/3
    ],
    _ = lists:foldl(
        fun(F, J) -> F(AccountId, Timestamp, J) end,
        JObj,
        Routines
    ),
    'ok'.

-spec update_pvt_parameters(
    kz_term:api_ne_binary(), kz_time:gregorian_seconds(), kz_call_event:doc()
) ->
    kz_call_event:doc().
...
-spec update_ccvs_foldl(kz_json:get_key(), kz_json:json_term(), {
...
update_ccvs_foldl(Key, Value, {JObj, CCVs} = Acc) ->
    case kz_doc:is_private_key(Key) of
        'false' -> Acc;
        'true' -> {kz_json:set_value(Key, Value, JObj), kz_json:delete_key(Key, CCVs)}
    end.
...
-spec set_call_priority(kz_term:api_ne_binary(), kz_time:gregorian_seconds(), kz_call_event:doc()) ->
    kz_call_event:doc().
set_call_priority(_AccountId, _Timestamp, JObj) ->
    maybe_leak_ccv(JObj, <<"Call-Priority">>).

-spec set_recording_url(kz_term:api_ne_binary(), kz_time:gregorian_seconds(), kz_call_event:doc()) ->
    kz_call_event:doc().
set_recording_url(_AccountId, _Timestamp, JObj) ->
    maybe_leak_ccv(JObj, <<"Recording-Url">>).

-spec maybe_set_e164_destination(
    kz_term:api_ne_binary(), kz_time:gregorian_seconds(), kz_call_event:doc()
) -> kz_call_event:doc().
maybe_set_e164_destination(_AccountId, _Timestamp, JObj) ->
    maybe_leak_ccv(JObj, <<"E164-Destination">>).

-spec is_conference(kz_term:api_ne_binary(), kz_time:gregorian_seconds(), kz_call_event:doc()) ->
    kz_call_event:doc().
...
set_interaction(_AccountId, _Timestamp, JObj) ->
    ...
    Interaction = kz_call_event:custom_channel_var(
        JObj, <<?CALL_INTERACTION_ID>>, ?CALL_INTERACTION_DEFAULT
    ),
