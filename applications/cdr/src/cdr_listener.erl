...
-define(REFRESH_ENABLED,
    kapps_config:get_is_true(?CONFIG_CAT, <<"refresh_view_enabled">>, 'false')
).
...
handle_info('timeout', #state{counter = Counter} = State) ->
    ...
    case ?REFRESH_ENABLED of
        'false' -> {'noreply', State};
        'true' -> {'noreply', State#state{counter = refresh_views(Counter)}}
    end;
...
refresh_views(Counter) ->
    ...
    HandleTimeout =
        fun(AccountId, {Map, List}) ->
            ...
            case
                Now - From > RefreshTimeout andalso
                    Count > 0
            of
...
handle_account(Counter, 'undefined') ->
    Counter;
handle_account(Counter, AccountId) ->
    RefreshThreshold = ?REFRESH_THRESHOLD,
    Count =
        case value(maps:find(AccountId, Counter)) of
