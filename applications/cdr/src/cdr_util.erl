...
do_save_cdr(AccountMODb, Doc) ->
    MediaNames = check_media_names(
        kz_json:get_value([<<"custom_channel_vars">>, <<"media_names">>], Doc)
    ),
    MediaRecordings = check_media_names(
        kz_json:get_value([<<"custom_channel_vars">>, <<"media_recordings">>], Doc)
    ),

    NewDoc = kz_json:set_values(
        [
            {[<<"custom_channel_vars">>, <<"media_names">>], MediaNames},
            {[<<"custom_channel_vars">>, <<"media_recordings">>], MediaRecordings}
        ],
        Doc
    ),
