...
authorize(Request, Limits) ->
    lager:debug(
        "checking if account ~s has available per-minute credit",
...
maybe_credit_available(ReserveUnits, Limits) ->
    maybe_credit_available(ReserveUnits, Limits, 'false').
...
maybe_credit_available(ReserveUnits, Limits, IsReal) ->
    ...
    PerMinuteCost =
        case kz_term:is_true(IsReal) of
            'true' -> j5_channels:real_per_minute_cost(AccountId);
            'false' -> j5_channels:per_minute_cost(AccountId)
        end,
    maybe_prepay_credit_available(AvailableUnits - PerMinuteCost, ReserveUnits, Limits) orelse
        maybe_postpay_credit_available(AvailableUnits - PerMinuteCost, ReserveUnits, Limits).

-spec maybe_prepay_credit_available(kz_currency:units(), kz_currency:units(), j5_limits:limits()) ->
    boolean().
...
-spec maybe_postpay_credit_available(kz_currency:units(), kz_currency:units(), j5_limits:limits()) ->
    boolean().
maybe_postpay_credit_available(AvailableUnits, ReserveUnits, Limits) ->
    ...
    case j5_limits:allow_postpay(Limits) of
        'false' ->
            lager:debug(
                "account ~s is restricted from using postpay",
                [AccountId]
            ),
            'false';
        'true' when (AvailableUnits - ReserveUnits) > MaxPostpay ->
            lager:debug(
                "using postpay from account ~s $~w/$~w",
                [
                    AccountId,
                    kz_currency:units_to_dollars(ReserveUnits),
                    kz_currency:units_to_dollars(AvailableUnits)
        ...
        'true' ->
            lager:debug(
                "account ~s would exceed the maximum postpay amount $~w/$~w",
                [
                    AccountId,
                    kz_currency:units_to_dollars(AvailableUnits),
...
%% @end
%%------------------------------------------------------------------------------
-spec create_ledger_usage(
    kz_currency:units(), kz_currency:units(), j5_request:request(), j5_limits:limits()
) -> any().
