...
print_authz_summary_header() ->
    ...
    io:format(
        "| Account ID                       | Calls | Resource Calls | Allotments | Inbound Trunks | Outbound Trunks | Per Minute |~n"
    ),
    io:format(
        "+==================================+=======+================+============+================+=================+============+~n"
    ).
...
authz_details([Channel | Channels]) ->
    ...
    pretty_print_field(
        <<"Answered Duration">>, authz_details_duration(<<"Answered-Timestamp">>, Props, Timestamp)
    ),
    ...
    pretty_print_field(
        <<"Discount Percentage">>, props:get_value(<<"Discount-Percentage">>, Props)
    ),
...
-spec authz_details_duration(kz_term:ne_binary(), kz_term:proplist(), non_neg_integer()) ->
    iolist().
authz_details_duration(Key, Props, Timestamp) ->
    case props:get_integer_value(Key, Props) of
        'undefined' -> "0s";
        Created -> [kz_term:to_list(Timestamp - Created), "s"]
    end.
...
limits_summary([Limit | Limits]) ->
    case j5_limits:enabled(Limit) of
        ...
        'true' ->
            io:format(
                "| ~-32s | ~-5w | ~-14w | ~-10w | ~-3w | ~-3w | ~-4w | ~-5w | ~-10s | ~-11s |~n",
                [
...
limits_summary_prepay(Limit) ->
    case j5_limits:allow_prepay(Limit) of
        'false' -> "disabled";
        'true' -> current_balance(j5_limits:account_id(Limit))
    end.
...
limit_summary_header() ->
    ...
    io:format(
        "| Account ID                       | Calls | Resource Calls | Allotments |           Trunks         | Per Minute | Max Postpay |~n"
    ),
    io:format(
        "|                                  |       |                |            |  In | Out | Both | Burst |            |             |~n"
    ),
...
limits_details(Account) ->
    ...
    pretty_print_field(
        "  Max Postpay Amount",
        kz_currency:units_to_dollars(props:get_value('max_postpay_amount', Props))
    ),
    pretty_print_field(
        "  Reserve Amount", kz_currency:units_to_dollars(props:get_value('reserve_amount', Props))
    ),
    ...
    pretty_print_field(
        "  Resources Hard Limit", props:get_value('resource_consuming_calls', Props)
    ),
    pretty_print_field("  Inbound Trunks", props:get_value('inbound_trunks', Props)),
    pretty_print_field(
        "  Bundled Inbound Trunks", props:get_value('bundled_inbound_trunks', Props)
    ),
    pretty_print_field("  Outbound Trunks", props:get_value('outbound_trunks', Props)),
    pretty_print_field(
        "  Bundled Outbound Trunks", props:get_value('bundled_outbound_trunks', Props)
    ),
...
pretty_print_field(_, 'undefined') ->
    'ok';
