...
from_jobj(JObj) ->
    ...
    #request{
        ...
        reseller_billing = kz_json:get_ne_value(
            <<"Reseller-Billing">>, CCVs, <<"limits_enforced">>
        ),
...
authorized_by_reseller(Reason, #request{reseller_id = _ResellerId} = Request) ->
    lager:debug(
        "reseller ~s authorized channel: ~s",
        [_ResellerId, Reason]
    ),
    Request#request{
        reseller_billing = Reason,
...
authorized_by_account(Reason, #request{account_id = _AccountId} = Request) ->
    lager:debug(
        "account ~s authorized channel: ~s",
        [_AccountId, Reason]
    ),
    Request#request{
        account_billing = Reason,
