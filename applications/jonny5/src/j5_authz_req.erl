...
maybe_inbound_account_by_ip(Request, IP) ->
    ...
    case
        j5_request:call_direction(Request) =:= <<"inbound">> andalso
            lists:member(AuthorizingType, ?INBOUND_ACCOUNT_TYPES)
    of
        'true' ->
...
inbound_account_by_ip(Request, IP) ->
    ...
    lager:debug(
        "source IP ~s belongs to account ~s, allowing",
        [IP, AccountId]
    ),
    Routines = [
        fun(R) ->
...
determine_account_id_from_number(Request) ->
    ...
    case knm_number:lookup_account(Number) of
        ...
        {'error', _R} ->
            lager:debug(
                "unable to determine account id for ~s: ~p",
                [Number, _R]
            ),
            'ok'
    end.

-spec send_disabled_deny(j5_request:request(), kapps_util:not_enabled_error(), kz_term:ne_binary()) ->
    'ok'.
send_disabled_deny(Request, 'account_disabled', AccountId) ->
...
maybe_local_resource(Props, Request) ->
    case knm_number_options:is_local_number(Props) of
        ...
        'false' -> maybe_account_limited(Request)
    end.
...
allow_local_resource(Request) ->
    Number = j5_request:number(Request),
    lager:debug(
        "number ~s is a local number for account ~s, allowing",
...
maybe_account_limited(Request) ->
    ...
    case j5_request:is_authorized(R, Limits) of
        ...
        'false' ->
            lager:debug(
                "account ~s is not authorized to create this channel",
                [AccountId]
            ),
            send_response(R)
    end.
...
check_reseller_limits(Request, ResellerId) ->
    ...
    R = maybe_authorize(Request, Limits),
    (not j5_request:is_authorized(R, Limits)) andalso
        lager:debug(
            "reseller ~s is not authorized to create this channel",
            [ResellerId]
        ),
    send_response(R).
...
maybe_authorize(Request, Limits) ->
    case j5_limits:enabled(Limits) of
        ...
        'false' ->
            lager:debug(
                "limits are disabled for account ~s",
...
-define(AUTZH_TYPES_FOR_OUTBOUND, [
    <<"account">>,
    <<"user">>,
    <<"device">>,
    <<"mobile">>
...
-spec maybe_get_outbound_flags(kz_term:api_binary(), kz_term:api_binary(), kz_term:ne_binary()) ->
    kz_term:api_binary().
maybe_get_outbound_flags('undefined', _AuthId, _AccountDb) ->
    'undefined';
maybe_get_outbound_flags(_AuthType, 'undefined', _AccountDb) ->
    'undefined';
maybe_get_outbound_flags(AuthType, AuthId, AccountDb) ->
    case
        lists:member(AuthType, ?AUTZH_TYPES_FOR_OUTBOUND) andalso
...
-spec maybe_publish_authz_resp(j5_request:request(), kz_term:ne_binary(), kz_term:proplist()) ->
    'ok'.
maybe_publish_authz_resp(Request, ServerId, Resp) ->
    maybe_publish_authz_resp(
        Request, ServerId, Resp, j5_channels:is_destroyed(j5_request:call_id(Request))
    ).

-spec maybe_publish_authz_resp(
    j5_request:request(), kz_term:ne_binary(), kz_term:proplist(), boolean()
) -> 'ok'.
...
maybe_publish_authz_resp(Request, ServerId, Resp, 'false') ->
    ...
    case j5_request:is_authorized(Request) of
        'false' ->
            j5_util:send_system_alert(Request);
        'true' ->
