...
%% 15 minutes
-define(SYNC_PERIOD, 900000).
-define(CLEANUP_PERIOD, ?MILLISECONDS_IN_MINUTE).

-record(channel, {
    call_id :: kz_term:api_binary() | '$1' | '$2' | '_',
...
-define(RESPONDERS, [
    {{?MODULE, 'handle_authz_resp'}, [{<<"authz">>, <<"authz_resp">>}]},
    {{?MODULE, 'handle_rate_resp'}, [{<<"rate">>, <<"resp">>}]}
]).
...
-spec total_inbound_channels_per_did_rules(kz_term:ne_binary(), kz_term:ne_binary()) ->
    non_neg_integer().
...
-spec allotment_consumed(
    non_neg_integer(),
    non_neg_integer(),
    kz_term:ne_binary(),
    kz_term:ne_binary() | j5_limits:limits()
) -> non_neg_integer().
...
per_minute_cost(AccountId) ->
    ...
    lists:foldl(
        fun(Channel, Cost) -> call_cost(Channel) + Cost end,
        0,
...
real_per_minute_cost(AccountId) ->
    ...
    lists:foldl(
        fun(Channel, Cost) -> call_cost(Channel, 0) + Cost end,
        0,
        ets:select(?TAB, MatchSpec)
    ).
...
insert_authorized(JObj) ->
    ...
    case ets:lookup(?TAB, CallId) of
        ...
        [#channel{destroyed = 'true'}] ->
            lager:info("channel ~s already destroyed, not inserting authz", [CallId]);
...
handle_cast('flush_channels', State) ->
    ...
    {'noreply', State};
handle_cast(
    {'kz_nodes', {'expire', #kz_node{node = NodeName, kapps = Whapps}}},
    ...
    case props:get_value(<<"ecallmgr">>, Whapps) of
        'undefined' ->
            {'noreply', State};
        _WhappInfo ->
            lager:debug("ecallmgr node ~s is no longer reachable, synchronizing channels", [
                NodeName
            ]),
            self() ! {'synchronize_channels', SyncRef},
            {'noreply', State}
...
handle_info(?HOOK_EVT(_, <<"CHANNEL_ANSWER">>, JObj), State) ->
    ...
    case ets:update_element(?TAB, CallId, Props) of
        ...
        'false' ->
            lager:debug("missing channel ~s, failed to update with answered timestamp", [CallId])
    end,
...
terminate(_Reason, _State) ->
    ets:info(?TAB) =/= 'undefined' andalso
        ets:delete(?TAB),
...
ecallmgr_channel_ids([], ChannelIds) ->
    ChannelIds;
ecallmgr_channel_ids([JObj | JObjs], ChannelIds) ->
    UpdatedChannelIds = lists:foldl(
        fun(ChannelId, Ids) ->
...
delete_destroyed_channels() ->
    % anything prior to 3s ago
    ThenS = kz_time:now_s() - 3,

    Deleted = ets:select_delete(?TAB, [
        {#channel{destroyed = 'true', timestamp = '$2', _ = '_'}, [{'<', '$2', {'const', ThenS}}], [
            'true'
        ]}
    ]),
    maybe_log_deleted(Deleted, ThenS).

maybe_log_deleted(0, _ThenS) ->
    'ok';
maybe_log_deleted(Deleted, ThenS) ->
...
-spec sum_allotment_consumed(non_neg_integer(), non_neg_integer(), non_neg_integers()) ->
    non_neg_integer().
sum_allotment_consumed(CycleStart, Span, Matches) ->
    sum_allotment_consumed(CycleStart, Span, kz_time:now_s(), 0, Matches).

-spec sum_allotment_consumed(
    non_neg_integer(), non_neg_integer(), non_neg_integer(), non_neg_integer(), non_neg_integers()
) -> non_neg_integer().
sum_allotment_consumed(_, _, _, Seconds, []) ->
    Seconds;
