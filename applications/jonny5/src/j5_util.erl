...
send_system_alert(Request) ->
    ...
    kz_notify:detailed_alert(
        "blocked ~s to ~s / Account ~s / Reseller ~s",
        [
...
-spec add_limit_details(kz_term:api_ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
    kz_term:proplist().
add_limit_details('undefined', _, Props) ->
    Props;
add_limit_details(Account, Prefix, Props) ->
    ...
    [
        ...
        {<<Prefix/binary, "-Resource-Calls">>, kz_term:to_binary(resource(AccountId, Limits))},
        {
            ...
            kz_term:to_binary(inbound_trunks(AccountId, Limits))
        },
        {
            <<Prefix/binary, "-Outbound-Trunks">>,
            kz_term:to_binary(outbound_trunks(AccountId, Limits))
        },
