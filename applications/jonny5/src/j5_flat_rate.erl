...
-define(WHITELIST,
    kapps_config:get_ne_binary(?APP_NAME, <<"flat_rate_whitelist">>, ?DEFAULT_WHITELIST)
).
-define(BLACKLIST,
    kapps_config:get_ne_binary(?APP_NAME, <<"flat_rate_blacklist">>, ?DEFAULT_BLACKLIST)
).
...
-spec authorize(j5_request:request(), j5_limits:limits(), kz_services_standing:acceptable_return()) ->
    j5_request:request().
authorize(Request, Limits, {'false', _Reason}) ->
    lager:debug(
        "account ~s has a billing issue can not use flat rate trunks",
...
authorize(Request, Limits, {'true', _Reason}) ->
    case is_number_eligible_for_flat_rate(Request) of
        'true' ->
            lager:debug(
                "checking if account ~s has available flat rate trunks",
                [j5_limits:account_id(Limits)]
            ),
            maybe_consume_flat_rate(Request, Limits);
        'false' ->
            lager:debug(
                "number '~s' is not eligible for flat rate trunks",
...
check_standing(Limits) ->
    CacheAcceptable = kapps_config:get_is_true(
        ?APP_NAME, <<"service_standing_cache_acceptable">>, 'true'
    ),
    CacheExpiration = kapps_config:get_integer(
        ?APP_NAME, <<"service_standing_cache_expiration_s">>, ?DEFAULT_EXPIRATION
    ),
    Options = #{
...
is_number_eligible_for_flat_rate(Request) ->
    ...
    (kz_term:is_empty(TrunkWhitelist) orelse
        re:run(Number, TrunkWhitelist) =/= 'nomatch') andalso
        (kz_term:is_empty(TrunkBlacklist) orelse
            re:run(Number, TrunkBlacklist) =:= 'nomatch').
...
-spec maybe_consume_flat_rate_burst(non_neg_integer(), j5_request:request(), j5_limits:limits()) ->
    j5_request:request().
...
consume_limit(Limit, Used, Type) ->
    case Used - Limit of
        Remaining when Remaining > 0 ->
            lager:info(
                "all ~p ~s trunks consumed leaving ~p channels unaccounted for",
                [Limit, Type, Remaining]
            ),
            Remaining;
        _Else ->
            lager:debug(
                "account is consuming ~p/~p ~s trunks",
                [Used - 1, Limit, Type]
            ),
            0
    end.
