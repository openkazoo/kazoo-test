    ...
    case kz_term:is_empty(JObj) of
        ...
        'false' when CacheOrigins =/= [] ->
            Limits = create_limits(
                AccountId, AccountDb, kz_json:delete_key(<<"pvt_cache_origins">>, JObj)
            ),
...
inbound_trunks(#limits{inbound_trunks = -1}) ->
    -1;
inbound_trunks(#limits{
    bundled_inbound_trunks = BundledTrunks,
...
outbound_trunks(#limits{outbound_trunks = -1}) ->
    -1;
outbound_trunks(#limits{
    bundled_outbound_trunks = BundledTrunks,
...
twoway_trunks(#limits{twoway_trunks = -1}) ->
    -1;
twoway_trunks(#limits{
    bundled_twoway_trunks = BundledTrunks,
...
create_limits(AccountId, AccountDb, JObj) ->
    #limits{
        ...
        reserve_amount = kzd_limits:reserve_units(
            JObj, kz_currency:dollars_to_units(?DEFAULT_RATE)
        ),
