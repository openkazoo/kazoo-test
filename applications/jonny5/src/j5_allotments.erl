...
-spec maybe_consume_allotment(kz_term:api_object(), j5_request:request(), j5_limits:limits()) ->
    j5_request:request().
...
maybe_consume_allotment(Allotment, Request, Limits) ->
    ...
    case allotment_consumed_so_far(Allotment, Limits) of
        {'error', _R} when GroupConsumed > (Amount - Minimum) ->
            lager:debug(
                "account ~s has used all ~ws of their allotment",
                [AccountId, Amount]
            ),
            Request;
        {'error', _R} ->
        ...
        Consumed ->
            lager:debug(
                "account ~s has ~ws remaining of their allotment",
...
-spec maybe_group_consumed(
    kz_term:binaries(), kz_json:object(), j5_limits:limits(), non_neg_integer()
) -> non_neg_integer().
maybe_group_consumed([], _Allotment, _Limits, Acc) ->
    Acc;
...
-spec reconcile_allotment(
    non_neg_integer(), kz_json:object(), j5_request:request(), j5_limits:limits()
) ->
    'ok'.
reconcile_allotment(0, _, _, _) ->
    'ok';
reconcile_allotment(Seconds, Allotment, Request, Limits) ->
    ...
    lager:debug(
        "adding allotment debit ~s to ledger ~s for ~wsec",
        [Id, LedgerDb, Seconds]
    ),
    Props =
...
-spec find_allotment_by_classification(
    kz_term:ne_binary(), kz_term:ne_binary(), j5_limits:limits()
) -> kz_term:api_object().
find_allotment_by_classification(Direction, Classification, Limits) ->
...
-spec find_allotment_by_classification(kz_term:ne_binary(), j5_limits:limits()) ->
    kz_term:api_object().
find_allotment_by_classification(Classification, Limits) ->
    Allotments = j5_limits:allotments(Limits),
    lager:debug(
        "checking if account ~s has any allotments for ~s",
...
-spec allotment_consumed_so_far(
    non_neg_integer(), non_neg_integer(), kz_term:ne_binary(), j5_limits:limits(), 0..3
) ->
    integer()
    | {'error', any()}.
...
allotment_consumed_so_far(CycleStart, CycleEnd, Classification, Limits, Attempts) ->
    ...
    LedgerDb = kz_util:format_account_mod_id(AccountId),
    ViewOptions = [
        {'startkey', [Classification, CycleStart]},
    ...
    case kz_datamgr:get_results(LedgerDb, <<"allotments/consumed">>, ViewOptions) of
        ...
        {'error', _R} = Error ->
            lager:debug(
                "unable to get consumed quantity for ~s allotment from ~s: ~p",
...
-spec sum_allotment_consumed_so_far(kz_json:objects(), non_neg_integer(), non_neg_integer()) ->
    non_neg_integer().
sum_allotment_consumed_so_far([], _, Seconds) ->
    Seconds;
