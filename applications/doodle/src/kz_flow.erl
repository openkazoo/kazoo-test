...
contains_no_match(Doc) ->
    lists:any(
        fun
            (Number) when Number =:= ?NO_MATCH_FLOW ->
                'true';
            (_) ->
                'false'
        end,
        kzd_flows:numbers(Doc)
    ).
...
-spec do_lookup(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
    lookup_ret().
do_lookup(Number, AccountId, ViewName, LookupOptions) ->
    ...
    case kz_datamgr:get_results(Db, ViewName, Options) of
        {'error', _} = E ->
            E;
        {'ok', []} when
            ViewName =/= ?CF_LIST_BY_NUMBER andalso
                LookupCallflows =:= 'true'
        ->
            do_lookup(Number, AccountId, ?CF_LIST_BY_NUMBER, LookupOptions);
        {'ok', []} when
            Number =/= ?NO_MATCH_FLOW andalso
                ViewName =:= ?CF_LIST_BY_NUMBER andalso
                LookupCallflows =:= 'true'
        ->
...
-spec cache_flow_number(kz_term:ne_binary(), kz_term:ne_binary(), kzd_callflows:doc()) ->
    lookup_ret().
cache_flow_number(Number, AccountId, Flow) ->
    AccountDb = kz_util:format_account_db(AccountId),
    CacheOptions = [
        {'origin', [{'db', AccountDb, kz_doc:type(Flow)}]},
        {'expires', ?MILLISECONDS_IN_HOUR}
    ],
    kz_cache:store_local(
        ?CACHE_NAME, ?MSG_FLOW_CACHE_KEY(Number, AccountId), kz_doc:id(Flow), CacheOptions
    ),
...
maybe_use_nomatch(Number, AccountId) ->
    case lists:all(fun is_digit/1, kz_term:to_list(Number)) of
        'true' ->
            lookup(?NO_MATCH_FLOW, AccountId);
        'false' ->
...
test_flow_patterns([], _, {_, 'undefined'}) ->
    'no_match';
test_flow_patterns([], _, Result) ->
    Result;
