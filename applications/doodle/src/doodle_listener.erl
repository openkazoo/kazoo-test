...
custom_vars(#{authorizing_id := _} = Map) ->
    Map;
            ...
            case kz_datamgr:open_cache_doc(AccountDb, AuthorizingId) of
                {'ok', Doc} ->
                    Props = props:filter_undefined([
                        {?CV(<<"Authorizing-Type">>), kz_doc:type(Doc)},
                ...
                _Else ->
                    lager:warning("unexpected result reading doc ~s/~s => ~p", [
                        AuthorizingId, AccountId, _Else
                    ]),
                    Map
            end;
        _Else ->
...
lookup_mdn(#{authorizing_id := _AuthorizingId} = Map) ->
    Map;
lookup_mdn(#{phone_number := KNumber, request := JObj} = Map) ->
    ...
    case doodle_util:lookup_mdn(Number) of
        {'ok', Id, OwnerId} ->
            Props = props:filter_undefined([
                {?CV(<<"Authorizing-Type">>), <<"device">>},
                {?CV(<<"Authorizing-ID">>), Id},
                {?CV(<<"Owner-ID">>), OwnerId}
            ]),
            JObj1 = kz_json:delete_keys(
                [
                    ?CV(<<"Authorizing-Type">>),
                    ?CV(<<"Authorizing-ID">>)
                ],
...
create_im(#{request := SmsReq, account_id := AccountId} = Map) ->
    Funs = [
        {fun kapps_im:set_application_name/2, ?APP_NAME},
    ...
    Map#{
        fetch_id => kapps_im:message_id(IM),
...
route_message(#{im := Im} = Context, Flow, NoMatch) ->
    lager:info(
        "flow ~s in ~s satisfies request",
        [kz_doc:id(Flow), kapps_im:account_id(Im)]
    ),
    Updaters = [
        {fun kapps_im:kvs_store_proplist/2, [
