...
-export([
    maybe_ensure_cid_valid/5,
    maybe_restrict_call/2
]).

-include("ts.hrl").
%% for hostent record, used in find_ip/1
-include_lib("kernel/include/inet.hrl").

-define(VALIDATE_CALLER_ID,
    kapps_config:get_is_true(?CONFIG_CAT, <<"ensure_valid_caller_id">>, 'false')
).
-define(HONOR_DIVERSION,
    kapps_config:get_is_true(?CONFIG_CAT, <<"honor_diversions_by_cid_validation">>, 'false')
).
    ...
    case inet_parse:address(Domain) of
        ...
        _Huh ->
            ...
            case inet:gethostbyname(Domain, 'inet') of
                ...
                {'ok', #hostent{h_addr_list = [Addr | _Rest]}} -> inet_parse:ntoa(Addr)
            end
    end.

%% FilterOn: CallID | flat_rate | per_min
%% Remove active call entries based on what Filter criteria is passed in
-spec filter_active_calls(kz_term:ne_binary() | 'flat_rate' | 'per_min', active_calls()) ->
    active_calls().
...
filter_active_calls(CallID, ActiveCalls) ->
    lists:filter(
        fun
            ({CallID1, _}) when CallID =:= CallID1 -> 'false';
...
lookup_did(DID, AccountId) ->
    ...
    of
        ...
        {'error', 'not_found'} ->
            Options = [{'key', DID}],
            CacheProps = [
                {'origin', [{'db', knm_converters:to_db(DID), DID}, {'type', <<"number">>}]}
            ],
            case kz_datamgr:get_results(AccountDb, ?TS_VIEW_DIDLOOKUP, Options) of
                {'ok', []} ->
                    ...
                    {'error', 'no_did_found'};
                {'ok', [ViewJObj]} ->
                    lager:info("cache miss for ~s, found result with id ~s", [
                        DID, kz_doc:id(ViewJObj)
                    ]),
                ...
                {'ok', [ViewJObj | _Rest]} ->
                    ...
                    lager:info(
                        "cache miss for ~s, found multiple results, using first with id ~s", [
                            DID, kz_doc:id(ViewJObj)
                        ]
                    ),
                    ValueJObj = kz_json:get_value(<<"value">>, ViewJObj),
            ...
            {'ok',
                kz_json:from_list(
                    [
                        {<<"server">>, Server},
                        {<<"account">>,
                            merge_account_attributes(
                                AccountId,
                                kz_json:get_json_value(<<"account">>, JObj, kz_json:new())
                            )},
        ...
        {'error', 'not_found'} ->
            ...
            case kz_datamgr:get_results(AccountDb, <<"trunkstore/lookup_user_flags">>, Options) of
                ...
                {'ok', []} ->
                    ...
                    {'ok', kz_json:new()};
                {'ok', [User | _]} ->
                    lager:info("cache miss, found view result for ~s@~s with id ~s", [
                        Name, Realm, kz_doc:id(User)
                    ]),
                    ...
                    Restriction = kz_json:get_value(
                        <<"call_restriction">>, AccountJObj, kz_json:new()
                    ),
                    Props = [
...
-spec merge_account_attributes(kz_term:ne_binary() | kzd_accounts:doc(), kz_json:object()) ->
    kz_json:object().
...
invite_format(<<"e164_without_plus">>, To) ->
    case knm_converters:normalize(To) of
        <<$+, PluslessDID/binary>> ->
            lager:info(
                "while processing 'e164_without_plus' flag, DID ~s converted to E.164 with truncated '+': ~s",
                [To, PluslessDID]
            ),
            [
                {<<"Invite-Format">>, <<"e164">>},
...
caller_id(['undefined' | T]) ->
    caller_id(T);
caller_id([CID | T]) ->
    case {kz_json:get_value(<<"cid_name">>, CID), kz_json:get_value(<<"cid_number">>, CID)} of
...
-spec maybe_ensure_cid_valid(
    cid_type(), kz_term:api_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()
) ->
    kz_term:ne_binary().
        ...
        [Diversion | _] ->
            ...
            case knm_number:lookup_account(CallerIdNumber) of
                {'ok', AccountId, _} ->
                    CIDNum;
                _ ->
                    DefaultCID = kapps_config:get_ne_binary(
                        <<"trunkstore">>,
                        <<"default_caller_id_number">>,
                        kz_privacy:anonymous_caller_id_number(AccountId)
                    ),
                    lager:info(
                        "wrong diversions cid detected! Will use default trunkstore caller id: ~s",
                        [DefaultCID]
                    ),
                    DefaultCID
            end;
...
-spec validate_external_cid(kz_term:api_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
validate_external_cid(CIDNum, FromUser, AccountId) ->
...
validate_from_user(FromUser, AccountId) ->
    ...
    case knm_number:lookup_account(NormalizedFromUser) of
        {'ok', AccountId, _} ->
            lager:info("CID Number derived from CID Name, normalized and set to: ~s", [
                NormalizedFromUser
            ]),
            NormalizedFromUser;
        _NothingLeft ->
            ...
            lager:info(
                "no valid caller id identified! Will use default trunkstore caller id: ~s", [
                    DefaultCID
                ]
            ),
            DefaultCID
