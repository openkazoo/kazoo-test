...
handle_req(ApiJObj, _Options) ->
    ...
    lager:info("received request ~s asking if trunkstore can route this call", [
        kapi_route:fetch_id(ApiJObj)
    ]),
    CCVs = kz_json:get_value(<<"Custom-Channel-Vars">>, ApiJObj),
    ApplicationName = kz_json:get_ne_binary_value(<<"Application-Name">>, CCVs),
    case
        kz_json:get_first_defined([<<"Referred-By">>, <<"Redirected-By">>], CCVs) =/= 'undefined'
    of
        'true' when ApplicationName =:= ?APP_NAME -> handle_onnet_req(ApiJObj);
        'true' ->
            lager:info(
                "request is the result of a transfer by another application, ~s, ignoring",
                [ApplicationName]
            );
        'false' ->
            case
                kz_json:get_ne_binary_value(<<"Authorizing-Type">>, CCVs) =:= <<"sys_info">> andalso
                    kz_json:get_ne_binary_value(<<"Authorizing-ID">>, CCVs) =/= 'undefined'
            of
...
handle_offnet_req(ApiJObj) ->
    ...
    CallId = kz_json:get_ne_binary_value(<<"Call-ID">>, ApiJObj),
    lager:info("call with fetch-id ~s began from outside the network", [
        kapi_route:fetch_id(ApiJObj)
    ]),
