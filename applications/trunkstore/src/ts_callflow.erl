    ...
    case is_trunkstore_acct(RouteReqJObj, Type) of
        ...
        'true' ->
            AccountId = kz_json:get_value(
                [<<"Custom-Channel-Vars">>, <<"Account-ID">>], RouteReqJObj
            ),
            #ts_callflow_state{
                aleg_callid = CallID,
...
cleanup_amqp(#ts_callflow_state{
    amqp_worker = Worker,
    aleg_callid = CallId
}) ->
...
route_won(#ts_callflow_state{amqp_worker = Worker, kapps_call = Call} = State, RouteWin) ->
    ...
    {'won', State#ts_callflow_state{
        callctl_q = kapi_route:control_queue(RouteWin),
...
-spec process_event_for_bridge(state(), kz_json:object(), event_type()) ->
    'ignore' | {'hangup' | 'error' | 'bridged', state()}.
process_event_for_bridge(
    #ts_callflow_state{aleg_callid = ALeg} = State,
    JObj,
    {<<"resource">>, <<"offnet_resp">>, _}
) ->
    CallId = kz_json:get_value(<<"Call-ID">>, JObj, ALeg),
    case
        is_success(<<"Response-Message">>, JObj) orelse
    ...
    of
        ...
        'false' when CallId =:= ALeg ->
            Failure = kz_json:get_first_defined(
                [
                    <<"Error-Message">>,
                    <<"Response-Code">>
                ],
...
process_event_for_bridge(State, _JObj, {<<"call_event">>, <<"CHANNEL_DESTROY">>, _}) ->
    ...
    {'hangup', State};
process_event_for_bridge(
    State, JObj, {<<"call_event">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"bridge">>}
) ->
    case
        was_bridge_successful(JObj) orelse
            was_bridge_blocked(JObj)
    of
        'true' ->
...
process_event_for_bridge(
    _State, _JObj, {<<"call_event">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"answer">>}
) ->
...
was_bridge_successful(JObj) ->
    is_success(<<"Disposition">>, JObj) orelse
        is_success(
            [
...
send_hangup(#ts_callflow_state{callctl_q = <<>>}) ->
    'ok';
send_hangup(#ts_callflow_state{callctl_q = 'undefined'}) ->
    'ok';
send_hangup(
    #ts_callflow_state{
        callctl_q = CtlQ,
...
send_hangup(#ts_callflow_state{callctl_q = <<>>}, _) ->
    'ok';
send_hangup(#ts_callflow_state{callctl_q = 'undefined'}, _) ->
    'ok';
send_hangup(
    #ts_callflow_state{
        callctl_q = CtlQ,
...
-spec is_trunkstore_acct(kz_json:object(), kz_term:api_binary() | kz_term:api_binaries()) ->
    boolean().
...
-spec check_ccvs_for_type(kz_json:object(), kz_term:api_binary() | kz_term:api_binaries()) ->
    boolean().
check_ccvs_for_type(CCVs, [Type | Types]) ->
    check_ccvs_for_type(CCVs, Type) orelse
        check_ccvs_for_type(CCVs, Types);
...
check_ccvs_for_type(CCVs, <<"sys_info">> = Type) ->
    is_authorized(CCVs, Type) orelse
        has_trunkstore_id(CCVs) andalso
...
is_not_redirected(CCVs) ->
    'undefined' =/= kz_json:get_ne_binary_value([<<"Referred-By">>], CCVs) orelse
        'undefined' =/= kz_json:get_ne_binary_value([<<"Redirected-By">>], CCVs).
