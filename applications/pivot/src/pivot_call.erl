...
-record(state, {
    voice_uri :: kz_term:api_ne_binary(),
    ...
    %% pid of the processing of the response
    response_pid :: kz_term:api_pid(),
    response_event_handlers = [] :: kz_term:pids(),
    %% monitor ref for the pid
    response_ref :: kz_term:api_reference(),
    debug = 'false' :: boolean(),
...
init([Call, JObj]) ->
    ...
    {'ok',
        #state{
            ...
            request_timeout_ms = kz_json:get_integer_value(
                <<"Request-Timeout">>, JObj, ?DEFAULT_REQ_TIMEOUT_MS
            ),
...
handle_info(
    {'http', {ReqId, 'stream_end', FinalHeaders}},
    #state{
        request_id = ReqId,
    ...
    {'noreply',
        ...
        'hibernate'};
handle_info(
    {'http', {ReqId, {{_, StatusCode, _}, RespHeaders, RespBody}}},
    #state{
        request_id = ReqId,
        requester_queue = RequesterQ,
        call = Call,
...
send_req(Call, Uri, 'get', BaseParams, _ReqBodyFormat, TimeoutMs, Debug) ->
    ...
    send(
        UpdatedCall, uri(Uri, format_request(Params, <<"form">>)), 'get', [], [], TimeoutMs, Debug
    );
send_req(Call, Uri, 'post', BaseParams, ReqBodyFormat, TimeoutMs, Debug) ->
    ...
    send(
        UpdatedCall, Uri, 'post', Headers, format_request(Params, ReqBodyFormat), TimeoutMs, Debug
    ).
...
-spec handle_resp(kz_term:api_binary(), kapps_call:call(), kz_term:ne_binary(), binary(), pid()) ->
    'ok'.
...
process_resp(RequesterQ, Call, Hdrs, RespBody) when is_list(Hdrs) ->
    handle_resp(
        RequesterQ,
        Call,
        props:get_value(<<"content-type">>, Hdrs),
        RespBody,
    ...
    case kz_http_util:urlsplit(URI) of
        ...
        {Scheme, Host, Path, QS, Fragment} ->
            kz_http_util:urlunsplit(
                {Scheme, Host, Path, <<QS/binary, "&", SuppliedQS/binary>>, Fragment}
            )
    end.
...
-spec maybe_debug_req(kapps_call:call(), binary(), atom(), kz_term:proplist(), iolist(), boolean()) ->
    'ok'.
maybe_debug_req(_Call, _Uri, _Method, _ReqHdrs, _ReqBody, 'false') ->
    'ok';
...
-spec maybe_debug_resp(
    boolean(), kapps_call:call(), kz_term:ne_binary(), kz_term:proplist(), binary()
) -> 'ok'.
maybe_debug_resp('false', _Call, _StatusCode, _RespHeaders, _RespBody) ->
    'ok';
...
debug_doc(Call, DebugJObj, AccountModDb) ->
    WithCallJObj = kz_json:set_values(
        [
