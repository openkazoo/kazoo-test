    ...
    MatchSpec = [
        {
            ...
            [
                {'andalso', {'=:=', '$1', {'const', AccountId}},
                    {'orelse', {'=:=', '$2', {'const', HookEvent}},
                        {'=:=', '$2', {'const', <<"all">>}}}}
            ],
            ['$_']
        }
...
do_fire(
    ...
    lager:debug("sending hook ~s(~s) with interaction id ~s via 'get' (retries ~b): ~s", [
        _HookEvent, _HookId, EventId, Retries, URI
    ]),

    Url = kz_term:to_list(
        list_to_binary([
            kz_term:to_binary(URI),
...
-spec handle_resp(
    webhook(), kz_term:ne_binary(), kz_json:object(), kz_term:proplist(), kz_http:ret()
) -> 'ok'.
handle_resp(
    #webhook{
        hook_event = _HookEvent,
    ...
    lager:debug("sent hook call event ~s(~s) with interaction id ~s successfully", [
        _HookEvent, _HookId, _EventId
    ]),
...
handle_resp(
    ...
    lager:debug(
        "non-200 response code: ~p on account ~s for event ~s(~s) with interaction id ~s",
...
handle_resp(
    ...
    lager:debug("failed to fire hook event ~s(~s) interaction id: ~p error: ~p", [
...
retry_hook(
    ...
    lager:debug("retries exhausted for event ~s(~s) with interaction id ~s for uri (~s)", [
        _HookEvent, _HookId, _EventId, _URI
    ]);
...
successful_hook(_Hook, _Debug, _Resp, 'false') ->
    'ok';
...
hook_event_lowered(<<"channel_create">>) ->
    <<"CHANNEL_CREATE">>;
hook_event_lowered(<<"channel_answer">>) ->
    <<"CHANNEL_ANSWER">>;
hook_event_lowered(<<"channel_destroy">>) ->
    <<"CHANNEL_DESTROY">>;
hook_event_lowered(<<"channel_bridge">>) ->
    <<"CHANNEL_BRIDGE">>;
hook_event_lowered(<<"all">>) ->
    <<"all">>;
...
maybe_need_migrate(WebHooks) ->
    ...
    Fun = fun(Hook) ->
        lists:member(kzd_webhook:event(kz_json:get_value(<<"doc">>, Hook)), NeedMigrate)
    end,
...
migrate_load_hook(Srv, WebHooks) ->
    case migrate_hooks(update_hook_docs(WebHooks, #{update => [], extra => []})) of
        {'ok', Hooks} ->
            ...
            lager:debug("successfully migrated and loaded ~b webhooks into server ~p", [
                length(Hooks), Srv
            ]);
        {'error', _Reason} ->
...
update_hook_doc(WebHook, <<"callflow">>, #{update := Update} = Acc) ->
    Acc#{
        update => [
            set_as_notifications_webhook(
                WebHook, <<"callflow">>, kz_doc:id(WebHook), 'undefined', kz_doc:revision(WebHook)
            )
            | Update
        ]
    };
update_hook_doc(WebHook, <<"inbound_fax">>, #{update := Update, extra := Extra} = Acc) ->
    ...
    Acc#{
        update => [
            set_as_notifications_webhook(
                WebHook, <<"inbound_fax">>, kz_doc:id(WebHook), FaxErrorId, kz_doc:revision(WebHook)
            )
            | Update
        ],
        extra => [
            set_as_notifications_webhook(
                WebHook, <<"inbound_fax_error">>, FaxErrorId, kz_doc:id(WebHook), 'undefined'
            )
            | Extra
        ]
    };
    ...
    Acc#{
        update => [
            set_as_notifications_webhook(
                ...
                kz_doc:id(WebHook),
                FaxErrorId,
                kz_doc:revision(WebHook)
            )
            | Update
        ],
        extra => [
            set_as_notifications_webhook(
                WebHook, <<"outbound_fax_error">>, FaxErrorId, kz_doc:id(WebHook), 'undefined'
            )
            | Extra
        ]
    }.

-spec set_as_notifications_webhook(
    kz_json:object(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
migrate_hooks(#{update := Update, extra := Extra}) ->
    case kz_datamgr:save_docs(?KZ_WEBHOOKS_DB, Update) of
        {'ok', JObjs} ->
            ...
            lager:debug(
                "~b/~b callflow,inbound/outbound fax hooks migrated to notifications, migrating ~b inbound/outbound fax error hooks to notifications",
...
is_other_hook_saved(JObj, SavedIds) ->
    ...
    OtherId =/= 'undefined' andalso
        sets:is_element(OtherId, SavedIds) orelse
        OtherId =:= 'undefined'.
    ...
    lists:foldl(
        fun(JObj, Ids) ->
            case kz_json:get_value(<<"error">>, JObj) of
                'undefined' ->
                    sets:add_element(kz_doc:id(JObj), Ids);
                _ ->
...
load_hook(Srv, WebHook) ->
    try jobj_to_rec(WebHook) of
        ...
        'throw':{'bad_hook', HookEvent} ->
            lager:debug(
                "failed to load hook ~s.~s: bad_hook: ~s",
                [
...
account_expires_time(AccountId) ->
    Expiry = kapps_account_config:get_global(
        AccountId,
        ?APP_NAME,
        ?ATTEMPT_EXPIRY_KEY,
    ...
    try
        kz_term:to_integer(
            kapps_account_config:get_global(
                AccountId, ?APP_NAME, ?FAILURE_COUNT_KEY, ?DEFAULT_FAILURE_COUNT
            )
        )
    catch
        _:_ -> ?DEFAULT_FAILURE_COUNT
    end.
...
enable_descendant_hooks(Account) ->
    ...
    of
        ...
        {'ok', Descendants} ->
            maybe_enable_descendants_hooks([
                AccountId
...
-spec init_metadata(
    kz_term:ne_binary(), kz_json:object(), {'ok', kz_term:ne_binary()} | {'error', any()}
) -> 'ok'.
