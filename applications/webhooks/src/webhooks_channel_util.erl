...
-spec maybe_handle_channel_event(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    'ok'.
maybe_handle_channel_event(AccountId, HookEvent, JObj) ->
    ...
    case webhooks_util:find_webhooks(HookEvent, AccountId) of
        ...
        Hooks -> maybe_fire_event(AccountId, HookEvent, JObj, Hooks)
    end.

-spec maybe_fire_event(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), webhooks()) ->
    'ok'.
...
is_fireable_hook(_JObj, #webhook{include_loopback = 'true'}) ->
    'true';
is_fireable_hook(JObj, #webhook{
    include_loopback = 'false',
    id = _Id
}) ->
    case
        is_loopback_channel_name(kz_call_event:channel_name(JObj)) orelse
            kz_json:is_true(<<"Channel-Is-Loopback">>, JObj, 'false')
    of
        'false' ->
            'true';
        'true' ->
...
-spec base_hook_event(kz_json:object(), kz_term:api_binary(), kz_term:proplist()) ->
    kz_json:object().
...
non_reserved_ccvs(JObj) ->
    ...
    non_reserved_ccvs(
        CCVs, kapps_config:get_ne_binaries(<<"call_command">>, <<"reserved_ccv_keys">>)
    ).
...
non_reserved_ccvs(CCVs, Keys) -> kz_json:filter(fun({K, _}) -> not lists:member(K, Keys) end, CCVs).
