...
-define(RESPONDERS, [
    {{?MODULE, 'presence_probe'}, [{<<"presence">>, <<"probe">>}]},
    {{?MODULE, 'mwi_update'}, [{<<"presence">>, <<"mwi_unsolicited_update">>}]},
    {{?MODULE, 'register_overwrite'}, [{<<"presence">>, <<"register_overwrite">>}]},
    {{?MODULE, 'notify_api'}, [{<<"switch_event">>, <<"notify">>}]}
]).
...
presence_probe(JObj, _Props) ->
    ...
    State =
        case ecallmgr_registrar:get_registration(Realm, Username) of
            'undefined' -> <<"offline">>;
            _Else -> <<"online">>
        end,
...
notify_api(JObj, _Props) ->
    ...
    maybe_send_notify(
        kapi_switch:notify_username(JObj),
...
maybe_send_notify(Username, Realm, JObj) ->
    ...
    case ecallmgr_registrar:lookup_registration(Realm, Username) of
        ...
        {'ok', Registration} ->
            ...
            lager:info("sending NOTIFY on ~s for ~s@~s and contact ~s", [
                Node, Username, Realm, Contact
            ]),
            case ensure_contact_user(Contact, Username, Realm) of
                'undefined' ->
                    lager:error("invalid contact ~p: ~p", [Contact, Registration]);
                Valid ->
                    send_notify(Node, Username, Realm, JObj, Valid)
            end
    end.

-spec send_notify(
    atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()
) -> 'ok'.
send_notify(Node, Username, Realm, JObj, Contact) ->
    ...
    lager:info(
        "send NOTIFY with Event '~s' (has body? ~w) to '~s@~s' via ~s: ~p",
...
mwi_update(JObj, Props) ->
    ...
    case ecallmgr_registrar:lookup_registration(Realm, Username) of
        {'error', 'not_found'} ->
            lager:warning("failed to find registration for ~s@~s, dropping MWI update", [
                Username, Realm
            ]);
        {'ok', Registration} ->
...
-spec send_mwi_update(
    kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary(), atom(), kz_json:object()
) -> 'ok'.
send_mwi_update(JObj, Username, Realm, Node, Registration) ->
    ToURI = #uri{
        user = kz_json:get_value(<<"To-User">>, Registration, Username),
        domain = kz_json:get_value(<<"To-Host">>, Registration, Realm)
    },
    To = kzsip_uri:uri(ToURI),
    ToAccount = kzsip_uri:ruri(ToURI),
    From = kzsip_uri:uri(#uri{
        user = kz_json:get_value(<<"From-User">>, Registration, Username),
        domain = kz_json:get_value(<<"From-Host">>, Registration, Realm)
    }),
    NewMessages = kz_json:get_integer_value(<<"Messages-New">>, JObj, 0),
    Body = io_lib:format(?MWI_BODY, [
        case NewMessages of
            0 -> "no";
            _ -> "yes"
        end,
    ...
    RegistrationContact = kz_json:get_first_defined(
        [<<"Bridge-RURI">>, <<"Contact">>], Registration
    ),
...
register_overwrite(JObj, Props) ->
    ...
    lager:debug(
        "sent registration overwrite update of old '~s' new '~s' via '~s'",
        [
            PrevContact,
            NewContact,
            Node
...
-spec ensure_contact_user(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:api_binary().
