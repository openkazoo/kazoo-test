...
-define(INTERACCOUNT_DELAY_MS,
    kapps_config:get_integer(?APP_NAME, <<"balance_crawler_interaccount_delay_ms">>, 10)
).
-define(FETCH_TIMEOUT_MS,
    kapps_config:get_integer(?APP_NAME, <<"balance_crawler_fetch_timeout_ms">>, 10000)
).
...
disconnect_account(AccountId) ->
    case ecallmgr_fs_channels:per_minute_channels(AccountId) of
        ...
        Channels ->
            lager:debug("account ~p has ~p per-minute channels, disconnect them", [
                AccountId, length(Channels)
            ]),
