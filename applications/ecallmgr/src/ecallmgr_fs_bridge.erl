...
-define(BYPASS_MEDIA_AFTER_BRIDGE,
    kapps_config:get_boolean(?APP_NAME, <<"use_bypass_media_after_bridge">>, 'false')
).
-define(CHANNEL_ACTIONS_KEY, [<<"Custom-Channel-Vars">>, <<"Channel-Actions">>]).

%% Keys in the Endpoint JSON that should not be de-duplicated
-define(UNLIFTABLE_KEYS, [
    <<"Endpoint-Type">>,
    <<"Failover">>,
    <<"Forward-IP">>,
    <<"Invite-Format">>,
...
unbridge(UUID, JObj) ->
    case kapi_dialplan:unbridge_v(JObj) of
        'false' ->
            {'error', <<"unbridge failed to execute as API did not validate">>};
        'true' ->
            Leg =
...
-spec maybe_early_media(atom(), kz_term:ne_binary(), kz_json:object(), kz_json:objects()) ->
...
-spec maybe_early_media_separator(
    atom(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()
) -> ecallmgr_util:send_cmd_ret().
...
-spec handle_hold_media(
    kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()
) -> kz_term:proplist().
...
-spec maybe_handle_bypass_media(
    kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()
) -> kz_term:proplist().
...
-spec handle_bypass_media(
    kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()
) -> kz_term:proplist().
handle_bypass_media(DP, _Node, _UUID, #channel{profile = ChannelProfile}, JObj) ->
    BridgeProfile = kz_term:to_binary(
        kz_json:get_value(<<"SIP-Interface">>, JObj, ?DEFAULT_FS_PROFILE)
    ),
...
-spec maybe_bypass_endpoint_media(
    kz_json:objects(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()
) -> kz_term:proplist().
maybe_bypass_endpoint_media(Endpoints, BridgeProfile, ChannelProfile, DP) ->
    ShouldBypass = lists:all(
        fun(Endpoint) ->
            bypass_endpoint_media_enabled(
                Endpoint,
                BridgeProfile,
                ChannelProfile
            )
...
-spec bypass_endpoint_media_enabled(kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    boolean().
bypass_endpoint_media_enabled(Endpoint, BridgeProfile, ChannelProfile) ->
    ...
    kz_json:is_true(<<"Bypass-Media">>, Endpoint) andalso
        EndpointProfile =:= ChannelProfile.

-spec handle_ccvs(kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()) ->
    kz_term:proplist().
...
-spec handle_cavs(kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()) ->
    kz_term:proplist().
...
-spec handle_loopback_keys(kz_term:ne_binaries(), kz_json:object(), kz_term:proplist()) ->
    kz_term:proplist().
handle_loopback_keys([], _JObj, Acc) ->
    Acc;
...
-spec pre_exec(kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()) ->
    kz_term:proplist().
...
-spec create_command(kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()) ->
    kz_term:proplist().
create_command(DP, Node, UUID, #channel{profile = ChannelProfile}, JObj) ->
    ...
    {CommonProperties, UniqueEndpoints} = kz_json:lift_common_properties(
        Endpoints, ?UNLIFTABLE_KEYS
    ),

    lager:debug("lifting from leg to channel: ~s", [kz_json:encode(CommonProperties)]),
    UpdatedJObj = kz_json:set_value(
        <<"Endpoints">>, UniqueEndpoints, kz_json:merge(JObj, CommonProperties)
    ),

    LiftedCmd = list_to_binary([
        "bridge ",
...
-spec maybe_bypass_after_bridge(
    boolean(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:objects()
) -> kz_json:objects().
...
-spec maybe_remove_endpoint_bypass(kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_json:object().
maybe_remove_endpoint_bypass(Endpoint, BridgeProfile, ChannelProfile) ->
...
-spec build_channels_vars(atom(), kz_term:ne_binary(), kz_json:objects(), kz_json:object()) ->
    iolist().
build_channels_vars(Node, UUID, Endpoints, JObj) ->
    Routines = [
        fun maybe_force_fax/4,
...
-spec maybe_force_fax(atom(), kz_term:ne_binary(), kz_json:objects(), kz_json:object()) ->
    kz_term:proplist().
...
-spec post_exec(kz_term:proplist(), atom(), kz_term:ne_binary(), channel(), kz_json:object()) ->
    kz_term:proplist().
...
-spec add_endpoints_channel_actions(atom(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
...
-spec build_endpoint_actions(
    atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), ep_actions()
) ->
    ep_actions().
...
-spec build_endpoint_action(
    atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), fs_apps()
) ->
    fs_apps().
...
-spec build_endpoint_action_dp(kz_term:ne_binary(), fs_apps(), pos_integer(), ep_actions()) ->
    ep_actions().
...
-spec add_bridge_actions(atom(), kz_term:ne_binary(), kz_json:objects(), kz_json:object()) ->
    kz_term:proplist().
...
-spec build_bridge_action_dp(kz_term:ne_binary(), fs_apps(), pos_integer(), ep_actions()) ->
    ep_actions().
...
normalize_event_action_char(C) when is_integer(C), 16#C0 =< C, C =< 16#D6 -> C + 32;
% so we only loop once
normalize_event_action_char(C) when is_integer(C), 16#D8 =< C, C =< 16#DE -> C + 32;
