...
-spec exec_cmd(atom(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()) ->
    api_response().
...
exec_cmd(_Node, _ConferenceId, JObj, _DestId) ->
    lager:debug(
        "command ~s not meant for us (~s) but for ~s",
        [
...
get_conf_command(<<"recordstop">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:recordstop_v(JObj) of
        'false' ->
            {'error', <<"conference recordstop failed validation">>};
        'true' ->
            MediaName = ecallmgr_util:recording_filename(
                kz_json:get_binary_value(<<"Media-Name">>, JObj)
            ),
...
get_conf_command(<<"tones">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:tones_v(JObj) of
        'false' ->
            {'error', <<"conference tones failed to validate">>};
        'true' ->
            ...
            FSTones = [
                begin
                    ...
                    Freqs = string:join(
                        [kz_term:to_list(V) || V <- kz_json:get_value(<<"Frequencies">>, Tone)], ","
                    ),
...
get_conf_command(<<"play">>, _Focus, ConferenceId, JObj) ->
    case kapi_conference:play_v(JObj) of
        ...
        'true' ->
            ...
            Media = list_to_binary([
                "'",
                ecallmgr_util:media_path(kz_json:get_value(<<"Media-Name">>, JObj), UUID, JObj),
                "'"
            ]),
            Args =
...
get_conf_command(<<"play_macro">>, _Focus, _ConferenceId, JObj) ->
    ...
    Media = list_to_binary([
        "'file_string://", kz_binary:join(Paths, <<"!">>), "'", " ", Participant
    ]),
    {<<"play">>, Media};
...
get_conf_command(Say, _Focus, _ConferenceId, JObj) when
    Say =:= <<"say">>;
    Say =:= <<"tts">>
->
    case kapi_conference:say_v(JObj) of
        'false' ->
            {'error', <<"conference say failed to validate">>};
        'true' ->
...
get_conf_command(<<"mute_participant">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:mute_participant_v(JObj) of
        'false' ->
            {'error',
                <<"conference mute_participant failed to execute as JObj did not validate.">>};
        'true' ->
...
get_conf_command(<<"deaf_participant">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:deaf_participant_v(JObj) of
        'false' ->
            {'error',
                <<"conference deaf_participant failed to execute as JObj did not validate.">>};
        'true' ->
...
get_conf_command(<<"participant_energy">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:participant_energy_v(JObj) of
        'false' ->
            {'error',
                <<"conference participant_energy failed to execute as JObj did not validate.">>};
        'true' ->
            Args = list_to_binary([
                kz_json:get_binary_value(<<"Participant-ID">>, JObj),
                " ",
...
get_conf_command(<<"relate_participants">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:relate_participants_v(JObj) of
        'false' ->
            {'error',
                <<"conference relate_participants failed to execute as JObj did not validate.">>};
        'true' ->
            Args = list_to_binary([
                kz_json:get_binary_value(<<"Participant-ID">>, JObj),
                " ",
...
get_conf_command(<<"set">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:set_v(JObj) of
        ...
        'true' ->
            Args = list_to_binary([
                kz_json:get_binary_value(<<"Parameter">>, JObj),
                " ",
...
get_conf_command(<<"undeaf_participant">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:undeaf_participant_v(JObj) of
        'false' ->
            {'error',
                <<"conference undeaf_participant failed to execute as JObj did not validate.">>};
        'true' ->
...
get_conf_command(<<"participant_volume_in">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:participant_volume_in_v(JObj) of
        'false' ->
            {'error',
                <<"conference participant_volume_in failed to execute as JObj did not validate.">>};
        'true' ->
            Args = list_to_binary([
                kz_json:get_binary_value(<<"Participant-ID">>, JObj),
                " ",
...
get_conf_command(<<"participant_volume_out">>, _Focus, _ConferenceId, JObj) ->
    case kapi_conference:participant_volume_out_v(JObj) of
        'false' ->
            {'error',
                <<"conference participant_volume_out failed to execute as JObj did not validate.">>};
        'true' ->
            Args = list_to_binary([
                kz_json:get_binary_value(<<"Participant-ID">>, JObj),
                " ",
...
dial(Node, ConferenceId, JObj, [_ | _] = Endpoints) ->
    ...
    ConferenceName =
        case kz_json:get_ne_binary_value(<<"Profile-Name">>, JObj) of
            'undefined' -> list_to_binary([ConferenceId, "@", ConferenceId, "_", AccountId]);
            ProfileName -> list_to_binary([ConferenceId, "@", ProfileName])
        end,

    DialCmd = list_to_binary([
        ecallmgr_fs_xml:get_channel_vars(JObj),
