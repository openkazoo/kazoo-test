...
-define(RESPONDERS, [{{?MODULE, 'handle_search_req'}, [{<<"conference">>, <<"search_req">>}]}]).
-define(BINDINGS, [
    {'conference', [
        {'restrict_to', ['discovery']},
        'federate'
    ]}
]).
-define(QUEUE_NAME, <<>>).
...
participants(#conference{name = Name}) ->
    participants(Name);
participants(Name) ->
...
init([]) ->
    ...
    _ = ets:new(?PARTICIPANTS_TBL, [
        'set', 'protected', 'named_table', {'keypos', #participant.uuid}
    ]),
    {'ok', #state{}}.
    ...
    _ =
        case ets:lookup(?CONFERENCES_TBL, UUID) of
            [#conference{account_id = 'undefined'}] ->
                lager:info(
                    "failed to find account_id in conference ~s, adding from participant",
                    [UUID]
                ),
                Conference = conference_from_props(Props, Node),
...
handle_cast({'sync_node', Node}, State) ->
    ...
    _ = [
     ...
     || [UUID] <- ets:match(?CONFERENCES_TBL, #conference{
            node = Node,
            uuid = '$1',
            _ = '_'
        })
...
find_account_id_from_call(CallId) ->
    case ecallmgr_fs_channel:fetch(CallId, 'record') of
        {'ok', #channel{account_id = AccountId}} ->
            AccountId;
        {'error', 'not_found'} ->
...
participant_to_props(#participant{
    uuid = UUID,
...
conference_to_props(#conference{
    name = Name,
    uuid = UUID,
    node = Node,
...
xml_to_conference(
    ...
    xml_attrs_to_conference(Attrs, #conference{
        node = Node,
        switch_hostname = Hostname,
        switch_url = ecallmgr_fs_nodes:sip_url(Node),
        switch_external_ip = ecallmgr_fs_nodes:sip_external_ip(Node)
...
-spec xml_attrs_to_conference(kz_types:xml_attribs(), conference()) ->
    conference().
xml_attrs_to_conference([], Conference) ->
    Conference;
xml_attrs_to_conference(
    [
...
xml_members_to_participants(
    ...
    #conference{name = Name, uuid = UUID, node = Node}
) ->
    [
        xml_member_to_participant(Xml, #participant{
            node = Node,
...
xml_member_to_participant([], Participant) ->
    Participant;
...
get_conference_dictionary([], Dictionary) ->
    Dictionary;
get_conference_dictionary(
    [
        #conference{uuid = UUID} = Conference
        | Conferences
    ],
    Dictionary
) ->
...
get_participant_dictionary([], Dictionary) ->
    Dictionary;
get_participant_dictionary(
    [
        #participant{uuid = UUID} = Participant
        | Participants
    ],
    Dictionary
) ->
...
print_summary(Match) ->
    ...
    io:format(
        "| Name                             | Node                                               | Participants | Uptime(sec) | Account-ID                       |Local|~n"
    ),
    io:format(
        "+==================================+====================================================+==============+=============+==================================+=====+~n"
    ),
...
print_details(
    {[#conference{name = Name} = Conference], Continuation},
    Count
) ->
    io:format("~n"),
    _ = [
