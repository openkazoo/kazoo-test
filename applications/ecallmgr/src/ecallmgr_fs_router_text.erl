...
handle_info({'route', Section, EventName, SubClass, Context, FSId, 'undefined', FSData}, State) ->
    ...
    handle_info(
        {'route', Section, EventName, SubClass, Context, FSId, MsgId, [
            {<<"Unique-ID">>, MsgId} | FSData
        ]},
        State
    );
handle_info(
    {'route', Section, <<"REQUEST_PARAMS">>, _SubClass, _Context, FSId, MsgId, FSData},
    #state{node = Node} = State
) ->
    _ = kz_util:spawn(fun process_route_req/5, [Section, Node, FSId, MsgId, FSData]),
    {'noreply', State, 'hibernate'};
handle_info(
    {'route', Section, <<"MESSAGE">>, _SubClass, _Context, FSId, MsgId, FSData},
    #state{node = Node} = State
) ->
    _ = kz_util:spawn(fun process_route_req/5, [Section, Node, FSId, MsgId, FSData]),
    {'noreply', State, 'hibernate'};
handle_info(
    {'route', Section, <<"CUSTOM">>, <<"KZ::", _/binary>>, _Context, FSId, MsgId, FSData},
    #state{node = Node} = State
) ->
    _ = kz_util:spawn(fun process_route_req/5, [Section, Node, FSId, MsgId, FSData]),
    {'noreply', State, 'hibernate'};
...
init_message_props(Props) ->
    Routines = [
        fun add_message_missing_props/1,
...
-spec process_route_req(
    atom(), atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()
) -> 'ok'.
...
-spec start_message_handling(atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    'ok'.
