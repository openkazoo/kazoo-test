...
-define(RESPONDERS, [{{?MODULE, 'handle_dial_req'}, [{<<"conference">>, <<"command">>}]}]).
-define(BINDINGS, [
    {'conference', [
        {'restrict_to', [{'command', kz_config:zone('binary')}]},
        'federate'
    ]}
]).
...
maybe_exec_dial(ConferenceNode, ConferenceId, JObj) ->
    {Loopbacks, Endpoints} = lists:splitwith(
        fun is_loopback/1, kz_json:get_list_value(<<"Endpoints">>, JObj, [])
    ),
...
-spec maybe_exec_dial(
    atom(), kz_term:ne_binary(), kapi_conference:doc(), kz_json:objects(), kz_json:objects()
) -> 'ok'.
maybe_exec_dial(ConferenceNode, ConferenceId, JObj, Endpoints, Loopbacks) ->
    lager:info("conference ~s is running on ~s, dialing out", [ConferenceId, ConferenceNode]),
    _ =
        (catch gproc:reg(
            {'p', 'l', ?FS_EVENT_REG_MSG(ConferenceNode, <<"conference::maintenance">>)}
        )),
...
update_endpoint(Endpoint, EndpointCallId) ->
    Updates = [
        {fun kz_json:insert_value/3, <<"Outbound-Call-ID">>, EndpointCallId},
...
exec_endpoint(Endpoint, {ConferenceNode, ConferenceId, JObj, Resps}) ->
    ...
    EndpointId = kz_json:get_first_defined(
        [
            <<"Route">>,
            <<"Endpoint-ID">>,
    ...
    try
        ecallmgr_conference_command:dial(
            ConferenceNode,
            ConferenceId,
            JObj,
            update_endpoint(Endpoint, EndpointCallId)
    ...
    of
        {'ok', _Resp} ->
            ...
            {ConferenceNode, ConferenceId, JObj, [
                {EndpointCallId, success_resp(EndpointId)} | Resps
            ]};
        _E ->
            lager:info("failed to exec: ~p", [_E]),
            {ConferenceNode, ConferenceId, JObj, [
                {EndpointCallId, error_resp(EndpointId, <<"unknown failure">>)} | Resps
            ]}
    catch
        'throw':{'msg', E} ->
        ...
        'throw':Msg when is_binary(Msg) ->
            ...
            {ConferenceNode, ConferenceId, JObj, [
                {'undefined', error_resp(EndpointId, Msg)} | Resps
            ]}
    end.
...
exec_loopback(Loopback, {ConferenceNode, ConferenceId, JObj, Resps}) ->
    ...
    Endpoint = kz_json:set_values(
        [
            {<<"Outbound-Call-ID">>, LoopbackId},
    ...
    exec_endpoint(
        Endpoint,
        {ConferenceNode, ConferenceId, kz_json:delete_key(<<"Outbound-Call-ID">>, JObj), Resps}
    ).
...
handle_responses(ConferenceNode, JObj, Responses) ->
    BaseResponses = [
        kz_json:from_list(handle_response(ConferenceNode, JObj, Response))
     || Response <- Responses
    ],
...
-record(outbound_dial, {
    loopback_a :: kz_term:ne_binary(),
...
handle_call_startup(ConferenceNode, JObj, LoopbackCallId, Resp) ->
    case
        wait_for_bowout(
            #outbound_dial{loopback_a = LoopbackCallId},
    ...
    of
        ...
        {'error', 'timeout'} ->
            props:insert_value(
                <<"Message">>, <<"dialing timed out before a call could be established">>, Resp
            );
        {'error', HangupCause, E} ->
            props:set_values(
                [
...
handle_call_event(
    ...
    case {kzd_freeswitch:call_id(Props), kzd_freeswitch:event_name(Props)} of
...
handle_create(
    #outbound_dial{
        loopback_a = <<?LB_ALEG_PREFIX, _/binary>>,
        channel_props = ChannelProps
    } = OutboundDial,
    Timeout,
    Start,
    ...
    case {kzd_freeswitch:other_leg_call_id(Props), kzd_freeswitch:loopback_other_leg(Props)} of
        {'undefined', 'undefined'} ->
            wait_for_bowout(
                OutboundDial#outbound_dial{
...
register_for_events(ConferenceNode, EndpointCallId) ->
    _ = [
        (catch gproc:reg({'p', 'l', ?FS_CALL_EVENT_REG_MSG(ConferenceNode, EndpointCallId)})),
...
-spec start_call_handlers(atom(), kapi_conference:doc(), outbound_dial()) ->
...
start_call_handlers(Node, JObj, #outbound_dial{loopback_b = LoopbackB, b_leg = CallId}) ->
    ...
    {'ok', CtlPid} = ecallmgr_call_sup:start_control_process(
        Node, CallId, FetchId, 'undefined', CCVs
    ),
...
-spec add_participant(
    kapi_conference:doc(), kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:proplist()
) -> 'ok'.
...
add_participant(JObj, CallId, ControlQueue, ChannelProps) ->
    ReqJObj = kz_json:set_values(
        [
            {<<"Conference-ID">>, kz_json:get_ne_binary_value(<<"Conference-ID">>, JObj)},
            {<<"Call-ID">>, CallId},
            ...
            | kz_api:default_headers(
                <<"conference">>, <<"add_participant">>, ?APP_NAME, ?APP_VERSION
            )
        ],
...
maybe_start_conference(JObj, ConferenceId) ->
    ...
    case
        find_media_server(
            kz_json:get_ne_binary_value(<<"Target-Call-ID">>, JObj), kz_api:node(JObj)
        )
    of
        'undefined' ->
            lager:info("no node found for the dial command, ignoring");
        MediaServer ->
...
-spec find_media_server_from_statuses(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:objects()) ->
    atom().
...
find_media_server_from_statuses(TargetCallId, IssuerNode, [Status | Statuses]) ->
    case kz_json:get_ne_binary_value([<<"Channels">>, TargetCallId, <<"Media-Node">>], Status) of
        ...
        MediaServer ->
            ...
            case
                lists:filter(
                    fun(MS) -> kz_term:to_binary(MS) =:= MediaServer end,
                    ecallmgr_fs_nodes:connected()
                )
            of
                [] ->
                    lager:info(
                        "media server ~s is not managed by us, not starting conference",
                        [MediaServer]
                    ),
                    'undefined';
                [MS] ->
...
-spec query_cluster_for_call(kz_term:ne_binary()) ->
    {'ok', kz_json:objects()}
    | {'error', any()}.
