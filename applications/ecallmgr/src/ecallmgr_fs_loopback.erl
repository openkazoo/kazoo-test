...
-define(IS_LOOPBACK(Props),
    props:get_value(<<"variable_loopback_leg">>, Props) =:= <<"B">> andalso
        props:get_value(<<"variable_", ?CHANNEL_VAR_PREFIX, ?LOOPBACK_FILTERED>>, Props) =:=
            'undefined' andalso
        props:get_value(<<?CHANNEL_VAR_PREFIX, ?LOOPBACK_FILTERED>>, Props) =:= 'undefined'
).
...
-spec filter(boolean(), atom(), kz_term:ne_binary(), kzd_freeswitch:data(), boolean()) ->
    kz_term:proplist().
filter('false', _Node, _UUID, Props, _) ->
    Props;
...
maybe_update_ccvs('false', _Fun, Filtered) ->
    Filtered;
...
filter_loopback(
    {<<"variable_", ?CHANNEL_VAR_PREFIX, ?CHANNEL_LOOPBACK_HEADER_PREFIX, K/binary>>, V}, Acc
) ->
    [{<<?CHANNEL_VAR_PREFIX, K/binary>>, V} | Acc];
filter_loopback(_, Acc) ->
    Acc.

-spec filter_props({kz_term:ne_binary(), any()}, {kz_term:proplist(), kz_term:proplist()}) ->
