...
export(Node, UUID, Props) ->
    ...
    _ = freeswitch:sendmsg(Node, UUID, [
...
%% @end
%%------------------------------------------------------------------------------
-spec bridge_export(atom(), kz_term:ne_binary(), kz_term:proplist()) ->
...
record_call(Node, UUID, Args) ->
    ...
    case freeswitch:api(Node, 'uuid_record', Args) of
        ...
        {'error', <<"-ERR ", E/binary>>} ->
            ...
            Evt = list_to_binary([
                ecallmgr_util:create_masquerade_event(<<"record_call">>, <<"RECORD_STOP">>),
