...
-spec search_for_route(
    atom(), atom(), kz_term:ne_binary(), kz_term:ne_binary(), kzd_freeswitch:data(), boolean()
) ->
    search_ret().
...
-spec spawn_authorize_call_fun(atom(), kz_term:ne_binary(), kzd_freeswitch:data()) ->
    kz_term:pid_ref().
...
wait_for_authz(Section, Node, FetchId, CallId, JObj, Props, {Pid, Ref}) ->
    ...
    receive
        {'authorize_reply', Ref, 'false'} ->
            reply_forbidden(Section, Node, FetchId);
        {'authorize_reply', Ref, 'true'} ->
            reply_affirmative(Section, Node, FetchId, CallId, JObj, Props);
...
reply_forbidden(Section, Node, FetchId) ->
    lager:info("received forbidden route response for ~s, sending 403 Incoming call barred", [
        FetchId
    ]),
    Props = [
        {<<"Method">>, <<"error">>},
    ...
    case
        freeswitch:fetch_reply(
            Node, FetchId, Section, iolist_to_binary(XML), 3 * ?MILLISECONDS_IN_SECOND
        )
    of
        'ok' ->
            lager:info("node ~s accepted ~s route response for request ~s", [Node, Section, FetchId]);
        {'error', Reason} ->
            lager:debug("node ~s rejected our ~s route unauthz: ~p", [Node, Section, Reason])
    end.

-spec reply_affirmative(
    atom(), atom(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), kz_term:proplist()
) -> search_ret().
...
route_resp_xml(Fun, Section, JObj, Props) when
    is_function(Fun, 3)
->
...
-spec route_req(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist(), atom()) ->
    kz_term:proplist().
...
route_req_ccvs(FetchId, Props) ->
    ...
    props:filter_undefined(
        [
            {
                <<?CALL_INTERACTION_ID>>,
                props:get_value(<<?CALL_INTERACTION_ID>>, CCVs, ?CALL_INTERACTION_DEFAULT)
            },
            {<<"Fetch-ID">>, FetchId},
