...
collect(ACLs, PidRefs, Timeout) ->
    ...
    receive
        ?ACL_RESULT(IP, ACL) ->
            lager:info("adding acl for '~s' to ~s", [
                IP, kz_json:get_value(<<"network-list-name">>, ACL)
            ]),
            collect(
                kz_json:set_value(IP, ACL, ACLs), PidRefs, kz_time:decr_timeout(Timeout, Start)
            );
...
maybe_resolve_cidr(CIDRS) when
    is_list(CIDRS)
->
    [maybe_resolve_cidr(CIDR) || CIDR <- CIDRS];
maybe_resolve_cidr(CIDR) when
    is_binary(CIDR)
->
...
sip_auth_ips(Collector) ->
    ...
    case kz_datamgr:get_results(?KZ_SIP_DB, <<"credentials/lookup_by_ip">>, ViewOptions) of
        ...
        {'ok', JObjs} ->
            ...
            PidRefs = [
                kz_util:spawn_monitor(fun resolve_hostname/4, [
                    Collector,
                    Host,
                    JObj,
...
wait_for_pid_refs(PidRefs, Timeout) ->
    ...
    receive
        {'DOWN', Ref, 'process', Pid, _Reason} ->
            case lists:keytake(Pid, 1, PidRefs) of
                'false' ->
                    wait_for_pid_refs(PidRefs, kz_time:decr_timeout(Timeout, Start));
                {'value', {Pid, Ref}, NewPidRefs} ->
...
resolve_hostname(Collector, ResolveMe, JObj, ACLBuilderFun) ->
    ...
    case
        (not kz_network_utils:is_ipv4(StrippedHost)) andalso
            kz_network_utils:resolve(StrippedHost)
    of
        'false' ->
...
local_resources(Collector) ->
    ...
    case
        kz_datamgr:get_results(?KZ_SIP_DB, <<"resources/listing_active_by_weight">>, ViewOptions)
    of
        {'error', _R} ->
...
offnet_resources(Collector) ->
    ViewOptions = ['include_docs'],
    case
        kz_datamgr:get_results(?KZ_OFFNET_DB, <<"resources/listing_active_by_weight">>, ViewOptions)
    of
        {'error', _R} ->
...
-spec add_trusted_objects(
    pid(), kz_term:api_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binaries()
) -> 'ok'.
add_trusted_objects(_Collector, _AccountId, _AuthorizingId, _AuthorizingType, []) ->
    'ok';
