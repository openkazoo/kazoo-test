    ...
    _ =
        case ecallmgr_fs_channel:fetch(UUID, 'record') of
            {'ok', #channel{
                ...
                lager:debug("sofia::intercepted: channel ~s Intercepted by ~s", [
                    UUID, InterceptedBy
                ]),
                Vars = [
...
-spec process_event(kz_term:ne_binary(), kz_term:api_binary(), kzd_freeswitch:data(), atom()) ->
    any().
...
process_event(?CHANNEL_MOVE_RELEASED_EVENT_BIN, _, Props, Node) ->
    ...
    gproc:send(
        {'p', 'l', ?CHANNEL_MOVE_REG(Node, UUID)},
...
process_event(<<"loopback::bowout">>, _UUID, Props, Node) ->
    ...
    lager:debug(
        "bowout detected on ~s, transferring to ~s",
...
-spec maybe_send_event(kz_term:ne_binary(), kz_term:api_binary(), kzd_freeswitch:data(), atom()) ->
    any().
maybe_send_event(<<"HEARTBEAT">>, _UUID, _Props, _Node) ->
    'ok';
            ...
            {BridgeID, <<"outbound">>, <<"inline">>, <<"intercept">>, 'undefined'} ->
                ...
                case ecallmgr_fs_channel:channel_data(Node, BLeg) of
                    {'ok', CData} ->
                        ...
                        gproc:send(
                            {'p', 'l', ?FS_EVENT_REG_MSG(Node, EventName)}, {'event', [BLeg | Data]}
                        ),
                        gproc:send(
                            {'p', 'l', ?FS_CALL_EVENT_REG_MSG(Node, BLeg)}, {'event', [BLeg | Data]}
                        );
                    _ ->
                        lager:debug("channel bridge intercept: failed to get channel data for ~s", [
                            BLeg
                        ])
                end;
            _Else ->
...
maybe_send_event(EventName, UUID, Props, Node) ->
    case kz_term:is_true(props:get_value(<<"variable_channel_is_moving">>, Props)) of
        'true' -> 'ok';
        'false' -> send_event(EventName, UUID, Props, Node)
    end.
...
-spec maybe_send_call_event(kz_term:api_binary(), kz_term:ne_binary(), kz_term:proplist(), atom()) ->
    any().
maybe_send_call_event('undefined', _, _, _) ->
    'ok';
maybe_send_call_event(CallId, EventName, Props, Node) ->
    gproc:send(
        {'p', 'l', ?FS_CALL_EVENT_MSG(Node, EventName, CallId)},
        {'event', EventName, [CallId | Props]}
    ),
    gproc:send({'p', 'l', ?FS_CALL_EVENT_REG_MSG(Node, CallId)}, {'event', [CallId | Props]}).

-spec maybe_start_event_listener(atom(), kz_term:ne_binary()) ->
    'ok' | kz_types:sup_startchild_ret().
maybe_start_event_listener(Node, UUID) ->
