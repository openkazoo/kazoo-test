...
%%------------------------------------------------------------------------------
-spec start_link(
    atom(), kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:api_ne_binary(), kz_json:object()
) ->
...
event_execute_complete('undefined', _CallId, _App) ->
    'ok';
...
handle_cast(
    {'event_execute_complete', CallId, AppName, JObj},
...
handle_info(
    {'usurp_control', CallId, FetchId, _JObj},
    #state{
        call_id = CallId,
        fetch_id = FetchId
    } = State
...
terminate(_Reason, #state{
    ...
    lager:debug("control queue was up for ~p microseconds", [
...
call_control_ready(#state{controller_q = 'undefined'}) ->
    'ok';
call_control_ready(State) ->
...
handle_loopback_destroyed(JObj, State) ->
    ...
    of
        ...
        {_Cause, _Bowout} ->
            lager:debug(
                "our loopback has ended with ~s(bowout ~s); treating as done",
                [_Cause, _Bowout]
            ),
...
do_handle_channel_destroyed(
    ...
    _ =
        case
            CurrentApp =:= 'undefined' orelse
                is_post_hangup_command(CurrentApp)
        of
            'true' ->
                'ok';
            'false' ->
    ...
    State#state{
        keep_alive_ref = get_keep_alive_ref(State#state{is_call_up = 'false'}),
        is_call_up = 'false',
        is_node_up = 'true'
    }.
...
force_queue_advance(
    ...
    lager:debug(
        "received control queue unconditional advance, skipping wait for command completion of '~s'",
        [CurrApp]
    ),
    case
        INU andalso
...
handle_execute_complete('undefined', _, State) ->
    State;
handle_execute_complete(<<"noop">>, JObj, #state{msg_id = CurrMsgId} = State) ->
    case kz_json:get_ne_binary_value(<<"Application-Response">>, JObj) of
        ...
        _NoopId ->
            lager:debug(
                "received noop execute complete with incorrect id ~s (expecting ~s)",
                [_NoopId, CurrMsgId]
            ),
            State
    end;
...
handle_playback_complete(AppName, JObj, #state{command_q = CmdQ} = State) ->
    ...
    case kz_json:get_ne_binary_value(<<"DTMF-Digit">>, JObj) of
        'undefined' ->
            handle_playback_looping(JObj, State);
        _DTMF ->
            GroupId = kz_json:get_ne_binary_value(<<"Group-ID">>, JObj),
            lager:debug(
                "DTMF ~s terminated playback, flushing all with group id ~s",
                [_DTMF, GroupId]
            ),
...
handle_playback_looping(JObj, #state{current_cmd = AppCmd} = State) ->
    case
        kz_json:is_true(<<"Endless-Playback">>, AppCmd, 'false') orelse
            kz_json:get_integer_value(<<"Loop-Count">>, AppCmd, 0)
    of
        'true' ->
...
handle_sofia_replaced(
    ...
    Commands = [
        kz_json:set_value(<<"Call-ID">>, ReplacedBy, JObj)
...
publish_leg_addition(Props) ->
    Event = ecallmgr_call_events:create_event(
        <<"LEG_CREATED">>,
        'undefined',
...
-spec maybe_add_cleg(kz_term:proplist(), kz_term:api_binary(), kz_term:api_binary(), state()) ->
    state().
...
add_cleg(_Props, _OtherLeg, 'undefined', State) ->
    State;
add_cleg(
    Props,
    OtherLeg,
...
-spec replace_call_id(
    kz_term:proplist(), kz_term:api_binary(), kz_term:ne_binary(), kz_term:proplist()
) -> kz_term:proplist().
replace_call_id([], _Call1, _Call2, Swap) ->
    Swap;
...
remove_leg(
    ...
    case lists:member(LegId, Legs) of
        ...
        'true' ->
            ...
            State#state{
                other_legs = lists:delete(LegId, Legs),
...
handle_dialplan(
    ...
    NewCmdQ =
        try
            insert_command(
                State, kz_term:to_atom(kz_json:get_value(<<"Insert-At">>, JObj, 'tail')), JObj
            )
        catch
            _T:_R ->
    ...
    case
        INU andalso
            (not queue:is_empty(NewCmdQ)) andalso
            CurrApp =:= 'undefined'
    of
        'true' ->
...
execute_queue_commands([Command | Commands], DefJObj, State) ->
    case
        kz_json:is_empty(Command) orelse
            'undefined' =:= kapi_dialplan:application_name(Command)
    of
        'true' ->
...
insert_command_into_queue(Q, Position, JObj) ->
    ...
    case kapi_dialplan:application_name(JObj) of
        %% list of commands that need to be added
        <<"queue">> ->
    ...
    fun(JObj, Q) ->
        ...
        case kapi_dialplan:application_name(JObj) of
            ...
            <<"noop">> = AppName ->
                ...
                lager:debug("inserting at the tail of the control queue call command ~s(~s)", [
                    AppName, MsgId
                ]),
                queue:in(JObj, Q);
            AppName ->
...
queue_insert_fun('head') ->
    fun(JObj, Q) ->
        ...
        case kapi_dialplan:application_name(JObj) of
            ...
            <<"noop">> = AppName ->
                MsgId = kz_api:msg_id(JObj),
                lager:debug("inserting at the head of the control queue call command ~s(~s)", [
                    AppName, MsgId
                ]),
                queue:in_r(JObj, Q);
            AppName ->
...
maybe_filter_queue('undefined', CommandQ) ->
    CommandQ;
maybe_filter_queue([], CommandQ) ->
    CommandQ;
...
maybe_filter_queue([AppJObj | T] = Apps, CommandQ) ->
    case queue:out(CommandQ) of
        ...
        {{'value', NextJObj}, CommandQ1} ->
            case
                (NextAppName = kapi_dialplan:application_name(NextJObj)) =:=
                    (AppName = kapi_dialplan:application_name(AppJObj)) orelse
            ...
            of
                ...
                'true' ->
                    lager:debug(
                        "app ~s matched next command ~s, checking fields",
                        [AppName, NextAppName]
                    ),
                    ...
                    of
                        ...
                        'true' ->
                            ...
                            % same app and all fields matched
                            maybe_filter_queue(Apps, CommandQ1)
                    end
            end
    end.
...
-spec which_call_leg(kz_term:ne_binary(), kz_term:ne_binaries(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
...
-spec maybe_send_error_resp(
    kz_json:object(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()
) -> 'ok'.
maybe_send_error_resp(_JObj, _CallId, _Cmd, <<"hangup">>) ->
    'ok';
...
send_error_resp(JObj, CallId, Cmd) ->
    Msg = <<"Could not execute dialplan action: ", (kapi_dialplan:application_name(Cmd))/binary>>,
    send_error_resp(JObj, CallId, Cmd, Msg).

-spec send_error_resp(kz_json:object(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()) ->
    'ok'.
send_error_resp(JObj, CallId, Cmd, Msg) ->
    case ecallmgr_fs_channel:fetch(CallId) of
...
error_ccvs(Channel) -> kz_json:from_list(ecallmgr_fs_channel:channel_ccvs(Channel)).
...
get_keep_alive_ref(#state{
    ...
    _ =
        case erlang:cancel_timer(TRef) of
            'false' ->
                'ok';
            %% flush the receive buffer of expiration messages
            _ ->
                receive
    ...
    of
        ...
        <<"CHANNEL_DESTROY">> ->
            case kz_json:is_true(<<"Channel-Moving">>, JObj) of
                ...
                'true' ->
                    lager:debug(
                        "channel destroy while moving to other node, deferring to new controller"
                    ),
                    {'stop', 'normal', State}
            end;
        <<"sofia::transferee">> ->
...
-spec handle_other_event_info(kz_term:api_binary(), kzd_freeswitch:data(), state()) ->
    {'noreply', state()}.
handle_other_event_info(CallId, Props, State) ->
    case
        props:get_first_defined(
