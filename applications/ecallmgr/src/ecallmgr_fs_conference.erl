...
init([Node, Options]) ->
    ...
    Events = kapps_config:get_ne_binaries(
        ?APP_NAME, <<"publish_conference_event">>, kapi_conference:events()
    ),
    {'ok', #state{
        node = Node,
        options = Options,
...
handle_cast('bind_to_events', #state{node = Node} = State) ->
    case
        gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"conference::maintenance">>)}) =:= 'true' andalso
...
-spec handle_conference_event(
    atom(), kz_term:ne_binaries(), kzd_freeswitch:data(), kz_term:proplist()
) -> 'ok'.
...
update_participant(Props) ->
    ...
    Update = [
        {#participant.conference_channel_vars, ConferenceVars},
...
maybe_publish_event(<<"conference-destroy">>, _, _, _) ->
    'ok';
...
publish_event(Action, Props, Node) ->
    ...
    case ecallmgr_fs_conferences:conference(UUID) of
        {'ok', #conference{} = Conference} ->
            publish_event(Action, Conference, Props, Node);
        {'error', 'not_found'} ->
            lager:debug("not publishing conference event ~s for nonexistent ~s ", [Action, UUID])
    end.
...
publish_event(Action, #conference{handling_locally = IsLocal} = Conference, Props, _Node) ->
    ...
    of
        'true' ->
            publish_event(conference_event(Action, Conference, Props));
        'false' ->
