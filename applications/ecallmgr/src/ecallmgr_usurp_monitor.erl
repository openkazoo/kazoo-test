...
init([]) ->
    ...
    {'ok', #{
        calls => ets:new('calls', ['set', {'keypos', #cache.key}]),
...
-spec handle_usurp(
    usurp_type(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), ets:tid()
) -> 'ok'.
handle_usurp(Type, CallId, RefId, JObj, Calls) ->
    _ = [
        Pid ! {Type, CallId, RefId, JObj}
     || #cache{pid = Pid} <- ets:lookup(Calls, {Type, CallId})
    ],
    'ok'.
...
handle_register(
    #cache{key = {Type, CallId}, ref_id = RefId, pid = Pid} = Cache,
    #{calls := Calls, pids := Pids} = State
) ->
