...
-define(RESPONDERS, [
    {{?MODULE, 'handle_reg_query'}, [{<<"directory">>, <<"reg_query">>}]},
    {{?MODULE, 'handle_reg_success'}, [{<<"directory">>, <<"reg_success">>}]},
    {{?MODULE, 'handle_reg_flush'}, [{<<"directory">>, <<"reg_flush">>}]}
]).
-define(BINDINGS, [
    {'registration', [
        {'restrict_to', [
...
handle_fs_reg(Node, Props) ->
    ...
    lager:debug(
        "sending successful registration for ~s@~s",
...
-spec lookup_contact(kz_term:ne_binary(), kz_term:ne_binary()) ->
    ...
    | {'error', 'not_found'}.
lookup_contact(<<>>, _Username) ->
    {'error', 'not_found'};
lookup_contact(_Realm, <<>>) ->
    {'error', 'not_found'};
        ...
        'false' ->
            case get_registration(Realm, Username) of
                #registration{original_contact = Contact} ->
                    lager:info(
                        "found user ~s@~s original contact ~s",
                        [Username, Realm, Contact]
                    ),
                    {'ok', Contact};
                'undefined' ->
...
details(Username, Realm) ->
    ...
    MatchSpec =
        [{#registration{id = '$1', _ = '_'}, [{'=:=', '$1', {const, Id}}], ['$_']}],
    print_details(ets:select(?MODULE, MatchSpec, 1)).
...
insert_registration(#registration{expires = 0} = Registration) ->
    lager:info(
        "deleting registration ~s@~s with contact ~s",
        [
...
insert_registration(#registration{initial = 'true'} = Registration) ->
    ...
    lager:info(
        "inserted registration ~s@~s with contact ~s",
        [
            Registration#registration.username,
            Registration#registration.realm,
            Registration#registration.contact
...
insert_registration(#registration{} = Registration) ->
    gen_server:cast(?SERVER, {'insert_registration', Registration}),
    lager:debug(
        "updated registration ~s@~s with contact ~s",
        [
            Registration#registration.username,
            Registration#registration.realm,
            Registration#registration.contact
...
find_newest_fetched_registration(Username, Realm, JObjs) ->
    ...
    case lists:sort(fun sort_fetched_registrations/2, Registrations) of
        ...
        _Else ->
            lager:info(
                "registration query for user ~s@~s returned an empty result",
                [Username, Realm]
            ),
            {'error', 'not_found'}
    end.
...
insert_fetched_registration(JObj) ->
    ...
    Expires =
        kz_json:get_integer_value(<<"Expires">>, JObj, ?EXPIRES_MISSING_VALUE) -
...
fetch_contact(Username, Realm) ->
    case fetch_registration(Username, Realm) of
        {'ok', JObj} ->
            ...
            lager:info(
                "found user ~s@~s contact ~s via fetch",
                [Username, Realm, Contact]
            ),
...
fetch_original_contact(Username, Realm) ->
    case fetch_registration(Username, Realm) of
        {'ok', JObj} ->
            ...
            lager:info(
                "found user ~s@~s original contact ~s via query",
                [Username, Realm, Contact]
            ),
            {'ok', Contact};
        {'error', _R} = Error ->
...
maybe_resp_to_query(JObj, RegistrarAge) ->
    case
        kz_json:get_value(<<"Node">>, JObj) =:=
            kz_term:to_binary(node()) andalso
            kz_json:get_value(<<"App-Name">>, JObj) =:=
                ?APP_NAME
    of
        'false' ->
...
build_query_spec(JObj, CountOnly) ->
    ...
    [{SelectFormat, [QueryFormat], [ResultFormat]}].
        ...
        'undefined' ->
            {
                ...
                {'orelse', {'=:=', '$1', {'const', Realm}}, {'=:=', '$2', {'const', Realm}}}
            };
        Username ->
            Id = registration_id(Username, Realm),
            {#registration{id = '$1', _ = '_'}, {'=:=', '$1', {'const', Id}}}
    end.
    ...
    case SelectFun(?MODULE, MatchSpec) of
        ...
        [_ | _] = Registrations ->
            Resp = [
                ...
                {<<"Registrar-Age">>, RegistrarAge},
                {<<"Fields">>, [
                    filter(Fields, kz_json:from_list(to_props(Registration)))
...
-spec registration_id(kz_term:ne_binary(), kz_term:ne_binary()) ->
...
create_registration(JObj) ->
    ...
    augment_registration(
        Reg#registration{
            ...
            previous_contact = kz_json:get_value(
                <<"Previous-Contact">>, JObj, Reg#registration.previous_contact
            ),
            last_registration = kz_json:get_integer_value(
                <<"Last-Registration">>, JObj, Reg#registration.last_registration
            ),
            initial_registration = kz_json:get_integer_value(
                <<"Initial-Registration">>, JObj, Reg#registration.initial_registration
            ),
            network_port = kz_json:get_value(
                <<"Network-Port">>, JObj, Reg#registration.network_port
            ),
...
augment_registration(Reg, JObj) ->
    ...
    Reg#registration{
        ...
        account_realm = kz_json:find(
            <<"Account-Realm">>,
            [JObj, CCVs],
            Reg#registration.account_realm
        ),
        account_name = kz_json:find(
            <<"Account-Name">>,
            [JObj, CCVs],
            Reg#registration.account_name
        ),
        owner_id = kz_json:find(
            <<"Owner-ID">>,
            [JObj, CCVs],
            Reg#registration.owner_id
        ),
        presence_id = kz_json:find(
            <<"Presence-ID">>,
            [JObj, CCVs],
            Reg#registration.presence_id
        ),
        authorizing_id = kz_json:find(
            <<"Authorizing-ID">>,
            [JObj, CCVs],
            Reg#registration.authorizing_id
        ),
        authorizing_type = kz_json:find(
            <<"Authorizing-Type">>,
            [JObj, CCVs],
...
fix_contact(Contact) -> binary:replace(Contact, [<<"<">>, <<">">>], <<>>, ['global']).

-spec bridge_uri(kz_term:api_binary(), kz_term:api_binary(), binary(), binary()) ->
    kz_term:api_binary().
bridge_uri(_Contact, 'undefined', _, _) ->
    'undefined';
bridge_uri('undefined', _Proxy, _, _) ->
    'undefined';
...
existing_or_new_registration(Username, Realm) ->
    case ets:lookup(?MODULE, registration_id(Username, Realm)) of
        [#registration{contact = Contact} = Reg] ->
            Reg#registration{
                last_registration = kz_time:now_s(),
...
maybe_query_authn(
    ...
    case
        kz_term:is_empty(AccountId) orelse
            kz_term:is_empty(AuthorizingId)
    of
...
maybe_send_deregister_notice(#registration{
    username = Username,
    realm = Realm,
...
maybe_registration_notify(#registration{register_overwrite_notify = 'false'} = Reg) ->
    Reg;
maybe_registration_notify(
    #registration{
...
registration_notify(#registration{
...
oldest_registrar() ->
    kz_nodes:whapp_zone_count(?APP_NAME) =:= 1 orelse
        kz_nodes:whapp_oldest_node(?APP_NAME, 'true') =:= node().
...
get_fs_contact(Props) ->
    ...
    % only one @ allowed
    [User, AfterAt] = binary:split(Contact, <<"@">>),
...
print_summary(Match) ->
    ...
    io:format(
        "| User                                          | Contact                | Path                   | Call-ID                          |  Exp |~n"
    ),
    io:format(
        "+===============================================+========================+========================+==================================+======+~n"
    ),
...
print_details({[#registration{} = Reg], Continuation}, Count) ->
    io:format("~n"),
    _ = [
        print_property(K, V, Reg)
     || {K, V} <- to_props(Reg)
    ],
    print_details(ets:select(Continuation), Count + 1).

print_property(<<"Expires">> = Key, Value, #registration{
    expires = Expires,
    last_registration = LastRegistration
}) ->
    Remaining = (LastRegistration + Expires) - kz_time:now_s(),
...
-spec proxy_path(
    kz_term:api_binary(), kz_term:api_binary(), kz_term:api_integer(), kz_term:api_binary()
) -> binary().
proxy_path(Proxy, IP, Port, 'undefined') ->
    proxy_path(Proxy, IP, Port, <<"udp">>);
proxy_path('undefined', 'undefined', 'undefined', _) ->
    <<>>;
proxy_path('undefined', 'undefined', Port, Proto) ->
    proxy_path('undefined', <<>>, Port, Proto);
proxy_path('undefined', IP, 'undefined', Proto) ->
    <<Proto/binary, ":", IP/binary>>;
proxy_path('undefined', IP, Port, Proto) ->
    <<Proto/binary, ":", IP/binary, ":", (kz_term:to_binary(Port))/binary>>;
