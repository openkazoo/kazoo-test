...
node(UUID) ->
    MatchSpec = [
        {#channel{uuid = '$1', node = '$2', _ = '_'}, [{'=:=', '$1', {'const', UUID}}], ['$2']}
    ],
...
former_node(UUID) ->
    MatchSpec = [
        {#channel{uuid = '$1', former_node = '$2', _ = '_'}, [{'=:=', '$1', {'const', UUID}}], [
            '$2'
        ]}
    ],
    case ets:select(?CHANNELS_TBL, MatchSpec) of
...
is_bridged(UUID) ->
    MatchSpec = [
        {#channel{uuid = '$1', other_leg = '$2', _ = '_'}, [{'=:=', '$1', {'const', UUID}}], ['$2']}
    ],
    case ets:select(?CHANNELS_TBL, MatchSpec) of
        ['undefined'] ->
            lager:debug("channel is not bridged"),
            'false';
        [Bin] when is_binary(Bin) ->
            lager:debug("is bridged to: ~s", [Bin]),
            'true';
        _E ->
            lager:debug("not bridged: ~p", [_E]),
            'false'
    end.
...
import_moh(UUID) ->
    try
        ets:lookup_element(?CHANNELS_TBL, UUID, #channel.import_moh)
    catch
        'error':'badarg' -> 'false'
    end.
...
channel_data(Node, UUID) ->
    case freeswitch:api(Node, 'uuid_dump', UUID) of
        {'error', _} = E -> E;
        {'ok', Dump} -> {'ok', ecallmgr_util:eventstr_to_proplist(Dump)}
    end.
...
handle_cast('bind_to_events', #state{node = Node} = State) ->
    ...
    case
        gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"CHANNEL_DATA">>)}) =:= 'true' andalso
            gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"CHANNEL_CREATE">>)}) =:= 'true' andalso
            gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"CHANNEL_DESTROY">>)}) =:= 'true' andalso
            gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"CHANNEL_ANSWER">>)}) =:= 'true' andalso
            gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"CHANNEL_BRIDGE">>)}) =:= 'true' andalso
            gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"CHANNEL_UNBRIDGE">>)}) =:= 'true' andalso
            gproc:reg({'p', 'l', ?FS_EVENT_REG_MSG(Node, <<"CALL_UPDATE">>)}) =:= 'true' andalso
...
handle_info({'event', [UUID | Props]}, #state{node = Node, options = Options} = State) ->
    ...
    {'noreply', State};
handle_info(
    {'fetch', 'channels', <<"channel">>, <<"uuid">>, UUID, FetchId, _}, #state{node = Node} = State
) ->
...
handle_info({_Fetch, _Section, _Something, _Key, _Value, ID, _Data}, #state{node = Node} = State) ->
    lager:debug("unhandled ~p with fetch id ~p from section ~p for ~p:~p:~p", [
        _Fetch, ID, _Section, _Something, _Key, _Value
    ]),
...
-spec handle_fs_event(kz_term:api_binary(), kzd_freeswitch:data(), atom(), kz_term:proplist()) ->
    'ok'.
...
handle_channel_req_legacy(UUID, FetchId, Node, Pid) ->
    ...
    case fetch_channel(UUID) of
        ...
        Channel ->
            ...
            try_channel_resp(FetchId, Node, [
                {<<"sip-url">>, URL},
                {<<"uuid">>, UUID}
            ])
    end.

-spec handle_channel_req(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist(), atom(), pid()
) -> 'ok'.
handle_channel_req(UUID, FetchId, Props, Node, Pid) ->
    ...
    case fetch_channel(UUID) of
        ...
        Channel ->
            ...
            URL = kzsip_uri:ruri(
                #uri{
                    user = props:get_value(<<"refer-to-user">>, Props),
...
-spec channel_resp_dialprefix(kz_term:proplist(), kz_term:proplist(), kz_term:proplist()) ->
    kz_term:ne_binary().
channel_resp_dialprefix(ReqProps, Channel, ChannelVars) ->
    Props = props:filter_undefined(
        [
            ...
            {
                <<"sip_h_X-ecallmgr_", ?CALL_INTERACTION_ID>>,
                props:get_value(<<"interaction_id">>, Channel)
            },
            {<<"sip_h_X-ecallmgr_replaces-call-id">>,
                props:get_value(<<"replaces-call-id">>, ReqProps)},
            {<<"sip_h_X-ecallmgr_refer-from-channel-id">>,
                props:get_value(<<"refer-from-channel-id">>, ReqProps)},
            {<<"sip_h_X-ecallmgr_refer-for-channel-id">>,
                props:get_value(<<"refer-for-channel-id">>, ReqProps)},
...
maybe_publish_channel_state(Props, Node) ->
    ...
    case
        props:is_true(<<"Publish-Channel-State">>, Props, 'true') andalso
            kapps_config:get_boolean(?APP_NAME, <<"publish_channel_state">>, 'true', Node)
    of
        'false' ->
            lager:debug("not publishing channel state ~s", [Event]);
        'true' ->
            case
                kapps_config:get_boolean(?APP_NAME, <<"restrict_channel_state_publisher">>, 'false')
            of
...
maybe_publish_restricted(Props) ->
    ...
    case props:get_value(?GET_CCV(<<"Ecallmgr-Node">>), Props) of
        ...
        _EventEcallmgr ->
            lager:debug(
                "channel state ~s for call controlled by another ecallmgr(~s), not publishing", [
                    Event, _EventEcallmgr
                ]
            )
    end.
...
handling_locally(Props, OtherLeg) ->
    ...
    case props:get_value(?GET_CCV(<<"Ecallmgr-Node">>), Props) of
        ...
        _OtherNode ->
            lager:debug(
                "ccv has ecallmgr ~s (we are ~s), checking other leg ~s",
...
props_to_update(Props) ->
    ...
    props:filter_undefined(
        [
            ...
            {
                #channel.import_moh,
                props:get_value(<<"variable_hold_music">>, Props) =:= 'undefined'
            },
...
get_other_leg('undefined', _Props) ->
    'undefined';
get_other_leg(UUID, Props) ->
    get_other_leg_name(UUID, Props, props:get_value(<<"Other-Leg-Channel-Name">>, Props)).

-spec get_other_leg_name(kz_term:ne_binary(), kz_term:proplist(), kz_term:ne_binary()) ->
    kz_term:api_binary().
...
-spec get_other_leg(kz_term:ne_binary(), kz_term:proplist(), kz_term:api_binary()) ->
    kz_term:api_binary().
get_other_leg(UUID, Props, 'undefined') ->
    maybe_other_bridge_leg(
        UUID,
        Props,
        props:get_value(<<"Bridge-A-Unique-ID">>, Props),
        props:get_value(<<"Bridge-B-Unique-ID">>, Props)
    );
get_other_leg(_UUID, _Props, OtherLeg) ->
    OtherLeg.

-spec maybe_other_bridge_leg(
    kz_term:ne_binary(), kz_term:proplist(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    kz_term:api_binary().
maybe_other_bridge_leg(UUID, _Props, UUID, OtherLeg) ->
    OtherLeg;
maybe_other_bridge_leg(UUID, _Props, OtherLeg, UUID) ->
    OtherLeg;
...
maybe_update_interaction_id(_Props, _Node, {CoreUUID, CoreUUID}) ->
    'ok';
