...
-define(WSD_TITLE(T),
    ?WSD_ENABLED andalso
        webseq:title(?WSD_ID, [T, " in ", kz_term:to_binary(self())])
).
-define(WSD_START(),
    ?WSD_ENABLED andalso
        webseq:start(?WSD_ID)
...
event(ServerRef, CallId, <<"DTMF">>, JObj) ->
    gen_statem:cast(ServerRef, {'dtmf', CallId, kz_call_event:dtmf_digit(JObj)});
...
transfer_to(Call, Leg) ->
    gen_statem:cast(
        kapps_call:kvs_fetch(?MODULE, Call),
        {'transfer_to', Call, Leg}
    ).
...
handle_event(
    ?EVENT(_CallId, <<"CHANNEL_ANSWER">>, Evt),
    StateName,
    State
) ->
    {'next_state', StateName, handle_channel_answer(State, kz_call_event:call_id(Evt), Evt)};
handle_event(
    ?EVENT(CallId, <<"CHANNEL_BRIDGE">>, Evt),
    StateName,
    #state{call_id = CallId} = State
) ->
    {'next_state', StateName,
        handle_channel_bridge(State, CallId, kz_call_event:other_leg_call_id(Evt))};
handle_event(
    ?EVENT(OtherLeg, <<"CHANNEL_BRIDGE">>, Evt),
    StateName,
    #state{other_leg = OtherLeg} = State
) ->
    {'next_state', StateName,
        handle_channel_bridge(State, kz_call_event:other_leg_call_id(Evt), OtherLeg)};
handle_event(
    ?EVENT(CallId, <<"CHANNEL_DESTROY">>, _Evt),
    StateName,
    #state{call_id = CallId} = State
) ->
    {'next_state', StateName, handle_channel_destroy(State, CallId)};
handle_event(
    ?EVENT(OtherLeg, <<"CHANNEL_DESTROY">>, _Evt),
    StateName,
    #state{other_leg = OtherLeg} = State
) ->
    {'next_state', StateName, handle_channel_destroy(State, OtherLeg)};
handle_event(
    ?EVENT(_UUID, _EventName, _Evt),
    StateName,
    #state{
        call_id = _CallId,
        other_leg = _OtherLeg,
...
patterns(Call, JObj) ->
    case kz_json:get_value(<<"Patterns">>, JObj) of
        'undefined' ->
            konami_config:patterns(kapps_call:account_id(Call));
        Patterns ->
...
is_a_leg(Call, <<_/binary>> = EndpointId) -> EndpointId =:= kapps_call:authorizing_id(Call);
is_a_leg(Call, JObj) -> is_a_leg(Call, kz_json:get_value(<<"Endpoint-ID">>, JObj)).
...
arm_aleg(#state{digit_timeout = Timeout} = State) ->
    State#state{
        a_digit_timeout_ref = erlang:start_timer(Timeout, self(), 'digit_timeout'),
        a_collected_dtmf = <<>>,
        a_leg_armed = 'true'
    }.
...
arm_bleg(#state{digit_timeout = Timeout} = State) ->
    State#state{
        b_digit_timeout_ref = erlang:start_timer(Timeout, self(), 'digit_timeout'),
        b_collected_dtmf = <<>>,
        b_leg_armed = 'true'
    }.
...
handle_channel_answer(#state{other_leg = OtherLeg} = State, OtherLeg, _Evt) ->
    ...
    State;
handle_channel_answer(
    #state{other_leg = 'undefined'} = State,
    CallId,
    Evt
) ->
    maybe_other_leg_answered(
        State,
        CallId,
