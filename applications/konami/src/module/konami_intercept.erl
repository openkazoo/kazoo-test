...
-spec maybe_update_metaflow(
    kz_json:object(), kapps_call:call(), kz_json:objects(), kz_term:api_binary()
) ->
    {'stop', kapps_call:call()}.
maybe_update_metaflow(Data, Call, Results, CallId) ->
    case [Result || Result <- Results, is_originate_uuid(Result, CallId)] of
        ...
        [OriginateUUID | _] ->
            ...
            maybe_update_metaflow_control(
                Data, Call, CallId, ControlQueue, source_leg_of_dtmf(Data, Call)
            )
    end.

-spec maybe_update_metaflow_control(
    kz_json:object(), kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary(), 'a' | 'b'
) ->
    {'stop', kapps_call:call()}.
...
maybe_update_metaflow_control(_Data, Call, CallId, _ControlQueue, 'b') ->
    lager:debug("update ~s to ~s with ctl ~s", [
        kapps_call:other_leg_call_id(Call), CallId, _ControlQueue
    ]),
...
get_originate_req(Data, Call) ->
    ...
    Params = kz_json:set_values(
        [
            {<<"source">>, <<?MODULE_STRING>>},
            {<<"can_call_self">>, 'true'}
        ],
        Data
    ),
...
build_endpoints(<<"user">>, OwnerId, SourceDeviceId, Params, Call) ->
    ...
    lists:foldr(
        fun
            (EndpointId, Acc) when EndpointId =:= SourceDeviceId ->
...
-spec build_originate(kz_json:objects(), kz_term:ne_binary(), boolean(), kapps_call:call()) ->
    kz_term:proplist().
...
is_originate_uuid(JObj, CallId) ->
    kapi_resource:originate_uuid_v(JObj) andalso
        (CallId =:= 'undefined' orelse
