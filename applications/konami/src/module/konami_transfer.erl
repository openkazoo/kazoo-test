...
-define(WSD_EVT(Fr, T, E),
    ?WSD_ENABLED andalso
        webseq:evt(?WSD_ID, Fr, T, <<(kz_term:to_binary(?LINE))/binary, "-", E/binary>>)
).

-define(WSD_NOTE(W, D, N),
    ?WSD_ENABLED andalso
        webseq:note(?WSD_ID, W, D, <<(kz_term:to_binary(?LINE))/binary, "-", N/binary>>)
).

-define(WSD_TITLE(T),
    ?WSD_ENABLED andalso
        webseq:title(?WSD_ID, T)
).
...
-record(state, {
    transferor :: kz_term:api_ne_binary(),
    transferee :: kz_term:api_ne_binary(),
    %% this is the real b-leg, if any
    target :: kz_term:api_ne_binary(),
    %% loopback-a
    target_a_leg :: kz_term:api_ne_binary(),
    %% loopback-b
    target_b_leg :: kz_term:api_ne_binary(),
...
-define(DEFAULT_TARGET_TIMEOUT,
    kapps_config:get_integer(
        ?CONFIG_CAT, [<<"transfer">>, <<"default_target_timeout_ms">>], 20 * ?MILLISECONDS_IN_SECOND
    )
).

-define(DEFAULT_RINGBACK,
    kapps_config:get_ne_binary(<<"ecallmgr">>, <<"default_ringback">>, <<"%(2000,4000,440,480)">>)
).

-define(TRANSFEROR_CALL_EVENTS, [
    <<"CHANNEL_BRIDGE">>,
    <<"CHANNEL_UNBRIDGE">>,
    <<"DTMF">>,
    <<"CHANNEL_DESTROY">>,
...
pre_originate(
    'cast',
    ?EVENT(UUID, <<"CHANNEL_UNBRIDGE">>, _Evt),
    #state{
        call = Call,
        moh = MOH,
        transferee = Transferee,
...
attended_wait(
    'cast',
    ?EVENT(Transferee, <<"CHANNEL_DESTROY">>, _Evt),
    #state{transferee = Transferee} = State
) ->
    lager:info(
        "transferee ~s hungup (~s) before target could be reached",
...
attended_wait(
    'cast',
    ?EVENT(Transferor, <<"CHANNEL_DESTROY">>, _Evt),
    #state{
        transferor = Transferor,
        target = Target,
        transferee = Transferee
    ...
    lager:info(
        "transferor ~s hungup (~s), connecting transferee ~s and target ~s (or tbd)",
...
attended_wait(
    'cast',
    ?EVENT(Transferor, <<"CHANNEL_BRIDGE">>, Evt),
    #state{
        transferor = Transferor,
        transferee = Transferee,
        target = Target,
...
attended_wait(
    'cast',
    ?EVENT(TargetA, <<"CHANNEL_ANSWER">>, _Evt),
...
attended_wait(
    'cast',
    ?EVENT(Target, <<"CHANNEL_BRIDGE">>, Evt),
    #state{
        target = Target,
        transferor = Transferor,
        purgatory_ref = Ref
    ...
    case kz_call_event:other_leg_call_id(Evt) of
        Transferor ->
            lager:info(
                "recv CHANNEL_BRIDGE on target ~s to transferor ~s, moving to attended_answer",
                [Target, Transferor]
            ),
...
attended_wait(
    'cast',
    ?EVENT(Target, <<"CHANNEL_DESTROY">>, _Evt),
...
attended_wait(
    'cast',
    ?EVENT(TargetA, <<"originate_uuid">>, Evt),
    #state{
    ...
    {'next_state', 'attended_wait', State#state{
...
attended_wait(
    ...
    case kz_call_event:other_leg_call_id(Evt) of
        ...
        _CallId ->
            ...
            lager:info(
                "transferor ~s bridged to ~s, moving to attended_answer",
                [Transferor, _CallId]
            ),
            {'next_state', 'attended_answer', State}
    end;
attended_wait(
    'cast',
    ?EVENT(TargetA, <<"CHANNEL_CREATE">>, _Evt),
    #state{
        target_a_leg = TargetA,
        transferor = Transferor,
        ringback = Ringback,
...
attended_wait(
    'cast',
    ?EVENT(TargetA, <<"originate_resp">>, _Evt),
    #state{target_a_leg = TargetA} = State
) ->
...
attended_wait(
    'cast',
    ?EVENT(TargetA, <<"CHANNEL_DESTROY">>, _Evt),
    #state{
        target_a_leg = TargetA,
        target = 'undefined',
    ...
    Ref = erlang:start_timer(?MILLISECONDS_IN_SECOND, self(), 'purgatory'),
    lager:debug(
        "target 'a' ~s has gone done (~s), going to purgatory in ~p",
        [TargetA, kz_call_event:hangup_cause(_Evt), Ref]
    ),
    {'next_state', 'attended_wait', State#state{purgatory_ref = Ref}};
attended_wait(
    'cast',
    ?EVENT(TargetB, <<"CHANNEL_DESTROY">>, _Evt),
    #state{target_b_leg = TargetB} = State
) ->
    lager:debug(
        "target 'b' ~s has gone done: ~s",
        [TargetB, kz_call_event:hangup_cause(_Evt)]
    ),
    {'next_state', 'attended_wait', State};
attended_wait(
    'cast',
    ?EVENT(TargetA, <<"CHANNEL_REPLACED">>, Evt),
    #state{
        target_a_leg = TargetA,
        target = 'undefined'
    } = State
...
attended_wait(
    'cast',
    ?EVENT(TargetB, <<"CHANNEL_ANSWER">>, _Evt),
    #state{
...
attended_wait(
    'cast',
    ?EVENT(TargetB, <<"LEG_DESTROYED">>, Evt),
    #state{
        target_b_leg = TargetB,
        target_legs = TargetLegs
    } = State
...
attended_wait(
    'cast',
    ?EVENT(TargetB, <<"CHANNEL_BRIDGE">>, Evt),
    #state{
        target_b_leg = TargetB,
        target = Target
    } = State
...
attended_wait('cast', ?EVENT(_CallId, <<"LEG_CREATED">>, _Evt), State) ->
    lager:debug(
        "ignoring leg_created for ~s and ~s",
        [_CallId, kz_call_event:other_leg_call_id(_Evt)]
    ),
    {'next_state', 'attended_wait', State};
attended_wait(
    'cast',
    ?EVENT(TargetA, _EventName, _Evt),
    #state{target_a_leg = TargetA} = State
) ->
...
partial_wait(
    'cast',
    ?EVENT(Transferee, <<"CHANNEL_BRIDGE">>, Evt),
    #state{
        transferee = Transferee,
        target = Target
    } = State
...
partial_wait(
    ...
    lager:info(
        "transferee ~s hungup (~s) while target was being rung",
        [Transferee, kz_call_event:hangup_cause(_Evt)]
    ),
    hangup_target(TargetCall),
    {'stop', 'normal', State};
partial_wait(
    ...
    lager:info(
        "transferor ~s hungup(~s), still waiting on target and transferee",
...
partial_wait(
    'cast',
    ?EVENT(Target, <<"CHANNEL_ANSWER">>, _Evt),
    #state{target = Target} = State
) ->
    lager:info(
        "target ~s has answered (with ~s)",
        [Target, kz_call_event:other_leg_call_id(_Evt)]
    ),
    {'next_state', 'partial_wait', State};
partial_wait(
    'cast',
    ?EVENT(Target, <<"originate_uuid">>, Evt),
    #state{
        target = Target,
        target_call = TargetCall
    } = State
    ...
    {'next_state', 'partial_wait', State#state{
        target_call = kapps_call:from_originate_uuid(Evt, TargetCall)
    }};
partial_wait(
    'cast',
    ?EVENT(TargetB, <<"CHANNEL_ANSWER">>, _Evt),
...
partial_wait(
    ...
    {'next_state', 'partial_wait',
        handle_real_target(State, kz_call_event:replaced_by(Evt), 'transferee')};
partial_wait(
    'cast',
    ?EVENT(TargetA, <<"CHANNEL_REPLACED">>, Evt),
    #state{
...
attended_answer(
    'cast', ?EVENT(Transferor, <<"DTMF">>, Evt), #state{transferor = Transferor} = State
) ->
...
attended_answer(
    ...
    lager:info(
        "transferee ~s hungup(~s) while transferor and target were talking",
        [Transferee, kz_call_event:hangup_cause(_Evt)]
    ),
    lager:info("transferor and target are on their own"),
...
attended_answer(
    ...
    lager:info(
        "transferor ~s hungup(~s), connecting transferee ~s and target ~s",
        [Transferor, kz_call_event:hangup_cause(_Evt), Transferee, Target]
    ),
...
attended_answer(
    ...
    lager:info(
        "target ~s hungup(~s), reconnecting transferor ~s to transferee ~s",
...
takeback(
    'cast',
    ?EVENT(Transferor, <<"CHANNEL_UNBRIDGE">>, _Evt),
    #state{
        transferor = Transferor,
        call = Call
    } = State
...
takeback(
    ...
    lager:debug(
        "transferor ~s bridged to ~s, tearing down target ~s",
        [
            Transferor,
            _OtherLeg,
            Target
...
takeback(
    ...
    _OtherLeg = kz_call_event:other_leg_call_id(Evt),
    lager:debug(
        "transferee ~s bridged to ~s, tearing down target ~s",
        [
            Transferee,
            _OtherLeg,
            Target
...
takeback(
    ...
    case kz_call_event:other_leg_call_id(Evt) of
        Transferor ->
            lager:debug(
                "target ~s is bridged to transferor ~s while in takeback",
                [Target, Transferor]
            ),
...
handle_info(
    ...
    case {kz_call_event:call_id(JObj), kz_api:node(JObj)} of
...
handle_info(
    ...
    lager:info(
        "target ~s hungup, sorry transferee ~s",
        [Target, _Transferee]
    ),
...
-spec originate_to_extension(kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    kz_term:ne_binary().
originate_to_extension(Extension, TransferorLeg, Call) ->
    ...
    CCVs = kz_json:from_list(
        [
            ...
            {<<"From-URI">>,
                <<CallerIdNumber/binary, "@", (kapps_call:account_realm(Call))/binary>>},
...
connect_transferor_to_target(Transferor, TargetCall) ->
    ...
    lager:debug("connecting transferor to target ~s, hangup is ~s", [
        kapps_call:call_id(TargetCall), 'false'
    ]),
...
connect_to_transferee(Call) ->
    Flags = [
        {<<"Target-Call-ID">>, kapps_call:other_leg_call_id(Call)},
        {<<"Continue-On-Fail">>, 'false'},
        {<<"Continue-On-Cancel">>, 'false'},
        {<<"Park-After-Pickup">>, 'false'},
        {<<"Hangup-After-Pickup">>, 'true'},
    ...
    lager:debug(
        "reconnecting transferor/ee: ~s and ~s, hangup is ~s",
        [
...
connect(Flags, Call) ->
    ...
    ?WSD_EVT(
        props:get_value(<<"Call-ID">>, Command),
...
-spec builder_check_option(kz_json:object(), string(), check_fun(), build_fun()) ->
    kz_term:api_object().
...
-spec metaflow_jobj(
    kz_json:object(), kz_term:api_binary(), kz_term:api_binary(), kz_term:api_binary()
) -> kz_json:object().
...
-spec transfer_data(kz_term:api_binary(), kz_term:api_binary(), kz_term:api_binary()) ->
    kz_json:object().
...
-spec maybe_start_transferor_ringback(kapps_call:call(), kz_term:ne_binary(), kz_term:api_binary()) ->
    'ok'.
maybe_start_transferor_ringback(_Call, _Transferor, 'undefined') ->
    'ok';
maybe_start_transferor_ringback(Call, Transferor, Ringback) ->
    ...
    kapps_call_command:send_command(
        kz_json:set_values([{<<"Insert-At">>, <<"now">>}], Command), Call
    ).
...
suppress_event(JObj, _EventNode, _OtherNode) ->
    lager:debug(
        "supressing event ~s from ~s (we want events from ~s): ~s",
        [
...
handle_real_target(
    ...
    State1 = State#state{
        target_call = kapps_call:set_call_id(ReplacementId, TargetCall),
