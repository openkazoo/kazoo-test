...
-define(RESPONDERS, [
    ...
    {{?MODULE, 'handle_metaflow_req'}, [{<<"metaflow">>, <<"action">>}]},
    {{?MODULE, 'handle_konami'}, [{?APP_NAME, <<"*">>}]}
]).
-define(QUEUE_NAME, <<>>).
-define(QUEUE_OPTIONS, []).
-define(CONSUME_OPTIONS, []).

-define(TRACKED_CALL_EVENTS, [
    <<"CHANNEL_ANSWER">>,
    <<"CHANNEL_BRIDGE">>,
    <<"CHANNEL_DESTROY">>,
    <<"CHANNEL_REPLACED">>,
...
-define(DYN_BINDINGS(CallId),
    'call',
    [
        {'restrict_to', ?TRACKED_CALL_EVENTS},
        {'callid', CallId}
    ]
).
-define(DYN_BINDINGS(CallId, Events),
    'call',
    [
        {'restrict_to', Events},
        {'callid', CallId}
...
-spec bindings(kz_term:ne_binary()) ->
    [{kz_term:ne_binary(), kz_term:atoms() | kz_term:ne_binaries()}].
bindings(CallId) ->
    [
...
add_konami_binding(CallId) -> gen_listener:add_binding(?SERVER, ?KONAMI_BINDINGS(CallId)).

-spec rm_konami_binding(kz_term:api_binary()) -> 'ok'.
rm_konami_binding('undefined') ->
    'ok';
...
-spec add_call_binding(
    kz_term:api_ne_binary() | kapps_call:call(), kz_term:ne_binaries() | kz_term:atoms()
) -> 'ok'.
add_call_binding('undefined', _) ->
    'ok';
...
rm_call_binding('undefined', _Evts) ->
    'ok';
...
handle_call_event(JObj, Props) ->
    'true' =
        kapi_call:event_v(JObj) orelse
            kapi_dialplan:error_v(JObj),
...
-spec maybe_remove_binding(kz_types:server_ref(), atom(), kz_term:proplist(), kz_term:api_binary()) ->
    'ok'.
...
maybe_remove_binding(Srv, Binding, Props, CallId) ->
    case {fsms_for_callid(CallId), pids_for_callid(CallId)} of
        {[], []} ->
            lager:debug("~p: no pids for call-id '~s', removing binding '~s'", [
                Srv, CallId, Binding
            ]),
