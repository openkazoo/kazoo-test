...
-define(TRANSFERRED_VALUES, [
    {<<"Event-Category">>, ?APP_NAME},
...
bind_for_transferred(Queue, CallId) ->
    manipulate_queue_bindings(
        Queue, transferred_routing_key(CallId), fun kz_amqp_util:bind_q_to_kapps/2
    ).

-spec unbind_for_transferred(kz_term:ne_binary(), kz_term:ne_binary()) -> 'ok'.
unbind_for_transferred(Queue, CallId) ->
    manipulate_queue_bindings(
        Queue, transferred_routing_key(CallId), fun kz_amqp_util:unbind_q_from_kapps/2
    ).
