...
-record(builder_action, {
    module_fun_name :: atom(),
...
is_running() ->
    case lists:keyfind('konami', 1, application:which_applications()) of
        ...
        {_App, _Desc, _Vsn} -> io:format("Konami (~s) is running~n", [_Vsn])
    end.
...
add_default_account_metaflow(AccountId) ->
    Default = kapps_account_config:get(
        AccountId, <<"metaflows">>, <<"default_metaflow">>, kz_json:new()
    ),
...
intro_builder(Default, SaveFun) ->
    ...
    io:format(
        "  Digit Timeout(ms): ~b~n",
        [kz_json:get_integer_value(<<"digit_timeout_ms">>, Default, konami_config:timeout())]
    ),

    io:format("  Numbers: ~s~n", [
        kz_json:encode(kz_json:get_value(<<"numbers">>, Default, kz_json:new()))
    ]),
    io:format("  Patterns: ~s~n~n", [
        kz_json:encode(kz_json:get_value(<<"patterns">>, Default, kz_json:new()))
    ]),
...
number_builder(Default, SaveFun) ->
    ...
    builder_menu(
        ...
        SaveFun,
        #builder_action{
            builders = lists:zip(lists:seq(1, length(Ms)), Ms),
