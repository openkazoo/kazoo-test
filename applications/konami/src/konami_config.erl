...
numbers(Account) ->
    case konami_doc(Account) of
        'undefined' -> numbers();
        KonamiDoc -> get_attribute(KonamiDoc, <<"numbers">>, fun numbers/0)
    end.
...
patterns(Account) ->
    case konami_doc(Account) of
        'undefined' -> patterns();
        KonamiDoc -> get_attribute(KonamiDoc, <<"patterns">>, fun patterns/0)
    end.

-spec binding_digit() -> <<_:8>>.
binding_digit() ->
    BindingDigit = kapps_config:get_ne_binary(
        <<"metaflows">>, <<"binding_digit">>, ?DEFAULT_BINDING_DIGIT
    ),
...
binding_digit(Account) ->
    case konami_doc(Account) of
        ...
        KonamiDoc ->
            get_attribute(
                KonamiDoc, <<"binding_digit">>, fun binding_digit/0, fun constrain_binding_digit/1
            )
    end.
...
timeout(Account) ->
    case konami_doc(Account) of
        ...
        KonamiDoc ->
            get_attribute(
                KonamiDoc, <<"digit_timeout_ms">>, fun timeout/0, fun kz_term:to_integer/1
            )
    end.
...
listen_on() ->
    constrain_listen_on(
        kapps_config:get_ne_binary(<<"metaflows">>, <<"listen_on">>, ?DEFAULT_LISTEN_ON)
    ).
...
konami_doc(Account) ->
    case kzd_accounts:fetch(Account) of
        {'ok', JObj} ->
            kz_json:get_value(<<"metaflows">>, JObj);
        {'error', 'not_found'} ->
            'undefined';
        {'error', _E} ->
