...
-define(ACCOUNT_BINDING(AccountId, ConferenceId, CallId, Event),
    <<"conference.event.", Event/binary, ".", AccountId/binary, ".", ConferenceId/binary, ".",
        CallId/binary>>
).
...
-define(EVENT_BINDINGS(ConferenceId, CallId),
    lists:foldl(
        fun(E, Acc) -> [?BINDING(ConferenceId, CallId, E) | Acc] end, [], ?CONFERENCE_EVENTS
    )
).
-define(ALL, <<"*">>).
...
validate(Context, #{keys := Keys}) ->
    bh_context:add_error(
        Context, <<"invalid format for conference subscription : ", (kz_binary:join(Keys))/binary>>
    ).
...
bindings(
    ...
    Listeners = [
        {'amqp', 'conference', event_binding_options(AccountId, ConferenceId, CallId, ?ALL)}
    ],
    Map#{
        requested => Requested,
        subscribed => Subscribed,
...
bindings(
    ...
    Listeners = [
        {'amqp', 'conference', event_binding_options(AccountId, ConferenceId, CallId, Event)}
    ],
    Map#{
        requested => Requested,
        subscribed => Subscribed,
