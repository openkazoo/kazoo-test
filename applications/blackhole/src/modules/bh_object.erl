...
validate(Context, #{keys := [Action, Type]}) ->
    case
        lists:member(Action, ?DOC_ACTIONS) andalso
            lists:member(Type, ?DOC_TYPES)
    of
        'true' ->
            Context;
        'false' ->
            bh_context:add_error(
                Context, <<"event ", Action/binary, ".", Type/binary, " not supported">>
            )
    end;
validate(Context, #{keys := Keys}) ->
    bh_context:add_error(
        Context, <<"invalid format for object subscription : ", (kz_binary:join(Keys))/binary>>
    ).
