...
-define(RESPONDERS, [
    {{?MODULE, 'handle_resource_response'}, [{<<"*">>, <<"*">>}]},
...
-define(PROMPT_DELAY,
    kapps_config:get_integer(?CCCP_CONFIG_CAT, <<"prompt_delay">>, 1) * ?MILLISECONDS_IN_SECOND
).
...
init([JObj]) ->
    ...
    RealCallbackDelay =
        case is_integer(CallbackDelay) of
            'true' ->
                CallbackDelay * ?MILLISECONDS_IN_SECOND;
            'false' ->
                kapps_config:get_integer(?CCCP_CONFIG_CAT, <<"callback_delay">>, 3) *
...
handle_resource_response(JObj, Props) ->
    ...
    case kz_util:get_event_type(JObj) of
        ...
        {<<"call_event">>, <<"CHANNEL_ANSWER">>} ->
            CallUpdate = kapps_call:kvs_store_proplist(
                [{'consumer_pid', self()}, {'auth_doc_id', props:get_value('auth_doc_id', Props)}],
                kapps_call:from_route_req(JObj, call(Props))
            ),
            gen_listener:cast(Srv, {'call_update', CallUpdate}),
            gen_listener:cast(
                Srv, {'parked', CallId, b_leg_number(props:set_value(call, CallUpdate, Props))}
            );
...
bridge_to_final_destination(CallId, ToDID, #state{
    ...
    cccp_util:bridge(
        CallId, ToDID, AuthorizingId, CtrlQ, AccountId, RetainCID, ALegName, ALegNumber
    ),
