...
maybe_known_number(ControllerQ, JObj) ->
    ...
    case knm_number:lookup_account(Number) of
        {'ok', _, _} ->
            choose_response(ControllerQ, JObj, 'false', <<"known_number">>);
        {'error', _R} ->
...
-spec choose_response(kz_term:ne_binary(), kz_json:object(), boolean(), kz_term:ne_binary()) ->
    'ok'.
...
send_response(JObj, ControllerQ, Reconcilable, <<"unknown_number">> = Type) ->
    ...
    Message = kapps_config:get_binary(
        ?APP_NAME, [Type, <<"response_message">>], <<"Nope Nope Nope">>
    ),
...
-spec send_response(
    kz_json:object(), kz_term:ne_binary(), boolean(), kz_term:ne_binary(), kz_term:api_binary()
) -> 'ok'.
send_response(JObj, ControllerQ, Reconcilable, Code, Message) ->
...
-spec maybe_send_transfer(kz_json:object(), kz_term:ne_binary(), boolean(), kz_term:ne_binary()) ->
    'ok'.
...
-spec maybe_send_bridge(kz_json:object(), kz_term:ne_binary(), boolean(), kz_term:ne_binary()) ->
    'ok'.
maybe_send_bridge(JObj, ControllerQ, Reconcilable, Type) ->
    ...
    case
        kz_term:is_empty(AccountId) orelse
            kz_term:is_empty(EndpointId)
    of
        'true' ->
            send_response(JObj, ControllerQ, Reconcilable, Type);
        'false' ->
...
-spec send_bridge(
    kz_json:object(), kz_term:ne_binary(), boolean(), kz_term:ne_binary(), kz_term:jobjs_return()
) -> 'ok'.
