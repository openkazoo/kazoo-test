...
-define(RESPONDERS, [
    {{?MODULE, 'relay_amqp'}, [{<<"call_event">>, <<"*">>}]},
    {{?MODULE, 'handle_conference_event'}, [{<<"conference">>, <<"event">>}]},
...
-record(participant, {
    participant_id = 0 :: non_neg_integer(),
...
start_sanity_check_timer() ->
    start_sanity_check_timer(
        kapps_config:get_integer(
            ?CONFIG_CAT, <<"participant_sanity_check_ms">>, ?MILLISECONDS_IN_MINUTE
        )
    ).
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_call(any(), kz_term:pid_ref(), participant()) ->
    kz_types:handle_call_ret_state(participant()).
...
handle_cast({'set_conference', Conference}, Participant = #participant{call = Call}) ->
    ...
    gen_listener:add_binding(self(), 'conference', [
        {'restrict_to', [{'event', {ConferenceId, CallId}}]}
    ]),
...
handle_cast(_Message, #participant{conference = 'undefined'} = Participant) ->
    ...
    lager:debug(
        "ignoring message prior to conference discovery: ~p",
        [_Message]
    ),
    {'noreply', Participant};
handle_cast(
    ...
    lager:debug("sending command for participant to join local conference ~s", [
...
handle_cast(
    ...
    lager:debug("sending command for participant to join remote conference ~s", [
        kapps_conference:id(Conference)
    ]),
    Route = binary:replace(
        kz_json:get_value(<<"Switch-URL">>, JObj),
        <<"mod_sofia">>,
        <<"conference">>
    ),
...
handle_event(JObj, #participant{
    call_event_consumers = Consumers,
    call = Call
}) ->
    CallId = kapps_call:call_id(Call),
...
log_conference_join('true' = _Moderator, ParticipantId, Conference) ->
    lager:debug("caller has joined the local conference ~s as moderator ~p", [
        kapps_conference:name(Conference), ParticipantId
    ]);
log_conference_join('false' = _Moderator, ParticipantId, Conference) ->
    lager:debug("caller has joined the local conference ~s as member ~p", [
        kapps_conference:name(Conference), ParticipantId
    ]).

-spec sync_participant(kz_term:ne_binary(), kz_json:objects(), kapps_call:call(), participant()) ->
    participant().
...
sync_participant(
    ...
    _ = kz_util:spawn(fun notify_requestor/4, [
        kapps_call:controller_queue(Call),
        ParticipantId,
        DiscoveryEvent,
        kapps_conference:id(Conference)
    ]),
...
-spec notify_requestor(
    kz_term:ne_binary(), non_neg_integer(), kz_json:object(), kz_term:ne_binary()
) -> 'ok'.
...
bridge_to_conference(Route, Conference, Call, Name) ->
    ...
    SIPHeaders = props:filter_undefined([
        {<<"X-Conf-Flags-Moderator">>, kapps_conference:moderator(Conference)}
...
set_enter_exit_sounds({_, AccountId, MediaId}, #participant{
    conference = Conference,
    call = Call,
    moderator = IsModerator
}) ->
    EntrySounds = [
        play_entry_tone(IsModerator, Conference),
        kz_media_util:media_path(MediaId, AccountId),
        kapps_call:get_prompt(Call, <<"conf-has_joined">>)
    ],

    ExitSounds = [
        play_exit_tone(IsModerator, Conference),
        kz_media_util:media_path(MediaId, AccountId),
    ...
    Sounds = [
        {<<"Conference-Entry-Sound">>, lists:filter(Fun, EntrySounds)},
...
-spec play_exit_tone_media(kz_term:ne_binary(), kapps_conference:conference()) ->
    kz_term:api_binary().
play_exit_tone_media(Tone, Conference) ->
    case kapps_conference:play_exit_tone(Conference) of
        'false' ->
            'undefined';
        MediaId = ?NE_BINARY ->
            kz_media_util:media_path(MediaId, kapps_conference:account_id(Conference));
        _Else ->
            Tone
    end.
...
-spec play_entry_tone_media(kz_term:ne_binary(), kapps_conference:conference()) ->
    kz_term:api_binary().
