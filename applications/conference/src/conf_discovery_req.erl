...
-spec create_conference(kapi_conference:discover_req(), kapps_call:call()) ->
    kapps_conference:conference().
create_conference(DiscoveryReq, Call) ->
    create_conference(
        DiscoveryReq, Call, kz_json:get_ne_binary_value(<<"Conference-ID">>, DiscoveryReq)
    ).

-spec create_conference(
    kapi_conference:discovery_req(), kapps_call:call(), kz_term:api_ne_binary()
) -> kapps_conference:conference().
...
-spec create_collected_conference(kz_json:object(), kapps_conference:conference()) ->
    kapps_conference:conference().
...
cleanup_conference_doc(DiscoveryReq) ->
    cleanup_conference_doc(
        DiscoveryReq, kz_json:get_json_value(<<"Conference-Doc">>, DiscoveryReq)
    ).

-spec cleanup_conference_doc(kapi_conference:discovery_req(), kzd_conferences:doc() | 'undefined') ->
    kapi_conference:discovery_req().
cleanup_conference_doc(DiscoveryReq, 'undefined') ->
    DiscoveryReq;
...
maybe_welcome_to_conference(Srv, Conference) ->
    lager:debug(
        "starting discovery process for conference ~s(id:~s)",
        [
...
welcome_to_conference(Srv, Conference) ->
    ...
    _ =
        case kz_json:get_ne_binary_value(<<"Play-Welcome-Media">>, DiscoveryJObj) of
            'undefined' ->
            ...
            Media ->
                ...
                kapps_call_command:play(
                    kz_media_util:media_path(Media, kapps_call:account_id(Call)), Call
                )
        end,
    maybe_collect_conference_id(Srv, Conference).
...
collect_conference_id(Srv, Conference, Loop) ->
    ...
    case
        kapps_call_command:b_prompt_and_collect_digits(
            1, 16, <<"conf-enter_conf_number">>, 1, Timeout, Call
        )
    of
        {'error', _} -> discovery_failed(Call, Srv);
        {'ok', Digits} -> validate_collected_conference_id(Srv, Conference, Loop, Digits)
    end.

-spec validate_collected_conference_id(
    pid(), kapps_conference:conference(), pos_integer(), binary()
) ->
    'ok'.
...
maybe_set_conference_tones(Conference, JObj) ->
    ShouldPlayOnEntry = kz_json:get_ne_value(
        <<"Play-Entry-Tone">>, JObj, kapps_conference:play_entry_tone(Conference)
    ),
    ShouldPlayOnExit = kz_json:get_ne_value(
        <<"Play-Exit-Tone">>, JObj, kapps_conference:play_exit_tone(Conference)
    ),
    kapps_conference:set_play_entry_tone(
        ShouldPlayOnEntry,
...
maybe_collect_pin(Conference, Call, Srv) ->
    case
        kz_term:is_empty(kapps_conference:moderator_pins(Conference)) andalso
            kz_term:is_empty(kapps_conference:member_pins(Conference))
    of
        'false' ->
...
-spec collect_conference_pin(
    kz_term:api_boolean(), kapps_conference:conference(), kapps_call:call(), pid()
) -> 'ok'.
...
maybe_set_as_moderator(Srv, Conference) ->
    ...
    case
        kz_json:is_true(<<"Moderator">>, JObj, 'false') orelse
            kz_json:is_true([<<"Conference-Doc">>, <<"moderator">>], JObj, 'undefined')
    of
        'undefined' ->
            Conference;
        'true' ->
            lager:debug(
                "discovery request defines participant as moderator, overriding previous value"
            ),
            kapps_conference:set_moderator('true', Conference);
        'false' ->
            lager:debug(
                "discovery request defines participant as member, overriding previous value"
            ),
...
handle_search_error(Conference, Call, Srv) ->
    ...
    try kz_amqp_util:basic_consume(Queue, [{'exclusive', 'true'}]) of
        'ok' ->
            lager:debug("initial participant creating conference on switch nodename '~s'", [
                kapps_call:switch_hostname(Call)
            ]),
        ...
        'exit':{{'shutdown', {'server_initiated_close', 403, <<"ACCESS_REFUSED", _/binary>>}}, _} ->
            lager:debug(
                "conference queue ~s is exclusive, waiting for conference creation by initial participant",
                [Queue]
            ),
...
-spec handle_search_resp(kz_json:object(), kapps_conference:conference(), kapps_call:call(), pid()) ->
    'ok'.
...
maybe_play_name(Conference, Call, Srv) ->
    case
        kapps_conference:play_name_on_join(Conference) andalso
            ?SUPPORT_NAME_ANNOUNCEMENT(kapps_call:account_id(Call))
    of
        'true' ->
            PronouncedName =
...
-spec add_participant_to_conference(
    kz_json:object(), kapps_conference:conference(), kapps_call:call(), pid()
) -> 'ok'.
add_participant_to_conference(JObj, Conference, Call, Srv) ->
    conf_participant:set_conference(Conference, Srv),
    ...
    case kz_json:get_first_defined([<<"Switch-Hostname">>, <<"Media-Server">>], JObj) of
        SwitchHostname ->
            ...
            conf_participant:join_local(Srv);
        _Else ->
            lager:debug("running conference is on a different switch, bridging to ~s: ~p", [
                _Else, JObj
            ]),
...
-spec validate_conference_pin(
    kz_term:api_boolean(), kapps_conference:conference(), kapps_call:call(), pos_integer()
) ->
...
validate_conference_pin('true', Conference, Call, Loop) ->
    ...
    case collect_pin(Conference, Call) of
        {'error', _} = E -> E;
        {'ok', Digits} -> validate_collected_conference_pin(Conference, Call, Loop, Digits)
    end;
validate_conference_pin('false', Conference, Call, Loop) ->
    lager:debug("requesting member pin from caller"),
    case collect_pin(Conference, Call) of
        {'error', _} = E -> E;
        {'ok', Digits} -> validate_collected_member_pins(Conference, Call, Loop, Digits)
    end;
validate_conference_pin(_, Conference, Call, Loop) ->
    lager:debug(
        "requesting conference pin from caller, which will be used to disambiguate member/moderator"
    ),
    case collect_pin(Conference, Call) of
        {'error', _} = E -> E;
        {'ok', Digits} -> validate_if_pin_is_for_moderator(Conference, Call, Loop, Digits)
    end.

collect_pin(Conference, Call) ->
    Timeout = get_pin_timeout(Conference),
    kapps_call_command:b_prompt_and_collect_digits(
        1, 16, <<"conf-enter_conf_pin">>, 1, Timeout, Call
    ).

-spec validate_if_pin_is_for_moderator(
    kapps_conference:conference(), kapps_call:call(), non_neg_integer(), binary()
) ->
    {'ok', kapps_conference:conference()}
    | {'error', any()}.
validate_if_pin_is_for_moderator(Conference, Call, Loop, Digits) ->
    ...
    case
        {
            (lists:member(Digits, MemberPins) orelse
                (MemberPins =:= [] andalso
                    Digits =:= <<>>)),

            (lists:member(Digits, ModeratorPins) orelse
                (MemberPins =:= [] andalso
                    Digits =:= <<>>))
        }
    of
...
-spec validate_collected_member_pins(
    kapps_conference:conference(), kapps_call:call(), non_neg_integer(), binary()
) ->
    {'ok', kapps_conference:conference()}
    | {'error', any()}.
validate_collected_member_pins(Conference, Call, Loop, Digits) ->
    validate_collected_member_pins(
        Conference, Call, Loop, Digits, kapps_conference:member_pins(Conference)
    ).
...
-spec validate_collected_conference_pin(
    kapps_conference:conference(), kapps_call:call(), pos_integer(), binary()
) ->
    {'ok', kapps_conference:conference()}
    | {'error', any()}.
validate_collected_conference_pin(Conference, Call, Loop, Digits) ->
    validate_collected_conference_pin(
        Conference, Call, Loop, Digits, kapps_conference:moderator_pins(Conference)
    ).

-spec validate_collected_conference_pin(
    kapps_conference:conference(), kapps_call:call(), pos_integer(), binary(), kz_term:ne_binaries()
) ->
    {'ok', kapps_conference:conference()}
    | {'error', any()}.
...
get_pin_timeout(Conference) ->
    ...
    lager:debug(
        "finding pin timeout for account '~s' conference '~s'",
...
get_account_pin_timeout(AccountId) ->
    kapps_account_config:get_global(
        AccountId, ?CONFIG_CAT, <<"pin_timeout">>, ?COLLECT_PIN_DEFAULT_TIMEOUT
    ).
...
get_number_timeout(Call) ->
    ...
    kapps_account_config:get_global(
        AccountId, ?CONFIG_CAT, <<"number_timeout">>, ?COLLECT_NUMBER_DEFAULT_TIMEOUT
    ).
