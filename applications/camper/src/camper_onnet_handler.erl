...
handle_cast({'add_request', JObj}, GlobalState) ->
    ...
    NewGlobal = with_state(
        AccountId,
        GlobalState,
        fun(Local) ->
            #state{
                'requests' = dict:merge(
                    fun(_K, _V1, V2) -> V2 end,
                    get_requests(Local),
...
with_state(AccountId, Global, F) ->
    Local =
        case dict:find(AccountId, Global) of
            {'ok', #state{} = S} ->
                S;
            _ ->
                #state{
...
-spec handle_request(kz_term:ne_binary(), {kz_term:ne_binary(), kz_term:ne_binary()}, state()) ->
    state().
...
originate_call({Id, Type}, Exten, AccountDb) ->
    Routines = [
        fun(C) -> kapps_call:set_account_db(AccountDb, C) end,
...
-spec get_endpoints(kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_json:objects().
get_endpoints(Call, EndpointId, Type) when
    Type =:= <<"device">>;
    Type =:= <<"mobile">>
->
...
-spec maybe_update_queues(
    kz_term:ne_binaries(), {kz_term:ne_binary(), kz_term:ne_binary()}, dict:dict()
) -> dict:dict().
maybe_update_queues(SIPNames, Requestor, Queues) ->
    lists:foldl(
        fun(SIPName, Acc) ->
            Q =
                case dict:find(SIPName, Acc) of
                    {'ok', Queue} -> Queue;
                    _ -> queue:new()
                end,
