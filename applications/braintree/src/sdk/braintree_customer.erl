...
get_subscription(PlanId, [
    #bt_subscription{plan_id = PlanId, status = Status} = Subscription
    | Subscriptions
]) ->
...
find(#bt_customer{id = CustomerId}) ->
    find(CustomerId);
find(CustomerId) ->
    Url = url(CustomerId),
    Xml = braintree_request:get(Url),
...
update_card(Customer, Card) ->
    Cards = [
        braintree_card:make_default(Card, 'false')
...
update_subsciption(NewPaymentToken, UpdatedCustomer) ->
    ...
    {[NewCard], OldCards} =
        lists:partition(
            fun(CC) -> braintree_card:payment_token(CC) =:= NewPaymentToken end,
            get_cards(UpdatedCustomer)
        ),

    lists:foreach(
        fun(Sub) -> update_subsciption_with_token(Sub, NewPaymentToken) end,
...
update_subsciption_with_token(Sub, NewPaymentToken) ->
    ShouldUpdate =
        not braintree_subscription:is_canceled(Sub) andalso
            not braintree_subscription:is_expired(Sub),
...
update_subsciption_with_token(Sub, NewPaymentToken, 'true') ->
    ...
    'ok';
update_subsciption_with_token(_Sub, _NewPaymentToken, 'false') ->
    'ok'.
...
delete_old_cards_and_addresses(OldCards, #bt_card{billing_address_id = NewAddressId}) ->
    lists:foreach(
        fun(Card) -> delete_old_stuff(Card, NewAddressId) end,
        OldCards
    ).
...
xml_to_record(Xml, Base) ->
    ...
    #bt_customer{
        ...
        credit_cards = [
            braintree_card:xml_to_record(Card)
         || Card <- xmerl_xpath:string(CreditCardPath, Xml)
        ],
        addresses = [
            braintree_address:xml_to_record(Address)
         || Address <- xmerl_xpath:string(AddressPath, Xml)
        ],
        subscriptions = [
            braintree_subscription:xml_to_record(Subscription)
...
record_to_json(Customer) ->
    kz_json:from_list(
        [
            ...
            {<<"credit_cards">>, [
                braintree_card:record_to_json(Card)
             || Card <- Customer#bt_customer.credit_cards
            ]},
            {<<"addresses">>, [
                braintree_address:record_to_json(Address)
