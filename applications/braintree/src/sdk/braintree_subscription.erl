...
get_addon_quantity(#bt_subscription{add_ons = AddOns}, AddOnId) ->
    case lists:keyfind(AddOnId, #bt_addon.id, AddOns) of
        ...
        #bt_addon{} = AddOn ->
            braintree_addon:get_quantity(AddOn)
    end.
...
-spec update_addon_amount(subscription(), kz_term:ne_binary(), kz_term:api_binary() | number()) ->
    subscription().
update_addon_amount(#bt_subscription{add_ons = AddOns} = Subscription, AddOnId, Amount) ->
    case lists:keyfind(AddOnId, #bt_addon.id, AddOns) of
        'false' ->
            case lists:keyfind(AddOnId, #bt_addon.inherited_from, AddOns) of
                'false' ->
                    braintree_util:error_not_found(<<"Add-On">>);
                #bt_addon{} = AddOn ->
                    AddOn1 = AddOn#bt_addon{amount = kz_term:to_binary(Amount)},
                    Subscription#bt_subscription{
                        add_ons = lists:keyreplace(
                            AddOnId, #bt_addon.inherited_from, AddOns, AddOn1
                        )
                    }
            end;
        #bt_addon{} = AddOn ->
            AddOn1 = AddOn#bt_addon{
                existing_id = AddOnId,
                amount = kz_term:to_binary(Amount)
            },
            Subscription#bt_subscription{
                add_ons = lists:keyreplace(AddOnId, #bt_addon.id, AddOns, AddOn1)
            }
    end.
...
-spec update_discount_amount(subscription(), kz_term:ne_binary(), kz_term:ne_binary() | number()) ->
    subscription().
update_discount_amount(#bt_subscription{discounts = Discounts} = Subscription, DiscountId, Amount) ->
    case lists:keyfind(DiscountId, #bt_discount.id, Discounts) of
        'false' ->
            case lists:keyfind(DiscountId, #bt_discount.inherited_from, Discounts) of
                'false' ->
                    braintree_util:error_not_found(<<"Discount">>);
                #bt_discount{} = Discount ->
                    Discount1 = Discount#bt_discount{amount = kz_term:to_binary(Amount)},
                    Subscription#bt_subscription{
                        discounts = lists:keyreplace(
                            DiscountId, #bt_discount.inherited_from, Discounts, Discount1
                        )
                    }
            end;
        #bt_discount{} = Discount ->
            Discount1 = Discount#bt_discount{
                existing_id = DiscountId,
                amount = kz_term:to_binary(Amount)
            },
            Subscription#bt_subscription{
                discounts = lists:keyreplace(DiscountId, #bt_discount.id, Discounts, Discount1)
            }
    end.
...
reset(Subscription) ->
    lists:foldl(fun(F, S) -> F(S) end, Subscription, [
        fun reset_addons/1,
...
reset_discounts(#bt_subscription{discounts = Discounts} = Subscription) ->
    Subscription#bt_subscription{
        discounts = [Discount#bt_discount{quantity = 0} || Discount <- Discounts]
    }.
...
-spec update_addon_quantity(
    subscription() | kz_term:ne_binary(), kz_term:ne_binary(), integer() | kz_term:api_binary()
) ->
    subscription().
...
-spec increment_addon_quantity(subscription() | kz_term:ne_binary(), kz_term:ne_binary()) ->
    subscription().
increment_addon_quantity(#bt_subscription{add_ons = AddOns} = Subscription, AddOnId) ->
    case lists:keyfind(AddOnId, #bt_addon.id, AddOns) of
        ...
        #bt_addon{quantity = Quantity} = AddOn ->
            AddOn1 = AddOn#bt_addon{
                existing_id = AddOnId, quantity = kz_term:to_integer(Quantity) + 1
            },
            Subscription#bt_subscription{
                add_ons = lists:keyreplace(AddOnId, #bt_addon.id, AddOns, AddOn1)
            }
...
-spec update_discount_quantity(
    subscription() | kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_integer()
) -> subscription().
...
update_discount_quantity(
    #bt_subscription{discounts = Discounts} = Subscription, DiscountId, Quantity
) ->
    case lists:keyfind(DiscountId, #bt_discount.id, Discounts) of
        'false' ->
            case lists:keyfind(DiscountId, #bt_discount.inherited_from, Discounts) of
...
-spec increment_discount_quantity(subscription() | kz_term:ne_binary(), kz_term:ne_binary()) ->
    subscription().
increment_discount_quantity(#bt_subscription{discounts = Discounts} = Subscription, DiscountId) ->
    case lists:keyfind(DiscountId, #bt_discount.id, Discounts) of
        ...
        #bt_discount{quantity = Quantity} = Discount ->
            Discount1 = Discount#bt_discount{
                existing_id = DiscountId, quantity = kz_term:to_integer(Quantity) + 1
            },
            Subscription#bt_subscription{
                discounts = lists:keyreplace(DiscountId, #bt_discount.id, Discounts, Discount1)
            }
...
xml_to_record(Xml, Base) ->
    ...
    #bt_subscription{
        ...
        discounts = [
            braintree_discount:xml_to_record(Discount)
...
record_to_xml(#bt_subscription{} = Subscription, ToString) ->
    ...
    Conditionals = [
        fun
            (#bt_subscription{descriptor = Descriptor}, P) when 'undefined' =/= Descriptor ->
...
record_to_json(Subscription) ->
    kz_json:from_list(
        [
            ...
            {<<"descriptor">>,
                braintree_descriptor:record_to_json(Subscription#bt_subscription.descriptor)},
...
update_options(_, 'undefined', Props) ->
    Props;
