...
%% @end
%%------------------------------------------------------------------------------
-spec update_services_card(kz_term:ne_binary() | kz_services:services(), bt_card()) ->
    {'ok' | 'error', kz_services:services()}.
...
%% @end
%%------------------------------------------------------------------------------
-spec update_services_cards(kz_term:ne_binary() | kz_services:services(), bt_cards()) ->
    {'ok' | 'error', kz_services:services()}.
update_services_cards(CustomerIdOrServices, Cards) ->
    Tokens = [braintree_card:record_to_payment_token(Card) || Card <- Cards],
    save_services(
        kz_services_payment_tokens:updates(CustomerIdOrServices, <<"braintree">>, Tokens)
    ).
...
%% @end
%%------------------------------------------------------------------------------
-spec delete_services_card(
    kz_term:ne_binary() | kz_services:services(), bt_card() | kz_term:ne_binary()
) -> {'ok' | 'error', kz_services:services()}.
    ...
    kz_json:from_list(
        [
            ...
            {<<"gateway_rejection_reason">>,
                BtVerification#bt_verification.gateway_rejection_reason}
        ]
    ).
...
error_to_props({'api_error' = Reason, Error}) ->
    ...
    case is_missing_payment_token(BraintreeErrors) of
        ...
        'false' ->
            Message = kz_json:get_ne_binary_value(
                <<"message">>,
                Details,
...
error_min_amount(Amount) ->
    Error =
        <<"Unable to process a transaction for less than ", ?DOLLAR_SIGN,
            (kz_term:to_binary(Amount))/binary>>,
    lager:debug("~s", [Error]),
...
error_max_amount(Amount) ->
    Error =
        <<"Unable to process a transaction for more than ", ?DOLLAR_SIGN,
            (kz_term:to_binary(Amount))/binary>>,
    lager:debug("~s", [Error]),
