...
-spec find_by_customer(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    bt_transactions().
...
sale(CustomerId, Transaction) ->
    create(Transaction#bt_transaction{
        type = ?BT_TRANS_SALE,
...
-spec quick_sale(kz_term:ne_binary(), number() | kz_term:ne_binary(), kz_term:proplist()) ->
    bt_transaction().
quick_sale(CustomerId, Amount, Props) ->
    case kz_term:to_float(Amount) < ?MIN_AMOUNT of
        'true' ->
            braintree_util:error_min_amount(?MIN_AMOUNT);
        'false' ->
...
xml_to_record(Xml, Base) ->
    ...
    #bt_transaction{
        id = kz_xml:get_value([Base, "/id/text()"], Xml),
        ...
        avs_postal_response = kz_xml:get_value(
            [Base, "/avs-postal-code-response-code/text()"], Xml
        ),
        avs_street_response = kz_xml:get_value(
            [Base, "/avs-street-address-response-code/text()"], Xml
        ),
        ...
        processor_authorization_code = kz_xml:get_value(
            [Base, "/processor-authorization-code/text()"], Xml
        ),
        ...
        add_ons = [
            braintree_addon:xml_to_record(Addon)
         || Addon <- xmerl_xpath:string(AddOnsPath, Xml)
        ],
        discounts = [
            braintree_discount:xml_to_record(Discounts)
...
record_to_xml(#bt_transaction{} = Transaction, ToString) ->
    ...
    Conditionals = [
        ...
        fun
            (#bt_transaction{store_on_success = 'true'}, P) ->
                case props:get_value('options', P) of
                    'undefined' ->
        ...
        fun
            (#bt_transaction{change_billing_address = 'true'}, P) ->
                case props:get_value('options', P) of
                    'undefined' ->
                    ...
                    Options ->
                        [
                            {'options', [
                                {'add-billing-address-to-payment-method', 'true'} | Options
                            ]}
                            | props:delete('options', P)
                        ]
                end;
    ...
    kz_json:from_list(
        [
            ...
            {<<"processor_authorization_code">>,
                Transaction#bt_transaction.processor_authorization_code},
            ...
            {<<"billing_address">>,
                braintree_address:record_to_json(Transaction#bt_transaction.billing_address)},
            {<<"shipping_address_id">>, Transaction#bt_transaction.shipping_address_id},
            {<<"shipping_address">>,
                braintree_address:record_to_json(Transaction#bt_transaction.shipping_address)},
            {<<"customer_id">>, Transaction#bt_transaction.customer_id},
            {<<"customer">>,
                braintree_customer:record_to_json(Transaction#bt_transaction.customer)},
            ...
            {<<"add_ons">>, [
                braintree_addon:record_to_json(Addon)
             || Addon <- Transaction#bt_transaction.add_ons
            ]},
            {<<"discounts">>, [
                braintree_discount:record_to_json(Discounts)
...
json_to_record(JObj) ->
    #bt_transaction{
        ...
        processor_authorization_code = kz_json:get_binary_value(
            <<"processor_authorization_code">>, JObj
        ),
        ...
        billing_address = braintree_address:json_to_record(
            kz_json:get_value(<<"billing_address">>, JObj)
        ),
        shipping_address_id = kz_json:get_binary_value(<<"shipping_address_id">>, JObj),
        shipping_address = braintree_address:json_to_record(
            kz_json:get_value(<<"shipping_address">>, JObj)
        ),
        ...
        add_ons = [
            braintree_addon:json_to_record(Addon)
         || Addon <- kz_json:get_value(<<"add_ons">>, JObj, [])
        ],
        discounts = [
            braintree_discount:json_to_record(Discount)
        ...
        [
            ...
            {<<"Response">>,
                kz_json:get_ne_value(
                    <<"processor_response_text">>, Transaction, <<"Missing Response">>
                )},
