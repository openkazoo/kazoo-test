...
-record(request, {
    activation_charges = [] :: kz_json:objects(),
...
handle_req(JObj, _Props) ->
    ...
    case kz_json:get_value(<<"Bookkeeper-Type">>, JObj) =:= ?APP_NAME of
        ...
        'true' ->
            sync(#request{
                activation_charges = kz_json:get_list_value(
                    [<<"Invoice">>, <<"activation_charges">>], JObj, []
                ),
                request_jobj = JObj,
...
results_reply([Result | Results], HasErrors) ->
    case props:get_value(<<"Status">>, Result) of
        <<"success">> ->
            results_reply(Results, HasErrors);
        <<"error">> ->
            results_reply(Results, 'true');
        <<"fatal">> ->
            [
                {<<"Status">>, <<"fatal">>},
...
sync(
    ...
    lager:debug(
        "fetching bookkeeper document ~s/~s",
    ...
    case kz_datamgr:open_cache_doc(VendorDb, BookkeeperId) of
        ...
        {'error', _Reason} ->
            lager:info(
                "unable to load bookkeeper document ~s/~s: ~p",
...
sync(
    ...
    case
        {
            kz_json:get_ne_binary_value(<<"plan">>, Mapping),
            kz_json:get_ne_binary_value(<<"addon">>, Mapping)
        }
    of
        {'undefined', _} ->
            lager:debug(
                "service item ~s/~s had no plan id",
                [CategoryName, ItemName]
            ),
            sync(Request#request{items = Items});
        {_, 'undefined'} ->
            lager:debug(
                "service item ~s/~s had no add on id",
                [CategoryName, ItemName]
            ),
            sync(Request#request{items = Items});
        {PlanId, AddOnId} ->
...
-spec handle_transaction_sale_response({'ok', kz_transaction:transaction()} | {'error', any()}) ->
    'ok'.
...
create_activation_transaction(#request{
    account_id = AccountId,
...
get_activation_amount(_, Activations, 'undefined') ->
    lists:sum([
        kzd_activation_item:total(ActivationItem)
...
create_activation_item_desc(ActivationItem) ->
    ...
    CatItem = kz_binary:join(
        [
            Bin
         || Bin <- [Category, ItemName],
            Bin =/= <<>>
        ],
        <<$/>>
    ),
...
%% @end
%%------------------------------------------------------------------------------
-spec get_plan_subscription(kz_term:ne_binary(), request()) ->
...
get_plan_subscription(
    ...
    case dict:find(PlanId, Updates) of
        ...
        'error' -> get_plan_subscription(PlanId, Request, Customer)
    end.
...
get_plan_subscription(PlanId, Request, #bt_customer{} = Customer) ->
    try braintree_customer:get_subscription(PlanId, Customer) of
        Subscription ->
            lager:debug(
                "found subscription ~s for plan id ~s",
...
update_addon_discount_amount(
    ...
    case kz_term:is_empty(Rate) of
        'true' ->
            ...
            Update#update{subscription = UpdatedSubscription};
        'false' ->
            UpdatedQuantity = braintree_subscription:update_discount_quantity(
                Subscription, DiscountId, 1
            ),
            UpdatedAmount =
