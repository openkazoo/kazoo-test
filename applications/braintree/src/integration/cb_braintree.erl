...
validate_customer(Context, ?HTTP_POST) ->
    ...
    Customer = braintree_customer:json_to_record(
        lists:foldr(fun(F, J) -> F(J) end, JObj, Generators)
    ),
...
validate_cards(Context, ?HTTP_PUT) ->
    ...
    Card = Card0#bt_card{
        customer_id = cb_context:account_id(Context),
        verify = 'true'
    },
...
-spec validate_transaction(cb_context:context(), path_token(), path_token()) ->
    cb_context:context().
    ...
    try braintree_card:update(cb_context:fetch(Context, 'braintree')) of
        ...
        'throw':{'not_found', _} ->
            cb_context:add_system_error(
                'bad_identifier', kz_json:from_list([{<<"cause">>, CardId}]), Context
            );
        'throw':{Error, Reason} ->
            crossbar_util:response('error', kz_term:to_binary(Error), 500, Reason, Context)
    end;
