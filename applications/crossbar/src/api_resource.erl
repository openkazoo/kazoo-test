...
rest_init(Req, Opts) ->
    ...
    Setters = [
        {fun cb_context:set_req_id/2, get_request_id(Req)},
...
maybe_trace(_Req, 'false') ->
    'ok';
maybe_trace(Req, 'true') ->
    case cowboy_req:header(<<"x-trace-request">>, Req) of
        'undefined' -> 'ok';
        ShouldTrace -> maybe_start_trace(kz_term:to_boolean(ShouldTrace))
    end.

-spec maybe_start_trace(boolean()) -> 'ok'.
maybe_start_trace('false') ->
    'ok';
...
maybe_decode_start_key(Context) ->
    case cb_context:req_value(Context, <<"start_key">>) of
        ...
        Value ->
            ...
            NewReqData =
                case kz_json:is_json_object(ReqData) of
            ...
            Setters = [
                {fun cb_context:set_req_data/2, NewReqData},
...
-spec maybe_allow_proxy_req(kz_term:ne_binary(), kz_term:ne_binary(), boolean()) ->
    kz_term:ne_binary().
...
maybe_allow_proxy_req(Peer, ForwardIP, 'true') ->
    case is_proxied(Peer) of
        ...
        'false' ->
            lager:warning(
                "request with \"X-Forwarded-For: ~s\" header, but peer (~s) is not allowed as proxy",
                [ForwardIP, Peer]
            ),
            Peer
    end.
...
is_proxied(Peer, [Proxy | Rest]) ->
    kz_network_utils:verify_cidr(Peer, kz_network_utils:to_cidr(Proxy)) orelse
        is_proxied(Peer, Rest).
...
rest_terminate(Req, Context, ?HTTP_OPTIONS) ->
    lager:info(
        "OPTIONS request fulfilled in ~p ms",
...
rest_terminate(Req, Context, Verb) ->
    ...
    lager:info(
        "~s request fulfilled in ~p ms ~s mem ~s bin",
        [
            Verb,
            kz_time:elapsed_ms(cb_context:start(Context)),
...
find_allowed_methods(Req, Context) ->
    ...
    Method = cowboy_req:method(Req),
    AllowMethods = api_util:allow_methods(
        Responses,
...
maybe_allow_method(Req, Context) ->
    maybe_allow_method(
        Req, Context, cb_context:allow_methods(Context), cb_context:req_verb(Context)
    ).
...
maybe_allow_method(Req, Context, Methods, Verb) ->
    case lists:member(Verb, Methods) of
        ...
        'false' -> api_util:stop(Req, cb_context:add_system_error('invalid_method', Context))
    end.
...
malformed_request(Req, Context, _ReqVerb) ->
    case props:get_value(<<"accounts">>, cb_context:req_nouns(Context)) of
        ...
        [<<>> | _] ->
            ...
            api_util:stop(
                Req, cb_context:add_system_error(404, <<"bad identifier">>, Error, Context)
            );
        [_Other | _] ->
...
forbidden(Req0, Context0) ->
    case api_util:is_permitted(Req0, Context0) of
        ...
        {IsPermitted, Req1, Context1} -> {not IsPermitted, Req1, Context1}
    end.
...
-spec content_handlers_to_cowboy_providers(crossbar_content_handler(), content_type_callbacks()) ->
...
default_content_types_accepted(Req, Context) ->
    CTA = [
        {?CROSSBAR_DEFAULT_CONTENT_TYPE, Fun}
     || {Fun, ContentTypes} <- cb_context:content_types_accepted(Context),
        lists:any(
            fun
                ({Type, SubType}) ->
                    api_util:content_type_matches(
                        ?CROSSBAR_DEFAULT_CONTENT_TYPE,
                        {Type, SubType, '*'}
                    );
                ({_, _, _} = ModCT) ->
                    api_util:content_type_matches(?CROSSBAR_DEFAULT_CONTENT_TYPE, ModCT)
            end,
            % check each type against the default
            ContentTypes
        )
    ],
...
-spec content_types_accepted(
    kz_term:ne_binary(), cowboy_req:req(), cb_context:context(), crossbar_content_handlers()
) ->
    {content_type_callbacks(), cowboy_req:req(), cb_context:context()}.
content_types_accepted(ClientCT, Req, Context, []) ->
    lager:debug("endpoint(s) specify no accepted content-types, using defaults"),
    content_types_accepted(
        ClientCT, Req, cb_context:set_content_types_accepted(Context, ?CONTENT_ACCEPTED)
    );
...
-spec content_types_accepted_fold(
    crossbar_content_handler(), content_type_callbacks(), kz_term:ne_binary()
) ->
    content_type_callbacks().
content_types_accepted_fold({Fun, ContentTypes}, Acc, ClientCT) ->
    lists:foldr(
        fun(ContentType, Acc1) ->
...
-spec content_type_accepted_fold(
    content_type(), content_type_callbacks(), content_conversion_fun(), kz_term:ne_binary()
) ->
    content_type_callbacks().
...
does_request_validate(Req, Context0) ->
    ...
    case api_util:succeeded(Context2) of
        ...
        'false' ->
            ...
            Msg =
                case {cb_context:resp_error_msg(Context2), cb_context:resp_data(Context2)} of
                    {'undefined', 'undefined'} ->
                        <<"validation failed">>;
                    {'undefined', Data} ->
...
create_from_response(Req, Context, Accept) ->
    ...
    case to_fun(Context, Accept, DefaultFun) of
        'to_json' ->
            api_util:create_push_response(Req, Context);
        'send_file' ->
            api_util:create_push_response(Req, Context, fun api_util:create_resp_file/2);
        'to_binary' ->
            api_util:create_push_response(Req, Context, fun api_util:create_binary_resp_content/2);
        'to_xml' ->
            api_util:create_push_response(Req, Context, fun api_util:create_xml_resp_content/2);
        _Else ->
...
to_json(Req0, Context0, 'undefined') ->
    case cb_context:fetch(Context0, 'is_chunked') of
        'true' ->
            to_chunk(<<"to_json">>, Req0, Context0);
        _ ->
...
to_binary(Req, Context, 'undefined') ->
    ...
    case
        kz_term:is_ne_binary(RespData) andalso
            cb_context:req_header(Context, <<"range">>)
    of
        'false' ->
        ...
        RangeHeader ->
            ...
            Setters = [
                ...
                {fun cb_context:add_resp_headers/2, #{
                    <<"content-range">> => kz_term:to_binary(
                        io_lib:fwrite("bytes ~B-~B/~B", [Start, End, FileLength])
                    ),
                    <<"accept-ranges">> => <<"bytes">>
                }}
            ],
            ...
            Req1 = cowboy_req:reply(
                kz_term:to_binary(ErrorCode), cb_context:resp_headers(NewContext), Content, Req
            ),
...
-type range_response() :: {
    kz_term:ne_binary(), pos_integer(), pos_integer(), pos_integer(), pos_integer()
}.
...
get_range(Data, RangeHeader) ->
    ...
    {Start, End} =
        case cow_http_hd:parse_range(RangeHeader) of
            {'bytes', [{S, 'infinity'}]} ->
                {S, FileLength};
...
to_fun(Context, Major, Minor, Default) ->
    case
        [
            F
         || {F, CTPs} <- cb_context:content_types_provided(Context),
...
-spec accept_matches_provided(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
    boolean().
accept_matches_provided(Major, Minor, CTPs) ->
    lists:any(
        fun({Pri, Sec}) ->
    ...
    case cb_context:fetch(Context0, 'is_chunked') of
        'true' ->
            to_chunk(
                <<"to_csv">>,
                Req0,
                cb_context:add_resp_header(Context0, <<"content-type">>, <<"text/csv">>)
            );
        _ ->
...
to_pdf(Req, Context, RespData) ->
    ...
    {RespData, api_util:set_resp_headers(Req, cb_context:add_resp_headers(Context, RespHeaders)),
        Context}.
...
next_chunk_fold(
    ...
    finish_chunked_response(ChunkMap#{
        context => reset_context_between_chunks(Context, StartedChunk)
    });
next_chunk_fold(
    #{
        chunking_started := StartedChunk,
...
next_chunk_results(
    ...
    case api_util:succeeded(Context1) of
        ...
        'false' ->
            ...
            finish_chunked_response(ChunkMap#{
                context => reset_context_between_chunks(Context1, StartedChunk)
            })
    end.
...
process_chunk(
    ...
    case cb_context:resp_data(Context) of
        0 ->
            ...
            next_chunk_fold(ChunkMap#{
                context => reset_context_between_chunks(Context, IsStarted),
                chunking_started => IsStarted
            });
        SentLength when is_integer(SentLength) ->
            lager:debug("(chunked) ~s sent ~p data", [ToFun, SentLength]),
            next_chunk_fold(ChunkMap#{
                context => reset_context_between_chunks(Context, 'true'),
        ...
        [] ->
            ...
            next_chunk_fold(ChunkMap#{
                context => reset_context_between_chunks(Context, IsStarted),
                chunking_started => IsStarted,
                %% the module filtered all queried result
                previous_chunk_length => 0
            });
...
reset_context_between_chunks(Context, _StartedChunk, 'false') ->
    RespData = cb_context:resp_data(Context),
    case {kz_json:is_json_object(RespData), kz_term:is_ne_binary(RespData)} of
        {'true', _} ->
...
%% @end
%%------------------------------------------------------------------------------
-spec finish_chunked_response(map()) ->
    {iolist() | kz_term:ne_binary() | 'stop', cowboy_req:req(), cb_context:context()}.
