...
%% @end
%%------------------------------------------------------------------------------
-spec build_load_params(cb_context:context(), kz_term:ne_binary(), options()) ->
...
ranged_start_end_keys(Context, Options, Direction, StartTime, EndTime) ->
    ...
    of
        {'undefined', 'undefined'} when Direction =:= 'ascending' ->
        ...
        {'undefined', 'undefined'} when Direction =:= 'descending' ->
            {EndKeyMap(EndTime), StartKeyMap(StartTime)};
        {StartKey, 'undefined'} when Direction =:= 'descending' ->
            {StartKey, StartKeyMap(StartTime)};
...
-spec expand_min_max_keys2(api_range_key(), direction(), kz_term:api_non_neg_integer()) ->
    api_range_key().
...
expand_min_max_keys2(RangeKey, Direction, _) ->
    expand_min_max_key(RangeKey, Direction).
...
expand_min_max_key('min_max', 'descending') ->
    ...
    <<16#fff0/utf8>>;
expand_min_max_key(RangeKey, _) ->
    RangeKey.

-spec maybe_min_max_pad(kz_term:api_non_neg_integer(), api_range_key()) ->
    api_range_key() | ['min_max'].
maybe_min_max_pad('undefined', RangeKey) ->
    RangeKey;
...
direction(Context, Options) ->
    case
        props:get_value('descending', Options, 'false') orelse
            kz_json:is_true(<<"descending">>, cb_context:query_string(Context)) orelse
            kz_json:is_false(<<"ascending">>, cb_context:query_string(Context), 'true')
    of
        'true' -> 'descending';
        'false' -> 'ascending'
    end.
...
time_range(Context, Options) ->
    time_range(
        Context, Options, props:get_ne_binary_value('range_key_name', Options, <<"created">>)
    ).

-spec time_range(cb_context:context(), options(), kz_term:ne_binary()) ->
...
%%------------------------------------------------------------------------------
-spec time_range(
    cb_context:context(), pos_integer(), kz_term:ne_binary(), pos_integer(), pos_integer()
) ->
    time_range() | cb_context:context().
time_range(Context, MaxRange, Key, RangeFrom, RangeTo) ->
    ...
    case RangeTo - RangeFrom of
        N when N < 0 ->
            Msg = kz_term:to_binary(
                io_lib:format("~s_to ~b is prior to ~s ~b", [Key, RangeTo, Path, RangeFrom])
            ),
        ...
        N when N > MaxRange ->
            Msg = kz_term:to_binary(
                io_lib:format("~s_to ~b is more than ~b seconds from ~s ~b", [
                    Key, RangeTo, MaxRange, Path, RangeFrom
                ])
            ),
...
get_page_size(Context, Options) ->
    case
        props:is_true('should_paginate', Options, 'true') andalso
...
next_chunk(
    ...
    ChunkMap#{
        total_queried => TotalQueried + PrevLength,
...
next_chunk(
    ...
    is_integer(PageSize) andalso
        PageSize > 0 andalso
        TotalQueried + PrevLength =:= PageSize andalso
        LastKey =/= 'undefined'
->
...
next_chunk(
    #{
        options := #{last_key := OldLastKey} = LoadMap,
        last_key := LastKey,
        total_queried := TotalQueried,
        previous_chunk_length := PrevLength,
        context := Context
...
next_chunk(
    #{
        options := #{databases := [_ | RestDbs], last_key := LastKey} = LoadMap,
        total_queried := TotalQueried,
        previous_chunk_length := PrevLength,
        context := Context
    } = ChunkMap
    ...
    case cb_context:fetch(Context, 'load_view_opts') of
        ...
        #{} = LoadMap ->
            chunk_map_roll_in(
                ChunkMap,
                get_results(LoadMap#{
                    context => cb_context:store(Context, 'load_view_opts', 'undefined'),
                    previous_chunk_length => 0
                })
            )
    end.
...
chunk_map_roll_in(
    ...
    ChunkMap#{
        ...
        options => maps:remove(context, LoadMap#{last_key => OldLastKey})
    }.
...
get_results(
    ...
    lager:debug(
        "querying view '~s' from '~s', starting at '~p' with page size ~p and limit ~p in direction ~s",
...
handle_query_results(LoadMap, JObjs, LimitWithLast, MemoryUsed, MemoryLimit) when
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_query_result(
    load_params(), kz_term:ne_binaries(), kz_json:objects(), kz_term:api_pos_integer()
) -> load_params().
handle_query_result(
    #{
        last_key := LastKey,
    ...
    case apply_filter(Mapper, JObjs) of
        {'error', Reason} ->
            LoadMap#{
                context => cb_context:add_system_error(
                    'datastore_fault', kz_term:to_binary(Reason), Context
                )
            };
        FilteredJObjs when is_list(FilteredJObjs) ->
            FilteredLength = length(FilteredJObjs),
            lager:debug(
                "db_returned: ~b(~p) passed_filter: ~p next_start_key: ~p",
...
-spec handle_query_result(
    load_params(), kz_term:ne_binaries(), kz_json:objects(), non_neg_integer(), last_key()
) -> load_params().
handle_query_result(
    #{
        is_chunked := 'true',
...
handle_query_result(
    ...
    case check_page_size_and_length(LoadMap, FilteredLength, FilteredJObjs, NewLastKey) of
        ...
        {'next_db', LoadMap2} ->
            get_results(LoadMap2#{databases => RestDbs})
    end.
...
check_page_size_and_length(
    ...
    {'next_db', LoadMap#{
        total_queried => TotalQueried + Length,
...
check_page_size_and_length(
    ...
    is_integer(PageSize) andalso
        PageSize > 0 andalso
        TotalQueried + Length == PageSize andalso
        LastKey =/= 'undefined'
->
...
limit_with_last_key('true', PageSize, ChunkSize, TotalQueried) when
    ChunkSize < (PageSize - TotalQueried)
->
    1 + ChunkSize;
...
%%------------------------------------------------------------------------------
-spec last_key(
    last_key(), kz_json:objects(), non_neg_integer() | 'undefined', non_neg_integer(), page_size()
) ->
...
-spec add_paging(api_range_key(), non_neg_integer(), api_range_key(), kz_json:object()) ->
    kz_json:object().
...
%% @end
%%------------------------------------------------------------------------------
-spec build_general_load_params(cb_context:context(), kz_term:ne_binary(), options()) ->
    load_params() | cb_context:context().
...
is_chunked(Context, Options) ->
    kz_json:is_true(
        ...
        not props:get_is_true('unchunkable', Options, 'false').
        ...
        Size ->
            try kz_term:to_integer(Size) of
                ...
                ChunkSize when ChunkSize > SystemSize ->
                    throw(
                        {'error',
                            <<"chunk size must be lower than ",
                                (integer_to_binary(SystemSize))/binary>>}
                    )
            catch
                _:_ ->
...
maybe_set_start_end_keys(LoadMap, 'undefined', 'undefined') ->
    LoadMap;
maybe_set_start_end_keys(LoadMap, StartKey, 'undefined') ->
    LoadMap#{start_key => StartKey};
maybe_set_start_end_keys(LoadMap, 'undefined', EndKey) ->
    LoadMap#{end_key => EndKey};
maybe_set_start_end_keys(LoadMap, StartKey, EndKey) ->
    LoadMap#{start_key => StartKey, end_key => EndKey}.

-spec get_page_size_from_request(cb_context:context()) -> pos_integer().
get_page_size_from_request(Context) ->
    case cb_context:req_value(Context, <<"page_size">>) of
        'undefined' ->
            cb_context:pagination_page_size();
        Size ->
            try kz_term:to_integer(Size) of
                PageSize when PageSize > 0 -> PageSize;
                _ -> throw({'error', <<"page size must be at least 1">>})
            catch
                _:_ ->
...
%% @end
%%------------------------------------------------------------------------------
-spec get_time_key(cb_context:context(), kz_term:ne_binary(), options(), pos_integer()) ->
    pos_integer().
get_time_key(Context, Key, Options, Default) ->
    case props:get_integer_value(Key, Options) of
        'undefined' ->
            case
                kz_term:safe_cast(
                    cb_context:req_value(Context, Key), Default, fun kz_term:to_integer/1
                )
            of
                T when T > 0 -> T;
                _ -> Default
...
get_start_end_keys(Context, Options) ->
    case props:get_value('keymap', Options) of
        'undefined' ->
            {
                map_keymap(
                    Context, Options, props:get_first_defined(['startkey', 'start_keymap'], Options)
                ),
                map_keymap(
                    Context, Options, props:get_first_defined(['endkey', 'end_keymap'], Options)
                )
            };
        KeyMap ->
            {map_keymap(Context, Options, KeyMap), map_keymap(Context, Options, KeyMap)}
    end.
...
get_range_key_maps(Options) ->
    case props:get_value('range_keymap', Options) of
        ...
        KeyMap ->
            {map_range_keymap(KeyMap), map_range_keymap(KeyMap)}
    end.
