...
-define(DEFAULT_AUTH_EXPIRY,
    kapps_config:get_integer(?AUTH_CONFIG_CAT, <<"token_auth_expiry_s">>, ?SECONDS_IN_HOUR)
).
-define(SHOULD_LOG_FAILED,
    kapps_config:get_is_true(?AUTH_CONFIG_CAT, <<"log_failed_attempts">>, 'true')
).
-define(SHOULD_LOG_SUCCESS,
    kapps_config:get_is_true(?AUTH_CONFIG_CAT, <<"log_successful_attempts">>, 'true')
).
...
create_auth_token(Context, AuthModule) ->
    ...
    AccountId = kz_json:get_first_defined(
        [<<"account_id">>, [<<"Claims">>, <<"account_id">>]], JObj
    ),
...
create_auth_token(Context, Method, JObj) ->
    ...
    case
        is_auth_module_enabled(Method, AuthConfig) andalso
            maybe_create_token(Context, Claims, AuthConfig, Method, IsMultiFactor)
    of
        'false' ->
        ...
        {'ok', Token} ->
            ...
            log_success_auth(
                Method,
                <<"jwt_auth_token">>,
                <<"authentication resulted in token creation">>,
                Context,
                AccountId,
                AuthConfig
            ),
        ...
        {'error', Reason, RespJObj} ->
            lager:debug(
                "authentication factor module requested that the client should preform second-factor authentication, returning ~s with response ~p",
...
-spec maybe_create_token(
    cb_context:context(), kz_term:proplist(), kz_json:object(), kz_term:ne_binary(), boolean()
) ->
...
maybe_create_token(Context, Claims, AuthConfig, Method, 'true') ->
    ...
    MultiFactorOpts = kz_json:get_json_value(
        method_config_path(Method, <<"multi_factor">>), AuthConfig
    ),
...
get_account_config(AccountId) ->
    kapps_account_config:get_hierarchy(
        AccountId, ?AUTH_CONFIG_CAT, <<"auth_modules">>, ?DEFAULT_AUTH_CONFIG
    ).
...
is_multi_factor_enabled(Claims, AuthConfig) ->
    ...
    MFAAccountId = kz_json:get_ne_binary_value(
        method_mfa_path(Method, <<"account_id">>), AuthConfig
    ),
    IncludeSubAccounts = kz_json:is_true(
        method_mfa_path(Method, <<"include_subaccounts">>), AuthConfig
    ),

    kz_json:is_true(method_mfa_path(Method, <<"enabled">>), AuthConfig) andalso
        multi_factor_allowed_for_account(
            MasterId, ClaimAccountId, MFAAccountId, IncludeSubAccounts
        ).
...
%% @end
%%------------------------------------------------------------------------------
-spec multi_factor_allowed_for_account(
    kz_term:api_binary(), kz_term:api_binary(), kz_term:api_binary(), boolean()
) -> boolean().
multi_factor_allowed_for_account(?NE_BINARY = Master, ?NE_BINARY = Master, _, _) ->
    'true';
multi_factor_allowed_for_account(_Master, _ClaimAccountId, 'undefined', _) ->
    'true';
multi_factor_allowed_for_account(_Master, AccountId, AccountId, _) ->
    'true';
multi_factor_allowed_for_account(_Master, _ClaimAccountId, _ParentAccount, IncludeSubAcc) ->
    IncludeSubAcc.
...
%% @end
%%------------------------------------------------------------------------------
-spec log_success_auth(
    atom() | kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), cb_context:context()
) -> 'ok'.
...
log_success_auth(AuthModule, AuthType, Reason, Context, AccountId, 'undefined') ->
    log_success_auth(
        AuthModule, AuthType, Reason, Context, AccountId, get_account_config(AccountId)
    );
log_success_auth(AuthModule, AuthType, Reason, Context, AccountId, AuthConfig) ->
    ...
    case is_log_type_enabled(<<"success">>, Method, AuthConfig) of
        'false' -> 'ok';
        'true' -> log_attempts(Context, AccountId, Method, <<"success">>, AuthType, Reason)
    end.
...
log_failed_auth(AuthModule, AuthType, Reason, Context, AccountId, AuthConfig) ->
    ...
    case is_log_type_enabled(<<"failed">>, Method, AuthConfig) of
        'false' -> 'ok';
        'true' -> log_attempts(Context, AccountId, Method, <<"failed">>, AuthType, Reason)
    end.
...
log_attempts(Context, AccountId, Method, Status, AuthType, Reason) ->
    ...
    Doc = kz_doc:update_pvt_parameters(Doc0, MODB, [
        {'type', <<"login_attempt">>},
        {'now', Now}
    ]),
