...
% mapping over path tokens in URI
-type payload() ::
    path_tokens()
    | [cb_context:context() | path_token() | 'undefined', ...]
    | cb_context:context()
    % v1_resource:rest_init/2
    | {cb_context:context(), kz_term:proplist()}
    % v1_util:execute_request/2
    | {'error', _}
    | {kz_json:path(), cb_context:context(), path_tokens()}
    % v1_resource:expires/2
    | {kz_time:datetime(), cowboy_req:req(), cb_context:context()}
    % mapping over the request/context records
...
matches([R | Restrictions], Tokens) ->
    ...
    kazoo_bindings:matches(Restriction, Tokens) orelse
        matches(Restrictions, Tokens).
