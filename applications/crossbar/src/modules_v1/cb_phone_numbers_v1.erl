...
-define(FIND_NUMBER_SCHEMA,
    "{\"$schema\": \"http://json-schema.org/draft-03/schema#\", \"id\": \"http://json-schema.org/draft-03/schema#\", \"properties\": {\"prefix\": {\"required\": \"true\", \"type\": \"string\", \"minLength\": 3, \"maxLength\": 10}, \"quantity\": {\"default\": 1, \"type\": \"integer\", \"minimum\": 1}}}"
).
...
init() ->
    _ = crossbar_bindings:bind(
        <<"v1_resource.content_types_accepted.phone_numbers">>, ?MODULE, 'content_types_accepted'
    ),
    _ = crossbar_bindings:bind(<<"v1_resource.authenticate">>, ?MODULE, 'authenticate'),
    _ = crossbar_bindings:bind(<<"v1_resource.authorize">>, ?MODULE, 'authorize'),
    _ = crossbar_bindings:bind(
        <<"v1_resource.allowed_methods.phone_numbers">>, ?MODULE, 'allowed_methods'
    ),
    _ = crossbar_bindings:bind(
        <<"v1_resource.resource_exists.phone_numbers">>, ?MODULE, 'resource_exists'
    ),
...
-spec validate_3(cb_context:context(), http_method(), path_token(), path_token()) ->
    cb_context:context().
...
summary(Context) ->
    Context1 = crossbar_doc:load_view(
        ?CB_LIST, [], rename_qs_filters(Context), fun normalize_view_results/2
    ),
...
set_response(CollectionJObjOrUnkown, Context) ->
    case kz_json:is_json_object(CollectionJObjOrUnkown) of
        'true' ->
            crossbar_util:response(CollectionJObjOrUnkown, Context);
        'false' ->
...
-spec collection_process(cb_context:context(), kz_term:ne_binary() | kz_term:ne_binaries()) ->
    kz_json:object().
collection_process(Context, ?ACTIVATE) ->
    Numbers = kz_json:get_value(<<"numbers">>, cb_context:req_data(Context), []),
    collection_process(Context, Numbers, ?ACTIVATE);
collection_process(Context, Numbers) ->
    Temp = kz_json:from_list([
        {<<"success">>, kz_json:new()},
        {<<"error">>, kz_json:new()}
    ]),
    lists:foldl(
...
-spec collection_process(
    cb_context:context(), kz_term:ne_binary() | kz_term:ne_binaries(), kz_term:ne_binary()
) -> kz_json:object().
collection_process(Context, Numbers, Action) ->
    Base = kz_json:from_list([
        {<<"success">>, kz_json:new()},
        {<<"error">>, kz_json:new()}
    ]),
    lists:foldl(
        fun(Number, Acc) -> collection_process_action_fold(Number, Acc, Context, Action) end,
        Base,
        Numbers
    ).

-spec collection_process_action_fold(
    kz_term:ne_binary(), kz_json:object(), cb_context:context(), kz_term:ne_binary()
) ->
    kz_json:object().
...
-spec collection_action(cb_context:context(), http_method(), kz_term:ne_binary()) ->
    knm_number_return().
...
-spec collection_action(
    cb_context:context(), http_method(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    knm_number_return()
    | {'ok', kz_json:object()}.
