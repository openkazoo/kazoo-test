...
init() ->
    _ = crossbar_bindings:bind(
        <<"v1_resource.allowed_methods.devices">>, ?MODULE, 'allowed_methods'
    ),
    _ = crossbar_bindings:bind(
        <<"v1_resource.resource_exists.devices">>, ?MODULE, 'resource_exists'
    ),
    ...
    JObj =
        case cb_context:req_value(Context, <<"outbound_flags">>) of
            ...
            Flags when is_list(Flags) ->
                ...
                kz_json:set_value(
                    <<"outbound_flags">>, OutboundFlags, cb_context:req_data(Context)
                );
            _Else ->
...
prepare_device_realm(DeviceId, Context) ->
    ...
    case AccountRealm =:= Realm of
        ...
        'false' ->
            validate_device_creds(
                Realm, DeviceId, cb_context:store(Context, 'aggregate_device', 'true')
            )
    end.
...
validate_device_ip_unique(IP, DeviceId, Context) ->
    case cb_devices_utils:is_ip_unique(IP, DeviceId) of
        'true' ->
            check_emergency_caller_id(
                DeviceId, cb_context:store(Context, 'aggregate_device', 'true')
            );
        'false' ->
            C = cb_context:add_validation_error(
                [<<"sip">>, <<"ip">>],
...
extract_device_registrations([JObj | JObjs], Set) ->
    S = lists:foldl(
        fun extract_device_registration/2,
        Set,
...
-spec maybe_aggregate_device(kz_term:api_binary(), cb_context:context(), crossbar_status()) ->
    boolean().
...
-spec maybe_remove_aggregate(kz_term:api_binary(), cb_context:context(), crossbar_status()) ->
    boolean().
maybe_remove_aggregate('undefined', _Context, _RespStatus) ->
    'false';
