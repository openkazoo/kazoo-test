...
maybe_dry_run(Context, CurrentJObj, ProposedJObj) ->
    ...
    lager:debug(
        "verifying billing services requirements for account ~s",
        [AccountId]
    ),
    AuthAccountId = cb_context:auth_account_id(Context),
    Services = kz_services:fetch(AuthAccountId),
    Updated = kz_services:set_updates(
        Services,
        AccountId,
...
should_dry_run(Context) ->
    cb_context:accepting_charges(Context) =/= 'true' andalso
        cb_context:api_version(Context) =/= ?VERSION_1.

-spec check_creditably(
    cb_context:context(),
    kz_services:services(),
    kz_services_invoices:invoices(),
    boolean() | number()
) ->
    cb_context:context().
...
check_creditably(Context, Services, Quotes, 'true') ->
    ...
    Activations = [
        kz_services_activation_items:calculate_total(
...
-spec update_subscriptions(
    cb_context:context(), kz_services_invoices:jobjs(), kz_services_invoices:jobjs()
) -> 'ok'.
update_subscriptions(Context, CurrentJObj, ProposedJObj) ->
    ...
    case kz_datamgr:open_cache_doc(AccountDb, UserId) of
        ...
        {'ok', JObj} ->
            [
                ...
                {<<"full_name">>,
                    kzd_users:full_name(JObj, kzd_users:username(JObj, kzd_users:email(JObj)))}
            ]
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec transaction_to_error(cb_context:context(), kz_transaction:transaction()) ->
    cb_context:context().
...
-spec transaction_to_error(
    cb_context:context(), kz_transaction:transaction(), kz_term:api_ne_binary()
) -> cb_context:context().
transaction_to_error(Context, Transaction, 'undefined') ->
    cb_context:add_system_error(
        500, 'unspecified_fault', kz_transaction:public_json(Transaction), Context
    );
