...
init() ->
    Bindings = [
        {<<"v2_resource.allowed_methods.devices">>, 'allowed_methods'},
...
validate_device_id(Context, DeviceId) ->
    case kz_datamgr:open_cache_doc(cb_context:account_db(Context), DeviceId) of
        ...
        {'error', 'not_found'} -> error_no_entity(Context, DeviceId);
...
put_action(Context, DeviceId, <<"notify">>) ->
    ...
    Req = props:filter_undefined(
        [
            ...
            {<<"Content-Type">>,
                cb_context:req_value(Context, [<<"data">>, <<"body">>, <<"content_type">>])},
...
prepare_mac_address(_DeviceId, Context) ->
    case get_mac_address(Context) of
        'undefined' ->
            Context;
        MacAddress ->
            JObj = kzd_devices:set_mac_address(
...
prepare_outbound_flags(_DeviceId, Context) ->
    JObj =
        case cb_context:req_value(Context, ?KEY_OUTBOUND_FLAGS) of
            'undefined' ->
                cb_context:req_data(Context);
            [] ->
                cb_context:req_data(Context);
...
%% @end
%%------------------------------------------------------------------------------
-spec prepare_provisioner_fields(kz_term:api_binary(), cb_context:context()) ->
    cb_context:context().
prepare_provisioner_fields(_DeviceId, Context) ->
    ...
    JObj =
        case cb_context:req_value(Context, <<"provision">>) of
            'undefined' ->
                ReqData;
            Provision ->
...
%% @end
%%------------------------------------------------------------------------------
-spec prepare_emergency_caller_id(kz_term:api_binary(), cb_context:context()) ->
    cb_context:context().
...
-spec check_mdn_changed(kz_term:ne_binary(), kz_term:api_binary(), cb_context:context()) ->
    cb_context:context().
...
error_mdn_changed(Context) ->
    ...
    lager:debug(
        "mobile device number attempted to be changed from ~p to ~p",
        [_OldMDN, NewMDN]
    ),
    Msg = kz_json:from_list(
        [
...
-spec check_mdn_taken(kz_term:ne_binary(), kz_term:api_binary(), cb_context:context()) ->
    cb_context:context().
check_mdn_taken(MDN, DeviceId, Context) ->
...
unique_mac_address('undefined', _Context) ->
    'true';
unique_mac_address(MacAddress, Context) ->
    DbName = cb_context:account_db(Context),
    not lists:member(MacAddress, get_mac_addresses(DbName)) andalso
        not provisioner_util:is_mac_address_in_use(Context, MacAddress).
...
-spec check_device_password(kz_term:ne_binary(), kz_term:api_binary(), cb_context:context()) ->
    cb_context:context().
check_device_password(Realm, DeviceId, Context) ->
...
-spec check_sip_creds_unique(
    kz_term:api_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_binary()
) ->
    boolean().
%% no account id and no doc id (ie initial create with no account)
check_sip_creds_unique('undefined', _, _, 'undefined') ->
    'true';
check_sip_creds_unique(AccountDb, Realm, Username, DeviceId) ->
    is_creds_locally_unique(AccountDb, Username, DeviceId) andalso
        is_creds_global_unique(Realm, Username, DeviceId).

-spec is_creds_locally_unique(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    boolean().
...
-spec is_creds_global_unique(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    boolean().
is_creds_global_unique(Realm, Username, DeviceId) ->
    ViewOptions = [
        {'key', [
...
maybe_aggregate_device(DeviceId, Context) ->
    case
        kz_term:is_true(cb_context:fetch(Context, 'aggregate_device')) andalso
            ?DEVICES_ALLOW_AGGREGATES
    of
...
maybe_add_mdn(_DeviceId, Context) ->
    ...
    case
        get_device_type(Context) =:= <<"mobile">> andalso
            kz_term:is_not_empty(MDN) andalso
            (is_new_device(Context) orelse
                was_mdn_changed_and_user_is_super_admin(Context, MDN))
    of
...
-spec was_mdn_changed_and_user_is_super_admin(cb_context:context(), kz_term:ne_binary()) ->
    boolean().
was_mdn_changed_and_user_is_super_admin(Context, NewMDN) ->
    %% Check if MDN has been changed.
    kz_json:get_ne_value(?KEY_MDN, cb_context:fetch(Context, 'db_doc')) =/= NewMDN andalso
...
add_mdn(MDN, Context) ->
    ...
    case knm_number:create(MDN, Options) of
        ...
        {'ok', _} ->
            lager:debug(
                "created new mdn ~s with public fields set to ~s",
...
maybe_remove_previous_mdn(_DeviceId, Context) ->
    ...
    case
        get_device_type(Context) =:= <<"mobile">> andalso
            PreviousMDN =/= CurrentMDN andalso
            kz_term:is_not_empty(PreviousMDN)
    of
...
maybe_remove_aggregate(DeviceId, Context) -> remove_aggregate(DeviceId, Context).
    ...
    case knm_number:get(MDN, knm_number_options:mdn_options()) of
        {'ok', Number} ->
            ...
            case kz_json:get_ne_value(<<"mobile">>, knm_number:to_public_json(Number)) of
                ...
                Mobile ->
                    lager:debug(
                        "hard removing old mdn ~s with mobile properties ~s",
