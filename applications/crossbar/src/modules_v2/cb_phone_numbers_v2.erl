...
init() ->
    ...
    _ = crossbar_bindings:bind(
        <<"v2_resource.allowed_methods.phone_numbers">>, ?MODULE, 'allowed_methods'
    ),
    _ = crossbar_bindings:bind(
        <<"v2_resource.resource_exists.phone_numbers">>, ?MODULE, 'resource_exists'
    ),
...
-spec classified_number(cb_context:context(), path_token(), kz_term:api_binary()) ->
    cb_context:context().
...
put(Context, ?COLLECTION, ?ACTIVATE) ->
    ...
    CB = fun() ->
        ?MODULE:put(cb_context:set_accepting_charges(Context), ?COLLECTION, ?ACTIVATE)
    end,
    set_response(Results, Context, CB);
...
delete(Context, Number) ->
    ...
    case knm_number:Releaser(Number, Options) of
        {'error', Data} = Error ->
            case
                kz_json:is_json_object(Data) andalso
...
maybe_find_port_number(Context, Number, 'true') ->
    ...
    case kz_datamgr:get_single_result(?KZ_PORT_REQUESTS_DB, ?PORT_NUM_LISTING, ViewOption) of
        ...
        {'ok', Result} ->
            ...
            Values = [
                {[<<"_read_only">>, <<"port_id">>], kz_json:get_value(<<"port_id">>, Port)},
...
-spec port_number_summary(kz_json:object(), cb_context:context(), boolean()) ->
    cb_context:context().
...
normalize_port_number(JObj, Num, AuthBy) ->
    knm_phone_number:setters(
        knm_phone_number:from_number_with_options(Num, [{'auth_by', AuthBy}]),
        [
...
view_account_phone_numbers(Context) ->
    Context1 = crossbar_doc:load_view(
        ?CB_LIST, [], rename_qs_filters(Context), fun normalize_view_results/3
    ),
    case cb_context:resp_status(Context1) of
        'success' ->
...
maybe_add_port_request_numbers(_Context, 'false') ->
    kz_json:new();
...
-spec pick_account_and_reseller_id(cb_context:context()) ->
    {'ok', kz_term:ne_binary(), kz_term:api_ne_binary()}
...
-spec find_numbers(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
find_numbers(Context, AccountId, ResellerId) ->
...
-spec maybe_reseller_id_lookup(kz_term:ne_binary()) ->
    {'ok', kz_term:ne_binary(), kz_term:ne_binary()}
    | {'error', kz_term:ne_binary()}.
maybe_reseller_id_lookup(ReqResellerId) ->
    try
        ?UNAUTHORIZED_NUMBERS_LOOKUP(ReqResellerId) andalso
            (kz_services_reseller:is_reseller(ReqResellerId) orelse
                cb_context:is_superduper_admin(ReqResellerId))
    of
...
validate_collection_request(Context, Numbers) when
    is_list(Numbers)
->
...
%% @end
%%------------------------------------------------------------------------------
-spec get_prefix(kz_term:ne_binary()) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
get_prefix(City) ->
    case kapps_config:get_string(?PHONE_NUMBERS_CONFIG_CAT, ?KEY_PHONEBOOK_FREE_URL) of
        ...
        Url ->
            Country = kapps_config:get_string(
                ?PHONE_NUMBERS_CONFIG_CAT,
                <<"default_country">>,
                ?KNM_DEFAULT_COUNTRY
            ),
...
maybe_update_locality(Context) ->
    ...
    ToUpdate =
        [
            Num
         || {Num, NumProps} <- kz_json:to_proplist(Numbers),
            kz_json:get_value(<<"locality">>, NumProps) =:= 'undefined' andalso
...
update_context_locality(Context, Localities) ->
    JObj = kz_json:foldl(
        fun update_context_locality_fold/3, cb_context:resp_data(Context), Localities
    ),
    cb_context:set_resp_data(Context, JObj).

-spec update_context_locality_fold(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    kz_json:object().
...
update_phone_numbers_locality(Context, Localities) ->
    ...
    case kz_datamgr:open_doc(AccountDb, DocId) of
        {'ok', JObj} ->
            J = kz_json:foldl(
                fun(K, V, J) -> update_phone_numbers_locality_fold(K, V, J, Context) end,
                JObj,
                Localities
            ),
...
-spec update_phone_numbers_locality_fold(
    kz_term:ne_binary(), kz_json:object(), kz_json:object(), cb_context:context()
) ->
    kz_json:object().
...
set_response(Ret = #{'quotes' := Quotes, 'options' := Options}, Context, CB) ->
    case {knm_number_options:dry_run(Options), kz_term:is_empty(Quotes)} of
        {'true', 'false'} ->
...
set_response({'error', 'not_found'}, Context, _) ->
    reply_number_not_found(Context);
set_response({'error', {Cause, AccountId}}, Context, _) when
...
set_response({'error', Data}, Context, _) when
    is_atom(Data)
->
...
%% @end
%%------------------------------------------------------------------------------
-spec collection_process(cb_context:context(), kz_term:ne_binary() | http_method()) ->
    knm_numbers:ret().
...
-spec numbers_action(
    cb_context:context(), kz_term:ne_binary() | http_method(), kz_term:ne_binaries()
) -> knm_numbers:ret().
...
-spec pick_release_or_delete(cb_context:context(), knm_number_options:options()) ->
    'release' | 'delete'.
pick_release_or_delete(Context, Options) ->
    AuthBy = knm_number_options:auth_by(Options),
    Pick =
        case
            kz_term:is_true(cb_context:req_param(Context, <<"hard">>, 'false')) andalso
