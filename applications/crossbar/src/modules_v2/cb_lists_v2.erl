...
migrate(AccountDb, [List | Lists]) ->
    ...
    Entries = kz_json:foldl(
        fun(Id, EntryJObj, Acc) ->
...
migrate_old_entry_docs(AccountDb, ListJObj) ->
    case kz_datamgr:get_results(AccountDb, ?ENTRIES_VIEW, ['include_docs']) of
        {'ok', Entries} ->
            Updated = [
                migrate_old_entry_doc(AccountDb, ListJObj, kz_json:get_json_value(<<"doc">>, Entry))
             || Entry <- Entries
            ],
...
%% @end
-spec migrate_old_entry_doc(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    kz_json:object().
migrate_old_entry_doc(AccountDb, ListJObj, EntryJObj) ->
    case
        kz_json:get_first_defined(
            [<<"capture_group_key">>, <<"cid_key">>, <<"list_entry_old_id">>], EntryJObj
        )
    of
        'undefined' ->
            lager:debug("unable to find entry id for ~s inside list ~p", [
                kz_doc:id(EntryJObj), kz_doc:id(ListJObj)
            ]),
            EntryJObj;
        EntryId ->
...
-spec migrate_to_entry_doc(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), kz_json:object()
) ->
    kz_json:object().
migrate_to_entry_doc(AccountDb, EntryId, EntryJObj, ListJObj) ->
    CaptureGroupKey = kz_json:get_first_defined(
        [<<"capture_group_key">>, <<"cid_key">>, <<"list_entry_old_id">>],
        EntryJObj,
        EntryId
    ),
    CaptureGroupLength = kz_json:get_first_defined(
        [<<"capture_group_length">>, <<"length">>],
        ListJObj
    ),

    CleanedUpEntry = kz_json:delete_keys(
        [<<"cid_name">>, <<"cid_number">>, <<"length">>, <<"list_entry_old_id">>],
        EntryJObj
    ),
...
content_types_provided(Context, _, ?ENTRIES, _, ?VCARD) ->
    cb_context:set_content_types_provided(
        Context,
        [
            {'to_binary', [
                {<<"text">>, <<"x-vcard">>},
...
validate_req(?HTTP_PATCH, Context, [ListId] = Path) ->
    crossbar_doc:patch_and_validate(ListId, Context, fun(_Id, C) ->
        validate_req(?HTTP_POST, C, Path)
    end);
...
validate_req(?HTTP_PATCH, Context, [_ListId, ?ENTRIES, EntryId] = Path) ->
    crossbar_doc:patch_and_validate(EntryId, Context, fun(_Id, C) ->
        validate_req(?HTTP_POST, C, Path)
    end);
...
-spec validate_doc(kz_term:api_binary(), kz_term:ne_binary(), cb_context:context()) ->
    cb_context:context().
...
-spec save(cb_context:context(), path_token(), path_token(), path_token(), path_token()) ->
    cb_context:context().
...
-spec on_successfull_validation(kz_term:api_binary(), kz_term:ne_binary(), cb_context:context()) ->
    cb_context:context().
...
-spec set_org(kz_json:object(), cb_context:context(), crossbar_status(), kz_term:ne_binary()) ->
    kz_json:object().
...
set_org(JObj, _, _, ListId) ->
    lager:debug(
        "failed to load list ~p while loading list entry ~p",
...
set_photo(JObj, Context) ->
    ...
    Attach = crossbar_doc:load_attachment(
        DocId, ?PHOTO, ?TYPE_CHECK_OPTION(?TYPE_LIST_ENTRY), Context
    ),
