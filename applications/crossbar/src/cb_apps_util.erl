...
log_allowed_apps([AppJObj | AppJObjs]) ->
    _ =
        case kzd_app:is_published(AppJObj) of
            ...
            'false' ->
                lager:debug(
                    "disallowing access to ~s due to ~s",
                    [
                        kzd_app:name(AppJObj),
                        kz_json:get_value(<<"authority">>, AppJObj, <<"unknown">>)
                    ]
...
%% @end
%%------------------------------------------------------------------------------
-spec allowed_service_plan(kz_term:ne_binary(), kz_term:api_ne_binary(), kz_json:objects()) ->
    kz_json:objects().
...
ensure_allowed_service_apps_fold(MasterAccountId) ->
    fun(AppId, ServicesAppJObj, AppJObjs) ->
        ...
        case
            VendorId =/= MasterAccountId andalso
                Enabled
        of
...
-spec maybe_append_app_doc(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:objects()) ->
    kz_json:objects().
maybe_append_app_doc(VendorId, AppId, AppJObjs) ->
    case kzd_app:fetch(VendorId, AppId) of
        {'ok', AppJObj} ->
            lager:debug(
                "including app ~s from ~s",
...
%% @end
%%------------------------------------------------------------------------------
-spec allowed_apps_store_doc(kz_term:ne_binary(), kz_term:api_binary(), kz_json:objects()) ->
    kz_json:objects().
allowed_apps_store_doc(AccountId, UserId, AppJObjs) ->
    case get_apps_store_doc(AccountId) of
        {'error', _R} ->
            lager:error(
                "failed to fetch apps store doc in ~s : ~p",
                [AccountId, _R]
            ),
            AppJObjs;
        {'ok', AppStoreJObj} ->
...
%% @end
%%------------------------------------------------------------------------------
-spec allowed_whitelabel_doc(kz_term:ne_binary(), kz_term:api_binary(), kz_json:objects()) ->
    kz_json:objects().
allowed_whitelabel_doc(AccountId, _UserId, AppJObjs) ->
    ...
    lists:map(
        fun(AppJObj) ->
            case
                kzd_app:name(AppJObj) =:= <<"port">> andalso
                    (not IsReseller) andalso
                    kzd_whitelabel:hide_port(WhitelabelJObj)
            of
                'false' ->
                    AppJObj;
                'true' ->
...
get_whitelabel_doc(AccountId) ->
    case kzd_whitelabel:fetch(AccountId) of
        ...
        {'error', _R} ->
            lager:error(
                "failed to load whitelabel doc for ~s: ~p",
                [AccountId, _R]
            ),
            kzd_whitelabel:new()
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec allowed_master_account(kz_term:ne_binary(), kz_term:api_binary(), kz_json:objects()) ->
    kz_json:objects().
...
maybe_set_account(Account, Doc) ->
    ...
    case
        kz_doc:account_db(JObj) =/= AccountDb orelse
            kz_doc:account_id(JObj) =/= AccountId
    of
        'false' -> JObj;
...
get_services_apps(AccountId) ->
    ...
    case
        kz_term:is_not_empty(ServicesApps) orelse
    ...
    of
        ...
        'false' ->
            ...
            lager:debug(
                "account ~s doesn't have apps in service plan, checking reseller ~s",
