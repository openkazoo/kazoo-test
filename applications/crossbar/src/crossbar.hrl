...
-record(cb_context, {
    ...
    %% English by default
    languages_provided = [<<"en">>, <<"en-us">>, <<"en-gb">>] :: kz_term:ne_binaries(),
    %% all charsets provided
    charsets_provided = [<<"iso-8859-1">>] :: kz_term:ne_binaries(),
    %% gzip and identity
    encodings_provided = [<<"gzip;q=1.0">>, <<"identity;q=0.5">>] :: kz_term:ne_binaries(),
    ...
    % see ?ALLOWED_METHODS
    req_verb = ?HTTP_GET :: http_method(),
    % {module, [id]} most typical
    req_nouns = [{<<"404">>, []}] :: req_nouns(),
    req_json = kz_json:new() :: req_json(),
    req_files = [] :: req_files(),
    % the "data" from the request JSON envelope
    req_data = kz_json:new() :: kz_json:json_term(),
    ...
    % Will be loaded in validate stage for endpoints such as /accounts/{acct-id}/users/{user-id}/*
    user_id :: kz_term:api_ne_binary(),
    % Will be loaded in validate stage for endpoints such as /accounts/{acct-id}/devices/{device-id}/*
    device_id :: kz_term:api_ne_binary(),
    ...
    resp_file = <<>> :: binary(),
    resp_data :: resp_data(),
    %% allow the modules to set headers (like Location: XXX to get a 201 response code)
    resp_headers = #{<<"content-type">> => <<"application/json">>} :: cowboy:http_headers(),
