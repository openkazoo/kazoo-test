...
-export([
    ...
    add_doc_validation_errors/2,
    update_successfully_validated_request/2,
    ...
    account_doc/1,
    auth_token_type/1,
    set_auth_token_type/2,
    auth_token/1,
    set_auth_token/2,
    auth_doc/1,
    set_auth_doc/2,
    auth_account_id/1,
    set_auth_account_id/2,
    auth_account_doc/1,
    auth_user_id/1,
    req_verb/1,
    set_req_verb/2,
    ...
    should_paginate/1,
    set_should_paginate/2,
    pagination_page_size/0, pagination_page_size/1,

    req_json/1,
    set_req_json/2,
    resp_error_code/1,
    set_resp_error_code/2,
    resp_error_msg/1,
    set_resp_error_msg/2,
    ...
    set_content_types_accepted/2,
    content_types_provided/1,
    set_content_types_provided/2,
    languages_provided/1,
    set_languages_provided/2,
    encodings_provided/1,
    set_encodings_provided/2,

    validation_errors/1,
    set_validation_errors/2,
...
account_doc(#cb_context{account_id = 'undefined'}) ->
    'undefined';
...
auth_account_doc(#cb_context{auth_account_id = 'undefined'}) ->
    'undefined';
...
auth_user_id(#cb_context{auth_doc = 'undefined'}) ->
    'undefined';
...
should_paginate(#cb_context{should_paginate = Should}) ->
    Should.
...
validators(#cb_context{} = Context, [_ | _] = Setters) -> validators_fold(Context, Setters).

-spec validators_fold(context(), setters()) -> context().
validators_fold(Context, []) ->
    Context;
...
-spec set_account_modb(
    context(), kz_time:year() | kz_term:ne_binary(), kz_time:month() | kz_term:ne_binary()
) -> context().
...
update_doc(#cb_context{doc = Doc} = Context, Updaters) when
    is_list(Updaters)
->
...
-spec add_content_types_provided(
    context(), crossbar_content_handler() | crossbar_content_handlers()
) ->
    context().
...
-spec add_content_types_accepted(
    context(), crossbar_content_handler() | crossbar_content_handlers()
) ->
    context().
...
has_errors(#cb_context{
    validation_errors = JObj,
    resp_status = 'success'
}) ->
...
response(#cb_context{
    ...
    ErrorCode =
        try
            kz_term:to_integer(Code)
        catch
            _:_ -> 500
        end,
    ErrorMsg =
        case kz_term:is_empty(Msg) of
            'false' -> kz_term:to_binary(Msg);
            'true' -> <<"generic_error">>
        end,
    ErrorData =
        case {kz_term:is_empty(ValidationJObj), kz_term:is_empty(DataJObj)} of
...
-spec validate_request_data(
    kz_term:ne_binary() | kz_term:api_object(), context(), after_fun(), after_fun()
) ->
    context().
...
-spec validate_request_data(
    kz_term:ne_binary() | kz_term:api_object(), context(), after_fun(), after_fun(), boolean()
) ->
    context().
...
failed_error(Error, Context) ->
    ...
    Context#cb_context{
        validation_errors = kz_json:merge_jobjs(ErrorJObj, JObj),
        resp_error_code = ErrorCode,
        resp_data = kz_json:new(),
...
passed(Context) ->
    Context1 =
        case 'error' =:= resp_status(Context) of
            'true' -> Context;
...
add_system_error('datastore_missing', Context) ->
    build_system_error(
        503, 'datastore_missing', <<"data collection missing: database not found">>, Context
    );
...
add_system_error('empty_tree_accounts_exist', Context) ->
    build_system_error(
        400, 'empty_tree_accounts_exist', <<"unable to create account tree">>, Context
    );
...
-spec add_system_error(atom() | binary(), kz_term:ne_binary() | kz_json:object(), context()) ->
    context().
...
add_system_error('invalid_bulk_type' = Error, JObj, Context) ->
    ...
    Message =
        <<"bulk operations do not support documents of type ", (kz_term:to_binary(Type))/binary>>,
...
-spec add_system_error(
    integer(), atom() | kz_term:ne_binary(), kz_term:ne_binary() | kz_json:object(), context()
) -> context().
add_system_error(Code, Error, JObj, Context) ->
    build_system_error(Code, Error, JObj, Context).

-spec build_system_error(
    integer(), atom() | kz_term:ne_binary(), kz_term:ne_binary() | kz_json:object(), context()
) -> context().
...
%% @end
%%------------------------------------------------------------------------------
-spec add_validation_error(
    kz_json:get_key(), kz_term:ne_binary(), kz_term:ne_binary() | kz_json:object(), context()
) ->
    context().
...
add_validation_error(Property, Code, Message, Context) ->
    {ErrorCode, ErrorMessage, ErrorJObj} =
        kz_json_schema:validation_error(
            Property,
            Code,
            Message,
    ...
    Context#cb_context{
        validation_errors = kz_json:merge(ErrorJObj, ErrorsJObj),
        resp_status = 'error',
        resp_error_code = ErrorCode,
        resp_data = kz_json:new(),
...
-spec maybe_fix_js_types(
    kz_json:object(), context(), after_fun(), after_fun(), validation_errors()
) -> context().
maybe_fix_js_types(SchemaJObj, Context, OnSuccess, OnFailure, Errors) ->
...
add_doc_validation_errors(Context, ValidationErrors) ->
    lists:foldl(
        fun({Path, Reason, Msg}, C) -> add_validation_error(Path, Reason, Msg, C) end,
        Context,
        ValidationErrors
    ).
