...
init() ->
    ...
    _ = crossbar_bindings:bind(
        <<"*.content_types_provided.port_requests">>, ?MODULE, 'content_types_provided'
    ),
    _ = crossbar_bindings:bind(
        <<"*.content_types_accepted.port_requests">>, ?MODULE, 'content_types_accepted'
    ),
...
authorize(Context, [{<<"port_requests">>, []}], ?HTTP_GET) ->
    ...
    case
        cb_context:fetch(C1, 'is_port_authority', 'false') orelse
            cb_context:is_superduper_admin(C1)
    of
        'true' -> {'true', C1};
        'false' -> {'stop', cb_context:add_system_error('forbidden', C1)}
    end;
...
-spec content_types_provided_get(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
...
validate(Context, Id) ->
    ...
    Setters = [
        {fun cb_context:store/3, 'req_comments', Comments},
...
validate_port_request(Context, ?PATH_TOKEN_LAST_SUBMITTED, ?HTTP_GET) ->
    C1 = cb_context:set_req_data(
        Context, kz_json:from_list([{<<"by_types">>, ?PATH_TOKEN_LAST_SUBMITTED}])
    ),
...
validate(Context, Id, ?PORT_ATTACHMENT, AttachmentId) ->
    validate_attachment(
        set_port_authority(Context), Id, AttachmentId, cb_context:req_verb(Context)
    ).

-spec validate_attachment(
    cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary(), http_method()
) ->
    cb_context:context().
...
post(Context, Id, ?PORT_ATTACHMENT, AttachmentId) ->
    ...
    _ =
        case kz_doc:attachment(cb_context:doc(Context), AttachmentId) of
            'undefined' ->
                lager:debug("no attachment named ~s", [AttachmentId]);
            _AttachmentMeta ->
...
-spec save_and_send_comments(cb_context:context(), kz_term:ne_binary(), kz_json:objects()) ->
    cb_context:context().
        ...
        {'ok', _} ->
            ...
            case cb_context:resp_status(Context1) of
                'success' ->
                    ...
                    RespData = filter_private_comments(
                        Context1, knm_port_request:public_fields(Doc)
                    ),
...
-spec save_then_maybe_notify(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
...
-spec date_as_configured_timezone(kz_term:ne_binary(), kz_term:api_ne_binary()) ->
    kz_time:gregorian_seconds().
date_as_configured_timezone(
    <<YYYY:4/binary, $-, MM:2/binary, $-, DD:2/binary, $\s, HH:2/binary, $:, Mm:2/binary>>,
    FromTimezone
) ->
...
handle_phonebook_error(
    ...
    Setters = [
        ...
        {
            fun cb_context:set_validation_errors/2,
            kz_json:get_value(<<"data">>, Payload, kz_json:new())
        },
...
handle_phonebook_error(Context, #{payload := Payload}) ->
    cb_context:add_system_error(
        'datastore_fault', kz_json:get_value(<<"message">>, Payload), Context
    ).
...
maybe_reject_port(Context, #{
    ...
    Reason =
        <<"There was some issue with your port request. Please wait while we investigate the issue.">>,
    ...
    Metadata = knm_port_request:transition_metadata(
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            ...
            DocSetters = [
                {
                    fun kzd_port_requests:set_comments/2,
                    kzd_port_requests:comments(Doc, []) ++ [Comment]
                },
                {fun kzd_port_requests:set_pvt_last_phonebook_error/2, PhonebookPayload}
            ],
            CtxSetters = [
                {fun cb_context:set_doc/2, kz_doc:setters(Doc, DocSetters)},
...
handle_phonebook_response(Context, Response) ->
    ...
    Merged = kz_json:merge([
        kz_json:delete_key(<<"comments">>, Doc),
...
fix_phonebook_comments(Comment) ->
    Setters = [
        {fun kzd_comment:set_action_required/2, kzd_comment:action_required(Comment, 'false')},
...
-spec patch_then_validate_then_maybe_transition(
    cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    cb_context:context().
...
%% @end
%%------------------------------------------------------------------------------
-spec validate_port_comments(cb_context:context(), fun(
...
-spec validate_port_comments(
    cb_context:context(), fun((cb_context:context()) -> cb_context:context()), kz_json:ojbects()
) -> cb_context:context().
validate_port_comments(Context, OnSuccess, []) ->
    Doc = cb_context:doc(Context),
...
validate_comments(Context, NewComments) ->
    IsPortAuthority =
        cb_context:fetch(Context, 'is_port_authority', 'false') orelse
            cb_context:is_superduper_admin(Context),
    ContextToValidate = cb_context:set_req_data(
        Context, kzd_port_requests:set_comments(kz_json:new(), NewComments)
    ),
...
validate_comments(Context, NewComments, 'false') ->
    Filters = [
        {kzd_comment:action_required_path(), fun kz_term:is_true/1},
        %% old key
        {kzd_comment:superduper_comment_path(), fun kz_term:is_true/1},
            ...
            Msg = kz_json:from_list_recursive(
                [
                    {<<"comments">>, [
                        {<<"forbidden">>, [
                            {<<"message">>,
                                <<"you're not allowed to make private comment or set action_required">>},
                            {<<"cause">>, <<"comments">>}
                        ]}
                    ]}
                ]
    ...
    of
        ...
        [_ | _] ->
            Msg = kz_json:from_list(
                [
                    {<<"message">>,
                        <<"setting action_required on private comment is not allowed">>},
                    {<<"cause">>, <<"comments">>}
                ]
            ),
    ...
    Setters = [
        {
            ...
            kzd_port_requests:set_comments(Doc, cb_comments:sort(DbComments ++ Updated))
        },
...
update_comment(Context, Comment) ->
    Setters = [
        {fun kzd_comment:set_is_private/2, kzd_comment:is_private_legacy(Comment)}
...
is_deletable(Context, ?PORT_UNCONFIRMED) ->
    Context;
is_deletable(Context, ?PORT_REJECTED) ->
    Context;
is_deletable(Context, ?PORT_CANCELED) ->
    Context;
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            ...
            cb_context:set_resp_data(
                cb_context:set_doc(Context1, UseDoc),
                UseDoc
            );
        _ ->
            Context1
...
summarize_by_types(Context, Types) when is_list(Types) ->
    case
        lists:partition(
            fun(Type) -> lists:member(Type, ?PORT_STATES) end,
    ...
    of
        ...
        {_, Unsatisfying} ->
            Message =
                <<"query string by_types contained invalid values: ",
...
load_summary_by_types(Context, Types) ->
    lager:debug(
        "loading summary for port requests with types ~s",
        [kz_binary:join(Types)]
    ),
    load_summary_by_types(Context, Types, []).

-spec load_summary_by_types(cb_context:context(), kz_term:ne_binaries(), kz_json:objects()) ->
    cb_context:context().
...
load_summary_by_type(Context, Type) ->
    lager:debug(
        "loading summary for port requests with type ~s",
        [Type]
    ),
...
-spec by_types_view_options(cb_context:context(), kz_term:ne_binary(), boolean(), authority_type()) ->
...
normalize_number_summarize_results(Context, JObj, Acc) ->
    ...
    case
        lists:member(AccountId, kzd_accounts:tree(kz_json:get_value(<<"doc">>, JObj))) orelse
            cb_context:is_superduper_admin(Context)
    of
...
-spec normalize_last_submitted(cb_context:context(), kz_json:object(), kz_json:objects()) ->
    kz_json:objects().
...
prepare_timeline(Context, Doc) ->
    ...
    Indexed = [
        {kz_json:get_integer_value(?TRANSITION_TIMESTAMP, JObj), JObj}
...
filter_private_comments(Context, JObj) ->
    ...
    case
        kzd_port_requests:pvt_port_authority(JObj) =:= cb_context:auth_account_id(Context) orelse
            cb_context:fetch(Context, 'is_port_authority') orelse
            cb_context:is_superduper_admin(Context)
    of
        'false' ->
...
-spec run_comment_filter([kzd_comment:doc()], [
    {kz_json:get_key(), fun((kz_json:json_term()) -> boolean())}
]) ->
    [kzd_comment:doc()].
...
on_successful_validation(Context, Id) ->
    Context1 = crossbar_doc:load_merge(
        Id,
...
successful_validation(Context, 'undefined') ->
    PortReq = knm_port_request:new(
        ...
        [
            ...
            {'port_authority_name',
                kzd_accounts:fetch_name(cb_context:fetch(Context, 'port_authority_id'))},
...
check_number_portability(PortId, Number, Context, E164, [PortReq]) ->
    case
        {
            kz_doc:account_id(PortReq) =:= cb_context:account_id(Context),
...
check_number_portability(_PortId, Number, Context, E164, [_ | _]) ->
    lager:debug(
        "number ~s(~s) exists on multiple port request docs. That's bad!",
        [E164, Number]
    ),
...
load_attachment(AttachmentId, Context) ->
    Context1 = crossbar_doc:load_attachment(
        cb_context:doc(Context),
        AttachmentId,
        ?TYPE_CHECK_OPTION(?TYPE_PORT_REQUEST),
        cb_context:set_account_db(Context, ?KZ_PORT_REQUESTS_DB)
...
-spec move_state(cb_context:context(), kz_term:ne_binary(), knm_port_request:transition_metadata()) ->
    cb_context:context().
move_state(Context, PortState, Metadata) ->
...
find_template(ResellerId, CarrierName) ->
    ...
    lager:debug(
        "looking for carrier template ~s or plain template for reseller ~s",
...
%% @end
%%------------------------------------------------------------------------------
-spec port_state_change_notify(cb_context:context(), path_token(), path_token()) ->
    cb_context:context().
port_state_change_notify(Context, Id, State) ->
    Req = props:filter_undefined(
        [
            ...
            {<<"Port-Request-ID">>, Id},
            {<<"Reason">>,
                state_change_reason(Context, cb_context:req_value(Context, ?REQ_TRANSITION))},
    ...
    try
        ...
        _E:_R ->
            lager:debug(
                "failed to send the  port ~s notification, maybe reverting the state change: ~s:~p",
                [State, _E, _R]
            ),
...
get_new_comments(Context) ->
    ...
    case
        cb_comments:sort(
            kz_json:get_list_value(<<"comments">>, cb_context:fetch(Context, 'db_doc'), [])
        )
    of
        [] ->
            ReqComments;
...
-spec send_port_comment_notifications(cb_context:context(), kz_term:ne_binary(), kz_json:objects()) ->
    'ok'.
send_port_comment_notifications(Context, Id, NewComments) ->
    _ = lists:foldl(
        fun send_port_comment_notification/2, {Context, Id, length(NewComments), 1}, NewComments
    ),
    'ok'.

-spec send_port_comment_notification(kz_json:object(), {
...
send_port_comment_notification(NewComment, {Context, Id, TotalNew, Index}) ->
    Setters = [
        {
            fun kzd_comment:set_user_id/2,
            kzd_comment:user_id(NewComment, cb_context:auth_user_id(Context))
        },
        {
            fun kzd_comment:set_account_id/2,
            kzd_comment:account_id(NewComment, cb_context:auth_account_id(Context))
        }
    ],
    ...
    try kapps_notify_publisher:cast(Req, fun kapi_notifications:publish_port_comment/1) of
        ...
        _E:_R ->
            lager:error("failed to send the port comment notification ~b/~b: ~s:~p", [
                Index, TotalNew, _E, _R
            ]),
            {Context, Id, TotalNew, Index + 1}
    end.
    ...
    TemplateData =
        props:filter_undefined(
            [
                ...
                {<<"numbers">>, [
                    knm_util:pretty_print(N)
                 || N <- kz_json:get_keys(<<"numbers">>, PortRequest)
                ]},
                {<<"bill">>,
                    kz_json:to_proplist(kz_json:get_value(<<"bill">>, PortRequest, kz_json:new()))},
...
-spec create_QR_code(kz_term:api_binary(), kz_term:api_binary()) ->
    kz_term:proplist() | 'undefined'.
create_QR_code('undefined', _) ->
    'undefined';
create_QR_code(_, 'undefined') ->
    'undefined';
...
-spec set_port_authority(cb_context:context(), authority_type(), kz_term:api_ne_binary()) ->
    cb_context:context().
set_port_authority(Context, Type, 'undefined') ->
    ...
    AuthAccountId = cb_context:auth_account_id(Context),
    Setters = [
        {fun cb_context:store/3, 'port_authority_id', MasterId},
...
set_port_authority(Context, Type, PortAuthority) ->
    AuthAccountId = cb_context:auth_account_id(Context),
    Setters = [
        {fun cb_context:store/3, 'port_authority_id', PortAuthority},
