...
validate(Context, ?SUMMARY) ->
    ...
    cb_context:setters(
        ...
        [
            ...
            {fun cb_context:set_resp_data/2, kz_services:summary(cb_context:account_id(Context))}
        ]
    );
...
%% @end
%%------------------------------------------------------------------------------
-spec audit_summary(cb_context:context(), kz_term:ne_binary(), crossbar_view:options()) ->
    cb_context:context().
...
maybe_add_max_range(Context, Options) ->
    case
        kz_term:safe_cast(
            cb_context:req_value(Context, <<"created_to">>), 'undefined', fun kz_term:to_integer/1
        )
    of
        T when
            is_integer(T) andalso
                T > 0
        ->
...
-spec validate_service_plan(cb_context:context(), path_token(), http_method()) ->
    cb_context:context().
...
load_audit_logs(Context) ->
    Options = [
        {'mapper', fun normalize_audit_view_results/2},
        {'range_start_keymap', []},
...
-spec override_plans(cb_context:context(), kz_services:services(), kz_term:api_json()) ->
...
maybe_merge_overrides(Context) ->
    case
        kz_json:is_true(<<"merge">>, cb_context:req_json(Context), 'false') orelse
            cb_context:req_verb(Context) =:= ?HTTP_PATCH
    of
        'false' -> [];
        'true' -> ['merge']
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec manual_quantities(cb_context:context(), kz_services:services(), kz_term:api_json()) ->
    kz_services:services().
...
assign_plans(Context, Services) ->
    Additions = kz_json:get_first_defined(
        [
            <<"add">>,
            <<"plans">>
        ],
...
unassign_plans(Context, Services) ->
    ...
    lists:foldl(
        fun(PlanId, S) -> kz_services_plan:unassign(S, PlanId) end,
        Services,
...
is_allowed(Context) ->
    ...
    case
        AuthAccountId =:= ResellerId orelse
...
-spec get_plan_ids(kz_term:ne_binaries() | kz_json:objects(), kz_term:ne_binaries()) ->
    kz_term:ne_binaries().
get_plan_ids([], Ids) ->
    Ids;
get_plan_ids([Item | Items], Ids) ->
    case kz_json:is_json_object(Item) of
        'false' ->
            get_plan_ids(Items, maybe_add_plan_id(Item, Ids));
        'true' ->
            Id = kz_doc:id(Item),
...
-spec check_plan_ids(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    cb_context:context().
...
check_plan_id(Context, PlanId, ResellerId) ->
    ...
    crossbar_doc:load(
        PlanId,
        cb_context:set_account_db(Context, ResellerDb),
        ?TYPE_CHECK_OPTION(kzd_service_plan:type())
    ).
...
maybe_forbid_delete(Context) ->
    case kz_json:get_value(<<"delete">>, cb_context:req_data(Context), []) of
        [] -> Context;
        DeletePlansIds -> maybe_forbid_delete(DeletePlansIds, Context)
    end.
