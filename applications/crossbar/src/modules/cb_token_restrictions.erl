...
init() ->
    ...
    _ = crossbar_bindings:bind(
        <<"*.allowed_methods.token_restrictions">>, ?MODULE, 'allowed_methods'
    ),
    _ = crossbar_bindings:bind(
        <<"*.resource_exists.token_restrictions">>, ?MODULE, 'resource_exists'
    ),
...
on_successful_validation(Context) ->
    Context1 = crossbar_doc:load_merge(
        ?CB_ACCOUNT_TOKEN_RESTRICTIONS, Context, ?TYPE_CHECK_OPTION(?PVT_TYPE)
    ),
...
add_doc_id(Context) ->
    cb_context:set_doc(
        Context, kz_doc:set_id(cb_context:doc(Context), ?CB_ACCOUNT_TOKEN_RESTRICTIONS)
    ).
...
maybe_deny_access(Context) ->
    case get_auth_restrictions(cb_context:auth_doc(Context)) of
        'undefined' -> 'false';
        Restrictions -> maybe_deny_access(Context, Restrictions)
    end.
...
get_auth_restrictions(AuthDoc) ->
    ...
    lager:debug("checking for restrictions for ~s in ~s using ~s method", [
        OwnerId, AccountId, AuthModule
    ]),
...
find_endpoint_restrictions_by_account(_Accounts, []) ->
    'undefined';
find_endpoint_restrictions_by_account(
    AllowedAccounts,
    [Restriction | Restrictions]
) ->
    case
        maybe_match_accounts(
            AllowedAccounts,
...
allowed_accounts(Context) ->
    ...
    case cb_context:account_id(Context) of
        ...
        AccountId -> allowed_accounts(AuthAccountId, AccountId)
    end.
...
allowed_accounts('undefined', _AccountId) ->
    [?CATCH_ALL];
allowed_accounts(_AuthAccountId, 'undefined') ->
    [?CATCH_ALL];
...
match_argument_patterns(ReqParams, RulesJObj, RuleKeys) ->
    case match_rules(ReqParams, RuleKeys) of
        'undefined' -> [];
        MatchedRuleKey -> kz_json:get_list_value(MatchedRuleKey, RulesJObj, [])
    end.
...
match_verb(_Context, [?CATCH_ALL]) ->
    'true';
match_verb(Context, RuleVerbs) ->
    lists:member(cb_context:req_verb(Context), RuleVerbs) orelse
        lists:member(?CATCH_ALL, RuleVerbs).
