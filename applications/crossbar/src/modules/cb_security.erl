...
-define(SYSTEM_AUTH_METHODS,
    kapps_config:get_ne_binaries(
        ?AUTH_CONFIG_CAT, <<"available_auth_methods">>, ?DEFAULT_AUTH_METHODS
    )
).
...
authorize(Context) ->
    authorize_list_available_module(
...
maybe_flush_config(Context) ->
    case cb_context:fetch(Context, 'flush', 'false') of
        'true' ->
            kapps_account_config:flush(
                cb_context:account_id(Context), ?AUTH_CONFIG_CAT, <<"hierarchy_merge">>
            );
        'false' ->
            Context
    end.
...
read(Context) ->
    ...
    case cb_context:resp_status(C1) of
        'success' ->
            add_inherited_config(
                cb_context:set_resp_data(C1, kz_json:delete_key(<<"id">>, cb_context:resp_data(C1)))
            );
        _ ->
...
add_inherited_config(Context) ->
    Props = [
        {<<"account">>, maybe_add_multi_factor_metadata(cb_context:resp_data(Context))},
        {<<"inherited_config">>,
            maybe_add_multi_factor_metadata(
                kz_json:from_list([
                    {<<"auth_modules">>, crossbar_auth:get_inherited_config(Context)}
                ])
            )}
    ],
...
maybe_add_multi_factor_metadata(AuthConfig) ->
    ...
    kz_json:foldl(
        Fun, AuthConfig, kz_json:get_value(<<"auth_modules">>, AuthConfig, kz_json:new())
    ).

-spec add_multi_factor_metadata(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    kz_json:object().
add_multi_factor_metadata(AuthModule, JObj, AuthConfig) ->
    ...
    case
        kz_term:is_not_empty(AccountId) andalso
            kz_term:is_not_empty(ConfigId) andalso
            get_metadata(AccountId, ConfigId)
    of
        'false' -> set_as_system(AuthConfig, Path);
        'undefined' -> set_as_system(AuthConfig, Path);
        Metadata -> kz_json:set_value(Path, Metadata, AuthConfig)
    end.
...
create_config_document(Context) ->
    ...
    crossbar_doc:handle_json_success(
        kz_doc:set_type(Doc, <<"account_config">>), cb_context:store(Context, flush, true)
    ).

check_multi_factor_setting(Context) ->
    ReqData = cb_context:fetch(Context, <<"orig_req_data">>, cb_context:req_data(Context)),
    kz_json:foldl(
        fun check_multi_factor_setting/3,
        Context,
        kz_json:get_value(<<"auth_modules">>, ReqData, kz_json:new())
    ).

check_multi_factor_setting(AuthModule, JObj, Context) ->
    case
        has_configuration_id(JObj) andalso
...
%% @end
%%------------------------------------------------------------------------------
-spec check_account_hierarchy(kz_term:ne_binary(), kz_json:object(), cb_context:context()) ->
    cb_context:context().
check_account_hierarchy(AuthModule, JObj, Context) ->
    ...
    case
        cb_context:is_superduper_admin(AuthAccountId) orelse
            kzd_accounts:is_in_account_hierarchy(AuthAccountId, AccountId, 'true') orelse
            kz_json:is_false([<<"multi_factor">>, <<"include_subaccounts">>], JObj) orelse
            kzd_accounts:is_in_account_hierarchy(AccountId, AuthAccountId)
    of
        'true' ->
            Context;
        'false' ->
            ErrMsg = kz_term:to_binary(
                io_lib:format("multi-factor account_id ~s is not in your account tree", [AccountId])
            ),
            failed_multi_factor_validation(AuthModule, ErrMsg, Context)
    end.

-spec failed_multi_factor_validation(
    kz_term:ne_binary(), kz_term:ne_binary(), cb_context:context()
) -> cb_context:context().
failed_multi_factor_validation(AuthModule, ErrMsg, Context) ->
    KeyPath = <<"auth_modules.", AuthModule/binary, ".account_id">>,
    JObj = cb_context:validation_errors(Context),
    ErrorJObj = kz_json:from_list_recursive([
        {KeyPath, [{<<"required">>, [{<<"message">>, ErrMsg}]}]}
    ]),
    Setters = [
