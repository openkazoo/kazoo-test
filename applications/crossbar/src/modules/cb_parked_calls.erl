...
-define(DB_DOC_NAME,
    kapps_config:get_ne_binary(?MOD_CONFIG_CAT, <<"db_doc_name">>, <<"parked_calls">>)
).
...
validate(Context) ->
    ...
    case
        cb_context:resp_status(Ctx) =:= 'success' andalso
            cb_context:doc(Ctx)
    of
        'false' ->
            Ctx;
        ...
        Slots ->
            cb_context:set_resp_data(Ctx, kz_json:from_list([{<<"slots">>, kz_json:merge(Slots)}]))
    end.
