...
validate(Context, ?HTTP_DELETE, ConfigId) ->
    Doc =
        case kapps_config_util:load_config_from_account(cb_context:account_id(Context), ConfigId) of
...
-spec validate_with_parent(cb_context:context(), kz_term:ne_binary(), kz_json:object()) ->
    cb_context:context().
validate_with_parent(Context, ConfigId, Parent) ->
...
set_id(ConfigId, JObj) ->
    kz_json:set_value(<<"id">>, kapps_config_util:account_doc_id(ConfigId), JObj).
...
maybe_save_or_delete(Context, ConfigId) ->
    Stored = kz_doc:private_fields(
        kapps_account_config:get(cb_context:account_id(Context), ConfigId)
    ),
    UpdatedContext =
        case {cb_context:req_data(Context), kz_doc:revision(Stored)} of
            {?EMPTY_JSON_OBJECT, 'undefined'} -> Context;
            {?EMPTY_JSON_OBJECT, _} -> crossbar_doc:delete(cb_context:set_doc(Context, Stored));
            {Diff, _} -> crossbar_doc:save(Context, kz_json:merge(Stored, Diff), [])
        end,
