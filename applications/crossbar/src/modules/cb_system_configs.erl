...
authorize(Context) ->
    case
        cb_context:is_superduper_admin(Context) andalso
...
validate_document_request(Context, Id, FullConfig) ->
    cb_context:validate_request_data(
        kapps_config_util:system_config_document_schema(Id),
...
-spec validate_document_node(
    cb_context:context(), path_token(), http_method(), kz_term:api_ne_binary()
) -> cb_context:context().
validate_document_node(Context, Id, ?HTTP_GET, Node) ->
    Config =
        case kz_term:is_true(cb_context:req_value(Context, <<"with_defaults">>, 'false')) of
            'true' ->
                set_id(Id, Node, kapps_config_doc:stored_node(Id, Node));
            'false' ->
                set_id(
                    Id,
                    Node,
                    kz_json:get_value(Node, kapps_config_doc:get_config(Id), kz_json:new())
                )
        end,
    crossbar_doc:handle_datamgr_success(Config, Context);
validate_document_node(Context, Id, ?HTTP_PUT, Node) ->
...
validate_node_request(Context, Id, Node, FullConfig) ->
    cb_context:validate_request_data(
        kapps_config_util:system_schema(Id),
        cb_context:set_req_data(Context, FullConfig),
        fun(Ctx) ->
    ...
    case cb_context:resp_status(Ctx) of
        'success' ->
            cb_context:set_resp_data(
                Ctx, set_id(Id, Node, kz_json:get_value(Node, cb_context:doc(Ctx)))
            );
        _ ->
            Ctx
    end.
...
-spec delete(
    cb_context:context(), path_token(), path_token(), kz_term:api_object() | kz_json:objects()
) ->
    cb_context:context().
...
delete(Context, Id, Node, Doc) ->
    case kz_json:get_ne_value(Node, Doc) of
        'undefined' ->
            crossbar_util:response_bad_identifier(Node, Context);
        _NodeValue ->
