...
-define(WHITELABEL_ICON_MIME_TYPES, [
    {<<"image">>, <<"ico">>},
...
content_types_provided(Context, Domain, AttachType) ->
    content_types_provided_for_domain_attachments(
        Context, Domain, AttachType, cb_context:req_verb(Context)
    ).

-spec content_types_provided_for_domain_attachments(
...
-spec content_types_provided_for_domain_attachments(
    cb_context:context(), path_token(), path_token()
) ->
    cb_context:context().
...
validate_upload(Context, FileJObj) ->
    ...
    case cb_context:resp_status(Context) of
        'success' ->
            Props = [
                {<<"content_type">>, content_type(FileJObj)},
...
missing_domain_error(Context) ->
    cb_context:add_validation_error(
        ...
        kz_json:from_list([
            {<<"message">>,
                <<"No domain found to use. Supply one on the request or create one via the whitelabel API">>}
        ]),
        Context
    ).
...
-spec test_host(
    kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary(), kz_network_utils:options()
) ->
    kz_json:object().
test_host(Host, HostConfig, DomainType, Options) ->
    kz_json:from_list([
        {<<"expected">>, kzd_domains:mappings(HostConfig)},
...
test_network_options(Context) ->
    ...
    case kz_network_utils:find_nameservers(Domain) of
        [] -> DefaultOptions;
        Nameservers -> kz_network_utils:set_option_nameservers(DefaultOptions, Nameservers)
    end.
...
load_whitelabel_binary(Context, AttachType) ->
    case whitelabel_binary_meta(Context, AttachType) of
        ...
        {AttachmentId, JObj} -> update_response_with_attachment(Context, AttachmentId, JObj)
    end.

-spec find_whitelabel_binary(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
find_whitelabel_binary(Context, Domain, AttachType) ->
    Context1 = find_whitelabel_meta(Context, Domain),
    case
        kz_doc:id(cb_context:doc(Context)) =:= ?WHITELABEL_ID orelse
            cb_context:resp_status(Context1) =:= 'success'
    of
    ...
    of
        ...
        'true' ->
            ...
            case whitelabel_attachment_id(JObj, AttachType) of
                'undefined' -> 'undefined';
                AttachmentId -> {AttachmentId, kz_json:get_value(AttachmentId, JObj)}
            end
    end.
...
update_response_with_attachment(Context, AttachmentId, JObj) ->
    LoadedContext = crossbar_doc:load_attachment(
        cb_context:doc(Context), AttachmentId, ?TYPE_CHECK_OPTION(kzd_whitelabel:type()), Context
    ),
    WithHeaders = cb_context:add_resp_headers(
        LoadedContext,
        #{
...
update_whitelabel_binary(AttachType, WhitelabelId, Context) ->
    ...
    JObj1 =
        case whitelabel_binary_meta(Context, AttachType) of
            'undefined' -> JObj;
            {AttachmentId, _} -> kz_doc:delete_attachment(JObj, AttachmentId)
        end,
...
maybe_update_account_definition(Context, 'success') ->
    Context1 = crossbar_doc:load(
        cb_context:account_id(Context), Context, ?TYPE_CHECK_OPTION(kzd_accounts:type())
    ),
    case cb_context:resp_status(Context1) of
        'success' ->
            ...
            _ = cb_accounts:post(
                cb_context:set_doc(Context1, AccountDoc1), cb_context:account_id(Context)
            ),
            Context;
        _Status ->
            Context1
    end;
maybe_update_account_definition(Context, _Status) ->
    Context.
...
maybe_cleanup_account_definition(Context, _Status) ->
    Context.
