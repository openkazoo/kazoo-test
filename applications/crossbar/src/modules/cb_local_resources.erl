...
-spec check_for_registering_gateways(kz_term:api_binary(), cb_context:context()) ->
    cb_context:context().
check_for_registering_gateways(ResourceId, Context) ->
    case
        lists:any(
            fun is_registering_gateway/1,
...
is_registering_gateway(Gateway) ->
    kz_json:is_true(<<"register">>, Gateway) andalso
        kz_json:is_true(<<"enabled">>, Gateway).

-spec check_if_peer(kz_term:api_binary(), cb_context:context()) -> cb_context:context().
check_if_peer(ResourceId, Context) ->
    case
        {
            kz_term:is_true(cb_context:req_value(Context, <<"peer">>)),
...
validate_gateway_ips(
    [{Idx, 'undefined', 'undefined'} | IPs], SIPAuth, ACLs, ResourceId, Context, 'success'
) ->
    C = cb_context:add_validation_error(
        [<<"gateways">>, Idx, <<"server">>],
        <<"required">>,
...
validate_gateway_ips(
    [{Idx, 'undefined', ServerIP} | IPs], SIPAuth, ACLs, ResourceId, Context, 'success'
) ->
    case kz_network_utils:is_ipv4(ServerIP) of
        'true' ->
            case validate_ip(ServerIP, SIPAuth, ACLs, ResourceId) of
                'true' ->
                    validate_gateway_ips(
                        IPs, SIPAuth, ACLs, ResourceId, Context, cb_context:resp_status(Context)
                    );
                'false' ->
                    C = cb_context:add_validation_error(
                        [<<"gateways">>, Idx, <<"server">>],
                    ...
                    validate_gateway_ips(
                        IPs, SIPAuth, ACLs, ResourceId, C, cb_context:resp_status(C)
                    )
            end;
        'false' ->
            validate_gateway_ips(
                [{Idx, 'undefined', 'undefined'} | IPs],
                SIPAuth,
                ACLs,
                ResourceId,
                Context,
...
validate_gateway_ips(
    [{Idx, InboundIP, ServerIP} | IPs], SIPAuth, ACLs, ResourceId, Context, 'success'
) ->
...
maybe_aggregate_resource(_Context, _Status) ->
    'false'.
...
-spec maybe_remove_aggregate(kz_term:ne_binary(), cb_context:context(), crossbar_status()) ->
    boolean().
...
maybe_remove_aggregate(_ResourceId, _Context, _Status) ->
    'false'.
...
get_all_acl_ips() ->
    ...
    case Resp of
        {'error', _} -> [];
        {'ok', JObj} -> extract_all_ips(kz_json:get_value(<<"Value">>, JObj, kz_json:new()))
    end.
...
extract_gateway_ips([Gateway | Gateways], Idx, IPs) ->
    IP =
        {Idx, kz_json:get_ne_value(<<"inbound_ip">>, Gateway),
            kz_json:get_ne_value(<<"server">>, Gateway)},
    extract_gateway_ips(Gateways, Idx + 1, [IP | IPs]).

-spec validate_ip(kz_term:api_binary(), sip_auth_ips(), acl_ips(), kz_term:api_binary()) ->
    boolean().
validate_ip(IP, SIPAuth, ACLs, ResourceId) ->
    lists:all(
        fun({CIDR, AuthId}) ->
            AuthId =:= ResourceId orelse
                not (kz_network_utils:verify_cidr(IP, CIDR))
        end,
        ACLs
    ) andalso
        lists:all(
