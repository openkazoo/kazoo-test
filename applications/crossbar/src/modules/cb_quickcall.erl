...
init() ->
    Bindings = [
        {<<"v2_resource.allowed_methods.quickcall">>, 'allowed_methods'},
...
validate_quickcall(Context) ->
    ...
    case cb_context:has_errors(Context1) of
        'true' ->
            ...
            Context1;
        'false' ->
            maybe_originate(Context1, cb_context:req_verb(Context))
    end.
...
-spec load_endpoint(cb_context:context(), path_token(), kz_term:ne_binary()) ->
    cb_context:context().
...
maybe_validate_quickcall(Context, 'success') ->
    ...
    case
        AllowAnon orelse
            cb_context:is_authenticated(Context) orelse
    ...
    Routines =
        [
         ...
         || {F, V} <- [
                ...
                {
                    fun kapps_call:set_owner_id/2,
                    kz_json:get_ne_binary_value(<<"owner_id">>, cb_context:doc(Context))
                }
...
filter_number_regex(Number, Regex) ->
    case re:run(Number, Regex, [{'capture', 'all_but_first', 'binary'}]) of
        {'match', [Match | _]} ->
            ...
            Match;
        _NotMatching ->
            lager:warning("tried to filter number ~p with regex ~p, but no match found", [
                Number, Regex
            ]),
            Number
    end.
...
originate_quickcall(Endpoints, Call, Context) ->
    ...
    MsgId =
        case kz_term:is_empty(cb_context:req_id(Context)) of
        ...
        kz_json:from_list(
            [
                ...
                | kz_api:default_headers(
                    <<"resource">>, <<"originate_req">>, ?APP_NAME, ?APP_VERSION
                )
            ]
        ),
...
originate(Request, Context, ?HTTP_GET, _CallTimeoutS) ->
    ...
    crossbar_util:response_202(
        <<"quickcall initiated">>, JObj, cb_context:set_resp_data(Context, Request)
    );
originate(Request, Context, ?HTTP_POST, CallTimeoutS) ->
    case
        kz_amqp_worker:call_collect(
            ...
            fun([Resp | _Resps]) ->
                kapi_resource:originate_resp_v(Resp) orelse
...
send_originate_resp(Request, Context, Response) ->
    ...
    crossbar_util:response_202(
        <<"quickcall initiated">>, APIResponse, cb_context:set_resp_data(Context, APIResponse)
    ).
...
update_quickcall_endpoints(AutoAnswer, [Endpoint]) ->
    WithAA = kz_json:set_value(
        [<<"Custom-Channel-Vars">>, <<"Auto-Answer">>], AutoAnswer, Endpoint
    ),
