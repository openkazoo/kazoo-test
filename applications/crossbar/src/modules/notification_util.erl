...
maybe_notify_account_change(Old, Context) ->
    ...
    Notify = fun({K, _}) ->
        notify_account_change(AccountId, {K, kz_json:get_value(K, New)}, Context)
    end,

    kz_json:foreach(Notify, Changed).

-spec notify_account_change(
    kz_term:api_binary(), {kz_term:ne_binary(), kz_json:object()}, cb_context:context()
) -> 'ok'.
...
notify_account_change(AccountId, {<<"pvt_enabled">>, IsEnabled}, Context) ->
    lager:info("account ~s enabled has changed, sending registrations flush: pvt_enable: ~p", [
        AccountId, IsEnabled
    ]),
