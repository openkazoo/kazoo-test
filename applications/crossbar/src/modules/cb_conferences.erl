...
-spec validate_resource_conference(http_method(), cb_context:context(), kz_term:ne_binary()) ->
    cb_context:context().
...
-spec validate_conference(http_method(), cb_context:context(), kz_term:ne_binary()) ->
    cb_context:context().
...
-spec validate_participants(http_method(), cb_context:context(), kz_term:ne_binary()) ->
    cb_context:context().
...
-spec validate_participant(
    http_method(), cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()
) -> cb_context:context().
    ...
    cb_context:setters(
        ...
        [
            ...
            {fun cb_context:set_resp_data/2, Conferences},
            {
                ...
                kz_json:set_value(
                    <<"page_size">>, length(Conferences), cb_context:resp_envelope(Context)
                )
            }
        ]
    ).
...
invalid_numbers(Context, JObjs) ->
    ...
    Error = kz_json:from_list([
        {<<"message">>, <<"Numbers already in use">>},
        {<<"cause">>, Numbers}
    ]),
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_conference_action(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
...
play(Context, ConferenceId, ParticipantId, Data) ->
    play_media(
        Context, ConferenceId, ParticipantId, kz_json:get_ne_binary_value(<<"media_id">>, Data)
    ).
...
data_required(Context, Action) ->
    cb_context:add_validation_error(
        ...
        <<"required">>,
        kz_json:from_list([
            {<<"message">>, <<"action '", Action/binary, "' requires a data object">>}
        ]),
        Context
    ).
...
play_media(Context, ConferenceId, ParticipantId, MediaId) ->
    case kz_media_util:media_path(MediaId, cb_context:account_id(Context)) of
        'undefined' ->
            media_id_invalid(Context, MediaId);
        Media ->
            lager:info("playing ~s to conference ~s participant ~p", [
                Media, ConferenceId, ParticipantId
            ]),
...
build_valid_endpoints(Context, ConferenceId, Data) ->
    case kz_json_schema:validate(<<"conferences.dial">>, Data) of
        {'ok', ValidData} ->
            build_endpoints_to_dial(
                Context, ConferenceId, kz_json:get_list_value(<<"endpoints">>, ValidData)
            );
        {'error', Errors} ->
    ...
    Command = [
        ...
        {<<"Caller-ID-Name">>,
            kz_json:get_ne_binary_value(
                <<"caller_id_name">>, Data, kz_json:get_ne_binary_value(<<"name">>, Conference)
            )},
...
zone(ConferenceId, TargetCallId) ->
    ...
    of
        ...
        _E ->
            lager:info(
                "target ~s not found (~p), checking conference id ~s",
...
-define(BUILD_ACC(Es, Call, Context, El), #build_acc{
    endpoints = Es,
    call = Call,
    context = Context,
    element = El
...
error_no_endpoints(Context) ->
    cb_context:add_validation_error(
        ...
        <<"minItems">>,
        kz_json:from_list([
            {<<"message">>, <<"endpoints failed to resolve to route-able destinations">>},
            {<<"target">>, 1}
        ]),
        Context
    ).
...
build_endpoint(?NE_BINARY = Number, ?BUILD_ACC(Endpoints, Call, Context, Element) = Acc) ->
    case
        knm_converters:is_reconcilable(Number) orelse
            byte_size(Number) < ?MIN_DIGITS_FOR_DID
    of
        'true' ->
            build_number_endpoint(Number, Acc);
        'false' ->
            ?BUILD_ACC(
                Endpoints,
                Call,
...
add_not_endpoint_error(Context, Element) when is_integer(Element) ->
    cb_context:add_validation_error(
        [<<"data">>, <<"endpoints">>, Element],
        <<"enum">>,
        kz_json:from_list([
            {<<"message">>, <<"Value not a number, device or user ID, or SIP endpoint">>}
        ]),
        Context
    ).
...
build_endpoint_from_doc(Endpoint, ?BUILD_ACC(_, _, _, _) = Acc) ->
    case
        kz_doc:is_soft_deleted(Endpoint) orelse
            kz_doc:is_deleted(Endpoint)
    of
        'true' ->
...
build_endpoint_from_doc(Device, ?BUILD_ACC(Endpoints, Call, Context, Element), <<"device">>) ->
    ...
    case kz_endpoint:build(Device, Properties, Call) of
        {'ok', Legs} ->
            ?BUILD_ACC(Endpoints ++ Legs, Call, Context, Element + 1);
        {'error', _E} ->
            lager:info("failed to build endpoint ~s: ~p", [kz_doc:id(Device), _E]),
            ?BUILD_ACC(
                Endpoints,
                Call,
                add_not_found_error(Context, kz_doc:id(Device), Element),
                Element + 1
            )
    end;
    ...
    case maybe_add_user_endpoints(User, Call) of
        [] ->
            ...
            ?BUILD_ACC(
                Endpoints,
                Call,
                add_no_devices_error(Context, kz_doc:id(User), Element),
                Element + 1
            );
        Legs ->
...
-spec add_no_devices_error(cb_context:context(), kz_term:ne_binary(), pos_integer()) ->
    cb_context:context().
add_no_devices_error(Context, UserId, Element) when is_integer(Element) ->
    cb_context:add_validation_error(
        ...
        kz_json:from_list([
            {<<"message">>,
                <<"user ", UserId/binary, " failed to resolve to route-able destinations">>},
            {<<"target">>, 1}
        ]),
        Context
    ).

-spec add_not_found_error(cb_context:context(), kz_term:ne_binary(), pos_integer()) ->
    cb_context:context().
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_participants_action(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
handle_participants_action(Context, ConferenceId, Action = ?MUTE) ->
    handle_participants_action(
        Context,
        ConferenceId,
        Action,
        fun(P) ->
            kz_json:is_false(<<"Is-Moderator">>, P) andalso
                kz_json:is_true(<<"Speak">>, P)
        end
    );
...
-spec handle_participants_action(
    cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary(), filter_fun()
) ->
    cb_context:context().
handle_participants_action(Context, ConferenceId, Action, Selector) ->
    ...
    _ = [
        perform_participant_action(
            Conf, Action, kz_json:get_ne_binary_value(<<"Participant-ID">>, P)
        )
     || P <- Participants,
...
relate(Context, ConferenceId, ParticipantId, OtherParticipantId) ->
    ...
    kapps_conference_command:relate_participants(
        ParticipantId, OtherParticipantId, Relationship, Conference
    ),
    lager:info(
        "relating ~p to ~p with ~s in conference ~s",
...
-spec perform_participant_action(
    kapps_conference:conference(), kz_term:ne_binary(), kz_term:api_integer()
) -> 'ok'.
...
-spec enrich_participant(kz_term:ne_binary(), kz_term:ne_binary(), cb_context:context()) ->
    cb_context:context().
...
conference_realtime_data(ConferenceId) ->
    ...
    C2 = kz_json:from_list(
        [
            ...
            {<<"participants">>, [
                kz_json:normalize_jobj(Participant)
             || Participant <- Participants
            ]}
        ]
    ),
    Routines = [
        {fun kz_json:delete_key/2, <<"Participants">>},
...
request_conference_details(ConferenceId) ->
    ...
    case
        kz_amqp_worker:call_collect(
            Req, fun kapi_conference:publish_search_req/1, {'ecallmgr', 'true'}
        )
    of
        {'error', _E} ->
...
partition_participants_count(Participants) ->
    partition_participants_count(Participants, fun(P) ->
        kz_json:is_true([<<"Conference-Channel-Vars">>, <<"Is-Moderator">>], P)
    end).

-spec partition_participants_count(kz_json:objects(), fun((kz_json:object()) -> boolean())) ->
    {integer(), integer()}.
