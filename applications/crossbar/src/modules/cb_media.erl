...
init() ->
    ...
    _ = crossbar_bindings:bind(
        <<"*.content_types_provided.media">>, ?MODULE, 'content_types_provided'
    ),
    _ = crossbar_bindings:bind(
        <<"*.content_types_accepted.media">>, ?MODULE, 'content_types_accepted'
    ),
...
-spec content_types_provided_for_media(
...
validate(Context, MediaId, ?BIN_DATA) ->
    ...
    validate_media_binary(
        Context, MediaId, cb_context:req_verb(Context), cb_context:req_files(Context)
    ).
...
-spec validate_media_doc(cb_context:context(), kz_term:ne_binary(), http_method()) ->
    cb_context:context().
...
-spec validate_media_binary(
    cb_context:context(), kz_term:ne_binary(), http_method(), kz_term:proplist()
) -> cb_context:context().
...
-spec maybe_normalize_upload(cb_context:context(), kz_term:ne_binary(), kz_json:object()) ->
    cb_context:context().
...
-spec normalize_upload(
    cb_context:context(), kz_term:ne_binary(), kz_json:object(), kz_term:api_binary()
) ->
    cb_context:context().
...
-spec validate_upload(cb_context:context(), kz_term:ne_binary(), kz_json:object()) ->
    cb_context:context().
validate_upload(Context, MediaId, FileJObj) ->
    CT = kz_json:get_value(
        [<<"headers">>, <<"content_type">>], FileJObj, <<"application/octet-stream">>
    ),
    Size = kz_json:get_integer_value(
...
get(Context, _MediaId, ?BIN_DATA) ->
    CT = kz_json:get_value(
        <<"content-type">>, cb_context:doc(Context), <<"application/octet-stream">>
    ),
...
-spec post_media_doc(cb_context:context(), kz_term:ne_binary(), kz_term:api_binary()) ->
    cb_context:context().
...
remove_tts_keys(Context) ->
    cb_context:set_doc(
        Context,
        kz_json:delete_keys(
            [<<"pvt_previous_tts">>, <<"pvt_previous_voice">>],
            cb_context:doc(Context)
        )
    ).

-spec post_media_binary(cb_context:context(), kz_term:ne_binary(), kz_term:api_binary()) ->
    cb_context:context().
...
create_update_tts(Context, <<"update">>) ->
    maybe_update_media_file(
        Context,
        <<"update">>,
        is_tts_changed(cb_context:doc(Context)),
        cb_context:resp_status(Context)
    ).

-spec maybe_update_media_file(
    cb_context:context(), kz_term:ne_binary(), boolean(), crossbar_status()
) ->
    cb_context:context().
maybe_update_media_file(Context, CreateOrUpdate, 'true', 'success') ->
    ...
    try kazoo_tts:create(Text, Voice) of
        ...
        {'ok', ContentType, Content} ->
            ...
            C1 = update_media_binary(
                cb_context:set_req_files(Context, [{FileName, FileJObj}]), MediaId
            ),
            case
                cb_context:resp_status(C1) =:= 'success' andalso
                    CreateOrUpdate
            of
                'false' ->
                    maybe_delete_tts(C1, <<"creating TTS failed unexpectedly">>, CreateOrUpdate);
                <<"create">> ->
                    Context;
                <<"update">> ->
                    C2 = crossbar_doc:load_merge(
                        MediaId,
...
-spec maybe_delete_tts(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
...
-spec load_media_docs_by_language(
    cb_context:context(), kz_term:ne_binary() | 'null', kz_term:api_binary()
) ->
    cb_context:context().
...
-spec load_media_docs_by_prompt(cb_context:context(), kz_term:ne_binary(), kz_term:api_binary()) ->
    cb_context:context().
...
fix_prompt_start_keys(Context) ->
    cb_context:set_resp_envelope(
        Context,
        lists:foldl(
            fun fix_prompt_start_keys_fold/2,
            cb_context:resp_envelope(Context),
            [<<"start_key">>, <<"next_start_key">>]
        )
    ).
...
maybe_validate_prompt(MediaId, Context, 'success') ->
    case kzd_media:prompt_id(cb_context:doc(Context)) of
        'undefined' -> Context;
        PromptId -> validate_prompt(MediaId, Context, PromptId)
    end;
...
validate_prompt(MediaId, Context, PromptId) ->
    ...
    case kz_media_util:prompt_id(PromptId, Language) of
        ...
        _OtherId ->
            lager:info(
                "attempt to change prompt id '~s' is not allowed on existing media doc '~s'",
                [PromptId, MediaId]
            ),
            cb_context:add_validation_error(
                <<"prompt_id">>,
                ...
                kz_json:from_list(
                    [
                        {<<"message">>,
                            <<"Changing the prompt_id on an existing prompt is not allowed">>},
                        {<<"cause">>, PromptId}
                    ]
                ),
                Context
...
maybe_add_prompt_fields(Context) ->
    ...
    case kzd_media:prompt_id(JObj) of
        'undefined' ->
            [];
        PromptId ->
            Language = kz_term:to_lower_binary(
                kzd_media:language(JObj, kz_media_util:default_prompt_language())
            ),
            ...
            case kz_doc:attachment_names(cb_context:doc(Context1)) of
                ...
                [Attachment | _] ->
                    ...
                    cb_context:add_resp_headers(
                        ...
                        #{
                            <<"content-disposition">> =>
                                <<"attachment; filename=", Attachment/binary>>,
                            <<"content-type">> => kz_doc:attachment_content_type(
                                cb_context:doc(Context1), Attachment
                            )
                        }
                    )
            end;
...
update_media_binary(Context, MediaId) ->
    update_media_binary(
        crossbar_util:maybe_remove_attachments(Context),
        MediaId,
...
-spec update_media_binary(cb_context:context(), path_token(), req_files()) ->
    cb_context:context().
update_media_binary(Context, _MediaId, []) ->
    Context;
update_media_binary(Context, MediaId, [{Filename, FileObj} | Files]) ->
    ...
    Context1 = crossbar_doc:save_attachment(
        MediaId,
        AttachmentName,
        Contents,
        Context,
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            update_media_binary(Context1, MediaId, Files);
        _Failure ->
...
%% @end
%%------------------------------------------------------------------------------
-spec delete_media_binary(path_token(), cb_context:context(), kz_term:api_binary()) ->
    cb_context:context().
...
delete_media_binary(MediaId, Context, _AccountId) ->
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            case kz_doc:attachment_names(cb_context:doc(Context1)) of
                [] -> crossbar_util:response_bad_identifier(MediaId, Context);
                [AttachmentId | _] -> crossbar_doc:delete_attachment(MediaId, AttachmentId, Context)
            end;
        _Status ->
            Context1
    end.
