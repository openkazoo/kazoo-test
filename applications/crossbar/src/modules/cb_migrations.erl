...
-spec render_migration_summary({binary(), binary(), atom()}, cb_context:context()) ->
    cb_context:context().
...
check_migration_performed(Id, Doc) ->
    case kz_json:get_value([<<"migrations_performed">>, Id], Doc) of
        ...
        Value ->
            [
                {<<"performed">>, 'true'},
                {<<"performed_time">>, kz_json:get_value(<<"performed_time">>, Value)}
            ]
    end.
    ...
    Result =
        case kz_json:is_true(<<"include_descendants">>, cb_context:req_data(Context)) of
            'true' ->
                ...
                List = [
                    migrate_on_account(kz_json:get_value(<<"id">>, X), MigId, Module, Context)
                 || X <- Filtered
                ],
...
mark_migration_complete(MigId, AccountId, Context) ->
    ...
    {'ok', _} = kz_datamgr:save_doc(
        AccountDb, kz_json:set_value(<<"migrations_performed">>, NewMigs, Doc)
    ),
