...
follow_group(JObj, JObjs, Ids, Context) ->
    ...
    Members = lists:foldr(
        ...
        sets:new(),
        kz_json:get_keys(<<"endpoints">>, JObj)
    ),
...
revalidate_doc(Id, JObj, Context) ->
    case get_validate_binding(JObj) of
        'undefined' ->
            ...
            InterimContext = cb_context:add_system_error(
                'invalid_bulk_type',
...
maybe_delete_doc(Id, JObj, DbDoc, Context) ->
    ...
    case get_delete_binding(JObj) of
        ...
        Binding ->
            Setters = [
                {fun cb_context:set_req_verb/2, ?HTTP_DELETE},
...
-spec run_binding(kz_term:ne_binary(), list(), kz_term:ne_binary(), cb_context:context()) ->
    cb_context:context().
run_binding(Binding, Payload, Id, Context) ->
