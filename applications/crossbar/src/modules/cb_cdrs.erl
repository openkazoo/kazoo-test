...
-define(CONTEXT_COLUMNS, [{<<"inception_direction">>, fun col_inception_direction/3}]).

-define(INTERACTION_COLUMNS, ?COLUMNS ++ ?CONTEXT_COLUMNS).

-type cdr_column_fun() :: fun(
    (kz_json:object(), kz_time:gregorian_seconds(), cb_context:context()) -> kz_term:ne_binary()
).
...
init() ->
    ...
    _ = crossbar_bindings:bind(
        <<"*.content_types_provided.cdrs">>, ?MODULE, 'content_types_provided'
    ),
...
-spec validate_utc_offset(cb_context:context(), kz_time:gregorian_seconds()) ->
    cb_context:context().
...
load_chunk_view(Context, ViewName, Options0) ->
    ...
    Setters = [
        {fun cb_context:store/3, 'has_cdr_filter', crossbar_filter:is_defined(Context)},
    ...
    crossbar_view:load_modb(
        cb_context:setters(fix_qs_filter_keys(Context), Setters), ViewName, Options
    ).

-spec fix_qs_filter_keys(cb_context:context()) -> cb_context:context().
fix_qs_filter_keys(Context) ->
    NewQs = kz_json:map(
        fun(K, V) -> fix_filter_key(kz_binary:reverse(K), V) end,
...
load_cdr_summary(Context) ->
    ...
    Options = [
        {'mapper', fun normalize_summary_results/2},
        {'range_start_keymap', []},
    ...
    case cb_context:resp_status(C1) of
        'success' ->
            ...
            cb_context:set_resp_data(
                C1, lists:foldl(fun merge_cdr_summary/2, kz_json:new(), JObjs)
            );
        _ ->
            C1
    end.
...
merge_cdr_summary(JObj1, JObj2) ->
    kz_json:foldl(
        fun(Key1, Value1, JObj) ->
            case kz_json:get_value(Key1, JObj1) of
                'undefined' ->
                    kz_json:set_value(Key1, Value1, JObj);
...
-spec cdrs_listing_mapper(cb_context:context(), kz_json:object(), kz_json:objects()) ->
    kz_json:objects().
...
-spec load_chunked_cdrs(cb_context:context(), kz_term:ne_binary(), kz_json:objects()) ->
    cb_context:context().
...
-spec load_chunked_cdr_ids(
    cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binaries()
) -> cb_context:context().
...
load_chunked_cdr_ids(Context, RespType, Db, Ids, 'success') ->
    case kz_datamgr:open_docs(Db, Ids, [{'doc_type', kzd_cdrs:type()}]) of
        {'ok', Results} ->
            Resp0 = [
                normalize_cdr(Context, RespType, Result)
             || Result <- Results,
                ...
                crossbar_filter:by_doc(
                    kz_json:get_json_value(<<"doc">>, Result),
                    Context,
                    cb_context:fetch(Context, 'has_cdr_filter')
                )
            ],
...
-spec normalize_cdr(cb_context:context(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object() | kz_term:binary().
normalize_cdr(Context, <<"json">>, Result) ->
    ...
    maybe_filter_empties(
        MappedRows,
        kapps_config:is_true(?MOD_CONFIG_CAT, <<"should_filter_empty_strings">>, 'false')
    );
...
-spec maybe_add_csv_header(
    cb_context:context(), kz_term:ne_binary(), kz_json:objects() | kz_term:binaries()
) -> cb_context:context().
...
col_owner_id(JObj, _Timestamp, _Context) ->
    kz_json:get_value([?KEY_CCV, <<"owner_id">>], JObj, <<>>).
...
col_authorizing_id(JObj, _Timestamp, _Context) ->
    case
        {
            kz_json:get_value([?KEY_CCV, <<"account_id">>], JObj, <<>>),
...
col_unix_timestamp(_JObj, Timestamp, _Context) ->
    kz_term:to_binary(kz_time:gregorian_seconds_to_unix_seconds(Timestamp)).
...
col_account_call_type(JObj, _Timestamp, _Context) ->
    kz_json:get_value([?KEY_CCV, <<"account_billing">>], JObj, <<>>).
col_rate(JObj, _Timestamp, _Context) ->
    kz_term:to_binary(
        kz_currency:units_to_dollars(kz_json:get_value([?KEY_CCV, <<"rate">>], JObj, 0))
    ).
col_rate_name(JObj, _Timestamp, _Context) ->
    kz_json:get_value([?KEY_CCV, <<"rate_name">>], JObj, <<>>).
col_bridge_id(JObj, _Timestamp, _Context) ->
    kz_json:get_value([?KEY_CCV, <<"bridge_id">>], JObj, <<>>).
col_recording_url(JObj, _Timestamp, _Context) ->
    kz_json:get_value([<<"recording_url">>], JObj, <<>>).
...
col_call_priority(JObj, _Timestamp, _Context) ->
    kz_json:get_value([?KEY_CCV, <<"call_priority">>], JObj, <<>>).

col_reseller_cost(JObj, _Timestamp, _Context) -> kz_term:to_binary(reseller_cost(JObj)).
col_reseller_call_type(JObj, _Timestamp, _Context) ->
    kz_json:get_value([?KEY_CCV, <<"reseller_billing">>], JObj, <<>>).
...
-spec handle_utc_time_offset(kz_time:gregorian_seconds(), kz_term:api_integer()) ->
    kz_time:gregorian_seconds().
handle_utc_time_offset(Timestamp, 'undefined') ->
    Timestamp;
...
calling_from(JObj) ->
    case kzd_cdrs:call_direction(JObj) of
        <<"inbound">> ->
            kzd_cdrs:caller_id_number(JObj, <<>>);
        <<"outbound">> ->
...
load_legs(<<BinTimestamp:11/binary, "-", _Key/binary>> = InteractionId, Context) ->
    ...
    Options = [
        {'mapper', fun normalize_leg_view_results/2},
