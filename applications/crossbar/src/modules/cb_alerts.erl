...
load_summary(Context) ->
    Routines = [
        fun maybe_check_port_requests/1,
...
maybe_check_port_requests(Context, AccountId) ->
    ...
    case AuthId =:= MasterId of
        'true' ->
            ...
            lager:debug("auth account is master ~p, showing port alerts for account path", [
                MasterId
            ]),
...
maybe_check_port_requests(Context, 'true', 'true') ->
    ...
    lager:debug(
        "auth account is both reseller and port authority, showing account path port alerts"
    ),
...
maybe_check_port_requests(Context, 'true', 'false') ->
    ...
    Ports =
        fetch_account_active_ports(ResellerId) ++
...
-spec should_hide_port(
    kz_term:api_ne_binary() | {'ok', kz_json:object()} | kz_datamgr:data_error()
) -> boolean().
...
do_check_port_requests([PortRequest | PortRequests], Context) ->
    Routines = [
        fun maybe_check_port_action_required/2,
        fun maybe_check_port_suspended/2
    ],
    Context1 = lists:foldl(
        fun(F, C) -> F(PortRequest, C) end,
        Context,
        Routines
    ),
...
check_port_suspended(PortRequest, Context) ->
    ...
    case
        lists:member(State, ?PORT_SUSPENDED_STATES) andalso
            Transition =/= []
    of
        'false' ->
            Context;
...
-spec check_low_balance(
    cb_context:context(), kz_currency:available_dollars_return(), float() | 'undefined'
) ->
    cb_context:context().
...
check_low_balance(Context, {'ok', AvailableDollars}, ThresholdDollars) when
    AvailableDollars < ThresholdDollars
->
...
-spec low_balance_alert(cb_context:context(), kz_currency:dollars(), number() | 'undefined') ->
    cb_context:context().
low_balance_alert(Context, AvailableDollars, ThresholdDollars) ->
    AccountId = cb_context:account_id(Context),
    Metadata = kz_json:from_list(
...
-spec payment_token_alert(kz_json:object() | 'no_payment_token', cb_context:context()) ->
    cb_context:context().
payment_token_alert('no_payment_token', Context) ->
    AccountId = cb_context:account_id(Context),
    From = kz_json:from_list(
...
check_system_alerts(Context) ->
    ...
    case kz_datamgr:get_results(?KZ_ALERTS_DB, ?AVAILABLE_LIST, ViewOptions) of
        ...
        {'ok', JObjs} ->
            Alerts = [
                kz_json:set_value(<<"clearable">>, 'true', JObj)
             || JObj <- JObjs
            ],
