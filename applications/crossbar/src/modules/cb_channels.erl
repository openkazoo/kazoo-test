            ...
            case find_channel(cb_context:account_id(Context), CallId, StatusJObjs) of
                'undefined' ->
                    lager:warning("trying to get info about a channel ~s not in the account ~s", [
                        CallId, cb_context:account_id(Context)
                    ]),
                    crossbar_util:response_bad_identifier(CallId, Context);
                Channel ->
...
-spec find_channel(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:objects()) ->
    kz_term:api_object().
find_channel(_AccountId, _CallId, []) ->
    'undefined';
...
-spec maybe_execute_command(cb_context:context(), kz_term:ne_binary(), kz_term:api_binary()) ->
    cb_context:context().
...
validate_action(Context, CallId) ->
    ...
    case cb_context:has_errors(Ctx) of
        'true' -> Ctx;
        'false' -> validate_action(Ctx, CallId, cb_modules_util:get_request_action(Context))
    end.
...
device_summary(Context, DeviceId) ->
    get_channels(
        Context,
        [cb_context:doc(crossbar_doc:load(DeviceId, Context, ?TYPE_CHECK_OPTION_ANY))],
    ...
    case kz_json:get_value(<<"type">>, EndpointData) of
        ...
        <<"device">> ->
            Context1 = crossbar_doc:load(
                EndpointId, Context, ?TYPE_CHECK_OPTION(kzd_devices:type())
            ),
...
get_channels(Context, Devices, PublisherFun) ->
    ...
    Req = props:filter_undefined(
        [
            ...
            {<<"Usernames">>, lists:usort(Usernames)},
...
-spec maybe_transfer(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
maybe_transfer(Context, Transferor, Transferee) ->
...
-spec transfer(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
transfer(Context, Transferor, _Transferee, Target) ->
    ...
    lager:debug("attempting ~s transfer ~s to ~s by ~s", [
        TransferType, _Transferee, Target, Transferor
    ]),
...
-spec maybe_intercept(
    cb_context:context(), kz_term:ne_binary(), kz_term:api_binary(), kz_term:api_binary()
) -> cb_context:context().
...
maybe_move(Context, CallId) ->
    API = [
        ...
        {<<"Data">>,
            kz_json:from_list([
                {<<"auto_answer">>, cb_context:req_value(Context, <<"auto_answer">>, 'false')},
                {<<"can_call_self">>, cb_context:req_value(Context, <<"can_call_self">>, 'true')},
                {<<"device_id">>, cb_context:req_value(Context, <<"device_id">>)},
                {<<"dial_strategy">>,
                    cb_context:req_value(Context, <<"dial_strategy">>, <<"simultaneous">>)},
...
get_account_id(Context) ->
    case cb_context:account_id(Context) of
        'undefined' ->
            case
                kapps_config:get_is_true(?MOD_CONFIG_CAT, <<"system_wide_channels_list">>, 'false')
            of
                'true' -> <<"all">>;
                'false' -> 'undefined'
            end;
