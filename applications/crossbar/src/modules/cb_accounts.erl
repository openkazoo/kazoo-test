...
init() ->
    Bindings = [
        {<<"*.allowed_methods.accounts">>, 'allowed_methods'},
...
-spec allowed_methods_on_account(
    kz_term:ne_binary(), {'ok', kz_term:ne_binary()} | {'error', any()}
) ->
    http_methods().
...
allowed_methods(_AccountId, ?CHILDREN) ->
    [?HTTP_GET];
allowed_methods(_AccountId, ?DESCENDANTS) ->
    [?HTTP_GET];
allowed_methods(_AccountId, ?SIBLINGS) ->
    [?HTTP_GET];
allowed_methods(_AccountId, ?API_KEY) ->
    [?HTTP_GET, ?HTTP_PUT];
allowed_methods(_AccountId, ?TREE) ->
    [?HTTP_GET];
allowed_methods(_AccountId, ?PARENTS) ->
    [?HTTP_GET].
...
-spec validate_resource(cb_context:context(), path_token(), kz_term:ne_binary()) ->
    cb_context:context().
...
-spec validate_account(cb_context:context(), kz_term:ne_binary(), http_method()) ->
    cb_context:context().
...
validate_account_path(Context, AccountId, ?API_KEY, ?HTTP_GET) ->
    Context1 = crossbar_doc:load(
        AccountId, prepare_context('undefined', Context), ?TYPE_CHECK_OPTION(?PVT_TYPE)
    ),
            ...
            case
                validate_move(
                    kapps_config:get_ne_binary(
                        ?ACCOUNTS_CONFIG_CAT, <<"allow_move">>, <<"superduper_admin">>
                    ),
                    Context,
                    AccountId,
                    ToAccount
...
post(Context, AccountId) ->
    ...
    case kzd_accounts:save(New) of
        {'ok', SavedAccount} ->
            ...
            _NotifyPid = kz_util:spawn(fun notification_util:maybe_notify_account_change/2, [
                Existing, Context1
            ]),
            _ProvisionerPid = kz_util:spawn(fun provisioner_util:maybe_update_account/1, [Context1]),

            lager:debug(
                "saved account doc, spawned workers for notification in ~p and provisioner in ~p",
            ...
            case cb_context:is_context(C) of
                ...
                'false' ->
                    ...
                    cb_context:add_system_error(
                        'unspecified_fault',
                        <<"internal error, unable to create the account">>,
                        Context
                    )
            end;
        _E:_R ->
            ...
            _ = delete(Context, NewAccountId),
            cb_context:add_system_error(
                'unspecified_fault', <<"internal error, unable to create the account">>, Context
            )
    end.
...
put(Context, AccountId, ?RESELLER) ->
    case kz_services_reseller:promote(AccountId) of
        {'error', 'master_account'} ->
            cb_context:add_system_error('forbidden', Context);
        {'error', 'reseller_descendants'} ->
            cb_context:add_system_error('account_has_descendants', Context);
        'ok' ->
...
delete_account(?MATCH_ACCOUNT_RAW(AccountId)) ->
    ...
    lager:info("attempting to delete ~s(~s)", [
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            lager:info("deleted account ~s", [AccountId]);
        _Resp ->
...
delete(Context, Account) ->
    ...
    case kapps_util:is_account_db(AccountDb) of
        'false' ->
            cb_context:add_system_error(
                'bad_identifier', kz_json:from_list([{<<"cause">>, AccountId}]), Context
            );
        'true' ->
...
maybe_update_descendants_count([]) ->
    'ok';
...
%%------------------------------------------------------------------------------
-spec validate_move(
    kz_term:ne_binary(), cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()
) -> boolean().
...
move_account(Context, AccountId) ->
    ...
    case crossbar_util:move_account(AccountId, ToAccount) of
        ...
        {'ok', _} -> load_account(AccountId, prepare_context(AccountId, Context))
    end.
...
-spec prepare_context(cb_context:context(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    cb_context:context().
prepare_context(Context, AccountId, AccountDb) ->
...
maybe_disallow_direct_clients(_AccountId, Context, 'false') ->
    ...
    case
        AuthUserReseller =/= MasterAccountId orelse
            kz_services_reseller:is_reseller(AuthAccountId)
    of
        'true' ->
            Context;
        'false' ->
            lager:debug("direct account ~p is disallowed from creating sub-accounts", [
                AuthAccountId
            ]),
    ...
    case kapps_util:account_has_descendants(AccountId) of
        ...
        'false' ->
            case knm_port_request:account_has_active_port(AccountId) of
                ...
                'true' ->
                    lager:debug("prevent deleting account ~s due to has active port request", [
                        AccountId
                    ]),
                    Msg = kz_json:from_list(
...
load_account(AccountId, Context) ->
    leak_pvt_fields(
        AccountId, crossbar_doc:load(AccountId, Context, ?TYPE_CHECK_OPTION(?PVT_TYPE))
    ).
...
-spec leak_pvt_fields(kz_term:api_binary(), cb_context:context(), crossbar_status()) ->
    cb_context:context().
leak_pvt_fields(AccountId, Context, 'success') ->
    Routines = [
        fun leak_pvt_allow_additions/1,
...
leak_pvt_fields(_AccountId, Context, _Status) ->
    Context.
...
leak_pvt_api_key(Context) ->
    case
        kz_term:is_true(cb_context:req_value(Context, <<"include_api_key">>, 'false')) orelse
            kapps_config:get_is_true(?ACCOUNTS_CONFIG_CAT, <<"expose_api_key">>, 'false')
    of
        'false' ->
            Context;
        'true' ->
...
leak_notification_preference(Context) ->
    leak_notification_preference(
        Context, kzd_accounts:notification_preference(cb_context:doc(Context))
    ).

-spec leak_notification_preference(cb_context:context(), kz_term:api_binary()) ->
    cb_context:context().
leak_notification_preference(Context, 'undefined') ->
    Context;
leak_notification_preference(Context, Pref) ->
    UpdatedRespJObj = kz_json:set_value(
        <<"notification_preference">>, Pref, cb_context:resp_data(Context)
    ),
...
leak_trial_time_left(Context, _JObj, 'undefined') ->
    RespData = kz_json:delete_key(
        <<"trial_time_left">>,
        cb_context:resp_data(Context)
    ),
    cb_context:set_resp_data(Context, RespData);
leak_trial_time_left(Context, JObj, _Expiration) ->
    RespData = kz_json:set_value(
        <<"trial_time_left">>,
...
-spec load_children(kz_term:ne_binary(), cb_context:context(), kz_term:ne_binary()) ->
    cb_context:context().
...
load_siblings(AccountId, Context) ->
    case
        kzd_accounts:is_superduper_admin(cb_context:auth_account_id(Context)) orelse
            (AccountId =/= cb_context:auth_account_id(Context) andalso
                kapps_config:get_is_true(?ACCOUNTS_CONFIG_CAT, <<"allow_sibling_listing">>, 'true'))
    of
        'true' -> load_siblings(AccountId, Context, cb_context:api_version(Context));
        'false' -> cb_context:add_system_error('forbidden', Context)
    end.

-spec load_siblings(kz_term:ne_binary(), cb_context:context(), kz_term:ne_binary()) ->
    cb_context:context().
...
-spec load_siblings_results(kz_term:ne_binary(), cb_context:context(), kz_json:objects()) ->
    cb_context:context().
...
load_account_db(Context, AccountId) when is_binary(AccountId) ->
    case kzd_accounts:fetch(AccountId) of
        {'ok', JObj} ->
            ...
            lager:debug("account ~s db exists, setting operating database as ~s", [
                AccountId, AccountDb
            ]),
...
create_new_account_db(Context) ->
    ...
    case
        kapps_util:is_account_db(AccountDb) andalso
            kz_datamgr:db_create(AccountDb)
    of
        'false' ->
...
set_notification_preference(Context, Preference) ->
    AccountDefinition = kzd_accounts:set_notification_preference(
        cb_context:doc(Context), Preference
    ),
...
set_trial_expires(JObj) ->
    TrialTime = kapps_config:get_integer(
        ?ACCOUNTS_CONFIG_CAT, <<"trial_time">>, ?SECONDS_IN_DAY * 14
    ),
...
notify_new_account(_Context, 'undefined') ->
    'ok';
notify_new_account(Context, _AuthDoc) ->
    ...
    Notify = [
        {<<"Account-Name">>, kzd_accounts:name(JObj)},
...
delete_remove_db(Context) ->
    Removed =
        case kz_datamgr:open_doc(cb_context:account_db(Context), cb_context:account_id(Context)) of
            {'ok', _} ->
            ...
            {'error', 'not_found'} ->
                'true';
            {'error', _R} ->
                lager:debug("failed to open account definition ~s: ~p", [
                    cb_context:account_id(Context), _R
                ]),
                'false'
        end,
    case Removed of
