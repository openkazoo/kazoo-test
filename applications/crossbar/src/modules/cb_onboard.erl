...
validate(Context, ?HTTP_PUT) ->
    ...
    Generators = [
        fun(R) -> create_extensions(JObj, Context, R) end,
    ...
    case kz_json:is_empty(Failures) of
        'true' ->
            cb_context:setters(Context, [
                {fun cb_context:set_doc/2, lists:flatten(P)},
...
%% @end
%%------------------------------------------------------------------------------
-spec create_extensions(kz_json:object(), cb_context:context(), {
...
create_extensions([Exten | Extens], Iteration, Context, {PassAcc, FailAcc}) ->
    Generators = [
        fun(R) -> create_exten_callflow(Exten, Iteration, Context, R) end,
    ...
    case kz_json:is_empty(F) of
        ...
        'false' ->
            Failures = kz_json:set_value(
                [<<"extensions">>, kz_term:to_binary(Iteration)], F, FailAcc
            ),
...
%% @end
%%------------------------------------------------------------------------------
-spec create_phone_numbers(kz_json:object(), cb_context:context(), {
    kz_term:proplist(), kz_json:object()
}) ->
    {kz_term:proplist(), kz_json:object()}.
create_phone_numbers(JObj, Context, Results) ->
    ...
    lists:foldr(
        ...
        Results,
        kz_json:get_keys(PhoneNumbers)
    ).

create_phone_number(Number, Properties, Context, {Pass, Fail}) ->
    Payload = [
        cb_context:setters(Context, [
            {fun cb_context:set_req_data/2, Properties},
    ...
    case cb_context:response(Context1) of
        {'ok', _} ->
            {[{<<"phone_numbers">>, cb_context:store(Context1, 'number', Number)} | Pass], Fail};
        {'error', {_, _, Errors}} ->
            {Pass, kz_json:set_value(<<"phone_numbers">>, Errors, Fail)}
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec create_braintree_cards(kz_json:object(), cb_context:context(), {
    kz_term:proplist(), kz_json:object()
}) -> {kz_term:proplist(), kz_json:object()}.
create_braintree_cards(JObj, Context, {Pass, Fail}) ->
    ...
    case kz_doc:id(Account) of
        'undefined' ->
            Error = kz_json:set_value(
                [<<"account_id">>, <<"required">>], <<"account failed validation">>, kz_json:new()
            ),
        ...
        AccountId ->
            ...
            Payload = [
                cb_context:setters(Context, [
                    {fun cb_context:set_req_data/2, NewReaData},
            ...
            case cb_context:response(Context1) of
                ...
                {'error', {_, _, Errors}} ->
                    {Pass, kz_json:set_value(<<"braintree">>, Errors, Fail)}
            end
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec create_user(kz_json:object(), pos_integer(), cb_context:context(), {
    kz_term:proplist(), kz_json:object()
}) ->
    {kz_term:proplist(), kz_json:object()}.
create_user(JObj, Iteration, Context, {Pass, Fail}) ->
    ...
    case cb_context:response(Context1) of
        {'ok', _} ->
            {[{<<"users">>, cb_context:store(Context1, 'iteration', Iteration)} | Pass], Fail};
        {'error', {_, _, Errors}} ->
            {Pass, kz_json:set_value(<<"users">>, Errors, Fail)}
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec create_device(kz_json:object(), pos_integer(), cb_context:context(), {
    kz_term:proplist(), kz_json:object()
}) ->
    {kz_term:proplist(), kz_json:object()}.
create_device(JObj, Iteration, Context, {Pass, Fail}) ->
    ...
    Generators = [
        ...
        fun(J) ->
            case kz_json:get_ne_value(<<"name">>, J) of
                'undefined' ->
                    ...
                    LastName = kz_json:get_value(
                        <<"last_name">>, User, kz_term:to_binary(Iteration)
                    ),
        ...
        fun(J) ->
            case kzd_devices:sip_username(J) of
                'undefined' ->
                    Strength = kapps_config:get_integer(
                        ?OB_CONFIG_CAT, <<"device_username_strength">>, 3
                    ),
                    kzd_devices:set_sip_username(
                        J, list_to_binary(["user_", kz_binary:rand_hex(Strength)])
                    );
                _ ->
                    J
            end
        ...
        fun(J) ->
            case kzd_devices:sip_password(J) of
                'undefined' ->
                    Strength = kapps_config:get_integer(
                        ?OB_CONFIG_CAT, <<"device_pwd_strength">>, 6
                    ),
    ...
    case cb_context:response(Context1) of
        {'ok', _} ->
            {[{<<"devices">>, cb_context:store(Context1, 'iteration', Iteration)} | Pass], Fail};
        {'error', {_, _, Errors}} ->
            {Pass, kz_json:set_value(<<"devices">>, Errors, Fail)}
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec create_vmbox(kz_json:object(), pos_integer(), cb_context:context(), {
    kz_term:proplist(), kz_json:object()
}) ->
    {kz_term:proplist(), kz_json:object()}.
create_vmbox(JObj, Iteration, Context, {Pass, Fail}) ->
    ...
    Generators = [
        ...
        fun(J) ->
            case kz_json:get_ne_value(<<"mailbox">>, J) of
                'undefined' ->
                    StartExten = kapps_config:get_integer(
                        ?OB_CONFIG_CAT, <<"default_vm_start_exten">>, 3 * ?MILLISECONDS_IN_SECOND
                    ),
    ...
    case cb_context:response(Context1) of
        {'ok', _} ->
            {[{<<"vmboxes">>, cb_context:store(Context1, 'iteration', Iteration)} | Pass], Fail};
        {'error', {_, _, Errors}} ->
            {Pass, kz_json:set_value(<<"vmboxes">>, Errors, Fail)}
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec create_exten_callflow(kz_json:object(), pos_integer(), cb_context:context(), {
    kz_term:proplist(), kz_json:object()
}) ->
    {kz_term:proplist(), kz_json:object()}.
create_exten_callflow(JObj, Iteration, Context, {Pass, Fail}) ->
    ...
    Generators = [
        fun(J) ->
            ...
            DefaultFlow = kapps_config:get_string(
                ?OB_CONFIG_CAT, <<"default_extension_callflow">>, ?DEFAULT_FLOW
            ),
            Flow = kz_json:decode(
                io_lib:format(DefaultFlow, [
                    kz_doc:id(User),
                    kz_doc:id(VMBox)
                ])
            ),
        ...
        fun(J) ->
            case
                [
                    Num
                 || Num <- kz_json:get_ne_value(<<"numbers">>, J, []),
            ...
            of
                [] ->
                    ...
                    kz_json:set_value(
                        <<"numbers">>, [kz_term:to_binary(StartExten + Iteration)], J
                    );
                Numbers ->
                    kz_json:set_value(<<"numbers">>, Numbers, J)
            end
        end
    ],
    ...
    case cb_context:response(Context1) of
        {'ok', _} ->
            {[{<<"callflows">>, cb_context:store(Context1, 'iteration', Iteration)} | Pass], Fail};
        {'error', {_, _, Errors}} ->
            {Pass, kz_json:set_value(<<"callflows">>, Errors, Fail)}
    end.
...
populate_new_account(Props, _) ->
    ...
    Context1 = crossbar_bindings:fold(<<"*.execute.put.accounts">>, [
        ...
        {'ok', _} ->
            ...
            case kz_json:get_ne_value(<<"errors">>, Results) of
                'undefined' ->
                    ...
                    cb_context:set_doc(
                        Context1, kz_json:set_value(<<"account_id">>, AccountId, Results)
                    );
                Failures ->
                    lager:debug("account creation errors: ~p", [Failures]),
                    catch (crossbar_bindings:fold(<<"*.execute.delete.accounts">>, [
                        Context1, AccountId
                    ])),
                    Ctx2 = cb_context:setters(Context1, [
                        {fun cb_context:set_doc/2, kz_json:delete_key(<<"owner_id">>, Results)},
                        {fun cb_context:set_account_id/2, 'undefined'},
                        {fun cb_context:set_account_db/2, 'undefined'},
...
-spec populate_new_account(kz_term:proplist(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
    ...
    case cb_context:response(Context1) of
        ...
        {'error', {_, _, Errors}} ->
            populate_new_account(
                ...
                AccountDb,
                kz_json:set_value(
                    [<<"errors">>, Event, kz_term:to_binary(Iteration)], Errors, Results
                )
            )
    end.
...
prepare_props(Props) ->
    lists:sort(
        fun
            ({<<"braintree">>, _}, {_, _}) -> 'true';
            (_, _) -> 'false'
        end,
        props:delete(?KZ_ACCOUNTS_DB, Props)
    ).
            ...
            case kz_datamgr:save_doc(?KZ_TOKEN_DB, kz_json:from_list(Token)) of
                {'ok', Doc} ->
                    ...
                    crossbar_util:response(
                        kz_json:set_value(<<"auth_token">>, AuthToken, JObj), Context1
                    );
                {'error', R} ->
