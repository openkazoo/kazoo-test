...
merge_responses([JObj | JObjs], Regs) -> merge_responses(JObjs, merge_response(JObj, Regs)).
...
normalize_registration(JObj) ->
    ...
    [#uri{scheme = Scheme, domain = Domain, port = Port, ext_opts = Opts} = _Uri] = kzsip_uri:uris(
        OriginalContact
    ),
    Transport = props:get_binary_value(<<"transport">>, Opts),
    Updaters = [
        fun(J) -> kz_json:delete_keys(?MASK_REG_FIELDS, J) end,
        fun(J) ->
            Values = [
                {<<"Contact-IP">>, Domain},
...
count_registrations(Context) ->
    ...
    case ReqResp of
        {'error', _E} ->
            lager:debug("no resps found: ~p", [_E]),
            0;
        {'ok', JObj} ->
            kz_json:get_integer_value(<<"Count">>, JObj, 0);
        {'timeout', _} ->
            lager:debug("timed out query for counting regs"),
            0
    end.
