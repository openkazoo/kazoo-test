        ...
        {ok, JObjs} ->
            ...
            lists:foldr(
                fun(JObj, Contacts) ->
                    ...
                    case
                        kz_term:is_empty(Includes) orelse
                            lists:member(Key, Includes)
                    of
                        'false' ->
                            Contacts;
                        'true' ->
...
maybe_append_module(Contacts, _) ->
    lists:map(
        fun
            (#contact{first_module = undefined} = Contact) ->
                Contact;
...
maybe_update_contact(_, _, _, Contact) ->
    Contact.
...
maybe_update_contact(_, _, Contact) ->
    Contact.
