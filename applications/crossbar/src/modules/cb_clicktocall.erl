...
-define(CONNECT_C2C_URL, [
    {<<"clicktocall">>, [_, ?CONNECT_CALL]},
    {?KZ_ACCOUNTS_DB, [_]}
]).
...
init() ->
    Bindings = [
        {<<"*.allowed_methods.clicktocall">>, 'allowed_methods'},
...
authenticate(Context) ->
    is_c2c_url(Context, cb_context:req_nouns(Context)) andalso
...
is_c2c_url(Context, ?CONNECT_C2C_URL) ->
    ...
    (Verb =:= ?HTTP_GET) orelse
        (Verb =:= ?HTTP_POST);
is_c2c_url(_Context, _Nouns) ->
    'false'.
...
migrate_history(AccountId, AccountDb, C2C) ->
    case kz_json:get_list_value(<<"pvt_history">>, C2C, []) of
        ...
        History ->
            ...
            lager:debug(
                "removed history from c2c ~s in ~s: ~p",
                [Id, AccountId, _Resp]
            )
    end.
...
save_history_item(AccountId, HistoryItem, C2CId) ->
    ...
    JObj = kz_doc:update_pvt_parameters(
        kz_json:set_value(<<"pvt_clicktocall_id">>, C2CId, HistoryItem),
        AccountModb,
        [
            {'type', <<"c2c_history">>},
...
-spec originate_call(kz_term:ne_binary(), cb_context:context(), kz_term:api_binary()) ->
    cb_context:context().
...
-spec originate_call(kz_term:ne_binary(), cb_context:context(), kz_term:api_binary(), boolean()) ->
    cb_context:context().
...
originate_call(C2CId, Context, Contact, 'true') ->
    ...
    do_originate_call(
        C2CId, Context, Contact, Request, cb_context:req_value(Context, <<"blocking">>, 'false')
    ).
...
do_originate_call(C2CId, Context, Contact, Request, 'false') ->
    ...
    crossbar_util:response_202(
        <<"processing request">>, JObj, cb_context:set_resp_data(Context, Request)
    );
...
handle_response(C2CId, Context, Contact, Resp) ->
    ...
    HistoryItem = kz_doc:update_pvt_parameters(
        crossbar_doc:update_pvt_parameters(JObj, Context),
        Modb,
        Options
    ),
    _ = kazoo_modb:save_doc(AccountId, HistoryItem).

-spec do_originate_call(
    kz_term:ne_binary(), cb_context:context(), kz_term:api_binary(), kz_term:proplist()
) ->
    {'ok', kz_json:object()}
...
handle_originate_resp({'ok', [Resp | _]}) ->
    AppResponse = kz_json:get_first_defined(
        [
...
handle_originate_resp({'returned', _JObj, Return}) ->
    case {kz_json:get_value(<<"code">>, Return), kz_json:get_value(<<"message">>, Return)} of
        {312, _Msg} ->
...
build_originate_req(Contact, Context) ->
    ...
    CalleeNumber = knm_converters:normalize(
        kzd_clicktocall:outbound_callee_id_number(C2CDoc, Exten)
    ),
    ...
    {Caller, Callee} = get_caller_callee(
        DialFirst,
        #contact{
...
get_c2c_contact(Contact) -> knm_converters:normalize(kz_http_util:urlencode(Contact)).

-spec create_c2c_history_item(
    {'success' | 'error', kz_json:object()}, kz_term:ne_binary(), kz_term:ne_binary()
) -> kz_term:proplist().
