...
-spec additional_assignment_validations(
    cb_context:context(), kz_term:proplist(), kz_term:objects(), non_neg_integer()
) ->
    cb_context:context().
...
additional_assignment_validations(
    Context, [{Address, {'error', 'not_found'}} | IPs], Assign, Index
) ->
...
assign_ips(Context, [JObj | JObjs]) ->
    Context1 =
        case kz_ip:assign(cb_context:account_id(Context), JObj) of
            {'ok', AssignedJObj} ->
                Resp = kz_json:set_value(
                    [<<"success">>, kz_doc:id(JObj)],
...
-spec additional_release_validations(
    cb_context:context(), kz_term:proplist(), kz_json:objects(), non_neg_integer()
) ->
    cb_context:context().
