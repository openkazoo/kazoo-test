...
-define(NOTIFY_SUPER_ADMIN_ONLY, [
    <<"system_alert">>,
    <<"webhook_disabled">>
]).
...
maybe_revise_schema({'ok', HookIds}) ->
    case kz_json_schema:load(<<"webhooks">>) of
        ...
        {'error', _E} -> lager:warning("failed to find webhooks schema: ~p", [_E])
    end.
...
-spec get_available_hook_ids(kz_term:ne_binary()) ->
    {'ok', kz_term:ne_binaries()} | kazoo_data:data_error().
...
authenticate(Context, ?HTTP_GET, [{<<"webhooks">>, [?PATH_TOKEN_SAMPLES | _]}]) ->
    ...
    {'true', Context};
authenticate(_Context, _Verb, _Nouns) ->
    'false'.
...
validate(Context) ->
    validate_webhooks(
        cb_context:set_account_db(Context, ?KZ_WEBHOOKS_DB), cb_context:req_verb(Context)
    ).
...
validate(Context, Id) ->
    validate_webhook(
        cb_context:set_account_db(Context, ?KZ_WEBHOOKS_DB), Id, cb_context:req_verb(Context)
    ).
...
validate_patch(Context, WebhookId) ->
    case cb_context:resp_status(Context) of
        'success' ->
            ...
            OnSuccess = fun(C) ->
                check_modifiers(
                    crossbar_doc:load_merge(WebhookId, C, ?TYPE_CHECK_OPTION(kzd_webhook:type()))
                )
            end,
            cb_context:validate_request_data(
                <<"webhooks">>, cb_context:set_req_data(Context, JObj), OnSuccess
            );
        _Status ->
            Context
    end.
...
get_available_hook_samples() ->
    [
     ...
     || Path <- filelib:wildcard(
            filename:join(code:priv_dir('crossbar'), "webhooks_*-samples.json")
        )
    ].
...
-spec maybe_filter_non_admin_hooks(
    cb_context:context(), kz_term:ne_binary(), kz_json:object(), kz_json:objects()
) -> kz_json:objects().
maybe_filter_non_admin_hooks(_, <<"webhooks_skel">>, _, Acc) ->
    Acc;
...
-spec maybe_filter_non_admin_notifications(cb_context:context(), kz_json:object()) ->
    kz_json:object().
maybe_filter_non_admin_notifications(Context, JObj) ->
    ...
    Filtered = kz_json:filter(
        fun(KV) -> filter_non_admin_notifications(KV, IsSuperAdmin) end, NotifyTypes
    ),
...
-spec check_modifiers(cb_context:context(), kz_json:object(), kz_term:api_object()) ->
    cb_context:context().
...
-spec check_modifiers(
    kz_json:object(), kz_term:ne_binary(), kz_json:object(), cb_context:context()
) -> cb_context:context().
...
handle_resp(Context, {'returned', _Request, BasicReturn}) ->
    ...
    crossbar_util:response(
        'error', <<"The backend is not configured for this request">>, 500, Resp, Context
    );
