...
should_send_to_phonebook(Context) ->
    ...
    phonebook_enabled() andalso
        cb_context:fetch(Context, 'port_authority_id', MasterId) =:= MasterId andalso
        not req_from_phonebook(Context).
...
add_comment(JObj, AuthToken, Comment) ->
    Url = phonebook_uri([
        <<"accounts">>, kz_doc:account_id(JObj), <<"ports">>, <<"in">>, kz_doc:id(JObj), <<"notes">>
    ]),
    Data = kz_json:set_value(<<"data">>, Comment, kz_json:new()),
    make_request('add_comment', JObj, AuthToken, Url, Data).

-spec make_request(req_type(), kz_json:object(), kz_term:ne_binary(), string(), kz_json:object()) ->
    response().
...
handle_resp(Type, Error) ->
    ...
    {'error', #{
        ...
        payload => kz_json:from_list([
            {<<"message">>, <<"failed to connect to port automation system">>},
