...
-define(UBIQUITI_AUTH_TOKENS,
    kapps_config:get_integer(?U_CONFIG_CAT, <<"tokens_per_request">>, 35)
).
...
-spec init() -> ok.
init() ->
    _ProdURI = kapps_config:get_ne_binary(
        ?U_CONFIG_CAT, [?SSO_PROD_ENV, ?SSO_URL_KEY], ?SSO_PROD_URI
    ),
    _StagURI = kapps_config:get_ne_binary(
        ?U_CONFIG_CAT, [?SSO_STAGING_ENV, ?SSO_URL_KEY], ?SSO_STAGING_URI
    ),
    ...
    case cb_context:resp_status(Context1) of
        'success' ->
            cb_context:validate_request_data(
                <<"ubiquiti_auth">>, Context, fun maybe_authenticate_user/1
            );
        _Status ->
            Context1
    end.
    ...
    of
        {'ok', 200, RespHeaders, RespBody} ->
            ...
            cb_context:setters(Context, [
                {fun cb_context:set_doc/2, auth_response(RespHeaders, RespBody)},
        ...
        {'error', _Error} ->
            ...
            crossbar_util:response(
                'error', <<"failed to query Ubiquiti SSO service">>, 500, Context
            )
    end.
