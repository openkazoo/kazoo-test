...
-define(RESPONDERS, [{{?MODULE, 'handle_job'}, {<<"delegate">>, <<"job">>}}]).
...
publish(AccountId, JobId, ReqId) ->
    Work = kz_json:from_list([
        {<<"Account-ID">>, AccountId},
        {<<"Job-ID">>, JobId}
    ]),
    kz_amqp_worker:cast(
        [
            {<<"Delegate-Message">>, Work},
            {<<"Msg-ID">>, ReqId}
...
handle_job(JObj, _Props) ->
    ...
    kz_util:put_callid(
        kz_json:get_first_defined([<<"Msg-ID">>, [<<"Delegate-Message">>, <<"Job-ID">>]], JObj)
    ),
    Job = kz_json:get_value(<<"Delegate-Message">>, JObj),
    process_job(
        kz_json:get_value(<<"Account-ID">>, Job),
...
select_carrier_module(Job) ->
    ...
    case kz_datamgr:open_cache_doc(?KZ_OFFNET_DB, ResourceId) of
        {'ok', _} ->
            lager:debug(
                "found resource ~s in ~s, using ~s",
                [ResourceId, ?KZ_OFFNET_DB, ?CARRIER_OTHER]
            ),
            ?CARRIER_OTHER;
        {'error', _E} ->
            lager:debug(
                "resource ~s is not a system resource(~p), using ~s",
                [ResourceId, _E, ?CARRIER_LOCAL]
            ),
            ?CARRIER_LOCAL
    end.

-spec maybe_create_number(
    kz_json:object(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:api_binary(),
...
create_number(Job, AccountId, AuthAccountId, CarrierModule, DID) ->
    ...
    try knm_number:create(DID, Options) of
        {'ok', Number} ->
            ...
            lager:debug(
                "successfully created number ~s for account ~s",
                [knm_phone_number:number(PhoneNumber), AccountId]
            ),
            update_status(
                kz_json:set_value(
                    [?KEY_SUCCESS, knm_phone_number:number(PhoneNumber)],
        ...
        E:_R ->
            ...
            lager:debug(
                "exception creating number ~s for account ~s: ~s: ~p",
                [DID, AccountId, E, _R]
            ),
            update_status(
                kz_json:set_value(
...
update_with_failure(Job, AccountId, Number, Failure, JObj) ->
    lager:debug("failed to create number ~s for account ~s: ~p ~p", [
        Number, AccountId, Failure, JObj
    ]),
    case
        kz_json:is_json_object(JObj) andalso
            kz_json:get_values(JObj)
    of
        {[V], [K]} ->
            update_status(
                kz_json:set_value(
        ...
        _ ->
            update_status(
                kz_json:set_value([<<"errors">>, Number], JObj, Job),
                <<"running">>
            )
    end.
...
update_status(Job, Status) ->
    {'ok', Job1} = kz_datamgr:save_doc(
        kz_doc:account_db(Job),
        kz_json:set_values(
            [
                {<<"pvt_status">>, Status},
...
maybe_recover_account_jobs(Year, Month, AccountId) ->
    ...
    case
        kz_datamgr:get_results(Modb, <<"resources/status_listing">>, [
            {'keys', [
                <<"pending">>,
                <<"running">>
            ]},
    ...
    of
        {'ok', []} -> 'ok';
        {'ok', IncompleteJobs} -> maybe_recover_incomplete_jobs(IncompleteJobs);
        {'error', _} -> 'ok'
    end.
...
maybe_recover_incomplete_job(Job) ->
    ...
    case (Now - kz_doc:modified(Job)) > ?RECOVERY_THRESHOLD_S of
        ...
        'true' ->
            lager:debug(
                "job ~s in ~s is old and incomplete, attempting to restart it",
