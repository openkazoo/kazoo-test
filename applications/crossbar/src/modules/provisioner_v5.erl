...
update_account(AccountId, JObj, AuthToken) ->
    Settings = [
        {<<"lines">>, [[{<<"sip">>, [{<<"realm">>, kzd_accounts:realm(JObj)}]}]]},
...
update_account(Account, AuthToken) ->
    ...
    case kzd_accounts:fetch(AccountId) of
        ...
        {'error', _R} -> lager:debug("unable to fetch account ~s: ~p", [AccountId, _R])
    end.
...
-spec maybe_save_device(
    kz_json:object(), kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    'ok' | {'EXIT', _}.
maybe_save_device(Device, Settings, AccountId, AuthToken) ->
...
-spec save_device(kz_term:ne_binary(), kz_json:object(), kz_json:object(), kz_term:ne_binary()) ->
    'ok'.
save_device(AccountId, Device, Request, AuthToken) ->
    case kzd_devices:mac_address(Device) of
        'undefined' -> 'ok';
        MacAddress -> send_req('devices_post', Request, AuthToken, AccountId, MacAddress)
    end.
...
%% kzd_users:name/1 possible result
is_empty_display_name(<<" ">>) -> 'true';
...
settings_sip(DeviceDoc) ->
    ...
    Props = [
        {<<"username">>, kzd_devices:sip_username(DeviceDoc)},
...
get_label(Doc) ->
    case
        {
            kz_json:get_ne_binary_value(<<"first_name">>, Doc),
...
-spec get_feature_key(
    kz_term:ne_binary(),
    kz_term:api_ne_binary() | 0..10 | kz_json:object(),
    binary(),
    binary(),
    kz_term:ne_binary(),
    kz_json:object()
...
-spec get_feature_key_type(kz_json:object(), kz_term:ne_binary(), binary(), binary()) ->
    kz_term:api_object().
...
-spec maybe_add_feature_key(kz_term:ne_binary(), kz_term:api_object(), kz_json:object()) ->
    kz_json:object().
maybe_add_feature_key(_Key, 'undefined', JObj) ->
    JObj;
...
-spec format_codecs(kz_term:ne_binaries(), kz_term:ne_binaries(), kz_json:object()) ->
    kz_json:object().
format_codecs(_Codecs, [], JObj) ->
    JObj;
...
%%------------------------------------------------------------------------------
-spec send_req(
    atom(), kz_term:api_object(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_binary()
) -> 'ok'.
...
create_alert(JObj, AccountId, AuthToken) ->
    ...
    To = [
        ...
        kz_json:from_list([
            {<<"type">>, kz_services_reseller:get_id(AccountId)},
            {<<"value">>, <<"admins">>}
        ])
    ],
