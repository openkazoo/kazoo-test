...
-export([
    ...
    create_json_chunk_response/2,
    create_csv_chunk_response/2,
...
%% read body in 8MB chunks
-define(READ_BODY_LENGTH, 8000000).
...
-define(ENVELOPE_SCHEMA,
    kz_json:from_list([{<<"properties">>, kz_json:from_list([{<<"data">>, ?DATA_SCHEMA}])}])
).
...
-type pull_file_response_return() ::
    {resp_file(), cowboy_req:req(), cb_context:context()}
    | stop_return().

-export_type([
    pull_file_response_return/0,
    stop_return/0
]).
...
is_cors_preflight(Req) ->
    is_cors_request(Req) andalso
        ?HTTP_OPTIONS =:= cowboy_req:method(Req).
...
is_cors_request(_Req, []) ->
    'false';
is_cors_request(Req, [ReqHdr | ReqHdrs]) ->
    'undefined' =/= cowboy_req:header(ReqHdr, Req) orelse
        is_cors_request(Req, ReqHdrs).
...
add_cors_headers(Req, Context) ->
    ...
    Allow =
        case
            kz_term:is_empty(ReqMethod) orelse
                lists:member(ReqMethod, Methods)
        of
...
get_cors_headers(Allow) ->
    [
        ...
        {<<"access-control-allow-headers">>,
            <<"content-type, depth, user-agent, x-http-method-override, x-file-size, x-requested-with, if-modified-since, x-file-name, cache-control, x-auth-token, x-kazoo-cluster-id, if-match">>},
        {<<"access-control-expose-headers">>,
            <<"content-type, x-auth-token, x-request-id, x-kazoo-cluster-id, location, etag, etag">>},
...
handle_failed_multipart(Context, Req0, QS) ->
    case get_request_body(Req0) of
        {'ok', ReqBody, Req1} ->
            try
                handle_url_encoded_body(Context, Req1, QS, ReqBody, get_url_encoded_body(ReqBody))
            catch
                _E:_R ->
...
-spec handle_url_encoded_body(
    cb_context:context(), cowboy_req:req(), kz_json:object(), binary(), kz_json:object()
) ->
    {cb_context:context(), cowboy_req:req()}
    | stop_return().
handle_url_encoded_body(Context, Req, QS, ReqBody, JObj) ->
    case kz_json:get_values(JObj) of
        {['true'], [_JSON]} ->
            lager:debug(
                "failed to parse url-encoded request body, but we'll give json a go on ~p", [_JSON]
            ),
...
-spec set_request_data_in_context(
    cb_context:context(), cowboy_req:req(), kz_term:api_object(), kz_json:object()
) ->
    {cb_context:context(), cowboy_req:req()}
    | stop_return().
    ...
    case is_valid_request_envelope(JObj, Context) of
        ...
        Errors ->
            ...
            ?MODULE:stop(
                Req,
                cb_context:failed(
                    cb_context:set_resp_error_msg(Context, <<"invalid request envelope">>),
                    Errors
                )
            )
    end.
...
set_valid_data_in_context(Context, JObj, QS) ->
    Setters = [
        {fun cb_context:set_req_json/2, JObj},
...
-type cowboy_multipart_response() ::
    {'ok', cow_multipart:headers(), cowboy_req:req()}
    | {'done', cowboy_req:req()}
    | cowboy_req:req().
...
handle_file_contents(Context, ContentType, Req, FileContents) ->
    ...
    case maps:get(TransferKey, MultiPartHeaders, cowboy_req:header(TransferKey, Req)) of
        ...
        _Else ->
            ...
            ContentLength = maps:get(
                <<"content-length">>, MultiPartHeaders, cowboy_req:header(<<"content-length">>, Req)
            ),
            FileHeaders = kz_json:from_list([
                {<<"content_type">>, ContentType},
                {<<"content_length">>, ContentLength}
            ]),
            FileJObj = kz_json:from_list([
                {<<"headers">>, FileHeaders},
...
get_request_body(Req) ->
    ...
    try
        get_request_body(Req, Params#{read_fun => fun cowboy_req:read_part_body/2})
    catch
        'error':{'badmatch', _} ->
...
get_request_body(_, <<>>, {'ok', <<>>, Req1}) ->
    ...
    {'ok', <<>>, Req1};
get_request_body(#{max_size := MaxSize}, Body, {'more', Data, Req1}) when
    size(Body) + size(Data) > MaxSize
->
    {'error', 'max_size', Req1};
...
-type get_json_return() ::
    {kz_term:api_object(), cowboy_req:req()}
...
decode_json_body(ReqBody, Req) ->
    try kz_json:unsafe_decode(ReqBody) of
        ...
        'throw':{'invalid_json', {'error', {ErrLine, ErrMsg}}, _JSON} ->
            ...
            {
                {'malformed', <<
                    (kz_term:to_binary(ErrMsg))/binary,
                    " around ",
                    (kz_term:to_binary(ErrLine))/binary
                >>},
                Req
            };
        _E:_R ->
...
-spec normalize_envelope_keys_foldl(kz_json:path(), kz_json:json_term(), kz_json:object()) ->
    kz_json:object().
...
%% @end
%%------------------------------------------------------------------------------
-spec is_valid_request_envelope(kz_json:object(), cb_context:context()) ->
...
-spec parse_path_tokens(cb_context:context(), kz_json:path(), cb_mods_with_tokens()) ->
    cb_mods_with_tokens()
    | {'stop', cb_context:context()}.
parse_path_tokens(_Context, [], Events) ->
    Events;
parse_path_tokens(_Context, [<<>>], Events) ->
    Events;
...
is_cb_module([Module | Modules]) ->
    'true' =:= kz_module:is_exported(Module, 'init', 0) orelse
        is_cb_module(Modules).
...
allow_methods(Responses, ReqVerb, HttpVerb) ->
    case crossbar_bindings:succeeded(Responses) of
        [] ->
            [];
        Succeeded ->
            AllowedSet = lists:foldr(
                fun allow_methods_fold/2,
                sets:new(),
                Succeeded
            ),
    ...
    case cowboy_req:header(<<"x-pretty-print">>, Req) of
        'undefined' ->
            case cb_context:req_value(Context, <<"pretty_print">>) of
                'undefined' ->
                    {Req, cb_context:set_pretty_print(Context, 'false')};
                Value ->
...
is_known_content_type(Req0, Context0, _ReqVerb) ->
    ...
    is_known_content_type(
        Req0, Context1, ensure_content_type(CT), cb_context:content_types_accepted(Context1)
    ).
...
content_type_matches({Type, SubType, Opts}, {Type, SubType, ModOpts}) ->
    lists:all(
        fun({K, V}) -> props:get_value(K, Opts) =:= V end,
        ModOpts
    );
...
execute_request(Req, Context, Mod, Params, Verb) ->
    ...
    Event = create_event_name(Context1, [
        <<"execute">>,
...
maybe_cleanup_file(Context) ->
    ...
    case
        kz_term:is_not_empty(File) andalso
            kz_term:is_true(CleanupRespFile)
    of
        'false' -> 'ok';
...
final_csv_resp_type(Context, 'false') ->
    case
        cb_context:fetch(Context, 'is_chunked', 'false') andalso
            not cb_context:fetch(Context, 'chunk_is_file', 'false')
    of
        'true' -> 'is_chunked';
        'false' -> 'binary'
    end.
...
-spec create_csv_resp_content_from_csv_acc(
    cowboy_req:req(), cb_context:context(), 'undefined' | kz_csv:file_return()
) ->
    {resp_file(), cowboy_req:req(), cb_context:context()}.
...
create_csv_resp_content_from_csv_acc(Req, Context, {_File, _} = CSVAcc) ->
    ...
    Headers = #{
        <<"content-type">> => maps:get(<<"content-type">>, ContextHeaders, <<"text/csv">>),
        <<"content-disposition">> => maps:get(
            <<"content-disposition">>, ContextHeaders, ContentDisposition
        )
    },
    {
        {'file', File},
...
-spec maybe_create_empty_csv_resp(
    cowboy_req:req(), cb_context:context(), 'binary' | 'is_chunked' | 'to_csv'
) ->
...
create_json_chunk_response(Req, Context) ->
    ...
    create_json_chunk_response(
        Req, Context, JObjs, cb_context:fetch(Context, 'chunking_started', 'false')
    ).

-spec create_json_chunk_response(
    cowboy_req:req(), cb_context:context(), kz_json:api_objects(), boolean()
) ->
    {boolean(), cowboy_req:req(), cb_context:context()}.
...
-spec stream_or_create_csv_chunk(
    cowboy_req:req(), cb_context:context(), kz_json:objects() | kz_term:ne_binaries(), boolean()
) ->
    {boolean(), cowboy_req:req(), cb_context:context()}.
...
-spec maybe_init_csv_chunk_stream(cowboy_req:req(), cb_context:context(), boolean()) ->
    cowboy_req:req().
...
-spec init_chunk_stream(cowboy_req:req(), cb_context:context(), kz_term:ne_binary()) ->
    cowboy_req:req().
...
init_chunk_stream(Req, Context, <<"to_csv">>) ->
    ...
    Headers = #{
        <<"content-type">> => maps:get(<<"content-type">>, ContextHeaders, <<"text/csv">>),
        <<"content-disposition">> => maps:get(
            <<"content-disposition">>, ContextHeaders, DefaultDisposition
        )
    },
...
csv_body(Context, JObjs) when is_list(JObjs) ->
    ...
    Setters = [
        {fun cb_context:store/3, 'csv_acc', Acc1},
...
-spec maybe_file_pull_response(
    kz_term:text() | resp_file(), cowboy_req:req(), cb_context:context()
) ->
    {kz_term:text() | 'stop', cowboy_req:req(), cb_context:context()}.
...
do_create_resp_envelope(Context) ->
    ...
    encode_start_keys(
        kz_json:set_values(Resp, cb_context:resp_envelope(Context)),
...
close_chunk_json_envelope(Req, Context) ->
    ...
    Encoded = [
        <<"\"", K/binary, "\":\"", (kz_term:to_binary(V))/binary, "\"">>
     || {K, V} <- Trailer,
    ...
    lists:foldl(
        fun(Key, JObj) ->
            case kz_json:get_value(Key, JObj) of
                'undefined' -> JObj;
                Value -> kz_json:set_value(Key, encode_start_key(Value), JObj)
            end
        end,
        Resp,
...
decode_start_key(Encoded) ->
    try
        erlang:binary_to_term(kz_base64url:decode(Encoded))
    catch
        _:_ -> Encoded
    end.
...
-spec create_event_name(cb_context:context(), kz_term:ne_binary() | kz_term:ne_binaries()) ->
    kz_term:ne_binary().
