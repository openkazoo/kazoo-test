...
is_ip_unique(IP, DeviceId) ->
    is_ip_acl_unique(IP, DeviceId) andalso
...
is_ip_unique(JObj, IP, DeviceId) ->
    case kz_json:get_value(?AUTHZ_ID, JObj) of
        DeviceId -> 'true';
        _AuthorizingId -> not (kz_network_utils:verify_cidr(IP, kz_json:get_value(<<"ip">>, JObj)))
    end.
...
get_all_acl_ips() ->
    ...
    case Resp of
        {'error', _} -> [];
        {'ok', JObj} -> extract_all_ips(kapi_sysconf:get_value(JObj, kz_json:new()))
    end.
