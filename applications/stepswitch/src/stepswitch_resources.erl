...
get_props(_ResourceId, 'undefined') ->
    'undefined';
...
sort_resources(Resources) ->
    lists:sort(
        fun(#resrc{weight = W1}, #resrc{weight = W2}) ->
            W1 =< W2
        end,
        Resources
    ).
...
-spec maybe_get_local_endpoints(
    kz_term:ne_binary(), kz_term:ne_binary(), kapi_offnet_resource:req()
) -> endpoints().
...
-spec get_local_endpoints(kz_term:ne_binary(), kz_term:ne_binary(), kapi_offnet_resource:req()) ->
    endpoints().
...
-spec maybe_find_local(
    kz_term:api_binary(), kz_term:api_integer(), kz_term:api_binary(), kz_term:api_binary()
) ->
    {'ok', kz_term:proplist()}
    | {'error', 'not_found'}.
maybe_find_local(_, _, _, 'undefined') -> {'error', 'not_found'};
maybe_find_local(IP, Port, Realm, AccountId) -> search_resources(IP, Port, Realm, get(AccountId)).

-spec search_resources(
    kz_term:api_binary(), kz_term:api_integer(), kz_term:api_binary(), resources()
) ->
    {'ok', kz_term:proplist()}
    | {'error', 'not_found'}.
search_resources(_IP, _Port, _Realm, []) ->
    ...
    {'error', 'not_found'};
search_resources(IP, Port, Realm, [
    #resrc{
        id = Id,
        gateways = Gateways,
        global = Global
...
-spec search_gateways(
    kz_term:api_binary(), kz_term:api_integer(), kz_term:api_binary(), gateways()
) ->
    gateway()
    | {'error', 'not_found'}.
search_gateways(_, _, _, []) ->
    {'error', 'not_found'};
...
resource_has_flag(Flag, #resrc{flags = ResourceFlags, id = _Id}) ->
    case
        kz_term:is_empty(Flag) orelse
            lists:member(Flag, ResourceFlags)
    of
        'true' ->
            'true';
        'false' ->
...
resources_to_endpoints(Resources, Number, OffnetJObj) ->
    ...
    case maps:get(Classification, ResourceMap, 'undefined') of
        'undefined' ->
            ...
            lager:debug(
                "no resources satisfy classifier look up, matching against resource rules..."
            ),
            RuleResources = maps:get('no_classification', ResourceMap, []),
            build_endpoints_from_resources(RuleResources, Number, OffnetJObj);
        ClassifiedResources ->
            lager:debug(
                "found resources to satisfy classifier ~s (number ~s), building against classification rules...",
                [Classification, Number]
            ),
            FilteredClassifiedResources = [
                Resource
             || Resource <- ClassifiedResources,
...
-spec build_endpoints_from_resources(resources(), kz_term:ne_binary(), kapi_offnet_resource:req()) ->
    kz_json:objects().
build_endpoints_from_resources(Resources, Number, OffnetJObj) ->
    build_endpoints_from_resources(Resources, Number, OffnetJObj, []).

-spec build_endpoints_from_resources(
    resources(), kz_term:ne_binary(), kapi_offnet_resource:req(), kz_json:objects()
) -> kz_json:objects().
build_endpoints_from_resources([], _Number, _OffnetJObj, Endpoints) ->
    Endpoints;
build_endpoints_from_resources([Resource | Resources], Number, OffnetJObj, Endpoints) ->
    case maybe_resource_to_endpoints(Resource, Number, OffnetJObj, Endpoints) of
        {'error', _} ->
            build_endpoints_from_resources(Resources, Number, OffnetJObj, Endpoints);
        MoreEndpoints ->
            build_endpoints_from_resources(Resources, Number, OffnetJObj, MoreEndpoints)
    end.

-spec maybe_resource_to_endpoints(
    resource(), kz_term:ne_binary(), kapi_offnet_resource:req(), kz_json:objects()
) ->
    {'error', 'no_match'}
    | kz_json:objects().
maybe_resource_to_endpoints(
    ...
    case filter_resource_by_rules(Id, Number, Rules, CallerIdNumber, CallerIdRules) of
        ...
        {'ok', NumberMatch} ->
            _MaybeClassifier =
                case kz_term:is_ne_binary(Classifier) of
                    'true' -> Classifier;
                    'false' -> <<"no_classifier">>
                end,
...
-spec maybe_add_proxies(kz_json:objects(), kz_term:proplist(), kz_json:objects()) ->
    kz_json:objects().
maybe_add_proxies([], _, Acc) ->
    Acc;
maybe_add_proxies(Endpoints, [], Acc) ->
    Acc ++ Endpoints;
...
-spec filter_resource_by_rules(
    kz_term:ne_binary(), kz_term:ne_binary(), re:mp(), kz_term:ne_binary(), re:mp()
) ->
...
-spec filter_resource_by_match(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), re:mp(), kz_term:ne_binary()
) ->
    {'ok', kz_term:ne_binary()}
    | {'error', 'no_match'}.
filter_resource_by_match(Id, Number, CallerIdNumber, CallerIdRules, Match) ->
    case evaluate_cid_rules(CallerIdRules, CallerIdNumber) of
        ...
        {'ok', _CIDMatch} ->
            lager:debug(
                "resource ~s matches number '~s' with regex match '~s' and caller id match '~s'",
                [Id, Number, Match, _CIDMatch]
            ),
            {'ok', Match};
        {'error', 'no_match'} ->
            lager:debug("resource ~s does not match caller id number '~s', skipping", [
                Id, CallerIdNumber
            ]),
            {'error', 'no_match'}
    end.
...
-spec evaluate_cid_rules(re:mp(), kz_term:ne_binary()) ->
    {'ok', kz_term:ne_binary()}
    %% empty rules, it`s ok, allow any number
    | {'ok', 'empty_rules'}
    | {'error', 'no_match'}.
...
%% @end
%%------------------------------------------------------------------------------
-spec gateways_to_endpoints(
    kz_term:ne_binary(), gateways(), kapi_offnet_resource:req(), kz_json:objects()
) ->
    kz_json:objects().
gateways_to_endpoints(_Number, [], _OffnetJObj, Endpoints) ->
    Endpoints;
gateways_to_endpoints(Number, [Gateway | Gateways], OffnetJObj, Endpoints) ->
    gateways_to_endpoints(
        Number,
        Gateways,
        OffnetJObj,
...
gateway_to_endpoint(
    ...
    RequestorCCVs = kz_json:get_ne_json_value(
        <<"Requestor-Custom-Channel-Vars">>, OffnetJObj, kz_json:new()
    ),
    ...
    kz_json:from_list(
        props:filter_empty(
            [
                ...
                {<<"Privacy-Hide-Name">>,
                    HideName orelse
                        kz_privacy:should_hide_name(OffnetJObj) orelse
                        kz_privacy:should_hide_name(RequestorCCVs)},
                {<<"Privacy-Hide-Number">>,
                    HideNumber orelse
                        kz_privacy:should_hide_number(OffnetJObj) orelse
                        kz_privacy:should_hide_number(RequestorCCVs)}
...
dynamic_sip_invite_parameters(#gateway{invite_parameters = Parameters}, OffnetJObj) ->
    CCVs = kz_json:normalize(
        kapi_offnet_resource:requestor_custom_channel_vars(OffnetJObj, kz_json:new())
    ),
    CSHs = kz_json:normalize(
        kapi_offnet_resource:requestor_custom_sip_headers(OffnetJObj, kz_json:new())
    ),
    Keys = kz_json:get_list_value(<<"dynamic">>, Parameters, []),
    dynamic_sip_invite_parameters(Keys, CCVs, CSHs).

-spec dynamic_sip_invite_parameters(kz_term:api_binaries(), kz_json:object(), kz_json:object()) ->
    kz_term:ne_binaries().
dynamic_sip_invite_parameters(Keys, CCVs, CSHs) ->
    dynamic_sip_invite_parameters(Keys, CCVs, CSHs, []).

-spec dynamic_sip_invite_parameters(
    kz_term:api_binaries(), kz_json:object(), kz_json:object(), kz_term:ne_binaries()
) -> kz_term:ne_binaries().
dynamic_sip_invite_parameters([], _, _, Parameters) ->
    Parameters;
dynamic_sip_invite_parameters([Key | Keys], CCVs, CSHs, Parameters) when is_binary(Key) ->
    case dynamic_sip_invite_value(Key, CCVs, CSHs) of
        'undefined' ->
            dynamic_sip_invite_parameters(Keys, CCVs, CSHs, Parameters);
        Parameter ->
...
-spec dynamic_sip_invite_value(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    kz_term:api_binary().
...
dynamic_sip_invite_value(_, _, _) ->
    'undefined'.

-spec default_gateway_cid(kapi_offnet_resource:req(), kz_term:api_binary()) ->
    ...
    case kz_datamgr:get_results(AccountDb, ?LIST_RESOURCES_BY_ID, ViewOptions) of
        ...
        {'ok', JObjs} ->
            ...
            kz_cache:store_local(
                ?CACHE_NAME, {'local_resources', AccountId}, LocalResources, CacheProps
            ),
            LocalResources
    end.
-endif.
...
fetch_account_dedicated_proxies(AccountId) ->
    UseSingle = kapps_config:get_is_true(
        ?SS_CONFIG_CAT, <<"use_first_dedicated_proxy_only">>, 'true'
    ),
...
-spec build_account_dedicated_proxy(kz_json:object()) ->
...
resources_from_jobjs([], Resources) ->
    Resources;
...
create_resource(ResourceJObj, Resources) ->
    case kzd_resources:classifiers(ResourceJObj) of
        'undefined' ->
            [resource_from_jobj(ResourceJObj) | Resources];
        ResourceClassifiers ->
            AvailableClassifiers = kz_json:to_proplist(knm_converters:available_classifiers()),
            create_resource(
                kz_json:to_proplist(ResourceClassifiers),
...
-spec create_resource(kz_term:proplist(), kz_term:proplist(), kz_json:object(), resources()) ->
    resources().
create_resource([], _ConfigClassifiers, _ResourceJObj, Resources) ->
    Resources;
create_resource(
    [{ResourceClassifier, ResourceClassifierJObj} | ResourceClassifiers],
    ConfigClassifiers,
    ResourceJObj,
    Resources
) ->
...
-spec create_classifier_resource(
    kz_json:object(), kz_json:object(), kz_term:ne_binary(), kz_term:proplist()
) -> kz_json:object().
create_classifier_resource(Resource, ClassifierJObj, Classifier, ConfigClassifier) ->
    ...
    Props =
        props:filter_undefined(
            [
                {<<"_id">>, <<
                    (kz_json:get_value(<<"_id">>, Resource))/binary, "-", Classifier/binary
                >>},
                {<<"name">>, <<
                    (kz_json:get_value(<<"name">>, Resource))/binary, " - ", Classifier/binary
                >>},
                {<<"rules">>, [kz_json:get_value(<<"regex">>, ClassifierJObj, DefaultRegex)]},
                {<<"weight_cost">>, kz_json:get_value(<<"weight_cost">>, ClassifierJObj)},
                {<<"emergency">>,
                    classifier_is_emergency(ClassifierJObj, Classifier, DefaultEmergency)},
                {<<"classifier">>, Classifier},
...
-spec classifier_is_emergency(kz_json:object(), kz_term:ne_binary(), boolean() | 'undefined') ->
    boolean().
...
resource_from_jobj(JObj) ->
    Resource = #resrc{
        ...
        gateway_strategy = kz_term:to_atom(
            kzd_resources:gateway_strategy(JObj, <<"sequential">>), 'true'
        )
    },
...
resource_codecs(JObj) ->
    ...
    case
        kzd_resources:media_audio_codecs(JObj, DefaultAudio) ++
...
resource_rules(JObj) ->
    ...
    lager:info(
        "compiling resource rules for ~s / ~s: ~p",
...
resource_rules_test(JObj) ->
    ...
    lager:info(
        "compiling resource test rules for ~s / ~s: ~p",
        [kz_doc:account_db(JObj, <<"offnet">>), kz_doc:id(JObj), Rules]
    ),
    resource_rules(Rules, []).
...
resource_cid_rules(JObj) ->
    lager:info(
        "compiling caller id rules for ~s / ~s",
...
resource_is_emergency(JObj) ->
    kzd_resources:emergency(JObj, 'false') orelse
        (kzd_resources:caller_id_options_type(JObj) =:= <<"emergency">>).
...
gateway_from_jobj(JObj, #resrc{
    ...
    #gateway{
        ...
        sip_headers = kz_custom_sip_headers:outbound(
            kz_json:get_json_value(<<"custom_sip_headers">>, JObj, kz_json:new())
        ),
        ...
        progress_timeout = kz_json:get_integer_value(
            <<"progress_timeout">>, JObj, ?DEFAULT_PROGRESS_TIMEOUT
        ),
...
gateway_is_emergency(JObj, IsEmergency) -> kz_json:is_true(<<"emergency">>, JObj, IsEmergency).
...
endpoint_options(JObj, <<"freetdm">>) ->
    kz_json:from_list(
        [
            ...
            {<<"Channel-Selection">>,
                kz_json:get_value(<<"channel_selection">>, JObj, <<"ascending">>)}
        ]
    );
...
gateway_dialstring(
    ...
    DialStringPort =
        case
            not kz_term:is_empty(Port) andalso
                Port =/= 5060
        of
...
get_resrc_gateways(#resrc{gateways = Gateways, gateway_strategy = 'sequential'}) ->
    Gateways;
get_resrc_gateways(#resrc{gateways = Gateways, gateway_strategy = 'random'}) ->
    kz_term:shuffle_list(Gateways).
...
set_resrc_format_from_uri(Resource, FormatFromUri) ->
    Resource#resrc{format_from_uri = FormatFromUri}.
...
set_resrc_from_account_realm(Resource, FromAccountRealm) ->
    Resource#resrc{from_account_realm = FromAccountRealm}.
