...
filter_list(Set, Set, <<"exact">>) ->
    'true';
filter_list(SetA, SetB, <<"subset">>) ->
    sets:is_subset(SetA, SetB);
filter_list(SetA, SetB, <<"ne_subset">>) ->
    sets:size(SetA) > 0 andalso
        sets:is_subset(SetA, SetB);
...
filter_list(SetA, SetB, <<"intersect">>) ->
    not sets:is_disjoint(SetA, SetB);
filter_list(SetA, SetB, <<"disjoint">>) ->
