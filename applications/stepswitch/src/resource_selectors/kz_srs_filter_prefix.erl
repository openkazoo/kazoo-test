...
handle_req(Resources, Number, OffnetJObj, DB, Params) ->
    ...
    SourceB = maybe_db_type(
        kz_srs_util:get_source(kz_json:get_value(<<"value_b">>, Params)), Resources, ValueA
    ),
    ...
    PrefixMode = kz_srs_util:select_filter_mode(
        Params, ?ALLOWED_FILTER_MODES, ?DEFAULT_FILTER_MODE
    ),
    case SourceB of
...
maybe_db_type({'database', SelectorName}, Resources, PrefixSrc) ->
    ...
    Keys = [
        [stepswitch_resources:get_resrc_id(R), SelectorName, P]
     || R <- Resources, P <- Prefixes
    ],
    Options = [{'keys', Keys}],
    View = <<"resource_selectors/resource_name_selector_listing">>,
    {'database', View, Options};
maybe_db_type(Other, _Resources, _PrefixSrc) ->
    Other.

-spec build_prefixes(kz_term:ne_binary()) -> kz_term:ne_binaries().
build_prefixes(<<"+", Rest/binary>>) -> build_prefixes(Rest);
build_prefixes(<<D:1/binary, Rest/binary>>) -> build_prefixes(Rest, D, [D]).

-spec build_prefixes(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    kz_term:ne_binaries().
