...
-define(DEFAULT_ACCEPT_HDR,
    <<"text/pbx,text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8">>
).
...
-define(HTTP_CONTENT_TYPE,
    kapps_config:get_string(
        ?CNAM_CONFIG_CAT, <<"http_content_type_header">>, ?DEFAULT_CONTENT_TYPE_HDR
    )
).
            ...
            case kz_http_util:urlsplit(Url) of
                ...
                {Scheme, Host, Path, QS, Segment} ->
                    kz_http_util:urlunsplit(
                        {Scheme, Host, Path, <<QS/binary, "&ref=2600hz&format=pbx">>, Segment}
                    )
            end
    end.
