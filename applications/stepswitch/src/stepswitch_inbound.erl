...
handle_req(JObj, _Props) ->
    ...
    case kz_json:get_ne_value(?CCV(<<"Account-ID">>), JObj) of
        ...
        AccountId ->
            lager:debug(
                "fetch-id ~s already has account-id ~s, skipping.",
...
maybe_relay_request(JObj) ->
    ...
    case knm_number:lookup_account(Number) of
        {'error', _R} ->
            lager:info(
                "unable to determine account for fetch-id ~s, ~s: ~p",
                [kapi_route:fetch_id(JObj), Number, _R]
            );
        {'ok', _, NumberProps} ->
            lager:debug(
                "running routines for number ~s, fetch-id : ~s",
...
maybe_find_resource(_, JObj) ->
    case stepswitch_resources:reverse_lookup(JObj) of
        ...
        {'ok', ResourceProps} ->
            Routines = [
                fun add_resource_id/2,
                fun maybe_add_t38_settings/2
            ],
            lists:foldl(
                fun(F, J) -> F(J, ResourceProps) end,
                JObj,
                Routines
            )
    end.
...
maybe_set_ringback(NumberProps, JObj) ->
    case knm_number_options:ringback_media_id(NumberProps) of
        'undefined' -> JObj;
        MediaId -> kz_json:set_value(?CCV(<<"Ringback-Media">>), MediaId, JObj)
    end.
...
maybe_set_transfer_media(NumberProps, JObj) ->
    case knm_number_options:transfer_media_id(NumberProps) of
        'undefined' -> JObj;
        MediaId -> kz_json:set_value(?CCV(<<"Transfer-Media">>), MediaId, JObj)
    end.
...
block_call_routines(JObj) ->
    Routines = [
        {fun should_block_anonymous/1, {<<"433">>, <<"Anonymity Disallowed">>}},
...
should_block_anonymous(JObj) ->
    kz_privacy:should_block_anonymous(JObj) orelse
        (kz_privacy:is_anonymous(JObj) andalso
            kz_json:is_true(<<"should_block_anonymous">>, get_blacklist(JObj))).
        ...
        fun(BlacklistId, Acc) ->
            case kz_datamgr:open_cache_doc(AccountDb, BlacklistId) of
                ...
                {'ok', Doc} ->
                    ...
                    BlackList = maybe_set_block_anonymous(
                        Numbers, kz_json:is_true(<<"should_block_anonymous">>, Doc)
                    ),
...
maybe_set_block_anonymous(JObj, 'false') ->
    JObj;
