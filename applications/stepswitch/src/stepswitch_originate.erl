...
handle_event(JObj, #state{
    ...
    case kz_util:get_event_type(JObj) of
        {<<"call_event">>, <<"CHANNEL_DESTROY">>} ->
            ...
            gen_listener:cast(
                RequestHandler, {'originate_result', originate_success(JObj, OffnetReq)}
            );
        ...
        {<<"resource">>, <<"originate_resp">>} ->
            ...
            case kz_call_event:application_response(JObj) =:= <<"SUCCESS">> of
                ...
                'false' ->
                    gen_listener:cast(
                        RequestHandler, {'originate_result', originate_failure(JObj, OffnetReq)}
                    )
            end;
        {<<"error">>, <<"originate_resp">>} ->
            MsgId = kz_api:msg_id(JObj),
            lager:debug(
                "channel execution error while waiting for originate: ~s",
                [kz_json:encode(JObj)]
            ),
            gen_listener:cast(
                RequestHandler, {'originate_result', originate_error(JObj, OffnetReq)}
            );
        {<<"dialplan">>, <<"originate_ready">>} ->
            gen_listener:cast(
                RequestHandler, {'originate_result', originate_ready(JObj, OffnetReq)}
            );
        _ ->
            'ok'
    end,
...
build_originate(#state{
    ...
    props:filter_undefined(
        [
            ...
            {<<"Fax-Identity-Name">>,
                kz_json:get_value(<<"Fax-Identity-Name">>, OffnetReq, CIDName)},
            {<<"Fax-Identity-Number">>,
                kz_json:get_value(<<"Fax-Identity-Number">>, OffnetReq, CIDNum)},
            ...
            {<<"Outbound-Call-ID">>,
                kz_json:get_first_defined(
                    [<<"Origination-Call-ID">>, <<"Outbound-Call-ID">>], OffnetReq
                )},
            {<<"Outbound-Callee-ID-Name">>,
                kz_json:get_value(<<"Outbound-Callee-ID-Name">>, OffnetReq)},
            {<<"Outbound-Callee-ID-Number">>,
                kz_json:get_value(<<"Outbound-Callee-ID-Number">>, OffnetReq)},
...
originate_from_uri(CIDNum, OffnetReq) ->
    ...
    case
        (kapps_config:get_is_true(?SS_CONFIG_CAT, <<"format_from_uri">>, 'false') orelse
            kz_json:is_true(<<"Format-From-URI">>, OffnetReq)) andalso
            (is_binary(CIDNum) andalso
                is_binary(Realm))
    of
        'false' ->
            'undefined';
...
originate_error(JObj, OffnetReq) ->
    ...
    [
        ...
        {<<"Response-Code">>, <<"sip:500">>},
        {<<"Error-Message">>,
            kz_json:get_value(<<"Error-Message">>, JObj, <<"failed to process request">>)},
...
originate_failure(JObj, OffnetReq) ->
    lager:debug("originate request failed: ~s", [
