...
number_tree(DID) ->
    case knm_number:lookup_account(DID) of
        {'error', _} ->
            io:format("DID ~s was not found~n", [DID]);
        {'ok', AccountId, _Props} ->
            case kz_datamgr:open_doc(?KZ_ACCOUNTS_DB, AccountId) of
                {'ok', AccountDoc} ->
                    number_tree(DID, AccountDoc);
                {'error', _E} ->
                    io:format("failed to find account doc for ~s(~s)~n", [AccountId, DID])
            end
    end.
...
process_number(Num, AccountId) ->
    JObj = kz_json:from_list([
        {<<"Account-ID">>, kz_term:to_binary(AccountId)},
