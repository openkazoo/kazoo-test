...
-ifdef(TEST).
-export([
    avoid_privacy_if_emergency_call/2,
...
-spec handle_error_event(kz_call_event:doc(), kapi_offnet_resource:req(), kz_term:api_pid()) ->
    'ok'.
handle_error_event(CallEvt, OffnetReq, RequestHandler) ->
    handle_error_event(
        CallEvt,
...
-spec handle_error_event(
    kz_call_event:doc(), kapi_offnet_resource:req(), kz_term:api_pid(), kz_term:ne_binary()
) -> 'ok'.
handle_error_event(CallEvt, OffnetReq, RequestHandler, <<"bridge">>) ->
    lager:info(
        "channel execution error while waiting for bridge: ~s",
...
-spec handle_channel_destroy(kz_call_event:doc(), kapi_offnet_resource:req(), kz_term:api_pid()) ->
    'ok'.
handle_channel_destroy(CallEvt, OffnetReq, RequestHandler) ->
    lager:debug("channel was destroyed while waiting for bridge"),
    handle_bridge_result(CallEvt, OffnetReq, RequestHandler).

-spec handle_channel_execute_complete(
    kz_call_event:doc(), kapi_offnet_resource:req(), kz_term:api_pid()
) -> 'ok'.
handle_channel_execute_complete(CallEvt, OffnetReq, RequestHandler) ->
    handle_channel_execute_complete(
        CallEvt,
        OffnetReq,
        RequestHandler,
...
-spec handle_channel_execute_complete(
    kz_call_event:doc(), kapi_offnet_resource:req(), kz_term:api_pid(), kz_term:api_ne_binary()
) -> 'ok'.
...
-spec handle_bridge_result(kz_call_event:doc(), kapi_offnet_resource:req(), kz_term:api_pid()) ->
    'ok'.
handle_bridge_result(CallEvt, OffnetReq, RequestHandler) ->
    Result =
        case <<"SUCCESS">> =:= kz_call_event:disposition(CallEvt) of
...
maybe_bridge_emergency(
    ...
    case kapi_offnet_resource:hunt_account_id(OffnetReq) of
        ...
        _Else ->
            ...
            lager:debug(
                "not enforcing emergency caller id validation when using resource from account ~s",
                [_Else]
            ),
...
-spec update_endpoints_emergency_cid(
    stepswitch_resources:endpoints(), kz_term:ne_binary(), kz_term:api_ne_binary()
) ->
...
-spec update_endpoint_emergency_cid(
    stepswitch_resources:endpoint(), kz_term:ne_binary(), kz_term:api_binary()
) -> stepswitch_resources:endpoint().
update_endpoint_emergency_cid(Endpoint, Number, Name) ->
    case
        {
            kz_json:get_ne_binary_value(<<"Outbound-Caller-ID-Number">>, Endpoint, Number),
    ...
    of
        {Number, Name} ->
            Endpoint;
        {Number, _} ->
            kz_json:set_value(<<"Outbound-Caller-ID-Name">>, Name, Endpoint);
        {_, Name} ->
            kz_json:set_value(<<"Outbound-Caller-ID-Number">>, Number, Endpoint);
        {_, _} ->
            Props = [
...
build_bridge(
    ...
    IgnoreEarlyMedia =
        kz_json:is_true(<<"Require-Ignore-Early-Media">>, ReqCCVs, 'false') orelse
            kapi_offnet_resource:ignore_early_media(OffnetReq, 'false'),

    FailOnSingleReject = kz_json:get_value(<<"Require-Fail-On-Single-Reject">>, ReqCCVs),

    AddCCVs = props:filter_undefined([
    ...
    {AssertedNumber, AssertedName} = maybe_override_asserted_identity(
        OffnetReq, {IsEmergency, Number, Name}
    ),
    ...
    props:filter_undefined(
        [
            ...
            {<<"Asserted-Identity-Realm">>,
                kapi_offnet_resource:asserted_identity_realm(OffnetReq, Realm)},
            ...
            {<<"Custom-Application-Vars">>,
                kapi_offnet_resource:custom_application_vars(OffnetReq)},
            ...
            {<<"Fax-Identity-Number">>,
                kapi_offnet_resource:fax_identity_number(OffnetReq, Number)},
            ...
            {<<"Outbound-Callee-ID-Name">>,
                kapi_offnet_resource:outbound_callee_id_name(OffnetReq)},
            {<<"Outbound-Callee-ID-Number">>,
                kapi_offnet_resource:outbound_callee_id_number(OffnetReq)},
...
-spec maybe_override_asserted_identity(kapi_offnet_resource:req(), emergency_override()) ->
    caller_id().
...
maybe_override_asserted_identity(OffnetReq, {'true', Number, Name}) ->
    ...
    case
        kz_term:is_empty(AssertedNumber) orelse
            kz_term:is_empty(AssertedName)
    of
...
bridge_from_uri(Number, OffnetReq) ->
    ...
    case
        (kapps_config:get_is_true(?SS_CONFIG_CAT, <<"format_from_uri">>, 'false') orelse
            kapi_offnet_resource:format_from_uri(OffnetReq)) andalso
            is_binary(Number) andalso
            is_binary(Realm)
    of
        'false' ->
            'undefined';
...
ensure_valid_emergency_number(OffnetReq) ->
    ...
    case {lists:member(Emergency, Numbers), lists:member(Outbound, Numbers)} of
        {'true', _} ->
            lager:info("determined emergency caller id number ~s is configured for e911", [
                Emergency
            ]),
            Emergency;
        {_, 'true'} ->
        ...
        {'false', 'false'} ->
            lager:notice(
                "emergency caller id number ~s nor outbound caller id number ~s configured for e911",
                [Emergency, Outbound]
            ),
...
bridge_error(CallEvt, OffnetReq) ->
    ...
    [
        ...
        {<<"Response-Code">>, <<"sip:500">>},
        {<<"Error-Message">>,
            kz_call_event:error_message(CallEvt, <<"failed to process request">>)},
        {<<"To-DID">>, kapi_offnet_resource:to_did(OffnetReq)}
        | kz_api:default_headers(?APP_NAME, ?APP_VERSION)
    ].
...
bridge_failure(CallEvt, OffnetReq) ->
    lager:debug(
        "resources for outbound request failed: ~s",
...
send_emergency_bridge_notification(Number, #state{resource_req = OffnetReq} = State) ->
    Setters = [
        {fun set_emergency_call_meta/2, {Number, State}},
...
-spec set_emergency_call_meta({kz_term:ne_binary(), state()}, kz_term:proplist()) ->
    kz_term:proplist().
...
-spec set_emergency_device_meta(kapi_offnet_resource:req(), kz_term:proplist()) ->
    kz_term:proplist().
set_emergency_device_meta(OffnetReq, Props) ->
    AccountID = kapi_offnet_resource:account_id(OffnetReq),
    DeviceID = kz_json:get_ne_value(
        <<"Authorizing-ID">>,
        kapi_offnet_resource:requestor_custom_channel_vars(OffnetReq),
        'undefined'
    ),
    OwnerID = kz_json:get_ne_value(
        <<"Owner-ID">>, kapi_offnet_resource:requestor_custom_channel_vars(OffnetReq), 'undefined'
    ),
...
-spec set_emergency_address_meta(
    {kz_term:ne_binary(), kapi_offnet_resource:req()}, kz_term:proplist()
) -> kz_term:proplist().
        ...
        {'ok', Doc} ->
            [
                {<<"Emergency-Address-Street-1">>,
                    extract_e911_street_address_field(Doc, <<"street_address">>)},
                {<<"Emergency-Address-Street-2">>,
                    extract_e911_street_address_field(Doc, <<"street_address_extended">>)},
                ...
                {<<"Emergency-Notfication-Contact-Emails">>,
                    kzd_phone_numbers:e911_notification_contact_emails(Doc)}
                | Props
            ];
        {'error', _} ->
            Props
...
-spec extract_e911_street_address_field(kz_doc:doc(), kz_term:ne_binary()) ->
    kz_term:ne_binary() | 'undefined'.
...
-spec maybe_use_e911_legacy_value(kz_doc:doc(), kz_term:ne_binary()) ->
    kz_term:ne_binary() | 'undefined'.
maybe_use_e911_legacy_value(Doc, <<"street_address">>) ->
    format_legacy_address(
        kzd_phone_numbers:e911_legacy_data_house_number(Doc),
...
-spec format_legacy_address(kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
...
%% @end
%%------------------------------------------------------------------------------
-spec avoid_privacy_if_emergency_call(boolean(), stepswitch_resources:endpoints()) ->
    stepswitch_resources:endpoints().
