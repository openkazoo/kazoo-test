...
shortdial_correction_length_test_() ->
    ...
    [
        {"If fixed is undefined, return calculated length if it is within bounds, 0 otherwise", [
            ...
            %% Difference = 6
            ?_assertEqual(0, Undefined(<<"123">>, <<"123456789">>)),
            %% Difference = 1
            ?_assertEqual(0, Undefined(<<"123">>, <<"1234">>))
        ]},
...
maybe_deny_reclassified_number_test_() ->
    ...
    %% Just to be sure ;)
    meck:expect('knm_converters', 'classify', fun(_) -> <<"unknown">> end),
...
do_correct_shortdial_test_() ->
    ...
    [
        {"Take up to Length digits from CallerNumber and prefix CalleeNumber with those digits", [
            ...
            ?_assertEqual(
                <<CallerNumber/binary, CalleeNumber/binary>>,
                ...
                [
                    ?_assertEqual(
                        ...
                        Test(
                            ...
                            CallerNumber,
                            kz_json:new(),
                            {'false', byte_size(CalleeNumber) + 1}
                        )
                    ),
                    ?_assertEqual(
                        'undefined',
