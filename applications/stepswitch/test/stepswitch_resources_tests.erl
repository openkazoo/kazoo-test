...
invite_parameters_test_() ->
    ...
    DynamicParameters1 = kz_json:from_list([
        {<<"key">>, <<"custom_sip_headers.x_auth_ip">>}, {<<"tag">>, <<"somethingelse">>}
    ]),
    DynamicParameters2 = kz_json:from_list([
    ...
    GatewayJObjs = [
        ...
        kz_json:set_value(
            [<<"invite_parameters">>, <<"dynamic">>], [DynamicParameters1], GatewayJObj
        ),
        kz_json:set_value(
            [<<"invite_parameters">>, <<"dynamic">>], [DynamicParameters2], GatewayJObj
        )
    ],

    Gateways = [
        stepswitch_resources:gateway_from_jobj(GatewayJObj2, Resource)
     || GatewayJObj2 <- GatewayJObjs
    ],

    Offnets = [
        kz_json:set_value(
            [<<"Requestor-Custom-Channel-Vars">>, <<"TNS-CIC">>], <<"cic=2002">>, Offnet
        ),
        kz_json:set_value(
            [<<"Requestor-Custom-Channel-Vars">>, <<"Account-ID">>], <<"12345">>, Offnet
        ),
        kz_json:set_value(
            [<<"Requestor-Custom-SIP-Headers">>, <<"X-Auth-IP">>], <<"127.0.0.1">>, Offnet
        ),
        kz_json:set_value(
            [<<"Requestor-Custom-SIP-Headers">>, <<"X-Auth-IP">>], <<"127.0.0.1">>, Offnet
        ),
    ...
    [
        ?_assertEqual(
            ExpectedValue, stepswitch_resources:sip_invite_parameters(Gateway, ModifiedOffnet)
        )
