...
handle_req(JObj, _Props) ->
    ...
    AccountDb =
        case
            {kz_json:get_value(<<"Account-DB">>, JObj), kz_json:get_value(<<"Account-ID">>, JObj)}
        of
    ...
    CustomTxtTemplate = kz_json:get_value(
        [<<"notifications">>, <<"new_account">>, <<"email_text_template">>], Account
    ),
    {'ok', TxtBody} = notify_util:render_template(CustomTxtTemplate, ?DEFAULT_TEXT_TMPL, Props),

    CustomHtmlTemplate = kz_json:get_value(
        [<<"notifications">>, <<"new_account">>, <<"email_html_template">>], Account
    ),
    {'ok', HTMLBody} = notify_util:render_template(CustomHtmlTemplate, ?DEFAULT_HTML_TMPL, Props),

    CustomSubjectTemplate = kz_json:get_value(
        [<<"notifications">>, <<"new_account">>, <<"email_subject_template">>], Account
    ),
...
%% @end
%%------------------------------------------------------------------------------
-spec create_template_props(
    kz_json:object(), kz_json:object(), kz_json:object(), kz_json:objects()
) ->
    kz_term:proplist().
create_template_props(Event, Admin, Account, AllDocs) ->
    Owners = [
        {kz_json:get_value([<<"doc">>, <<"_id">>], J1), kz_json:get_value(<<"doc">>, J1)}
     || J1 <- AllDocs,
    ...
    DevicesWithOwners = [
        begin
            ...
            kz_json:set_value(
                <<"user">>, props:get_value(kz_json:get_value(<<"owner_id">>, J3), Owners), J3
            )
        end
     || J2 <- AllDocs,
...
-spec build_and_send_email(
    iolist(), iolist(), iolist(), kz_term:api_binary() | kz_term:ne_binaries(), kz_term:proplist()
) -> send_email_return().
