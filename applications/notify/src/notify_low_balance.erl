...
handle_req(JObj, _Props) ->
    ...
    notify_util:maybe_send_update(
        send(kz_json:get_integer_value(<<"Current-Balance">>, JObj), Account),
        RespQ,
        MsgId
    ).
    ...
    case collect_recipients(AccountId) of
        ...
        To ->
            ...
            CustomTxtTemplate = kz_json:get_value(
                [<<"notifications">>, <<"low_balance">>, <<"email_text_template">>], Account
            ),
            {'ok', TxtBody} = notify_util:render_template(
                CustomTxtTemplate, ?DEFAULT_TEXT_TMPL, Props
            ),
            CustomHtmlTemplate = kz_json:get_value(
                [<<"notifications">>, <<"low_balance">>, <<"email_html_template">>], Account
            ),
            {'ok', HTMLBody} = notify_util:render_template(
                CustomHtmlTemplate, ?DEFAULT_HTML_TMPL, Props
            ),
            CustomSubjectTemplate = kz_json:get_value(
                [<<"notifications">>, <<"low_balance">>, <<"email_subject_template">>], Account
            ),
            {'ok', Subject} = notify_util:render_template(
                CustomSubjectTemplate, ?DEFAULT_SUBJ_TMPL, Props
            ),
...
-spec get_email(kz_term:ne_binary(), kz_term:ne_binary()) ->
...
get_email(MasterAccountId, MasterAccountId) ->
    ...
    lager:debug(
        "attempting to email low balance to master account ~s",
        [MasterAccountId]
    ),
...
get_email(AccountId, MasterAccountId) ->
    lager:debug(
        "attempting to email low balance to account ~s",
        [AccountId]
    ),
...
-spec get_email(kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:api_binaries() | kz_term:api_binary().
get_email(JObj, AccountId, MasterAccountId) ->
    case find_billing_email(JObj) of
        'undefined' ->
            lager:debug(
                "billing contact email not set or low balance disabled for account ~s",
                [AccountId]
            ),
...
find_billing_email(JObj) ->
    case is_notify_enabled(JObj) of
        'false' -> 'undefined';
        'true' -> kz_json:get_ne_value([<<"contact">>, <<"billing">>, <<"email">>], JObj)
    end.
