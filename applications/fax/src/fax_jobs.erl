...
distribute_jobs(
    ...
    map_size(Pending) + map_size(Running) >= MaxAccount
->
...
maybe_distribute_job(Job, #state{account_id = AccountId} = State) ->
    ...
    case is_number_valid(Number, AccountId) of
        'true' ->
            distribute_job(knm_converters:normalize(Number, AccountId), Job, State#state{stale = 0});
        'false' ->
            invalidate_job(Job, State)
    end.
...
distribute_job(ToNumber, Job, #state{jobs = #{numbers := Numbers}} = State) ->
    JobId = kz_doc:id(Job),
    case
        ?SERIALIZE_OUTBOUND_NUMBER andalso
            maps:is_key(ToNumber, Numbers)
    of
...
start_job(
    ...
    lager:debug("starting job: ~s for account id: ~s fax destination: ~s", [
        JobId, AccountId, ToNumber
    ]),
    case
        kz_amqp_worker:call(Payload, fun kapi_fax:publish_start_job/1, fun validate_job_started/1)
    of
...
-spec set_pending_job(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary(), state()
) -> state().
set_pending_job(
    JobId,
    Node,
...
is_number_valid(Number, AccountId) -> knm_converters:is_reconcilable(Number, AccountId).
...
check_pending(
    JobId, #{number := ToNumber, start := Start}, #{pending := Pending, numbers := Numbers} = Map
) ->
    case kz_time:now_ms() - Start > ?MAX_WAIT_FOR_PENDING of
        'true' ->
            lager:debug("recovering pending fax job ~s", [JobId]),
            Map#{
                pending => maps:remove(JobId, Pending),
                numbers => maps:remove(ToNumber, Numbers)
            };
        'false' ->
            Map
    end;
check_pending(_JobId, _Running, Map) ->
    Map.

-spec check_running(kz_term:ne_binary(), map(), map()) -> map().
check_running(
    JobId, #{number := ToNumber, start := Start}, #{running := Running, numbers := Numbers} = Map
) ->
...
check_running(_JobId, _Running, Map) ->
    Map.
...
lock_account_jobs([]) -> [];
lock_account_jobs(Jobs) -> lists:foldr(fun lock_account_jobs_fold/2, [], Jobs).
...
cleanup_jobs(AccountId) ->
    ...
    case kz_datamgr:get_results(?KZ_FAXES_DB, <<"faxes/locked_jobs_by_account">>, ViewOptions) of
        ...
        {'ok', JObjs} ->
            ...
            'ok';
        {'error', _R} ->
            lager:debug("unable to cleanup account_id ~s fax jobs: ~p", [AccountId, _R])
    end.
...
maybe_serialize(
    <<"not_found">> = Status, <<"acquire">> = Stage, JobId, AccountId, Number, _Job, Jobs
) ->
...
maybe_serialize(Status, Stage, JobId, AccountId, Number, Job, Jobs) ->
    lager:debug("serializing job ~s/~s into cache in stage ~s : ~s", [
        AccountId, JobId, Stage, Status
    ]),
    #{
        pending := Pending,
        numbers := Numbers,
...
handle_job_error(JobId, AccountId, JObj, #{pending := Pending} = Jobs) ->
    ...
    case kz_maps:get([JobId, job], Pending, 'undefined') of
        'undefined' ->
            lager:warning("received error on account (~s) for a not pending job ~s (~s) : ~s", [
                AccountId, JobId, Stage, Status
            ]),
            maybe_remove_delayed_error_job(Status, Stage, JobId, Jobs);
        Job ->
            Number = knm_converters:normalize(number(Job), AccountId),
            lager:warning("received error for fax job ~s/~s/~s on stage ~p: ~p", [
                AccountId, JobId, Number, Stage, Status
            ]),
            maybe_serialize(Status, Stage, JobId, AccountId, Number, Job, Jobs)
    end.
...
handle_job_start(
    ...
    case kz_maps:get([JobId, number], Pending, 'undefined') of
        'undefined' ->
            lager:warning(
                "received job start on account (~s) for a not pending job ~s (~s) : ~s", [
                    AccountId, JobId, Stage, Status
                ]
            ),
            Jobs#{
                distribute => remove_delayed_reply_job(JobId, Distribute),
...
handle_job_end(
    ...
    case kz_maps:get(JobId, Running, 'undefined') of
        'undefined' ->
            lager:debug(
                "received fax end control in account ~s for a not controlled job ~s : (~s) : ~s", [
                    AccountId, JobId, Stage, Status
                ]
            ),
            Jobs;
        #{queue := ServerId, number := Number} ->
            lager:debug("received fax end control status in account ~s for ~s : (~s) : ~s", [
                AccountId, JobId, Stage, Status
            ]),
            Jobs#{
                pending => maps:remove(JobId, Pending),
        ...
        #{number := Number, queue := OurServerId} ->
            lager:debug(
                "received fax end control in account ~s for ~s from a different worker ~s / ~s : (~s) : ~s",
                [AccountId, JobId, ServerId, OurServerId, Stage, Status]
            ),
            Jobs#{
                pending => maps:remove(JobId, Pending),
                running => maps:remove(JobId, Running),
        ...
        _Other ->
            lager:debug(
                "received fax end control in account ~s for ~s from worker ~s : (~s) : ~s", [
                    AccountId, JobId, ServerId, Stage, Status
                ]
            ),
            Jobs#{
                pending => maps:remove(JobId, Pending),
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_remove_delayed_error_job(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), map()
) -> map().
maybe_remove_delayed_error_job(
    <<"not_found">>,
    <<"acquire">>,
    JobId,
    #{
...
maybe_remove_delayed_error_job(_, _, _, Jobs) ->
    Jobs.
