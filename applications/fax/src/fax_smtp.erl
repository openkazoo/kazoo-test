...
-define(IMAGE_SIZE_CMD_FMT,
    kapps_config:get_binary(?CONFIG_CAT, <<"image_size_cmd_format">>, ?DEFAULT_IMAGE_SIZE_CMD_FMT)
).
...
handle_EHLO(Hostname, Extensions, #state{options = Options, proxy = Proxy} = State) ->
    ...
    MyExtensions =
        case props:get_is_true('auth', Options, 'false') of
            'false' ->
                Extensions;
            'true' ->
...
handle_DATA(From, To, Data, #state{options = Options} = State) ->
    ...
    case decode_data(Data) of
        ...
        'error' ->
            ...
            {'error', "554 Message decode failed", State#state{
...
decode_data(Data) ->
    ...
    DecodeOptions = [
        {'encoding', <<"utf-8">>},
        %% decode base64/quoted-printable attachments
        {'decode_attachments', 'true'},
        %% assume default MIME version
    ...
    try
        mimemail:decode(Data, DecodeOptions)
    catch
        _What:_Why ->
            ...
            case filelib:ensure_dir(File) of
                'ok' ->
                    case kz_util:write_file(File, Data) of
                        'ok' ->
                            lager:debug("wrote DATA exception data to ~s", [File]);
                        {'error', _E} ->
                            lager:debug("failed to write DATA exception data to ~s: ~p", [File, _E])
                    end;
                {'error', _E} ->
                    lager:debug("failed to ensure ~s: ~p", [File, _E])
            end
    end.
...
-spec handle_AUTH(
    'login' | 'plain' | 'cram-md5', binary(), binary() | {binary(), binary()}, state()
) ->
    'error'.
            ...
            case kz_fax_attachment:save_outbound(?KZ_FAXES_DB, Doc, Content, ContentType) of
                {'ok', NewDoc} ->
                    ...
                    case kz_datamgr:save_doc(?KZ_FAXES_DB, kz_json:set_values(Updates, NewDoc)) of
                        ...
                        {'error', Error} ->
                            lager:debug("error ~p setting fax jobid ~s to pending", [
                                Error, kz_doc:id(NewDoc)
                            ]),
                            maybe_faxbox_log(State#state{errors = [Error]})
                    end;
                {'error', Error} ->
                    lager:error("failed converting attachment with error: ~p", [Error]),
                    Message = kz_term:to_binary(
                        io_lib:format("error converting attachment ~s", [Filename])
                    ),
...
maybe_faxbox_log(AccountId, JObj, State) ->
    case
        kz_json:is_true(<<"log_errors">>, JObj, 'false') orelse
            (kapps_account_config:get_global(
                AccountId, ?CONFIG_CAT, <<"log_faxbox_errors">>, 'true'
            ) andalso
                kz_json:is_true(<<"log_errors">>, JObj, 'true'))
    of
...
check_faxbox(#state{to = To} = State) ->
    case binary:split(kz_term:to_lower_binary(To), <<"@">>) of
        ...
        _ ->
            ...
            {'error', "554 Not Found", State#state{
...
check_permissions(
    ...
    case
        lists:any(fun(A) -> match(From, A) end, Permissions) orelse
            From =:= kz_term:to_lower_binary(OwnerEmail)
    of
        'true' ->
            add_fax_document(State);
        'false' ->
            Error = kz_term:to_binary(
                io_lib:format(
                    "address ~s is not allowed on faxbox ~s",
...
match(Address, Element) ->
    Address =:= kz_term:to_lower_binary(Element) orelse
...
maybe_faxbox_by_owner_email(
    ...
    case kz_datamgr:get_results(AccountDb, <<"users/list_by_email">>, ViewOptions) of
        {'ok', []} ->
            Error = kz_term:to_binary(
                io_lib:format("user ~s does not exist in account ~s, trying by rules", [
                    From, AccountId
                ])
            ),
            lager:debug(Error),
        ...
        {'ok', [_JObj | _JObjs]} ->
            Error = kz_term:to_binary(
                io_lib:format("more then one user with email ~s in account ~s, trying by rules", [
                    From, AccountId
                ])
            ),
            lager:debug(Error),
        ...
        {'error', _E} ->
            Error = kz_term:to_binary(
                io_lib:format("error ~p getting user by email ~s  in account ~s, trying by rules", [
                    _E, From, AccountId
                ])
            ),
            lager:debug(Error),
...
maybe_faxbox_by_owner_id(AccountId, OwnerId, #state{errors = Errors, from = From} = State) ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"faxbox/list_by_ownerid">>, ViewOptions) of
        {'ok', [JObj]} ->
            State#state{
                faxbox = kz_json:get_value(<<"doc">>, JObj),
                owner_id = OwnerId,
                owner_email = From,
                errors = []
            };
        {'ok', [_JObj | _JObjs]} ->
            Error = kz_term:to_binary(
                io_lib:format("user ~s : ~s has multiples faxboxes", [OwnerId, From])
            ),
            maybe_faxbox_by_rules(
                AccountId,
                State#state{
                    owner_id = OwnerId,
                    owner_email = From,
        ...
        _ ->
            Error = kz_term:to_binary(
                io_lib:format("user ~s : ~s does not have a faxbox", [OwnerId, From])
            ),
            lager:debug(
                "user ~s : ~s from account ~s does not have a faxbox, trying by rules",
...
maybe_faxbox_by_rules(AccountId, #state{errors = Errors} = State) when
    is_binary(AccountId)
->
    ...
    case kz_datamgr:get_results(AccountDb, <<"faxbox/email_permissions">>, ViewOptions) of
        ...
        {'ok', JObjs} ->
            maybe_faxbox_by_rules(JObjs, State#state{account_id = AccountId});
        {'error', _E} ->
            Error = <<"error getting faxbox email permissions for account ", AccountId/binary>>,
            lager:debug("error getting faxbox email permissions for account ~s : ~p", [
                AccountId, _E
            ]),
            State#state{errors = [Error | Errors]}
    end;
...
add_fax_document(
    ...
    ResellerId =
        case kzd_services:reseller_id(FaxBoxDoc) of
    ...
    FaxBoxNotify = kz_json:set_value(
        SendToKey,
...
-spec process_message(
    ...
    {'ok', state()}.
process_message(
    <<"multipart">>, Multipart, Headers, _Parameters, Body, #state{errors = Errors} = State
) ->
...
process_parts(
    ...
    {'ok', State#state{
        errors = [?ERROR_NO_VALID_ATTACHMENT | Errors],
        has_smtp_errors = 'true'
    }};
process_parts([], State) ->
    {'ok', State};
process_parts(
    [
        {Type, SubType, _Headers, Parameters, BodyPart}
        | Parts
    ],
    State
) ->
    {_, NewState} =
        maybe_process_part(
            kz_mime:normalize_content_type(<<Type/binary, "/", SubType/binary>>),
            Parameters,
            BodyPart,
            State
        ),
...
-spec maybe_process_part(
    ...
    {'ok', state()}.
maybe_process_part(
    <<"multipart/", Multipart/binary>>, _Parameters, Body, #state{errors = Errors} = State
) ->
    lager:debug("processing multipart/~s", [Multipart]),
    case Body of
        {Type, SubType, _HeadersPart, ParametersPart, BodyPart} ->
...
-spec process_part(kz_term:ne_binary(), binary() | mimemail:mimetuple(), state()) ->
    {'ok', state()}.
process_part(CT, Body, State) ->
...
maybe_ignore_no_valid_attachment(#state{filename = 'undefined'} = State) ->
    State;
...
is_allowed_content_type(CT) ->
    AllowedCT = kapps_config:get(
        ?CONFIG_CAT, <<"allowed_content_types">>, ?DEFAULT_ALLOWED_CONTENT_TYPES
    ),
    DeniedCT = kapps_config:get(
        ?CONFIG_CAT, <<"denied_content_types">>, ?DEFAULT_DENIED_CONTENT_TYPES
    ),
...
-spec content_type_matched_by(
    kz_term:ne_binary(), [kz_term:ne_binary() | kz_json:object()], binary()
) -> binary().
...
content_type_matched_json(CT, Type) ->
    case kz_json:is_json_object(Type) of
        'false' -> <<>>;
        'true' -> content_type_matched_json(CT, Type, <<"type">>)
    end.

-spec content_type_matched_json(kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()) ->
    binary().
...
-spec maybe_process_image(
    kz_term:ne_binary(), binary() | mimemail:mimetuple(), kz_term:ne_binary(), state()
) ->
    {'ok', state()}.
maybe_process_image(CT, Body, Size, State) ->
    {MinX, MinY} =
        case re:split(Size, "x") of
