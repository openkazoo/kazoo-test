...
migrate() ->
    ...
    lists:foldr(
        fun(A, C) -> migrate_faxes_fold(A, C, Total, ?DEFAULT_MIGRATE_OPTIONS) end, 1, Accounts
    ),
...
-spec migrate(
    kz_term:ne_binaries() | kz_term:ne_binary(), kz_term:ne_binary() | kz_term:proplist()
) -> 'ok'.
migrate([], _) ->
    'ok';
...
migrate_private_media(AccountDb, Doc, <<"tiff">>) ->
    ...
    'ok';
migrate_private_media(_AccountDb, _JObj, _MediaType) ->
    'ok'.
...
migrate_faxes_to_modb(Account, Options) ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"maintenance/listing_by_type">>, ViewOptions) of
        {'ok', []} ->
            io:format("no fax docs in db for fax migration ~s~n", [AccountDb]);
        {'ok', JObjs3} ->
            ...
            case kz_doc:attachments(Doc) of
                'undefined' ->
                    case kapps_config:get_is_true(?CONFIG_CAT, <<"delete_empty_faxes">>, 'false') of
                        'true' ->
                            io:format("deleting no attachments fax doc ~s from ~s~n", [
                                DocId, AccountDb
                            ]),
...
pending_jobs() ->
    ...
    io:format(FormatString, [
        <<"Job">>, <<"Date">>, <<"Account">>, <<"Faxbox">>, <<"From">>, <<"To">>, <<"Tries">>
    ]),
    io:format(
        "+==================================+===================+==================================+==================================+======================+======================+=======+~n",
        []
...
active_jobs() ->
    ...
    io:format(FormatString, [
        <<"Node">>, <<"Job">>, <<"Date">>, <<"Account">>, <<"FaxBox">>, <<"From">>, <<"To">>
    ]),
    io:format(
        "+======================+==================================+===================+==================================+==================================+======================+======================+~n",
        []
    ...
    _ =
        case kz_datamgr:get_results(?KZ_FAXES_DB, <<"faxes/processing_by_node">>) of
            {'ok', Jobs} ->
                [
                    io:format(FormatString, [
                        kz_json:get_value([<<"value">>, <<"node">>], JObj),
                        kz_json:get_value([<<"value">>, <<"id">>], JObj),
                        kz_time:format_datetime(
                            kz_json:get_value([<<"value">>, <<"modified">>], JObj)
                        ),
...
restart_job(JobID) ->
    _ =
        case update_job(JobID, <<"pending">>) of
            'ok' -> 'ok';
            {'error', _} = E -> lager:debug("restarting job ~p failed : ~p", [JobID, E])
        end,
    'no_return'.
...
update_job(JobID, State) ->
    case kz_datamgr:open_doc(?KZ_FAXES_DB, JobID) of
        {'error', _} = E -> E;
        {'ok', JObj} -> update_job(JobID, State, JObj)
    end.
...
maybe_migrate_outbound_fax(<<"_design/", _/binary>>) ->
    'ok';
maybe_migrate_outbound_fax(DocId) ->
...
maybe_migrate_outbound_fax(_Type, _JObj) ->
    'ok'.
...
migrate_outbound_fax(JObj) ->
    ...
    case kazoo_modb:move_doc(FromDB, FromId, ToDB, ToId, ?OVERRIDE_DOCS) of
        {'ok', _} ->
            io:format("document ~s/~s moved to ~s/~s~n", [FromDB, FromId, ToDB, ToId]);
        {'error', _E} ->
            io:format("error ~p moving document ~s/~s to ~s/~s~n", [_E, FromDB, FromId, ToDB, ToId])
    end.
...
load_smtp_attachment(DocId, Filename) ->
    case file:read_file(Filename) of
        ...
        Error ->
            io:format("error obtaining file ~s contents for docid ~s : ~p~n", [
                Filename, DocId, Error
            ])
    end.
    ...
    case kz_datamgr:open_cache_doc(?KZ_FAXES_DB, DocId) of
        {'ok', JObj} ->
            case kz_fax_attachment:save_outbound(?KZ_FAXES_DB, JObj, FileContents, CT) of
                {'ok', _Doc} ->
                    io:format("attachment ~s for docid ~s recovered~n", [Filename, DocId]);
                {'error', E} ->
                    io:format("error attaching ~s to docid ~s : ~p~n", [Filename, DocId, E])
            end;
        {'error', E} ->
            io:format("error opening docid ~s for attaching ~s : ~p~n", [DocId, Filename, E])
    end.
