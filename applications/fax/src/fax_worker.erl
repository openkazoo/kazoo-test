...
-define(RESPONDERS, [
    {{?MODULE, 'handle_tx_resp'}, [{<<"resource">>, <<"offnet_resp">>}]},
    {{?MODULE, 'handle_fax_event'}, [{<<"call_event">>, <<"CHANNEL_FAX_STATUS">>}]},
    {{?MODULE, 'handle_job_status_query'}, [{<<"fax">>, <<"query_status">>}]}
]).
-define(QUEUE_NAME, <<>>).
...
-define(DEFAULT_RETRY_PERIOD,
    kapps_config:get_integer(?CONFIG_CAT, <<"default_retry_period">>, 300)
).
-define(DEFAULT_RETRY_COUNT, kapps_config:get_integer(?CONFIG_CAT, <<"default_retry_count">>, 3)).
-define(DEFAULT_COMPARE_FIELD,
    kapps_config:get_binary(?CONFIG_CAT, <<"default_compare_field">>, <<"result_cause">>)
).
...
-define(NOTIFICATION_OUTBOUND_EMAIL, [
    <<"notifications">>,
    <<"outbound">>,
    <<"email">>,
    <<"send_to">>
...
handle_cast({'fax_status', Event, JobId, _}, State) ->
    ...
    {'noreply', State};
handle_cast(
    {'query_status', JobId, Queue, MsgId, _JObj},
    #state{
        status = Status,
        job_id = JobId,
...
handle_cast({'error', 'invalid_number', Number}, #state{job = JObj} = State) ->
    send_error_status(State, <<"invalid fax number">>),
    {Resp, Doc} = release_failed_job('invalid_number', Number, JObj),
    gen_server:cast(self(), 'stop'),
    {'noreply', State#state{job = Doc, resp = Resp}};
handle_cast({'error', 'invalid_cid', Number}, #state{job = JObj} = State) ->
...
terminate('normal' = _Reason, #state{
    error = Error, stage = Stage, job_id = JobId, controller = CtrlQ
}) when
    Error =/= 'undefined'
->
...
terminate('normal' = _Reason, #state{
    stage = Stage, job_id = JobId, controller = CtrlQ, queue_name = Q
}) ->
...
terminate(_Reason, #state{
    job = JObj, stage = Stage, job_id = JobId, controller = CtrlQ, queue_name = Q
}) ->
...
apply_reschedule_logic(JObj) ->
    ...
    case apply_reschedule_rules(kz_json:get_values(Map), set_default_update_fields(JObj)) of
        ...
        {'ok', JObj2} ->
            lager:debug(
                "rule '~s' applied in fax reschedule logic",
...
apply_reschedule_rules({[Rule | Rules], [Key | Keys]}, JObj) ->
    ...
    case
        (Attempt =:= Attempts orelse
            Attempt =:= -1) andalso
            lists:member(ResultValue, ValueList)
    of
        'true' ->
...
move_doc(JObj) ->
    ...
    lager:debug("moving fax outbound document ~s from faxes to ~s with id ~s", [
        FromId, AccountMODb, ToId
    ]),
...
-spec handle_if_moved_successfully(
    kz_json:object(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
fax_error(JObj) ->
    kz_json:get_first_defined(
        [
            [<<"Application-Data">>, <<"Fax-Result-Text">>],
...
notify_emails(JObj) ->
    Emails = kz_json:get_first_defined(
        [
            ?NOTIFICATION_OUTBOUND_EMAIL,
            ?NOTIFICATION_EMAIL
        ],
        JObj,
        []
...
notify_fields(JObj, Resp) ->
    ...
    FaxFields = [
        {<<"Fax-Hangup-Code">>, kz_term:to_integer(HangupCode)},
...
fax_fields(JObj) -> [{K, V} || {<<"Fax-", _/binary>> = K, V} <- kz_json:to_proplist(JObj)].
...
-spec send_fax(kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary(), binary() | 'undefined') ->
    'ok'.
...
send_fax(JobId, JObj, Q, ToDID) ->
    %kz_term:to_binary(kapps_config:get_is_true(?CONFIG_CAT, <<"ignore_early_media">>, 'false')),
...
get_hunt_account_id(AccountId) ->
    ...
    case kz_datamgr:get_results(AccountDb, ?CALLFLOW_LIST, Options) of
        {'ok', [JObj]} ->
            maybe_hunt_account_id(kz_json:get_value([<<"doc">>, <<"flow">>], JObj), AccountId);
        _ ->
            'undefined'
    end.

-spec maybe_hunt_account_id(kz_term:api_object(), kz_term:ne_binary()) -> kz_term:api_binary().
maybe_hunt_account_id('undefined', _) ->
    'undefined';
