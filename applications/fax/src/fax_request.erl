...
-define(RESPONDERS, [
    {fun handle_fax_event/2, [{<<"call_event">>, <<"CHANNEL_FAX_STATUS">>}]},
    {fun handle_channel_event/2, [{<<"call_event">>, <<"CHANNEL_DESTROY">>}]}
]).

-define(NOTIFICATION_INBOUND_EMAIL, [
    <<"notifications">>,
    <<"inbound">>,
    <<"email">>,
    <<"send_to">>
...
start_link(Call, JObj, Storage) ->
    gen_listener:start_link(
        {'local', kz_term:to_atom(Storage#fax_storage.id, 'true')},
        ?MODULE,
        [
...
start_receive_fax(
    ...
    send_status(
        NewState,
        list_to_binary(["New Fax from ", kapps_call:caller_id_number(Call)]),
        ?FAX_START,
        'undefined'
    ),
...
maybe_update_fax_settings(
    ...
    case kz_datamgr:open_cache_doc(AccountDb, OwnerId) of
        {'ok', JObj} ->
            ...
            Notify =
                case kz_json:get_value(<<"email">>, JObj) of
                    'undefined' ->
                        kz_json:new();
                    UserEmail ->
                        kz_json:set_value([<<"email">>, <<"send_to">>], [UserEmail], kz_json:new())
                end,
...
get_faxbox_user_email(FaxBoxDoc, AccountDb) ->
    case kz_json:get_value(<<"owner_id">>, FaxBoxDoc) of
        ...
        OwnerId ->
            case kz_datamgr:open_cache_doc(AccountDb, OwnerId) of
                {'ok', UserDoc} ->
                    kz_json:get_ne_binary_value(<<"email">>, UserDoc);
                _ ->
...
end_receive_fax(#state{page = Page, fax_result = JObj} = State) when Page =:= 0 ->
    Props = [
        {[<<"Application-Data">>, <<"Fax-Success">>], 'false'},
...
store_document(
    ...
    case create_fax_doc(JObj, State) of
        {'ok', FaxDoc} ->
            lager:debug("fax document stored successfully into ~s / ~s", [
                kz_doc:account_db(FaxDoc), FaxId
            ]),
...
store_attachment(
    ...
    case kapps_call_command:store_file(FaxFile, FaxUrlFun, Call) of
        'ok' ->
            lager:debug("fax attachment stored successfully into ~s / ~s / ~s", [
                kz_doc:account_db(FaxDoc), FaxId, AttachmentId
            ]),
        ...
        {'error', _} ->
            case check_fax_attachment(FaxDb, FaxId, AttachmentId) of
                {'ok', _} ->
                    lager:debug(
                        "fax attachment stored successfully despite error into ~s / ~s / ~s", [
                            kz_doc:account_db(FaxDoc), FaxId, AttachmentId
                        ]
                    ),
                    gen_server:cast(Pid, 'success');
...
new_request(JObj, _Props) ->
    ...
    Props = [
        {<<"Fax-Doc-ID">>, Storage#fax_storage.id},
