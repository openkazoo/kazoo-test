...
handle_job_notify(JObj, _Props) ->
    ...
    case
        FetchRes =:= 'ok' andalso
            kz_json:get_value(<<"cloud_job_id">>, MaybeFaxJObj)
    of
        'false' ->
        ...
        {'ok', Authorization} ->
            ...
            case kz_http:get(kz_term:to_list(URL), Headers) of
                ...
                {'ok', 403, _RespHeaders, _RespBody} ->
                    ...
                    _ = [
                        lager:debug("resp header: ~p", [_RespHeader])
                     || _RespHeader <- _RespHeaders
                    ],
...
maybe_process_job([], _Authorization) ->
    'ok';
    ...
    StateObj = kz_json:from_list(
        [
            {<<"state">>,
                ?JSON([
                    {<<"type">>, Status},
                    {<<"device_action_cause">>, ?JSON([{<<"error_code">>, <<"OTHER">>}])}
                ])}
        ]
    ),
    update_job_status(PrinterId, JobId, StateObj);
update_job_status(PrinterId, JobId, Status) ->
    case get_printer_oauth_credentials(PrinterId) of
        ...
        {'error', E} ->
            lager:debug(
                "error getting printer (~s) oauth credentials when updating job (~s) status : ~p", [
                    PrinterId, JobId, E
                ]
            )
    end.
...
-spec maybe_save_fax_attachment(
...
maybe_save_fax_attachment(JObj, JobId, PrinterId, FileURL) ->
    case get_printer_oauth_credentials(PrinterId) of
        ...
        {'error', E} ->
            lager:debug("error getting printer (~s) oauth credentials for JobId (~s) : ~p", [
                PrinterId, JobId, E
            ])
    end.

-spec save_fax_document(
    kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
save_fax_document(Job, JobId, PrinterId, FaxNumber) ->
    ...
    FaxBoxEmailNotify = kz_json:get_value(
        [
            <<"notifications">>,
            <<"outbound">>,
            <<"email">>,
...
get_faxbox_doc(PrinterId) ->
    case kz_cache:peek_local(?CACHE_NAME, {'faxbox', PrinterId}) of
        {'ok', _Doc} = OK -> OK;
        {'error', _} -> fetch_faxbox_doc(PrinterId)
    end.
...
get_printer_oauth_credentials(PrinterId) ->
    case kz_cache:peek_local(?CACHE_NAME, {'gcp', PrinterId}) of
        {'ok', _Auth} = OK -> OK;
        {'error', _} -> fetch_printer_oauth_credentials(PrinterId)
    end.
...
fetch_printer_oauth_credentials(PrinterId) ->
    case get_faxbox_doc(PrinterId) of
        {'error', _} = E ->
            E;
        {'ok', JObj} ->
            {'ok', App} = kazoo_oauth_util:get_oauth_app(
                kz_json:get_value(<<"pvt_cloud_oauth_app">>, JObj)
            ),
            RefreshToken = #oauth_refresh_token{
                token = kz_json:get_value(<<"pvt_cloud_refresh_token">>, JObj)
            },
            {'ok', #oauth_token{expires = Expires} = Token} = kazoo_oauth_util:token(
                App, RefreshToken
            ),
...
check_registration(_, 'undefined', _JObj) ->
    'ok';
check_registration(_, <<"expired">>, _JObj) ->
    'ok';
...
check_registration(_, _, _JObj) ->
    'ok'.

-spec process_registration_result(
    boolean(), kz_term:ne_binary(), kz_json:object(), kz_json:object()
) -> any().
process_registration_result('true', AppId, JObj, Result) ->
    ...
    {'ok', Token} = kazoo_oauth_util:refresh_token(
        App, Scope, AuthorizationCode, [?GPC_PROXY_HEADER], <<"oob">>
    ),
...
process_registration_result('false', AppId, JObj, _Result) ->
    ...
    case Elapsed > TokenDuration of
        ...
        'false' ->
            SleepTime = kapps_config:get_integer(
                ?CONFIG_CAT, <<"cloud_registration_pool_interval">>, ?DEFAULT_CLOUD_REG_SLEEP
            ),
            lager:debug(
                "Printer ~s not claimed at ~s. sleeping for ~B seconds, Elapsed/Duration (~p/~p).",
