...
tone_or_echo(Call) ->
    ...
    case {kapps_config:get_json(?CONFIG_CAT, <<"echo">>, ?ECHO), ?TONE} of
        {'undefined', 'undefined'} -> 'undefined';
        {Echo, 'undefined'} -> maybe_echo(Echo, To, From);
        {'undefined', Tone} -> maybe_tone(Tone, To, From);
        {Echo, Tone} -> maybe_echo_maybe_tone(Echo, Tone, To, From)
    end.
...
-spec maybe_echo_maybe_tone(
    kz_json:object(), kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    'undefined' | 'tone' | 'echo'.
maybe_echo_maybe_tone(Echo, Tone, To, From) ->
    case {rule_exist(Echo, To, From), rule_exist(Tone, To, From)} of
        {'true', 'true'} ->
            lager:info("conflict on milliwatt actions switching back to default action: ~p", [
                ?CONFLICT_ACTION
            ]),
            ?CONFLICT_ACTION;
        {'true', _} ->
...
rule_exist(JObj, To, From) ->
    ...
    (not (not lists:member(To, Numbers)) andalso
        (not lists:member(From, CallerIds))).
