...
validate(Context, #{keys := [Severity, Verbosity, _AppName]}) ->
    case
        {
            lists:member(Severity, ?EDR_SEVERITY_BINARIES),
            lists:member(Verbosity, ?EDR_VERBOSITY_BINARIES)
        }
    of
        {'false', _} ->
            bh_context:add_error(
                Context,
                <<"severity must be one of: ", (kz_binary:join(?EDR_SEVERITY_BINARIES))/binary>>
            );
        {_, 'false'} ->
            bh_context:add_error(
                Context,
                <<"verbosity must be one of: ", (kz_binary:join(?EDR_VERBOSITY_BINARIES))/binary>>
            );
        {'true', 'true'} ->
            Context
    end;
validate(Context, #{keys := _Keys}) ->
    bh_context:add_error(
        Context, <<"subscription should be in format: edr.severity.verbosity.app_name">>
    ).
