...
init(#backend{options = Options}) ->
    HttpcOptions = props:filter_undefined([
        {'connect_timeout', kz_json:get_integer_value(<<"connect_timeout">>, Options)},
    ...
    case kz_json:get_value(<<"url">>, Options) of
        ...
        Url ->
            {'ok', #state{
                url = kz_term:to_list(Url),
                method = kz_json:get_atom_value(<<"method">>, Options, 'post'),
                headers = kz_json:to_proplist(
                    kz_json:get_json_value(<<"headers">>, Options, kz_json:new())
                ),
...
push(
    #state{async = 'false', httpc_options = Opts, url = Url, method = Method, headers = Headers} =
        State,
    #edr_event{} = Event
) ->
...
push(
    #state{async = 'true', httpc_options = _Opts, url = _Url, method = _Method, headers = _Headers} =
        _State,
    #edr_event{} = _Event
) ->
