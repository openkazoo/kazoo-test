...
-record(state, {
    tasks = #{} :: #{kz_tasks:id() => kz_tasks:task()},
...
%% @end
%%------------------------------------------------------------------------------
-spec worker_finished(kz_tasks:id(), non_neg_integer(), non_neg_integer(), kz_tasks:columns()) ->
    'ok'.
worker_finished(TaskId = ?NE_BINARY, TotalSucceeded, TotalFailed, Columns) when
    is_integer(TotalSucceeded), is_integer(TotalFailed)
->
...
maybe_strip_columns(Columns, CSVPath, ColumnsWritten) ->
    ...
    case length(FullHeader) of
        ...
        _DamnYouMoreColumn ->
            throw({'error', <<"more columns were written">>})
    end.
...
finish_task(API, Data) ->
    ...
    case tasks_bindings:apply(API, <<"finish">>, [Action, Data]) of
        ...
        [{'EXIT', {_E, _Rs}}] -> lager:debug("finish ~p: ~p", [_E, hd(_Rs)]);
...
cleanup_task(API, Data) ->
    ...
    case tasks_bindings:apply(API, <<"cleanup">>, [Action, Data]) of
        ...
        [{'EXIT', {_E, _Rs}}] -> lager:debug("cleanup ~p: ~p", [_E, hd(_Rs)]);
...
task_by_pid(Pid, #state{tasks = Tasks}) ->
    F = fun(_, #{worker_pid := WorkerPid} = Task, Acc) ->
        Pid =:= WorkerPid andalso
            throw({'task', Task}),
        Acc
    end,
    try
        maps:fold(F, 'undefined', Tasks)
    catch
        'throw':{'task', Task} -> Task
    end.
...
-spec update_task(kz_tasks:task()) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
