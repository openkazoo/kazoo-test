...
action(<<"list_all">>) ->
    #{
        <<"description">> =>
            <<"List all numbers assigned to the account starting the task & its subaccounts">>,
        <<"doc">> => list_doc()
    };
action(<<"find">>) ->
    #{
...
action(<<"import">>) ->
    #{
        ...
        <<"doc">> => <<
            "Creates numbers from fields similar to list tasks.\n"
            ...
            "Note: `carrier_module` defaults to '",
            (?IMPORT_DEFAULTS_TO_CARRIER)/binary,
            "'.\n"
...
action(<<"assign_to">>) ->
    #{
        ...
        <<"doc">> => <<
            "Assign existing numbers to another account.\n"
            "Note: number must be E164-formatted.\n"
...
action(<<"update_merge">>) ->
    #{
        ...
        <<"doc">> => <<
            "Update features and/or public fields of existing numbers.\n"
...
action(<<"update_overwrite">>) ->
    #{
        <<"description">> => <<"Bulk-update numbers">>,
        <<"doc">> => <<
            "Reset features and/or public fields of existing numbers.\n"
...
action(<<"release">>) ->
    #{
        ...
        <<"doc">> => <<
            "Release numbers (removing happens if account is configured so).\n"
            "Note: number must be E164-formatted.\n"
            "Note: number must already exist.\n"
            "Note: account creating the task (or `auth_by` account) must have permissions on number.\n"
        >>,
...
action(<<"reserve">>) ->
    #{
        ...
        <<"doc">> => <<
            "Sets numbers to state 'reserved' (creating number if it is missing).\n"
            "Note: number must be E164-formatted.\n"
...
action(<<"delete">>) ->
    #{
        ...
        <<"doc">> => <<
            "Forces numbers to be deleted from the system.\n"
            "Note: number must be E164-formatted.\n"
            "Note: number must already exist.\n"
            "Note: account creating the task (or `auth_by` account) must have permissions on number.\n"
        >>,
...
import(
    ...
    Args = #{
        ...
        <<"created">> := _,
        <<"modified">> := _,
        %%FIXME: decide whether to use this one
        <<"used_by">> := _
    }
) ->
...
import_module_name(AuthBy, Carrier) ->
    case
        kzd_accounts:is_superduper_admin(AuthBy) andalso
            Carrier
    of
...
assign_to(
    #{auth_account_id := AuthBy, account_id := Account},
    _IterValue,
...
-spec update_merge(kz_tasks:extra_args(), kz_tasks:iterator(), kz_tasks:args()) ->
    kz_tasks:return().
update_merge(
...
-spec update_overwrite(kz_tasks:extra_args(), kz_tasks:iterator(), kz_tasks:args()) ->
    kz_tasks:return().
update_overwrite(
    #{auth_account_id := AuthBy},
    _IterValue,
...
handle_result(Args, {'error', Reason}) when
    is_atom(Reason)
->
...
-spec format_result(kz_tasks:args(), kz_term:ne_binary() | knm_number:knm_number()) ->
    kz_csv:mapped_row().
...
-type accountid_or_startkey_and_numberdbs() :: [
    {kz_term:ne_binary() | kz_term:ne_binaries(), kz_term:ne_binary()}
].
...
-spec view_for_list_assigned(kz_term:ne_binary() | kz_term:ne_binaries()) ->
    kz_datamgr:view_options().
...
-spec dump_next(
    fun((startkey_or_numberdb()) -> {kz_term:ne_binary(), kz_datamgr:view_options()}),
    startkey_or_numberdb()
) ->
...
-spec db_and_view_for_dump(kz_term:ne_binary() | kz_term:ne_binaries()) ->
    {kz_term:ne_binary(), kz_datamgr:view_options()}.
db_and_view_for_dump(NumberDb = ?NE_BINARY) ->
    {NumberDb, []};
...
-spec db_and_view_for_dump_by_state(
