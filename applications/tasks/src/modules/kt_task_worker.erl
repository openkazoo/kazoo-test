...
init(TaskId, API, ExtraArgs) ->
    case kz_datamgr:fetch_attachment(?KZ_TASKS_DB, TaskId, ?KZ_TASKS_ANAME_IN) of
        {'error', _R} = Error ->
            lager:error(
                "failed loading attachment ~s from ~s/~s: ~p",
...
teardown(API, IterValue, #state{
    ...
    _ = kz_tasks_scheduler:worker_finished(
        TaskId,
        TotalSucceeded,
        TotalFailed,
        Columns
...
-spec new_state_after_writing(kz_tasks:columns(), non_neg_integer(), non_neg_integer(), state()) ->
    state().
new_state_after_writing(
    NewColumns,
    WrittenSucceeded,
    WrittenFailed,
    State = #state{
    ...
    S = State#state{
        total_succeeded = NewTotalSucceeded = WrittenSucceeded + TotalSucceeded,
...
-spec store_return(state(), kz_csv:mapped_row(), kz_tasks:return()) ->
    {kz_tasks:columns(), pos_integer()}.
