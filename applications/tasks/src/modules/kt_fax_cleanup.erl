...
process_chunks(#{<<"start_key">> := 'done'} = State) ->
    State;
...
move_docs([Doc | Docs], State) -> move_docs(Docs, move_doc(Doc, State)).
...
handle_doc(Doc, 'true', State) ->
    ...
    case kazoo_modb:move_doc(FromDB, {<<"fax">>, FromId}, ToDB, ToId, ?MOVE_OPTIONS) of
        ...
        {'error', 'not_found'} ->
            lager:debug("doc move failed for ~s because modb ~s not found, deleting doc", [
                FromId, ToDB
            ]),
...
fetch_fax_docs(State, Options) ->
    case kz_datamgr:paginate_results(?KZ_FAXES_DB, ?CROSSBAR_LISTING, Options) of
        {'ok', Page, 'undefined'} ->
            {
                filter_datetime(Page),
                bump(State#{<<"start_key">> => 'done'}, <<"total_docs">>, length(Page))
            };
        {'ok', Page, NextKey} ->
            {
                filter_datetime(Page),
                bump(State#{<<"start_key">> => NextKey}, <<"total_docs">>, length(Page))
            };
        {'error', Message} ->
...
-spec maybe_update_status(kzd_fax:doc(), kz_term:ne_binary(), kz_term:ne_binary(), boolean()) ->
    kzd_fax:doc().
...
maybe_fetch_account(State, 'undefined') ->
    State;
...
account_exists(#{<<"accounts">> := Accounts}, AccountId) -> maps:get(AccountId, Accounts, 'false').
...
%% @end
%%------------------------------------------------------------------------------
-spec log_doc_moved(kzd_fax:doc(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
log_doc_moved(Doc, FromId, ToDB, ToId) ->
    Status = kzd_fax:job_status(Doc, <<"unknown">>),
    lager:debug(
        "moving: ~s modified: ~s to_modb: ~s new_id ~s, job_status: ~s,  tx_result ~s folder: ~s attachments ~p",
        [
            FromId,
...
log_doc_deleted(Doc) ->
    Status = kzd_fax:job_status(Doc, <<"unknown">>),
    lager:debug(
        "removing: ~s modified: ~s account: ~s job_status: ~s tx_result: ~s folder: ~s attachments: ~p",
        [
            kz_doc:id(Doc),
            kz_date:to_iso8601_extended(kz_doc:created(Doc)),
...
log_report(#{<<"counter">> := Report}) ->
    ...
    Message = maps:fold(
        fun(K, V, Acc) -> [<<K/binary, ": ", (kz_term:to_binary(V))/binary>> | Acc] end,
        [],
        NewMap
    ),
