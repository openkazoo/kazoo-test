...
action(<<"descendant_quantities">>) ->
    [
        ...
        {<<"doc">>, <<
            "Attempts to create a month-on-month listing of quantities used by descendant accounts.\n"
...
descendant_quantities(#{account_id := AccountId}, 'init') ->
    ...
    lager:debug(
        "found ~p descendants & ~p MoDBs in total",
...
modb_service_quantities(MoDB, Id) ->
    case kz_datamgr:open_doc(MoDB, Id) of
        {'ok', JObj} ->
            kz_json:get_value(<<"quantities">>, JObj);
        {'error', _R} ->
...
-spec maybe_integer_to_binary(kz_term:ne_binary(), kz_term:api_object()) ->
    kz_term:api_non_neg_integer().
maybe_integer_to_binary(_, 'undefined') ->
    'undefined';
...
cleanup_orphaned_services_docs() ->
    case kz_datamgr:all_docs(?KZ_SERVICES_DB) of
        ...
        {'error', _E} -> lager:debug("failed to get all docs from ~s: ~p", [?KZ_SERVICES_DB, _E])
    end.
...
cleanup_orphaned_services_doc(<<"_design/", _/binary>>) ->
    'ok';
