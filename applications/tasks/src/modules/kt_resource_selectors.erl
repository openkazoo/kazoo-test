...
import(
    ...
    Acc =
        case (ProcessedRows + 1) rem Limit =:= 0 of
            'false' ->
                [Doc | Docs];
            'true' ->
...
do_insert(Db, Docs) ->
    case kz_datamgr:save_docs(Db, Docs) of
        ...
        {'error', 'not_found'} ->
            ...
            do_insert(Db, Docs);
        {'error', E} ->
            lager:error("error adding selectors: ~p", [E])
    end.
...
-spec split_keys(kz_term:ne_binaries(), [kz_term:ne_binaries()], non_neg_integer()) ->
    [kz_term:ne_binaries()].
split_keys([], Acc, _BlockSize) ->
    Acc;
...
refresh_selectors_index(Db) ->
    ...
    {'ok', _} = kz_datamgr:get_results(
        Db, <<"resource_selectors/resource_name_selector_listing">>, [{'limit', 1}]
    ),
    'ok'.

-spec generate_selector_doc(
    kz_term:ne_binary(),
    kz_term:api_binary(),
    kz_term:api_binary(),
    kz_term:api_binary(),
