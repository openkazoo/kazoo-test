...
action(<<"delete">>) ->
    ...
    [
        {<<"description">>,
            <<"Bulk-remove rates where they match with all defined fields in CSV row">>},
...
export(ExtraArgs, 'init') ->
    case is_allowed(ExtraArgs) of
        ...
        'false' ->
            lager:warning(
                "rates exporting is forbidden for account ~s, auth account ~s",
                [
...
export(_ExtraArgs, State) ->
    ...
    case kz_datamgr:get_results(Db, ?RATES_VIEW, Options) of
        ...
        {'ok', Results} when length(Results) >= Limit ->
            ...
            NewOptions = props:set_values(
                [
                    {'startkey', kz_json:get_value(<<"key">>, Last)},
...
-spec import(kz_tasks:extra_args(), map() | kz_tasks:iterator(), kz_tasks:args()) ->
    kz_tasks:iterator().
import(ExtraArgs, 'init', Args) ->
    case is_allowed(ExtraArgs) of
        ...
        'false' ->
            lager:warning(
                "rates importing is forbidden for account ~s, auth account ~s",
                [
                    maps:get('account_id', ExtraArgs),
                    maps:get('auth_account_id', ExtraArgs)
                ]
...
import(_ExtraArgs, Map, Args) ->
    ...
    case maps:get(Db, Map, 'undefined') of
        'undefined' ->
            lager:debug(
                "adding rate ~s (prefix ~p) to ratedeck '~s'",
...
delete(ExtraArgs, 'init', Args) ->
    ...
    case is_allowed(ExtraArgs) of
        ...
        'false' ->
            lager:warning(
                "rates deleting is forbidden for account ~s, auth account ~s",
                [
                    maps:get('account_id', ExtraArgs),
                    maps:get('auth_account_id', ExtraArgs)
                ]
...
delete(_ExtraArgs, State, Args) ->
    ...
    case Count rem Limit of
        ...
        _Rem ->
            lager:debug(
                "adding rate ~s (prefix ~p) to bulk delete list",
...
-spec import_rates_into_ratedeck(kz_term:ne_binary(), {non_neg_integer(), kz_json:objects()}) ->
    'true'.
...
import_rates_into_ratedeck(RatedeckDb, {_C, Rates}) ->
    lager:debug(
        "importing ~p rate~s into ratedeck '~s'",
...
is_allowed(ExtraArgs) ->
    ...
    kzd_accounts:is_in_account_hierarchy(AuthAccountId, AccountId, 'true') andalso
        %% Serve request for reseller rates
        kzd_accounts:is_reseller(AccountDoc) orelse
...
validate_field(RateJObj, <<"caller_id_numbers">>) ->
    case maybe_generate_caller_id_numbers(RateJObj) of
        ...
        Value ->
            kzd_rates:set_caller_id_numbers(RateJObj, kz_binary:join(Value, <<":">>))
    end;
...
maybe_override_rate(Db, Rates) ->
    ...
    case kz_datamgr:open_docs(Db, Ids) of
        {'ok', Result} ->
            ...
            [
                kz_json:merge(
                    maps:get(kz_doc:id(JObj), DbJObjs, kz_json:new()), update_pvt(Db, JObj)
                )
             || JObj <- Rates
            ];
        {'error', _Reason} ->
...
delete_rates(Db, JObjs, 'ok') ->
    ...
    case kz_datamgr:get_results(Db, ?RATES_VIEW, Options) of
        ...
        {'ok', Results} ->
            {NotFound, Docs} = lists:foldl(
                fun maybe_delete_rate/2,
                {PrefixGrouped, []},
                Results
            ),
...
log_not_found(JObj) ->
    lager:debug(
        "requested rate ~s (prefix: ~p) (id: ~s) was not found",
        [
...
-spec should_delete_rate(kz_json:objects(), kz_json:object()) ->
    'false' | {'true', kz_json:objects()}.
...
maybe_generate_name(RateJObj, 'undefined') ->
    generate_name(
        kzd_rates:prefix(RateJObj),
        kzd_rates:iso_country_code(RateJObj, <<"XX">>),
        kzd_rates:direction(RateJObj, [])
    );
maybe_generate_name(_RateJObj, Name) ->
    Name.

-spec generate_name(
    kz_term:ne_binary() | pos_integer(), kz_term:api_ne_binary(), kz_term:ne_binaries()
) -> kz_term:ne_binary().
...
maybe_generate_weight(_RateJObj, Weight) ->
    kzd_rates:constrain_weight(Weight).

-spec generate_weight(
    kz_term:ne_binary() | pos_integer(), kz_currency:units(), kz_currency:units()
) ->
...
-spec maybe_generate_routes(kzd_rates:doc(), binary() | [binary()] | any()) ->
    kz_term:api_ne_binaries().
