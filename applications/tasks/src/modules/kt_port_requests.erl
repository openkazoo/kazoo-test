...
-define(UNFINISHED_PORT_REQUEST_LIFETIME,
    kapps_config:get_integer(
        ?CONFIG_CAT, <<"unfinished_port_request_lifetime_s">>, ?SECONDS_IN_DAY * 30
    )
).
...
unconfirmed_port_reminder(AccountDb) ->
    ...
    ViewOpts = [
        {'startkey', [AccountId, ?PORT_UNCONFIRMED, kz_json:new()]},
...
cleanup(Db, OldPortRequests) ->
    ...
    Deletable = [
        kz_json:get_value(<<"doc">>, OldPortRequest)
...
unconfirmed_port_reminder(AccountId, UnfinishedPorts) ->
    lager:debug("found ~p unfinished port requests, sending notifications", [
        length(UnfinishedPorts)
    ]),
