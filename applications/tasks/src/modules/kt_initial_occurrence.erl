...
maybe_test_for_registrations(AccountId, AccountJObj) ->
    ...
    case
        Realm =:= 'undefined' orelse
            kzd_accounts:sent_initial_registration(AccountJObj)
    of
        'true' -> 'ok';
...
notify_initial_registration(AccountJObj) ->
    ...
    Req = [
        {<<"Account-ID">>, kz_doc:id(AccountJObj)},
...
%% First Call
-spec maybe_test_for_initial_call(kz_term:ne_binary(), kz_term:ne_binary(), kzd_accounts:doc()) ->
    'ok'.
maybe_test_for_initial_call(AccountId, AccountDb, AccountJObj) ->
