...
-define(JOB_TABLE_HEADER, begin
    ?JOB_TABLE_EDGE,
    io:format(?FORMAT_STRING, [
        <<"Job">>, <<"Modified">>, <<"Type">>, <<"Attempts">>, <<"Failure Reason">>
    ]),
...
pending(ShowCount, <<"true">>) ->
    ...
    _ =
        case kz_datamgr:get_results(?KZ_PENDING_NOTIFY_DB, ?PENDING_VIEW, ViewOptions) of
...
failed(ShowCount, <<"true">>) ->
    ...
    _ =
        case kz_datamgr:get_results(?KZ_PENDING_NOTIFY_DB, ?FAILED_VIEW, ViewOptions) of
            {ok, JObjs} -> [print_json(kz_json:get_value(<<"doc">>, J)) || J <- JObjs];
    ...
    ToBeDeleted =
        case
            kz_datamgr:get_results(
                ?KZ_PENDING_NOTIFY_DB, <<"pending_notify/pending_range">>, ViewOptions
            )
        of
            {ok, []} ->
...
delete_between(StartTimestamp, EndTimestamp) ->
    ...
    ToBeDeleted =
        ...
        of
            {ok, []} ->
                io:format(
                    "no notification between start timestamp ~b and end timestamp ~b has been found~n",
                    [Start, End]
                ),
                [];
            {ok, JObjs} ->
...
statistics() ->
    ...
    case
        kz_datamgr:get_results(
            ?KZ_PENDING_NOTIFY_DB, <<"pending_notify/pending_by_type">>, ViewOptions
        )
    of
        {'ok', Rs} ->
            ...
            {'ok', JObjs} ->
                [
                 ...
                 || JObj <- [
                        J
                     || J <- JObjs,
                        not lists:member(kz_json:get_value([<<"value">>, <<"id">>], J), RunningIds)
                    ]
                ];
            {'error', _Reason} ->
...
format_modified_date(JObj) ->
    Modified = kz_json:get_integer_value(
        <<"modified">>, JObj, kz_json:get_integer_value(<<"pvt_modified">>, JObj)
    ),
