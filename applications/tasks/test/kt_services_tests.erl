...
rows_for_missing_eom_test() ->
    ...
    ?assertEqual(
        Expected, kt_services:rows_for_quantities(AccountId, YYYY, MM, bom_2(), kz_json:new())
    ).
...
rows_for_missing_bom_test() ->
    ...
    ?assertEqual(
        Expected, kt_services:rows_for_quantities(AccountId, YYYY, MM, kz_json:new(), bom_2())
    ).
