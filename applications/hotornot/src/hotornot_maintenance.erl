...
-spec rates_for_did(
    kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:api_ne_binary(), trunking_options()
) -> 'ok'.
rates_for_did(DID, Direction, AccountId, RouteOptions) when is_list(RouteOptions) ->
    case hon_util:candidate_rates(DID, AccountId) of
        {'ok', []} ->
            io:format("rate lookup had no results~n");
        {'error', _E} ->
            io:format("rate lookup error: ~p~n", [_E]);
        {'ok', Rates} ->
...
rates_between(Pre, Post) ->
    ViewOpts = [
        {'startkey', kz_term:to_binary(Pre)},
