...
-define(ROUTE_WIN_TIMEOUT,
    kapps_config:get_integer(?CF_CONFIG_CAT, ?ROUTE_WIN_TIMEOUT_KEY, ?DEFAULT_ROUTE_WIN_TIMEOUT)
).
...
handle_req(RouteReq, Props) ->
    ...
    case
        is_binary(kapps_call:account_id(Call)) andalso
            callflow_should_respond(Call)
    of
        'true' ->
            lager:info(
                "received request ~s asking if callflows can route the call to ~s",
...
allow_no_match(Call) ->
    is_valid_endpoint(kapps_call:custom_channel_var(<<"Referred-By">>, Call), Call) orelse
        is_valid_endpoint(kapps_call:custom_channel_var(<<"Redirected-By">>, Call), Call) orelse
        allow_no_match_type(Call) orelse
        is_authz_context(Call).

-spec is_authz_context(kapps_call:call()) -> boolean().
is_authz_context(Call) ->
    is_authz_context(
        Call, kapps_config:is_true(?APP_NAME, <<"allow_authz_context_overrides">>, 'false')
    ).
...
is_authz_context(Call, 'true') ->
    ...
    is_binary(CallContext) andalso
        lists:member(CallContext, AuthzContexts).
...
send_route_response(Flow, RouteReq, Call) ->
    ...
    of
        {'ok', RouteWin} ->
            ...
            NewCall = cf_route_win:execute_callflow(
                RouteWin, kapps_call:from_route_win(RouteWin, Call)
            ),
...
response_ccvs_from_device(Call, {'ok', DeviceJObj}) ->
    kz_json:set_values(
        [{<<"Presence-ID">>, kzd_devices:calculate_presence_id(DeviceJObj)}],
        default_response_ccvs(Call)
    );
...
wait_for_running(Call, N) ->
    ...
    receive
        {'channel_destroy', 'undefined'} ->
            ...
            lager:info(
                "received channel destroy with undefined fetch-id while setting up callflow executor, exiting"
            );
...
pre_park_action(Call) ->
    case
        kapps_config:get_is_true(?CF_CONFIG_CAT, <<"ring_ready_offnet">>, 'true') andalso
            kapps_call:inception(Call) =/= 'undefined' andalso
            kapps_call:authorizing_type(Call) =:= 'undefined'
    of
        'false' -> <<"none">>;
        'true' -> <<"ring_ready">>
    end.
...
update_call(Flow, NoMatch, ControllerQ, Call) ->
    ...
    Props = [
        {'cf_flow_id', FlowId},
        {'cf_flow_name',
            kz_json:get_ne_binary_value(<<"name">>, Flow, kapps_call:request_user(Call))},
        ...
        {'cf_metaflow',
            kz_json:get_value(
                <<"metaflows">>, Flow, ?DEFAULT_METAFLOWS(kapps_call:account_id(Call))
            )}
    ],

    Updaters = [
        {fun kapps_call:kvs_store_proplist/2, Props},
...
maybe_fix_restrictions('undefined', Call) ->
    Call;
