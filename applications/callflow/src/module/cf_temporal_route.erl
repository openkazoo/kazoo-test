...
process_rules(
    ...
    case {BaseTime + TStart, BaseTime + TStop} of
        {Start, _} when LSec < Start ->
            lager:info("rule applies in the future ~w", [
                calendar:gregorian_seconds_to_datetime(Start)
            ]),
            process_rules(T, Rules, Call);
        {_, End} when LSec > End ->
            lager:info("rule was valid today but expired ~w", [
                calendar:gregorian_seconds_to_datetime(End)
            ]),
            process_rules(T, Rules, Call);
        {_, End} ->
            lager:info("within active time window until ~w", [
                calendar:gregorian_seconds_to_datetime(End)
            ]),
            Id
    end;
...
-spec get_temporal_rules(
    kz_json:path(), non_neg_integer(), kz_term:ne_binary(), kz_term:ne_binary(), rules()
) -> rules().
...
get_temporal_rules([], _, _, _, _, Rules) ->
    lists:reverse(Rules);
...
maybe_build_rule(Routes, LSec, AccountDb, TZ, Now, Rules, Id, RulesDoc) ->
    ...
    case kz_date:relative_difference(Now, {StartDate, {0, 0, 0}}) of
        'future' ->
            ...
            get_temporal_rules(Routes, LSec, AccountDb, TZ, Now, Rules);
        _ ->
            get_temporal_rules(Routes, LSec, AccountDb, TZ, Now, [
                build_rule(Id, RulesDoc, StartDate, RuleName) | Rules
            ])
    end.

-spec build_rule(
    kz_term:ne_binary(), kzd_temporal_rules:doc(), kz_time:date(), kz_term:ne_binary()
) -> rule().
build_rule(Id, RulesDoc, StartDate, RuleName) ->
    #rule{
        cycle = kzd_temporal_rules:cycle(RulesDoc, ?RULE_DEFAULT_CYCLE),
...
load_current_time(#temporal{timezone = Timezone} = Temporal) ->
    ...
    Temporal#temporal{
        local_sec = calendar:datetime_to_gregorian_seconds({LocalDate, LocalTime}),
...
next_rule_date(
    ...
    lager:debug(
        "today is: ~p dow: ~p, startdate is: ~p, start dow is ~b, interval is: ~b, distance is: ~b, offset is: ~b, rule days found: ~p",
        [
            Today,
            DOW0,
            StartDate,
...
next_rule_date(
    ...
    Offset = trunc(Distance / I0) * I0,
    case
        Distance =:= Offset andalso
            kz_date:find_next_weekday({Y1, M1, D1}, Weekday)
    of
...
next_rule_date(
    ...
    case
        Distance =:= Offset andalso
            find_last_weekday({Y1, M1, 1}, Weekday)
    of
...
next_rule_date(
    ...
    case
        Distance =:= Offset andalso
            {find_ordinal_weekday(Y1, M1, Weekday, Ordinal), I0}
    of
...
next_rule_date(
    ...
    case
        Distance =:= Offset andalso
            kz_date:find_next_weekday({Y1, Month, D1}, Weekday)
    of
...
next_rule_date(
    ...
    case
        Distance =:= Offset andalso
            find_last_weekday({Y1, Month, 1}, Weekday)
    of
        %% During an 'active' year before the target month the calculated
        %%   occurrence is accurate
...
next_rule_date(
    ...
    case
        Distance =:= Offset andalso
            find_ordinal_weekday(Y1, Month, Weekday, Ordinal)
    of
        %% During an 'active' year before the target month the calculated
        %%   occurrence is accurate
        {Y1, Month, _} = Date when M1 < Month ->
...
-spec find_next_yearly_ordinal_weekday(
    kz_time:year(), kz_time:month(), wday(), kz_time:ordinal(), interval()
) -> kz_time:date().
...
%% @end
%%------------------------------------------------------------------------------
-spec find_ordinal_weekday(kz_time:year(), improper_month(), wday(), kz_time:ordinal()) ->
    kz_time:date().
...
date_of_dow(Year, Month, Weekday, Ordinal) ->
    ...
    Days =
        case calendar:day_of_the_week(RefDate) of
            DOW ->
...
iso_week_difference({Y0, M0, D0}, {Y1, M1, D1}) ->
    DS0 = calendar:date_to_gregorian_days(
        kz_date:from_iso_week(calendar:iso_week_number({Y0, M0, D0}))
    ),
    DS1 = calendar:date_to_gregorian_days(
        kz_date:from_iso_week(calendar:iso_week_number({Y1, M1, D1}))
    ),
    trunc(abs(DS0 - DS1) / 7).
...
sort_wdays([]) ->
    [kz_date:dow_to_wday(D) || D <- lists:seq(1, 7)];
sort_wdays(WDays0) ->
