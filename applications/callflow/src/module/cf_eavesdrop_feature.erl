...
handle(Data, Call) ->
    ...
    case
        cf_util:check_value_of_fields(Table, 'false', Data, Call) andalso
            maybe_correct_target(Target, kz_json:get_ne_binary_value(<<"group_id">>, Data), Call)
    of
        'true' ->
            Flow = kz_json:from_list([
                {<<"data">>, build_data(Target, Call)},
                {<<"module">>, <<"eavesdrop">>},
...
-spec build_flow_data(kapps_call:call(), kz_term:proplist(), kz_term:api_binary()) ->
    kz_json:object().
build_flow_data(Call, Data, AuthorizingType) when
    AuthorizingType =:= <<"device">>;
    AuthorizingType =:= <<"mobile">>
->
    kz_json:from_list([
        {<<"approved_device_id">>, kapps_call:authorizing_id(Call)}
        | Data
    ]);
build_flow_data(Call, Data, <<"user">>) ->
    kz_json:from_list([
        {<<"approved_user_id">>, kapps_call:authorizing_id(Call)}
        | Data
    ]);
...
find_group_members(GroupId, Call) ->
    case kz_datamgr:open_cache_doc(kapps_call:account_db(Call), GroupId) of
        {'error', _E} -> [];
        {'ok', GroupJObj} -> kz_json:get_keys(<<"endpoints">>, GroupJObj)
    end.

-spec lookup_endpoint(kz_term:api_object()) -> target().
lookup_endpoint('undefined') -> 'error';
lookup_endpoint(Flow) -> lookup_endpoint(Flow, kz_json:get_ne_binary_value(<<"module">>, Flow)).
