...
maybe_build_call_waiting_record(Data, Call) ->
    ...
    case maybe_get_doc(AccountDb, DocId) of
        ...
        {'ok', JObj} ->
            ...
            {'ok', #call_waiting{
                enabled = kz_json:is_true([<<"call_waiting">>, <<"enabled">>], JObj, 'true'),
                jobj = JObj,
...
maybe_execute_action(#call_waiting{action = Action} = CW, Call) ->
    case props:get_value(Action, actions()) of
        ...
        ActionFun ->
            execute_action(ActionFun, CW, Call)
    end.
...
maybe_update_doc(Enabled, #call_waiting{enabled = Enabled}, _Retries) ->
    'ok';
maybe_update_doc(
    Enabled,
    #call_waiting{
    ...
    case kz_datamgr:save_doc(AccountDb, Updated) of
        {'ok', _} ->
            lager:info(
                "call_waiting.enabled set to ~s on ~s ~s",
        ...
        {'error', _R} ->
            lager:debug(
                "unable to update call_waiting.enabled on ~s ~s: ~p",
...
retry_update_doc(_Enabled, #call_waiting{jobj = JObj}, Retries) when Retries >= 3 ->
    lager:info(
        "to many attempts to update call_waiting for ~s ~s: ~p",
...
retry_update_doc(
    ...
    case kz_datamgr:open_doc(AccountDb, kz_doc:id(JObj)) of
        ...
        {'error', _R} ->
            lager:info(
                "unable to retry call_waiting update for ~s ~s: ~p",
                [kz_doc:type(JObj), kz_doc:id(JObj), _R]
            ),
            'error'
    end.
