...
-define(REJECT_PROMPT,
    kapps_config:get_ne_binary(
        ?MOD_CONFIG_CAT, <<"reject_prompt">>, <<"dynamic-cid-invalid_using_default">>
    )
).
...
-record(prompts, {
    accept_tone =
        kapps_config:get_ne_binary(
            ?MOD_CONFIG_CAT, <<"accept_prompt">>, <<"tone_stream://%(250,50,440)">>
        ) ::
        kz_term:ne_binary(),
    ...
    default_prompt =
        kz_media_util:get_prompt(
            kapps_config:get_ne_binary(
                ?MOD_CONFIG_CAT, <<"default_prompt">>, <<"dynamic-cid-enter_cid">>
            )
        ) ::
        kz_term:ne_binary()
}).
...
-spec handle(kz_json:object(), kapps_call:call(), kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
    'ok'.
...
handle(Data, Call, _Manual, CaptureGroup) ->
    lager:info(
        "capture group is not present, forcing manual action. user must manually enter on keypad the caller id for this call"
    ),
    handle_manual(Data, Call, CaptureGroup).
...
%% @end
%%------------------------------------------------------------------------------
-spec update_call_and_continue(
    kz_json:object(),
    kapps_call:call(),
    kz_term:ne_binary(),
    kz_term:ne_binary() | 'no_name',
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_route_to_callflow(
    kz_json:object(), kapps_call:call(), kz_term:api_ne_binary(), kz_term:ne_binary()
) -> 'ok'.
...
-spec maybe_restrict_call(
    kz_json:object(), kapps_call:call(), kz_term:ne_binary(), kzd_callflow:doc()
) -> 'ok'.
maybe_restrict_call(Data, Call, Number, Flow) ->
...
collect_cid_number(Data, Call) ->
    ...
    Media =
        case kz_json:get_ne_binary_value(<<"media_id">>, Data) of
...
get_caller_id_from_entries(Call, ListId, KeyDest) ->
    case
        kz_datamgr:get_results(kapps_call:account_db(Call), <<"lists/entries">>, [{'key', ListId}])
    of
        {'ok', Entries} ->
...
-spec captured_key_and_destination(non_neg_integer(), kz_term:ne_binary()) ->
    key_dest()
    | {'error', 'not_found'}.
captured_key_and_destination(LengthDigits, CaptureGroup) when
    byte_size(CaptureGroup) >= LengthDigits
->
    <<CIDKey:LengthDigits/binary, Destination/binary>> = CaptureGroup,
    {CIDKey, Destination};
captured_key_and_destination(_LengthDigits, _CaptureGroup) ->
    lager:warning(
        "failed to get cid_key (with length ~b) from capture group '~s'",
        [_LengthDigits, _CaptureGroup]
    ),
    {'error', 'not_found'}.

-spec get_cid_length_from_list_document(kapps_call:call(), kz_term:ne_binary()) ->
    non_neg_integer().
get_cid_length_from_list_document(Call, ListId) ->
    case kz_datamgr:open_cache_doc(kapps_call:account_db(Call), ListId) of
        ...
        {'error', _Reason} ->
            lager:info("failed to load list document ~s; using default length '2': ~p", [
                ListId, _Reason
            ]),
            2
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_set_default_cid(kz_term:api_ne_binary(), kz_term:api_ne_binary(), kapps_call:call()) ->
    cid().
maybe_set_default_cid('undefined', 'undefined', Call) ->
