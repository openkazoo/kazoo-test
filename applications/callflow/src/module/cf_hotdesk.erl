...
-define(HOTDESK_ID_LENGTH,
    kapps_config:get_integer(?HD_CONFIG_CAT, <<"max_hotdesk_id_length">>, 10)
).
...
hotdesk_id(Data, <<"logout">>, Call) ->
    ...
    case
        UserId =:= 'undefined' andalso
            kz_attributes:owner_ids(kapps_call:authorizing_id(Call), Call)
    of
        'false' -> UserId;
        [Id] -> Id;
        [] -> UserId;
...
bridge_to_endpoints(#hotdesk{endpoint_ids = EndpointIds}, Call) ->
    ...
    kapps_call_command:b_bridge(
        Endpoints, ?DEFAULT_TIMEOUT_S, <<"simultaneous">>, IgnoreEarlyMedia, Call
    ).
...
-spec maybe_keep_logged_in_elsewhere(hotdesk(), kapps_call:call()) ->
    kapps_api_std_return().
maybe_keep_logged_in_elsewhere(
    #hotdesk{keep_logged_in_elsewhere = 'true'} = Hotdesk,
    Call
) ->
...
find_hotdesk_profile(Call, Data, Max, Loop) ->
    ...
    Interdigit = kz_json:get_integer_value(
        <<"interdigit_timeout">>, Data, kapps_call_command:default_interdigit_timeout()
    ),
    ...
    of
        ...
        {'ok', HotdeskId} ->
            case lookup_hotdesk_id(HotdeskId, Data, Call) of
                ...
                {'error', _} -> find_hotdesk_profile(Call, Data, Max, Loop + 1)
            end;
        {'error', R} = E ->
...
from_json(JObj, Data, Call) ->
    ...
    #hotdesk{
        ...
        owner_id = OwnerId,
        interdigit_timeout = kz_json:find(
            <<"interdigit_timeout">>, [JObj, Data], kapps_call_command:default_interdigit_timeout()
        )
    }.
...
remove_from_endpoints(#hotdesk{endpoint_ids = []} = Hotdesk, _) ->
    Hotdesk;
remove_from_endpoints(
    #hotdesk{
        endpoint_ids = [EndpointId | Endpoints],
        owner_id = OwnerId
    } = Hotdesk,
    Call
) ->
...
-spec update_hotdesk_endpoint(
    kz_term:ne_binary(), kz_term:api_binary() | kz_json:object(), function()
) ->
    kz_term:jobj_return().
