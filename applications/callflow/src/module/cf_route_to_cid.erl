...
maybe_filter_results(Data, JObjs) ->
    ...
    [
     ...
     || JObj <- JObjs,
        ...
        'undefined' =/= EndpointId andalso
            ([] =:= EndpointTypes orelse
                lists:member(EndpointType, EndpointTypes))
    ].

-spec endpoint_format(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
endpoint_format(EndpointId, EndpointType, Data) ->
    Values = [
