...
update_presence(<<"idle">>, PresenceId, Call) ->
    ...
    kapps_call_command:presence(
        <<"terminated">>, PresenceId, kz_term:to_hex_binary(crypto:hash('md5', PresenceId))
    );
update_presence(<<"ringing">>, PresenceId, Call) ->
    _ = update_doc(Call, PresenceId, <<"early">>),
    kapps_call_command:presence(
        <<"early">>, PresenceId, kz_term:to_hex_binary(crypto:hash('md5', PresenceId))
    );
update_presence(<<"busy">>, PresenceId, Call) ->
    _ = update_doc(Call, PresenceId, <<"confirmed">>),
    kapps_call_command:presence(
        <<"confirmed">>, PresenceId, kz_term:to_hex_binary(crypto:hash('md5', PresenceId))
    ).
