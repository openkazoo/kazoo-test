...
attempt_endpoints(JObj, Data, Call) ->
    ...
    case
        kapps_call_command:b_bridge(
            Endpoints, Timeout, Strategy, <<"true">>, Ringback, 'undefined', IgnoreForward, Call
        )
    of
        {'ok', _} ->
...
-spec build_device_endpoints(endpoints_acc(), kz_term:proplist(), kapps_call:call()) ->
    endpoints_acc().
build_device_endpoints(Endpoints, [], _) ->
    Endpoints;
build_device_endpoints(Endpoints, [{MemberId, Member} | Members], Call) ->
    case
        kz_json:get_value(<<"type">>, Member, <<"device">>) =:= <<"device">> andalso
            props:get_value(MemberId, Endpoints) =:= 'undefined' andalso
            MemberId =/= kapps_call:authorizing_id(Call)
    of
        'true' ->
        ...
        'false' ->
            build_device_endpoints(Endpoints, Members, Call)
    end.

-spec build_user_endpoints(endpoints_acc(), kz_term:proplist(), kapps_call:call()) ->
    endpoints_acc().
build_user_endpoints(Endpoints, [], _) ->
    Endpoints;
build_user_endpoints(Endpoints, [{MemberId, Member} | Members], Call) ->
    case <<"user">> =:= kz_json:get_value(<<"type">>, Member, <<"user">>) of
        'false' ->
            build_user_endpoints(Endpoints, Members, Call);
        'true' ->
            DeviceIds = kz_attributes:owned_by(MemberId, <<"device">>, Call),
            M = kz_json:set_values(
                [
