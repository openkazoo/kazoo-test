        ...
        CF ->
            ...
            CF1 =
                case kz_json:get_ne_binary_value(<<"action">>, Data) of
                    %% Support for NANPA *72
                    <<"activate">> ->
                        cf_activate(CF, CaptureGroup, Call);
                    %% Support for NANPA *73
                    <<"deactivate">> ->
                        cf_deactivate(CF, Call);
                    %% Support for NANPA *56
                    <<"update">> ->
                        cf_update_number(CF, CaptureGroup, Call);
                    <<"toggle">> ->
                        cf_toggle(CF, CaptureGroup, Call);
                    <<"menu">> ->
...
cf_update_number(#callfwd{interdigit_timeout = Interdigit} = CF, CaptureGroup, Call) when
    is_atom(CaptureGroup); CaptureGroup =:= <<>>
->
...
-spec update_callfwd(callfwd(), kapps_call:call()) ->
    ...
    | {'error', atom()}.
update_callfwd('undefined', _Call) ->
    {'ok', 'undefined'};
update_callfwd(
    #callfwd{
        doc_id = Id,
    ...
    Updates = [
        fun(J) -> kz_json:set_value(<<"enabled">>, Enabled, J) end,
...
maybe_get_call_forward(Call, OwnerId) ->
    case kz_datamgr:open_cache_doc(kapps_call:account_db(Call), OwnerId) of
        {'ok', UserJObj} ->
            ...
            #callfwd{
                doc_id = kz_doc:id(UserJObj),
                enabled = kz_json:is_true([<<"call_forward">>, <<"enabled">>], UserJObj),
                number = kz_json:get_ne_value([<<"call_forward">>, <<"number">>], UserJObj, <<>>),
                require_keypress = kz_json:is_true(
                    [<<"call_forward">>, <<"require_keypress">>], UserJObj, 'true'
                ),
                keep_caller_id = kz_json:is_true(
                    [<<"call_forward">>, <<"keep_caller_id">>], UserJObj, 'true'
                )
            };
        {'error', R} ->
