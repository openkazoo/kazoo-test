...
-spec find_and_change_ring_group(kz_json:object(), kapps_call:call(), integer()) ->
    kz_json:object().
find_and_change_ring_group(Action, Call, DisableUntil) ->
    NewAction =
        case kz_json:get_binary_value(<<"module">>, Action) of
    ...
    case kz_json:get_value(<<"children">>, NewAction) of
        ...
        Children ->
            kz_json:set_value(
                <<"children">>, maybe_modify_children(Children, Call, DisableUntil), NewAction
            )
    end.
...
alter_endpoints(Module, Call, DisableUntil) ->
    ...
    {Found, EndPoints} = lists:foldl(
        fun(Endpoint, {Found, UpdatedEndpoints}) ->
            {ThisFound, Res} = maybe_alter_disable_until(
                kz_json:get_value(<<"endpoint_type">>, Endpoint), Endpoint, Call, DisableUntil
            ),
...
-spec maybe_alter_disable_until(
    kz_term:ne_binary(), kz_json:object(), kapps_call:call(), integer()
