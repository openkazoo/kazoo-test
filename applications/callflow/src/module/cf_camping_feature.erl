...
get_channels(#state{type = TargetType, id = TargetId} = S, Call) ->
    ...
    Usernames =
        case TargetType of
            <<"device">> ->
                cf_util:sip_users_from_device_ids([TargetId], Call);
            <<"user">> ->
...
get_sip_usernames_for_target(TargetId, TargetType, Call) ->
    Targets =
        case TargetType of
            <<"user">> ->
                kz_attributes:owned_by(TargetId, <<"device">>, Call);
            <<"device">> ->
                [TargetId];
            _Else ->
...
-spec no_channels(state(), kapps_call:call()) ->
    maybe_m('accepted')
    | maybe_m('connected').
no_channels(
    #state{
...
no_channels(
    ...
    JObj = kz_json:from_list([
        {<<"Delegate-Message">>, kz_json:from_list(MsgProps)}
