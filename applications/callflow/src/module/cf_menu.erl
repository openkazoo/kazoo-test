...
menu_handle_digits(
    ...
    AllowRecord =
        (RecOffnet orelse
            kapps_call:inception(Call) =:= 'undefined'),

    case try_match_digits(Digits, Menu, Call) of
        'true' ->
            lager:debug("hunt callflow found");
...
menu_handle_no_digits(#cf_menu_data{retries = Retries} = Menu, Call) ->
    ...
    case try_match_digits(<<"timeout">>, Menu, Call) of
        'true' ->
            lager:debug("timeout hunt callflow found");
        'false' ->
...
try_match_digits(Digits, Menu, Call) ->
    ...
    is_callflow_child(Digits, Menu, Call) orelse
        (is_hunt_enabled(Digits, Menu, Call) andalso
            is_hunt_allowed(Digits, Menu, Call) andalso
            not is_hunt_denied(Digits, Menu, Call) andalso
...
record_greeting(AttachmentName, #cf_menu_data{greeting_id = MediaId} = Menu, Call) ->
    ...
    case kapps_call_command:b_record(AttachmentName, Call) of
        ...
        {'ok', JObj} ->
            NoRec =
                kapps_config:get_integer(?MOD_CONFIG_CAT, <<"min_greeting_length">>, 1500) >
...
store_recording(AttachmentName, MediaId, Call) ->
    ...
    kapps_call_command:b_store(
        AttachmentName, get_new_attachment_url(AttachmentName, MediaId, Call), Call
    ).
            ...
            #cf_menu_data{
                ...
                max_length =
                    kz_json:get_integer_value(
                        <<"max_extension_length">>, JObj, Default#cf_menu_data.max_length
                    ),
                hunt =
                ...
                record_from_offnet =
                    kz_json:is_true(
                        ...
                        JObj,
                        Default#cf_menu_data.record_from_offnet
                    ),
                greeting_id =
                    kz_json:get_ne_value([<<"media">>, <<"greeting">>], JObj),
                exit_media =
                    (not kz_json:is_false([<<"media">>, <<"exit_media">>], JObj)) andalso
                        kz_json:get_ne_value([<<"media">>, <<"exit_media">>], JObj, 'true'),
                transfer_media =
                    (not kz_json:is_false([<<"media">>, <<"transfer_media">>], JObj)) andalso
                        kz_json:get_ne_value([<<"media">>, <<"transfer_media">>], JObj, 'true'),
                invalid_media =
                    (not kz_json:is_false([<<"media">>, <<"invalid_media">>], JObj)) andalso
                        kz_json:get_ne_value([<<"media">>, <<"invalid_media">>], JObj, 'true'),
                interdigit_timeout =
                    kz_term:to_integer(
                        kz_json:find(
                            <<"interdigit_timeout">>,
