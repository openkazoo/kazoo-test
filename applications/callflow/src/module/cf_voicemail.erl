...
-define(MAILBOX_SEEK_DURATION,
    kapps_config:get_non_neg_integer(
        ?CF_CONFIG_CAT,
...
-record(mailbox, {
    mailbox_id :: kz_term:api_ne_binary(),
...
-spec check_mailbox(mailbox(), boolean(), kapps_call:call(), non_neg_integer()) ->
    'ok' | {'error', 'channel_hungup'}.
check_mailbox(#mailbox{max_login_attempts = MaxLoginAttempts}, _, Call, Loop) when
    Loop > MaxLoginAttempts
->
...
find_mailbox(#mailbox{max_login_attempts = MaxLoginAttempts}, _Call, _VmEntryIdMedia, Loop) when
    Loop > MaxLoginAttempts
->
    %% if we have exceeded the maximum loop attempts then terminate this call
...
find_mailbox(#mailbox{interdigit_timeout = Interdigit} = Box, Call, VmEntryIdMedia, Loop) ->
    ...
    case
        kapps_call_command:collect_digits(
            ?MAILBOX_DEFAULT_BOX_NUMBER_LENGTH,
            kapps_call_command:default_collect_timeout(),
            Interdigit,
            NoopId,
            Call
...
-spec find_destination_mailbox(
    mailbox(), kapps_call:call(), kz_term:ne_binary(), non_neg_integer()
) -> mailbox().
find_destination_mailbox(
    #mailbox{max_login_attempts = MaxLoginAttempts}, Call, _SrcBoxId, Loop
) when
    Loop > MaxLoginAttempts
->
    ...
    #mailbox{};
find_destination_mailbox(
    #mailbox{max_login_attempts = MaxLoginAttempts} = Box, Call, SrcBoxId, Loop
) ->
...
play_greeting(#mailbox{skip_greeting = 'true'}, _Call) ->
    'ok';
play_greeting(
    #mailbox{temporary_unavailable_media_id = <<_/binary>> = MediaId},
    Call
) ->
    Corrected = kz_media_util:media_path(MediaId, kapps_call:account_id(Call)),
    lager:info(
        "mailbox has a temporary greeting which always overrides standard greeting: '~s', corrected to '~s'",
        [MediaId, Corrected]
    ),
...
record_voicemail(AttachmentName, #mailbox{max_message_length = MaxMessageLength} = Box, Call) ->
    ...
    case
        kapps_call_command:b_record(
            AttachmentName, ?ANY_DIGIT, kz_term:to_binary(MaxMessageLength), Call
        )
    of
        {'ok', Msg} ->
...
message_prompt([H | _] = Messages, Message, Count, #mailbox{
    timezone = Timezone,
    skip_envelope = 'false',
    keys = Keys,
    ...
    [
        ...
        {'prompt', <<"vm-received">>},
        {'say', get_unix_epoch(kz_json:get_integer_value(<<"timestamp">>, H), Timezone),
            <<"current_date_time">>},
        {'prompt', <<"vm-message_menu">>}
    ];
message_prompt(Messages, Message, Count, #mailbox{
    is_ff_rw_enabled = AllowFfRw,
    keys = Keys,
    skip_envelope = 'true'
}) ->
...
-spec play_messages(
    kz_json:objects(), kz_json:objects(), non_neg_integer(), mailbox(), kapps_call:call()
) ->
    'ok' | 'complete'.
play_messages(
    ...
    case message_menu(Prompt, Box, Call) of
        ...
        {'ok', 'delete'} ->
            ...
            JObj = hd(
                kz_json:get_list_value(
                    <<"succeeded">>, kvm_messages:fetch(AccountId, [MessageId], BoxId)
                )
            ),
...
-spec maybe_restrict_call(kz_term:ne_binary(), kapps_call:call(), kz_json:object()) ->
    'ok' | 'error'.
maybe_restrict_call(CallerIdNumber, Call, Endpoint) ->
    ...
    case cf_flow:lookup(CallerIdNumber, AccountId) of
        ...
        _ ->
            ...
            _ = kapps_call_command:audio_macro(
                [{'prompt', <<"fault-can_not_be_completed_as_dialed">>}], Call
            ),
            'error'
    end.
...
should_restrict_call(CallerIdNumber, Call, Endpoint) ->
    ...
    lager:debug(
        "classified number ~s as ~s, testing for call restrictions",
        [CallerIdNumber, Classification]
    ),
    Restriction = kz_json:get_ne_binary_value(
        [<<"call_restriction">>, Classification, <<"action">>], Endpoint
    ),

    {<<"deny">> =:= Restriction, NormalizedNumber}.

-spec play_next_message(
    kz_json:objects(), kz_json:objects(), non_neg_integer(), mailbox(), kapps_call:call()
) ->
    'ok' | 'complete'.
...
forward_message(
    ...
    case
        ForwardType =:= <<"prepend_forward">> andalso
            forward_message_menu(DestBox, Call)
    of
        'false' ->
...
-spec compose_forward_message(kz_json:object(), kz_term:ne_binary(), mailbox(), kapps_call:call()) ->
    'ok'.
...
-spec record_forward(
    kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary(), mailbox(), kapps_call:call()
) -> 'ok'.
record_forward(
    AttachmentName,
    Message,
    SrcBoxId,
    #mailbox{max_message_length = MaxMessageLength} = DestBox,
    Call
) ->
    Tone = kz_json:from_list([
        {<<"Frequencies">>, [<<"440">>]},
...
maybe_forward(
    ...
    Message,
    SourceBoxId,
    #mailbox{media_extension = Ext} = DestinationBox,
    Call,
    Msg,
    {'ok', _Status}
) ->
...
-spec config_menu(mailbox(), kapps_call:call(), pos_integer()) ->
    ...
    | {'error', 'channel_hungup'}.
config_menu(
    #mailbox{interdigit_timeout = Interdigit} = Box,
    Call,
    Loop
...
handle_config_selection(
    #mailbox{
        keys = #keys{rec_unavailable = Selection},
        media_extension = Ext
    } = Box,
    Call,
    _Loop,
...
handle_config_selection(
    #mailbox{keys = #keys{set_pin = Selection}} = Box,
    Call,
    _Loop,
    Selection
) ->
...
handle_config_selection(
    #mailbox{
        keys = #keys{rec_temporary_unavailable = Selection},
        media_extension = Ext
    } = Box,
    Call,
    _Loop,
...
handle_config_selection(
    #mailbox{keys = #keys{del_temporary_unavailable = Selection}} = Box,
    Call,
    _Loop,
    Selection
) ->
...
handle_config_selection(
    #mailbox{keys = #keys{return_main = Selection}} = Box,
    Call,
    _Loop,
    Selection
) ->
...
handle_config_selection(
    #mailbox{keys = #keys{toggle_announcement_mode = Selection}} = Box,
    Call,
    _Loop,
    Selection
) ->
...
record_temporary_unavailable_greeting(
    ...
    lager:info(
        "no temporary greetings was recorded before so new media document should be created"
    ),
    MediaId = recording_media_doc(<<"temporary unavailable greeting">>, Box, Call),
    overwrite_temporary_unavailable_greeting(
        AttachmentName,
...
%% @end
%%------------------------------------------------------------------------------
-spec overwrite_temporary_unavailable_greeting(
    kz_term:ne_binary(), mailbox(), kapps_call:call(), 'new' | 'update'
) ->
    'ok' | mailbox().
overwrite_temporary_unavailable_greeting(
    AttachmentName,
    #mailbox{
        temporary_unavailable_media_id = MediaId,
        media_extension = Ext
    } = Box,
    Call,
    UpdateOrNew
        ...
        {'ok', Msg} ->
            case review_recording(AttachmentName, 'false', Box, Call) of
                ...
                {'ok', 'save'} ->
                    ...
                    'ok' = update_doc(
                        [<<"media">>, <<"temporary_unavailable">>], MediaId, Box, Call
                    ),
                    maybe_update_recording_media_doc(
                        AttachmentName, Box, Call, MediaId, UpdateOrNew
                    ),
                    _ = kapps_call_command:b_prompt(<<"vm-saved">>, Call),
                    Box;
                {'ok', 'no_selection'} ->
...
delete_temporary_unavailable_greeting(
    #mailbox{temporary_unavailable_media_id = 'undefined'} = Box, _Call
) ->
    Box;
delete_temporary_unavailable_greeting(Box, Call) ->
...
-spec record_unavailable_greeting(kz_term:ne_binary(), mailbox(), kapps_call:call()) ->
    'ok' | mailbox().
record_unavailable_greeting(
    AttachmentName, #mailbox{unavailable_media_id = 'undefined'} = Box, Call
) ->
    MediaId = recording_media_doc(<<"unavailable greeting">>, Box, Call),
    overwrite_unavailable_greeting(
        AttachmentName, Box#mailbox{unavailable_media_id = MediaId}, Call, MediaId, 'new'
    );
...
check_media_source(AttachmentName, Box, Call, JObj) ->
    case kz_json:get_ne_binary_value(<<"media_source">>, JObj) of
        <<"upload">> ->
            lager:debug(
                "The voicemail greeting media is a web upload, let's not touch it," ++
                    " it may be in use in some other mailbox. We create new media document."
            ),
            record_unavailable_greeting(
                AttachmentName, Box#mailbox{unavailable_media_id = 'undefined'}, Call
            );
        _ ->
            overwrite_unavailable_greeting(AttachmentName, Box, Call, JObj, 'update')
    end.

-spec overwrite_unavailable_greeting(
    kz_term:ne_binary(),
    mailbox(),
    kapps_call:call(),
        ...
        {'ok', Msg} ->
            case review_recording(AttachmentName, 'false', Box, Call) of
                ...
                {'ok', 'save'} ->
                    ...
                    maybe_update_recording_media_doc(
                        AttachmentName, Box, Call, JObjOrID, UpdateOrNew
                    ),
                    _ = kapps_call_command:b_prompt(<<"vm-saved">>, Call),
                    Box;
                {'ok', 'no_selection'} ->
...
record_name(
    ...
    lager:info("created recorded name media doc: ~s , saving recorded name id into mailbox", [
        MediaId
    ]),
...
-spec record_name(
    kz_term:ne_binary(), mailbox(), kapps_call:call(), kz_term:ne_binary(), 'new' | 'update'
) ->
    'ok' | mailbox().
record_name(
    AttachmentName,
    #mailbox{
        name_media_id = MediaId,
        media_extension = Ext
    } = Box,
    Call,
    DocId,
...
new_message(_, Length, #mailbox{min_message_length = MinLength}, _) when
    Length < MinLength
->
...
new_message(
    ...
    case kvm_message:new(Call, NewMsgProps) of
        {'ok', _NewCall} ->
            send_mwi_update(Box);
        {'error', _, _Msg} ->
            lager:warning("failed to save voice mail message recorded media : ~p", [_Msg])
    end.
...
get_mailbox_profile(Data, Call) ->
    ...
    case get_mailbox_doc(AccountDb, Id, Data, Call) of
        {'ok', MailboxJObj} ->
            ...
            lager:info(
                "mailbox limited to ~p voicemail messages (has ~b currently)",
            ...
            #mailbox{
                ...
                announcement_only =
                    kzd_voicemail_box:announcement_only(
                        MailboxJObj, Default#mailbox.announcement_only
                    ),
                skip_instructions =
                    kzd_voicemail_box:skip_instructions(
                        MailboxJObj, Default#mailbox.skip_instructions
                    ),
                skip_greeting =
                ...
                play_greeting_intro =
                    kz_json:is_true(
                        <<"play_greeting_intro">>, MailboxJObj, Default#mailbox.play_greeting_intro
                    ),
                use_person_not_available =
                    kz_json:is_true(
                        <<"use_person_not_available">>,
                        MailboxJObj,
                        Default#mailbox.use_person_not_available
                    ),
                not_configurable =
...
populate_keys(Call) ->
    ...
    #keys{
        operator = kz_json:get_binary_value(<<"operator">>, JObj, Default#keys.operator),
        ...
        rec_unavailable = kz_json:get_binary_value(
            <<"record_unavailable">>, JObj, Default#keys.rec_unavailable
        ),
        ...
        return_main = kz_json:get_binary_value(
            <<"return_main_menu">>, JObj, Default#keys.return_main
        ),
...
%% @end
%%------------------------------------------------------------------------------
-spec get_mailbox_doc(
    kz_term:ne_binary(), kz_term:api_binary(), kz_json:object(), kapps_call:call()
) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
...
get_user_mailbox_doc(Data, Call, OwnerId) ->
    ...
    case kz_attributes:owned_by_docs(OwnerId, <<"vmbox">>, Call) of
        ...
        [Box] when SingleMailboxLogin ->
            lager:debug(
                "owner ~s has one vmbox ~s, and single mailbox login is enabled",
                [OwnerId, kz_doc:id(Box)]
            ),
            {'ok', Box};
        Boxes ->
            lager:debug(
                "found ~p vmboxes assigned to owner ~s",
...
-spec store_recording(
    kz_term:ne_binary(), non_neg_integer(), kz_term:ne_binary(), mailbox(), kapps_call:call()
) -> 'ok' | {'error', kapps_call:call()}.
store_recording(_, Length, _, #mailbox{min_message_length = MinLength}, _) when
    Length < MinLength
->
    lager:info("attachment length is ~B and must be larger than ~B to be stored", [
        Length, MinLength
...
-spec get_new_attachment_url(
    kz_term:ne_binary(), kz_term:ne_binary(), mailbox(), kapps_call:call()
) ->
    kz_term:ne_binary().
...
recording_media_doc(Recording, Box, Call) ->
    AccountDb = kapps_call:account_db(Call),
    {'ok', JObj} = kz_datamgr:save_doc(
        AccountDb, set_recording_media_doc(Recording, Box, AccountDb, kz_json:new())
    ),
    kz_doc:id(JObj).

-spec maybe_update_recording_media_doc(
    kz_term:ne_binary(),
    mailbox(),
    kapps_call:call(),
    kz_json:object() | kz_term:ne_binary(),
...
-spec update_recording_media_doc(
    kz_term:ne_binary(), mailbox(), kapps_call:call(), kz_term:ne_binary()
) -> 'ok'.
update_recording_media_doc(AttachmentName, Box, Call, MediaId) ->
    AccountDb = kapps_call:account_db(Call),
    case kz_datamgr:open_doc(AccountDb, MediaId) of
        {'ok', JObj} ->
            case
                kz_datamgr:save_doc(
                    AccountDb, set_recording_media_doc(AttachmentName, Box, AccountDb, JObj)
                )
            of
                {'ok', _} ->
                    'ok';
                {'error', _Reason} ->
                    lager:info("unable to update media_source for media doc ~s in ~s: ~p", [
                        MediaId, AccountDb, _Reason
                    ])
            end;
        {'error', _Reason} ->
            lager:info("unable to update media_source for media doc ~s in ~s: ~p", [
...
-spec set_recording_media_doc(
    kz_term:ne_binary(), mailbox(), kz_term:ne_binary(), kz_json:object()
) -> kz_json:object().
set_recording_media_doc(
    Recording,
    #mailbox{
        mailbox_number = BoxNum,
        mailbox_id = Id,
...
get_unix_epoch(Epoch, Timezone) ->
    ...
    kz_term:to_binary(
        calendar:datetime_to_gregorian_seconds(LocalDateTime) - ?UNIX_EPOCH_IN_GREGORIAN
    ).
...
find_max_message_length([]) ->
    ?MAILBOX_DEFAULT_MSG_MAX_LENGTH;
