...
%% what fields to convert/keep for searching
-define(FIELDS, [<<"last_name">>, <<"first_name">>]).
...
%% Please enter the first few letters of the person's lastname
-define(PROMPT_ENTER_PERSON_LASTNAME, <<"dir-enter_person_lastname">>).
%% Please enter the first few letters of the person's firstname
-define(PROMPT_ENTER_PERSON_FIRSTNAME, <<"dir-enter_person_firstname">>).
%% Please enter the first few letters of the person's name
-define(PROMPT_ENTER_PERSON_NAME, <<"dir-enter_person_name">>).
%% first name
-define(PROMPT_FIRSTNAME, <<"dir-first_name">>).
%% last name
-define(PROMPT_LASTNAME, <<"dir-last_name">>).
%% You need to specify a minimum of two digits
-define(PROMPT_SPECIFY_MINIMUM, <<"dir-specify_minimum">>).
%% letters of the person's name
-define(PROMPT_LETTERS_OF_NAME, <<"dir-letters_of_person_name">>).
%% No match found
-define(PROMPT_NO_RESULTS_FOUND, <<"dir-no_results_found">>).
%% no more results
-define(PROMPT_NO_MORE_RESULTS, <<"dir-no_more_results">>).
%% press 1. to start over press 3
-define(PROMPT_CONFIRM_MENU, <<"dir-confirm_menu">>).
%% One match found. To connect to
-define(PROMPT_FOUND, <<"dir-found">>).
%% invalid key pressed
-define(PROMPT_INVALID_KEY, <<"dir-invalid_key">>).
%% To call
-define(PROMPT_RESULT_NUMBER, <<"dir-result_number">>).
%% press one. For the next result press two. To start over press three
-define(PROMPT_RESULT_MENU, <<"dir-result_menu">>).
...
handle(Data, Call) ->
    {'ok', DirJObj} = kz_datamgr:open_cache_doc(
        ...
        {'ok', Users} ->
            State = #directory{
                ...
                search_fields = get_search_fields(
                    kz_json:get_value(<<"search_fields">>, DirJObj, <<"both">>)
                ),
...
directory_start(Call, State, CurrUsers, Loop) ->
    ...
    case play_directory_instructions(Call, search_fields(State)) of
        {'ok', <<>>} ->
            directory_start(Call, State, CurrUsers, Loop - 1);
        {'ok', DTMF} ->
...
-spec maybe_match_user(kapps_call:call(), directory_user(), pos_integer(), non_neg_integer()) ->
    dtmf_action().
...
route_to_match(Call, Callflow) -> cf_exe:branch(kz_json:get_value(<<"flow">>, Callflow), Call).
...
play_user(Call, UsernameTuple, _MatchNum) ->
    play_and_collect(Call, [
        {'prompt', ?PROMPT_RESULT_NUMBER},
        UsernameTuple,
...
-spec username_audio_macro(kapps_call:call(), directory_user()) ->
...
% noop id
-spec play_no_users(kapps_call:call()) -> kz_term:ne_binary().
play_no_users(Call) ->
    kapps_call_command:audio_macro([{'prompt', ?PROMPT_NO_MORE_RESULTS}], Call).

% noop id
-spec play_no_users_found(kapps_call:call()) -> kz_term:ne_binary().
...
-spec play_and_collect(
    kapps_call:call(), kapps_call_command:audio_macro_prompts(), non_neg_integer()
) ->
    kapps_call_command:collect_digits_return().
...
get_directory_listing(Db, DirId) ->
    case
        kz_datamgr:get_results(Db, ?DIR_DOCS_VIEW, [
            {'startkey', [DirId]}, {'endkey', [DirId, kz_json:new()]}, 'include_docs'
        ])
    of
        {'ok', []} ->
        ...
        {'ok', Users} ->
            {'ok', [
                get_directory_user(
                    kz_json:get_json_value(<<"doc">>, U), kz_json:get_value(<<"value">>, U)
                )
             || U <- Users
            ]};
        {'error', _E} = E ->
...
sort_by_first(
    #directory_user{first_name = AFirst, last_name = ALast},
    #directory_user{first_name = AFirst, last_name = BLast}
) ->
    ALast < BLast;
sort_by_first(
    #directory_user{first_name = AFirst},
...
sort_by_last(
    ...
    AFirst < BFirst;
sort_by_last(
    #directory_user{last_name = ALast},
...
-spec maybe_queue_user(
    directory_user(), queue:queue(), kz_term:ne_binary(), pos_integer(), search_field()
) ->
    queue:queue().
...
log(Users) ->
    [
        lager:info("user: ~s: ~s: ~s", [full_name(U), last_first_dtmfs(U), first_last_dtmfs(U)])
     || U <- Users
    ].
