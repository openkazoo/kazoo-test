...
maybe_require_ignore_early_media(Call, Acc) ->
    [
        {<<"Require-Ignore-Early-Media">>,
            kapps_call:custom_channel_var(<<"Require-Ignore-Early-Media">>, Call)}
        | Acc
    ].
...
maybe_require_single_fail(Call, Acc) ->
    [
        {<<"Require-Fail-On-Single-Reject">>,
            kapps_call:custom_channel_var(<<"Require-Fail-On-Single-Reject">>, Call)}
        | Acc
    ].
    ...
    case kapps_call:is_call_forward(Call) of
        'true' ->
            [
                ...
                {<<"Application-Other-Leg-UUID">>,
                    kapps_call:custom_channel_var(<<"Call-Forward-For-UUID">>, Call)}
                | Acc
            ];
        'false' ->
            Acc
...
get_privacy_flags(Call) ->
    ...
    case
        kapps_call:kvs_fetch(<<"use_endpoint_privacy">>, 'true', Call) andalso
            not kz_json:is_true(<<"Retain-CID">>, CCVs)
    of
...
get_endpoint_privacy_flags(Call, CCVs) ->
    case get_endpoint(Call) of
        ...
        {'ok', Endpoint} ->
            [
                {?KEY_PRIVACY_METHOD, kz_privacy:get_method(CCVs)},
                {?KEY_PRIVACY_HIDE_NAME,
                    kz_privacy:should_hide_name(Endpoint) orelse
                        kz_privacy:should_hide_name(CCVs)},
                {?KEY_PRIVACY_HIDE_NUMBER,
                    kz_privacy:should_hide_number(Endpoint) orelse
...
get_bypass_e164(Data) ->
    kz_json:is_true(<<"do_not_normalize">>, Data) orelse
        kz_json:is_true(<<"bypass_e164">>, Data).
...
-spec maybe_set_bridge_generate_comfort_noise(kapps_call:call(), kz_term:proplist()) ->
    kz_term:proplist().
...
-spec maybe_has_comfort_noise_option_enabled(kz_json:object(), kz_term:proplist()) ->
    kz_term:proplist().
maybe_has_comfort_noise_option_enabled(Endpoint, Acc) ->
...
-spec maybe_apply_dialplan(kz_json:object(), kz_json:object(), kapps_call:call()) ->
    kz_term:ne_binary().
...
get_sip_headers(Data, Call) ->
    Routines = [
        fun(J) -> maybe_emit_account_id(J, Data, Call) end,
...
maybe_include_diversions(JObj, Call) ->
    case kapps_call:custom_sip_header(<<"Diversions">>, Call) of
        'undefined' -> JObj;
        Diversions -> kz_json:set_value(<<"Diversions">>, Diversions, JObj)
    end.
...
maybe_emit_account_id(JObj, Data, Call) ->
    ...
    case kz_json:is_true(<<"emit_account_id">>, Data, Default) of
        'false' -> JObj;
        'true' -> kz_json:set_value(<<"X-Account-ID">>, kapps_call:account_id(Call), JObj)
    end.
        ...
        {'ok', JObj} ->
            case kz_util:get_event_type(JObj) of
                {<<"resource">>, <<"offnet_resp">>} ->
                    {kz_call_event:response_message(JObj), kz_call_event:response_code(JObj)};
