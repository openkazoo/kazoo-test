...
token_check(Call, Flow) ->
    case kapps_config:get_is_true(?CF_CONFIG_CAT, <<"calls_consume_tokens">>, 'true') of
        ...
        'true' ->
            ...
            DryRun = kapps_config:get_is_true(
                ?CF_CONFIG_CAT, <<"should_dry_run_token_restrictions">>, 'false'
            ),
            ...
            case kz_buckets:consume_tokens(?APP_NAME, Name, Cost) of
                ...
                'false' when DryRun ->
                    lager:info(
                        "dry-run: bucket ~s does not have enough tokens(~b needed) for this call", [
                            Name, Cost
                        ]
                    ),
                    'true';
                'false' ->
                    lager:warning(
                        "bucket ~s does not have enough tokens(~b needed) for this call", [
                            Name, Cost
                        ]
                    ),
                    'false'
...
-spec maybe_presence_parking_slot_resp(
...
maybe_presence_parking_slot_resp(Username, Realm, AccountDb) ->
    case kz_cache:fetch_local(?CACHE_NAME, ?PARKING_PRESENCE_KEY(AccountDb, Username)) of
        {'ok', 'false'} -> 'not_found';
        {'ok', SlotNumber} -> presence_parking_slot_resp(Username, Realm, AccountDb, SlotNumber);
        {'error', 'not_found'} -> maybe_presence_parking_flow(Username, Realm, AccountDb)
    end.

-spec maybe_presence_parking_flow(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok' | 'not_found'.
            ...
            case kz_json:get_value([<<"flow">>, <<"module">>], Flow) of
                <<"park">> ->
                    ...
                    kz_cache:store_local(
                        ?CACHE_NAME, ?PARKING_PRESENCE_KEY(AccountDb, Username), SlotNumber
                    ),
                    presence_parking_slot_resp(Username, Realm, AccountDb, SlotNumber);
                _Else ->
                    kz_cache:store_local(
                        ?CACHE_NAME, ?PARKING_PRESENCE_KEY(AccountDb, Username), 'false'
                    ),
                    'not_found'
            end
    end.

-spec presence_parking_slot_resp(
...
-spec fetch_manual_presence_doc(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok' | 'not_found'.
fetch_manual_presence_doc(Username, Realm, AccountDb) ->
    case kz_datamgr:open_doc(AccountDb, ?MANUAL_PRESENCE_DOC) of
        ...
        {'error', 'not_found'} ->
            CacheProps = [{'origin', {'db', AccountDb, ?MANUAL_PRESENCE_DOC}}],
            kz_cache:store_local(
                ?CACHE_NAME, ?MANUAL_PRESENCE_KEY(AccountDb), kz_json:new(), CacheProps
            );
        {'error', _} ->
            'not_found'
    end.

-spec manual_presence_resp(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    'ok' | 'not_found'.
manual_presence_resp(Username, Realm, JObj) ->
...
-spec maybe_vm_mwi_resp(
    kz_term:api_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()
) -> 'ok'.
maybe_vm_mwi_resp('undefined', _Realm, _AccountDb, _JObj) ->
    'ok';
...
-spec mwi_resp(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    'ok'.
...
dialpad_digit(ABC) when
    ABC =:= $a orelse
        ABC =:= $b orelse
        ABC =:= $c
->
    $2;
dialpad_digit(DEF) when
    DEF =:= $d orelse
        DEF =:= $e orelse
        DEF =:= $f
->
    $3;
dialpad_digit(GHI) when
    GHI =:= $g orelse
        GHI =:= $h orelse
        GHI =:= $i
->
    $4;
dialpad_digit(JKL) when
    JKL =:= $j orelse
        JKL =:= $k orelse
        JKL =:= $l
->
    $5;
dialpad_digit(MNO) when
    MNO =:= $m orelse
        MNO =:= $n orelse
        MNO =:= $o
->
    $6;
dialpad_digit(PQRS) when
    PQRS =:= $p orelse
        PQRS =:= $q orelse
        PQRS =:= $r orelse
        PQRS =:= $s
->
    $7;
dialpad_digit(TUV) when
    TUV =:= $t orelse
        TUV =:= $u orelse
        TUV =:= $v
->
    $8;
dialpad_digit(WXYZ) when
    WXYZ =:= $w orelse
        WXYZ =:= $x orelse
        WXYZ =:= $y orelse
        WXYZ =:= $z
...
owner_ids_by_sip_username(AccountDb, Username) ->
    case kz_cache:peek_local(?CACHE_NAME, ?SIP_USER_OWNERS_KEY(AccountDb, Username)) of
        {'ok', _} = Ok -> Ok;
        {'error', 'not_found'} -> get_owner_ids_by_sip_username(AccountDb, Username)
    end.
...
get_owner_ids_by_sip_username(AccountDb, Username) ->
    ...
    case kz_datamgr:get_single_result(AccountDb, <<"attributes/sip_username">>, ViewOptions) of
        {'ok', JObj} ->
            ...
            kz_cache:store_local(
                ?CACHE_NAME, ?SIP_USER_OWNERS_KEY(AccountDb, Username), OwnerIds, CacheProps
            ),
            {'ok', OwnerIds};
        {'error', _R} = E ->
...
endpoint_id_by_sip_username(AccountDb, Username) ->
    case kz_cache:peek_local(?CACHE_NAME, ?SIP_ENDPOINT_ID_KEY(AccountDb, Username)) of
        {'ok', _} = Ok -> Ok;
        {'error', 'not_found'} -> get_endpoint_id_by_sip_username(AccountDb, Username)
    end.
...
get_endpoint_id_by_sip_username(AccountDb, Username) ->
    ...
    case kz_datamgr:get_single_result(AccountDb, <<"attributes/sip_username">>, ViewOptions) of
        {'ok', JObj} ->
            ...
            CacheProps = [{'origin', {'db', AccountDb, EndpointId}}],
            kz_cache:store_local(
                ?CACHE_NAME, ?SIP_ENDPOINT_ID_KEY(AccountDb, Username), EndpointId, CacheProps
            ),
            {'ok', EndpointId};
        {'error', _R} ->
...
handle_bridge_failure(Cause, Code, Call) ->
    ...
    case
        (handle_bridge_failure(Cause, Call) =:= 'ok') orelse
            (handle_bridge_failure(Code, Call) =:= 'ok')
    of
        'true' -> 'ok';
        'false' -> 'not_found'
    end.
...
send_default_response(Cause, Call) ->
    case cf_exe:wildcard_is_empty(Call) of
        'false' ->
            lager:debug("non-empty wildcard; not sending ~s", [Cause]);
        'true' ->
...
-spec maybe_apply_dialplan(kz_json:path(), kz_json:object(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
...
fold_system_dialplans(Names) ->
    fun
        ({Key, Val}, Acc) when is_list(Val) ->
            lists:foldl(
                fun(ValElem, A) -> maybe_dialplan_suits({Key, ValElem}, A, Names) end, Acc, Val
            );
        ({Key, Val}, Acc) ->
            maybe_dialplan_suits({Key, Val}, Acc, Names)
    end.

-spec maybe_dialplan_suits(
...
-spec maybe_system_dialplan_name(
    {kz_term:ne_binary(), kz_json:object()}, kz_json:object(), kz_term:ne_binaries()
) -> kz_json:object().
...
start_event_listener(Call, Mod, Args) ->
    ...
    try cf_event_handler_sup:new(Name, Mod, [kapps_call:clear_helpers(Call) | Args]) of
        {'ok', P} ->
            {'ok', P};
        _E ->
            lager:debug("error starting event listener ~p: ~p", [Mod, _E]),
            {'error', _E}
    catch
        _:_R ->
...
-spec maybe_belongs_to_group(kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    boolean().
...
-spec find_user_endpoints(kz_term:ne_binaries(), kz_term:ne_binaries(), kapps_call:call()) ->
    kz_term:ne_binaries().
find_user_endpoints([], DeviceIds, _) ->
    DeviceIds;
...
sip_users_from_device_ids(EndpointIds, Call) ->
    lists:foldl(
        fun(EID, Acc) -> sip_users_from_device_id(EID, Acc, Call) end,
        [],
        EndpointIds
    ).
...
sip_user_from_device_id(EndpointId, Call) ->
    case kz_endpoint:get(EndpointId, Call) of
        {'error', _} -> 'undefined';
        {'ok', Endpoint} -> kzd_devices:sip_username(Endpoint)
    end.
...
process_event(Call, NoopId, JObj) ->
    ...
    case kapps_call_command:get_event_type(JObj) of
        ...
        {<<"error">>, _, <<"noop">>} ->
            ...
            {'error', JObj};
        {<<"call_event">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"noop">>} when
            NoopId =:= MsgId
        ->
...
get_timezone(JObj, Call) ->
    case kz_json:get_ne_binary_value(<<"timezone">>, JObj) of
        ...
        %% UI-1808
        <<"inherit">> -> kzd_accounts:timezone(kapps_call:account_id(Call));
        TZ -> TZ
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec mailbox(kz_term:ne_binary(), kz_term:ne_binary()) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
...
-spec maybe_cached_mailbox(kz_term:ne_binary(), integer()) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
...
-spec get_mailbox(kz_term:ne_binary(), integer()) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
get_mailbox(AccountDb, VMNumber) ->
    ...
    case kz_datamgr:get_single_result(AccountDb, <<"vmboxes/listing_by_mailbox">>, ViewOptions) of
        ...
        {'error', 'multiple_results'} ->
            lager:debug("multiple voicemail boxes with same number (~b)  in account db ~s", [
                VMNumber, AccountDb
            ]),
            {'error', 'not_found'};
        {'error', _R} = E ->
            lager:warning("unable to lookup voicemail number ~b in account ~s: ~p", [
                VMNumber, AccountDb, _R
            ]),
            E
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec normalize_capture_group(kz_term:api_binary(), kapps_call:call() | kapps_call:api_ne_binary()) ->
