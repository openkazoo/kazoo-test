...
-spec amqp_send(pid() | kapps_call:call(), kz_term:api_terms(), kz_amqp_worker:publish_fun()) ->
    'ok'.
...
handle_cast({'callid_update', NewCallId}, #state{call = Call} = State) ->
    ...
    lager:info("updating callid to ~s (from ~s), catch you on the flip side", [
        NewCallId, PrevCallId
    ]),
...
handle_cast('initialize', #state{call = Call} = State) ->
    ...
    Updaters = [
        fun(C) -> kapps_call:kvs_store('consumer_pid', self(), C) end,
...
handle_cast({'amqp_send', API, PubFun}, #state{queue = Q} = State) ->
    ...
    {'noreply', State};
handle_cast(
    {'gen_listener', {'is_consuming', 'true'}},
...
handle_event(JObj, #state{
    ...
    case {kz_util:get_event_type(JObj), kz_call_event:call_id(JObj)} of
        ...
        {{_Cat, _Name}, _Else} when Others =:= [] ->
            lager:info(
                "received ~s (~s) from call ~s while relaying for ~s",
...
do_launch_cf_module(
    ...
    Routines = [
        {fun kapps_call:kvs_store/3, 'cf_old_action', OldAction},
...
maybe_start_cf_module(ModuleBin, Data, Call) ->
    ...
    of
        ...
        'false' ->
            lager:debug(
                "skipping callflow module ~s (handle exported: ~s skip_module: ~s)",
...
spawn_cf_module(CFModule, Data, Call) ->
    ...
    {kz_util:spawn_monitor(fun cf_module_task/4, [CFModule, Data, Call, AMQPConsumer]), CFModule}.
...
%% @end
%%------------------------------------------------------------------------------
-spec amqp_send_message(kz_term:api_terms(), kz_amqp_worker:publish_fun(), kz_term:ne_binary()) ->
    'ok'.
amqp_send_message(API, PubFun, Q) ->
    PubFun(add_server_id(Q, API)).

-spec amqp_call_message(
    kz_term:api_terms(),
    kz_amqp_worker:publish_fun(),
    kz_amqp_worker:validate_fun(),
    kz_term:ne_binary()
...
send_command(_, 'undefined', _) ->
    lager:debug("no control queue to send command to");
send_command(_, _, 'undefined') ->
    lager:debug("no call id to send command to");
...
set_channel_destroyed(Self, JObj) ->
    ...
    DestroyHeaders = [
        {<<"Disposition">>, kz_json:get_ne_binary_value(<<"Disposition">>, JObj)},
        {<<"Hangup-Cause">>, Cause},
        {<<"Hangup-Code">>, Code}
    ],
...
relay_message(Notify, Message) ->
    _ = [
        kapps_call_command:relay_event(Pid, Message)
     || Pid <- Notify,
        is_pid(Pid)
    ],
    'ok'.
...
-spec handle_channel_pivoted(
    kz_types:server_ref(), kz_term:api_pid_ref(), kz_call_event:doc(), kapps_call:call()
) -> 'ok'.
handle_channel_pivoted(Self, PidRef, JObj, Call) ->
