    ...
    ets:select(table_id(), [
        {
            ...
            [
                {'orelse', {'=:=', '$1', <<Vsn/binary, ".", Action/binary>>},
                    {'orelse', {'=:=', '$1', <<"*.", Action/binary>>},
                        {'orelse', {'=:=', '$1', <<Vsn/binary, ".*">>}, {'=:=', '$1', <<"*.*">>}}}}
            ],
            ['$_']
        }
    ]).
...
matches([<<"#">>, <<"#">> | Bs], Rs) ->
    matches([<<"#">> | Bs], Rs);
% sadly, #.# would match foo, foo.bar, foo.bar.baz, etc
matches([<<"#">> | Bs], []) ->
    % so keep checking by stripping of the first #
    matches(Bs, []);
...
matches([<<"*">> | Bs], [_ | Rs]) ->
    % so ignore what the routing segment is and continue
    matches(Bs, Rs);
matches([<<"#">>, B | Bs], [B, B | Rs]) ->
    matches(Bs, Rs) orelse
        matches([<<"#">> | Bs], [B | Rs]);
...
matches([<<"#">>, <<"*">> | _] = Bs, [_ | Rs]) ->
    matches(Bs, Rs);
matches([<<"#">> | _] = Bs, [_ | Rs]) ->
    % otherwise leave the # in to continue matching
    matches(Bs, Rs);
...
bind(Binding, Module, Fun, Payload) when
    is_function(Fun, 1)
->
...
maybe_rm_binding(Binding, Mod, Fun, Payload) ->
    ...
    case ets:lookup(table_id(), Binding) of
        [] -> {'error', 'not_found'};
        [#kz_binding{} = Bind] -> maybe_rm_responder(Binding, Responder, Bind)
    end.
...
maybe_rm_responder(Binding, Responder, #kz_binding{binding_responders = Responders} = Bind) ->
    case queue:member(Responder, Responders) of
        'false' ->
            {'error', 'not_found'};
        'true' ->
            NewResponders = queue:filter(
                fun(QueueResponder) -> QueueResponder =/= Responder end, Responders
            ),
            case queue:len(NewResponders) of
                0 ->
                    ...
                    {'ok', 'deleted_binding'};
                _Len ->
                    ets:update_element(table_id(), Binding, {
                        #kz_binding.binding_responders, NewResponders
                    }),
                    {'ok', 'updated_binding'}
            end
    end.

-spec add_optimized_binding(
    kz_term:ne_binary(),
    kz_responder(),
    kz_term:ne_binaries(),
    kz_term:ne_binary(),
...
-spec add_binding(kz_term:ne_binary(), kz_responder(), kz_term:ne_binaries(), kz_term:api_binary()) ->
    boolean().
add_binding(Binding, Responder, Pieces, Prefix) ->
    Bind = #kz_binding{
        binding = Binding,
...
flush_mod(ClientMod, #kz_binding{
    binding = Binding,
        ...
        'false' ->
            case queue:len(Filtered) of
                ...
                _Len ->
                    ...
                    ets:update_element(table_id(), Binding, {
                        #kz_binding.binding_responders, Filtered
                    })
            end
    end.
...
-spec filter_bindings(
    filter_fun(), kz_term:ne_binary() | '$end_of_table', filter_updates(), kz_term:ne_binaries()
) -> 'ok'.
...
fold_bind_results(_, {'error', _} = E, _) ->
    [E];
fold_bind_results([], Payload, _Route) ->
    Payload;
fold_bind_results(Responders, Payload, Route) ->
    fold_bind_results(Responders, Payload, Route, length(Responders), []).

-spec fold_bind_results(
    kz_responders(), any(), kz_term:ne_binary(), non_neg_integer(), kz_responders()
) -> any().
...
log_function_clause(M, F, Length, [{RealM, RealF, RealArgs, Where} | _ST]) ->
    ?LOG_ERROR(
        "unable to find function clause for ~s:~s(~s) in ~s:~p",
        [
            RealM,
            RealF,
            kz_binary:join(
                [kz_term:to_binary(io_lib:format("~p", [A])) || A <- RealArgs], <<", ">>
            ),
            props:get_value('file', Where),
            props:get_value('line', Where)
        ]
    ),
...
pmap_responders(Acc, Responders, Payload) ->
    plists:map(
        fun(R) -> apply_map_responder(R, Payload) end,
...
apply_map_responder(_M, Fun, Payload) when
    is_function(Fun, 1)
->
    Fun(Payload);
...
maybe_log_undefined(M, F, Payload, [{M, F, Arity, _} | _]) when
    is_integer(Arity),
    length(Payload) == Arity
->
    'ok';
maybe_log_undefined(M, F, Payload, [{M, F, Payload, _} | _]) ->
    'ok';
maybe_log_undefined(M, F, Payload, ST) ->
    log_undefined(M, F, length(Payload), ST).

-spec maybe_log_function_clause(atom(), atom(), list(), list()) -> 'ok'.
maybe_log_function_clause(M, F, Payload, [{M, F, Arity, _} | _]) when
    is_integer(Arity),
    length(Payload) == Arity
->
    'ok';
maybe_log_function_clause(M, F, Payload, [{M, F, Payload, _} | _]) ->
    'ok';
maybe_log_function_clause(M, F, Payload, ST) ->
    log_function_clause(M, F, length(Payload), ST).
    ...
    [Reply | _] =
        lists:foldl(
            fun(
                ...
                case
                    Binding =:= Routing orelse
                        kazoo_bindings_rt:matches(Options, BParts, RoutingParts)
                of
                    'true' ->
