...
apply(JObj, MetaFormatters, Direction) ->
    Fs = [
        fun format_request/3,
        fun maybe_format_sip_headers/3
    ],

    lists:foldl(
        fun(F, J) -> F(J, MetaFormatters, Direction) end,
        JObj,
        Fs
    ).
...
maybe_format_sub_object(JObj, MetaFormatters, Direction, SubKey) ->
    case kz_json:get_json_value(SubKey, JObj) of
        'undefined' ->
            JObj;
        SubJObj ->
            Formatted = format_request(
                maybe_set_invite_format(JObj, SubJObj), MetaFormatters, Direction
            ),
...
format_request(JObj, MetaFormatters, Direction) ->
    ...
    kz_json:foldl(
        fun(MetaKey, Formatters, AccJObj) ->
            lager:debug("processing ~s ~p", [MetaKey, Formatters]),
            maybe_apply_formatters_fold(
                AccJObj,
                JObjKeys,
                MetaKey,
...
-spec filter_formatters_by_direction(direction(), kz_json:object() | kz_json:objects()) ->
    kz_json:objects().
...
-spec maybe_apply_formatters_fold(
    kz_json:object(), kz_json:path(), kz_term:ne_binary(), kz_json:objects()
) ->
    kz_json:object().
maybe_apply_formatters_fold(JObj, _JObjKeys, _MetaKey, []) ->
    JObj;
...
-spec maybe_apply_formatters(
    kz_json:object(), kz_term:ne_binary(), kz_json:json_term(), kz_json:objects()
) ->
    kz_json:object().
maybe_apply_formatters(JObj, _Key, _Value, []) ->
    JObj;
...
-type formatter_fun_4() :: fun(
    (kz_json:object(), kz_term:ne_binary(), kz_json:json_term(), kz_json:object()) -> ffun_return()
).
-type formatter_funs_4() :: [formatter_fun_4()].

-spec apply_formatter_funs(
    kz_json:object(),
    kz_term:ne_binary(),
    kz_json:json_term(),
    kz_json:objects(),
...
-spec maybe_match_invite_format(
    kz_json:object(), kz_term:ne_binary(), kz_json:json_term(), kz_json:object()
) ->
    ffun_return().
...
-spec maybe_apply_formatters(
    ...
    kz_json:object().
maybe_apply_formatters(JObj, _Key, _User, _Realm, []) ->
    JObj;
...
maybe_match_invite_format(JObj, Formatter) ->
    case
        kz_json:is_true(<<"match_invite_format">>, Formatter, 'false') andalso
            kz_json:get_ne_binary_value(<<"Invite-Format">>, JObj)
    of
        'false' -> 'false';
        'undefined' -> 'false';
        <<"loopback">> -> 'false';
...
apply_formatter(Captured, Formatter) ->
    list_to_binary([
        kz_json:get_binary_value(<<"prefix">>, Formatter, <<>>),
        Captured,
