...
-define(ACCOUNT_INBOUND_RECORDING(Network), [
    <<"call_recording">>, <<"account">>, <<"inbound">>, Network
]).
-define(ACCOUNT_OUTBOUND_RECORDING(Network), [
    <<"call_recording">>, <<"account">>, <<"outbound">>, Network
]).

-define(ACCOUNT_INBOUND_RECORDING_LABEL(Network),
    <<"inbound from ", Network/binary, " to account">>
).
-define(ACCOUNT_OUTBOUND_RECORDING_LABEL(Network),
    <<"outbound to ", Network/binary, " from account">>
).
...
maybe_record_inbound(FromNetwork, Endpoint, Call) ->
    maybe_record_inbound(
        ...
        Endpoint,
        Call,
        kz_json:get_json_value(?ACCOUNT_INBOUND_RECORDING(FromNetwork), Endpoint)
    ).
    ...
    case kz_json:is_true(<<"enabled">>, Data) of
        ...
        'true' ->
            LabeledData = kz_json:set_value(
                <<"origin">>, ?ACCOUNT_INBOUND_RECORDING_LABEL(FromNetwork), Data
            ),
            {'true',
...
maybe_record_outbound(ToNetwork, Endpoint, Call) ->
    maybe_record_outbound(
        ...
        Endpoint,
        Call,
        kz_json:get_json_value(?ACCOUNT_OUTBOUND_RECORDING(ToNetwork), Endpoint)
    ).
...
maybe_record_outbound(ToNetwork, _Endpoint, Call, Data) ->
    case kz_json:is_true(<<"enabled">>, Data) of
        ...
        'true' ->
            LabeledData = kz_json:set_value(
                <<"origin">>, ?ACCOUNT_OUTBOUND_RECORDING_LABEL(ToNetwork), Data
            ),
            {'true',
                kapps_call:start_recording(
                    LabeledData, kapps_call:kvs_store('recording_follow_transfer', 'false', Call)
