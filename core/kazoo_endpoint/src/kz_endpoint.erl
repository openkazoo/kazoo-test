...
-define(DEFAULT_MOBILE_SMS_OPTIONS,
    kz_json:from_list([
        {<<"Route-ID">>, ?DEFAULT_MOBILE_SMS_ROUTE},
...
-define(ENCRYPTION_MAP, [
    {<<"srtp">>, [{<<"RTP-Secure-Media">>, <<"true">>}]},
    {<<"zrtp">>, [
        {<<"ZRTP-Secure-Media">>, <<"true">>},
...
-spec check_endpoint_enabled(
    kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
check_endpoint_enabled(JObj, EndpointId, AccountDb, EndpointType) ->
    case
        {
            kz_doc:is_soft_deleted(JObj) orelse
                kz_doc:is_deleted(JObj),
...
cache_origin(JObj, EndpointId, AccountDb) ->
    Routines = [
        fun(P) -> [{'db', AccountDb, EndpointId} | P] end,
        fun(P) ->
            [
...
-spec maybe_cached_owner_id(kz_term:proplist(), kz_json:object(), kz_term:ne_binary()) ->
    kz_term:proplist().
...
-spec maybe_cached_hotdesk_ids(kz_term:proplist(), kz_json:object(), kz_term:ne_binary()) ->
    kz_term:proplist().
maybe_cached_hotdesk_ids(Props, JObj, AccountDb) ->
    case kz_json:get_keys([<<"hotdesk">>, <<"users">>], JObj) of
        ...
        OwnerIds ->
            lists:foldl(
                fun(Id, P) -> [{'db', AccountDb, Id} | P] end,
                Props,
                OwnerIds
            )
    end.
...
-spec merge_attributes(kz_json:object(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    kz_json:object().
...
-spec merge_attributes(
    kz_term:ne_binaries(), kz_term:api_object(), kz_term:api_object(), kz_term:api_object()
) ->
    kz_json:object().
...
-spec merge_attribute(
    kz_term:ne_binary(), kz_term:api_object(), kz_term:api_object(), kz_term:api_object()
) -> kz_json:object().
...
merge_attribute(<<"name">> = Key, Account, Endpoint, Owner) ->
    Name = create_endpoint_name(
        kz_json:get_ne_value(<<"first_name">>, Owner),
...
merge_attribute(<<"outbound_flags">>, Account, Endpoint, Owner) ->
    Static = lists:flatten([
        kzd_devices:outbound_static_flags(Account),
        kzd_devices:outbound_static_flags(Owner),
        kzd_devices:outbound_static_flags(Endpoint)
    ]),
    Dynamic = lists:flatten([
        kzd_devices:outbound_dynamic_flags(Account),
...
merge_attribute(Key, Account, Endpoint, Owner) ->
    ...
    EndpointAttr = kz_json:get_ne_value(Key, Endpoint, kz_json:new()),
    OwnerAttr = kz_json:get_ne_value(Key, Owner, kz_json:new()),
    Merged = kz_json:merge_recursive(
        [AccountAttr, EndpointAttr, OwnerAttr],
        fun(_, V) -> not kz_term:is_empty(V) end
    ),
    kz_json:set_value(Key, Merged, Endpoint).

-spec merge_attribute_caller_id(
    kz_term:api_object(), kz_term:api_object(), kz_term:api_object(), kz_term:api_object()
) -> kz_term:api_object().
...
-spec merge_attribute_caller_id_emergency(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    kz_json:object().
...
merge_call_recording(RecordingSettings) ->
    ...
    F1 = fun(Direction, RSAcc1) ->
        merge_call_recording_direction(Direction, RSAcc1, AnyDirectionSettings)
    end,
    RecordingSettings1 = lists:foldl(
        F1, kz_json:delete_key(<<"any">>, RecordingSettings), AnyDirections
    ),

    F2 = fun(Direction, Networks, RSAcc2) ->
        merge_call_recording_networks(Direction, Networks, RSAcc2, AnyNets)
    end,
    kz_json:foldl(F2, RecordingSettings1, RecordingSettings1).

-spec merge_call_recording_direction(
    kz_term:ne_binary(), kzd_call_recording:doc(), kz_json:object()
) -> kz_json:object().
...
-spec merge_call_recording_networks(
    kz_term:ne_binary(), kz_json:object(), kz_json:object(), kz_term:ne_binaries()
) -> kz_json:object().
merge_call_recording_networks(Direction, NetworksJObj, RecordingSettings, AnyNetworks) ->
    AnyNetworkSettings = kzd_call_recording:any(
        NetworksJObj, kz_json:from_list([{<<"enabled">>, 'false'}])
    ),
    Fun = fun(Network, NetworksAcc) ->
        merge_call_recording_direction(Network, NetworksAcc, AnyNetworkSettings)
    end,
...
get_account_record_call_properties(AccountDoc) ->
    ...
    kz_json:foldl(
        fun(Type, RecordingSettings, Acc) ->
...
get_device_record_call_properties('undefined') ->
    kz_json:new();
...
get_legacy_record_call_properties(EndpointDoc) ->
    ...
    case kz_json:is_true(<<"record_call">>, Legacy) of
        ...
        'true' ->
            ...
            AnyNet = kz_json:from_list([
                {<<"onnet">>, Settings},
                {<<"offnet">>, Settings}
            ]),
            RecordCall = kz_json:from_list([
                {<<"inbound">>, AnyNet},
                {<<"outbound">>, AnyNet}
            ]),
...
endpoint_recording(RecordCall, 'true') ->
    RecordCall.
...
-spec merge_value(
    kz_term:ne_binary(), kz_term:api_object(), kz_json:object(), kz_term:api_object()
) ->
    kz_json:object().
merge_value(Key, Account, Endpoint, Owner) ->
...
caller_id_owner_attr(Owner) ->
    ...
    case kz_json:get_ne_binary_value(L, OwnerAttr) of
        'undefined' ->
            Name = create_endpoint_name(
                kz_json:get_ne_binary_value(<<"first_name">>, Owner),
    ...
    lists:foldl(
        fun(Classifier, Acc) ->
            ...
            case lists:all(Fun, UserJObjs) of
                'false' -> Acc;
                'true' -> kzd_user:set_classifier_restriction(Acc, Classifier, <<"deny">>)
            end
        end,
        AccJObj,
        kz_json:get_keys(knm_converters:available_classifiers())
...
-spec create_endpoint_name(
    kz_term:api_binary(), kz_term:api_binary(), kz_term:api_binary(), kz_term:api_binary()
) -> kz_term:api_binary().
...
flush_account(AccountDb) ->
    ToRemove =
        kz_cache:filter_local(?CACHE_NAME, fun
            ({?MODULE, Db, _Id}, _Value) ->
                Db =:= AccountDb;
            (_, _) ->
                'false'
        end),
...
-type build_errors() ::
    'db_not_reachable'
    | 'endpoint_disabled'
    | 'endpoint_called_self'
    | 'endpoint_id_undefined'
    | 'invalid_endpoint_id'
    | 'not_found'
    | 'owner_called_self'
    | 'do_not_disturb'
...
evaluate_rules_for_creation(Endpoint, Properties, Call) ->
    Routines = [
        fun maybe_missing_resource_type/3,
    ...
    lists:foldl(
        fun should_create_endpoint_fold/2,
        {Endpoint, Properties, Call},
        Routines
    ).

-type create_ep_acc() ::
    {kz_json:object(), kz_json:object(), kapps_call:call()}
    | {'error', any()}.
...
should_create_endpoint_fold(Routine, {Endpoint, Properties, Call} = Acc) when
    is_function(Routine, 3)
->
    case Routine(Endpoint, Properties, Call) of
        'ok' -> Acc;
        Error -> Error
    end;
should_create_endpoint_fold(_Routine, Error) ->
    Error.
...
-spec maybe_owner_called_self(
    kz_json:object(), kz_json:object(), kz_term:api_binary(), kapps_call:call()
) ->
    'ok'
    | {'error', 'owner_called_self'}.
maybe_owner_called_self(Endpoint, Properties, <<"audio">>, Call) ->
    ...
    case
        CanCallSelf orelse
            (not is_binary(OwnerId)) orelse
            (not is_binary(EndpointOwnerId)) orelse
...
maybe_owner_called_self(Endpoint, Properties, <<"sms">>, Call) ->
    ...
    DefTextSelf = kapps_account_config:get_global(
        AccountId, ?CONFIG_CAT, <<"default_can_text_self">>, 'true'
    ),
...
-spec maybe_endpoint_called_self(
    kz_json:object(), kz_json:object(), kz_term:api_binary(), kapps_call:call()
) ->
    'ok'
    | {'error', 'endpoint_called_self'}.
maybe_endpoint_called_self(Endpoint, Properties, <<"audio">>, Call) ->
    ...
    case
        CanCallSelf orelse
            (not is_binary(AuthorizingId)) orelse
            (not is_binary(EndpointId)) orelse
            AuthorizingId =/= EndpointId
    of
        'true' ->
            'ok';
        'false' ->
...
maybe_exclude_from_queues(Endpoint, _Properties, Call) ->
    case
        is_binary(kapps_call:custom_channel_var(<<"Queue-ID">>, Call)) andalso
            kz_json:is_true(<<"exclude_from_queues">>, Endpoint)
    of
        'false' -> 'ok';
...
create_endpoints(Endpoint, Properties, Call) ->
    Routines = [
        fun maybe_create_fwd_endpoint/3,
...
maybe_start_metaflows(_Call, _Endpoints, 'undefined') ->
    'ok';
maybe_start_metaflows(Call, Endpoints, _CallId) ->
    case
        not is_sms(Call) andalso
            kapps_call:custom_channel_var(<<"Metaflow-App">>, Call)
    of
        'false' ->
            'ok';
        'undefined' ->
...
maybe_start_metaflow(Call, Endpoint) ->
    case kz_json:get_first_defined([<<"metaflows">>, <<"Metaflows">>], Endpoint) of
        ...
        Metaflow ->
            ...
            lager:debug(
                "sending metaflow for endpoint: ~s: ~s",
...
-type ep_routine() :: fun(
    (kz_json:object(), kz_json:object(), kapps_call:call()) -> {'error', _} | kz_json:object()
).
-spec try_create_endpoint(
    ep_routine(), kz_json:objects(), kz_json:object(), kz_json:object(), kapps_call:call()
) ->
    kz_json:objects().
...
maybe_create_endpoint(Endpoint, Properties, Call) ->
    case
        is_call_forward_enabled(Endpoint, Properties) andalso
            kzd_devices:call_forward_substitute(Endpoint, 'false')
    of
        'true' ->
            {'error', 'call_forward_substitute'};
        'false' ->
...
is_call_forward_enabled(Endpoint, Properties) ->
    ...
    kz_json:is_true(<<"enabled">>, CallForwarding) andalso
        not kz_term:is_empty(Number) andalso
        (kz_json:is_false(<<"direct_calls_only">>, CallForwarding, 'true') orelse
            (not lists:member(Source, ?NON_DIRECT_MODULES))).

-spec maybe_create_endpoint(
    kz_term:ne_binary(), kz_json:object(), kz_json:object(), kapps_call:call()
) ->
...
get_endpoint_type(Endpoint, Call) ->
    ...
    Type = lists:foldl(
        fun
            (Fun, 'undefined') -> Fun(Endpoint, Call);
            (_Fun, T) -> T
        end,
        'undefined',
        Routines
...
%%------------------------------------------------------------------------------
-record(clid, {
    caller_number :: kz_term:api_binary(),
...
-spec get_clid(kz_json:object(), kz_json:object(), kapps_call:call(), kz_term:ne_binary()) ->
    clid().
get_clid(Endpoint, Properties, Call, Type) ->
    case kz_json:is_true(<<"suppress_clid">>, Properties) of
        'true' ->
            maybe_move_privacy(Endpoint, Properties, Call, #clid{});
        'false' ->
            {Number, Name} = kz_attributes:caller_id(Type, Call),
            CallerNumber =
                case kapps_call:caller_id_number(Call) of
                    Number -> 'undefined';
                    _Number -> Number
                end,
            CallerName =
                case kapps_call:caller_id_name(Call) of
                    Name -> 'undefined';
                    _Name -> Name
                end,
...
maybe_move_privacy(Endpoint, _Properties, Call, Clid) ->
    ...
    RetainCID =
        kz_json:is_true(<<"keep_caller_id">>, CallForward) orelse
            kz_json:is_true(<<"Retain-CID">>, CCVs),
...
maybe_build_failover(Endpoint, Call) ->
    ...
    case
        IsFailover andalso
            not kz_term:is_empty(Number)
    of
        'false' -> 'undefined';
        'true' -> create_call_fwd_endpoint(Endpoint, kz_json:new(), Call)
    end.

-spec create_push_endpoint(kz_json:object(), kz_json:object(), kapps_call:call()) ->
    kz_term:api_object().
create_push_endpoint(Endpoint, Properties, Call) ->
    Clid = get_clid(Endpoint, Properties, Call),
    ...
    case kz_json:get_ne_binary_value(<<"Token-Type">>, PushJObj) of
        ...
        TokenType ->
            ...
            ExtraHeaders = kapps_config:get_json(
                <<"pusher">>, [TokenType, <<"extra_headers">>], kz_json:new(), TokenApp
            ),
...
create_call_fwd_endpoint(Endpoint, Properties, Call) ->
    ...
    IgnoreEarlyMedia =
        case
            kz_json:is_true(<<"require_keypress">>, CallForward) orelse
                not kz_json:is_true(<<"substitute">>, CallForward)
        of
            'true' -> <<"true">>;
    ...
    kz_json:from_list(
        [
            ...
            {<<"To-DID">>,
                kz_json:get_value(<<"number">>, Endpoint, kapps_call:request_user(Call))},
            {<<"Route">>, ToDID},
...
build_mobile_route(MDN) ->
    ...
    case re:run(MDN, Regex, [{'capture', 'all', 'binary'}]) of
        ...
        {'match', Captures} ->
            ...
            Prefix = kapps_config:get_binary(
                ?MOBILE_CONFIG_CAT, <<"prefix">>, ?DEFAULT_MOBILE_PREFIX
            ),
            Suffix = kapps_config:get_binary(
                ?MOBILE_CONFIG_CAT, <<"suffix">>, ?DEFAULT_MOBILE_SUFFIX
            ),
            Realm = kapps_config:get_binary(?MOBILE_CONFIG_CAT, <<"realm">>, ?DEFAULT_MOBILE_REALM),
            Route = list_to_binary([
                "sip:",
                Prefix,
                Root,
                Suffix,
...
-spec generate_sip_headers(
    kz_json:object(), kz_term:ne_binary(), kz_json:object(), kapps_call:call()
) ->
    kz_json:object().
generate_sip_headers(Endpoint, Type, Acc, Call) ->
    ...
    HeaderFuns = [
        fun(J) -> maybe_add_sip_headers(J, Endpoint, Call) end,
...
-spec maybe_add_push_headers(
    kz_json:object(), kz_json:object(), kz_term:ne_binary(), kapps_call:call()
) -> kz_json:object().
...
maybe_add_push_headers(JObj, _Endpoint, _Type, _Call) ->
    JObj.

-spec maybe_add_diversion(
    kz_json:object(), kz_json:object(), kz_term:api_binary(), kapps_call:call()
) -> kz_json:object().
maybe_add_diversion(JObj, Endpoint, _Inception, Call) ->
    ShouldAddDiversion =
        kapps_call:authorizing_id(Call) =:= 'undefined' andalso
            kzd_devices:call_forward_keep_caller_id(Endpoint, 'false') andalso
            kapps_config:get_is_true(?CONFIG_CAT, <<"should_add_diversion_header">>, 'false'),
    case ShouldAddDiversion of
        'true' ->
            Diversion = list_to_binary([
                "<sip:", kapps_call:request(Call), ">", ";reason=unconditional"
            ]),
...
-spec maybe_add_sip_headers(kz_json:object(), kz_json:object(), kapps_call:call()) ->
    kz_json:object().
...
maybe_add_alert_info(JObj, Endpoint, _, Call) ->
    case kapps_call:kvs_fetch(<<"Override-Ringtone">>, Call) of
        'undefined' ->
            maybe_add_alert_info_from_endpoint(JObj, Endpoint, kapps_call:inception(Call));
        Ringtone ->
            set_alert_info(Ringtone, JObj)
    end.
...
-spec maybe_add_alert_info_from_endpoint(kz_json:object(), kz_json:object(), kz_term:api_binary()) ->
    kz_json:object().
...
-spec maybe_add_invite_format(kz_json:object(), kz_json:object(), kapps_call:call()) ->
    kz_json:object().
...
-spec maybe_add_invite_format(
    kz_json:object(), kz_json:object(), kapps_call:call(), kz_term:ne_binary()
) ->
    kz_json:object().
...
-spec maybe_add_aor(kz_json:object(), kz_json:object(), kz_term:api_binary(), kz_term:ne_binary()) ->
    kz_json:object().
maybe_add_aor(JObj, _, 'undefined', _Realm) ->
    JObj;
...
maybe_set_endpoint_id({Endpoint, Call, CallFwd, CCVs}) ->
    {Endpoint, Call, CallFwd,
        case kz_doc:id(Endpoint) of
            ...
            EndpointId ->
                AuthType =
                    case get_endpoint_type(Endpoint, Call) of
                        <<"mobile">> -> <<"mobile">>;
...
maybe_set_account_id({Endpoint, Call, CallFwd, CCVs}) ->
    ...
    {Endpoint, Call, CallFwd, kz_json:set_value(<<"Account-ID">>, AccountId, CCVs)}.
    ...
    case kz_json:is_true(<<"require_keypress">>, CallFwd) of
        ...
        'true' ->
            ...
            Confirm = [
                ...
                {<<"Require-Ignore-Early-Media">>, <<"true">>},
                {<<"Require-Fail-On-Single-Reject">>,
                    <<"USER_BUSY,CALL_REJECTED,NO_ANSWER,NORMAL_CLEARING,PROGRESS_TIMEOUT">>}
            ],
            {Endpoint, Call, CallFwd, kz_json:merge_jobjs(kz_json:from_list(Confirm), CCVs)}
    end.
...
maybe_enable_fax({Endpoint, Call, CallFwd, CCVs} = Acc) ->
    case kz_json:get_value([<<"media">>, <<"fax_option">>], Endpoint) of
        <<"auto">> ->
            {Endpoint, Call, CallFwd, kz_json:set_value(<<"Fax-Enabled">>, <<"true">>, CCVs)};
        _Else ->
            Acc
    end.
...
maybe_enforce_security({Endpoint, Call, CallFwd, CCVs}) ->
    EnforceSecurity = kz_json:is_true(
        [<<"media">>, <<"encryption">>, <<"enforce_security">>], Endpoint, 'true'
    ),
    {Endpoint, Call, CallFwd,
...
maybe_set_encryption_flags({Endpoint, Call, CallFwd, CCVs}) ->
    {Endpoint, Call, CallFwd, encryption_method_map(CCVs, Endpoint)}.

-spec encryption_method_map(kz_term:api_object(), kz_term:api_binaries() | kz_json:object()) ->
    kz_term:api_object().
...
set_sip_invite_domain({Endpoint, Call, CallFwd, CCVs}) ->
    ...
    {Endpoint, Call, CallFwd, kz_json:set_value(<<"SIP-Invite-Domain">>, SipRealm, CCVs)}.
...
get_codecs(JObj) ->
    case
        kz_json:get_value([<<"media">>, <<"audio">>, <<"codecs">>], JObj, []) ++
...
get_ignore_completed_elsewhere(JObj) ->
    case
        kz_json:get_first_defined(
            [
                [<<"caller_id_options">>, <<"ignore_completed_elsewhere">>],
    ...
    of
        'undefined' ->
            kapps_config:get_is_true(?CONFIG_CAT, <<"default_ignore_completed_elsewhere">>, 'true');
        IgnoreCompletedElsewhere ->
            kz_term:is_true(IgnoreCompletedElsewhere)
    end.
...
create_mobile_sms_endpoint_failover(Endpoint, []) ->
    Endpoint;
...
-spec build_mobile_sms_route(kz_term:ne_binary()) ->
    ...
    | {'error', 'invalid_mdn'}.
build_mobile_sms_route(MDN) ->
    Type = kapps_config:get_ne_binary(
        ?MOBILE_CONFIG_CAT, <<"sms_interface">>, ?DEFAULT_MOBILE_SMS_INTERFACE
    ),
...
build_mobile_sms_route(<<"amqp">>, _MDN) ->
    Connections = kapps_config:get_json(
        ?MOBILE_CONFIG_CAT, [<<"sms">>, <<"connections">>], ?DEFAULT_MOBILE_AMQP_CONNECTIONS
    ),
    {<<"amqp">>, kz_json:foldl(fun build_mobile_sms_amqp_route/3, [], Connections)}.

-spec build_mobile_sms_amqp_route(kz_json:path(), kz_json:json_term(), kz_term:proplist()) ->
    sms_routes().
...
build_mobile_sms_amqp_route_options(JObj) ->
    [
        ...
        {<<"Exchange-ID">>,
            kz_json:get_ne_binary_value(<<"exchange">>, JObj, ?DEFAULT_MOBILE_SMS_EXCHANGE)},
        {<<"Exchange-Type">>,
            kz_json:get_ne_binary_value(<<"type">>, JObj, ?DEFAULT_MOBILE_SMS_EXCHANGE_TYPE)},
        {<<"Exchange-Options">>,
            kz_json:get_json_value(<<"options">>, JObj, ?DEFAULT_MOBILE_SMS_EXCHANGE_OPTIONS)}
    ].
...
-spec get_sip_realm(kz_json:object(), kz_term:ne_binary(), Default) ->
    Default | kz_term:ne_binary().
...
-spec endpoint_actions(kz_json:object(), kapps_call:call(), kz_term:api_object()) ->
    kz_json:object().
...
-type actions_acc() :: {
    kz_json:object(), kapps_call:call(), kz_term:api_object(), kz_json:object()
}.
