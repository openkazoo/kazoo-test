...
-define(OFFNET_REQ,
    kz_json:from_list([
        {<<"Custom-SIP-Headers">>,
            kz_json:from_list([{<<"Diversions">>, [<<"sip:14158867900@1.2.3.4;counter=1">>]}])},
        {<<"Invite-Format">>, <<"npan">>}
    ])
).
...
diversion_match_invite_test_() ->
    Formatter = kz_json:from_list([
        {<<"diversions">>, [kz_json:from_list([{<<"match_invite_format">>, 'true'}])]}
    ]),
    Bridge = kz_formatters:apply(?OFFNET_REQ, Formatter, 'outbound'),

    [
        ?_assertEqual(
            <<"sip:4158867900@1.2.3.4">>,
...
diversion_strip_test_() ->
    Formatter = kz_json:from_list([{<<"diversions">>, [kz_json:from_list([{<<"strip">>, 'true'}])]}]),
    Bridge = kz_formatters:apply(?OFFNET_REQ, Formatter, 'outbound'),
...
replace_value_test_() ->
    ...
    Formatter = kz_json:from_list([
        {<<"from">>, [kz_json:from_list([{<<"value">>, Replace}])]},
        {<<"caller_id_number">>, [kz_json:from_list([{<<"value">>, Replace}])]}
    ]),

    Bridge = kz_formatters:apply(?ROUTE_REQ, Formatter, 'outbound'),
    [
        ?_assertEqual(
            <<Replace/binary, "@2600Hz.com">>,
...
-define(CALLER_ID_NAME,
    kz_json:from_list([
        {<<"caller_id_number">>, kz_json:from_list([{<<"value">>, <<"0099887766">>}])}
    ])
).
...
formatter_validation_test_() ->
    ...
    [
        ...
        ?_assertMatch(
            {'ok', _}, kz_json_schema:validate(Schema, ?CALLER_ID_NAME, ?SCHEMA_TEST_OPTIONS)
        )
    ].
...
formatters_test_() ->
    ...
    [
        ?_assertEqual(
            <<"+14158867900@2600Hz.com">>, kz_json:get_ne_binary_value(<<"From">>, FromInboundRoute)
        ),
        ?_assertEqual(
            <<"4158867900@2600Hz.com">>, kz_json:get_ne_binary_value(<<"From">>, FromOutboundRoute)
        ),
        ?_assertEqual(<<"0099887766">>, kz_json:get_value(<<"Caller-ID-Number">>, CIDRoute)),

        ?_assertEqual(
            <<"+14158867900@2600Hz.com">>, kz_json:get_ne_binary_value(<<"From">>, MergedRoute)
        ),
