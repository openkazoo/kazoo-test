...
-define(SIG_RESP(AuthSig, AppSig), [?NE_BINARY = AuthSig, ?NE_BINARY = AppSig]).
...
-spec sign_value(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), integer()) ->
    kz_term:ne_binary().
...
verify_user(#{<<"user_name">> := _UserId}, _AuthUser, _AppUser) ->
    lager:debug(
        "user ~s mismatched (user_name: ~s auth_user: ~s app_user: ~s)",
...
do_parse_fold(Maps, [Fun | Funs]) -> do_parse_fold(Fun(Maps), Funs).
...
verify_prefix(
    ...
    PrefixCheck orelse
        lager:debug("invalid prefix, expects ~s got ~s", [OrigPrefix, RespPrefix]),
    Maps#{verify_result => PrefixCheck}.

-spec verify_signature(map()) -> map().
verify_signature(
    #{
        response_signature := UserSig,
        response_cookie := Cookie,
        response_prefix := Prefix,
        key := Key
    } = Maps
    ...
    SignatureCheck orelse
        lager:debug("invalid ~s signature", [Prefix]),
...
verify_integeration_key(
    ...
    CheckIKey orelse
        lager:debug("invalid integration key"),
...
verify_expiration(
    ...
    CheckExpire orelse
        lager:debug(
            "~s part of duo response is expired (expects lower than ~b got ~b )",
...
validate_values(Identity) ->
    try
        lists:all(
            fun(ReqV) ->
                validate_value(ReqV, Identity) orelse
                    throw({'error', ReqV})
            end,
            ?REQ_VALUES
        )
...
validate_value(<<"integration_key">> = K, Identity) ->
    ...
    kz_term:is_ne_binary(IKey) andalso
        erlang:size(IKey) == ?IKEY_LEN;
validate_value(<<"secret_key">> = K, Identity) ->
    SKey = maps:get(K, Identity),
    kz_term:is_ne_binary(SKey) andalso
        erlang:size(SKey) >= ?SKEY_LEN;
validate_value(<<"application_secret_key">> = K, Identity) ->
    AKey = maps:get(K, Identity),
    kz_term:is_ne_binary(AKey) andalso
        erlang:size(AKey) >= ?AKEY_LEN;
