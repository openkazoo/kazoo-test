...
-spec decode(kz_term:ne_binary()) ->
    {'ok', kz_term:proplist(), kz_term:proplist()} | {'error', any()}.
decode(JWTToken) ->
    decode(JWTToken, 'true').

-spec decode(kz_term:ne_binary() | map(), boolean()) ->
    {'ok', kz_term:proplist(), kz_term:proplist()} | {'error', any()}.
...
do_verify_fold(Token, [Fun | Funs]) -> do_verify_fold(Fun(Token), Funs).
...
verify_signature(#{} = Token) ->
    case kz_auth_keys:from_token(Token) of
        {'ok', Key} ->
            verify_signature(Token#{jwt_public_key => Key});
        {'error', _Error} ->
...
add_kid(#{auth_app := #{pvt_server_key := Key}} = Map) -> Map#{kid => Key};
add_kid(Map) -> {error, Map#{error => <<"no key identifier">>}}.
...
add_alg(#{auth_app := #{jwt_algorithm := Alg}} = Map) -> Map#{alg => Alg};
add_alg(Map) -> Map#{alg => ?DEFAULT_ALGORITHM}.

-spec add_digest(map()) -> map().
add_digest(#{digest := _} = Map) -> Map;
add_digest(#{alg := Alg} = Map) -> Map#{digest => alg_2_digest_type(Alg)}.
...
parse(JWTToken) when is_binary(JWTToken) ->
    case binary:split(JWTToken, <<".">>, ['global']) of
        [Header, Payload, Signature] ->
            Map = #{
                header => kz_json:to_map(kz_json:decode(kz_base64url:decode(Header))),
