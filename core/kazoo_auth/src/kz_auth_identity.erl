...
%% @end
%%------------------------------------------------------------------------------
-spec sign(map() | kz_term:proplist() | kz_json:object()) ->
...
->
    ...
    of
        #{
            ...
            {'ok', crypto:mac('hmac', HashMethod, CryptoKey, Identity)};
        #{} ->
...
identity_secret(
    #{
        auth_provider := #{
            jwt_user_id_claim := IdentityField,
            name := Name
        },
        payload := Claims
    } = Token
...
get_identity_secret(
    ...
    case kz_datamgr:open_cache_doc(Db, Key) of
        {'ok', JObj} ->
            check_kazoo_secret(Token#{user_doc => JObj, user_map => kz_json:to_map(JObj)});
        {'error', 'not_found'} ->
...
get_identity_secret(
    #{
        auth_provider := #{profile_cache_timer := _Timer},
        auth_db_id := Key
    } = Token
) ->
...
check_cache_expiration(
    #{
        auth_provider := #{
            cached_profile_field := ProfileField,
...
check_secret(
    #{
        auth_provider := #{profile_signature_secret_field := Field},
        user_doc := JObj
    } = Token
) ->
    ...
    case kz_json:get_value(?PVT_SIGNING_SECRET, JObj) of
        'undefined' ->
            case update_kazoo_secret(Token) of
                #{} = NewToken -> NewToken;
                {'error', _} -> {'error', {500, 'datastore_fault'}}
            end;
        Secret ->
            Token#{identity_secret => Secret}
    end.
...
%%------------------------------------------------------------------------------
-spec token(map()) -> map().
token(#{identify_verified := _} = Token) ->
    Token;
token(
    #{
        auth_provider := #{
    ...
    case
        kz_term:is_not_empty(IdentitySig) andalso
            identity_secret(Token)
    of
        'false' ->
        ...
        Token1 ->
            verify_identity_signature(Token1, Secret, Hash, IdentitySig)
    end;
token(
    #{
        payload := Payload,
...
-spec verify_identity_signature(
    map(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()
) -> map().
verify_identity_signature(
    #{identity_secret := IdentitySecret, auth_id := Identity} = Token, Secret, Hash, IdentitySig
) ->
    ...
    ExpectedSignature = crypto:mac('hmac', HashMethod, CryptoKey, Identity),
...
verify_identity_signature(Token, _IdentitySignature, _ExpectedSignature) ->
    lager:info("provided identity signature (~s) did not match the expected signature", [
        _IdentitySignature
    ]),
    Token#{identify_verified => 'false', identity_error => 'invalid_identity_signature'}.
...
reset_identity_secret(
    ...
    case kz_datamgr:open_cache_doc(Db, Key) of
        ...
        {'error', _Reason} = Error ->
            lager:debug("failed to read identity document, auth_db ~s auth_id ~s: ~p", [
                Db, Key, _Reason
            ]),
            Error
    end.
