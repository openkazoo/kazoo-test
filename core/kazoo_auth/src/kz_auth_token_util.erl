...
add_application(#{auth_app := _App} = Token) ->
    Token;
...
access_token(
    ...
    Token#{
        static_token => kz_json:from_list([
            {<<"token_type">>, TokenType},
            {<<"access_token">>, AccessToken}
        ])
    };
...
-spec verify(map()) -> map().
verify(#{verified_token := _} = Token) ->
    Token;
verify(
    #{
        auth_provider := #{token_info_url := TokenInfoUrl},
        access_token := AccessToken
    } = Token
) ->
...
create_claims(#{user_doc := Doc, profile := Profile} = Token) ->
    case
        build_claims(?JWT_CLAIMS, [Doc, Profile]) ++
...
build_claims([], _, Claims) ->
    Claims;
build_claims([{K1, K2} | KVs], Map, Claims) when
    is_map(Map)
->
...
build_claims([{K1, K2} | KVs], JObjs, Claims) when
    is_list(JObjs)
->
