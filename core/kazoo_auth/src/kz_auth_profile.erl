...
-define(TOKEN_VERIFIED_EMAIL_FIELDS, [
    <<"email_verified">>,
    <<"verified_email">>
]).
...
maybe_load_profile(#{profile := _Profile} = Token) ->
    Token;
maybe_load_profile(#{user_map := #{<<"profile">> := Profile}} = Token) ->
    Token#{profile => kz_json:from_map(Profile)};
maybe_load_profile(
    #{
        auth_provider := #{profile_url := _ProfileURL} = Provider,
    ...
    case kz_http:req(Verb, URL, Headers, <<>>, Options) of
        ...
        {'ok', Code, _RespHeaders, _RespXML} ->
            lager:debug("received code ~b while getting auth profile from ~s : ~p", [
                Code, URL, _RespXML
            ]),
...
maybe_load_profile(
    #{
        auth_provider := #{profile_url := _ProfileURL},
        original := Original
    } = Token
) ->
...
maybe_load_profile(
    #{
        auth_provider := #{profile_discovery_field := ProfileField} = AuthProvider,
        discovery := Discovery
    } = Token
) ->
    maybe_load_profile(Token#{
        auth_provider => AuthProvider#{
            profile_url => kz_json:get_ne_binary_value(ProfileField, Discovery)
        }
    });
maybe_load_profile(
    #{
...
profile_url(
    #{
        auth_provider := #{profile_url := ProfileURL} = Provider,
        access_token := AccessToken
    } = Token
) ->
    case maps:get('profile_access_auth_type', Provider, <<"token">>) of
...
maybe_add_user_identity(#{user_identity := _Identity} = Token) ->
    Token;
maybe_add_user_identity(#{profile_error_code := _Error} = Token) ->
    Token;
maybe_add_user_identity(
    #{
        auth_provider := #{profile_identity_field := Field},
        profile := Profile
    } = Token
) ->
    case kz_json:get_first_defined([Field], Profile) of
        'undefined' ->
            lager:debug(
                "user identity from field '~s' not found in profile: ~s",
...
maybe_add_user_identity(
    #{
        auth_provider := #{profile_identity_fields := Fields},
        profile := Profile
    } = Token
) ->
    Keys = lists:filter(
        fun(Field) -> kz_json:get_value(Field, Profile) =/= 'undefined' end, Fields
    ),
...
maybe_add_display_name(#{display_name := _DisplayName} = Token) ->
    Token;
maybe_add_display_name(#{profile_error_code := _Error} = Token) ->
    Token;
maybe_add_display_name(
    #{
        auth_provider := #{profile_displayName_field := Field},
        profile := Profile
    } = Token
) ->
    case kz_json:get_first_defined([Field], Profile) of
...
maybe_add_display_name(#{auth_provider := Provider} = Token) ->
    maybe_add_display_name(Token#{
        auth_provider => Provider#{profile_displayName_field => <<"name">>}
    }).

-spec maybe_add_photo_url(map()) -> map().
maybe_add_photo_url(#{photo_url := _PhotoUrl} = Token) ->
    Token;
maybe_add_photo_url(#{profile_error_code := _Error} = Token) ->
    Token;
maybe_add_photo_url(
    #{
        auth_provider := #{profile_photo_url_field := Field},
        profile := Profile
    } = Token
) ->
    case kz_json:get_first_defined([Field], Profile) of
...
maybe_add_user_email(#{user_email := _UserEmail} = Token) ->
    Token;
maybe_add_user_email(#{verified_token := VerifiedToken} = Token) when
    VerifiedToken =/= ?EMPTY_JSON_OBJECT
->
    Token#{user_email => kz_json:get_first_defined(?PROFILE_EMAIL_FIELDS, VerifiedToken)};
maybe_add_user_email(#{profile_error_code := _Error} = Token) ->
    Token;
maybe_add_user_email(
    #{
        auth_provider := #{profile_email_field := Field},
        profile := Profile
    } = Token
) ->
...
maybe_add_user(#{user_doc := _DocObj, user_map := _Map} = Token) ->
    Token;
maybe_add_user(#{user_doc := DocObj} = Token) ->
    Token#{user_map => kz_json:to_map(DocObj)};
maybe_add_user(#{profile_error_code := _Error} = Token) ->
    Token;
maybe_add_user(
    #{
        auth_app := #{pvt_user_prefix := Prefix},
...
-spec ensure_profile_properties(kz_term:ne_binary(), kz_term:proplist(), kz_term:proplist(), map()) ->
    map().
    ...
    case kz_datamgr:open_cache_doc(?KZ_AUTH_DB, DocId) of
        {'ok', Doc} ->
            case Missing -- kz_json:get_keys(Doc) of
                ...
                _StillMissing ->
                    lager:info("missing properties when updating user : ~p", [
                        kz_binary:join(_StillMissing)
                    ]),
...
do_update_user(DocId, Props, Token) ->
    ...
    case kz_datamgr:update_doc(?KZ_AUTH_DB, DocId, UpdateOptions) of
        ...
        {'error', _Error} ->
            ...
            Token#{
                profile_error_code => {500, 'datastore_fault'},
                profile => kz_json:new()
            }
    end.
...
maybe_cache_user(
    #{
        auth_provider := #{profile_cache_timer := Timer},
        user_identity := Identity
    } = Token,
    DocId
) ->
    Props = [
        {'origin', {'db', ?KZ_AUTH_DB, DocId}},
        {'expires', Timer}
    ],
    kz_cache:store_local(?PROFILE_CACHE, DocId, Identity, Props),
    Token;
maybe_cache_user(Token, _DocId) ->
    Token.
...
maybe_required_properties_missing(
    #{auth_provider := #{profile_required_props := RequiredProps}} = Token, Props, JObj
) ->
    case RequiredProps -- props:get_keys(Props) of
        [] ->
            maybe_cache_user(
...
updates_needed(JObj, Props) ->
    lists:foldl(
        fun(K, KVs) ->
            case {props:get_value(K, Props), kz_json:get_value(K, JObj)} of
                {'undefined', _} -> KVs;
                {Value, Value} -> KVs;
                {Value, _} -> [{K, Value} | KVs]
            end
...
format_user_doc(
    #{
        auth_provider := #{name := ProviderId} = Provider,
        profile := Profile,
        user_identity := Identity
    } = Token
) ->
    ...
    Props =
        [
            {<<"email">>, EMail},
            {<<"verified_email">>,
                kz_json:get_first_defined(?TOKEN_VERIFIED_EMAIL_FIELDS, Verified)},
