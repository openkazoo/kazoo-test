...
-define(TEST_SIGN_CLAIM, [{<<"owner_id">>, ?USER}]).
...
-define(VALID_SIG_AUTH_RESP,
    <<"AUTH|dGVzdHVzZXJ8RElYWFhYWFhYWFhYWFhYWFhYWFh8MTYxNTcyNzI0Mw==|d20ad0d1e62d84b00a3e74ec201a5917e77b6aef">>
).

-define(INVALID_AUTH_RESP, <<"AUTH|INVALID|SIG">>).
-define(EXPIRED_AUTH_RESP,
    <<"AUTH|dGVzdHVzZXJ8RElYWFhYWFhYWFhYWFhYWFhYWFh8MTMwMDE1Nzg3NA==|cb8f4d60ec7c261394cd5ee5a17e46ca7440d702">>
).
-define(WRONG_PARAMS_AUTH_RESP,
...
-define(WRONG_PARAMS_APP_RESP,
    <<"APP|dGVzdHVzZXJ8RElYWFhYWFhYWFhYWFhYWFhYWFh8MTYxNTcyNzI0M3xpbnZhbGlkZXh0cmFkYXRh|7c2065ea122d028b03ef0295a4b4c5521823b9b5">>
).
    ...
    [
        ...
        {"Verifying sign request with invalid akey",
            ?_assertEqual(
                'true',
                is_validation_error(
                    kz_mfa_duo:authenticate(
                        ?TEST_SIGN_CLAIM, ?WRONG_XKEY(<<"application_secret_key">>)
                    )
                )
            )},
        {"Verifying sign request without api_hostname",
            ?_assertEqual(
                ...
                is_validation_error(
                    kz_mfa_duo:authenticate(
                        ?TEST_SIGN_CLAIM, kz_json:delete_key(<<"api_hostname">>, ?TEST_CONFIG_JOBJ)
                    )
                )
            )},
        {"Verifying sign request with extra config variable",
            ?_assertEqual(
                ...
                is_challenge_resp(
                    kz_mfa_duo:authenticate(
                        ?TEST_SIGN_CLAIM,
                        kz_json:set_value(<<"happy_var">>, <<"happy_val">>, ?TEST_CONFIG_JOBJ)
                    )
                )
            )}
    ].
