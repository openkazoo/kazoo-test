...
-export([
    get_db/1, get_db/2,
    get_range_db/1, get_range_db/2,
    split_to_modbs/2,
    open_modb_doc/3,
    open_accountdb_doc/3,
    check_doc_type/3,
...
-spec get_range_db(kz_term:ne_binary()) ->
    {kz_time:gregorian_seconds(), kz_time:gregorian_seconds(), kz_term:ne_binaries()}.
...
%% @end
%%------------------------------------------------------------------------------
-spec get_range_db(kz_term:ne_binary(), pos_integer()) ->
    {kz_time:gregorian_seconds(), kz_time:gregorian_seconds(), kz_term:ne_binaries()}.
...
-spec check_msg_belonging(kz_term:api_ne_binary(), kz_json:object(), kz_term:api_ne_binary()) ->
    boolean().
check_msg_belonging(_BoxId, _JObj, 'undefined') ->
    'true';
check_msg_belonging('undefined', _JObj, _SourceId) ->
    'true';
check_msg_belonging(BoxId, _JObj, BoxId) ->
    'true';
check_msg_belonging(_BoxId, _JObj, _SourceId) ->
    lager:debug(
        "message ~s belongs to mailbox ~s but claims to belong to ~s",
...
retention_days(AccountId) ->
    case
        kapps_account_config:get_pos_integer(AccountId, ?VM_CONFIG_CAT, [
...
%% @end
%%------------------------------------------------------------------------------
-spec enforce_retention(kz_json:object(), kz_time:gregorian_seconds() | boolean()) ->
    kz_json:object().
enforce_retention(JObj, RetentionTimestamp) when
    is_integer(RetentionTimestamp)
->
...
is_prior_to_retention(JObj, RetentionTimestamp) ->
    TsTampPath = [
        <<"utc_seconds">>, <<"timestamp">>, [<<"metadata">>, <<"timestamp">>], <<"pvt_create">>
    ],
    MsgTstamp = kz_term:to_integer(kz_json:get_first_defined(TsTampPath, JObj, 0)),
    MsgTstamp =/= 0 andalso
        MsgTstamp < RetentionTimestamp.

%% @equiv get_change_vmbox_funs(AccountId, NewBoxId, NBoxJ, OldBoxId, 'undefined')
-spec get_change_vmbox_funs(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()
) ->
...
%% @end
%%------------------------------------------------------------------------------
-spec publish_saved_notify(
    kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call(), pos_integer(), kz_term:proplist()
) ->
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_transcribe(kapps_call:call(), kz_term:ne_binary(), boolean()) ->
    'undefined' | kazoo_speech:asr_resp().
...
maybe_transcribe(Call, MediaId, 'true') ->
    ...
    case asr_request:error(Req0) of
        'undefined' ->
            asr_request:transcription(Req0);
        Error ->
