...
%% @end
%%------------------------------------------------------------------------------
-spec new(Call, Options) -> Result when
...
new(Call, Options) ->
    ...
    case create_new_message_doc(Call, Options) of
        {'error', _} ->
            Msg = io_lib:format(
                "failed to create and save voicemail document for voicemail box ~s of account ~s",
        ...
        {'ok', MessageDoc} ->
            ...
            Msg = io_lib:format(
                "failed to store voicemail media ~s in voicemail box ~s of account ~s",
                [MessageId, BoxId, kapps_call:account_id(Call)]
            ),
            Funs = [
                {fun kapps_call:kvs_store/3, 'mailbox_id', BoxId},
            ...
            lager:debug("storing voicemail media recording ~s in doc ~s", [
                AttachmentName, MessageId
            ]),
            case
                store_recording(AttachmentName, MediaUrl, kapps_call:exec(Funs, Call), MessageId)
            of
                'ok' ->
...
-spec new_forward_message(
    kapps_call:call(), kz_json:object(), kz_term:ne_binary(), kz_term:proplist()
) -> new_msg_ret().
new_forward_message(Call, Metadata, SrcBoxId, Options) ->
    ...
    case create_forward_message_doc(Call, Metadata, SrcBoxId, Options) of
        {'error', _} ->
            Msg = io_lib:format(
                "failed to create and save voicemail document for forwarded message ~s to voicemail box ~s of account ~s",
                [kzd_box_message:media_id(Metadata), DestBoxId, kapps_call:account_id(Call)]
            ),
            {'error', Call, Msg};
        {ForwardId, MediaUrl} ->
            Msg = io_lib:format(
                "failed to store forward voicemail media ~s in voicemail box ~s of account ~s",
                [ForwardId, DestBoxId, kapps_call:account_id(Call)]
            ),
            Funs = [
                {fun kapps_call:kvs_store/3, 'dest_mailbox_id', DestBoxId},
                {fun kapps_call:kvs_store/3, 'attachment_name', AttachmentName},
                {fun kapps_call:kvs_store/3, 'media_id', ForwardId},
                {fun kapps_call:kvs_store/3, 'media_length', Length}
            ],

            lager:debug("storing forward voicemail media recording ~s in doc ~s", [
                AttachmentName, ForwardId
            ]),
            case
                store_recording(AttachmentName, MediaUrl, kapps_call:exec(Funs, Call), ForwardId)
            of
                'ok' ->
...
-spec fetch(AccountId, MessageId) -> db_ret() when
    AccountId :: kz_term:ne_binary(), MessageId :: kz_term:ne_binary().
...
%% @end
%%------------------------------------------------------------------------------
-spec fetch(AccountId, MessageId, BoxId) -> db_ret() when
...
-spec do_fetch(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_ne_binary(), kz_time:gregorian_seconds()
) -> {boolean(), db_ret()}.
...
-spec message(AccountId, MessageId) -> db_ret() when
    AccountId :: kz_term:ne_binary(), MessageId :: kz_term:ne_binary().
...
%% @end
%%------------------------------------------------------------------------------
-spec message(AccountId, MessageId, BoxId) -> db_ret() when
    AccountId :: kz_term:ne_binary(),
    MessageId :: kz_term:ne_binary(),
    BoxId :: kz_term:api_ne_binary().
...
%% @end
%%------------------------------------------------------------------------------
-spec set_folder(Folder, Message, AccountId) -> db_ret() when
    Folder :: kz_term:ne_binary(),
...
-spec maybe_set_folder(
    kz_term:ne_binary(), vm_folder(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()
) -> db_ret().
maybe_set_folder(_, ToFolder, MessageId, AccountId, _Msg) when
    ToFolder == ?VM_FOLDER_DELETED;
...
-spec change_folder(Folder, Message, AccountId, BoxId) -> db_ret() when
    Folder :: vm_folder(),
    Message :: message(),
    AccountId :: kz_term:ne_binary(),
...
change_folder(Folder, Message, AccountId, BoxId, Funs0) ->
    Funs = [
        fun(J) -> kzd_box_message:apply_folder(Folder, J) end
        | Funs0
    ],
...
%%------------------------------------------------------------------------------
-spec maybe_do_move(
    AccountId, MessageId, OldBoxId, NewBoxId, NewBoxJObj, Functions, RetenTimestamp
) ->
    db_ret()
when
    AccountId :: kz_term:ne_binary(),
...
do_move(AccountId, FromId, OldBoxId, NewBoxId, NBoxJ, Funs) ->
    ...
    lager:debug(
        "moving voicemail ~s/~s (vmbox ~s) to ~s/~s (vmbox ~s)",
        [FromDb, FromId, OldBoxId, ToDb, ToId, NewBoxId]
    ),

    Opts = [
        {'transform', fun(_, B) -> lists:foldl(fun(F, J) -> F(J) end, B, TransformFuns ++ Funs) end},
        {'max_retries', 3}
    ],
    case kazoo_modb:move_doc(FromDb, {kzd_box_message:type(), FromId}, ToDb, ToId, Opts) of
        ...
        {'error', 'conflict'} ->
            Msg = io_lib:format(
                "conflict occurred during moving voicemail ~s / ~s to ~s / ~s",
...
copy_to_vmboxes(AccountId, ?NE_BINARY = MessageId, OldBoxId, NewBoxIds, Funs) ->
    ...
    kz_json:from_list_recursive(
        maps:to_list(
            maybe_copy_to_vmboxes(
                AccountId, MessageId, OldBoxId, NewBoxIds, #{}, Funs, RetenTimestamp
            )
        )
    );
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_copy_to_vmboxes(
    AccountId, FromId, OldBoxId, NewBoxIds, Acc, Functions, RetenTimestamp
) ->
    bulk_map()
when
    AccountId :: kz_term:ne_binary(),
...
-spec copy_to_vmbox(
    ...
    kz_term:ne_binary(),
    bulk_map(),
    kz_datamgr:data_error() | {'ok', kz_json:object()},
    update_funs()
) ->
    bulk_map().
copy_to_vmbox(
...
-spec do_copy(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), update_funs()) ->
    db_ret().
do_copy(AccountId, ?NE_BINARY = FromId, ToId, Funs) ->
    ...
    lager:debug(
        "copying voicemail ~s/~s to ~s/~s",
        [FromDb, FromId, ToDb, ToId]
    ),

    Opts = [
        {'transform', fun(_, B) -> lists:foldl(fun(F, J) -> F(J) end, B, Funs) end},
        {'max_retries', 3}
    ],
    case kazoo_modb:copy_doc(FromDb, {kzd_box_message:type(), FromId}, ToDb, ToId, Opts) of
        ...
        {'error', 'conflict'} ->
            Msg = io_lib:format(
                "conflict occurred during forwarding voicemail ~s / ~s to ~s / ~s",
                [FromDb, FromId, ToDb, ToId]
            ),
            Subject = <<"Conflict during forward voicemail message">>,
            send_system_alert('undefined', AccountId, Subject, kz_term:to_binary(Msg)),
...
-spec move_copy_final_check(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    db_ret().
move_copy_final_check(AccountId, FromId, ToId) ->
    FromDb = kvm_util:get_db(AccountId, FromId),
    ToDb = kvm_util:get_db(AccountId, ToId),
    case fetch(AccountId, ToId) of
        %% message was saved somehow(network glitch?), moving on
        {'ok', _} = OK ->
            OK;
        {'error', _} ->
            lager:error(
                "max retries to copy or move voicemail message ~s/~s to ~s/~s",
                [FromDb, FromId, ToDb, ToId]
            ),
...
%% @end
%%------------------------------------------------------------------------------
-spec media_url(AccountId, Message) -> binary() when
    AccountId :: kz_term:ne_binary(), Message :: message().
...
create_new_message_doc(Call, Props) ->
    ...
    Metadata = kzd_box_message:build_metadata_object(
        Length, Call, kz_doc:id(JObj), CIDNumber, CIDName, Timestamp
    ),
...
-spec create_forward_message_doc(
    kapps_call:call(), kz_json:object(), kz_term:ne_binary(), kz_term:proplist()
) ->
...
create_forward_message_doc(Call, Metadata, SrcBoxId, Props) ->
    ...
    {_NewId, VMChangeFuns} = kvm_util:get_change_vmbox_funs(
    ...
    Updates = [
        fun(M) -> kz_json:set_value(<<"length">>, props:get_value(<<"Length">>, Props), M) end
        | VMChangeFuns
    ],
...
try_save_document(_Call, MsgJObj, 0) ->
    case fetch(kz_doc:account_id(MsgJObj), kz_doc:id(MsgJObj)) of
        ...
        {'error', _} ->
            lager:error(
                "max retries to save voicemail message ~s in db ~s",
...
try_save_document(Call, MsgJObj, Loop) ->
    case kazoo_modb:save_doc(kz_doc:account_db(MsgJObj), MsgJObj, [{'max_retries', 3}]) of
        ...
        {'error', _Reason} = Error ->
            lager:error(
                "failed to save voicemail message ~s in db ~s : ~p",
...
store_recording(AttachmentName, Url, Call, MessageId) ->
    case kapps_call_command:store_file(<<"/tmp/", AttachmentName/binary>>, Url, Call) of
        'ok' ->
            lager:debug("stored ~s to ~p", [AttachmentName, Url]);
        {'error', _R} ->
            lager:warning(
                "error during storing voicemail recording ~s , checking attachment existence: ~p", [
                    MessageId, _R
                ]
            ),
...
-spec check_attachment_exists(kapps_call:call(), kz_term:ne_binary()) ->
    ...
    case fetch(kapps_call:account_id(Call), MessageId) of
        {'ok', JObj} ->
            case kz_term:is_empty(kz_doc:attachments(JObj)) of
                ...
                'false' ->
                    lager:debug(
                        "freeswitch returned error during store voicemail recording, but attachments is saved anyway"
                    )
            end;
        {'error', _R} ->
...
-spec forward_to_vmbox(
    kapps_call:call(), kz_json:object(), kz_term:ne_binary(), kz_term:proplist(), update_funs()
) -> new_msg_ret().
...
prepend_forward_message(Call, ForwardId, Metadata, _SrcBoxId, Props) ->
    ...
    {'ok', _} = kz_datamgr:delete_attachment(
        kvm_util:get_db(AccountId, ForwardId), ForwardId, TmpAttachmentName
    ),
...
try_put_fwd_attachment(AccountId, ForwardId, _JoinFilename, _FileContents, _FileProps, 0) ->
    lager:error(
        "max retries to save prepend forward voicemail attachment ~s in db ~s",
...
try_put_fwd_attachment(AccountId, ForwardId, JoinFilename, FileContents, FileProps, Loop) ->
    case
        kz_datamgr:put_attachment(
            kvm_util:get_db(AccountId, ForwardId), ForwardId, JoinFilename, FileContents, FileProps
        )
    of
        {'ok', _} = OK ->
            OK;
        {'error', 'conflict'} ->
            try_put_fwd_attachment(
                AccountId, ForwardId, JoinFilename, FileContents, FileProps, Loop - 1
            );
        {'error', 'timeout'} ->
            try_put_fwd_attachment(
                AccountId, ForwardId, JoinFilename, FileContents, FileProps, Loop - 1
        ...
        {'error', _Reason} = Error ->
            lager:error(
                "failed to save prepend forward voicemail message ~s in db ~s : ~p",
                [ForwardId, kvm_util:get_db(AccountId, ForwardId), _Reason]
            ),
            Error
    end.

-spec write_attachment_to_file(kz_term:ne_binary(), kz_term:ne_binary()) ->
...
-spec write_attachment_to_file(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    {'ok', kz_term:ne_binary()} | {'error', any()}.
...
-spec notify_and_update_meta(kapps_call:call(), kz_term:ne_binary(), integer(), kz_term:proplist()) ->
    {'ok', kapps_call:call()}.
...
%% @end
%%------------------------------------------------------------------------------
-spec is_notified_successfully(
    kapps_call:call(), kz_term:ne_binary(), kz_json:objects(), notify_action()
) -> notify_action().
is_notified_successfully(Call, _MediaId, [], _) ->
    lager:debug(
        "failed to send new voicemail notification for message ~s in account ~s: timeout", [
            _MediaId, kapps_call:account_id(Call)
        ]
    ),
    'nothing';
...
-spec update_metadata(kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary(), update_funs()) ->
    {'ok', kapps_call:call()}.
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_retry_conflict(kapps_call:call() | 'undefined', kz_json:object(), fun(
    (kz_json:object()) -> db_ret()
)) -> db_ret().
...
-spec check_for_collision(kapps_call:call() | 'undefined', kz_json:object(), kz_json:object(), fun(
    (kz_json:object()) -> db_ret()
)) -> db_ret().
check_for_collision(Call, JObj, SavedJObj, DieAnotherDay) ->
    ...
    case kz_json:are_equal(OrigPublic, SavedPublic) of
        'true' ->
            Msg = io_lib:format(
                "saving new voicemail ~s in account ~p resulted in conflict but it saved to db anyway",
        ...
        'false' ->
            Msg = io_lib:format(
                "found document id collision during saving a new voicemail, id ~s account_id ~p",
                [kz_doc:id(JObj), kz_doc:account_id(JObj)]
            ),
            lager:critical(Msg),
