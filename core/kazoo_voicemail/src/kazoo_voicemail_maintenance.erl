...
migrate() ->
    ...
    receive
        'done' ->
            io:format("~nmigration finished~n");
        {'EXIT', Pid, 'normal'} ->
            io:format("~nmigration finished~n");
        {'EXIT', Pid, _Reason} ->
...
-spec migrate(kz_term:ne_binary(), kz_term:ne_binary() | kz_term:ne_binaries() | kz_json:object()) ->
    'ok'.
...
recover_messages_month(Year, Month) ->
    MODbs = [
        MODb
     || MODb <- kapps_util:get_all_account_mods(),
...
-spec recover_messages_account(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
recover_messages_account(Account, Year, Month) ->
    MODbs = [
        MODb
     || MODb <- kapps_util:get_account_mods(Account),
        modb_filter(MODb, Year, Month)
    ],
    recover_messages(MODbs, length(MODbs)).
...
recover_messages([MODb | MODbs], Total) ->
    ?SUP_LOG_DEBUG("(~p/~p) attempting to recover voicemail messages from ~s", [
        length(MODbs) + 1, Total, MODb
    ]),
...
recover_missing_metadata(MODb) ->
    ...
    case kz_datamgr:get_results(MODb, ?VIEW_MISSING_METADATA, ViewOptions) of
        {'ok', []} ->
            ?SUP_LOG_DEBUG("  no messages found with missing metadata", []);
        {'ok', Messages} ->
...
rebuild_message_metadata(JObj, AttachmentName) ->
    ...
    Routines = [
        {fun kapps_call:set_to/2, <<CIDNumber/binary, "@nodomain">>},
    ...
    Metadata = kzd_box_message:build_metadata_object(
        Length, Call, MediaId, CIDNumber, CIDName, Timestamp
    ),
...
renotify(Account, MessageId) ->
    ...
    case kz_datamgr:open_doc(MODb, MessageId) of
        {'error', _R} ->
            ?SUP_LOG_DEBUG("unable to find message ~s in ~s: ~p", [MODb, MessageId, _R]);
        {'ok', JObj} ->
...
-spec log_renotify_result(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_amqp_worker:request_return()
) -> 'ok'.
log_renotify_result(MessageId, BoxId, {'ok', JObj}) ->
    ?SUP_LOG_DEBUG(
        "re-notify sent message ~s from mailbox ~s: ~s",
        [MessageId, BoxId, kz_json:encode(JObj)]
    );
log_renotify_result(MessageId, BoxId, {'error', JObj}) ->
    ?SUP_LOG_DEBUG(
        "re-notify failed to send message ~s from mailbox ~s: ~s",
        [MessageId, BoxId, kz_json:encode(JObj)]
    );
log_renotify_result(MessageId, BoxId, {'timeout', JObjs}) ->
    ?SUP_LOG_DEBUG(
        "re-notify timed out sending message ~s from mailbox ~s: ~s",
        [MessageId, BoxId, kz_json:encode(JObjs)]
    );
log_renotify_result(MessageId, BoxId, Result) ->
    ?SUP_LOG_DEBUG(
        "unexpected error in re-notify sending message ~s from mailbox ~s: ~p",
        [MessageId, BoxId, Result]
    ).

-spec get_modb(kz_term:ne_binary(), kz_term:ne_binary()) -> kz_term:ne_binary().
get_modb(?MATCH_MODB_SUFFIX_ENCODED(_A, _B, _C, _Y, _M) = MODb, _) ->
    MODb;
get_modb(?MATCH_MODB_SUFFIX_encoded(_A, _B, _C, _Y, _M) = MODb, _) ->
    MODb;
...
rebuild_kapps_call(JObj, AccountId) ->
    ...
    Props = [
        {<<"Call-ID">>, kz_json:get_value(<<"call_id">>, Metadata, kz_binary:rand_hex(12))},
