...
-type workers() :: [worker()].
-record(state, {
    max_worker = ?MAX_PROCESS :: integer(),
    workers = [] :: workers(),
...
-type next_account_ret() ::
    {next_account() | 'account_hit_retention', queue:queue()}
    | 'empty'
    | 'retention_passed'
...
%% Workers API
-spec account_is_done(
    pid(), kz_term:ne_binary(), kz_time:gregorian_seconds(), kz_time:gregorian_seconds()
) -> 'ok'.
...
-spec account_maybe_failed(
    pid(), kz_term:ne_binary(), kz_time:gregorian_seconds(), kz_time:gregorian_seconds(), any()
) -> 'ok'.
...
handle_call(
    {'account_is_done', {_, _, _, Reason} = MaybeDone},
    _From,
    #state{
        account_queue = Queue,
...
handle_call(
    ...
    NewState = State#state{
        total_processed = TotalProcessed + CycleTotalProcessed,
...
handle_info({'EXIT', Pid, 'normal'}, #state{workers = Workers} = State) ->
    ...
    {'noreply', State#state{
        workers = props:delete(Pid, Workers),
        timer_ref = cleanup_account_timer()
    }};
...
maybe_spawn_worker(#state{account_ids = AccountIds} = State, 'retention_passed') ->
    ?SUP_LOG_WARNING(
        ":: all voicemails in retention duration are migrated, beginning a migrating older voicemails",
        []
    ),
    State#state{
        retention_passed = 'true',
...
maybe_spawn_worker(
    ...
    lager:debug(
        ":: started ~p (~b/~b) to process account ~s",
...
populate_queue(AccountIds, LastOfMonth) ->
    ...
    Props = [
        {AccountId, FirstOfMonth, LastOfMonth}
     || AccountId <- AccountIds
    ],
    queue:from_list(Props).
