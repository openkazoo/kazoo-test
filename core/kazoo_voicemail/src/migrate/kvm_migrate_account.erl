...
-define(MAX_BULK_INSERT,
    kapps_config:get_integer(
        ?VM_CONFIG_CAT,
        [?KEY_VOICEMAIL, <<"migrate_max_bulk_insert">>],
...
%% @end
%%------------------------------------------------------------------------------
-spec manual_vmbox_migrate(kz_term:ne_binary(), kz_term:ne_binary() | kz_term:ne_binaries()) ->
    kz_term:proplist().
manual_vmbox_migrate(Account, ?NE_BINARY = BoxId) ->
    manual_vmbox_migrate(Account, [BoxId]);
manual_vmbox_migrate(Account, BoxIds) ->
    AccountId = kz_util:format_account_id(Account),
    ?SUP_LOG_INFO(":: beginning migrating voicemails for ~b mailbox(es) in account ~s", [
        length(BoxIds), AccountId
    ]),
    migration_loop(#ctx{
        mode = <<"vmboxes">>,
        account_id = AccountId,
...
migration_loop(#ctx{account_id = _AccountId, retries = Retries, last_error = LastError} = Ctx) when
    Retries > 3
->
    Reason =
        case LastError of
...
handle_result(#ctx{account_id = _AccountId} = Ctx, {'ok', ViewResults}) ->
    ...
    NewCtx = reset_context_msgs(Ctx#ctx{
        total_msgs = MsgCount,
        last_error = 'undefined'
    }),
    maybe_next_cycle(migrate_messages(NewCtx, ViewResults));
handle_result(
    #ctx{account_id = _AccountId, account_db = AccountDb, retries = Retries} = Ctx,
    {'error', 'not_found'}
) when Retries > 3 ->
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_migrate(ctx(), kz_term:ne_binaries(), map(), kz_term:ne_binaries() | non_neg_integer()) ->
    ctx().
...
update_mailboxes(
    ...
    _
) when TotalMsgs == length(Failed) + length(NoModb) + NoIds ->
    ?SUP_LOG_ERROR("  [~s] bulk save failed for all docs: ~p", [_AccountId, Reason]),
    update_total_stats(Ctx#ctx{
        last_error = kz_term:to_binary(io_lib:format("bulk save failed: ~p", [Reason]))
    });
update_mailboxes(
    #ctx{
        account_id = _AccountId,
    ...
    case kz_datamgr:open_cache_docs(AccountDb, BoxIds) of
        {'ok', BoxJObjs} ->
            ...
            case kz_datamgr:save_docs(AccountDb, NewBoxJObjs) of
                {'ok', _} ->
                    update_total_stats(Ctx);
                {'error', _Reason} ->
                    ?SUP_LOG_ERROR("  [~s] failed to save new message array into mailboxes: ~p", [
                        _AccountId, _Reason
                    ]),
                    reset_context_msgs(Ctx#ctx{
                        last_error = kz_term:to_binary(
                            io_lib:format("update vmbox failed: ~p", [_Reason])
                        )
                    })
            end;
        {'error', _Reason} ->
            ...
            reset_context_msgs(Ctx#ctx{
                last_error = kz_term:to_binary(io_lib:format("open vmbox failed: ~p", [_Reason]))
            })
    end.
...
-spec update_vmbox_message(
    kz_json:object(), sets:set(), map(), map(), kz_term:binary(), kz_time:api_seconds()
) -> kz_term:api_object().
...
get_messages_from_vmboxes(AccountDb, ExpectedBoxIds) ->
    case kz_datamgr:open_cache_docs(AccountDb, ExpectedBoxIds) of
        ...
        {'error', _E} = Error ->
            ?SUP_LOG_ERROR("  [~s] failed to open mailbox(es)", [
...
check_dbs_existence(Ctx, [], MsgMap) ->
    {Ctx, MsgMap};
check_dbs_existence(Ctx, [Db | Dbs], MsgMap) ->
    case kz_datamgr:db_exists(Db) of
        ...
        'false' ->
            ...
            check_dbs_existence(
                ctx_field_update(Ctx, 'no_modb', [kz_doc:id(Msg) || Msg <- maps:get(Db, MsgMap)]),
                Dbs,
...
normalize_bulk_result(#ctx{account_id = _AccountId} = Ctx, Db, [S | Saved]) ->
    ...
    case kz_json:get_value(<<"error">>, S) of
        ...
        <<"conflict">> ->
            ...
            normalize_bulk_result(ctx_field_update(Ctx, 'succeeded', [Id]), Db, Saved);
        Reason ->
            lager:debug("  [~s] failed to save voice mail message ~s in db ~s: ~p", [
                _AccountId, Id, Db, Reason
            ]),
...
process_messages(Ctx0, JObjs) ->
    {Ctx1, BoxSet, MsgMap0, NeedTimestamp} = lists:foldl(
        fun check_create_and_map/2, {Ctx0, sets:new(), #{}, []}, JObjs
    ),
    {Ctx2, TimestampUpdated} = maybe_get_timestamp(Ctx1, NeedTimestamp),
    {Ctx3, MsgMap1} = lists:foldl(
        fun(J, Acc) -> create_and_map(J, Acc) end, {Ctx2, MsgMap0}, TimestampUpdated
    ),
...
check_create_and_map(JObj, {Ctx, BoxSet, MsgMap, NoTimestamp}) ->
    ...
    case
        MediaId =/= 'undefined' andalso
            kz_json:get_integer_value([<<"metadata">>, <<"timestamp">>], Value)
    of
        'false' ->
...
create_and_map(JObj, {Ctx, MsgMap}) ->
    ...
    {Ctx, maps:update_with(kz_doc:account_db(Doc), fun(List) -> [Doc | List] end, [Doc], MsgMap)}.
...
maybe_get_timestamp(
    ...
    case kz_datamgr:open_docs(AccountDb, maps:keys(IdsMap)) of
        ...
        {'error', _Reason} ->
            ?SUP_LOG_ERROR(
                "  [~s] failed to open private media to find timestamp, setting vmbox/current_time as timestamp: ~p",
                [_AccountId, _Reason]
            ),
...
-spec all_no_timestamp(kz_term:ne_binary(), kz_json:object(), {
    kz_term:proplist(), kz_json:objects()
}) -> {kz_term:proplist(), kz_json:objects()}.
all_no_timestamp(Id, JObj, {NoTime, JAcc}) ->
    Timestamp = kz_json:get_integer_value(<<"box_timestamp">>, JObj, kz_time:now_s()),
    {[{Id, Timestamp} | NoTime], [
        kz_json:set_value([<<"metadata">>, <<"timestamp">>], Timestamp, JObj) | JAcc
    ]}.
...
get_timestamp(Ctx, IdsMap, PrivateMedias) ->
    Fun = fun(PrivateMedia, {TimeMap, Messages}) ->
        ...
        {[{Id, Timestamp} | TimeMap], [
            kz_json:set_value([<<"metadata">>, <<"timestamp">>], Timestamp, Message) | Messages
        ]}
    end,
...
update_total_stats(
    ...
    ?SUP_LOG_INFO(
        "  [~s] finished a cycle, processed: ~b succeeded: ~b save_failed: ~b no_modb: ~b fix_timestamp: ~b no_ids: ~b",
        [_AccountId, TotalMsgs, CurrSucceeded, CurrFailed, CurrNoModb, CurrNoTime, CurrNoIds]
    ),

    Ctx#ctx{
        total_stats = TotalStats#total_stats{
            total_processed = TotalMsgs + TotalProcessed,
...
total_stats_to_prop(#total_stats{
