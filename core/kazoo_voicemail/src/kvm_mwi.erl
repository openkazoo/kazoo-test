...
-spec maybe_send_unsolicited_mwi_update(
    kz_json:object(), kz_term:ne_binary(), integer(), integer()
) -> 'ok'.
maybe_send_unsolicited_mwi_update(JObj, AccountId, New, Saved) ->
    ...
    case
        <<"password">> =:= kzd_devices:sip_method(J) andalso
            'undefined' =/= Username andalso
            'undefined' =/= Realm andalso
            'undefined' =/= OwnerId andalso
            kzd_devices:mwi_unsolicited_updates(J)
    of
...
unsolicited_endpoint_mwi_update(AccountDb, EndpointId, 'true') ->
    case kz_datamgr:open_cache_doc(AccountDb, EndpointId) of
        {'error', _Error} ->
            lager:error("opening endpoint document ~s from db ~s", [EndpointId, AccountDb]);
        {'ok', JObj} ->
...
maybe_send_endpoint_mwi_update(AccountDb, JObj, 'true') ->
    ...
    case
        <<"password">> =:= kzd_devices:sip_method(JObj) andalso
            'undefined' =/= Username andalso
            'undefined' =/= Realm
    of
        'false' ->
...
-spec send_unsolicited_mwi_update(vm_count(), vm_count(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
send_unsolicited_mwi_update(New, Saved, Username, Realm) ->
    send_unsolicited_mwi_update(New, Saved, Username, Realm, kz_json:new()).

-spec send_unsolicited_mwi_update(
    vm_count(), vm_count(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()
) -> 'ok'.
...
is_unsolicited_mwi_enabled(AccountId) ->
    kapps_config:get_is_true(?VM_CONFIG_CAT, ?MWI_SEND_UNSOLICITED_UPDATES, 'true') andalso
        kz_term:is_true(
            kapps_account_config:get(
                AccountId, ?VM_CONFIG_CAT, ?MWI_SEND_UNSOLICITED_UPDATES, 'true'
            )
        ).

-spec vm_count_by_owner(kz_term:ne_binary(), kz_term:api_binary()) ->
    {non_neg_integer(), non_neg_integer()}.
vm_count_by_owner(_AccountDb, 'undefined') ->
    {0, 0};
