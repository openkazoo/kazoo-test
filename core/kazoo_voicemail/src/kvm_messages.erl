...
-export([
    count/1,
    count_per_folder/1, count_per_folder/2,
    count/2,
    count_by_owner/2,
    count_non_deleted/2
]).
...
get(AccountId) ->
    ...
    normalize_account_listing(
        get_view_results(AccountId, ?MSG_LISTING_BY_TIMESTAMP, ViewOpts, 'undefined')
    ).
...
%% @end
%%------------------------------------------------------------------------------
-spec get(AccountId, BoxId) -> kz_json:objects() when
...
%% @end
%%------------------------------------------------------------------------------
-spec count(AccountId, BoxId) -> non_neg_integer() when
    AccountId :: kz_term:ne_binary(), BoxId :: kz_term:ne_binary().
...
%% @end
%%------------------------------------------------------------------------------
-spec count_non_deleted(AccountId, BoxId) -> Count when
    AccountId :: kz_term:ne_binary(),
...
count_non_deleted(AccountId, BoxId) ->
    ...
    {maps:get(<<"new">>, VMBoxCount, 0), maps:get(<<"saved">>, VMBoxCount, 0)}.
...
%% @end
%%------------------------------------------------------------------------------
-spec count_by_owner(AccountId, OwnerId) -> Count when
    AccountId :: kz_term:ne_binary(),
...
-spec sum_owner_mailboxes(kz_term:ne_binary(), kz_term:ne_binaries(), non_deleted_tuple()) ->
    non_deleted_tuple().
sum_owner_mailboxes(_, [], Results) ->
    Results;
sum_owner_mailboxes(AccountId, [BoxId | BoxIds], {New, Saved}) ->
    {BoxNew, BoxSaved} = count_non_deleted(AccountId, BoxId),
    lager:debug(
        "adding mailbox ~s with ~p new and ~p saved messages to user's quantities",
...
%% @end
%%------------------------------------------------------------------------------
-spec count_per_folder(AccountId) -> Count when
    AccountId :: kz_term:ne_binary(), Count :: count_map().
count_per_folder(AccountId) ->
...
%% @end
%%------------------------------------------------------------------------------
-spec count_per_folder(AccountId, BoxId) -> Count when
    AccountId :: kz_term:ne_binary(),
    BoxId :: kz_term:ne_binary(),
    Count :: count_map().
...
%% @end
%%------------------------------------------------------------------------------
-spec count_per_folder(
    kz_term:ne_binary(), kz_term:ne_binaries(), kz_term:ne_binary(), kz_term:ne_binaries()
) -> count_map().
count_per_folder(AccountId, Keys, View, Folders) ->
    lists:foldl(
        fun(Folder, ResultMap) ->
...
-spec do_update(kz_term:ne_binary(), bulk_map(), update_funs(), kz_time:gregorian_seconds()) ->
    kz_json:object().
do_update(AccountId, FetchMap, Funs, RetenTimestamp) ->
    #{
...
-spec fetch(
    kz_term:ne_binary(), kz_term:ne_binaries(), kz_term:api_ne_binary(), kz_time:gregorian_seconds()
) -> bulk_map().
...
fetch_fun(Db, BoxId, Ids, ResultMap, RetenTimestamp) ->
    case
        kz_datamgr:db_exists(Db) andalso
            kz_datamgr:open_docs(Db, Ids)
    of
        'false' ->
...
-spec fetch_faild_with_reason(any(), kz_term:ne_binary(), kz_term:ne_binaries(), bulk_map()) ->
    bulk_map().
...
do_move(AccountId, [FromId | FromIds], OldboxId, NewBoxId, NBoxJ, ResultMap, Funs, RetenTimestamp) ->
    case
        kvm_message:maybe_do_move(
            AccountId, FromId, OldboxId, NewBoxId, NBoxJ, Funs, RetenTimestamp
        )
    of
        {'ok', Moved} ->
            NewMap = maps:update_with(
                succeeded, fun(List) -> [kz_doc:id(Moved) | List] end, [kz_doc:id(Moved)], ResultMap
            ),
            do_move(AccountId, FromIds, OldboxId, NewBoxId, NBoxJ, NewMap, Funs, RetenTimestamp);
        {'error', Reason} ->
            IdReason = {FromId, kz_term:to_binary(Reason)},
            NewMap = maps:update_with(
                failed, fun(List) -> [IdReason | List] end, [IdReason], ResultMap
            ),
            do_move(AccountId, FromIds, OldboxId, NewBoxId, NBoxJ, NewMap, Funs, RetenTimestamp)
    end.
...
copy_to_vmboxes(AccountId, Ids, OldBoxId, NewBoxIds, Funs) ->
    ...
    bulk_result(
        lists:foldl(
            fun(Id, AccMap) ->
                kvm_message:maybe_copy_to_vmboxes(
                    AccountId, Id, OldBoxId, NewBoxIds, AccMap, Funs, RetenTimestamp
                )
            end,
            #{},
            Ids
...
%% @end
%%------------------------------------------------------------------------------
-spec get_view_results(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist(), norm_fun()) ->
    kz_json:objects().
...
-spec get_view_results(
    kz_term:ne_binaries(), kz_term:ne_binary(), kz_term:proplist(), norm_fun(), kz_json:objects()
) -> kz_json:objects().
...
get_view_results([Db | Dbs], View, ViewOpts, NormFun, Acc) ->
    case kazoo_modb:get_results(Db, View, ViewOpts) of
        ...
        {'ok', Msgs} ->
            get_view_results(Dbs, View, ViewOpts, NormFun, Acc ++ Msgs);
        {'error', _} = _E ->
            lager:debug(
                "failed to get voicemail message ~s view results from db ~s with ViewOpts ~s",
                [View, Db, ViewOpts]
            ),
            get_view_results(Dbs, View, ViewOpts, NormFun, Acc)
    end.

-spec maybe_add_range_to_keys(
    kz_time:gregorian_seconds(), kz_time:gregorian_seconds(), kz_term:proplist()
) -> kz_term:proplist().
...
-spec add_timestamp_if_defined(
    'startkey' | 'endkey', kz_time:gregorian_seconds(), kz_term:proplist()
) -> kz_json:path().
...
normalize_account_listing(JObj, Map) ->
    ...
    case kz_json:get_value([<<"key">>, ?LISTING_BY_TIMESTAMP_BOX_ID_KEY_INDEX], JObj) of
        'undefined' -> Map;
        BoxId -> maps:update_with(BoxId, fun(List) -> List ++ [Value] end, [Value], Map)
    end.
...
normalize_bulk_results(ResultMap, [], _Method, _BoxId, _) ->
    lager:debug(
        "voicemail ~s bulk for mailbox ~s resulted in ~b succeeded and ~b failed docs",
        [
            _Method,
            _BoxId,
...
normalize_bulk_results(
    ...
    NewMap =
        case
            kvm_util:check_msg_belonging(BoxId, JObj) andalso
                        ...
                        ResultMap#{
                            succeeded => [
                                kvm_util:enforce_retention(
                                    kz_json:get_value(<<"doc">>, JObj), 'true'
                                )
                                | Succeeded
                            ]
                        };
...
normalize_count(?MSG_COUNT_PER_BOX_FOLDER, ViewResults, ResultMap) ->
    lists:foldl(
        fun(JObj, Map) ->
            sum_per_folder(
                JObj,
                ?PER_BOX_FOLDER_VIEW_FOLDER_KEY_INDEX,
                ?PER_BOX_FOLDER_VIEW_BOX_ID_KEY_INDEX,
                Map
            )
        end,
        ResultMap,
...
normalize_count(?MSG_COUNT_PER_FOLDER, ViewResults, ResultMap) ->
    lists:foldl(
        fun(JObj, Map) ->
            sum_per_folder(
                JObj, ?PER_FOLDER_VIEW_FOLDER_KEY_INDEX, ?PER_FOLDER_VIEW_BOX_ID_KEY_INDEX, Map
            )
        end,
        ResultMap,
        ViewResults
...
-spec sum_per_folder(kz_json:object(), non_neg_integer(), non_neg_integer(), count_map()) ->
    count_map().
...
%% @end
%%------------------------------------------------------------------------------
-spec split_to_modbs_and_apply_funs(
    bulk_map(), kz_term:ne_binary(), update_funs(), kz_time:gregorian_seconds()
) -> bulk_map().
split_to_modbs_and_apply_funs(FetchMap, AccountId, Funs, RetenTimestamp) ->
...
-spec split_to_modbs_and_apply_funs(
    bulk_map(), kz_term:ne_binary(), kz_json:objects(), update_funs(), kz_time:gregorian_seconds()
) -> bulk_map().
split_to_modbs_and_apply_funs(SplitMap, _AccountId, [], _Funs, _RetenTimestamp) ->
    SplitMap;
split_to_modbs_and_apply_funs(
    #{
        to_update_map := ToUpdateMap,
        enforce_set := EnforceSet
    } = SplitMap,
    ...
    case kvm_util:is_prior_to_retention(JObj, RetenTimestamp) of
        'true' ->
            ...
            NewMap = SplitMap#{
                to_update_map => maps:update_with(
                    Db, fun(List) -> [NewJObj | List] end, [NewJObj], ToUpdateMap
                ),
        ...
        'false' ->
            ...
            NewToUpdateMap = maps:update_with(
                Db, fun(List) -> [NewJObj | List] end, [NewJObj], ToUpdateMap
            ),
            split_to_modbs_and_apply_funs(
                SplitMap#{to_update_map => NewToUpdateMap}, AccountId, JObjs, Funs, RetenTimestamp
            )
    end.
