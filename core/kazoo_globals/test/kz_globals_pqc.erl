...
next_state(
    ...
    case name_exists(Name, Registries) of
        ...
        'false' ->
            maps:fold(
                fun
                    (Node, Registry, Acc) when Remote =:= Node ->
...
next_state(
    ...
    case lists:keysearch(Name, 1, Registry) of
        'false' -> Registries;
        {'value', ?REG(Name, _P, 'local')} -> unregister_name(Name, Registries);
        {'value', _} -> Registries
    end;
...
next_state(
    ...
    case name_exists(Name, Registries) of
        ...
        'false' ->
            maps:fold(
                fun
                    (Node, Registry, Acc) when ?LOCAL_NODE =:= Node ->
                        Acc#{Node => [?REG(Name, Pid, 'local') | Registry]};
                    (Node, Registry, Acc) ->
                        Acc#{Node => [?REG(Name, Pid, 'remote') | Registry]}
                end,
...
next_state(
    #{?LOCAL_NODE := LocalRegistry} = Registries,
    _V,
    {'call', 'kz_globals', 'unregister_name', [Name]}
) ->
    case lists:keysearch(Name, 1, LocalRegistry) of
        'false' -> Registries;
        {'value', ?REG(Name, _Pid, 'local')} -> unregister_name(Name, Registries);
