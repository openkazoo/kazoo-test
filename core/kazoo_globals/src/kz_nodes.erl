...
-record(state, {
    heartbeat_ref = erlang:make_ref() :: reference(),
    tab :: ets:tid(),
...
node_to_json(#kz_node{
    ...
    kz_json:from_list([
        {<<"node">>, kz_term:to_binary(NodeName)},
...
determine_whapp_count(Whapp, MatchSpec) ->
    lists:foldl(
        fun(Whapps, Acc) when is_list(Whapps) ->
...
-spec determine_whapp_count_fold(kz_types:kapps_info(), non_neg_integer(), kz_term:ne_binary()) ->
    non_neg_integer().
determine_whapp_count_fold(Whapps, Acc, Whapp) ->
...
determine_whapp_zones(Whapp, MatchSpec) ->
    {Whapp, Zones, _} =
        lists:foldl(
            fun determine_whapp_zones_fold/2,
            {Whapp, [], 0},
            ets:select(?MODULE, MatchSpec)
        ),
    Zones.
...
-spec determine_whapp_zones_fold({atom(), kz_types:kapps_info()}, fold_zones_acc()) ->
    fold_zones_acc().
determine_whapp_zones_fold({Zone, Whapps}, {Whapp, Zones, C} = Acc) ->
    case
        props:is_defined(Whapp, Whapps) andalso
...
node_status_to_json(
    ...
    kz_json:set_value(
        [kz_term:to_binary(NodeZone), NodeType, kz_term:to_binary(N)],
...
-spec check_node_versions(kz_types:kz_nodes(), {kz_term:ne_binary(), node(), kz_term:ne_binary()}) ->
    'ok'.
check_node_versions([], _) ->
    'ok';
check_node_versions(
    [
        #kz_node{
    ...
    CheckAgainst =
        case binary:split(kz_term:to_binary(N), <<"@">>) of
            ...
            [Name, _Host] when Vsn > Version ->
                lager:warning(
                    "node ~s is running a newer version (~s) than ~s (~s)",
                    [N, Vsn, Node, Version]
                ),
                Current;
            [Name, _Host] ->
                lager:warning(
                    "node ~s is running an older version (~s) than ~s (~s)",
                    [N, Vsn, Node, Version]
                ),
                Current;
            [NewName, _Host] ->
...
maybe_print_row('true', Args) -> print_simple_row(Args).
...
maybe_print_md5(MD5) -> print_simple_row_str([<<"md5">>, MD5]).
...
print_media_server({Name, JObj}, Format) ->
    io:format(
        lists:flatten([Format, ?MEDIA_SERVERS_DETAIL, "~n"]),
        [
            Name,
...
maybe_print_node_info('undefined') ->
    'ok';
...
status_list([{Whapp, #whapp_info{startup = Started, roles = []}} | Whapps], Column) ->
    ...
    Print = <<
        (kz_term:to_binary(Whapp))/binary,
        "(",
        (kz_time:pretty_print_elapsed_s(Elapsed))/binary,
        ")"
    >>,
    io:format("~-25s", [Print]),
    status_list(Whapps, Column + 1);
...
init([]) ->
    ...
    Version = list_to_binary([
        kz_util:kazoo_version(),
        " - ",
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_call(any(), kz_term:pid_ref(), nodes_state()) ->
    kz_types:handle_call_ret_state(nodes_state()).
...
handle_info(
    ...
    {'noreply', State#state{
        notify_new = sets:del_element(Pid, NewSet),
...
create_node(Heartbeat, #state{
    zone = Zone,
    version = Version,
    md5 = MD5
}) ->
    add_kapps_data(#kz_node{
        expires = Heartbeat,
...
normalize_amqp_uri(URI) -> kz_term:to_binary(amqp_uri:remove_credentials(kz_term:to_list(URI))).
...
kapp_data(
    ...
    Node#kz_node{
        kapps = maybe_add_info(App, props:get_value('info', RequestAcc), Kapps),
...
-spec maybe_add_info(atom(), 'undefined' | kz_types:whapp_info(), kz_types:kapps_info()) ->
    kz_types:kapps_info().
maybe_add_info(_App, 'undefined', Kapps) -> Kapps;
maybe_add_info(App, AppInfo, Kapps) -> [{kz_term:to_binary(App), AppInfo} | Kapps].

-spec get_whapp_info(atom() | pid() | kz_term:proplist() | 'undefined') -> kz_types:whapp_info().
get_whapp_info('undefined') ->
    #whapp_info{};
...
get_whapp_process_info('undefined') ->
    #whapp_info{};
get_whapp_process_info([]) ->
    #whapp_info{};
...
media_servers_to_json(Servers) -> kz_json:from_list(Servers).
...
from_json(JObj, State) ->
    ...
    #kz_node{
        node = kz_term:to_atom(Node, 'true'),
        md5 = kz_json:get_value(<<"md5">>, JObj),
        expires = kz_term:to_integer(
            kz_json:get_integer_value(<<"Expires">>, JObj, 0) * ?FUDGE_FACTOR
        ),
        kapps = kapps_from_json(kz_json:get_value(<<"WhApps">>, JObj, [])),
        media_servers = media_servers_from_json(
            kz_json:get_value(<<"Media-Servers">>, JObj, kz_json:new())
        ),
        ...
        version = kz_json:get_first_defined(
            [<<"Version">>, <<"App-Version">>], JObj, <<"unknown">>
        ),
...
-spec whapp_info_from_json(kz_term:ne_binary(), kz_types:whapp_info(), {
    kz_json:json_terms(), kz_json:keys()
}) -> kz_types:whapp_info().
whapp_info_from_json(_Key, Info, {[], []}) ->
    Info;
...
whapp_info_from_json(<<"kamailio">> = Key, Info, {[V | V1], [<<"Startup">> | K1]}) ->
    whapp_info_from_json(
        Key, Info#whapp_info{startup = kz_time:unix_seconds_to_gregorian_seconds(V)}, {V1, K1}
    );
...
-spec whapp_to_json({kz_term:ne_binary(), kz_types:whapp_info()}) ->
...
notify_expire([#kz_node{node = NodeName} = Node | Nodes], Pids) ->
    ...
    _ = [
        gen_listener:cast(Pid, {?MODULE, {'expire', Node}})
     || Pid <- Pids
    ],
...
notify_new(#kz_node{node = NodeName} = Node, Pids) ->
    ...
    _ = [
        gen_listener:cast(Pid, {?MODULE, {'new', Node}})
     || Pid <- Pids
    ],
    'ok'.
...
-spec whapp_oldest_node(kz_term:text(), kz_term:text() | boolean() | atom()) ->
    kz_term:api_integer().
whapp_oldest_node(Whapp, Federated) when
    is_binary(Federated)
->
...
whapp_oldest_node(Whapp, Zone) when
    is_atom(Zone)
->
    MatchSpec = [
        {
...
oldest_whapp_node(Whapp, MatchSpec) ->
    lists:foldl(
        fun({Whapps, _Node} = Info, Acc) when is_list(Whapps) ->
...
-spec determine_whapp_oldest_node_fold(
    {kz_types:kapps_info(), node()}, oldest_whapp_node(), kz_term:ne_binary()
) ->
    oldest_whapp_node().
...
determine_whapp_oldest_node_fold({Whapps, Node}, {_, Startup} = Acc, Whapp) ->
    case props:get_value(Whapp, Whapps) of
        'undefined' ->
            Acc;
        #whapp_info{startup = 'undefined'} ->
            Acc;
        #whapp_info{startup = Start} when
            Start =< Startup
        ->
            {Node, Start};
        _ ->
...
maybe_add_zone(#kz_node{zone = 'undefined'}, #state{} = State) ->
    State;
...
-spec amqp_status_connection(kz_amqp_connections:kz_amqp_connections()) ->
    {kz_term:ne_binary(), kz_json:object()}.
...
-spec pool_state(atom(), atom(), integer(), integer(), integer()) ->
...
-spec whapp_role_count(kz_term:text(), kz_term:text(), kz_term:text() | boolean() | 'remote') ->
    integer().
...
-spec determine_whapp_role_count(kz_term:ne_binary(), kz_term:ne_binary(), ets:match_spec()) ->
    non_neg_integer().
...
-spec determine_whapp_role_count_fold(
    kz_types:kapps_info(), kz_term:ne_binary(), non_neg_integer(), kz_term:ne_binary()
) -> non_neg_integer().
determine_whapp_role_count_fold(Whapps, Role, Acc, Whapp) ->
    case
        props:is_defined(Whapp, Whapps) andalso
            lists:member(Role, (props:get_value(Whapp, Whapps))#whapp_info.roles)
    of
        'true' -> Acc + 1;
        'false' -> Acc
    end.
...
with_role_filter(Role, MatchSpec) ->
    lists:foldl(
        fun(#kz_node{roles = Roles} = Node, Acc) when is_list(Roles) ->
