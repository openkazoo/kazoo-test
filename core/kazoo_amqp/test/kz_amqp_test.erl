...
loop_publisher(Payload, Timeout) ->
    ...
    receive
        'stop' ->
            lager:info("publisher instructed to stop");
...
loop_consumer(Queue, Timeout) ->
    ...
    receive
        'stop' ->
            lager:info("consumer stopping");
        {#'basic.deliver'{} = _Deliver, #amqp_msg{props = #'P_basic'{}, payload = Payload}} ->
...
decr(Timeout, Start) -> kz_time:decr_timeout(Timeout, Start).
