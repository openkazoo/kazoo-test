...
-record(state, {
    ...
    % [{From, TRef}]
    consuming_requests = [] :: [{kz_term:pid_ref(), reference()}],
    %% {{EvtCat, EvtName}, Module}
    responders = [] :: listener_utils:responders(),
    %% {authentication, [{key, value},...]}
    bindings = [] :: bindings(),
    params = [] :: kz_term:proplist(),
    module :: atom(),
    module_state :: module_state(),
    % when the client sets a timeout, gen_listener calls shouldn't negate it, only calls that pass through to the client
    module_timeout_ref :: kz_term:api_reference(),
    %% {QueueName, {Bindings, QueueProps}}
    other_queues = [] :: [{kz_term:ne_binary(), {bindings(), kz_term:proplist()}}],
...
-callback handle_event(kz_json:object(), kz_term:proplist(), module_state()) ->
    handle_event_return().
...
start_link(Module, Params, InitArgs) when
    is_atom(Module),
    is_list(Params),
    is_list(InitArgs)
->
...
-spec start_link(
    kz_types:gen_server_name() | atom(),
    atom() | start_params(),
    start_params() | list(),
    kz_types:gen_server_options() | list()
) -> kz_types:startlink_ret().
start_link(Module, Params, InitArgs, Options) when
    is_atom(Module),
    is_list(Params),
    is_list(InitArgs),
    is_list(Options)
...
start_link(Name, Module, Params, InitArgs) when
    is_atom(Module),
    is_list(Params),
    is_list(InitArgs)
->
...
-spec start_link(
    kz_types:gen_server_name(), atom(), start_params(), list(), kz_types:gen_server_options()
) -> kz_types:startlink_ret().
start_link(Name, Module, Params, InitArgs, Options) when
    is_atom(Module),
    is_list(Params),
    is_list(InitArgs),
    is_list(Options)
...
enter_loop(Module, Options, ModuleState, {Scope, _Name} = ServerName) when
    Scope =:= 'local' orelse
        Scope =:= 'global'
->
...
-spec add_responder(
    kz_types:server_ref(),
    responder_callback(),
    responder_callback_mapping() | responder_callback_mappings()
) -> 'ok'.
...
-spec rm_responder(kz_types:server_ref(), responder_callback(), responder_callback_mappings()) ->
    'ok'.
...
add_binding(Srv, {Binding, Props}) when
    is_list(Props) andalso
        (is_atom(Binding) orelse
            is_binary(Binding))
->
    gen_server:cast(Srv, {'add_binding', kz_term:to_binary(Binding), Props});
add_binding(Srv, Binding) when
    is_binary(Binding) orelse
        is_atom(Binding)
->
...
b_add_binding(Srv, {Binding, Props}) when
    is_list(Props),
    (is_atom(Binding) orelse
        is_binary(Binding))
->
    gen_server:call(Srv, {'add_binding', kz_term:to_binary(Binding), Props});
b_add_binding(Srv, Binding) when
    is_binary(Binding) orelse
        is_atom(Binding)
->
    gen_server:call(Srv, {'add_binding', kz_term:to_binary(Binding), []}).

-spec b_add_binding(kz_types:server_ref(), kz_term:ne_binary() | atom(), kz_term:proplist()) ->
    'ok'.
b_add_binding(Srv, Binding, Props) when
    is_binary(Binding) orelse
        is_atom(Binding)
...
b_rm_binding(Srv, {Binding, Props}) when
    is_list(Props),
    (is_atom(Binding) orelse
        is_binary(Binding))
->
    gen_server:call(Srv, {'rm_binding', kz_term:to_binary(Binding), Props});
b_rm_binding(Srv, Binding) when
    is_binary(Binding) orelse
        is_atom(Binding)
->
...
init([Module, Params, InitArgs]) ->
    ...
    lager:debug(
        "starting new gen_listener proc : ~s ~s",
        [Application, Module]
    ),
    case
        erlang:function_exported(Module, 'init', 1) andalso
...
maybe_set_queue_name(Module, Params) ->
    case
        props:is_false('queue_name_always_generate', Params, 'true') andalso
            props:get_value('queue_name', Params) =:= <<>>
    of
        'false' -> Params;
...
handle_cast({'rm_queue', QueueName}, #state{other_queues = OtherQueues} = State) ->
    _ = [
        remove_binding(Binding, Props, QueueName)
...
handle_cast({'federated_event', JObj, Props}, #state{params = Params} = State) ->
    ...
    case props:is_true('spawn_handle_event', Params, 'false') of
        'true' ->
            ...
            {'noreply', State};
        'false' ->
            distribute_event(JObj, {Deliver, Basic}, State)
    end;
handle_cast(
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'$execute', Module, Function, Args} = Msg,
    #state{federators = Federators} = State
) ->
    erlang:apply(Module, Function, Args),
    _ = [
        ?MODULE:cast(Federator, Msg)
...
handle_cast(
    ...
    _ = [
        start_consumer(
            Q1, maybe_configure_auto_ack(props:get_value('consume_options', P, []), AutoAck)
        )
     || {Q1, {_, P}} <- OtherQueues
    ],
    {'noreply', State};
handle_cast(
    {'federator_is_consuming', Broker, 'true'},
    ...
    case lists:subtract(Waiting, [Broker]) of
        [] ->
            lager:info("federator for ~s is consuming, all waiting federators are available!", [
                Broker
            ]),
            handle_module_cast({?MODULE, {'federators_consuming', 'true'}}, State);
        Remaining ->
            lager:info("federator for ~s is consuming, still waiting for federators: ~p", [
                Broker, Remaining
            ]),
...
handle_cast({'federator_listener', {Broker, Pid}}, #state{federators = Fs} = State) ->
    case lists:keytake(Pid, 2, Fs) of
        ...
        {'value', _FL, _Feds} ->
            lager:debug(
                "ignoring federator_listener message about ~p; have ~p already",
                [Pid, _FL]
            ),
            {'noreply', State}
    end;
...
-spec maybe_remove_binding(binding(), binding_module(), kz_term:proplist(), kz_term:ne_binary()) ->
    boolean().
maybe_remove_binding({B, P}, B, P, Q) ->
    lager:debug("removing ~s: ~p", [B, P]),
    remove_binding(B, P, Q),
    'false';
maybe_remove_binding(_BP, _B, _P, _Q) ->
    'true'.
...
handle_info({'$channel_flow', Active}, State) ->
    ...
    {'noreply', State};
handle_info(
    {'kz_amqp_channel', {Client, Ref, 'consumer_tags'}}, #state{consumer_tags = CTags} = State
) ->
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_event(kz_term:ne_binary(), content(), deliver(), state()) ->
    kz_types:handle_info_ret().
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_return(kz_term:ne_binary(), content(), #'basic.return'{}, state()) ->
    handle_cast_return().
...
basic_return_to_jobj(#'basic.return'{
    reply_code = Code,
    reply_text = Msg,
    exchange = Exchange,
...
maybe_log_module_terminate(_Module, _Reason, 'ok') ->
    'ok';
maybe_log_module_terminate(Module, Reason, Terminated) ->
...
handle_down({Pid, Ref}, Reason, #state{federators = Fs} = State) ->
    case lists:keytake({Pid, Ref}, 2, Fs) of
        'false' ->
            handle_callback_info({'DOWN', Ref, 'process', Pid, Reason}, State);
...
distribute_event(JObj, {_, #'P_basic'{headers = 'undefined'}} = BasicDeliver, State) ->
    case callback_handle_event(JObj, BasicDeliver, State) of
        'ignore' ->
            {'noreply', State};
        {'ignore', ModuleState} ->
            {'noreply', State#state{module_state = ModuleState}};
        {'ignore', ModuleState, 'hibernate'} ->
            {'noreply', State#state{module_state = ModuleState}, 'hibernate'};
        {'ignore', ModuleState, Timeout} ->
            {'noreply', State#state{module_state = ModuleState}, Timeout};
        {CallbackData, ModuleState} ->
            distribute_event(CallbackData, JObj, BasicDeliver, State#state{
                module_state = ModuleState
            });
        CallbackData ->
            distribute_event(CallbackData, JObj, BasicDeliver, State)
    end;
...
-spec distribute_event(callback_data(), kz_json:object(), deliver(), state()) ->
    {'noreply', state()}.
distribute_event(
    CallbackData,
    JObj,
    ...
    _ = [
        kz_util:spawn(fun client_handle_event/6, [
            JObj,
            Channel,
            ConsumerKey,
            Callback,
...
-spec client_handle_event(
    kz_json:object(),
    pid() | kz_amqp_channel:consumer_channel(),
    kz_amqp_channel:consumer_pid(),
    responder_mfa(),
    callback_data(),
    deliver()
) -> any().
...
client_handle_event(
    JObj, #kz_amqp_assignment{channel = Channel}, ConsumerKey, Callback, CallbackData, Deliver
) when
    is_pid(Channel)
->
    _ = kz_util:put_callid(JObj),
    ...
    client_handle_event(JObj, Callback, CallbackData, Deliver);
client_handle_event(JObj, Channel, ConsumerKey, Callback, CallbackData, Deliver) when
    is_pid(Channel)
->
    _ = kz_util:put_callid(JObj),
    _ = kz_amqp_channel:consumer_pid(ConsumerKey),
...
callback_handle_event(
    ...
    case callback_handle_event(JObj, Props, MFA, ModuleState) of
        'ignore' ->
            'ignore';
        {'ignore', _NewModuleState} = Reply ->
            Reply;
        {'ignore', _NewModuleState, _TimeoutOrHibernate} = Reply ->
            Reply;
...
-spec maybe_event_matches_key(responder_callback_mapping(), responder_callback_mapping()) ->
    boolean().
...
remove_binding(Binding, Props, Q) ->
    ...
    try
        (kz_term:to_atom(Wapi, 'true')):unbind_q(Q, Props)
    catch
        'error':'undef' ->
...
-spec add_other_queue(binary(), kz_term:proplist(), kz_term:proplist(), state()) ->
    {kz_term:ne_binary(), state()}.
add_other_queue(
    <<>>, QueueProps, Bindings, #state{other_queues = OtherQueues, auto_ack = AutoAck} = State
) ->
...
add_other_queue(
    QueueName, QueueProps, Bindings, #state{other_queues = OtherQueues, auto_ack = AutoAck} = State
) ->
    ...
    case props:get_value(QueueName, OtherQueues) of
        ...
        OldBindings ->
            {Q, State#state{
                other_queues = [
                    {Q, {Bindings ++ OldBindings, QueueProps}}
...
update_federated_bindings(#state{bindings = [{Binding, _Props} | _]} = State, []) ->
    ...
    State;
update_federated_bindings(
    #state{federators = ExistingFederators} = State,
    FederatedBrokers
) ->
    maybe_bind_to_federated_bindings(
        State,
...
maybe_bind_to_federated_bindings(
    ...
    State#state{
        federators = NewListeners ++ ExistingFederators,
...
maybe_remove_federated_binding(_Flag, _Binding, _Props, #state{federators = []}) ->
    'ok';
...
start_new_listeners(Brokers, Binding, Props, State) ->
    {'ok', [
        start_new_listener(Broker, Binding, Props, State)
     || Broker <- Brokers
    ]}.

-spec start_new_listener(kz_term:ne_binary(), binding_module(), kz_term:proplist(), state()) ->
    federator_listener().
...
remove_federated_binding(Listeners, Binding, Props) ->
    _ = [
        ?MODULE:rm_binding(Pid, Binding, Props)
     || {_Broker, {Pid, _Ref}} <- Listeners
    ],
    'ok'.

-spec update_existing_listeners_bindings(
    federator_listeners(), binding_module(), kz_term:proplist()
) -> 'ok'.
...
-spec update_existing_listener_bindings(federator_listener(), binding_module(), kz_term:proplist()) ->
    'ok'.
...
maybe_configure_auto_ack(Props, 'true') -> [{'no_ack', 'false'} | props:delete('no_ack', Props)].
...
maybe_gc(Reply) ->
    maybe_gc(
        process_info(self(), ['total_heap_size']),
        100 * ?BYTES_K
    ),
    Reply.
