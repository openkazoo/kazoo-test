...
handle_info(
    ...
    {'noreply', Connection, 'hibernate'};
handle_info(
    {'DOWN', Ref, 'process', _Pid, ?SERVER_REPLY_NOT_FOUND},
    #kz_amqp_connection{
        available = 'true',
        channel_ref = Ref,
        broker = _Broker
...
handle_info(
    ...
    lager:warning(
        "command channel to the AMQP broker ~s died: ~p",
        [_Broker, _Reason]
    ),
    {'noreply', create_control_channel(Connection), 'hibernate'};
handle_info(
    ...
    lager:critical(
        "connection to the AMQP broker ~s (~p) died: ~p",
...
handle_info(#'basic.cancel_ok'{} = _Cancel, Connection) ->
    {'noreply', Connection};
handle_info(
    #'connection.blocked'{reason = _Reason}, #kz_amqp_connection{broker = _Broker} = Connection
) ->
    lager:warning(
        "connection ~p is being blocked on the server, check broker health: ~s",
        [_Broker, _Reason]
    ),
    {'noreply', Connection};
...
terminate(_Reason, #kz_amqp_connection{broker = _Broker} = Connection) ->
    lager:debug(
        "connection to amqp broker '~s' terminated: ~p",
        [_Broker, _Reason]
    ),
...
connected(#kz_amqp_connection{reconnect_ref = Ref} = Connection) when
    is_reference(Ref)
->
...
connected(#kz_amqp_connection{channel = 'undefined'} = Connection) ->
    case create_control_channel(Connection) of
        #kz_amqp_connection{channel = Pid} = Success when
            is_pid(Pid)
        ->
            connected(Success);
...
connected(#kz_amqp_connection{prechannels_initialized = 'false'} = Connection) ->
    case initialize_prechannels(Connection) of
        ...
        #kz_amqp_connection{prechannels_initialized = 'true'} = Success -> connected(Success)
    end;
...
disconnected(#kz_amqp_connection{channel_ref = Ref} = Connection, Timeout) when
    is_reference(Ref)
->
    demonitor_refs([Ref]),
    disconnected(Connection#kz_amqp_connection{channel_ref = 'undefined'}, Timeout);
disconnected(#kz_amqp_connection{channel = Pid} = Connection, Timeout) when
    is_pid(Pid)
->
    shutdown_channel(Pid),
    disconnected(Connection#kz_amqp_connection{channel = 'undefined'}, Timeout);
disconnected(#kz_amqp_connection{connection_ref = Ref} = Connection, Timeout) when
    is_reference(Ref)
->
    demonitor_refs([Ref]),
    disconnected(Connection#kz_amqp_connection{connection_ref = 'undefined'}, Timeout);
disconnected(#kz_amqp_connection{connection = Pid} = Connection, Timeout) when
    is_pid(Pid)
->
...
shutdown_channel(ChannelPid) when is_pid(ChannelPid) ->
    try kz_amqp_channel:close(ChannelPid) of
        ...
        _E:_R -> lager:debug("closing channel ~p failed: ~s: ~p", [ChannelPid, _E, _R])
    end.
...
shutdown_connection(ConnectionPid) when is_pid(ConnectionPid) ->
    ...
    try amqp_connection:close(ConnectionPid, 5 * ?MILLISECONDS_IN_SECOND) of
        ...
        _E:_R -> lager:debug("closing connection ~p failed: ~s: ~p", [ConnectionPid, _E, _R])
    end;
shutdown_connection(_ConnectionPid) ->
    'ok'.
...
next_timeout(MaxTimeout, MaxTimeout) ->
    MaxTimeout;
...
maybe_connect(
    ...
    try amqp_connection:start(Params) of
        {'error', 'auth_failure'} ->
            lager:warning(
                "amqp authentication failure with '~s', will retry",
                [_Broker]
            ),
        ...
        {'error', _Reason} ->
            lager:warning(
                "failed to connect to '~s' will retry: ~p",
                [_Broker, _Reason]
            ),
            disconnected(Connection, Timeout);
        {'ok', Pid} ->
            Ref = erlang:monitor('process', Pid),
            connected(Connection#kz_amqp_connection{
                connection = Pid,
                connection_ref = Ref
            });
        _E ->
            lager:critical(
                "unhandled case on connect to '~s' will retry: ~p",
                [_Broker, _E]
            ),
            disconnected(Connection, Timeout)
    catch
        _E:_R ->
            lager:warning(
                "exception connecting to '~s' will retry: ~s: ~p",
                [_Broker, _E, _R]
            ),
            disconnected(Connection, Timeout)
    end.
...
create_control_channel(#kz_amqp_connection{channel_ref = Ref} = Connection) when
    is_reference(Ref)
->
    erlang:demonitor(Ref, ['flush']),
    create_control_channel(Connection#kz_amqp_connection{channel_ref = 'undefined'});
create_control_channel(#kz_amqp_connection{channel = ChannelPid} = Connection) when
    is_pid(ChannelPid)
->
...
create_control_channel(#kz_amqp_connection{broker = Broker} = Connection) ->
    case open_channel(Connection) of
        {'error', _R} ->
            lager:critical(
                "unable to establish command channel to ~s, assuming connection is invalid: ~p",
                [Broker, _R]
            ),
            disconnected(Connection);
        {'ok', Pid} ->
...
initialize_prechannels(#kz_amqp_connection{} = Connection, Count) ->
    case establish_prechannel(Connection) of
        #kz_amqp_connection{connection = Pid} = Success when
            is_pid(Pid)
        ->
...
establish_prechannel(
    ...
    case open_channel(Connection) of
        {'error', _R} ->
            lager:critical(
                "unable to establish prechannel to ~s, assuming connection is invalid: ~p",
                [Broker, _R]
            ),
            disconnected(Connection);
        {'ok', Pid} ->
...
-spec declare_exchanges_fold(kz_amqp_exchanges(), kz_amqp_connection()) ->
    kz_amqp_connection() | {'error', any()}.
declare_exchanges_fold([], Connection) ->
    Connection;
declare_exchanges_fold(
    [Exchange | Exchanges],
...
-spec declare_exchange(kz_amqp_exchange(), kz_amqp_connection()) ->
    kz_amqp_connection() | 'ok' | {'error', any()}.
declare_exchange(
    Exchange,
    #kz_amqp_connection{channel = Channel} = Connection
) ->
...
-spec declare_exchange(kz_amqp_connection(), pid(), kz_amqp_exchange()) ->
    kz_amqp_connection() | 'ok' | {'error', any()}.
declare_exchange(
    #kz_amqp_connection{
        broker = _Broker,
...
->
    ...
    try
        not maps:is_key(ExchangeName, ExchangeMap) andalso
    ...
    of
        ...
        #'exchange.declare_ok'{} ->
            lager:debug(
                "declared ~s exchange ~s on ~s via ~p",
                [
        ...
        _Else when Exchange#'exchange.declare'.passive ->
            lager:debug(
                "failed to declare ~s passive exchange ~s on ~s via ~p: ~p",
                [
                    Exchange#'exchange.declare'.type,
        ...
        _Else ->
            lager:critical(
                "failed to declare ~s exchange ~s on ~s via ~p: ~p",
                [
                    Exchange#'exchange.declare'.type,
                    Exchange#'exchange.declare'.exchange,
                    _Broker,
        ...
        _E:{?SERVER_REPLY_NOT_FOUND, _} when
            Exchange#'exchange.declare'.passive
        ->
            {'error', 'not_found'};
        _E:_R ->
            lager:critical(
                "exception while declaring ~s exchange ~s on ~s via ~p: ~p",
                [
                    Exchange#'exchange.declare'.type,
                    Exchange#'exchange.declare'.exchange,
                    _Broker,
