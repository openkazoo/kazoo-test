...
channel_publish_method(Method) when
    Method =:= 'cast';
    Method =:= 'call'
->
...
close(Channel, []) when is_pid(Channel) ->
    _ =
        (catch gen_server:call(Channel, {'close', 200, <<"Goodbye">>}, 5 * ?MILLISECONDS_IN_SECOND)),
...
maybe_publish(#'basic.publish'{routing_key = RoutingKey} = BasicPub, AmqpMsg) ->
    case maybe_split_routing_key(RoutingKey) of
        ...
        {ConsumerPid, RemoteRoutingKey} ->
            basic_publish(
                kz_amqp_assignments:find(ConsumerPid),
...
publish(#'basic.publish'{routing_key = RoutingKey} = BasicPub, AmqpMsg) ->
    case maybe_split_routing_key(RoutingKey) of
        ...
        {ConsumerPid, RemoteRoutingKey} ->
            basic_publish(
                kz_amqp_assignments:get_channel(ConsumerPid, ?ASSIGNMENT_TIMEOUT),
                BasicPub#'basic.publish'{routing_key = RemoteRoutingKey},
                AmqpMsg
            )
    end.
...
-spec basic_publish(
    kz_amqp_assignment() | pid() | {'error', 'no_channel'}, basic_publish(), amqp_msg()
) -> 'ok'.
basic_publish(
    #kz_amqp_assignment{
        channel = Channel,
    ...
    lager:debug(
        "published(~s ~s) to ~s(~s) exchange (routing key ~s) via ~p",
        [
            MsgId,
            kz_util:pretty_print_bytes(iolist_size(AmqpMsg#'amqp_msg'.payload), 'truncated'),
            _Exchange,
            _Broker,
            _RK,
            Channel
...
basic_publish(
    ...
    lager:debug(
        "dropping payload to ~s exchange (routing key ~s): ~s",
...
basic_publish(
    ...
    lager:debug(
        "published to ~s(direct ~s) exchange (routing key ~s) via ~p",
        [
            _Exchange,
            kz_util:pretty_print_bytes(iolist_size(AmqpMsg#'amqp_msg'.payload), 'truncated'),
            _RK,
...
exec_command(
    ...
    case amqp_channel:subscribe(Channel, C, Consumer) of
        ...
        _Else ->
            lager:warning(
                "failed to re-establish consumer for ~p: ~p",
                [Consumer, _Else]
            )
    end;
exec_command(
...
exec_command(
    ...
    case is_process_alive(Consumer) of
        ...
        'true' -> cancel_consumer_tag(Consumer, Channel, CTag, NoWait)
    end;
exec_command(
...
exec_command(#kz_amqp_assignment{channel = Channel} = Assignment, Command) ->
    Result =
        try
            amqp_channel:call(Channel, Command)
        catch
            E:R ->
...
-spec handle_command_result(command_ret(), kz_amqp_command(), kz_amqp_assignment()) ->
    command_ret().
handle_command_result(_, _, #kz_amqp_assignment{reconnect = 'true'}) ->
    'ok';
...
handle_command_result(
    ...
    lager:debug(
        "unbound ~s from ~s exchange (routing key ~s) via channel ~p",
...
handle_command_result(
    ...
    lager:debug(
        "bound ~s to ~s exchange (routing key ~s) via channel ~p",
...
handle_command_result('ok', Command, #kz_amqp_assignment{channel = Channel} = _Assignment) ->
    lager:debug(
        "executed AMQP command ~s with no_wait option via channel ~p",
...
assert_valid_amqp_method(Command) ->
    try
        rabbit_framing_amqp_0_9_1:encode_method_fields(Command)
    catch
        E:R ->
