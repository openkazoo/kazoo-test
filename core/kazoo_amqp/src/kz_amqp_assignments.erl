...
get_channel(Consumer, Timeout) when is_pid(Consumer) ->
    case find(Consumer) of
        #kz_amqp_assignment{channel = Channel} = Assignment when
            is_pid(Channel)
        ->
            Assignment;
        #kz_amqp_assignment{} ->
...
add_channel(Broker, Connection, Channel) when
    is_pid(Channel) andalso
        is_binary(Broker)
->
...
channel_count(AMQPConn) when is_pid(AMQPConn) ->
    MatchSpec = [{#kz_amqp_assignment{connection = AMQPConn, _ = '_'}, [], ['true']}],
...
handle_cast({'maybe_reassign', Consumer}, State) ->
    ...
    {'noreply', State};
handle_cast(
    {'maybe_defer_reassign', #kz_amqp_assignment{
        timestamp = Timestamp,
        consumer = Consumer,
        type = Type
    }},
...
release_assignments('$end_of_table') ->
    'ok';
release_assignments({[#kz_amqp_assignment{consumer_ref = Ref} = Assignment], Continuation}) when
    is_reference(Ref)
->
    demonitor(Ref, ['flush']),
    release_assignments({[Assignment#kz_amqp_assignment{consumer_ref = 'undefined'}], Continuation});
release_assignments({[#kz_amqp_assignment{channel_ref = Ref} = Assignment], Continuation}) when
    is_reference(Ref)
->
    demonitor(Ref, ['flush']),
    release_assignments({[Assignment#kz_amqp_assignment{channel_ref = 'undefined'}], Continuation});
release_assignments({
    [
        #kz_amqp_assignment{
            channel = Channel,
...
->
    ...
    release_assignments({[Assignment#kz_amqp_assignment{channel = 'undefined'}], Continuation});
release_assignments({
    [
        #kz_amqp_assignment{
            timestamp = Timestamp,
...
maybe_reassign(Consumer) when is_pid(Consumer) ->
    ...
    case ets:match_object(?TAB, Pattern, 1) of
        '$end_of_table' ->
            'ok';
        {[#kz_amqp_assignment{channel = Channel}], _} when
            is_pid(Channel)
        ->
            'ok';
        {
...
maybe_reassign(_, 'undefined') ->
    'undefined';
maybe_reassign(_, '$end_of_table') ->
    'undefined';
maybe_reassign(
    #kz_amqp_assignment{consumer = _Consumer} = ConsumerAssignment,
    {
        [
            #kz_amqp_assignment{
                timestamp = Timestamp,
    ...
    case is_process_alive(Channel) of
        ...
        'true' ->
            lager:debug(
                "attempting to move consumer ~p to a channel on ~s",
                [_Consumer, _Broker]
            ),
...
%% @end
%%------------------------------------------------------------------------------
-spec assign_or_reserve(pid(), kz_term:api_binary(), atom(), kz_amqp_type()) ->
    kz_amqp_assignment().
...
assign_consumer(#kz_amqp_assignment{} = ChannelAssignment, Consumer, Application, Type) ->
    ...
    case ets:match_object(?TAB, Pattern, 1) of
        ...
        {[#kz_amqp_assignment{channel = Channel} = ConsumerAssignment], _} when
            is_pid(Channel)
        ->
            ...
            lager:debug(
                "consumer ~p already has a valid channel ~p",
                [Consumer, Channel]
            ),
            ConsumerAssignment;
...
move_channel_to_consumer(
    ...
    Assignment =
        ConsumerAssignment#kz_amqp_assignment{
            channel = Channel,
            channel_ref = ChannelRef,
            broker = Broker,
            connection = Connection,
    ...
    ets:insert(?TAB, Assignment#kz_amqp_assignment{
        reconnect = 'false',
        watchers = sets:new()
    }),
    ...
    lager:debug(
        "assigned existing consumer ~p an available channel ~p on ~s(~p)",
...
%% @end
%%------------------------------------------------------------------------------
-spec add_consumer_to_channel(kz_amqp_assignment(), pid(), atom(), kz_amqp_type()) ->
    kz_amqp_assignment().
add_consumer_to_channel(
    #kz_amqp_assignment{
        channel = Channel,
        broker = _Broker,
        connection = Connection
    ...
    Assignment =
        ChannelAssignment#kz_amqp_assignment{
            consumer = Consumer,
    ...
    lager:debug(
        "assigned existing channel ~p on ~s(~p) to new consumer ~p",
...
assign_channel(
    #kz_amqp_assignment{channel_ref = Ref} = Assignment,
    Broker,
    Connection,
    Channel
) when
...
->
    demonitor(Ref, ['flush']),
    assign_channel(
        Assignment#kz_amqp_assignment{channel_ref = 'undefined'},
        Broker,
        Connection,
        Channel
    );
assign_channel(
    ...
    is_pid(CurrentChannel)
->
    lager:debug(
        "reassigning consumer ~p, closing current channel ~p on ~s",
...
assign_channel(
    ...
    ets:insert(?TAB, Assigment#kz_amqp_assignment{
        reconnect = 'false',
        watchers = sets:new()
    }),
    lager:debug(
        "assigned consumer ~p new channel ~p on ~s(~p) after ~pμs",
...
cache(Broker, Connection, Channel) ->
    ChannelRef = erlang:monitor('process', Channel),
    lager:debug(
        "added new channel ~p on ~s(~p) to available pool",
        [Channel, Broker, Connection]
    ),
    ets:insert(?TAB, #kz_amqp_assignment{
        channel = Channel,
        channel_ref = ChannelRef,
        broker = Broker,
        connection = Connection
...
notify_consumer(#kz_amqp_assignment{
    consumer = Consumer,
    reconnect = Reconnect,
    channel = Channel
}) ->
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_reserve(kz_term:api_pid(), kz_term:api_binary(), atom(), kz_amqp_type()) ->
    kz_amqp_assignment().
maybe_reserve(Consumer, Broker, Application, Type) ->
    Pattern = #kz_amqp_assignment{consumer = Consumer, _ = '_'},
    case ets:match_object(?TAB, Pattern, 1) of
        ...
        {[#kz_amqp_assignment{} = ExistingAssignment], _} ->
            ...
            lager:debug(
                "consumer ~p still waiting on AMQP channel from ~s",
                [Consumer, Broker]
            ),
            ExistingAssignment
    end.

-spec reserve(kz_term:api_pid(), kz_term:api_binary(), atom(), kz_amqp_type()) ->
    kz_amqp_assignment().
reserve(Consumer, Broker, Application, 'sticky') when Broker =/= 'undefined' ->
    ...
    lager:debug(
        "consumer ~p waiting on sticky AMQP channel from ~s",
        [Consumer, Broker]
    ),
    Assignment;
...
handle_down_msg([], _Pid, _Reason) ->
    lager:info("no matches for downed pid ~p: ~p", [_Pid, _Reason]);
...
handle_down_match(
    {'consumer', #kz_amqp_assignment{consumer = Consumer}},
    'shutdown'
) ->
    Pattern = #kz_amqp_assignment{consumer = Consumer, _ = '_'},
    release_assignments(ets:match_object(?TAB, Pattern, 1));
handle_down_match(
    {'consumer', #kz_amqp_assignment{consumer = Consumer} = Assignment},
    _Reason
) ->
    lager:debug(
        "consumer ~p, went down without closing channel: ~p",
        [Consumer, _Reason]
    ),
...
handle_down_match(
    ...
    lager:debug(
        "unused channel ~p on ~s went down: ~p",
...
handle_down_match(
    ...
    lager:debug(
        "floating channel ~p on ~s went down while still assigned to consumer ~p: ~p",
...
handle_down_match(
    ...
    lager:debug(
        "sticky channel ~p on ~s went down while still assigned to consumer ~p: ~p",
        [Channel, Broker, Consumer, Reason]
    ),
    Consumer ! {'kz_amqp_assignment', 'lost_channel'},
    maybe_defer_reassign(Assignment, Reason).
...
add_watcher(_Consumer, 'undefined') ->
    'ok';
add_watcher(Consumer, Watcher) when
    is_pid(Consumer)
->
    case find(Consumer) of
...
add_watcher(#kz_amqp_assignment{channel = Channel} = Assignment, Watcher) when
    is_pid(Channel)
->
    Watcher ! Assignment,
    'ok';
...
log_short_lived(#kz_amqp_assignment{assigned = 'undefined'}) ->
    'ok';
...
log_short_lived(
    ...
    lager:warning(
        "short lived assignment (~ps) for ~p (channel ~p type ~p broker ~p)",
        [DurationS, Consumer, Channel, Type, Broker]
    );
log_short_lived(_Assignment, _Duration) ->
    'ok'.
...
-spec release_handlers({kz_amqp_assignments(), ets:continuation()} | '$end_of_table' | pid()) ->
    'ok'.
release_handlers(Consumer) when
    is_pid(Consumer)
->
    Pattern = #kz_amqp_assignment{consumer = Consumer, _ = '_'},
    release_handlers(ets:match_object(?TAB, Pattern, 1));
release_handlers('$end_of_table') ->
    'ok';
release_handlers({[#kz_amqp_assignment{channel = Channel}], Continuation}) when
    is_pid(Channel)
->
    case is_process_alive(Channel) of
