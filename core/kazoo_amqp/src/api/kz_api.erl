...
-spec default_headers(kz_term:api_ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:proplist().
...
-spec default_headers(
    kz_term:api_ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()
) -> kz_term:proplist().
...
-spec disambiguate_and_publish(kz_json:object(), kz_json:object(), kz_term:ne_binary() | atom()) ->
    any().
...
-type prepare_option_el() ::
    {'formatter', api_formatter_fun()}
    | {'remove_recursive', boolean()}.
-type prepare_options() :: [prepare_option_el()].

-spec prepare_api_payload(kz_term:api_terms(), kz_term:proplist()) ->
    api_formatter_return() | kz_term:proplist().
prepare_api_payload(Prop, HeaderValues) ->
    prepare_api_payload(Prop, HeaderValues, []).

-spec prepare_api_payload(
    kz_term:api_terms(), kz_term:proplist(), api_formatter_fun() | prepare_options()
) ->
    api_formatter_return() | kz_term:proplist().
...
prepare_api_payload(Prop, HeaderValues, Options) when is_list(Prop) ->
    ...
    CleanupFuns = [
        fun(P) ->
            remove_empty_values(P, props:get_is_true('remove_recursive', Options, 'true'))
        end,
...
do_empty_value_removal([{K, V} = KV | T], Recursive, Acc) ->
    case should_strip_from_payload(V) of
        'true' ->
            do_empty_value_removal(T, Recursive, Acc);
        'false' ->
            case
                (kz_json:is_json_object(V) orelse
                    kz_term:is_proplist(V)) andalso
                    Recursive
            of
                'true' ->
...
extract_defaults(Prop) when is_list(Prop) ->
    ...
    [
        {H, V}
     || H <- ?DEFAULT_HEADERS ++ ?OPTIONAL_DEFAULT_HEADERS,
...
-spec validate_message(kz_term:api_terms(), api_headers(), kz_term:proplist(), kz_term:proplist()) ->
    boolean().
...
build_message(Prop, ReqH, OptH) when is_list(Prop) ->
    case defaults(Prop, ReqH ++ OptH) of
        {'error', _Reason} = Error ->
            lager:debug(
                "API message does not have the default headers ~s: ~p",
...
-spec build_message_specific_headers(
    kz_term:proplist() | {api_headers(), kz_term:proplist()}, api_headers(), api_headers()
) ->
    {'ok', kz_term:proplist()}
    | {'error', string()}.
build_message_specific_headers({Headers, Prop}, ReqH, OptH) ->
    case update_required_headers(Prop, ReqH, Headers) of
        {'error', _Reason} = Error ->
            lager:debug(
                "API message does not have the required headers ~s: ~p : ~p",
...
build_message_specific({Headers, Prop}, ReqH, OptH) ->
    case update_required_headers(Prop, ReqH, Headers) of
        {'error', _Reason} = Error ->
            lager:debug(
                "API message does not have the required headers ~s: ~p",
...
headers_to_json([_ | _] = HeadersProp) ->
    ...
    try kz_json:encode(kz_json:from_list(HeadersProp)) of
        ...
        _What:_Why ->
            {'error', io_lib:format("KAZOO TO_JSON ~p: ~p~n~p~n", [_What, _Why, HeadersProp])}
    end.
...
-spec expand_header(kz_term:ne_binary() | kz_term:ne_binaries(), kz_term:ne_binaries()) ->
    kz_term:ne_binaries().
expand_header(Header, Acc) when
    is_binary(Header)
->
    [Header | Acc];
expand_header(Headers, Acc) when
    is_list(Headers)
->
    expand_headers(Headers) ++ Acc.
    ...
    case props:get_value(Key, Prop) of
        ...
        V ->
            case lists:member(V, Vs) of
                ...
                'false' ->
                    lager:debug(
                        "API key '~s' value '~p' is not one of the values: ~p",
                        [Key, V, Vs]
                    ),
                    'false'
            end
    ...
    case props:get_value(Key, Prop) of
        ...
        Value ->
            ...
            try Fun(Value) of
                ...
                _:_R ->
                    lager:debug("API key '~s' value '~p' caused validation fun exception: ~p", [
                        Key, Value, _R
                    ]),
                    'false'
            end
...
-define(is_json(Obj),
    is_tuple(Obj) andalso
        is_list(element(1, Obj))
).
