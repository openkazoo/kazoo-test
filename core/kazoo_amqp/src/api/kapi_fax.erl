...
-define(OPTIONAL_FAX_STATUS_HEADERS, [
    <<"Status">>,
    <<"FaxBox-ID">>,
    <<"Account-ID">>,
    <<"Fax-Info">>,
...
-define(FAX_START_ACCOUNT_VALUES, [
    {<<"Event-Category">>, <<"start">>},
    {<<"Event-Name">>, <<"account">>}
]).
-define(FAX_START_ACCOUNT_TYPES, []).

-define(FAX_START_JOB_HEADERS, [
    <<"Job-ID">>, <<"Account-ID">>, <<"To-Number">>, <<"Control-Queue">>
]).
-define(OPTIONAL_FAX_START_JOB_HEADERS, []).
-define(FAX_START_JOB_VALUES, [
    {<<"Event-Category">>, <<"start">>},
    {<<"Event-Name">>, <<"job">>}
]).
...
start_account(Prop) when is_list(Prop) ->
    case start_account_v(Prop) of
        'false' ->
            {'error', "Proplist failed validation for fax_start account"};
        'true' ->
            kz_api:build_message(
                Prop, ?FAX_START_ACCOUNT_HEADERS, ?OPTIONAL_FAX_START_ACCOUNT_HEADERS
            )
    end;
start_account(JObj) ->
...
start_account_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?FAX_START_ACCOUNT_HEADERS, ?FAX_START_ACCOUNT_VALUES, ?FAX_START_ACCOUNT_TYPES
    );
...
start_job(Prop) when is_list(Prop) ->
    case start_job_v(Prop) of
        'false' ->
            {'error', "Proplist failed validation for fax_start account"};
        'true' ->
            kz_api:build_message(Prop, ?FAX_START_JOB_HEADERS, ?OPTIONAL_FAX_START_JOB_HEADERS)
    end;
start_job(JObj) ->
...
unbind_q(Queue, AccountId, FaxId, StartId, ['status' | Restrict]) ->
    'ok' = kz_amqp_util:unbind_q_from_exchange(
        Queue, status_routing_key(AccountId, FaxId), ?FAX_EXCHANGE
    ),
...
publish_status(API, ContentType) ->
    ...
    kz_amqp_util:basic_publish(
        ?FAX_EXCHANGE, status_routing_key(AccountId, FaxId), Payload, ContentType
    ).
...
-spec publish_targeted_status(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
...
publish_start_account(API, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        API, ?FAX_START_ACCOUNT_VALUES, fun start_account/1
    ),
    AccountId = props:get_value(<<"Account-ID">>, API, <<"*">>),
    kz_amqp_util:basic_publish(
        ?FAX_EXCHANGE, fax_start_key(<<"account">>, AccountId), Payload, ContentType
    ).
...
publish_start_job(API, ContentType) ->
    ...
    kz_amqp_util:basic_publish(
        ?FAX_EXCHANGE, fax_start_key(<<"job">>, JobId), Payload, ContentType
    ).
