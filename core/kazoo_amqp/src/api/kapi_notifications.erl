...
-export([
    ...
    fax_outbound/1,
    fax_outbound_v/1,
    fax_outbound_error/1,
    fax_outbound_error_v/1,
    fax_outbound_smtp_error/1,
    ...
    number_feature_manual_action/1,
...
-export([
    ...
    publish_emergency_bridge/1, publish_emergency_bridge/2,
...
-define(DEFAULT_OPTIONAL_HEADERS, [
    <<"To">>,
    <<"Cc">>,
    <<"Bcc">>,
    <<"From">>,
...
-define(BINDING_STRING(Category, Name),
    <<"notifications.", (Category)/binary, ".", (Name)/binary>>
).

-define(NOTIFY_VALUES(Name), [
...
bill_reminder_definition() ->
    #kapi_definition{
        ...
        description = <<
            "This event is triggered before a few days before the end of the month to"
...
-define(COMMON_TRANSACTION_HEADERS, [
    <<"Add-Ons">>,
    <<"Billing-Address">>,
    <<"Card-Last-Four">>,
    <<"Currency-Code">>,
...
outbound_smtp_fax_error_definition() ->
    #kapi_definition{
        ...
        types = [
            {<<"Errors">>, fun
                (L) when is_list(L) -> kz_term:is_not_empty(L);
                (_) -> 'false'
            end}
        ]
    }.
...
-define(PORT_OPTIONAL_HEADERS, [
    ...
    <<"Port">>,
    <<"Reason">>,
    %% for stupid notify app (port_request)
    <<"Version">>
    | ?DEFAULT_OPTIONAL_HEADERS
]).
%%------------------------------------------------------------------------------
...
-define(OPTIONAL_VOICEMAIL_NEW_HEADERS, [
    <<"Call-ID">>,
    <<"Caller-ID-Name">>,
    <<"Caller-ID-Number">>,
    <<"Voicemail-Length">>,
...
-define(VOICEMAIL_DELETED_HEADERS, [
    <<"Account-ID">>,
    <<"From-Realm">>,
    <<"From-User">>,
    <<"To-Realm">>,
...
-define(OPTIONAL_VOICEMAIL_DELETED_HEADERS, [
    <<"Call-ID">>,
    <<"Caller-ID-Name">>,
    <<"Caller-ID-Number">>,
    <<"Voicemail-Length">>,
...
account_db('undefined', _) ->
    'undefined';
account_db(Req, StrictMODB) when is_list(Req) ->
    find_account_db(Req, StrictMODB, fun props:get_first_defined/2);
account_db(Req, StrictMODB) ->
    find_account_db(Req, StrictMODB, fun kz_json:get_first_defined/2).
...
find_account_db(Req, StrictMODB, GetFun) ->
    ...
    case GetFun(Paths, Req) of
        ...
        ?MATCH_MODB_SUFFIX_RAW(_, _, _) = Db ->
            maybe_strict_modb(Db, StrictMODB);
        ?MATCH_MODB_SUFFIX_UNENCODED(_, _, _) = Db ->
            maybe_strict_modb(Db, StrictMODB);
        ?MATCH_MODB_SUFFIX_ENCODED(_, _, _) = Db ->
            maybe_strict_modb(Db, StrictMODB);
        ?MATCH_ACCOUNT_RAW(_) = Db ->
            kz_util:format_account_db(Db);
        ?MATCH_ACCOUNT_UNENCODED(_) = Db ->
            kz_util:format_account_db(Db);
        ?MATCH_ACCOUNT_ENCODED(_) = Db ->
            kz_util:format_account_db(Db);
        ?MATCH_ACCOUNT_encoded(_) = Db ->
            kz_util:format_account_db(Db);
        OtherDb ->
    ...
    try api_definition(Name) of
        Definition ->
            kapi_definition:required_headers(Definition) ++
                kapi_definition:optional_headers(Definition)
    catch
        error:undef ->
...
build_message(Prop, #kapi_definition{
    required_headers = ReqH,
