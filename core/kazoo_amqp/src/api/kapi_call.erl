    ...
    case channel_status_req_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?CHANNEL_STATUS_REQ_HEADERS, ?OPTIONAL_CHANNEL_STATUS_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for channel status req"}
    end;
channel_status_req(JObj) ->
    channel_status_req(kz_json:to_proplist(JObj)).

-spec channel_status_req_v(kz_term:api_terms()) -> boolean().
channel_status_req_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?CHANNEL_STATUS_REQ_HEADERS, ?CHANNEL_STATUS_REQ_VALUES, ?CHANNEL_STATUS_REQ_TYPES
    );
channel_status_req_v(JObj) ->
    channel_status_req_v(kz_json:to_proplist(JObj)).
...
channel_status_resp(Prop) when is_list(Prop) ->
    case channel_status_resp_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?CHANNEL_STATUS_RESP_HEADERS, ?OPTIONAL_CHANNEL_STATUS_RESP_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for channel status resp"}
    end;
channel_status_resp(JObj) ->
    channel_status_resp(kz_json:to_proplist(JObj)).

-spec channel_status_resp_v(kz_term:api_terms()) -> boolean().
channel_status_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?CHANNEL_STATUS_RESP_HEADERS, ?CHANNEL_STATUS_RESP_VALUES, ?CHANNEL_STATUS_RESP_TYPES
    );
channel_status_resp_v(JObj) ->
    channel_status_resp_v(kz_json:to_proplist(JObj)).
...
query_auth_id_req(Prop) when is_list(Prop) ->
    case query_auth_id_req_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?QUERY_AUTH_ID_REQ_HEADERS, ?OPTIONAL_QUERY_AUTH_ID_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for auth id query req"}
    end;
query_auth_id_req(JObj) ->
    query_auth_id_req(kz_json:to_proplist(JObj)).

-spec query_auth_id_req_v(kz_term:api_terms()) -> boolean().
query_auth_id_req_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?QUERY_AUTH_ID_REQ_HEADERS, ?QUERY_AUTH_ID_REQ_VALUES, ?QUERY_AUTH_ID_REQ_TYPES
    );
query_auth_id_req_v(JObj) ->
    query_auth_id_req_v(kz_json:to_proplist(JObj)).
...
query_auth_id_resp(Prop) when is_list(Prop) ->
    case query_auth_id_resp_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?QUERY_AUTH_ID_RESP_HEADERS, ?OPTIONAL_QUERY_AUTH_ID_RESP_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for auth id query resp"}
    end;
query_auth_id_resp(JObj) ->
    query_auth_id_resp(kz_json:to_proplist(JObj)).

-spec query_auth_id_resp_v(kz_term:api_terms()) -> boolean().
query_auth_id_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?QUERY_AUTH_ID_RESP_HEADERS, ?QUERY_AUTH_ID_RESP_VALUES, ?QUERY_AUTH_ID_RESP_TYPES
    );
query_auth_id_resp_v(JObj) ->
    query_auth_id_resp_v(kz_json:to_proplist(JObj)).
...
query_user_channels_req(Prop) when is_list(Prop) ->
    case query_user_channels_req_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?QUERY_USER_CHANNELS_REQ_HEADERS, ?OPTIONAL_QUERY_USER_CHANNELS_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for users channels query req"}
    end;
query_user_channels_req(JObj) ->
    query_user_channels_req(kz_json:to_proplist(JObj)).

-spec query_user_channels_req_v(kz_term:api_terms()) -> boolean().
query_user_channels_req_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?QUERY_USER_CHANNELS_REQ_HEADERS,
        ?QUERY_USER_CHANNELS_REQ_VALUES,
        ?QUERY_USER_CHANNELS_REQ_TYPES
    );
query_user_channels_req_v(JObj) ->
    query_user_channels_req_v(kz_json:to_proplist(JObj)).
...
query_user_channels_resp(Prop) when is_list(Prop) ->
    case query_user_channels_resp_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?QUERY_USER_CHANNELS_RESP_HEADERS, ?OPTIONAL_QUERY_USER_CHANNELS_RESP_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for users channels query resp"}
    end;
query_user_channels_resp(JObj) ->
    query_user_channels_resp(kz_json:to_proplist(JObj)).

-spec query_user_channels_resp_v(kz_term:api_terms()) -> boolean().
query_user_channels_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?QUERY_USER_CHANNELS_RESP_HEADERS,
        ?QUERY_USER_CHANNELS_RESP_VALUES,
        ?QUERY_USER_CHANNELS_RESP_TYPES
    );
query_user_channels_resp_v(JObj) ->
    query_user_channels_resp_v(kz_json:to_proplist(JObj)).
...
query_account_channels_req(Prop) when is_list(Prop) ->
    case query_account_channels_req_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop,
                ?QUERY_ACCOUNT_CHANNELS_REQ_HEADERS,
                ?OPTIONAL_QUERY_ACCOUNT_CHANNELS_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for account channels query req"}
    end;
query_account_channels_req(JObj) ->
    query_account_channels_req(kz_json:to_proplist(JObj)).
...
query_account_channels_req_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?QUERY_ACCOUNT_CHANNELS_REQ_HEADERS,
        ?QUERY_ACCOUNT_CHANNELS_REQ_VALUES,
        ?QUERY_ACCOUNT_CHANNELS_REQ_TYPES
    );
query_account_channels_req_v(JObj) ->
    query_account_channels_req_v(kz_json:to_proplist(JObj)).
...
query_account_channels_resp(Prop) when is_list(Prop) ->
    case query_account_channels_resp_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop,
                ?QUERY_ACCOUNT_CHANNELS_RESP_HEADERS,
                ?OPTIONAL_QUERY_ACCOUNT_CHANNELS_RESP_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for accounts channels query resp"}
    end;
query_account_channels_resp(JObj) ->
    query_account_channels_resp(kz_json:to_proplist(JObj)).
...
query_account_channels_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?QUERY_ACCOUNT_CHANNELS_RESP_HEADERS,
        ?QUERY_ACCOUNT_CHANNELS_RESP_VALUES,
        ?QUERY_ACCOUNT_CHANNELS_RESP_TYPES
    );
query_account_channels_resp_v(JObj) ->
    query_account_channels_resp_v(kz_json:to_proplist(JObj)).
...
query_channels_req(Prop) when is_list(Prop) ->
    case query_channels_req_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?QUERY_CHANNELS_REQ_HEADERS, ?OPTIONAL_QUERY_CHANNELS_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for channels query req"}
    end;
query_channels_req(JObj) ->
    query_channels_req(kz_json:to_proplist(JObj)).

-spec query_channels_req_v(kz_term:api_terms()) -> boolean().
query_channels_req_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?QUERY_CHANNELS_REQ_HEADERS, ?QUERY_CHANNELS_REQ_VALUES, ?QUERY_CHANNELS_REQ_TYPES
    );
query_channels_req_v(JObj) ->
    query_channels_req_v(kz_json:to_proplist(JObj)).
...
query_channels_resp(Prop) when is_list(Prop) ->
    case query_channels_resp_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?QUERY_CHANNELS_RESP_HEADERS, ?OPTIONAL_QUERY_CHANNELS_RESP_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for channels query resp"}
    end;
query_channels_resp(JObj) ->
    query_channels_resp(kz_json:to_proplist(JObj)).

-spec query_channels_resp_v(kz_term:api_terms()) -> boolean().
query_channels_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?QUERY_CHANNELS_RESP_HEADERS, ?QUERY_CHANNELS_RESP_VALUES, ?QUERY_CHANNELS_RESP_TYPES
    );
query_channels_resp_v(JObj) ->
    query_channels_resp_v(kz_json:to_proplist(JObj)).
...
usurp_control(Prop) when is_list(Prop) ->
    case usurp_control_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?CALL_USURP_CONTROL_HEADERS, ?OPTIONAL_CALL_USURP_CONTROL_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for usurp_control"}
    end;
usurp_control(JObj) ->
    usurp_control(kz_json:to_proplist(JObj)).

-spec usurp_control_v(kz_term:api_terms()) -> boolean().
usurp_control_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?CALL_USURP_CONTROL_HEADERS, ?CALL_USURP_CONTROL_VALUES, ?CALL_USURP_CONTROL_TYPES
    );
usurp_control_v(JObj) ->
    usurp_control_v(kz_json:to_proplist(JObj)).
...
usurp_publisher(Prop) when is_list(Prop) ->
    case usurp_publisher_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?PUBLISHER_USURP_CONTROL_HEADERS, ?OPTIONAL_PUBLISHER_USURP_CONTROL_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for usurp_publisher"}
    end;
usurp_publisher(JObj) ->
    usurp_publisher(kz_json:to_proplist(JObj)).

-spec usurp_publisher_v(kz_term:api_terms()) -> boolean().
usurp_publisher_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?PUBLISHER_USURP_CONTROL_HEADERS,
        ?PUBLISHER_USURP_CONTROL_VALUES,
        ?PUBLISHER_USURP_CONTROL_TYPES
    );
usurp_publisher_v(JObj) ->
    usurp_publisher_v(kz_json:to_proplist(JObj)).
...
find_event_call_id(Event) ->
    ...
    Keys =
        case
            kz_api:event_name(Event) =/= <<"CHANNEL_FAX_STATUS">> andalso
                props:is_true(<<"Channel-Is-Loopback">>, Event, 'false')
        of
...
-spec publish_channel_status_req(kz_term:api_terms(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
publish_channel_status_req(Req, CallId, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Req, ?CHANNEL_STATUS_REQ_VALUES, fun channel_status_req/1
    ),
    kz_amqp_util:callevt_publish(
        ?CALL_EVENT_ROUTING_KEY('status_req', CallId), Payload, ContentType
    ).
...
-spec publish_channel_status_resp(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_channel_status_resp(RespQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?CHANNEL_STATUS_RESP_VALUES, fun channel_status_resp/1
    ),
...
-spec publish_query_auth_id_req(kz_term:api_terms(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
publish_query_auth_id_req(Req, AuthId, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Req, ?QUERY_AUTH_ID_REQ_VALUES, fun query_auth_id_req/1
    ),
    kz_amqp_util:callevt_publish(
        ?CALL_EVENT_ROUTING_KEY('status_req', AuthId), Payload, ContentType
    ).
...
-spec publish_query_auth_id_resp(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_query_auth_id_resp(RespQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?QUERY_AUTH_ID_RESP_VALUES, fun query_auth_id_resp/1
    ),
    kz_amqp_util:targeted_publish(RespQ, Payload, ContentType).

-spec publish_query_user_channels_req(kz_term:api_terms()) -> 'ok'.
publish_query_user_channels_req(API) ->
    publish_query_user_channels_req(
        API,
        username(API),
        realm(API),
...
-spec publish_query_user_channels_req(
    kz_term:api_terms(), kz_term:api_binary(), kz_term:api_binary(), kz_term:ne_binary()
) -> 'ok'.
publish_query_user_channels_req(Req, 'undefined', 'undefined', ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Req, ?QUERY_USER_CHANNELS_REQ_VALUES, fun query_user_channels_req/1
    ),
...
-spec publish_query_user_channels_resp(
    kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()
) -> 'ok'.
publish_query_user_channels_resp(RespQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?QUERY_USER_CHANNELS_RESP_VALUES, fun query_user_channels_resp/1
    ),
    kz_amqp_util:targeted_publish(RespQ, Payload, ContentType).

-spec publish_query_account_channels_req(kz_term:api_terms()) -> 'ok'.
publish_query_account_channels_req(API) ->
    publish_query_account_channels_req(
        API,
        kz_api:account_id(API),
        ?DEFAULT_CONTENT_TYPE
    ).

-spec publish_query_account_channels_req(
    kz_term:api_terms(), kz_term:ne_binary(), kz_term:ne_binary()
) -> 'ok'.
publish_query_account_channels_req(Req, AccountId, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Req, ?QUERY_ACCOUNT_CHANNELS_REQ_VALUES, fun query_account_channels_req/1
    ),
    kz_amqp_util:callevt_publish(
        ?CALL_EVENT_ROUTING_KEY('status_req', AccountId), Payload, ContentType
    ).
...
-spec publish_query_account_channels_resp(
    kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()
) -> 'ok'.
publish_query_account_channels_resp(RespQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?QUERY_ACCOUNT_CHANNELS_RESP_VALUES, fun query_account_channels_resp/1
    ),
    kz_amqp_util:targeted_publish(RespQ, Payload, ContentType).
...
publish_query_channels_req(ApiProps, ContentType) when is_list(ApiProps) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        ApiProps, ?QUERY_CHANNELS_REQ_VALUES, fun query_channels_req/1
    ),
    kz_amqp_util:callevt_publish(
        ?CALL_EVENT_ROUTING_KEY('status_req', <<"channels">>), Payload, ContentType
    );
...
-spec publish_query_channels_resp(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_query_channels_resp(RespQ, ApiProps, ContentType) when is_list(ApiProps) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        ApiProps, ?QUERY_CHANNELS_RESP_VALUES, fun query_channels_resp/1
    ),
    kz_amqp_util:targeted_publish(RespQ, Payload, ContentType);
...
publish_usurp_control(CallId, JObj, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        JObj, ?CALL_USURP_CONTROL_VALUES, fun usurp_control/1
    ),
    kz_amqp_util:callevt_publish(
        ?CALL_EVENT_ROUTING_KEY('usurp_control', CallId), Payload, ContentType
    ).
...
-spec publish_usurp_publisher(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_usurp_publisher(CallId, JObj, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        JObj, ?PUBLISHER_USURP_CONTROL_VALUES, fun usurp_publisher/1
    ),
    kz_amqp_util:callevt_publish(
        ?CALL_EVENT_ROUTING_KEY('usurp_publisher', CallId), Payload, ContentType
    ).
