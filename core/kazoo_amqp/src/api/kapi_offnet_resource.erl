...
-define(OPTIONAL_OFFNET_RESOURCE_RESP_HEADERS, [
    <<"Error-Message">>,
    <<"Response-Code">>,
    ?KEY_CALL_ID,
    <<"Resource-Response">>,
...
-define(OFFNET_RESOURCE_RESP_VALUES, [
    ...
    case req_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?OFFNET_RESOURCE_REQ_HEADERS, ?OPTIONAL_OFFNET_RESOURCE_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for offnet_resource_req"}
    end;
req(JObj) ->
...
req_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?OFFNET_RESOURCE_REQ_HEADERS, ?OFFNET_RESOURCE_REQ_VALUES, ?OFFNET_RESOURCE_REQ_TYPES
    );
req_v(JObj) ->
...
resp(Prop) when is_list(Prop) ->
    case resp_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?OFFNET_RESOURCE_RESP_HEADERS, ?OPTIONAL_OFFNET_RESOURCE_RESP_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for offnet_resource_resp"}
    end;
resp(JObj) ->
...
resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?OFFNET_RESOURCE_RESP_HEADERS,
        ?OFFNET_RESOURCE_RESP_VALUES,
        ?OFFNET_RESOURCE_RESP_TYPES
    );
resp_v(JObj) ->
...
bind_q(Queue, Props) ->
    _ = [
        kz_amqp_util:bind_q_to_resource(Queue, Key)
...
unbind_q(Queue, Props) ->
    _ = [
        kz_amqp_util:unbind_q_from_resource(Queue, Key)
     || Key <- binding_keys(Props)
    ],
    'ok'.
