...
-define(INBOUND_HEADERS, [
    <<"Message-ID">>,
    <<"Body">>,
    <<"From">>,
    <<"To">>
]).
-define(OPTIONAL_INBOUND_HEADERS, [
    <<"Custom-Vars">>,
    <<"Custom-SIP-Headers">>,
    <<"Request">>,
    <<"Account-ID">>,
...
-define(OUTBOUND_HEADERS, [
    <<"Message-ID">>,
    <<"Body">>,
    <<"From">>,
    <<"To">>
]).
-define(OPTIONAL_OUTBOUND_HEADERS, [
    <<"Custom-Vars">>,
    <<"Request">>,
    <<"Account-ID">>,
    <<"Application-ID">>,
...
-spec inbound(kz_term:api_terms()) ->
    {'ok', iolist()}
    | {'error', string()}.
...
inbound(JObj) ->
    inbound(kz_json:to_proplist(JObj)).
...
inbound_v(JObj) ->
    inbound_v(kz_json:to_proplist(JObj)).

-spec outbound(kz_term:api_terms()) ->
    {'ok', iolist()}
    | {'error', string()}.
...
outbound(JObj) ->
    outbound(kz_json:to_proplist(JObj)).
...
outbound_v(JObj) ->
    outbound_v(kz_json:to_proplist(JObj)).
...
bind_q(Queue, Props, 'undefined') ->
    _ = [
        kz_amqp_util:bind_q_to_exchange(
            Queue, ?BIND_INBOUND_RK(Category, Props), bind_exchange_id(Props)
        )
     || Category <- bind_categories(Props)
    ],
    _ = [
        kz_amqp_util:bind_q_to_exchange(
            Queue, ?BIND_OUTBOUND_RK(Category, Props), bind_exchange_id(Props)
        )
     || Category <- bind_categories(Props)
    ],
    'ok';
...
publish_inbound(Req, AMQPOptions) ->
    ...
    kz_amqp_util:basic_publish(
        Exchange, RK, Payload, ?IM_CONTENT_TYPE, maybe_gzip(Req, AMQPOptions)
    ).
...
-spec outbound_routing_key(kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
outbound_routing_key(Category, RouteId, MsgId) ->
    routing_key(Category, <<"outbound">>, RouteId, MsgId).

-spec inbound_routing_key(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
inbound_routing_key(Category, RouteType, MsgId) ->
    routing_key(Category, <<"inbound">>, RouteType, MsgId).

-spec routing_key(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:ne_binary()
) -> kz_term:ne_binary().
routing_key(Category, Direction, RouteType, MsgId) ->
    Parts = [
        kz_term:to_binary(Category),
        Direction,
