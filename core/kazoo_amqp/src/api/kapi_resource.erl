...
-export([
    publish_originate_req/1, publish_originate_req/2,
...
%% corresponds to eavesdrop_req/1 api call
-define(KEY_EAVESDROP_REQ, <<"eavesdrop.resource.req">>).
...
-define(OPTIONAL_EAVESDROP_RESP_HEADERS, [
    <<"Eavesdropper-Call-ID">>,
    <<"Error-Msg">>
]).
-define(EAVESDROP_RESP_VALUES, [
    {<<"Event-Category">>, <<"resource">>},
...
-define(OPTIONAL_ORIGINATE_REQ_HEADERS, [
    <<"Application-Data">>,
    <<"Custom-Application-Vars">>,
    <<"Custom-Channel-Vars">>,
    % If set, use this node, otherwise ignore
    <<"Existing-Call-ID">>,
    ...
    <<"Loopback-Bowout">>,
    %% loopback_bowout flag
    <<"Simplify-Loopback">>,
    <<"Start-Control-Process">>
    | kapi_dialplan:optional_bridge_req_headers()
]).
-define(ORIGINATE_REQ_VALUES, [
    {<<"Event-Category">>, <<"resource">>},
...
-define(OPTIONAL_ORIGINATE_REQ_ENDPOINT_HEADERS,
    kapi_dialplan:optional_bridge_req_endpoint_headers()
).
-define(ORIGINATE_REQ_ENDPOINT_VALUES, [{<<"Endpoint-Type">>, [<<"sip">>, <<"freetdm">>]}]).
-define(ORIGINATE_REQ_ENDPOINT_TYPES, [
    {<<"Custom-SIP-Headers">>, fun kz_json:is_json_object/1},
    {<<"Custom-Channel-Vars">>, fun kz_json:is_json_object/1},
    {<<"Custom-Application-Vars">>, fun kz_json:is_json_object/1},
...
-define(OPTIONAL_ORIGINATE_RESP_HEADERS, [
    <<"Channel-Call-State">> | kapi_call:optional_call_event_headers()
]).
-define(ORIGINATE_RESP_VALUES, [
    {<<"Event-Category">>, <<"resource">>},
    {<<"Event-Name">>, <<"originate_resp">>}
]).
-define(ORIGINATE_RESP_TYPES, [
    {<<"Custom-Channel-Vars">>, fun kz_json:is_json_object/1},
    {<<"Custom-Application-Vars">>, fun kz_json:is_json_object/1}
]).
...
-define(OPTIONAL_ORIGINATE_STARTED_HEADERS, [
    <<"Channel-Call-State">> | kapi_call:optional_call_event_headers()
]).
-define(ORIGINATE_STARTED_VALUES, [
    {<<"Event-Category">>, <<"resource">>},
    {<<"Event-Name">>, <<"originate_started">>}
]).
-define(ORIGINATE_STARTED_TYPES, [
    {<<"Custom-Channel-Vars">>, fun kz_json:is_json_object/1},
    {<<"Custom-Application-Vars">>, fun kz_json:is_json_object/1}
]).
...
originate_req(Prop) when is_list(Prop) ->
    ...
    case originate_req_v(Prop1) of
        'true' ->
            kz_api:build_message(Prop1, ?ORIGINATE_REQ_HEADERS, ?OPTIONAL_ORIGINATE_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for originate_req"}
    end;
originate_req(JObj) ->
...
originate_req_endpoint_v(Prop) when is_list(Prop) ->
    kz_api:validate_message(
        Prop,
        ?ORIGINATE_REQ_ENDPOINT_HEADERS,
        ?ORIGINATE_REQ_ENDPOINT_VALUES,
        ?ORIGINATE_REQ_ENDPOINT_TYPES
    );
...
originate_resp(Prop) when is_list(Prop) ->
    case originate_resp_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?ORIGINATE_RESP_HEADERS, ?OPTIONAL_ORIGINATE_RESP_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for originate response"}
    end;
originate_resp(JObj) ->
    ...
    case originate_started_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?ORIGINATE_STARTED_HEADERS, ?OPTIONAL_ORIGINATE_STARTED_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for originate started"}
    end;
...
originate_started_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?ORIGINATE_STARTED_HEADERS, ?ORIGINATE_STARTED_VALUES, ?ORIGINATE_STARTED_TYPES
    );
...
originate_uuid(Prop) when is_list(Prop) ->
    case originate_uuid_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?ORIGINATE_UUID_HEADERS, ?OPTIONAL_ORIGINATE_UUID_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for originate uuid"}
    end;
originate_uuid(JObj) ->
...
eavesdrop_req(Prop) when is_list(Prop) ->
    case eavesdrop_req_v(Prop) of
        ...
        'false' ->
            {'error', "Proplist failed validation for eavesdrop request"}
    end;
eavesdrop_req(JObj) ->
...
eavesdrop_resp(Prop) when is_list(Prop) ->
    case eavesdrop_resp_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?EAVESDROP_RESP_HEADERS, ?OPTIONAL_EAVESDROP_RESP_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for eavesdrop response"}
    end;
eavesdrop_resp(JObj) ->
...
publish_originate_resp(TargetQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?ORIGINATE_RESP_VALUES, fun originate_resp/1
    ),
...
-spec publish_originate_started(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_originate_started(TargetQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?ORIGINATE_STARTED_VALUES, fun originate_started/1
    ),
    kz_amqp_util:targeted_publish(TargetQ, Payload, ContentType).
...
publish_originate_uuid(TargetQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?ORIGINATE_UUID_VALUES, fun originate_uuid/1
    ),
    kz_amqp_util:targeted_publish(TargetQ, Payload, ContentType).
...
publish_eavesdrop_resp(TargetQ, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?EAVESDROP_RESP_VALUES, fun eavesdrop_resp/1
    ),
    kz_amqp_util:targeted_publish(TargetQ, Payload, ContentType).
