...
-define(OPTIONAL_AUTHZ_REQ_HEADERS, [
...
-define(AUTHZ_REQ_VALUES, [
    {<<"Event-Category">>, ?EVENT_CATEGORY},
    {<<"Event-Name">>, <<"authz_req">>}
]).
-define(BALANCE_CHECK_REQ_VALUES, [
    {<<"Event-Category">>, ?EVENT_CATEGORY},
    {<<"Event-Name">>, <<"balance_check_req">>}
]).
-define(AUTHZ_REQ_TYPES, [
    {<<"To">>, fun is_binary/1},
    ...
    case balance_check_req_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?BALANCE_CHECK_REQ_HEADERS, ?OPTIONAL_BALANCE_CHECK_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for authz_req"}
    end;
...
balance_check_req_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?BALANCE_CHECK_REQ_HEADERS, ?BALANCE_CHECK_REQ_VALUES, ?BALANCE_CHECK_REQ_TYPES
    );
...
balance_check_resp(Prop) when is_list(Prop) ->
    case balance_check_resp_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?BALANCE_CHECK_RESP_HEADERS, ?OPTIONAL_BALANCE_CHECK_RESP_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for authz_resp"}
    end;
...
balance_check_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?BALANCE_CHECK_RESP_HEADERS, ?BALANCE_CHECK_RESP_VALUES, ?BALANCE_CHECK_RESP_TYPES
    );
...
publish_balance_check_req(Req, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Req, ?BALANCE_CHECK_REQ_VALUES, fun balance_check_req/1
    ),
...
-spec publish_balance_check_resp(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_balance_check_resp(Queue, Resp, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        Resp, ?BALANCE_CHECK_RESP_VALUES, fun balance_check_resp/1
    ),
    kz_amqp_util:targeted_publish(Queue, Payload, ContentType).
