...
-export([
    publish_action/2, publish_action/3,
...
bridge_endpoint(Prop) when is_list(Prop) ->
    case bridge_endpoint_v(Prop) of
        'true' ->
            kz_api:build_message_specific(
                Prop, ?BRIDGE_REQ_ENDPOINT_HEADERS, ?OPTIONAL_BRIDGE_REQ_ENDPOINT_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for bridge_req_endpoint"}
    end;
...
bridge_endpoint_v(Prop) when is_list(Prop) ->
    kz_api:validate_message(
        Prop, ?BRIDGE_REQ_ENDPOINT_HEADERS, ?BRIDGE_REQ_ENDPOINT_VALUES, ?BRIDGE_REQ_ENDPOINT_TYPES
    );
...
page_v(JObj) ->
    page_v(kz_json:to_proplist(JObj)).
...
store(JObj) ->
    store(kz_json:to_proplist(JObj)).
...
store_v(JObj) ->
    store_v(kz_json:to_proplist(JObj)).
...
store_amqp_resp(Prop) when is_list(Prop) ->
    case store_amqp_resp_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?STORE_AMQP_RESP_HEADERS, ?OPTIONAL_STORE_AMQP_RESP_HEADERS);
        'false' ->
            {'error', "Proplist failed validate for store_amqp_resp"}
    end;
store_amqp_resp(JObj) ->
    store_amqp_resp(kz_json:to_proplist(JObj)).

-spec store_amqp_resp_v(kz_term:api_terms()) -> boolean().
store_amqp_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?STORE_AMQP_RESP_HEADERS, ?STORE_AMQP_RESP_VALUES, ?STORE_AMQP_RESP_TYPES
    );
store_amqp_resp_v(JObj) ->
    store_amqp_resp_v(kz_json:to_proplist(JObj)).
...
store_http_resp(Prop) when is_list(Prop) ->
    case store_http_resp_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?STORE_HTTP_RESP_HEADERS, ?OPTIONAL_STORE_HTTP_RESP_HEADERS);
        'false' ->
            {'error', "Proplist failed validate for store_http_resp"}
    end;
store_http_resp(JObj) ->
    store_http_resp(kz_json:to_proplist(JObj)).

-spec store_http_resp_v(kz_term:api_terms()) -> boolean().
store_http_resp_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?STORE_HTTP_RESP_HEADERS, ?STORE_HTTP_RESP_VALUES, ?STORE_HTTP_RESP_TYPES
    );
store_http_resp_v(JObj) ->
    store_http_resp_v(kz_json:to_proplist(JObj)).
...
send_dtmf(JObj) ->
    send_dtmf(kz_json:to_proplist(JObj)).
...
send_dtmf_v(JObj) ->
    send_dtmf_v(kz_json:to_proplist(JObj)).
...
recv_dtmf(JObj) ->
    recv_dtmf(kz_json:to_proplist(JObj)).
...
recv_dtmf_v(JObj) ->
    recv_dtmf_v(kz_json:to_proplist(JObj)).
...
tones_req_tone(Prop) when is_list(Prop) ->
    case tones_req_tone_v(Prop) of
        'true' ->
            kz_api:build_message_specific(
                Prop, ?TONES_REQ_TONE_HEADERS, ?OPTIONAL_TONES_REQ_TONE_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for tones_req_tone"}
    end;
tones_req_tone(JObj) ->
    tones_req_tone(kz_json:to_proplist(JObj)).

-spec tones_req_tone_v(kz_term:api_terms()) -> boolean().
tones_req_tone_v(Prop) when is_list(Prop) ->
    kz_api:validate_message(
        Prop, ?TONES_REQ_TONE_HEADERS, ?TONES_REQ_TONE_VALUES, ?TONES_REQ_TONE_TYPES
    );
tones_req_tone_v(JObj) ->
    tones_req_tone_v(kz_json:to_proplist(JObj)).
...
tone_detect(Prop) when is_list(Prop) ->
    case tone_detect_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?TONE_DETECT_REQ_HEADERS, ?OPTIONAL_TONE_DETECT_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for tone_detect"}
    end;
tone_detect(JObj) ->
    tone_detect(kz_json:to_proplist(JObj)).

-spec tone_detect_v(kz_term:api_terms()) -> boolean().
tone_detect_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?TONE_DETECT_REQ_HEADERS, ?TONE_DETECT_REQ_VALUES, ?TONE_DETECT_REQ_TYPES
    );
tone_detect_v(JObj) ->
    tone_detect_v(kz_json:to_proplist(JObj)).
...
break(JObj) ->
    break(kz_json:to_proplist(JObj)).
...
break_v(JObj) ->
    break_v(kz_json:to_proplist(JObj)).
...
playstop(Prop) when is_list(Prop) ->
    case playstop_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?PLAY_STOP_REQ_HEADERS, ?OPTIONAL_PLAY_STOP_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for playstop"}
    end;
playstop(JObj) ->
    playstop(kz_json:to_proplist(JObj)).
...
playstop_v(JObj) ->
    playstop_v(kz_json:to_proplist(JObj)).
...
playseek(Prop) when is_list(Prop) ->
    case playseek_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?PLAY_SEEK_REQ_HEADERS, ?OPTIONAL_PLAY_SEEK_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for playseek"}
    end;
playseek(JObj) ->
    playseek(kz_json:to_proplist(JObj)).
...
playseek_v(JObj) ->
    playseek_v(kz_json:to_proplist(JObj)).
...
tts_v(JObj) ->
    tts_v(kz_json:to_proplist(JObj)).
...
record_call(Prop) when is_list(Prop) ->
    case record_call_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?RECORD_CALL_REQ_HEADERS, ?OPTIONAL_RECORD_CALL_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for record_call_req"}
    end;
record_call(JObj) ->
    record_call(kz_json:to_proplist(JObj)).

-spec record_call_v(kz_term:api_terms()) -> boolean().
record_call_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?RECORD_CALL_REQ_HEADERS, ?RECORD_CALL_REQ_VALUES, ?RECORD_CALL_REQ_TYPES
    );
record_call_v(JObj) ->
    record_call_v(kz_json:to_proplist(JObj)).
...
answer(JObj) ->
    answer(kz_json:to_proplist(JObj)).
...
answer_v(JObj) ->
    answer_v(kz_json:to_proplist(JObj)).
...
echo_v(JObj) ->
    echo_v(kz_json:to_proplist(JObj)).
...
privacy(JObj) ->
    privacy(kz_json:to_proplist(JObj)).
...
privacy_v(JObj) ->
    privacy_v(kz_json:to_proplist(JObj)).
...
progress(JObj) ->
    progress(kz_json:to_proplist(JObj)).
...
ring_v(JObj) ->
    ring_v(kz_json:to_proplist(JObj)).
...
receive_fax(JObj) ->
    receive_fax(kz_json:to_proplist(JObj)).
...
receive_fax_v(JObj) ->
    receive_fax_v(kz_json:to_proplist(JObj)).
...
soft_hold(Prop) when is_list(Prop) ->
    case soft_hold_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?SOFT_HOLD_REQ_HEADERS, ?OPTIONAL_SOFT_HOLD_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for hold_req"}
    end;
soft_hold(JObj) ->
    ...
    case play_and_collect_digits_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?PLAY_COLLECT_DIGITS_REQ_HEADERS, ?OPTIONAL_PLAY_COLLECT_DIGITS_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for play_collect_digits_req"}
    end;
...
play_and_collect_digits_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?PLAY_COLLECT_DIGITS_REQ_HEADERS,
        ?PLAY_COLLECT_DIGITS_REQ_VALUES,
        ?PLAY_COLLECT_DIGITS_REQ_TYPES
    );
...
call_pickup(Prop) when is_list(Prop) ->
    case call_pickup_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?CALL_PICKUP_REQ_HEADERS, ?OPTIONAL_CALL_PICKUP_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for call_pickup_req"}
    end;
call_pickup(JObj) ->
...
call_pickup_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?CALL_PICKUP_REQ_HEADERS, ?CALL_PICKUP_REQ_VALUES, ?CALL_PICKUP_REQ_TYPES
    );
call_pickup_v(JObj) ->
...
connect_leg(Prop) when is_list(Prop) ->
    case connect_leg_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?CONNECT_LEG_REQ_HEADERS, ?OPTIONAL_CONNECT_LEG_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for connect_leg_req"}
    end;
connect_leg(JObj) ->
...
connect_leg_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?CONNECT_LEG_REQ_HEADERS, ?CONNECT_LEG_REQ_VALUES, ?CONNECT_LEG_REQ_TYPES
    );
connect_leg_v(JObj) ->
...
eavesdrop(Prop) when is_list(Prop) ->
    case eavesdrop_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?EAVESDROP_REQ_HEADERS, ?OPTIONAL_EAVESDROP_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for eavesdrop_req"}
    end;
eavesdrop(JObj) ->
...
execute_extension(Prop) when is_list(Prop) ->
    case execute_extension_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?EXECUTE_EXTENSION_REQ_HEADERS, ?OPTIONAL_EXECUTE_EXTENSION_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for execute_extension_req"}
    end;
...
execute_extension_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop,
        ?EXECUTE_EXTENSION_REQ_HEADERS,
        ?EXECUTE_EXTENSION_REQ_VALUES,
        ?EXECUTE_EXTENSION_REQ_TYPES
    );
...
conference(Prop) when is_list(Prop) ->
    case conference_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?CONFERENCE_REQ_HEADERS, ?OPTIONAL_CONFERENCE_REQ_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for conference_req"}
    end;
conference(JObj) ->
    conference(kz_json:to_proplist(JObj)).
...
conference_v(JObj) ->
    conference_v(kz_json:to_proplist(JObj)).
...
originate_ready(Prop) when is_list(Prop) ->
    case originate_ready_v(Prop) of
        'true' ->
            kz_api:build_message(Prop, ?ORIGINATE_READY_HEADERS, ?OPTIONAL_ORIGINATE_READY_HEADERS);
        'false' ->
            {'error', "Proplist failed validation for originate_ready"}
    end;
...
originate_ready_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?ORIGINATE_READY_HEADERS, ?ORIGINATE_READY_VALUES, ?ORIGINATE_READY_TYPES
    );
...
originate_execute(Prop) when is_list(Prop) ->
    case originate_execute_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?ORIGINATE_EXECUTE_HEADERS, ?OPTIONAL_ORIGINATE_EXECUTE_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for originate_execute"}
    end;
...
originate_execute_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?ORIGINATE_EXECUTE_HEADERS, ?ORIGINATE_EXECUTE_VALUES, ?ORIGINATE_EXECUTE_TYPES
    );
...
publish_error(CallID, API, ContentType) ->
    ...
    kz_amqp_util:callevt_publish(
        kapi_call:event_routing_key(<<"dialplan">>, CallID), Payload, ContentType
    ).
...
-spec publish_originate_ready(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_originate_ready(ServerId, API, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        API, ?ORIGINATE_READY_VALUES, fun originate_ready/1
    ),
...
-spec publish_originate_execute(kz_term:ne_binary(), kz_term:api_terms(), kz_term:ne_binary()) ->
    'ok'.
publish_originate_execute(ServerId, API, ContentType) ->
    {'ok', Payload} = kz_api:prepare_api_payload(
        API, ?ORIGINATE_EXECUTE_VALUES, fun originate_execute/1
    ),
    kz_amqp_util:targeted_publish(ServerId, Payload, ContentType).
...
fax_detection(Prop) when is_list(Prop) ->
    case fax_detection_v(Prop) of
        'true' ->
            kz_api:build_message(
                Prop, ?FAX_DETECTION_REQ_HEADERS, ?OPTIONAL_FAX_DETECTION_REQ_HEADERS
            );
        'false' ->
            {'error', "Proplist failed validation for tone_detect"}
    end;
fax_detection(JObj) ->
    fax_detection(kz_json:to_proplist(JObj)).

-spec fax_detection_v(kz_term:api_terms()) -> boolean().
fax_detection_v(Prop) when is_list(Prop) ->
    kz_api:validate(
        Prop, ?FAX_DETECTION_REQ_HEADERS, ?FAX_DETECTION_REQ_VALUES, ?FAX_DETECTION_REQ_TYPES
    );
fax_detection_v(JObj) ->
    fax_detection_v(kz_json:to_proplist(JObj)).
...
store_vm(JObj) ->
    store_vm(kz_json:to_proplist(JObj)).
...
store_vm_v(JObj) ->
    store_vm_v(kz_json:to_proplist(JObj)).
...
transfer(JObj) ->
    transfer(kz_json:to_proplist(JObj)).
...
transfer_v(JObj) ->
    transfer_v(kz_json:to_proplist(JObj)).
...
media_macro(JObj) ->
    media_macro(kz_json:to_proplist(JObj)).
...
media_macro_v(JObj) ->
    media_macro_v(kz_json:to_proplist(JObj)).
...
play_macro_v(JObj) ->
    play_macro_v(kz_json:to_proplist(JObj)).
...
sound_touch(JObj) ->
    sound_touch(kz_json:to_proplist(JObj)).
...
sound_touch_v(JObj) ->
    sound_touch_v(kz_json:to_proplist(JObj)).
