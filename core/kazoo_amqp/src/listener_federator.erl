...
%% @end
%%------------------------------------------------------------------------------
-spec start_link(pid(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'gen_listener', {'is_consuming', 'false'}},
...
handle_event(JObj, Props, #state{
    ...
    lager:debug(
        "relaying federated ~s event (~p) ~s from ~s to ~p with consumer pid ~p",
        [
            kz_api:event_category(JObj),
            kz_api:msg_id(JObj),
    ...
    RemoteServerId = list_to_binary([
        "consumer://",
        Self,
        "/",
        kz_api:server_id(JObj, <<>>)
    ]),
    gen_listener:federated_event(
        Parent,
        kz_json:set_values(
            [
                {<<"Server-ID">>, RemoteServerId},
                {<<"AMQP-Broker">>, Broker},
...
terminate(_Reason, _State) -> lager:debug("listener federator terminating: ~p", [_Reason]).
