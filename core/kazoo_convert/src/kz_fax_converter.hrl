...
-define(CONVERT_PDF_CMD, <<
    ...
    "-sOutputFile=$TO -- $FROM"
>>).
-define(RESAMPLE_IMAGE_CMD, <<
    "convert $FROM "
    "-resample 200x200 "
    "-page +0+0 "
    "-compress group4 $TO"
>>).
-define(CONVERT_IMAGE_CMD, <<
    "convert $FROM "
    "-resample 200x200 "
    "-units PixelsPerInch "
    "-size 1728x2200 "
...
-define(RESAMPLE_IMAGE_COMMAND,
    kapps_config:get_binary(
        ?CONFIG_CAT, [<<"fax">>, <<"resample_image_command">>], ?RESAMPLE_IMAGE_CMD
    )
).
-define(CONVERT_IMAGE_COMMAND,
    kapps_config:get_binary(
        ?CONFIG_CAT, [<<"fax">>, <<"convert_image_command">>], ?CONVERT_IMAGE_CMD
    )
).
...
-define(VALIDATE_TIFF_COMMAND,
    kapps_config:get_binary(
        ?CONFIG_CAT, [<<"fax">>, <<"validate_tiff_command">>], ?VALIDATE_TIFF_CMD
    )
).
-define(CONVERT_OPENOFFICE_COMMAND,
    kapps_config:get_binary(
        ?CONFIG_CAT, [<<"fax">>, <<"convert_openoffice_command">>], ?CONVERT_OPENOFFICE_CMD
    )
).
...
-define(CONVERT_TIMEOUT,
    kapps_config:get_integer(
        ?CONFIG_CAT, [<<"fax">>, <<"convert_command_timeout">>], 180 * ?MILLISECONDS_IN_SECOND
    )
).
