...
-export([
    set_custom_publish_function/2,
    clear_custom_publish_function/1,
...
-export([
    start_recording/1, start_recording/2,
...
-record(kapps_call, {
    ...
    %% A function used when requesting the call id, to ensure it is up-to-date
    call_id_helper = fun default_helper_function/2 :: kapps_helper_function(),
    %% need to track the originating call id, if found
    origination_call_id :: kz_term:api_ne_binary(),
    context :: kz_term:api_ne_binary(),
    %% The control queue provided on route win
    control_q :: kz_term:api_binary(),
    %% A function used when requesting the call id, to ensure it is up-to-date
    control_q_helper = fun default_helper_function/2 :: kapps_helper_function(),
    %%
    controller_q :: kz_term:api_binary(),
    %% The caller name
    caller_id_name :: kz_term:api_ne_binary(),
    %% The caller number
    caller_id_number :: kz_term:api_ne_binary(),
    %% The callee name
    callee_id_name :: kz_term:api_binary(),
    %% The callee number
    callee_id_number :: kz_term:api_binary(),
    %% The switch node name (as known in ecallmgr)
    switch_nodename = <<>> :: binary(),
    %% The switch hostname (as reported by the switch)
    switch_hostname :: kz_term:api_ne_binary(),
    %% The switch url
    switch_url :: kz_term:api_binary(),
    %% The switch uri
    switch_uri :: kz_term:api_binary(),
    %% The request of sip_request_user + @ + sip_request_host
    request = ?NO_USER_REALM :: kz_term:ne_binary(),
    %% SIP request user
    request_user = ?NO_USER :: kz_term:ne_binary(),
    %% SIP request host
    request_realm = ?NO_REALM :: kz_term:ne_binary(),
    %% Result of sip_from_user + @ + sip_from_host
    from = ?NO_USER_REALM :: kz_term:ne_binary(),
    %% SIP from user
    from_user = ?NO_USER :: kz_term:ne_binary(),
    %% SIP from host
    from_realm = ?NO_REALM :: kz_term:ne_binary(),
    %% Result of sip_to_user + @ + sip_to_host
    to = ?NO_USER_REALM :: kz_term:ne_binary(),
    %% SIP to user
    to_user = ?NO_USER :: kz_term:ne_binary(),
    %% SIP to host
    to_realm = ?NO_REALM :: kz_term:ne_binary(),
    %% Origin of the call <<"onnet">> | <<"offnet">>
    inception :: kz_term:api_binary(),
    %% The database name of the account that authorized this call
    account_db :: kz_term:api_binary(),
    %% The account id that authorized this call
    account_id :: kz_term:api_binary(),
    %% The ID of the record that authorized this call
    authorizing_id :: kz_term:api_binary(),
    %% The pvt_type of the record that authorized this call
    authorizing_type :: kz_term:api_binary(),
    %% The ID of the owner of this calling device, if any
    owner_id :: kz_term:api_binary(),
    %% The Fetch ID of the Call
    fetch_id :: kz_term:api_binary(),
    %% The Bridge ID of the Call
    bridge_id :: kz_term:api_binary(),
    %% Language of the call to use
    language :: kz_term:api_binary(),
    %% The application name used during kapps_call_command
    app_name = <<"kapps_call">> :: kz_term:ne_binary(),
    %% The application version used during kapps_call_command
    app_version = <<"1.0.0">> :: kz_term:ne_binary(),
    %% A custom command used to publish kapps_call_command
    custom_publish_fun :: kapps_custom_publish() | 'undefined',
    %% Any custom channel vars that where provided with the route request
    ccvs = kz_json:new() :: kz_json:object(),
    %% Any custom application vars that where provided with the route request
    cavs = kz_json:new() :: kz_json:object(),
    %% Custom SIP Headers
    sip_headers = kz_json:new() :: kz_json:object(),
    %% allows callflows to set values that propagate to children
    ...
    %% Specified during call termination whether the call had been bridged
    call_bridged = 'false' :: boolean(),
    %% Specified during call termination whether the caller left a voicemail message
    message_left = 'false' :: boolean(),
    %% Control account level recording
    is_recording = 'false' :: boolean(),
    %% is this call forward call
    is_call_forward = 'false' :: boolean()
}).
...
clear_helpers(#kapps_call{} = Call) ->
    Fs = [
        fun clear_custom_publish_function/1,
...
put_callid(#kapps_call{call_id = CallId}) -> kz_util:put_callid(CallId).
...
from_route_req(
    ...
    Call1#kapps_call{
        call_id = CallId,
        origination_call_id = kz_json:get_ne_binary_value(
            <<"Origination-Call-ID">>, RouteReq, origination_call_id(Call1)
        ),
        ...
        switch_hostname = kz_json:get_ne_binary_value(
            <<"Switch-Hostname">>, RouteReq, switch_hostname(Call)
        ),
        switch_nodename = kz_json:get_ne_binary_value(
            <<"Switch-Nodename">>, RouteReq, switch_nodename(Call)
        ),
        ...
        authorizing_id = kz_json:get_ne_binary_value(
            <<"Authorizing-ID">>, CCVs, authorizing_id(Call)
        ),
        authorizing_type = kz_json:get_ne_binary_value(
            <<"Authorizing-Type">>, CCVs, authorizing_type(Call)
        ),
        ...
        caller_id_name = kz_json:get_binary_value(
            <<"Caller-ID-Name">>, RouteReq, caller_id_name(Call)
        ),
        callee_id_name = kz_json:get_binary_value(
            <<"Callee-ID-Name">>, RouteReq, callee_id_name(Call)
        ),
        caller_id_number = kz_json:get_binary_value(
            <<"Caller-ID-Number">>, RouteReq, caller_id_number(Call)
        ),
        callee_id_number = kz_json:get_binary_value(<<"Callee-ID-Number">>, RouteReq, ToUser),
        ccvs = CCVs,
        cavs = CAVs,
        sip_headers = SHs,
        resource_type = kz_json:get_ne_binary_value(
            <<"Resource-Type">>, RouteReq, resource_type(Call)
        ),
...
from_route_win(
    ...
    {AccountId, AccountDb} =
        find_account_info(
            OldAccountId, OldAccountDb, kz_json:get_ne_binary_value(<<"Account-ID">>, CCVs)
        ),
...
find_account_info('undefined', _OldDb, AccountId) ->
    {AccountId, kz_util:format_account_id(AccountId, 'encoded')};
...
merge(OldJObj, JObj) -> kz_json:merge(OldJObj, JObj).
...
from_originate_uuid(JObj, #kapps_call{} = Call) ->
    ...
    Call#kapps_call{
        control_q = kz_json:get_value(<<"Outbound-Call-Control-Queue">>, JObj, control_queue(Call)),
...
from_originate_ready(JObj, #kapps_call{} = Call) ->
    ...
    Call#kapps_call{
        control_q = kz_json:get_value(<<"Control-Queue">>, JObj, control_queue(Call)),
...
from_json(
    ...
    CCVs = kz_json:merge(
        OldCCVs, kz_json:get_json_value(<<"Custom-Channel-Vars">>, JObj, kz_json:new())
    ),
    CAVs = kz_json:merge(
        OldCAVs, kz_json:get_json_value(<<"Custom-Application-Vars">>, JObj, kz_json:new())
    ),
    SHs = kz_json:merge(OldSHs, kz_json:get_value(<<"Custom-SIP-Headers">>, JObj, kz_json:new())),
    KVS = orddict:from_list(
        kz_json:to_proplist(kz_json:get_value(<<"Key-Value-Store">>, JObj, kz_json:new()))
    ),
    Call#kapps_call{
        call_id = kz_json:get_ne_binary_value(<<"Call-ID">>, JObj, call_id_direct(Call)),
        origination_call_id = kz_json:get_ne_binary_value(
            <<"Origination-Call-ID">>, JObj, origination_call_id(Call)
        ),
        context = kz_json:get_ne_binary_value(<<"Context">>, JObj, context(Call)),
        control_q = kz_json:get_ne_binary_value(
            <<"Control-Queue">>, JObj, control_queue_direct(Call)
        ),
        controller_q = kz_json:get_ne_binary_value(
            <<"Controller-Queue">>, JObj, controller_queue(Call)
        ),
        caller_id_name = kz_json:get_ne_binary_value(
            <<"Caller-ID-Name">>, JObj, caller_id_name(Call)
        ),
        caller_id_number = kz_json:get_ne_binary_value(
            <<"Caller-ID-Number">>, JObj, caller_id_number(Call)
        ),
        callee_id_name = kz_json:get_ne_binary_value(
            <<"Callee-ID-Name">>, JObj, callee_id_name(Call)
        ),
        callee_id_number = kz_json:get_ne_binary_value(
            <<"Callee-ID-Number">>, JObj, callee_id_number(Call)
        ),
        ...
        switch_hostname = kz_json:get_ne_binary_value(
            <<"Switch-Hostname">>, JObj, switch_hostname(Call)
        ),
        switch_nodename = kz_json:get_ne_binary_value(
            <<"Switch-Nodename">>, JObj, switch_nodename(Call)
        ),
        ...
        authorizing_id = kz_json:get_ne_binary_value(
            <<"Authorizing-ID">>, JObj, authorizing_id(Call)
        ),
        authorizing_type = kz_json:get_ne_binary_value(
            <<"Authorizing-Type">>, JObj, authorizing_type(Call)
        ),
        ...
        app_version = kz_json:get_ne_binary_value(
            <<"App-Version">>, JObj, application_version(Call)
        ),
        ccvs = CCVs,
        cavs = CAVs,
        sip_headers = SHs,
        kvs = orddict:merge(fun(_, _, V2) -> V2 end, Kvs, KVS),
        other_leg_call_id = kz_json:get_ne_binary_value(
            <<"Other-Leg-Call-ID">>, JObj, other_leg_call_id(Call)
        ),
...
to_json(#kapps_call{} = Call) ->
    ...
    kz_json:from_list([
        KV
     || {_, V} = KV <- [
            {<<"Key-Value-Store">>, kz_json:from_list(KVS)}
...
control_queue(#kapps_call{control_q = ControlQ, control_q_helper = Fun} = Call) when
    is_function(Fun, 2)
->
    Fun(ControlQ, Call);
...
maybe_format_caller_id(Call, 'undefined') ->
    Call;
...
maybe_format_caller_id_str(Cid, 'undefined') ->
    Cid;
...
maybe_reformat_caller_id(CallerId, 'undefined') ->
    CallerId;
...
-spec maybe_regex_caller_id(kz_term:ne_binary(), kz_term:api_binary(), kz_json:object()) ->
    kz_term:ne_binary().
maybe_regex_caller_id(CallerId, 'undefined', _) ->
    CallerId;
maybe_regex_caller_id(CallerId, Regex, Format) ->
    ...
    case re:run(Normalized, Regex, [{'capture', 'all_but_first', 'binary'}]) of
        {'match', [UseCid | _]} ->
            lager:info(
                "cid rewrite match found ~s from normalized caller id ~s",
                [UseCid, Normalized]
            ),
...
maybe_prepend_caller_id(CallerId, 'undefined') ->
    CallerId;
...
maybe_append_caller_id(CallerId, 'undefined') ->
    CallerId;
...
set_caller_id(CIDNumber, CIDName, #kapps_call{} = Call) when
    is_binary(CIDNumber) andalso
        is_binary(CIDName)
->
    Call#kapps_call{
...
->
    ...
    JObj = kz_json:from_list([
        ...
        {<<"Privacy-Hide-Name">>, kz_privacy:should_hide_name(CCVs)},
        {<<"Privacy-Hide-Number">>, kz_privacy:should_hide_number(CCVs)}
    ]),
...
unknown_caller_id_name('undefined') ->
    kapps_config:get_ne_binary(
        ?CALL_CMD_CAT, ?UNKNOWN_CALLER_ID_NAME_KEY, ?DEFAULT_UNKNOWN_CALLER_ID_NAME
    );
unknown_caller_id_name(AccountId) ->
    kapps_account_config:get_global(
        AccountId, ?CALL_CMD_CAT, ?UNKNOWN_CALLER_ID_NAME_KEY, ?DEFAULT_UNKNOWN_CALLER_ID_NAME
    ).
...
-ifdef(TEST).
set_callee_id(CIDNumber, CIDName, #kapps_call{} = Call) when
    is_binary(CIDNumber) andalso
        is_binary(CIDName)
->
    Call#kapps_call{
...
set_callee_id_number(CIDNumber, #kapps_call{} = Call) when is_binary(CIDNumber) ->
    kapps_call_command:set(
        kz_json:from_list([{<<"Callee-ID-Number">>, CIDNumber}]), 'undefined', Call
    ),
    Call#kapps_call{callee_id_number = CIDNumber}.
-endif.
...
to_e164(Number, 'undefined') -> knm_converters:normalize(Number);
to_e164(Number, AccountId) -> knm_converters:normalize(Number, AccountId).
-endif.
...
request_user(#kapps_call{
    request = ?NO_USER_REALM,
...
set_resource_type(ResourceType, #kapps_call{} = Call) ->
    insert_custom_channel_var(<<"Resource-Type">>, ResourceType, Call#kapps_call{
...
set_account_db(<<_/binary>> = AccountDb, #kapps_call{} = Call) ->
    ...
    set_custom_channel_var(<<"Account-ID">>, AccountId, Call#kapps_call{
        account_db = AccountDb,
        account_id = AccountId
    }).

-spec account_db(call()) -> kz_term:api_ne_binary().
account_db(#kapps_call{
...
set_authorizing_id(AuthorizingId, #kapps_call{} = Call) when is_binary(AuthorizingId) ->
    set_custom_channel_var(<<"Authorizing-ID">>, AuthorizingId, Call#kapps_call{
...
set_authorizing_type(AuthorizingType, #kapps_call{} = Call) when is_binary(AuthorizingType) ->
    set_custom_channel_var(<<"Authorizing-Type">>, AuthorizingType, Call#kapps_call{
...
set_authorization(AuthorizingType, AuthorizingId, #kapps_call{} = Call) when
    is_binary(AuthorizingType) andalso
        is_binary(AuthorizingId)
->
    set_custom_channel_vars(
        [
...
language(#kapps_call{language = Language}) ->
    Language.
-endif.

-spec get_prompt(call(), kz_term:api_ne_binary()) -> kz_term:api_ne_binary().
get_prompt(#kapps_call{}, 'undefined') -> 'undefined';
get_prompt(#kapps_call{} = Call, Media) -> get_prompt(Call, Media, language(Call)).

-spec get_prompt(call(), kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
    kz_term:api_ne_binary().
get_prompt(_Call, 'undefined', _Lang) ->
    'undefined';
...
-spec update_custom_channel_vars([fun((kz_json:object()) -> kz_json:object()), ...], call()) ->
    call().
-ifdef(TEST).
...
kvs_erase(Keys, #kapps_call{kvs = Dict} = Call) when is_list(Keys) ->
    Call#kapps_call{
        kvs = lists:foldl(fun(K, D) -> orddict:erase(kz_term:to_binary(K), D) end, Dict, Keys)
    };
...
kvs_fetch(Key, Default, #kapps_call{kvs = Dict}) ->
    try
        orddict:fetch(kz_term:to_binary(Key), Dict)
    catch
...
start_recording('undefined', Call) ->
    Call;
