...
initialize_state(Call, Data) ->
    ...
    AccountMODb = kz_util:format_account_modb(
        kazoo_modb:get_modb(AccountId, Year, Month), 'encoded'
    ),
...
handle_cast('channel_destroyed', State) ->
    ...
    {'noreply', State};
handle_cast(
    {'record_stop', {_, MediaName} = Media, FS, EventJObj},
    #state{
        media = {_, MediaName},
        is_recording = 'true',
        stop_received = 'false',
...
handle_cast(
    {'record_stop', {_, MediaName}, _FS, _JObj},
    #state{
        media = {_, MediaName},
        is_recording = 'false',
        stop_received = 'false',
...
handle_cast('maybe_start_recording_on_bridge', #state{is_recording = 'true'} = State) ->
    {'noreply', State};
handle_cast(
    'maybe_start_recording_on_bridge',
    #state{
        is_recording = 'false',
...
handle_cast('maybe_start_recording_on_answer', #state{is_recording = 'true'} = State) ->
    {'noreply', State};
handle_cast(
    'maybe_start_recording_on_answer',
    #state{
        is_recording = 'false',
...
handle_cast({'gen_listener', {'created_queue', Queue}}, #state{call = Call} = State) ->
    Funs = [
        {fun kapps_call:kvs_store/3, 'consumer_pid', self()},
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'gen_listener', {'is_consuming', 'true'}}, #state{is_recording = 'true', call = Call} = State
) ->
...
store_recording_meta(#state{
    ...
    MediaDoc = kz_doc:update_pvt_parameters(BaseMediaDoc, AccountMODb, [
...
-spec maybe_store_recording_meta(state()) ->
    {'ok', kzd_call_recording:doc()}
    | {'error', any()}.
...
handler_fields_for_protocol(
    ...
    {S1, S2} = check_url(Url),
    [
        {'const', <<S1/binary, "call_recording_">>},
        {'field', <<"call_id">>},
        <<".", Ext/binary>>,
        {'const', <<S2/binary, "from=">>},
        {'field', <<"from">>},
        {'const', <<"&to=">>},
        {'field', <<"to">>},
        {'const', <<"&caller_id_name=">>},
...
-spec store_recording(
    {kz_term:ne_binary(), kz_term:ne_binary()}, kapps_call_command:store_fun(), kapps_call:call()
) ->
    pid().
...
-spec store_recording(
    pid(), kz_term:ne_binary(), kapps_call_command:store_fun(), kapps_call:call()
) -> 'ok'.
store_recording(Pid, Filename, StoreUrl, Call) ->
    case kapps_call_command:store_file(Filename, StoreUrl, Call) of
        ...
        'ok' ->
            gen_server:cast(Pid, 'store_succeeded')
    end.
...
start_recording_timer(TimeLimit) ->
    ...
    erlang:start_timer(
        TimeLimit * ?MILLISECONDS_IN_SECOND,
        self(),
...
start_recording_stop_timer() ->
    ...
    erlang:start_timer(
        ?RECORDING_STOP_TIMER_EXPIRED_MS,
        self(),
...
handle_channel_status_resp(JObj, Pid) ->
    case kz_json:get_ne_binary_value(<<"Status">>, JObj) of
        <<"active">> ->
            lager:info("channel is still active");
        _Status ->
