...
-define(BRIDGE_USER,
    kapps_config:get_ne_binary(<<"conferences">>, <<"bridge_username">>, kz_binary:rand_hex(12))
).
-define(BRIDGE_PWD,
    kapps_config:get_ne_binary(<<"conferences">>, <<"bridge_password">>, kz_binary:rand_hex(12))
).
...
do_from_json(JObj, Conference) ->
    ...
    Conference#kapps_conference{
        id = ConferenceId,
        name = ConferenceName,
        profile_name = kz_json:get_ne_binary_value(
            <<"Profile-Name">>, JObj, profile_name(Conference)
        ),
        ...
        controller_q = kz_json:get_ne_binary_value(
            <<"Controller-Queue">>, JObj, controller_queue(Conference)
        ),
        bridge_username = kz_json:get_ne_binary_value(
            <<"Bridge-Username">>, JObj, bridge_username(Conference)
        ),
        bridge_password = kz_json:get_ne_binary_value(
            <<"Bridge-Password">>, JObj, bridge_password(Conference)
        ),
        member_pins = kz_json:get_list_value(<<"Member-Pins">>, JObj, member_pins(Conference)),
        moderator_pins = kz_json:get_list_value(
            <<"Moderator-Pins">>, JObj, moderator_pins(Conference)
        ),
        moderator = kz_json:is_true(<<"Moderator">>, JObj, moderator(Conference)),
        member_join_muted = kz_json:is_true(
            <<"Member-Join-Muted">>, JObj, member_join_muted(Conference)
        ),
        member_join_deaf = kz_json:is_true(
            <<"Member-Join-Deaf">>, JObj, member_join_deaf(Conference)
        ),
        moderator_join_muted = kz_json:is_true(
            <<"Moderator-Join-Muted">>, JObj, moderator_join_muted(Conference)
        ),
        moderator_join_deaf = kz_json:is_true(
            <<"Moderator-Join-Deaf">>, JObj, moderator_join_deaf(Conference)
        ),
        max_participants = kz_json:get_integer_value(
            <<"Max-Participants">>, JObj, max_participants(Conference)
        ),
        max_members_media = kz_json:get_ne_binary_value(
            <<"Max-Members-Media">>, JObj, max_members_media(Conference)
        ),
        require_moderator = kz_json:is_true(
            <<"Require-Moderator">>, JObj, require_moderator(Conference)
        ),
        wait_for_moderator = kz_json:is_true(
            <<"Wait-For-Moderator">>, JObj, wait_for_moderator(Conference)
        ),
        play_name_on_join = kz_json:is_true(
            <<"Play-Name-On-Join">>, JObj, play_name_on_join(Conference)
        ),
        play_entry_prompt = kz_json:is_true(
            <<"Play-Entry-Prompt">>, JObj, play_entry_prompt(Conference)
        ),
        play_exit_tone = get_tone(
            kz_json:get_value(<<"Play-Exit-Tone">>, JObj, play_exit_tone(Conference))
        ),
        play_entry_tone = get_tone(
            kz_json:get_value(<<"Play-Entry-Tone">>, JObj, play_entry_tone(Conference))
        ),
        ...
        moderator_controls = kz_json:get_ne_binary_value(
            <<"Moderator-Controls">>, JObj, moderator_controls(Conference)
        ),
        caller_controls = kz_json:get_ne_binary_value(
            <<"Caller-Controls">>, JObj, caller_controls(Conference)
        ),
        discovery_request = JObj
    }.

-spec load_call(kz_json:object(), kapps_call:call() | 'undefined') ->
...
reload(#kapps_conference{account_id = undefined} = Conference) ->
    Conference;
reload(#kapps_conference{id = undefined} = Conference) ->
    Conference;
reload(
    #kapps_conference{
    ...
    case kz_datamgr:open_cache_doc(AccountDB, ConferenceId) of
        ...
        {error, Error} ->
            lager:debug(
                "conference document ~s/~s not found => ~p",
                [
                    AccountId,
                    ConferenceId,
                    Error
...
from_conference_doc(JObj, Conference) ->
    ...
    Conference#kapps_conference{
        ...
        play_entry_prompt = kzd_conferences:member_play_entry_prompt(
            JObj, play_entry_prompt(Conference)
        ),
        play_exit_tone = get_tone(kzd_conferences:play_exit_tone(JObj, play_exit_tone(Conference))),
        play_entry_tone = get_tone(
            kzd_conferences:play_entry_tone(JObj, play_entry_tone(Conference))
        ),
        play_welcome = kzd_conferences:play_welcome(JObj, play_welcome(Conference)),
        moderator_join_muted = kzd_conferences:moderator_join_muted(
            JObj, moderator_join_muted(Conference)
        ),
        moderator_join_deaf = kzd_conferences:moderator_join_deaf(
            JObj, moderator_join_deaf(Conference)
        ),
        ...
        wait_for_moderator = kzd_conferences:wait_for_moderator(
            JObj, wait_for_moderator(Conference)
        ),
        controls = kzd_conferences:controls(JObj, raw_controls(Conference)),
        moderator_controls = kzd_conferences:moderator_controls(
            JObj, moderator_controls(Conference)
        ),
...
controls(#kapps_conference{controls = Controls} = Conference, ControlsName) when
    Controls =/= 'undefined'
->
...
controls(#kapps_conference{account_id = 'undefined'}, ?DEFAULT_PROFILE_NAME) ->
    kapps_config:get(
        ?CONFERENCE_CONFIG_CAT, [<<"controls">>, ?DEFAULT_PROFILE_NAME], ?DEFAULT_CONTROLS
    );
controls(#kapps_conference{account_id = AccountId} = Conference, ?DEFAULT_PROFILE_NAME) ->
    case
        kapps_account_config:get_global(AccountId, ?CONFERENCE_CONFIG_CAT, [
            <<"controls">>, ?DEFAULT_PROFILE_NAME
        ])
    of
        'undefined' ->
            controls(Conference#kapps_conference{account_id = 'undefined'}, ?DEFAULT_PROFILE_NAME);
        Controls ->
            Controls
    end;
...
controls(#kapps_conference{account_id = AccountId} = Conference, ?PAGE_PROFILE_NAME) ->
    case
        kapps_account_config:get_global(AccountId, ?CONFERENCE_CONFIG_CAT, [
            <<"controls">>, ?PAGE_PROFILE_NAME
        ])
    of
        'undefined' ->
            controls(Conference#kapps_conference{account_id = 'undefined'}, ?PAGE_PROFILE_NAME);
        Controls ->
            Controls
    end;
...
controls(#kapps_conference{account_id = AccountId} = Conference, ControlsName) ->
    ...
    of
        'undefined' ->
            controls(Conference#kapps_conference{account_id = 'undefined'}, ControlsName);
        Controls ->
            Controls
    end.
...
build_conference_profile(
    #kapps_conference{
        profile_name = ?PAGE_PROFILE_NAME,
        account_id = 'undefined'
    } = Conference
) ->
...
build_conference_profile(
    ...
    of
        'undefined' ->
            build_conference_profile(Conference#kapps_conference{
                profile_name = ?DEFAULT_PROFILE_NAME
            });
        Profile ->
            {ProfileName, Profile}
...
build_account_profile(Conference) ->
    ...
    LanguageProfile = kapps_account_config:get_global(AccountId, ?CONFERENCE_CONFIG_CAT, [
        <<"profiles">>, Language, ProfileName
    ]),
    Profile = kapps_account_config:get_global(AccountId, ?CONFERENCE_CONFIG_CAT, [
...
build_system_profile(Conference) ->
    ...
    LanguageProfile = kapps_config:get_json(
        ?CONFERENCE_CONFIG_CAT,
        [<<"profiles">>, Language, ProfileName],
        default_profile(Language, 'undefined')
    ),

    case
        kapps_config:get_json(
            ?CONFERENCE_CONFIG_CAT,
            [<<"profiles">>, ProfileName]
        )
    of
...
-spec update_profile_language(kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:proplist()) ->
    kz_term:proplist().
...
cache(#kapps_conference{id = ConferenceId} = Conference, Expires) ->
    ...
    kz_cache:store_local(
        ?KAPPS_CALL_CACHE, {?MODULE, 'conference', ConferenceId}, Conference, CacheProps
    ).
...
get_tone(Thing) ->
    case kz_term:is_boolean(Thing) of
        'true' ->
            kz_term:is_true(Thing);
        'false' ->
            case is_binary(Thing) of
                'true' -> Thing;
                'false' -> 'true'
            end
    end.
