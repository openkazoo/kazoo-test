...
-export([
    pickup/2, pickup/3, pickup/4, pickup/5, pickup/6,
...
-export([
    connect_leg/2, connect_leg/3, connect_leg/4, connect_leg/5, connect_leg/6,
    connect_leg_command/2,
    connect_leg_command/3,
    connect_leg_command/4,
    connect_leg_command/5,
    connect_leg_command/6,
...
-export([
    bridge/2, bridge/3, bridge/4, bridge/5, bridge/6, bridge/7, bridge/8, bridge/9,
...
-export([
    record_call/2, record_call/3, record_call/4, record_call/5,
...
-export([
    prompt_and_collect_digits/4,
    prompt_and_collect_digits/5,
    prompt_and_collect_digits/6,
    prompt_and_collect_digits/7,
    prompt_and_collect_digits/8,
    prompt_and_collect_digits/9
]).
-export([play_and_collect_digit/2]).
-export([
    play_and_collect_digits/4,
    play_and_collect_digits/5,
    play_and_collect_digits/6,
    play_and_collect_digits/7,
    play_and_collect_digits/8,
    play_and_collect_digits/9
]).
-export([
    say/2, say/3, say/4, say/5, say/6,
    say_command/2, say_command/3, say_command/4, say_command/5, say_command/6,
    b_say/2, b_say/3, b_say/4, b_say/5, b_say/6
]).

-export([conference/2, conference/3, conference/4, conference/5, conference/6, conference/7]).
-export([
    b_conference/2, b_conference/3, b_conference/4, b_conference/5, b_conference/6, b_conference/7
]).
...
-export([
    b_store/3, b_store/4, b_store/5, b_store/6,
    b_store_fax/2,
    b_store_vm/6
]).
-export([b_prompt_and_collect_digit/2]).
-export([
    b_prompt_and_collect_digits/4,
    b_prompt_and_collect_digits/5,
    b_prompt_and_collect_digits/6,
    b_prompt_and_collect_digits/7,
    b_prompt_and_collect_digits/8,
    b_prompt_and_collect_digits/9
]).
-export([b_play_and_collect_digit/2]).
-export([
    b_play_and_collect_digits/4,
    b_play_and_collect_digits/5,
    b_play_and_collect_digits/6,
    b_play_and_collect_digits/7,
    b_play_and_collect_digits/8,
    b_play_and_collect_digits/9
]).
...
-export([wait_for_message/2, wait_for_message/3, wait_for_message/4, wait_for_message/5]).
-export([
    wait_for_application/2, wait_for_application/3, wait_for_application/4, wait_for_application/5
]).
-export([
    wait_for_headless_application/1,
    wait_for_headless_application/2,
    wait_for_headless_application/3,
    wait_for_headless_application/4,
...
-export([
    wait_for_hangup/0, wait_for_hangup/1,
...
-export([
    attended_transfer/2, attended_transfer/3,
...
-export([
    play_macro/2,
    b_play_macro/2,
    play_macro_command/2,
    media_macro/2,
    media_macro_command/2
]).

-export([sound_touch_command/2, start_sound_touch/2, stop_sound_touch/1]).
-export([
    hold_control/1, hold_control/2,
...
-type audio_macro_prompt() ::
    ...
    | {'prompt', binary()}
    | {'prompt', binary(), kz_term:ne_binaries()}
...
-define(MAX_WAIT_MS,
    kapps_config:get_integer(?CONFIG_CAT, <<"max_wait_ms">>, ?MILLISECONDS_IN_DAY)
).

-define(CONFIG_CAT, <<"call_command">>).

-define(DEFAULT_COLLECT_TIMEOUT,
    kapps_config:get_integer(?CONFIG_CAT, <<"collect_timeout">>, 5 * ?MILLISECONDS_IN_SECOND)
).
-define(DEFAULT_DIGIT_TIMEOUT,
    kapps_config:get_integer(?CONFIG_CAT, <<"digit_timeout">>, 3 * ?MILLISECONDS_IN_SECOND)
).
-define(DEFAULT_INTERDIGIT_TIMEOUT,
    kapps_config:get_integer(?CONFIG_CAT, <<"interdigit_timeout">>, 2 * ?MILLISECONDS_IN_SECOND)
).

-define(DEFAULT_MESSAGE_TIMEOUT,
    kapps_config:get_integer(?CONFIG_CAT, <<"message_timeout">>, 5 * ?MILLISECONDS_IN_SECOND)
).
-define(DEFAULT_APPLICATION_TIMEOUT,
    kapps_config:get_integer(?CONFIG_CAT, <<"application_timeout">>, 500 * ?MILLISECONDS_IN_SECOND)
).

-define(STORAGE_TIMEOUT(App),
    kapps_config:get_integer(
        ?CONFIG_CAT,
        [<<"store_file">>, kz_term:to_binary(App), <<"save_timeout_ms">>],
        5 * ?MILLISECONDS_IN_MINUTE,
        <<"default">>
    )
).
-define(STORAGE_RETRIES(App),
    kapps_config:get_integer(
        ?CONFIG_CAT, [<<"store_file">>, kz_term:to_binary(App), <<"retries">>], 5, <<"default">>
    )
).

-define(EXTRA_BRIDGE_TIMEOUT,
    kapps_config:get_integer(
        ?CONFIG_CAT, <<"bridge_timeout_extended_ms">>, 20 * ?MILLISECONDS_IN_SECOND
    )
).
-define(BRIDGE_DEFAULT_TIMEOUT, ?BRIDGE_DEFAULT_SYSTEM_TIMEOUT_S * ?MILLISECONDS_IN_SECOND).

-define(BRIDGE_EXPORT_VARS,
    kapps_config:get_ne_binaries(
        ?CONFIG_CAT, <<"export_bridge_variables">>, ?BRIDGE_DEFAULT_EXPORT_VARS
    )
).
...
-spec presence(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_binary() | kapps_call:call()) ->
    'ok'.
presence(State, PresenceId, 'undefined') ->
...
-spec presence(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_binary(), kapps_call:call() | 'undefined'
) -> 'ok'.
...
-spec channel_status_command(kz_term:ne_binary() | kapps_call:call(), kz_term:api_boolean()) ->
    kz_term:proplist().
...
b_channel_status(ChannelId) when is_binary(ChannelId) ->
    ...
    Resp = kz_amqp_worker:call_collect(
        Command,
...
b_channel_status(Call) ->
    b_channel_status(kapps_call:call_id(Call)).
...
build_macro({'prompt', PromptName, Lang, Leg}, {Call, GroupId, Queue}) ->
    Command = play_command(
        kapps_call:get_prompt(Call, PromptName, Lang),
        ?ANY_DIGIT,
        Leg,
        Call
    ),
...
build_macro({'say', Say}, {Call, GroupId, Queue}) ->
    Command = say_command(
        Say, <<"name_spelled">>, <<"pronounced">>, kapps_call:language(Call), Call
    ),
    {Call, GroupId, [kz_json:set_value(<<"Group-ID">>, GroupId, Command) | Queue]};
...
-spec pickup(kz_term:ne_binary(), kz_term:api_binary(), boolean(), boolean(), kapps_call:call()) ->
    'ok'.
...
-spec pickup(
    kz_term:ne_binary(), kz_term:api_binary(), boolean(), boolean(), boolean(), kapps_call:call()
) -> 'ok'.
pickup(TargetCallId, Insert, ContinueOnFail, ContinueOnCancel, ParkAfterPickup, Call) ->
    Command = pickup_command(
        TargetCallId, Insert, ContinueOnFail, ContinueOnCancel, ParkAfterPickup, Call
    ),
    send_command(Command, Call).
...
-spec pickup_command(kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    kz_term:proplist().
pickup_command(TargetCallId, Insert, Call) ->
    pickup_command(TargetCallId, Insert, 'false', Call).

-spec pickup_command(kz_term:ne_binary(), kz_term:ne_binary(), boolean(), kapps_call:call()) ->
    kz_term:proplist().
pickup_command(TargetCallId, Insert, ContinueOnFail, Call) ->
    pickup_command(TargetCallId, Insert, ContinueOnFail, 'true', Call).

-spec pickup_command(
    kz_term:ne_binary(), kz_term:ne_binary(), boolean(), boolean(), kapps_call:call()
) -> kz_term:proplist().
...
-spec pickup_command(
    kz_term:ne_binary(), kz_term:ne_binary(), boolean(), boolean(), boolean(), kapps_call:call()
) -> kz_term:proplist().
...
-spec connect_leg(
    kz_term:ne_binary(), kz_term:api_binary(), boolean(), boolean(), kapps_call:call()
) -> 'ok'.
...
connect_leg(TargetCallId, Insert, ContinueOnFail, ContinueOnCancel, ParkAfterConnect_Leg, Call) ->
    Command = connect_leg_command(
        TargetCallId, Insert, ContinueOnFail, ContinueOnCancel, ParkAfterConnect_Leg, Call
    ),
    send_command(Command, Call).
...
-spec connect_leg_command(kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    kz_term:proplist().
connect_leg_command(TargetCallId, Insert, Call) ->
    connect_leg_command(TargetCallId, Insert, 'false', Call).

-spec connect_leg_command(kz_term:ne_binary(), kz_term:ne_binary(), boolean(), kapps_call:call()) ->
    kz_term:proplist().
...
-spec set(kz_term:api_object(), kz_term:api_object(), kz_term:api_object(), kapps_call:call()) ->
    'ok'.
set('undefined', CallVars, AppVars, Call) ->
    set(kz_json:new(), CallVars, AppVars, Call);
set(ChannelVars, 'undefined', AppVars, Call) ->
    set(ChannelVars, kz_json:new(), AppVars, Call);
set(ChannelVars, CallVars, AppVars, Call) ->
    case
        kz_json:is_empty(ChannelVars) andalso
            kz_json:is_empty(CallVars) andalso
            kz_json:is_empty(AppVars)
    of
        'true' ->
            'ok';
        'false' ->
...
receive_fax(ResourceFlag, ReceiveFlag, LocalFilename, Call) ->
    Commands = props:filter_undefined([
...
-spec page(
    kz_json:objects(), integer(), kz_term:api_binary(), kz_term:api_binary(), kapps_call:call()
) -> 'ok'.
...
bridge_command(Endpoints, Timeout, Strategy, IgnoreEarlyMedia, Ringback, SIPHeaders, Call) ->
    bridge_command(
        Endpoints, Timeout, Strategy, IgnoreEarlyMedia, Ringback, SIPHeaders, <<"false">>, Call
    ).
bridge_command(
    Endpoints, Timeout, Strategy, IgnoreEarlyMedia, Ringback, SIPHeaders, IgnoreForward, Call
) ->
    bridge_command(
        Endpoints,
        Timeout,
...
bridge(Endpoints, Timeout, Strategy, IgnoreEarlyMedia, Ringback, SIPHeaders, Call) ->
    Command = bridge_command(
        Endpoints, Timeout, Strategy, IgnoreEarlyMedia, Ringback, SIPHeaders, Call
    ),
    send_command(Command, Call).
...
b_bridge_wait(Timeout, Call) ->
    wait_for_bridge(
        (kz_term:to_integer(Timeout) * ?MILLISECONDS_IN_SECOND) + ?EXTRA_BRIDGE_TIMEOUT, Call
    ).
...
-spec unbridge_command(kapps_call:call(), kz_term:api_ne_binary(), kz_term:ne_binary()) ->
    kz_term:proplist().
...
-spec soft_hold_command(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_binary(), kz_term:api_binary()
) ->
    kz_term:proplist().
...
hold_command(MOH, Call) ->
    hold_command(
        kz_media_util:media_path(MOH, kapps_call:account_id(Call)),
...
-spec b_hold(timeout() | kz_term:api_binary(), kapps_call:call()) ->
    kapps_api_std_return().
b_hold(Timeout, Call) when
    is_integer(Timeout);
    Timeout =:= 'infinity'
->
    b_hold(Timeout, 'undefined', Call);
b_hold(MOH, Call) ->
    b_hold(?MAX_WAIT_MS, MOH, Call).
...
-spec b_prompt(kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    kapps_api_std_return().
...
-spec play_command(
    kz_term:ne_binary(), kz_term:api_binaries(), kapps_call:call() | kz_term:ne_binary()
) ->
    kz_json:object().
...
-spec tts(kz_term:ne_binary(), kz_term:api_binary(), kz_term:api_binary(), kapps_call:call()) ->
    kz_term:ne_binary().
...
-spec tts_command(kz_term:api_binary(), kz_term:api_binary(), kapps_call:call()) ->
    kz_json:object().
tts_command(SayMe, Voice, Call) ->
    tts_command(SayMe, Voice, kapps_call:language(Call), Call).

-spec tts_command(
    kz_term:api_binary(), kz_term:api_binary(), kz_term:api_binary(), kapps_call:call()
) -> kz_json:object().
...
-spec b_tts(kz_term:api_binary(), kz_term:api_binary(), kapps_call:call()) ->
    kapps_api_std_return().
b_tts(SayMe, Voice, Call) ->
    wait_for_noop(Call, tts(SayMe, Voice, Call)).

-spec b_tts(kz_term:api_binary(), kz_term:api_binary(), kz_term:api_binary(), kapps_call:call()) ->
    kapps_api_std_return().
...
-spec record(
    kz_term:ne_binary(), kz_term:binaries(), kz_term:api_binary() | integer(), kapps_call:call()
) -> 'ok'.
...
-spec b_record(kz_term:ne_binary(), kz_term:binaries(), kapps_call:call()) ->
    kapps_api_std_return().
...
-spec start_record_call(
    kz_term:proplist(), kz_term:api_binary() | pos_integer(), kapps_call:call()
) -> 'ok'.
...
-spec start_record_call(
    kz_term:proplist(),
    kz_term:api_binary() | pos_integer(),
    kz_term:ne_binaries(),
    kapps_call:call()
) -> 'ok'.
...
-spec record_call(
    kz_term:proplist(), kz_term:ne_binary(), kz_term:api_binary() | pos_integer(), kapps_call:call()
) -> 'ok'.
...
-spec store(kz_term:ne_binary(), kz_term:api_binary(), kz_term:api_binary(), kapps_call:call()) ->
    'ok'.
...
-spec b_store(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    b_store_return().
...
-spec audio_level_command(
    kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary() | integer()
) ->
    kz_term:proplist().
...
-spec prompt_and_collect_digits(
    integer(),
    integer(),
    kz_term:ne_binary(),
    kapps_call:call()
...
prompt_and_collect_digits(MinDigits, MaxDigits, Prompt, Tries, Call) ->
    prompt_and_collect_digits(
        MinDigits, MaxDigits, Prompt, Tries, 3 * ?MILLISECONDS_IN_SECOND, Call
    ).
...
prompt_and_collect_digits(MinDigits, MaxDigits, Prompt, Tries, Timeout, InvalidPrompt, Call) ->
    prompt_and_collect_digits(
        MinDigits, MaxDigits, Prompt, Tries, Timeout, InvalidPrompt, <<"\\d+">>, Call
    ).
...
prompt_and_collect_digits(MinDigits, MaxDigits, Prompt, Tries, Timeout, InvalidPrompt, Regex, Call) ->
    prompt_and_collect_digits(
        MinDigits, MaxDigits, Prompt, Tries, Timeout, InvalidPrompt, Regex, [<<"#">>], Call
    ).
...
-spec prompt_and_collect_digits(
    ...
    'ok'.
prompt_and_collect_digits(
    MinDigits, MaxDigits, Prompt, Tries, Timeout, InvalidPrompt, Regex, Terminators, Call
) ->
...
-spec b_prompt_and_collect_digits(
    integer(),
    integer(),
    kz_term:ne_binary(),
    kapps_call:call()
...
b_prompt_and_collect_digits(MinDigits, MaxDigits, Prompt, Tries, Call) ->
    b_prompt_and_collect_digits(
        MinDigits, MaxDigits, Prompt, Tries, ?DEFAULT_COLLECT_TIMEOUT, Call
    ).
...
b_prompt_and_collect_digits(
    ...
    b_prompt_and_collect_digits(
        MinDigits, MaxDigits, Prompt, Tries, Timeout, InvalidPrompt, Regex, ?ANY_DIGIT, Call
    ).
...
-spec b_prompt_and_collect_digits(
    ...
    b_play_and_collect_digits_return().
b_prompt_and_collect_digits(
    _MinDigits, _MaxDigits, _Prompt, 0, _Timeout, 'undefined', _Regex, _Terminators, _Call
) ->
    {'ok', <<>>};
b_prompt_and_collect_digits(
    _MinDigits, _MaxDigits, _Prompt, 0, _Timeout, InvalidPrompt, _Regex, _Terminators, Call
) ->
...
play_and_collect_digits(MinDigits, MaxDigits, Media, Tries, Timeout, MediaInvalid, Call) ->
    play_and_collect_digits(
        MinDigits, MaxDigits, Media, Tries, Timeout, MediaInvalid, <<"\\d+">>, Call
    ).
...
play_and_collect_digits(MinDigits, MaxDigits, Media, Tries, Timeout, MediaInvalid, Regex, Call) ->
    play_and_collect_digits(
        MinDigits, MaxDigits, Media, Tries, Timeout, MediaInvalid, Regex, [<<"#">>], Call
    ).
...
-spec play_and_collect_digits(
    ...
    'ok'.
play_and_collect_digits(
    MinDigits, MaxDigits, Media, Tries, Timeout, MediaInvalid, Regex, Terminators, Call
) ->
    Command = [
...
-spec b_play_and_collect_digits(
    integer(),
    integer(),
    kz_term:ne_binary(),
    kapps_call:call()
...
b_play_and_collect_digits(MinDigits, MaxDigits, Media, Tries, Timeout, MediaInvalid, Regex, Call) ->
    b_play_and_collect_digits(
        MinDigits, MaxDigits, Media, Tries, Timeout, MediaInvalid, Regex, ?ANY_DIGIT, Call
    ).
...
-spec b_play_and_collect_digits(
    ...
    b_play_and_collect_digits_return().
b_play_and_collect_digits(
    _MinDigits, _MaxDigits, _Media, 0, _Timeout, 'undefined', _Regex, _Terminators, _Call
) ->
    {'ok', <<>>};
b_play_and_collect_digits(
    _MinDigits, _MaxDigits, _Media, 0, _Timeout, MediaInvalid, _Regex, _Terminators, Call
) ->
...
-spec say_command(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    kz_json:object().
...
say_gender(Gender) -> say_gender_validate(kz_term:to_lower_binary(Gender)).
...
-spec b_say(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    kapps_api_std_return().
...
wait_for_say(Call) ->
    wait_for_message(
        Call, <<"say">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"call_event">>, ?MAX_WAIT_MS
    ).
...
-spec conference(
    kz_term:ne_binary(), boolean(), boolean(), boolean(), kz_term:ne_binary(), kapps_call:call()
) -> 'ok'.
...
-spec b_conference(kz_term:ne_binary(), boolean(), boolean(), kapps_call:call()) ->
    kapps_api_std_return().
b_conference(ConfId, Mute, Deaf, Call) ->
    b_conference(ConfId, Mute, Deaf, 'false', Call).

-spec b_conference(kz_term:ne_binary(), boolean(), boolean(), boolean(), kapps_call:call()) ->
    kapps_api_std_return().
...
%% @end
%%------------------------------------------------------------------------------
-type collect_digits_return() ::
    {'error', 'channel_hungup' | 'channel_unbridge' | kz_json:object()}
    | {'ok', binary()}.

-record(wcc_collect_digits, {
    max_digits :: pos_integer(),
...
collect_digits(MaxDigits, Call) ->
    do_collect_digits(#wcc_collect_digits{
        max_digits = kz_term:to_integer(MaxDigits),
        call = Call,
        after_timeout = ?DEFAULT_DIGIT_TIMEOUT
    }).

-spec collect_digits(
    integer() | kz_term:ne_binary(), integer() | kz_term:ne_binary(), kapps_call:call()
...
-spec collect_digits(
    integer(), integer(), integer(), kz_term:api_binary(), list(), kapps_call:call()
) ->
    collect_digits_return().
...
-spec collect_digits(
    integer(), integer(), integer(), kz_term:api_binary(), list(), boolean(), kapps_call:call()
) ->
    collect_digits_return().
    ...
    case receive_event(After) of
        {'ok', JObj} ->
            case handle_collect_digit_event(JObj, NoopId) of
                ...
                {'decrement'} ->
                    do_collect_digits(Collect#wcc_collect_digits{
                        after_timeout = kz_time:decr_timeout(After, Start)
                    });
                {'ok', Digit} ->
                    ...
                    case lists:member(Digit, Terminators) of
                        'true' ->
                            lager:debug(
                                "collected digits ('~s') from caller, terminated with ~s", [
                                    Digits, Digit
                                ]
                            ),
                            {'ok', Digits};
...
handle_collect_digit_event(JObj, NoopId, {<<"error">>, _, <<"noop">>}) ->
    case kz_json:get_value([<<"Request">>, <<"Msg-ID">>], JObj, NoopId) of
        NoopId when is_binary(NoopId), NoopId =/= <<>> ->
            lager:debug(
                "channel execution error while collecting digits: ~s",
        ...
        _NID ->
            lager:debug(
                "channel execution error while collecting digits with noop-id ~s: ~s",
                [NoopId, kz_json:encode(JObj)]
            ),
            {'error', JObj}
    end;
handle_collect_digit_event(
    JObj, NoopId, {<<"call_event">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"noop">>}
) ->
...
-spec wait_for_message(
    kapps_call:call(), binary(), kz_term:ne_binary(), kz_term:ne_binary(), timeout()
) ->
    kapps_api_std_return().
    ...
    case receive_event(Timeout) of
        ...
        {'ok', JObj} = Ok ->
            case get_event_type(JObj) of
                ...
                _ ->
                    wait_for_message(
                        Call, Application, Event, Type, kz_time:decr_timeout(Timeout, Start)
                    )
            end
    end.
...
%% @end
%%------------------------------------------------------------------------------
-type wait_for_application_return() ::
    {'error', 'timeout' | kz_json:object()}
    | {'ok', kz_json:object()}.
...
-spec wait_for_application(
    kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), timeout()
) ->
    wait_for_application_return().
...
-type wait_for_headless_application_return() ::
    {'error', 'timeout' | kz_json:object()}
    | {'ok', kz_json:object()}.
...
-spec wait_for_headless_application(
    kz_term:ne_binary(),
    headless_event(),
    kz_term:ne_binary(),
    timeout()
    ...
    case receive_event(Timeout) of
        {'ok', JObj} = Ok ->
            case get_event_type(JObj) of
                {<<"error">>, _, Application} ->
                    lager:debug(
                        "channel execution error while waiting for ~s: ~s",
                        [Application, kz_json:encode(JObj)]
                    ),
                    {'error', JObj};
                {<<"call_event">>, <<"CHANNEL_DESTROY">>, _} ->
                ...
                {Type, StartEv, Application} ->
                    ...
                    wait_for_headless_application(
                        Application, StopEv, Type, Fun, kz_time:decr_timeout(Timeout, Start)
                    );
                {Type, StopEv, Application} ->
                    case Fun(JObj) of
                        ...
                        'false' ->
                            wait_for_headless_application(
                                Application, Event, Type, Fun, kz_time:decr_timeout(Timeout, Start)
                            )
                    end;
                _T ->
        ...
        {'ok', JObj} = Ok ->
            case get_event_type(JObj) of
                ...
                {<<"call_event">>, <<"CHANNEL_DESTROY">>, _} ->
                    ...
                    wait_for_headless_application(
                        Application, Event, Type, Fun, 60 * ?MILLISECONDS_IN_SECOND
                    );
                {Type, Event, Application} ->
                    case Fun(JObj) of
                        'true' ->
        ...
        {'ok', JObj} ->
            case kz_util:get_event_type(JObj) of
                {<<"call_event">>, <<"CHANNEL_DESTROY">>} ->
                    ...
                    {'error', 'channel_hungup'};
                {<<"error">>, _} ->
                    lager:debug("channel execution error while waiting for DTMF: ~s", [
                        kz_json:encode(JObj)
                    ]),
                    {'error', JObj};
                {<<"call_event">>, <<"DTMF">>} ->
...
wait_for_noop(Call, NoopId, Timeout) ->
    case
        wait_for_message(
            Call, <<"noop">>, <<"CHANNEL_EXECUTE_COMPLETE">>, <<"call_event">>, Timeout
        )
    of
        {'ok', JObj} = OK ->
            case kz_json:get_value(<<"Application-Response">>, JObj) of
        ...
        {'ok', JObj} = Ok ->
            case get_event_type(JObj) of
                {<<"call_event">>, <<"CHANNEL_DESTROY">>, _} ->
                    ...
                    {'error', 'channel_hungup'};
                {<<"error">>, _, Application} ->
                    lager:debug("channel execution error while waiting ~s or DTMF: ~s", [
                        Application, kz_json:encode(JObj)
                    ]),
                    {'error', JObj};
...
-type wait_for_fax_ret() ::
    {'ok', kz_json:object()}
    | {'error', 'timeout' | kz_json:object()}.

-define(WAIT_FOR_FAX_TIMEOUT,
    kapps_config:get_integer(<<"fax">>, <<"wait_for_fax_timeout_ms">>, ?MILLISECONDS_IN_HOUR)
).
...
wait_for_fax(Timeout) ->
    ...
    case receive_event(Timeout) of
        ...
        {'ok', JObj} ->
            case get_event_type(JObj) of
                {<<"error">>, _, <<"receive_fax">>} ->
                    lager:debug("channel execution error while waiting for fax: ~s", [
                        kz_json:encode(JObj)
                    ]),
                    {'error', JObj};
                ...
                _ ->
                    wait_for_fax(kz_time:decr_timeout(Timeout, Start))
            end
    end.
...
send_command(JObj, Call) ->
    send_command(kz_json:to_proplist(JObj), Call).
...
fax_detection(Direction, Duration, Call) ->
    start_fax_detection(Direction, Duration, Call),
    Result =
        case wait_for_fax_detection((Duration + 2) * ?MILLISECONDS_IN_SECOND, Call) of
            {'error', 'timeout'} -> 'false';
            {'ok', _JObj} -> 'true'
        end,
    stop_fax_detection(Call),
        ...
        {'ok', JObj} ->
            case get_event_type(JObj) of
                {<<"call_event">>, <<"FAX_DETECTED">>, _} ->
                    {'ok', kz_json:set_value(<<"Fax-Success">>, 'true', JObj)};
                _ ->
                    wait_for_fax_detection(kz_time:decr_timeout(Timeout, Start), Call)
            end
    end.
        ...
        {'ok', JObj} = Ok ->
            case get_event_type(JObj) of
                ...
                {<<"call_event">>, <<"CHANNEL_INTERCEPTED">>, _} ->
                    ...
                    Ok;
                {<<"error">>, _, <<"hold">>} ->
                    lager:debug("channel execution error while waiting for unparked call: ~s", [
                        kz_json:encode(JObj)
                    ]),
                    {'error', JObj};
...
store_file(Filename, URLFun, Tries, Timeout, Call) ->
    Msg =
        case kapps_call:kvs_fetch('alert_msg', Call) of
            'undefined' ->
                io_lib:format(
                    "Error Storing File ~s From Media Server ~s",
...
do_store_file(Tries, Timeout, API, Msg, Call) ->
    ...
    case
        kz_amqp_worker:call(
            Payload, fun kapi_switch:publish_command/1, fun kapi_switch:fs_reply_v/1, Timeout
        )
    of
        {'ok', JObj} ->
            case kz_json:get_ne_binary_value(<<"Result">>, JObj) of
                <<"success">> ->
                    'ok';
                <<"error">> ->
                    Error = kz_json:get_first_defined(
                        [[<<"Event-Data">>, <<"API-Error">>], <<"Error">>],
                        JObj,
                        <<"error not available">>
                    ),
                    retry_store_file(
                        Tries - 1, Timeout, API, Msg, Error, maybe_add_debug_data(JObj, Call)
                    );
                _Other ->
                    Error = kz_term:to_binary(
                        io_lib:format("unhandled return ('~s') from store file", [_Other])
                    ),
                    retry_store_file(
                        Tries - 1, Timeout, API, Msg, Error, maybe_add_debug_data(JObj, Call)
                    )
        ...
        {'returned', _JObj, _Basic} ->
            Error = io_lib:format(
                "message returned from amqp. is ~s down ?",
                [kapps_call:switch_nodename(Call)]
            ),
            Funs = [{fun kapps_call:kvs_store/3, 'basic_return', kz_json:to_proplist(_Basic)}],
            retry_store_file(
                Tries - 1, Timeout, API, Msg, kz_term:to_binary(Error), kapps_call:exec(Funs, Call)
            );
        {'timeout', _JObj} ->
            Error = io_lib:format(
                "timeout publishing message to amqp. is ~s down ?",
                [kapps_call:switch_nodename(Call)]
            ),
            retry_store_file(Tries - 1, Timeout, API, Msg, kz_term:to_binary(Error), Call);
        {'error', Err} ->
            Error = io_lib:format(
                "error publishing message to amqp. is ~s down ? : ~p",
                [kapps_call:switch_nodename(Call), Err]
            ),
            retry_store_file(Tries - 1, Timeout, API, Msg, kz_term:to_binary(Error), Call)
    end.

-spec retry_store_file(
    integer(),
    timeout(),
    kz_term:proplist() | function(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kapps_call:call()
) ->
...
-spec transfer(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_binary(), kapps_call:call()) ->
    'ok'.
transfer(TransferType, To, TransferLeg, Call) ->
    Command = transfer_command(TransferType, To, TransferLeg, Call),
    send_command(Command, Call).

-spec transfer_command(kz_term:ne_binary(), kz_term:ne_binary(), kapps_call:call()) ->
    kz_term:api_terms().
transfer_command(TransferType, TransferTo, Call) ->
    transfer_command(TransferType, TransferTo, 'undefined', Call).

-spec transfer_command(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_ne_binary(), kapps_call:call()
) -> kz_term:api_terms().
