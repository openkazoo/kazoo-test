...
-define(DEFAULT_ENTRY_TONE,
    <<"tone_stream://v=-7;>=2;+=.1;%(300,0,523,659);v=-7;>=3;+=.1;%(800,0,659,783)">>
).
-define(ENTRY_TONE(AccountId),
    kapps_account_config:get_global(
        AccountId, ?CONFERENCE_CONFIG_CAT, <<"entry_tone">>, ?DEFAULT_ENTRY_TONE
    )
).
-define(MOD_ENTRY_TONE(AccountId),
    kapps_account_config:get_global(
        AccountId, ?CONFERENCE_CONFIG_CAT, <<"moderator_entry_tone">>, ?DEFAULT_ENTRY_TONE
    )
).

-define(DEFAULT_EXIT_TONE,
    <<"tone_stream://v=-7;>=2;+=.1;%(300,0,523,440);v=-7;>=3;+=.1;%(800,0,349,440)">>
).
-define(EXIT_TONE(AccountId),
    kapps_account_config:get_global(
        AccountId, ?CONFERENCE_CONFIG_CAT, <<"exit_tone">>, ?DEFAULT_EXIT_TONE
    )
).
-define(MOD_EXIT_TONE(AccountId),
    kapps_account_config:get_global(
        AccountId, ?CONFERENCE_CONFIG_CAT, <<"moderator_exit_tone">>, ?DEFAULT_EXIT_TONE
    )
).

-define(SUPPORT_NAME_ANNOUNCEMENT(AccountId),
    kz_term:is_true(
        kapps_account_config:get_global(
            AccountId, ?CONFERENCE_CONFIG_CAT, <<"support_name_announcement">>, 'true'
        )
    )
).

-define(DEFAULT_PROFILE_CONFIG, [
