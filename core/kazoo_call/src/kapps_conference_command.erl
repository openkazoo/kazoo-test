...
-spec participant_energy(non_neg_integer(), kz_term:ne_binary(), kapps_conference:conference()) ->
    'ok'.
...
-spec prompt(kz_term:ne_binary(), non_neg_integer() | 'undefined', kapps_conference:conference()) ->
    'ok'.
...
-spec play(kz_term:ne_binary(), non_neg_integer() | 'undefined', kapps_conference:conference()) ->
    'ok'.
...
-spec relate_participants(non_neg_integer(), non_neg_integer(), kapps_conference:conference()) ->
    'ok'.
...
-spec relate_participants(
    non_neg_integer(), non_neg_integer(), kz_term:api_binary(), kapps_conference:conference()
) -> 'ok'.
...
-spec stop_play(
    non_neg_integer() | 'undefined', kz_term:api_binary(), kapps_conference:conference()
) -> 'ok'.
...
-spec participant_volume_in(non_neg_integer(), kz_term:ne_binary(), kapps_conference:conference()) ->
    'ok'.
...
-spec participant_volume_out(non_neg_integer(), kz_term:ne_binary(), kapps_conference:conference()) ->
    'ok'.
...
-spec dial(
    ...
    kz_term:api_ne_binary(),
    kz_term:api_object(),
    kapps_conference:conference() | kz_term:ne_binary()
) -> 'ok'.
dial(Endpoints, CallerIdNumber, CallerIdName, CCVs, ConferenceId) when
    is_list(Endpoints),
...
send_command([_ | _] = Command, Conference) ->
    ...
    case kz_term:is_empty(Focus) of
        'true' ->
            kz_amqp_worker:cast(Prop, fun(P) -> kapi_conference:publish_command(ConferenceId, P) end);
        'false' ->
            kz_amqp_worker:cast(Prop, fun(P) ->
                kapi_conference:publish_targeted_command(Focus, P)
            end)
    end;
send_command(JObj, Conference) ->
    send_command(kz_json:to_proplist(JObj), Conference).
