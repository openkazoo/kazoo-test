...
base_call_cost(RateCost, RateMin, RateSurcharge) when
    is_integer(RateCost),
    is_integer(RateMin),
...
calculate_call(JObj) ->
    CCVs = kz_json:get_first_defined(
        [
            <<"Custom-Channel-Vars">>,
            <<"custom_channel_vars">>
        ],
    ...
    BillingSecs =
        get_integer_value(<<"Billing-Seconds">>, JObj) -
    ...
    case BillingSecs =< 0 of
        ...
        'false' ->
            ...
            {ChargedSeconds, Cost} = calculate_call(
                Rate, RateIncr, RateMin, Surcharge, BillingSecs
            ),
...
-spec calculate_cost(kz_currency:units(), integer(), integer(), kz_currency:units(), integer()) ->
    kz_currency:units().
calculate_cost(_, _, _, _, 0) ->
    0;
...
-spec calculate_call(kz_currency:units(), integer(), integer(), kz_currency:units(), integer()) ->
    {integer(), kz_currency:units()}.
calculate_call(_, _, _, _, 0) ->
    0;
