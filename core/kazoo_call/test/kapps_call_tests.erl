...
-define(UPDATERS, [
    fun(C) -> kapps_call:set_custom_channel_var(<<"key1">>, <<"value1">>, C) end,
    ...
    fun(C) ->
        kapps_call:kvs_store(
            <<"kvs_key_2">>, kz_json:from_list([{<<"sub_key_1">>, <<"sub_value_1">>}]), C
        )
    end
]).

route_test_() ->
    {'setup', fun kazoo_call_test_util:setup_db/0, fun kazoo_call_test_util:terminate_db/1, fun(
        _ReturnOfSetup
    ) ->
        [
            {"Testing Callflow route request", test_callflow_route_request()},
            {"Testing Callflow route win", test_callflow_route_win()},
            {"Testing Trunkstore route request", test_trunkstore_route_request()},
            {"Testing Trunkstore route win", test_trunkstore_route_win()},
            test_encode_decode()
        ]
    end}.

ensure_call_id_binary_test_() ->
    RouteReq = kz_json:set_value(
        <<"Call-ID">>,
        'atom',
...
test_encode_decode() ->
    ...
    [{"Testing kapps_call encode decode", ?_assert(kapps_call:eq(Call, Call1))}].
...
validate_kapps_call_basic(Call) ->
    [
        ...
        {"verify the account db is the expected value",
            ?_assertEqual(
                <<"account%2Fac%2Fco%2Funt0000000000000000000000001">>, kapps_call:account_db(Call)
            )},
        ...
        {"verify the CCV caller privacy hide name is the expected value",
            ?_assertEqual(
                'false', kapps_call:custom_channel_var(<<"Caller-Privacy-Hide-Name">>, Call)
            )},
        {"verify the CCV caller privacy hide number is the expected value",
            ?_assertEqual(
                'false', kapps_call:custom_channel_var(<<"Caller-Privacy-Hide-Number">>, Call)
            )},
...
validate_kapps_call_callflow_req(Call) ->
    [
        {"verify the call id is the expected value",
            ?_assertEqual(
                <<"82159NjBmNTI0YjQ5MTE2NmFkNmQ3NmRmYzFiYWRhMGQ0NGI">>, kapps_call:call_id(Call)
            )},
        ...
        {"verify inception is 'undefined'", ?_assertEqual('undefined', kapps_call:inception(Call))},
        ...
        {"verify the CCV call interaction id is the expected value",
            ?_assertEqual(
                <<"63670304795-386ddd58">>,
                kapps_call:custom_channel_var(<<"Call-Interaction-ID">>, Call)
            )}
    ].
...
validate_kapps_call_callflow_win(Call) ->
    [
        {"verify control queue is 'undefined'",
            ?_assertEqual(
                <<"ecallmgr@apps001.2600hz.local-<0.29590.28>-521b30b6">>,
                kapps_call:control_queue(Call)
            )},
        {"verify the CCV channel authorized is the expected value",
            ?_assertEqual(
                <<"true">>, kapps_call:custom_channel_var(<<"Channel-Authorized">>, Call)
            )},
        ...
        {"verify the CCV application name is the expected value",
            ?_assertEqual(
                <<"callflow">>, kapps_call:custom_channel_var(<<"Application-Name">>, Call)
            )},
        {"verify the CCV application node is the expected value",
            ?_assertEqual(
                <<"kazoo_apps@apps001.2600hz.local">>,
                kapps_call:custom_channel_var(<<"Application-Node">>, Call)
            )},
        {"verify the CCV callflow id is the expected value",
            ?_assertEqual(
                <<"callflow000000000000000000000001">>,
                kapps_call:custom_channel_var(<<"CallFlow-ID">>, Call)
            )},
        {"verify the CCV call interaction id is the expected value",
            ?_assertEqual(
                <<"63670304795-386ddd58">>,
...
validate_kapps_call_trunkstore_req(Call) ->
    [
        {"verify the call id is the expected value",
            ?_assertEqual(
                <<"82159ZWEwNTQ2NGY4Yzc2YzRiNDE4YzY1YzdmZmVjNWQ3ZjQ">>, kapps_call:call_id(Call)
            )},
        {"verify the authorizing id is the expected value",
        ...
        {"verify owner id is 'undefined'", ?_assertEqual('undefined', kapps_call:owner_id(Call))},
        {"verify the fetch id is the expected value",
...
validate_kapps_call_trunkstore_win(Call) ->
    [
        {"verify control queue is 'undefined'",
            ?_assertEqual(
                <<"ecallmgr@apps001.2600hz.local-<0.28561.28>-a4a36185">>,
                kapps_call:control_queue(Call)
            )},
        {"verify the CCV channel authorized is the expected value",
            ?_assertEqual(
        ...
        {"verify the CCV application name is the expected value",
            ?_assertEqual(
                <<"trunkstore">>, kapps_call:custom_channel_var(<<"Application-Name">>, Call)
            )},
        {"verify the CCV application node is the expected value",
            ?_assertEqual(
                <<"kazoo_apps@apps001.2600hz.local">>,
