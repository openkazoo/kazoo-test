...
-spec split_account_mod(kz_term:ne_binary()) ->
    {kz_term:ne_binary(), kz_time:year(), kz_time:month()}.
...
%% @equiv modb_id(Year, Month, kz_binary:rand_hex(16))
-spec modb_id(kz_term:ne_binary() | kz_time:year(), kz_term:ne_binary() | kz_time:month()) ->
    kz_term:ne_binary().
modb_id(Year, Month) ->
    modb_id(Year, Month, kz_binary:rand_hex(16)).
...
db_list(MODb, MODbs) ->
    case kz_datamgr:db_exists(MODb) of
        'false' -> MODbs;
        'true' -> db_list(prev_year_month_mod(MODb), [MODb | MODbs])
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec get_modb_suffix(kz_term:ne_binary() | pos_integer()) ->
    {kz_term:api_integer(), kz_term:api_integer()}.
