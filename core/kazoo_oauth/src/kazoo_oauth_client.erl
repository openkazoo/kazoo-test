...
%% @end
%%------------------------------------------------------------------------------
-spec authenticate(kz_json:object()) ->
    {'ok', kz_json:object()}
    | {'error', kz_term:ne_binary()}.
authenticate(JObj) ->
    case {kz_json:get_value(<<"access_token">>, JObj), kz_json:get_value(<<"provider">>, JObj)} of
        {'undefined', 'undefined'} ->
...
authenticate(AccessToken, ProviderId, JObj) ->
    case kazoo_oauth_util:verify_token(ProviderId, AccessToken) of
        {'ok', Token} ->
            maybe_add_oauth_user(JObj, Token);
        {'error', _R} = Error ->
...
get_refresh_token(_, _, _, _) ->
    kz_json:new().

save_oauth_doc(App, DocId, JObj, TokenObj, RefreshTokenObj) ->
    Doc = props:filter_undefined([
        {<<"email">>, kz_json:get_value(<<"email">>, TokenObj)},
    ...
    case kz_datamgr:update_doc(?KZ_OAUTH_DB, DocId, UpdateOptions) of
        {'ok', DocObj} ->
            load_profile(App, JObj, TokenObj, DocObj);
        {'error', _R} ->
...
load_profile(
    #oauth_app{provider = #oauth_provider{profile_url = ProfileURL}}, JObj, TokenObj, AuthDoc
) ->
