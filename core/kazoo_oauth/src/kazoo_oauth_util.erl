...
-spec get_oauth_provider(kz_term:ne_binary()) ->
    {'ok', oauth_provider()}
...
-spec get_oauth_app(kz_term:ne_binary()) ->
    {'ok', oauth_provider()}
    | {'error', kz_term:ne_binary()}.
get_oauth_app(AppId) ->
...
oauth_service_app_from_keys(PublicKey, PrivateKey, App) ->
    App#oauth_service_app{
        public_key = pem_to_rsa(PublicKey),
...
token(#oauth_app{} = App, DocId) when is_binary(DocId) ->
    case kz_datamgr:open_doc(?KZ_OAUTH_DB, DocId) of
        {'ok', JObj} ->
            token(App, #oauth_refresh_token{token = kz_json:get_value(<<"refresh_token">>, JObj)});
        {'error', _R} = Error ->
...
-spec refresh_token(
    ...
    {'ok', kz_term:api_object()}
    | {'error', any()}.
refresh_token(AppId, Scope, AuthorizationCode, ExtraHeaders) when
    is_binary(AppId)
->
    lager:debug("getting oauth-app ~p", [AppId]),
    case get_oauth_app(AppId) of
...
refresh_token(
    ...
    Body = string:join(
        lists:append(
            lists:map(fun({K, V}) -> [string:join([K, kz_term:to_list(V)], "=")] end, Fields)
        ),
        "&"
    ),
