...
put_attachment(Params, DbName, DocId, AName, Contents, Options) ->
    ...
    case ?REQUIRED_PROPS -- props:get_keys(Props) of
        ...
        Missing ->
            ...
            Routines = kz_att_error:put_routines(
                Params,
                DbName,
                DocId,
                AName,
...
maybe_fetch_attachment(HandlerProps, [AName | _]) -> fetch_attachment(HandlerProps, AName).
...
-spec handle_fetch_attachment_resp(
...
handle_fetch_attachment_resp({'error', {Code, Body}} = _E, Routines) ->
    lager:debug("Error fetching attachment: ~p", [_E]),
    NewRoutines = [
        {fun kz_att_error:set_resp_code/2, Code},
