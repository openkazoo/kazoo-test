...
-define(GRAPH_ME_UPLOAD_URL(PATH), [
    ?GRAPH_API_URL,
    ?GRAPH_ME,
    ?GRAPH_DRIVE,
    ?GRAPH_ROOT,
...
-define(DRV_FETCH_URL(D, I),
    <<(?GRAPH_API_URL)/binary, "/drives/", D/binary, "/items/", I/binary, "/content">>
).

%% TODO review scope
-define(DRV_PUT_SCOPE, <<"Files.ReadWrite.All">>).
-define(DRV_PUT_SCOPES, [?DRV_PUT_SCOPE]).
-define(DRV_PUT_TOKEN_OPTIONS, #{
    scopes => ?DRV_PUT_SCOPES,
    http_options => ?GRAPH_HTTP_OPTIONS
}).

-define(DRV_GET_SCOPE, <<"Files.Read.All">>).
-define(DRV_GET_SCOPES, [?DRV_GET_SCOPE]).
-define(DRV_GET_TOKEN_OPTIONS, #{
    scopes => ?DRV_GET_SCOPES,
    http_options => ?GRAPH_HTTP_OPTIONS
}).
...
fetch_attachment(HandlerProps, DbName, DocId, AName) ->
    case kz_json:get_value(<<"onedrive">>, HandlerProps) of
        ...
        GData ->
            {#{oauth_doc_id := TokenDocId}, {DriveId, ContentId}} = binary_to_term(
                base64:decode(GData)
            ),
            Authorization = kz_auth_client:token_for_auth_id(TokenDocId, ?DRV_GET_TOKEN_OPTIONS),
            do_fetch_attachment(
                Authorization, DriveId, ContentId, HandlerProps, DbName, DocId, AName
            )
    end.
...
onedrive_put(Url, Headers, Body) ->
    case kz_http:put(Url, Headers, Body, ?GRAPH_HTTP_OPTIONS) of
        {'ok', Code, ResponseHeaders, ResponseBody} when
            Code < 300
        ->
            ...
            case
                {
                    kz_json:get_value(<<"id">>, BodyJObj),
                    kz_json:get_value([<<"parentReference">>, <<"driveId">>], BodyJObj)
                }
            of
                {'undefined', 'undefined'} ->
                    {'error', Url, 'return_info_missing'};
                {'undefined', _DriveId} ->
                    {'error', Url, 'return_id_missing'};
                {_Id, 'undefined'} ->
                    {'error', Url, 'return_drive_missing'};
                {Id, DriveId} ->
                    {'ok', {DriveId, Id}, [
                        {<<"body">>, BodyJObj}
