...
format_url(Map, AttInfo) -> format_url(Map, AttInfo, default_format_url_fields()).

-spec format_url(gen_attachment:settings(), attachment_info(), url_fields()) -> kz_term:ne_binary().
format_url(#{file := Path}, _AttInfo, _Format) ->
    Path;
...
-spec format_url_from_metadata(
    gen_attachment:settings(), attachment_info(), url_fields(), kz_json:object()
) -> kz_term:ne_binary().
...
-spec format_url(gen_attachment:settings(), kz_json:object(), kz_term:proplist(), url_fields()) ->
    kz_term:ne_binary().
format_url(#{file := Path}, _JObj, _Args, _DefaultFields) ->
    Path;
...
-spec do_format_url(url_fields(), kz_json:object(), kz_term:proplist(), binary()) ->
    kz_term:ne_binary().
do_format_url(Fields, JObj, Args, Separator) ->
    FormattedFields = lists:foldl(
        fun(F, Acc) ->
...
format_url_field(_JObj, _Args, {'const', Const}, Fields) ->
    [Const | Fields];
format_url_field(_JObj, _Args, Bin, Fields) when
    is_binary(Bin)
->
    [Bin | Fields].
...
combined_path(BasePath, OtherPath) -> filename:join(BasePath, OtherPath).

-spec path_from_settings(map()) -> kz_term:api_ne_binary().
path_from_settings(#{file := Path}) ->
    Path;
path_from_settings(#{path := Path}) ->
    Path;
