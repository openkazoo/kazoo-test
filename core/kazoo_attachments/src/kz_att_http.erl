...
put_attachment(Settings, DbName, DocId, AName, Contents, Options) ->
    ...
    {ContentType, Body} = build_req_body(
        Settings, DbName, DocId, AName, Contents, DefaultContentType
    ),
    ...
    case send_request(Url, format_verb(Verb), Headers, Body) of
        ...
        {'error', ErrorUrl, Resp} ->
            Routines = [
                {fun kz_att_error:set_req_url/2, ErrorUrl}
...
build_req_body(
    #{'send_multipart' := SendMultipart} = Options,
    DbName,
    DocId,
    AName,
    Contents,
...
maybe_encode_content_part(_Settings, Contents) ->
    Contents.
...
join_url_and_querystring({Scheme, Location, Path, QueryString, Frag}, QS) ->
    kz_http_util:urlunsplit(
        {Scheme, Location, Path, kz_binary:join([QueryString, QS], <<"&">>), Frag}
    ).
...
send_request(Url, _Verb, _Headers, _Contents, Redirects, _) when
...
send_request(Url, Verb, Headers, Contents, Redirects, Debug) ->
    case kz_http:req(Verb, Url, Headers, Contents) of
        ...
        ->
            ...
            Fun = fun(URL, Tries, Data) ->
                send_request(URL, Verb, Headers, Contents, Tries, Data)
            end,
...
maybe_redirect_loop(ToUrl, Url, Redirects, Debug, Fun) ->
    case kz_json:get_value(Url, Debug) of
        ...
        _ ->
            lager:debug(
                "the request ~s got redirect to ~s but this is already in the list of visited urls",
                [ToUrl, Url]
            ),
...
-spec fetch_attachment(kz_term:ne_binary(), integer(), kz_json:object()) ->
    gen_attachment:fetch_response()
    | {'error', kz_term:ne_binary(), atom() | kz_http:ret()}.
fetch_attachment(Url, Redirects, _) when
    Redirects > ?MAX_REDIRECTS
->
    {'error', Url, 'too_many_redirects'};
