...
-define(DRV_MULTIPART_FILE_URL,
    <<(?DRV_FILE_UPLOAD_URL)/binary, "?fields=id&uploadType=multipart">>
).
...
do_put_attachment(
    ...
    case send_attachment(Authorization, Folder, TokenDocId, Name, CT, Options, Contents) of
        {'error', _, _} when Folder /= [<<"root">>] ->
            Resp = send_attachment(
                Authorization, [<<"root">>], TokenDocId, Name, CT, Options, Contents
            ),
...
handle_put_attachment_resp(Resp, _) ->
    Resp.
...
%% @end
%%------------------------------------------------------------------------------
-spec get_json_from_url(kz_term:ne_binary(), kz_term:proplist()) ->
...
resolve_ids([Id | Ids], [Parent | _] = Acc, Authorization) ->
    case resolve_id(Id, Parent, Authorization) of
        {'ok', FolderId} ->
            resolve_ids(Ids, [FolderId | Acc], Authorization);
        {'error', 'not_found'} ->
...
-spec resolve_path(map(), attachment_info(), binary()) ->
    {kz_term:ne_binaries(), kz_term:ne_binary()}.
    ...
    case kz_http:post(Url, Headers, Body) of
        {'ok', 200, ResponseHeaders, ResponseBody} ->
            ...
            case kz_json:get_value(<<"id">>, BodyJObj) of
                ...
                ContentId ->
                    {'ok', ContentId, [
                        {<<"body">>, BodyJObj} | kz_att_util:headers_as_binaries(ResponseHeaders)
                    ]}
            end;
        Resp ->
            {'error', Url, Resp}
...
-spec send_attachment(
    binary(), kz_term:binaries(), binary(), binary(), binary(), kz_term:proplist(), binary()
) ->
    {'ok', kz_term:ne_binary(), kz_term:proplist()}
    | {'error', kz_term:ne_binary(), kz_http:ret() | atom()}.
