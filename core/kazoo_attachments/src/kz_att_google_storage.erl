...
-define(DRV_BUCKET_UPLOAD_URL(B),
    <<"https://www.googleapis.com/upload/storage/v1/b/", B/binary, "/o">>
).
...
fetch_attachment(HandlerProps, DbName, DocId, AName) ->
    case kz_json:get_value(<<"gstorage">>, HandlerProps) of
        ...
        GData ->
            ...
            do_fetch_attachment(
                gstorage_token(Settings), Bucket, ContentId, HandlerProps, DbName, DocId, AName
            )
    end.
...
do_fetch_attachment(
    ...
    case kz_http:get(ContentId, Headers) of
        ...
        Resp ->
            Routines = [
                {fun kz_att_error:set_req_url/2, ContentId}
...
-spec send_attachment(
    binary(), kz_term:ne_binary(), kz_term:ne_binary(), binary(), kz_term:proplist(), binary()
) ->
    {'ok', kz_term:proplist()}
