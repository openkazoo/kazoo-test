...
%%------------------------------------------------------------------------------
-spec save_outbound(
    kz_term:ne_binary(), kz_json:object(), kz_term:api_binary(), kz_term:api_binary()
) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
...
save_outbound(Db, Doc, Original, ContentType) ->
    ...
    case
        kapps_config:get_is_true(?FAX_CONFIG_CAT, <<"store_fax_tiff">>, true) orelse
            kapps_config:get_is_true(?FAX_CONFIG_CAT, <<"store_fax_pdf">>, true)
    of
        'true' ->
            case convert_to_fax(ContentType, Original, Id) of
                {'ok', Tiff, Props} ->
                    ...
                    store_attachments(
                        Db, NewDoc, NewAtt ++ maybe_convert_to_pdf(<<"image/tiff">>, Tiff, Id)
                    );
                Error ->
                    Error
            end;
...
update_fax_props(Doc, Props) ->
    kz_json:set_values(
        [
            {<<"pvt_pages">>, props:get_value(<<"page_count">>, Props, 0)},
...
fetch(Format, Db, Doc) ->
    case kz_json:get_binary_value(<<"folder">>, Doc) of
        ...
        'undefined' ->
            {'error', <<"no folder defined in doc">>}
    end.
...
-spec maybe_save_faxable(kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()) ->
    kz_json:object().
        ...
        'true' ->
            case save_fax_doc(Db, Doc, Content, <<"image/tiff">>, ?FAX_FILENAME) of
                {'ok', NewDoc} ->
                    NewDoc;
                _Else ->
                    lager:error("failed to save fax tiff attachment for document ~p", [
                        kz_doc:id(Doc)
                    ]),
                    Doc
            end;
...
fetch_url(Url, FetchRequest) ->
    ...
    Headers = props:filter_undefined(
        [
            ...
            {"Content-Type",
                kz_json:get_string_value(<<"content_type">>, FetchRequest, <<"text/plain">>)}
        ]
    ),
...
-spec maybe_store_url_attachment(
    kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    kz_json:object().
...
-spec store_attachment(
    kz_term:ne_binary(), kz_term:api_object(), binary(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    {'ok', kz_json:object()}
    | {'error', kz_term:ne_binary()}.
...
store_attachment(_, Doc, _Content, _CT, _Name, 0) ->
    lager:error(
        "max retry saving attachment ~s on fax id ~s rev ~s",
...
-spec attempt_save(
    kz_term:ne_binary(), kz_json:object(), binary(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    {'ok', kz_json:object()}
