...
maybe_log_unlock(_Key, _Type, _Peek) ->
    'ok'.
...
exec_timeout_callbacks(_MonitorName, _MonitorRef, 'false') ->
    'ok';
...
exec_timeout_callbacks(Name, MatchSpec) ->
    _ = [
        exec_timeout_callback(Name, list_to_tuple(Callback))
     || Callback <- ets:select(Name, MatchSpec)
    ],
    'ok'.

-spec exec_timeout_callback(atom(), {any(), reference(), fun()}) -> 'ok'.
exec_timeout_callback(Name, {Key, Value, Callback}) when
...
expire_objects(Name, Tables, Objects) ->
    ...
    lager:debug("  ~p", [Keys]),
    lists:foreach(
        fun(Table) -> maybe_remove_objects(Table, Keys) end,
        [Name | Tables]
    ),
    length(Objects).
...
exec_store_callbacks(_MonitorName, _Key, _Value, 'false') ->
    'ok';
...
exec_callbacks(Name, MatchSpec, Msg) ->
    _Pids = [
        exec_callback(Callback, [K, V, Msg])
