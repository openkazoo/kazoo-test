...
-type store_options() :: [
    {'origin', origin_tuple() | origin_tuples()}
    | {'expires', timeout()}
...
dump_table(Tab, ShowValue) ->
    ...
    _ = [
        display_cache_obj(CacheObj, ShowValue, Now)
...
-spec wait_for_key_local(kz_types:server_ref(), any()) ->
    {'ok', any()}
    | {'error', 'timeout'}.
