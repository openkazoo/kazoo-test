...
-spec add_origin_pointers(atom(), cache_obj(), 'undefined' | origin_tuple() | origin_tuples()) ->
    'ok'.
add_origin_pointers(_Name, _CacheObj, 'undefined') ->
    'ok';
...
add_origin_pointers(Name, CacheObj, Origins) ->
    ...
    _ = [
        add_origin_pointer(PointerTab, CacheObj, Origin)
     || Origin <- Origins
    ],
    'ok'.
...
maybe_add_db_binding([[]]) -> [[]];
maybe_add_db_binding(BindingProps) -> [?DATABASE_BINDING | BindingProps].
-endif.
...
handle_cast(
    {'kz_amqp_channel', {'new_channel', 'false'}},
    #state{
        name = Name,
    ...
    {'noreply', State};
handle_cast(
    {'kz_amqp_channel', {'new_channel', 'true'}},
    #state{
        name = Name,
...
match_db_changed(Db) ->
    [
        ...
        {#cache_obj{origin = {'db', Db, '_'}, _ = '_'}, [], ['$_']},
        {#cache_obj{origin = {'type', <<"database">>, Db}, _ = '_'}, [], ['$_']}
    ].

-spec match_doc_changed(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    ets:match_spec().
match_doc_changed(Db, Type, Id) ->
    [
        {#cache_obj{origin = {'db', Db}, _ = '_'}, [], ['$_']},
        {#cache_obj{origin = {'db', Db, Type}, _ = '_'}, [], ['$_']},
        {#cache_obj{origin = {'db', Db, Id}, _ = '_'}, [], ['$_']},
        {#cache_obj{origin = {'type', Type, Id}, _ = '_'}, [], ['$_']},
        {#cache_obj{origin = {'type', Type}, _ = '_'}, [], ['$_']}
    ].
