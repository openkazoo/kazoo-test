...
command(#state{} = _Model) ->
    frequency([
        {1,
            {'call', 'kz_cache', 'store_local', [
                ?SERVER, key(), value(), [{'expires', range(1, 2)}]
            ]}},
        ...
        {1,
            {'call', 'kz_cache', 'mitigate_stampede_local', [
                ?SERVER, key(), [{'expires', range(1, 2)}]
            ]}},
...
next_state(
    #state{cache = Cache, now_ms = NowMs} = Model,
    _V,
    ...
    case lists:keyfind(Key, #cache_obj.key, Cache) of
        ...
        'false' ->
            next_state(
                ...
                V,
                {'call', 'kz_cache', 'store_local', [
                    ?SERVER, Key, {kz_cache:mitigation_key(), self()}, Options
                ]}
            )
    end;
next_state(
...
next_state(
    ...
    case lists:keyfind(Key, #cache_obj.key, Cache) of
        ...
        #cache_obj{} = Obj ->
            Model#state{
                cache = lists:keyreplace(Key, #cache_obj.key, Cache, Obj#cache_obj{
                    timestamp_ms = NowMs
                })
            }
    end;
...
next_state(
    ...
    Model#state{
        cache = lists:foldl(fun(Obj, Acc) -> maybe_expire(Obj, Acc, NowMs) end, [], Cache),
        now_ms = NowMs
    }.
