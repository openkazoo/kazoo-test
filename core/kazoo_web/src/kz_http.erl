...
-type httpc_ret() ::
    ...
    | {'ok', 'saved_to_file'}
    | {'error',
        {'connect_failed' | 'malformed_url', any()}
        | {'send_failed', any()}
        | any()}.
...
-type req_id() ::
    {'http_req_id', reference()}
    | {'ok', reference()}
    | reference().

-type ret() ::
    {'ok', pos_integer(), headers(), kz_term:text()}
    | {'ok', 'saved_to_file'}
    | {'error', any()}
    | req_id().
...
execute_request(Method, Request, Opts) ->
    ...
    F = fun() ->
        {Method, element(?REQ_URL_INDEX, Request),
...
handle_response({'ok', 'saved_to_file'} = Ok) ->
    Ok;
handle_response({'ok', ReqId}) when
    is_reference(ReqId)
->
    {'http_req_id', ReqId};
handle_response({'ok', {{_, StatusCode, _}, Headers, Body}}) when
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_basic_auth(kz_term:proplist(), kz_term:proplist()) ->
...
build_request(Method, Url, Headers, _Body) when
    Method =:= 'options';
    Method =:= 'get';
    Method =:= 'head';
    Method =:= 'trace'
->
    {lists:flatten(kz_term:to_list(Url)), ensure_string_headers(Headers)};
build_request(Method, Url, Headers, Body) when
    Method =:= 'post';
    Method =:= 'put';
    Method =:= 'patch';
    Method =:= 'delete'
->
    ContentType = props:get_first_defined(
        [
            "Content-Type",
            "content-type",
            'content_type',
