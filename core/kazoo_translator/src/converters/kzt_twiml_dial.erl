...
exec(Call, [#xmlText{type = 'text'} | _] = DialMeTxts, Attrs) ->
    ...
    case cleanup_dial_me(kz_xml:texts_to_binary(DialMeTxts)) of
        ...
        DialMe ->
            dial_me(Call, Attrs, knm_converters:normalize(DialMe))
    end;
exec(
    Call,
    [
...
exec(
    ...
    gen_listener:add_binding(
        kzt_util:get_amqp_listener(Call),
        'conference',
        [{'restrict_to', [{'event', {ConfId, kapps_call:call_id_direct(Call)}}]}]
    ),
...
dial_me(Call, Attrs, DialMe) ->
    ...
    Call1 = setup_call_for_dial(
        kapps_call:set_request(request_id(DialMe, Call), Call),
        Props
    ),
...
setup_call_for_dial(Call, Props) ->
    Setters = [
        {fun kapps_call:set_caller_id_number/2, caller_id(Props, Call)},
...
maybe_end_dial(Call, _Props, 'undefined') ->
    ...
    % will progress to next TwiML element
    {'ok', Call};
...
-spec xml_elements_to_endpoints(kapps_call:call(), kz_types:xml_els(), kz_json:objects()) ->
    kz_json:objects().
xml_elements_to_endpoints(_, [], Acc) ->
    Acc;
xml_elements_to_endpoints(
    Call,
    [
        #xmlElement{
            name = 'Device',
    ...
    case kz_endpoint:build(DeviceId, Call) of
        {'ok', DeviceEPs} ->
            xml_elements_to_endpoints(Call, EPs, DeviceEPs ++ Acc);
        {'error', _E} ->
...
xml_elements_to_endpoints(
    ...
    case get_endpoints(UserId, kz_json:new(), Call) of
        [] ->
            ...
            xml_elements_to_endpoints(Call, EPs, Acc);
        UserEPs ->
            xml_elements_to_endpoints(Call, EPs, UserEPs ++ Acc)
    end;
xml_elements_to_endpoints(
    Call,
    [
...
member_flags(ConfProps, _) ->
    kz_json:from_list([
        {<<"join_muted">>, props:is_true('muted', ConfProps, 'false')},
...
conference_profile(AccountId, ConfProps) ->
    kz_json:from_list(
        [
            ...
            {<<"caller-controls">>,
                props:get_binary_value('callerControls', ConfProps, <<"default">>)},
            {<<"caller-id-name">>,
                props:get_binary_value(
                    'callerIdName', ConfProps, kapps_call:unknown_caller_id_name(AccountId)
                )},
            {<<"caller-id-number">>,
                props:get_binary_value(
                    'callerIdNumber', ConfProps, kz_privacy:anonymous_caller_id_number(AccountId)
                )},
            ...
            {<<"moderator-controls">>,
                props:get_binary_value('moderatorControls', ConfProps, <<"default">>)},
...
get_moh(ConfProps) ->
    Default = kapps_config:get_ne_binary(
        <<"pivot.twiml">>, <<"conference_moh">>, <<"$${hold_music}">>
    ),
