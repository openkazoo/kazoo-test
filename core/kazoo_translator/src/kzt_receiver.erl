...
-export([
    wait_for_offnet/1, wait_for_offnet/2,
    wait_for_noop/2,
    wait_for_hangup/1,
...
-spec collect_dtmfs(
    kapps_call:call(), kz_term:api_ne_binary(), timeout(), pos_integer(), function()
) ->
    collect_dtmfs_return().
collect_dtmfs(
    Call,
...
-spec collect_dtmfs(
    kapps_call:call(), kz_term:api_ne_binary(), timeout(), pos_integer(), function(), binary()
) ->
    collect_dtmfs_return().
collect_dtmfs(
    Call,
        ...
        {'other', {'DOWN', Ref, 'process', Pid, _Reason}} ->
            case kzt_util:get_gather_pidref(Call) of
                {Pid, Ref} when is_pid(Pid), is_reference(Ref) ->
                    ...
                    collect_dtmfs(
                        kzt_util:set_gather_pidref('undefined', Call),
                        FinishKey,
                        collect_decr_timeout(Call, Timeout, Start),
                        N,
                        OnFirstFun,
...
-spec handle_dtmf(
    ...
    collect_dtmfs_return().
handle_dtmf(Call, FinishKey, _Timeout, _N, _OnFirstFun, _Collected, FinishKey) when
    is_binary(FinishKey)
->
...
handle_dtmf(Call, FinishKey, Timeout, N, OnFirstFun, <<>>, DTMF) ->
    ...
    _ =
        try OnFirstFun(Call1) of
            _ -> 'ok'
        catch
            _:_ -> 'ok'
        end,
    collect_dtmfs(
        kzt_util:set_gather_pidref('undefined', Call1),
        FinishKey,
        Timeout,
        N,
        fun default_on_first_fun/1,
...
handle_dtmf(Call, FinishKey, Timeout, N, OnFirstFun, Collected, DTMF) ->
    ...
    collect_dtmfs(
        kzt_util:add_digit_collected(DTMF, Call),
        FinishKey,
        Timeout,
        N,
        OnFirstFun,
...
record_loop(Call, SilenceTimeout) ->
    case wait_for_call_event(Call, <<"RECORD_STOP">>) of
        {'ok', EvtJObj} ->
            ...
            case
                {kz_json:is_true(<<"Silence-Terminated">>, EvtJObj, 'false'), SilenceTimeout >= Len}
            of
                {'true', 'true'} ->
                    lager:debug(
                        "recording stopped by silence, and was ~b s long (timeout was ~b, considering this empty",
                        [Len, SilenceTimeout]
                    ),
                    {'empty', Call};
                _ ->
                    Fs = [
                        {fun kzt_util:set_digit_pressed/2, DTMF},
        ...
        {'error', 'channel_destroy', EvtJObj} ->
            ...
            Fs = [
                {fun kzt_util:set_digit_pressed/2, <<"hangup">>},
                {fun kzt_util:set_recording_duration/2, Len}
            ],
            {'ok', kapps_call:exec(Fs, Call)};
        {'error', E, _} = ERR ->
...
wait_for_offnet(Call, DialProps) ->
    ...
    wait_for_offnet_events(#dial_req{
        call = kzt_util:clear_digits_collected(Call),
        ...
        {'ok', JObj} ->
            case kz_util:get_event_type(JObj) of
                ...
                {<<"call_event">>, <<"CHANNEL_DESTROY">>} ->
                    ...
                    {'ok',
                        kzt_util:update_call_status(
                            call_status(kz_json:get_value(<<"Hangup-Cause">>, JObj)), Call
                        )};
                _Type ->
                    wait_for_hangup(Call)
            end;
...
wait_for_conference(Call) ->
    ...
    wait_for_conference_events(#dial_req{
        call = Call,
        hangup_dtmf = HangupDTMF,
        collect_dtmf = 'true',
        record_call = RecordCall,
...
call_status(<<"ORIGINATOR_CANCEL">>) ->
    ?STATUS_COMPLETED;
call_status(<<"NORMAL_CLEARING">>) ->
    ?STATUS_COMPLETED;
call_status(<<"SUCCESS">>) ->
    ?STATUS_COMPLETED;
call_status(<<"NO_ANSWER">>) ->
    ?STATUS_NOANSWER;
call_status(<<"USER_BUSY">>) ->
    ?STATUS_BUSY;
...
-spec process_offnet_event(dial_req(), kz_json:object()) ->
    {'ok', kapps_call:call()}.
process_offnet_event(
    #dial_req{call = Call} = OffnetReq,
    JObj
) ->
    ...
    case {kz_util:get_event_type(JObj), kz_call_event:call_id(JObj)} of
        ...
        {{<<"call_event">>, <<"CHANNEL_BRIDGE">>}, CallId} ->
            ...
            Updates = [
                {fun kzt_util:set_media_meta/2, MediaJObj},
        ...
        {{<<"call_event">>, <<"CHANNEL_PARK">>}, CallId} ->
            lager:debug(
                "channel ~s has parked, probably means none of the bridge strings succeeded", [
                    CallId
                ]
            ),
            {'ok', Call};
...
handle_offnet_b_leg(#dial_req{call = Call} = OffnetReq, JObj) ->
    ...
    _ = gen_listener:add_binding(Srv, 'call', [
        {'callid', BLeg},
        {'restrict_to', ['events']}
    ]),

    Updates = [
        {fun kzt_util:set_dial_call_sid/2, BLeg},
...
handle_hangup(Call, _JObj, HangupCause) ->
    ...
    Updates = [
        {fun kzt_util:update_call_status/2, call_status(HangupCause)},
...
maybe_hangup_other_leg(_Call, 'undefined') ->
    'ok';
...
process_conference_event(#dial_req{call = Call} = OffnetReq, JObj) ->
    case kz_util:get_event_type(JObj) of
        ...
        {_Cat, _Name} ->
            lager:debug(
                "unhandled event for ~s: ~s: ~s",
                [_Cat, _Name, kz_call_event:application_name(JObj)]
            ),
            wait_for_conference_events(update_offnet_timers(OffnetReq))
    end.

maybe_start_recording(#dial_req{record_call = 'false'}) ->
    'undefined';
maybe_start_recording(#dial_req{
    record_call = 'true',
    call = Call,
    call_time_limit = CallTimeLimit
}) ->
    ...
    kapps_call_command:record_call(
        [{<<"Media-Name">>, RecordingName}],
        <<"start">>,
        CallTimeLimit,
        Call
    ),
...
which_time('undefined', Timelimit) when
    is_integer(Timelimit), Timelimit > 0
->
    Timelimit;
which_time(Timeout, _) when
