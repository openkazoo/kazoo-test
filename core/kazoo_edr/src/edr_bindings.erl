...
bind(Binding, Module, Fun, Payload) ->
    kazoo_bindings:bind(binding_keys(Binding), ?MODULE, 'push', #binding_payload{
        module = Module,
        function = Fun,
        binding = Binding,
        payload = Payload
...
%% Severity
binding_keys(#edr_binding{exact_severity = 'false', severity = Severity} = Binding) when
    not is_list(Severity)
->
    binding_keys(Binding#edr_binding{
        exact_severity = 'true', severity = levels(Severity, ?EDR_SEVERITY_LEVELS)
    });
...
%% Verbosity
binding_keys(#edr_binding{exact_verbosity = 'false', verbosity = Verbosity} = Binding) when
    not is_list(Verbosity)
->
    binding_keys(Binding#edr_binding{
        exact_verbosity = 'true', verbosity = levels(Verbosity, ?EDR_VERBOSITY_LEVELS)
    });
...
binding_keys(#edr_binding{
    ...
    [
        binding_key(S, V, Acc, App)
     || S <- Severities, V <- Verbosities, Acc <- AccountIds, App <- AppNames
    ].

%% Levels equal to or more significant than the specified level
-spec levels
    (edr_severity(), edr_severities()) -> edr_severities();
    (edr_verbosity(), edr_verbosities()) -> edr_verbosities().
levels(Level, AllLevels) ->
    lists:dropwhile(fun(L) -> L =/= Level end, AllLevels).

-spec binding_key(edr_severity(), edr_verbosity(), kz_term:api_ne_binary(), kz_term:ne_binary()) ->
    kz_term:ne_binary().
...
should_push(
    #edr_binding{account_id = AccountId, include_descendants = true},
    #edr_event{account_tree = AccountTree}
) ->
    AccountTree =/= 'undefined' andalso
        lists:member(AccountId, AccountTree);
should_push(_Binding, _Event) ->
    'true'.

-spec bindings_from_json
    (kz_json:object()) -> edr_binding();
...
-spec bindings_to_json
    (edr_binding()) -> kz_json:object();
...
bindings_to_json(Binding = #edr_binding{}) ->
    kz_json:from_list([
        {<<"account_id">>, Binding#edr_binding.account_id},
...
-spec severity_or_verbosity_to_json
    (edr_severity() | edr_verbosity()) -> kz_term:ne_binary();
