...
%% @end
%%------------------------------------------------------------------------------
-spec update_tree(kz_term:ne_binary(), kz_term:ne_binaries(), kz_term:ne_binary()) ->
...
update_tree(Account, NewTree, NewResellerId) ->
    ...
    {'ok', NewServicesJObj} = kz_datamgr:save_doc(
        ?KZ_SERVICES_DB, kz_json:set_values(Props, ServicesJObj)
    ),
...
js_design_doc() ->
    ...
    Views = [
        {<<"quantify">>, kz_json:from_list(Quantify)},
...
migrate_service_plans(_AccountDb, []) ->
    'no_return';
...
migrate_service_plan(AccountDb, JObj) ->
    ...
    Routines = [
        fun migrate_service_plans_bookkeeper/2,
...
migrate_service_plans_bookkeeper(AccountDb, JObj) ->
    ...
    case migrate_bookkeepers(AccountDb, JObj, Keys) of
        ...
        BookkeeperId ->
            ...
            Setters = [
                {fun kzd_service_plan:set_bookkeeper_id/2, BookkeeperId},
...
-spec migrate_bookkeepers(kz_term:ne_binary(), kz_json:object(), kz_term:ne_binaries()) ->
    kz_term:api_binary().
migrate_bookkeepers(AccountDb, JObj, Keys) ->
    migrate_bookkeepers(AccountDb, JObj, Keys, 'undefined').

-spec migrate_bookkeepers(
    kz_term:ne_binary(), kz_json:object(), kz_term:ne_binaries(), kz_term:api_binary()
) -> kz_term:api_binary().
migrate_bookkeepers(_AccountDb, _JObj, [], BraintreeId) ->
    BraintreeId;
...
-spec merge_bookkeeper(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    kz_term:api_binary().
merge_bookkeeper(AccountDb, Bookkeeper, NewMappings) ->
    ...
    {'ok', JObj} = kz_datamgr:save_doc(
        AccountDb, kzd_bookkeeper:set_mappings(Bookkeeper, Mappings)
    ),
    kz_doc:id(JObj).

-spec create_bookkeeper(kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary()) ->
    kz_term:api_binary().
create_bookkeeper(AccountDb, Mappings, Key) ->
    ...
    Setters = [
        {fun kzd_bookkeeper:set_name/2, Name},
...
-spec migrate_service_plans_pvt_parameters(kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object().
...
migrate_service_plans_ratedeck(_AccountDb, JObj) ->
    ...
    Setters = props:filter_empty(
        [
            {fun kzd_service_plan:set_ratedeck_id/2, get_ratedeck_binary(<<"ratedeck">>, Plan)},
            {
...
migrate_services_doc(Services, JObj) ->
    ...
    Routines = [
        fun migrate_services_ratedeck/2,
...
migrate_services_ratedeck(Services, JObj) ->
    ...
    Setters = props:filter_empty(
        [
            {fun kzd_services:set_ratedeck_id/2, get_ratedeck_binary(<<"ratedeck">>, Quantities)},
            {
...
-spec migrate_services_plans(kz_json:object(), kz_term:ne_binaries(), kz_json:object()) ->
    kz_json:object().
migrate_services_plans(_JObj, [], Plans) ->
    Plans;
...
plan_overrides(Plan) ->
    PlanOverrides = kz_json:get_ne_json_value(
        <<"overrides">>,
        Plan,
        kz_json:new()
    ),
...
migrate_account_apps(PlanOverrides, [AppName | AppNames]) ->
    ...
    case kz_term:is_empty(Cleaned) of
        'true' ->
            migrate_account_apps(PlanOverrides, AppNames);
        'false' ->
            JObj = kz_json:set_value(
                [<<"account_apps">>, AppName],
                Cleaned,
                PlanOverrides
            ),
...
plan_vendor_id(JObj) ->
    Account = kz_json:get_first_defined(
        [
            <<"account_id">>,
            <<"account_db">>,
            <<"vendor_id">>
...
-spec migrate_ui_apps(kz_json:object(), kz_term:ne_binaries(), kz_json:object()) ->
    kz_json:object().
migrate_ui_apps(_JObj, [], Apps) ->
    Apps;
migrate_ui_apps(JObj, [AppName | AppNames], Apps) ->
    case kz_json:get_ne_json_value([<<"ui_apps">>, AppName], JObj) of
        'undefined' ->
            migrate_ui_apps(JObj, AppNames, Apps);
        App ->
...
-spec maybe_add_ui_app(
    kz_term:ne_binary(), kz_term:api_binary(), kz_term:ne_binary(), boolean(), kz_json:object()
) -> kz_json:object().
...
fix_plan_app(AccountDb, JObj) ->
    io:format(
        "verifying service plan ~s/~s applications~n",
        [
...
-spec fix_plan_app(kz_term:ne_binary(), kz_json:object(), kz_term:ne_binaries()) ->
    kz_json:object().
fix_plan_app(_AccountDb, JObj, []) ->
    JObj;
fix_plan_app(AccountDb, JObj, [AppId | AppIds]) ->
    ...
    case kz_term:is_empty(Props) of
        'true' ->
            fix_plan_app(AccountDb, JObj, AppIds);
        'false' ->
            UpdatedApps =
                kz_json:set_values(
                    Props,
...
-spec maybe_open_app(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    kz_term:proplist().
...
maybe_find_app(?NE_BINARY = VendorId, Name) ->
    ...
    case kz_datamgr:get_results(VendorDb, <<"apps_store/crossbar_listing">>, ViewOptions) of
        {'ok', [App | _]} ->
            Id = kz_doc:id(App),
            io:format(
                "  correcting invalid application ~s with ~s/~s~n",
                [Name, VendorId, Id]
            ),
            [
                {[Id, <<"name">>], Name},
        ...
        _Else ->
            io:format(
                "  unable to find valid application for ~s in vendor ~s~n",
                [Name, VendorId]
            ),
            []
    end.
...
log_discrepancy_correction(CurrentJObj, ReconciledJObj) ->
    ...
    ReconciledQuantities = kz_json:get_ne_json_value(
        <<"quantities">>, ReconciledJObj, kz_json:new()
    ),
    FlattenedCurrent = flatten_quantities(CurrentQuantities),
    _ = [
        io:format(
            "      updated ~s from ~p to ~p~n",
            [
                kz_binary:join(Key, <<".">>),
                CurrentValue,
                ReconciledValue
...
flatten_quantities(JObj) ->
    [
        {[Type, Category, Item], kz_json:get_integer_value([Type, Category, Item], JObj, 0)}
...
log_services_recovery_results([]) ->
    'ok';
log_services_recovery_results([JObj | JObjs]) ->
    Id = kz_doc:id(JObj),
    _ =
        case kz_json:get_value(<<"reason">>, JObj) of
...
rebuild_services_db() ->
    ...
    case compare_services_backup() of
        'true' ->
            io:format("rebuild completed successfully~n", []);
        'false' ->
            io:format(
                "discrepancies found between restored and backed up service documents!~n", []
            ),
...
compare_services_backup() ->
    ...
    lists:all(
        fun({Id, Original}) -> kz_json:are_equal(props:get_value(Id, Backups), Original) end,
        Originals
    ).
...
log_services_backup_failures([]) ->
    'error';
log_services_backup_failures([JObj | JObjs]) ->
    _ = io:format(
        "  failed to backup ~s: ~s~n",
        [
            kz_doc:id(JObj),
...
remove_orphaned_services() ->
    ...
    Count > 0 andalso
        io:format("removed ~p service docs~n", [Count]),
    'no_return'.

-spec maybe_remove_orphan(kz_json:object() | kz_term:ne_binary(), non_neg_integer()) ->
    non_neg_integer().
maybe_remove_orphan(<<"_design/", _/binary>>, Count) ->
    Count;
...
topup_status() ->
    IsEnabled =
        case kapps_config:get_is_true(?TOPUP_CONFIG, <<"enable">>) of
            'true' -> "enabled";
            _ -> "disabled"
        end,
...
topup_status(AccountId) ->
    ...
    io:format(
        "| Current Balance | Notify threshold | Top up threshold | Top up Amount | Auto top up in 24 hours |~n"
    ),
    io:format(
        "+=================+==================+==================+===============+=========================+~n"
    ),
    io:format(
        "| ~-15w | ~-16w | ~-16w | ~-13w | ~-23w |~n",
        [
