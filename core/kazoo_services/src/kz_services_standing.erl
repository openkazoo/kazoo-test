...
-type unacceptable_details() :: #{
    reason => kz_term:ne_binary(),
    message => kz_term:ne_binary()
}.

-type acceptable_return() ::
    {'true', kz_term:ne_binary()}
...
-spec acceptable(kz_term:ne_binary() | kz_services:services(), acceptable_options()) ->
    acceptable_return().
...
acceptable(Services, Options) ->
    lager:debug(
        "checking if account ~s is in good standing: ~p",
    ...
    GoodFuns = [
        fun should_enforce_good_standing/2,
        fun no_plan_is_good/2,
        fun check_bookkeeper/2
    ],
    maybe_use_cache(Services, NewOptions, GoodFuns).

-spec maybe_use_cache(kz_services:services(), acceptable_options(), acceptable_funs()) ->
    acceptable_return().
...
-spec maybe_cache_result(kz_services:services(), acceptable_options(), acceptable_return()) ->
    acceptable_return().
...
-spec acceptable_fold(kz_services:services(), acceptable_options(), acceptable_funs()) ->
    acceptable_return().
acceptable_fold(Services, _Options, []) ->
    Msg = io_lib:format(
        "account ~s is in good standing",
...
acceptable_fold(Services, Options, [Fun | Funs]) ->
    case Fun(Services, Options) of
        {'true', Reason} = _TheGood ->
            lager:debug(
                "account ~s is in good standing: ~s",
                [kz_services:account_id(Services), Reason]
            ),
            {'true', Reason};
        {'false', Reason} = _TheBad ->
            lager:debug(
                "account ~s is not in good standing: ~p",
                [kz_services:account_id(Services), Reason]
            ),
            {'false', Reason};
        'not_applicable' ->
            acceptable_fold(Services, Options, Funs)
    end.
...
should_enforce_good_standing(_Services, _Options) ->
    case ?KZ_SERVICE_ENFORCE_GOOD_STANDING of
        'true' -> 'not_applicable';
        'false' -> {'true', <<"good standing not required">>}
    end.
...
no_plan_is_good(Services, _Options) ->
    case kz_services:has_plans(Services) of
        'true' -> 'not_applicable';
        'false' -> {'true', <<"no service plans assigned">>}
    end.
...
handle_bookkeeper_results([{_Invoice, {'ok', Result}} | Results]) ->
    case kz_json:get_ne_binary_value(<<"Status">>, Result) =:= kzd_services:status_good() of
        ...
        'false' ->
            DefaultMessage =
                <<"There is a billing issue with this account, please contact your sales representative.">>,
            {'false', #{
                reason => kz_json:get_ne_binary_value(
                    <<"Reason">>, Result, <<"bookkeeper_rejection">>
                ),
...
-spec check_bookkeeper(
    kz_term:ne_binary(), kz_services_invoice:invoice(), kz_services:services(), acceptable_options()
) ->
