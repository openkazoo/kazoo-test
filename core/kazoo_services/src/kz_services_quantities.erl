...
fetch_port(AccountId, View) ->
    ...
    case kz_datamgr:get_results(?KZ_PORT_REQUESTS_DB, View, ViewOptions) of
        {'ok', JObjs} ->
            Quantities = [
                {
                    port_key(kz_json:get_value(<<"key">>, JObj)),
        ...
        {'error', _Message} ->
            ?SUP_LOG_ERROR("failed to query port quantities account: ~s with error: ~p", [
                AccountId, _Message
            ]),
            kz_json:new()
    end.
...
fetch(Database, View, ViewOptions) ->
    case kz_datamgr:get_results(Database, View, ViewOptions) of
        ...
        {'error', _Message} ->
            ?SUP_LOG_ERROR("failed to query account quantities: ~s with error: ~p", [
                Database, _Message
            ]),
            kz_json:new()
    end.
...
-spec category(kz_services:services(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    non_neg_integer().
category(Services, CategoryName, ItemExceptions) ->
    maybe_log(
        "account category ~s has a quantity of ~p",
        [CategoryName],
        logless_category(Services, CategoryName, ItemExceptions)
    ).

-spec logless_category(kz_services:services(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    non_neg_integer().
logless_category(Services, CategoryName, ItemExceptions) ->
...
item(Services, CategoryName, ItemName) ->
    maybe_log(
        "account item ~s/~s has a quantity of ~p",
        [CategoryName, ItemName],
        logless_item(Services, CategoryName, ItemName)
    ).

-spec logless_item(kz_services:services(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    non_neg_integer().
logless_item(Services, CategoryName, ItemName) ->
    case kz_services:is_deleted(Services) of
        'true' ->
...
-spec cascade_category(kz_services:services(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    non_neg_integer().
cascade_category(Services, CategoryName, ItemExceptions) ->
    case kz_services:is_deleted(Services) of
        ...
        'false' ->
            ...
            maybe_log(
                "cascade category ~s has a quantity of ~p",
                [CategoryName],
...
%% @end
%%------------------------------------------------------------------------------
-spec cascade_item(kz_services:services(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    non_neg_integer().
cascade_item(Services, CategoryName, ItemName) ->
    case kz_services:is_deleted(Services) of
        ...
        'false' ->
            CascadeQuantities = kz_services:cascade_quantities(Services),
            maybe_log(
                "cascade item ~s/~s has a quantity of ~p",
                [CategoryName, ItemName],
...
calculate_updates(Services, CurrentJObj, ProposedJObjs) when
    not is_list(CurrentJObj)
->
    calculate_updates(Services, [CurrentJObj], ProposedJObjs);
calculate_updates(Services, CurrentJObjs, ProposedJObj) when
    not is_list(ProposedJObj)
->
...
-spec calculate_updates(kz_services:services(), billables(), billables(), kz_term:proplist()) ->
    kz_term:proplist().
calculate_updates(_Services, [], [], Updates) ->
    Updates;
...
-spec split_jobjs(kz_json:objects(), kz_term:ne_binary()) ->
    {kz_term:api_object(), kz_json:objects()}.
...
calculate_updates(Services, JObj) ->
    case
        kz_json:is_false(<<"enabled">>, JObj) orelse
            kz_json:is_true(<<"pvt_deleted">>, JObj, 'false')
    of
        'true' ->
            [];
        'false' ->
...
-spec calculate_qubicle_recipient_updates(kz_json:object(), kz_term:proplist()) ->
    kz_term:proplist().
calculate_qubicle_recipient_updates(JObj, Updates) ->
    case
        kz_doc:type(JObj) =:= <<"user">> andalso
            kz_json:is_true([<<"qubicle">>, <<"enabled">>], JObj, 'false')
    of
        'false' ->
            Updates;
        'true' ->
            Offering = kz_json:get_ne_value(
                [<<"qubicle">>, <<"recipient">>, <<"offering">>], JObj, <<"basic">>
            ),
...
-spec calculate_vmbox_updates_foldl(kz_json:object()) ->
    fun((kz_term:ne_binary(), kz_term:proplist()) -> kz_term:proplist()).
calculate_vmbox_updates_foldl(JObj) ->
    fun
        (<<"transcription">> = Key, Updates) ->
...
is_first_transition_to_state(JObj) ->
    StateTransitions =
        kzd_port_requests:get_transition(
            JObj,
...
compare_serivce_update(Current, CurrentValue, Key, ProposedValue, Proposed, Updates) ->
    ...
    case ProposedValue - CurrentValue of
        ...
        Value ->
            compare_service_updates(proplists:delete(Key, Current), Proposed, [
                {Key, Value} | Updates
            ])
    end.
...
sum_values(JObj, InitialSum) ->
    kz_json:foldl(
        fun(_ItemName, ItemQuantity, Sum) ->
            ItemQuantity + Sum
        end,
        InitialSum,
        JObj
