...
create(Services, Plan) ->
    lists:foldl(
        fun({CategoryName, ItemName}, Items) ->
...
-spec get_item_plan(kz_services_plan:plan(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kzd_item_plan:doc().
get_item_plan(Plan, CategoryName, ItemName) ->
    ...
    case kzd_service_plan:item(PlanJObj, CategoryName, ItemName) of
        'undefined' ->
            lager:info(
                "unable to find item plan ~s/~s, using generic",
...
-type tentative_items() :: [
    {kz_services_item:item() | 'undefined', kz_services_item:item() | 'undefined'}
].
...
-spec maybe_annotate(
    kz_term:ne_binary(), kz_json:object(), kz_term:api_binary(), kz_services_item:item()
...
%% @end
%%------------------------------------------------------------------------------
-spec difference(kz_services_item:item(), kz_services_item:item(), difference_routines()) ->
    kz_json:object().
difference(Item1, Item2, Routines) ->
...
-spec difference_number(kz_term:api_float() | integer(), kz_term:api_float() | integer()) ->
    kz_term:api_float() | integer().
difference_number(Value, Value) -> 'undefined';
difference_number('undefined', Value) -> difference_number(0, Value);
difference_number(Value, 'undefined') -> difference_number(Value, 0);
difference_number(Value1, Value2) -> Value2 - Value1.

-spec difference_list(kz_term:api_ne_binaries(), kz_term:api_ne_binaries()) ->
    kz_term:proplist() | 'undefined'.
difference_list(List, List) ->
    'undefined';
