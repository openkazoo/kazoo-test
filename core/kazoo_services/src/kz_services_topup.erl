...
-type topup_return() ::
    {'ok', kz_transaction:transaction() | 'undefined', kz_ledger:ledger()}
...
should_topup(?NE_BINARY = AccountId, AvailableUnits) ->
    case get_topup(AccountId) of
        ...
        {'ok', _ReplenishUnits, ThresholdUnits} ->
            lager:info(
                "checking if account ~s balance $~w is below top up threshold $~w",
                [
                    AccountId,
...
-spec should_topup(kz_term:ne_binary(), kz_currency:units(), kz_currency:units()) ->
    'true'
    | {'error', error()}.
should_topup(AccountId, AvailableUnits, ThresholdUnits) when
    AvailableUnits < ThresholdUnits
->
    case
        do_transactions_have_topup(AccountId) orelse
            do_ledgers_have_topup(AccountId)
    of
        'true' ->
            lager:info(
                "today auto top up for ~s already done, skipping...",
                [AccountId]
            ),
            {'error', 'topup_daily_limit'};
        'false' ->
            lager:info(
                "no top up transactions or ledgers found for ~s, processing...",
                [AccountId]
            ),
            'true'
    end;
should_topup(_AccountId, _AvailableUnits, _ThresholdUnits) ->
    lager:debug(
        "balance (~p) is still > to threshold (~p) for account ~s",
        [
...
do_transactions_have_topup([]) ->
    'false';
do_transactions_have_topup([Transaction | Transactions]) ->
    case
        kz_transaction:executor_module(Transaction) =:= ?EXECUTOR_MODULE andalso
            automatic_trigger(kz_transaction:executor_trigger(Transaction))
    of
...
do_ledgers_have_topup([]) ->
    'false';
do_ledgers_have_topup([Ledger | Ledgers]) ->
    case
        kz_ledger:source_service(Ledger) =:= ?SOURCE_SERVICE andalso
            kz_ledger:executor_module(Ledger) =:= ?EXECUTOR_MODULE andalso
            automatic_trigger(kz_ledger:executor_trigger(Ledger))
    of
...
-spec get_account_topup(kzd_accounts:doc()) ->
    ...
    | {'error', error()}.
get_account_topup(AccountJObj) ->
    case {kzd_accounts:topup_amount(AccountJObj), kzd_accounts:topup_threshold(AccountJObj)} of
        {'undefined', _} ->
            {'error', 'amount_undefined'};
        {_, 'undefined'} ->
            {'error', 'threshold_undefined'};
        {Amount, Threshold} when
            Amount > 0 andalso
                Threshold > 0
        ->
            {'ok', kz_currency:dollars_to_units(Amount), kz_currency:dollars_to_units(Threshold)};
        {_, _} ->
            {'error', 'amount_and_threshold_undefined'}
    end.
...
-spec maybe_topup(
    kz_term:ne_binary(), kz_currency:units(), kz_currency:units(), kz_currency:units()
) ->
    topup_return().
maybe_topup(AccountId, AvailableUnits, ReplenishUnits, ThresholdUnits) ->
...
-spec create_topup_transaction(
    kz_term:ne_binary(), kz_currency:units(), kz_term:ne_binary(), kz_term:api_object()
) ->
...
create_ledger(AccountId, ReplenishUnits, Trigger, Audit, Metadata) ->
    SourceId = kz_json:get_ne_binary_value(
        [<<"transaction">>, <<"id">>],
        Metadata,
