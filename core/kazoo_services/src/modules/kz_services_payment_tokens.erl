...
-spec default(kz_services:services() | kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:api_object().
default(Thing, Bookkeeper) ->
    Now = kz_time:now_s(),
    Pays = kz_json:filter(
        fun({_TokenId, Token}) ->
            kz_json:is_true(<<"default">>, Token) andalso
                kz_json:get_integer_value(<<"expiration">>, Token, Now) > Now
        end,
...
update(Services, Bookkeeper, Token) ->
    ...
    Setters = [
        {
            fun kz_services:set_services_jobj/2,
...
%%------------------------------------------------------------------------------
-spec delete(
    kz_services:services() | kz_term:ne_binary(),
    kz_term:ne_binary(),
...
delete(Services, _Bookkeeper, ?NE_BINARY = TokenId) ->
    ServicesJObj = kz_services:services_jobj(Services),
    NewTokens = kz_json:delete_key(
        TokenId, kzd_services:payment_tokens(ServicesJObj, kz_json:new())
    ),
    Setters = [
        {
            fun kz_services:set_services_jobj/2,
