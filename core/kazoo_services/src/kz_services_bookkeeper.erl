...
maybe_store_audit_log(Services, Invoice) ->
    case
        kz_services:account_id(Services) =:= master_account_id() andalso
            (not ?KZ_SERVICE_STORE_MASTER_AUDIT)
    of
        'true' -> 'undefined';
...
%% @end
%%------------------------------------------------------------------------------
-spec update_audit_log(
    kz_services:services(),
    kz_term:api_object(),
    kz_amqp_worker:request_return() | kz_term:proplist()
) ->
    kz_term:api_object().
update_audit_log(_Services, 'undefined', _Result) ->
    'undefined';
...
update_audit_log(Services, AuditJObj, {'ok', Result}) ->
    Props = [
        ...
        {[<<"bookkeeper">>, <<"results">>], kz_json:get_ne_value(<<"Details">>, Result, [])}
    ],
    update_audit_log(Services, AuditJObj, Props);
...
%% @end
%%------------------------------------------------------------------------------
-spec notify_reseller(kz_services:services(), kz_services_invoice:invoice(), kz_term:api_object()) ->
    'ok'.
...
%% @end
%%------------------------------------------------------------------------------
-spec update_bookkeeper(
    kz_term:ne_binary(), kz_services_invoice:invoice(), kz_services:services(), kz_term:api_object()
) ->
