...
%% a dictionary of the plan json from the db
-type fetched_plans() :: dict:dict().
...
-spec get_services_plan(kz_services:services(), fetch_context(), fetch_options()) ->
    fetch_context().
...
-spec get_service_plan(kz_services:services(), kz_term:ne_binary(), fetch_context()) ->
    fetch_context().
get_service_plan(Services, PlanId, FetchContext) ->
    Overrides = get_services_plan_overrides(Services, PlanId),
    VendorId = kzd_services:plan_vendor_id(
...
-spec build_object_plan(kz_services:services(), fetch_context(), kz_json:objects()) ->
    fetch_context().
build_object_plan(Services, FetchContext, ObjectPlans) ->
    Props = [
        {PlanId, kz_json:get_value([<<"value">>, PlanId], ObjectPlan)}
     || ObjectPlan <- ObjectPlans,
...
-type object_plans_fold() :: fun(
    ({kz_term:ne_binary(), kz_json:object()}, fetch_context()) -> fetch_context()
).
...
-spec get_object_plan(
    kz_services:services(), kz_term:ne_binary(), kz_json:object(), fetch_context()
) -> fetch_context().
get_object_plan(Services, PlanId, JObj, FetchContext) ->
...
maybe_append_plan(Services, PlanId, VendorId, Overrides, {FetchedPlans, ServicePlans}) ->
    case maybe_fetch_plan(PlanId, VendorId, FetchedPlans) of
        ...
        {Plan, UpdatedFetchedPlans} ->
            ...
            lager:debug(
                "adding plan ~s/~s for bookkeeper ~s",
                [
            ...
            {UpdatedFetchedPlans, dict:append(BookkeeperHash, UpdatedPlan, ServicePlans)}
    end.

-spec prepare_plan(kz_services:services(), kz_json:object(), kz_services_plan:plan()) ->
    kz_services_plan:plan().
prepare_plan(Services, Overrides, Plan) ->
    Routines = [
        {fun kz_services_plan:set_overrides/2, Overrides},
        {fun kz_services_plan:set_default_bookkeeper/2, default_bookkeeper(Services)}
    ],
...
default_bookkeeper(Services) ->
    ...
    Routines = [
        {fun kzd_services:set_bookkeeper_type/2, Type},
...
-spec maybe_append_plan_jobjs(
    plan_jobjs_key(), kz_services_plan:plan() | 'undefined', fetched_plans()
) -> fetched_plans().
maybe_append_plan_jobjs(_Key, 'undefined', FetchedPlans) -> FetchedPlans;
maybe_append_plan_jobjs(Key, Plan, FetchedPlans) -> dict:store(Key, Plan, FetchedPlans).
...
foldl(FoldFun, Acc, Plans) ->
    lists:foldl(
        fun({BookkeeperHash, PlansList}, A) ->
...
public_json(Plans) ->
    kz_json:from_list([
        {BookkeeperHash, kz_services_plan:public_json(merge(PlansList))}
...
%% @end
%%------------------------------------------------------------------------------
-spec override(kz_services:services(), kz_json:object(), kz_term:proplist()) ->
    kz_services:services().
override(Services, Overrides, Options) ->
    ServicesJObj = kz_services:services_jobj(Services),
    kz_services:set_services_jobj(
        Services,
        set_or_merge_override(ServicesJObj, Overrides, Options)
    ).

-spec set_or_merge_override(kz_json:object(), kz_json:object(), kz_term:proplist()) ->
    kz_json:object().
set_or_merge_override(ServicesJObj, Overrides, Options) ->
...
merge_plans_by_strategy([], MergedPlansJObjs) ->
    MergedPlansJObjs;
merge_plans_by_strategy([{<<"simple">>, Group} | Tail], MergedPlansJObjs) ->
    ...
    Setters = [
        {fun kz_doc:set_id/2, <<"simple">>},
        {fun kzd_service_plan:set_plan/2, kzd_service_plan:plan(simple_merge_plans(Sorted))}
    ],
...
merge_plans_by_strategy([{<<"cumulative">>, Group} | Tail], MergedPlansJObjs) ->
    ...
    JObj = recursive_merge_plans(Sorted),
    Props = [
        {[CategoryName, ItemName], kzd_service_plan:item(PlanJObj, CategoryName, ItemName)}
    ...
    Values = [
        {cumulative_merge_keys(Root, Key), Fun(Key, JObjs)}
     || {Root, JObjs} <- dict:to_list(Dict),
        {Key, Fun} <- kzd_item_plan:cumulative_merge_scheme()
    ],
    Setters = [
        {fun kz_doc:set_id/2, <<"cumulative">>},
        {fun kzd_service_plan:set_plan/2, kz_json:set_values(Values, kz_json:new())}
    ],
    MergedPlanJObj = kz_doc:setters(JObj, Setters),
    merge_plans_by_strategy(Tail, [MergedPlanJObj | MergedPlansJObjs]).
...
editable_fields(ResellerId) ->
    ...
    Routines = [
        fun editable_phone_numbers_fields/2,
...
-spec editable_number_carriers_fields(kz_term:proplist(), kz_term:api_binary()) ->
    kz_term:proplist().
...
-spec editable_number_services_fields(kz_term:proplist(), kz_term:api_binary()) ->
    kz_term:proplist().
...
editable_qubicle_fields(Fields, _ResellerId) ->
    case kz_nodes:whapp_count('qubicle') > 0 of
        'true' -> Fields;
        'false' -> props:delete(<<"qubicle">>, Fields)
    end.
...
-spec editable_fields_to_json(kz_term:proplist(), kz_json:object(), kz_json:object()) ->
    kz_json:object().
editable_fields_to_json([], _Properties, JObj) ->
    JObj;
editable_fields_to_json([{Category, Items} | Fields], Properties, JObj) ->
    Props = [
        {[Category, kzd_service_plan:all_items_key()], Properties}
        | [
