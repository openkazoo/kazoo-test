...
-define(DONT_CASCADE_MASTER,
    kapps_config:get_is_false(?CONFIG_CAT, <<"cascade_commits_to_master_account">>, 'true')
).
...
-type fetch_option() ::
    ...
    | 'hydrate_invoices'
    | {'updates', kz_term:ne_binary(), kz_services_quantities:billables(),
        kz_services_quantities:billables()}
...
current_plans(Services) ->
    ...
    Routines = [
        {
            kz_json:are_equal(CurrentPlans, ProposedPlans),
            {'services_jobj', current_services_jobj(Services)}
        },
        {kz_term:is_empty(UpdatedObjectPlans), {'modified_object_plans', UpdatedObjectPlans}}
    ],
...
-spec find_object_plans(kz_services_quantities:billables(), kz_services_quantities:billables()) ->
    kz_json:objects().
...
-spec find_object_plans_filter(kz_services_quantities:billables()) ->
    fun((kz_services_quantities:billable()) -> boolean()).
find_object_plans_filter(ObjectPlans) ->
    ...
    fun(JObj) ->
        case props:get_value(kz_doc:id(JObj), Plans) of
            'undefined' -> 'true';
            Plan -> not kz_json:are_equal(JObj, Plan)
        end
    end.
...
has_updates(Services) ->
    not kz_term:is_empty(account_updates(Services)) orelse
        not kz_term:is_empty(cascade_updates(Services)) orelse
        not kz_term:is_empty(manual_updates(Services)).
...
-spec set_account_updates(
    services(), kz_services_quantities:billables(), kz_services_quantities:billables()
) -> services().
set_account_updates(#kz_services{} = Services, Current, Proposed) when
    is_list(Current), is_list(Proposed)
->
...
set_manual_updates(Services, Quantities, Options) ->
    case props:get_is_true('merge', Options, 'false') of
        'false' ->
            Services#kz_services{manual_updates = Quantities};
        'true' ->
...
master_bookeeper_type() ->
    kapps_config:get_ne_binary(
        ?CONFIG_CAT, <<"master_account_bookkeeper">>, kzd_services:default_bookkeeper_type()
    ).
...
set_bookkeeper_vendor_id(Services, BookkeeperVendorId) ->
    ServicesJObj = kzd_services:set_bookkeeper_vendor_id(
        services_jobj(Services), BookkeeperVendorId
    ),
    set_services_jobj(Services, ServicesJObj).
...
add_audit_log_changes_type(_Services, AuditLog, _Type) ->
    AuditLog.

-spec maybe_set_change_type({kz_json:key(), boolean()}, kz_json:object()) -> kz_json:object().
maybe_set_change_type({_Type, 'false'}, AuditLog) ->
    AuditLog;
...
-spec add_audit_log_changes_account(kz_term:ne_binary() | services(), kz_json:object()) ->
    kz_json:object().
...
summary(Services) ->
    ...
    kz_json:from_list(
        [
            ...
            {<<"overrides">>, kzd_services:overrides(ServicesJObj)},
            {<<"invoices">>, kz_services_invoices:public_json(invoices(Services))},
            {<<"quantities">>, summary_quantities(Services)},
            {<<"reseller">>, summary_reseller(Services)},
            {<<"ratedeck">>, kz_services_ratedecks:fetch(Services)},
            {<<"applications">>, kz_services_applications:fetch(Services)},
            {<<"limits">>,
                kz_doc:public_fields(
                    kz_services_limits:fetch(Services)
                )},
            {<<"im">>, kz_services_im:fetch(Services)},
            {<<"payment_tokens">>, kz_services_payment_tokens:fetch(Services)},
            {<<"billing_cycle">>, summary_billing_cycle(Services)},
            {<<"status">>, summary_status(Services)}
        ]
    ).
...
summary_billing_cycle(_Services) ->
    ...
    NextBillDate =
        calendar:datetime_to_gregorian_seconds(
            {kz_date:normalize({Y, M + 1, 1}), {0, 0, 0}}
        ),
    kz_json:from_list(
        [
            {<<"next">>, NextBillDate},
...
-spec handle_fetched_doc(
    kz_term:ne_binary(), fetch_options(), {'ok', kz_json:object()} | {'error', 'not_found'}
) ->
    services().
...
choose_open_doc_fun(Options, AccountId) ->
    case props:is_true('skip_cache', Options, 'false') of
        'false' ->
            ?LOG_DEV(
                "fetching services doc ~s (with cache)",
                [AccountId]
            ),
            fun kz_datamgr:open_cache_doc/2;
        'true' ->
            lager:debug(
                "fetching services doc ~s (without cache)",
                [AccountId]
            ),
...
create(AccountId, {'ok', AccountJObj}) ->
    ...
    BaseJObj = kz_doc:update_pvt_parameters(
        kz_json:new(),
...
%% @end
%%------------------------------------------------------------------------------
-type billable_updates() ::
    'undefined' | kz_services_quantities:billables() | kz_services_quantities:billable().
...
should_skip_updates(Services) ->
    kz_term:is_empty(account_updates(Services)) andalso
        kz_term:is_empty(cascade_updates(Services)) andalso
        kz_term:is_empty(manual_updates(Services)) andalso
        (not has_proposed_object_plans(Services)).
...
cascade_commit_updates(_FetchOptions, []) ->
    'ok';
...
commit_account(Services) ->
    lager:debug(
        "commit any changes to account ~s",
        [account_id(Services)]
    ),
    Routines = [
        fun maybe_hydrate_account_quantities/1,
...
maybe_save_services_jobj(Services) ->
    ...
    case
        kz_json:are_equal(
            kz_doc:public_fields(CurrentServicesJObj),
...
commit_quantities(Services) ->
    Routines = [
        fun commit_cascade_quantities/1,
...
save_services_jobj(Services, ProposedJObj) ->
    case kz_datamgr:save_doc(?KZ_SERVICES_DB, ProposedJObj) of
        {'ok', UpdatedJObj} ->
            lager:info(
                "updated services document ~s",
                [account_id(Services)]
            ),
            Setters = [
                {fun set_services_jobj/2, UpdatedJObj},
                {fun set_current_services_jobj/2, UpdatedJObj}
            ],
            setters(set_dirty(Services), Setters);
        {'error', _Reason} ->
            lager:info(
                "unable to update services document ~s: ~p",
...
sum_quantities_updates(Quantities, Updates) ->
    Props = [
        {[CategoryName, ItemName], kz_json:get_integer_value([CategoryName, ItemName], Updates, 0)}
     || CategoryName <- kz_json:get_keys(Updates),
        ItemName <- kz_json:get_keys(CategoryName, Updates)
    ],
    lists:foldl(
        fun({Key, UpdateValue}, JObj) ->
