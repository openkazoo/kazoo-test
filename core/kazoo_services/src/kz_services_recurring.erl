...
%% @end
%%------------------------------------------------------------------------------
-spec force_reminder(kz_term:ne_binary(), kz_time:gregorian_seconds()) ->
    {'ok', kz_term:ne_binary()}.
...
%% @end
%%------------------------------------------------------------------------------
-spec is_last_month_collected(
    kz_term:ne_binary(), kz_term:ne_binary() | kz_time:year(), kz_term:ne_binary() | kz_time:month()
) ->
    boolean().
is_last_month_collected(Account, Year, Month) ->
    Timestamp =
        calendar:datetime_to_gregorian_seconds({
            {kz_term:to_integer(Year), kz_term:to_integer(Month), 1}, {0, 0, 0}
        }),
...
%%------------------------------------------------------------------------------
-spec early_collect(
    kz_term:ne_binary(), kz_time:gregorian_seconds(), kz_time:year(), kz_time:month(), boolean()
) -> run_return().
...
collect(AccountId, DueTimestamp) ->
    ...
    Results = kz_services_invoices:foldl(
        collect_invoices_fold_fun(Services, DueTimestamp), [], Invoices
    ),
    handle_collect_bookkeeper_results(Results).

-type collect_invoice_fold() :: fun(
    (kz_services_invoice:invoice(), kz_amqp_worker:request_return()) -> [
        kz_amqp_worker:request_return()
    ]
).
-spec collect_invoices_fold_fun(kz_services:services(), kz_time:gregorian_seconds()) ->
    collect_invoice_fold().
...
-spec collect_bookkeeper(
    kz_services_invoice:invoice(), kz_services:services(), kz_time:gregorian_seconds()
) ->
    kz_amqp_worker:request_return().
collect_bookkeeper(Invoice, Services, DueTimestamp) ->
    Request = [
        {<<"Account-ID">>, kz_services:account_id(Services)},
...
%% @end
%%------------------------------------------------------------------------------
-spec do_notify_reseller(
    kz_term:ne_binary(), kz_services:services(), kz_services_invoice:invoice(), non_neg_integer()
) -> 'ok'.
do_notify_reseller(AccountId, Services, Invoice, DueTimestamp) ->
    ...
    kapps_notify_publisher:cast(
        props:filter_undefined(Props), fun kapi_notifications:publish_bill_reminder/1
    ).
...
%% @end
%%------------------------------------------------------------------------------
-spec should_process(kz_term:ne_binary(), kz_term:ne_binary(), kz_time:year(), kz_time:month()) ->
    boolean().
should_process(AccountId, MarkerId, Year, Month) ->
    is_account_enabled(AccountId) andalso
...
%% @end
%%------------------------------------------------------------------------------
-spec is_proccessed(kz_term:ne_binary(), kz_term:ne_binary(), kz_time:year(), kz_time:month()) ->
    boolean().
is_proccessed(AccountId, MarkerId, Year, Month) ->
    ...
    case kazoo_modb:open_doc(AccountMODB, MarkerId) of
        {'ok', JObj} ->
            lager:debug(
                "recurring charges for account ~s for ~b-~b is processed on ~s",
