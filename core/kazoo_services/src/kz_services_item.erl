...
-export([
    cumulative_discount_quantity/1,
...
%% @end
%%------------------------------------------------------------------------------
-spec create(kz_services:services(), kz_json:object(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    item().
create(Services, ItemPlan, CategoryName, ItemName) ->
    Setters = [
        {fun set_item_plan/2, ItemPlan},
        {fun set_category_name/2, CategoryName},
        {fun set_item_name/2, ItemName}
    ],
    Routines = [
        fun calculate_quantity/2,
...
log_item(_Services, Item) ->
    case billable_quantity(Item) > 0 of
        ...
        'true' ->
            lager:debug(
                "~p billable ~s/~s at ~p in ~.2f total amount with ~.2f total discounts",
                [
...
calculate_billable_quantity(_Services, Item) ->
    ...
    case handle_step(quantity(Item), Step) of
        BillableQuantity when BillableQuantity < Minimum ->
            lager:debug(
                "minimum '~s' not met with ~p, enforcing quantity ~p",
        ...
        BillableQuantity when BillableQuantity > Maximum ->
            lager:debug(
                "maximum '~s' exceeded with ~p, enforcing quantity ~p",
...
calculate_flat_rates(_Services, Item) ->
    ...
    case find_tiered_rate(Rates, billable_quantity(Item)) of
        ...
        Rate ->
            lager:debug(
                "~s should be billed as a flat rate",
                [display_name(Item)]
            ),
            Setters = [
                {fun set_billing_rate/2, Rate},
...
%% @end
%%------------------------------------------------------------------------------
-type discount_routine() :: fun((kzd_item_plan:doc(), item()) -> item()).
...
-spec maybe_set_discounts_fold(kzd_item_plan:doc()) ->
    fun((discount_routine(), item()) -> item()).
maybe_set_discounts_fold(ItemPlan) ->
    fun(Routine, Item) ->
...
set_cumulative_discount(Item, DiscountPlan) ->
    ...
    Routines = [
        {fun set_cumulative_discount_quantity/2, CumulativeQuantity},
...
cumulative_quantity(Item, DiscountPlan, BillableQuantity) ->
    case kz_json:get_integer_value(<<"maximum">>, DiscountPlan, 0) of
        Maximum when Maximum < BillableQuantity ->
            lager:debug(
                "item '~s/~s' quantity ~p exceeds cumulative discount max, using ~p",
                [
                    category_name(Item),
                    item_name(Item),
                    BillableQuantity,
...
should_set_discount(_, 'undefined') ->
    'false';
...
find_tiered_rate(Rates, Quantity) ->
    ...
    case lists:dropwhile(fun(K) -> Quantity > K end, lists:sort(L1)) of
        [] -> 'undefined';
        Range -> kz_json:get_float_value(kz_term:to_binary(hd(Range)), Rates)
    end.
