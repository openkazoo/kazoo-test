...
create_new_number_rename_carrier_test_() ->
    ...
    {'error', Error2} = knm_number:create(
        ?TEST_CREATE_NUM, props:set_value('public_fields', WrongCarrier, Options)
    ),

    {'error', Error3} = knm_number:create(
        ?TEST_CREATE_NUM, props:set_value('auth_by', ?RESELLER_ACCOUNT_ID, Options)
    ),
    ...
    [
        ...
        {"Verify bad carrier module is bad",
            ?_assertEqual(<<"invalid">>, knm_errors:error(Error2))},
        {"Verify bad carrier module message",
...
rename_carrier_test_() ->
    ...
    #{ko := #{?TEST_TELNYX_NUM := Error5}} = knm_numbers:update(
        [N4],
    ...
    [
        ...
        {"Verify carrier name is right",
            ?_assertEqual(<<"knm_telnyx">>, knm_phone_number:module_name(PN1))},
        ?_assertEqual(
            undefined, kz_json:get_value(?FEATURE_RENAME_CARRIER, knm_phone_number:doc(PN1))
        ),
        ...
        {"Verify feature is now removed",
            ?_assertEqual(
                undefined, kz_json:get_value(?FEATURE_RENAME_CARRIER, knm_phone_number:doc(PN2))
            )},

        {"Verify renamed failed",
            ?_assertEqual(
                <<"'blabla' is not known by the system">>,
                kz_json:get_value(
                    <<"cause">>, kz_maps:get([ko, ?TEST_TELNYX_NUM], E3, kz_json:new())
                )
            )},
        ...
        ?_assertEqual(
            undefined, kz_json:get_value(?FEATURE_RENAME_CARRIER, knm_phone_number:doc(PN4))
        ),
        ...
        {"Verify feature is now removed",
            ?_assertEqual(
                undefined, kz_json:get_value(?FEATURE_RENAME_CARRIER, knm_phone_number:doc(PN6))
            )}
    ].
