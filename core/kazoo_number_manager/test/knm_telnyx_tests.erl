...
cnam_test_() ->
    ...
    [
        ?_assert(knm_phone_number:is_dirty(PN1)),
        {"Verify inbound CNAM is properly activated", ?_assertEqual(true, is_cnam_activated(PN1))},
        {"Verify outbound CNAM is properly set", ?_assertEqual(<<"my CNAM">>, cnam_name(PN1))},
        ?_assertEqual(false, knm_phone_number:is_dirty(PN2)),
        ...
        {"Verify inbound CNAM is indeed deactivated", ?_assertEqual(false, is_cnam_activated(PN3))},
        {"Verify outbound CNAM is indeed reset", ?_assertEqual(undefined, cnam_name(PN3))},
        ...
        {"Verify pub features were cleansed",
            ?_assertEqual(
                undefined, kz_json:get_ne_value(?FEATURE_CNAM, knm_phone_number:doc(PN3))
            )}
    ].
