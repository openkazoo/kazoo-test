...
to_db_test_() ->
    Ns = [
        {<<"+14158867900">>, <<?KNM_DB_PREFIX_ENCODED, "%2B1415">>},
...
available_classifiers_test_() ->
    ...
    NameClassifier =
        {<<"name">>,
            kz_json:from_list([
                {<<"friendly_name">>, kz_binary:rand_hex(4)},
...
correct_correction(
    {ClassifierName, ClassifierSettings},
    CorrectedClassifiers
) ->
    [
        ?_assertEqual(
            kz_json:get_value(<<"friendly_name">>, ClassifierSettings, ClassifierName),
            kz_json:get_value([ClassifierName, <<"friendly_name">>], CorrectedClassifiers)
        ),
        ?_assertEqual(
            kz_json:get_value(<<"regex">>, ClassifierSettings),
