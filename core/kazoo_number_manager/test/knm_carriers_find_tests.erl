...
info_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            [?NUMBER_STATE_IN_SERVICE, ?NUMBER_STATE_RESERVED],
...
find_no_phonebook(Options0) ->
    ...
    [{"Verify no phonebook url yields no results", ?_assertEqual([], knm_search:find(Options))}].

find_blocks(Options0) ->
    Prefix = <<"415">>,
    Limit = 10,
    Options = [
        {'phonebook_url', ?BLOCK_PHONEBOOK_URL},
        {'blocks', 'true'},
...
find_numbers(Options0) ->
    ...
    Limit = 10,
    Options = [
        {'phonebook_url', ?NUMBER_PHONEBOOK_URL},
    ...
    [
        {"Verify results returned is the expected amount", ?_assertEqual(Limit, length(Results))}
