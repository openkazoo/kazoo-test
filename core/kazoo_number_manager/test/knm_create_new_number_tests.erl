...
create_with_carrier_test_() ->
    ...
    [
        ...
        {"Verify no features were set", ?_assertEqual([], knm_phone_number:features_list(PN))},
        {"Verify getting created field returns a number",
            ?_assert(is_integer(knm_phone_number:created(PN)))},
        {"Verify the created field is stored as a number",
            ?_assert(is_integer(kz_json:get_value([<<"_read_only">>, <<"created">>], JObj)))}
...
fail_new_number_test_() ->
    ...
    [
        ...
        {"Verify that without the allow number additions the proper message is thrown",
            ?_assertEqual(
                <<"requestor is unauthorized to perform operation">>,
                kz_json:get_value(<<"message">>, Reason)
            )}
    ].
...
create_existing_in_service_test_() ->
    ...
    [
        {"Verifying that IN SERVICE numbers can't be created", ?_assertMatch({'error', _}, Resp)}
...
move_non_existing_mobile_number_test_() ->
    ...
    [
        {"Verify a non existing mdn cannot be moved to in_service",
            ?_assertEqual(
                {error, not_found}, knm_number:move(?TEST_CREATE_NUM, ?RESELLER_ACCOUNT_ID, Props)
            )}
    ].
...
create_with_no_auth_by() ->
    Resp = knm_number:attempt(
        fun knm_number:ensure_can_create/2,
        [?TEST_CREATE_NUM, []]
    ),
    [
