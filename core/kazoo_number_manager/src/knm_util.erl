...
pretty_print(Number) ->
    case pretty_print_format(Number) of
        'undefined' -> Number;
        Format -> pretty_print(Format, Number)
    end.
...
pretty_print_format(Num, [{Classification, Classifier} | Classifiers]) ->
    case re:run(Num, get_classifier_regex(Classifier)) of
        'nomatch' ->
            pretty_print_format(Num, Classifiers);
        _ when is_binary(Classifier) ->
            lager:debug("number '~s' is classified as ~s but no pretty print format available", [
                Num, Classification
            ]),
            maybe_use_us_default(Num);
        _ ->
