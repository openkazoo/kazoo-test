...
create_address(Number, AddressJObj) ->
    ...
    try knm_telnyx_util:req('post', ["e911_addresses"], Body) of
        Rep ->
            ...
            case
                'false' =/= kz_json:get_ne_binary_value(<<"success">>, Rep) andalso
                    <<"success">> =:= kz_json:get_ne_binary_value(<<"status">>, Rep)
            of
                'false' ->
                    {'error', reason(Rep)};
                'true' ->
...
-spec set_address_id(knm_number:knm_number(), kz_term:ne_binary() | 'null') ->
