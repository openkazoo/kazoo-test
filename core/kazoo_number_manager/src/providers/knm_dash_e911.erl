...
-define(DEBUG(Fmt, Args),
    ?DEBUG andalso
        file:write_file("/tmp/dash_e911.xml", io_lib:format(Fmt, Args))
).
...
delete(Number) ->
    case feature(Number) of
        ...
        _Else ->
            lager:debug(
                "removing e911 information from ~s",
...
update_e911(_Number, Address, 'true') ->
    Address;
...
add_location(Number, Location, CallerName) ->
    Props = [
        {'uri', [
            {'uri', [kz_term:to_list(<<"tel:", (knm_converters:to_1npan(Number))/binary>>)]},
...
provision_location(LocationId) ->
    ...
    case emergency_provisioning_request('provisionLocation', Props) of
        {'error', _} -> 'undefined';
        {'ok', Response} -> kz_xml:get_value("//LocationStatus/code/text()", Response)
    end.
...
%% @end
%%------------------------------------------------------------------------------
-type emergency_provisioning_error() ::
    'authentication'
    | 'authorization'
    | 'not_found'
    | 'server_error'
...
json_address_to_xml_location(JObj) ->
    Props = [
        {'address1', [kz_json:get_string_value(?E911_STREET1, JObj)]},
...
%% @end
%%------------------------------------------------------------------------------
-spec location_xml_to_json_address(kz_types:xml_el() | kz_types:xml_els()) ->
