...
%% @end
%%------------------------------------------------------------------------------
-spec get_portin_number(kz_term:ne_binary(), kz_term:ne_binary()) ->
    {'ok', kz_json:objects()}
    | kz_datamgr:data_error().
...
%% @end
%%------------------------------------------------------------------------------
-spec account_active_ports(kz_term:ne_binary()) ->
    {'ok', kz_json:objects()}
    | kz_datamgr:data_error().
account_active_ports(AccountId) ->
    ...
    case kz_datamgr:get_results(?KZ_PORT_REQUESTS_DB, ?ACTIVE_PORT_LISTING, ViewOptions) of
        {'ok', Ports} ->
            {'ok', [kz_json:get_value(<<"doc">>, Doc) || Doc <- Ports]};
        {'error', _R} = Err ->
...
%% @end
%%------------------------------------------------------------------------------
-spec account_ports_by_state(kz_term:ne_binary(), kz_term:ne_binary()) ->
    {'ok', kz_json:objects()}
    | kz_datamgr:data_error().
account_ports_by_state(AccountId, PortState) ->
    ViewOptions = [
        {'startkey', [AccountId, PortState]},
...
%% @end
%%------------------------------------------------------------------------------
-spec descendant_active_ports(kz_term:ne_binary()) ->
    {'ok', kz_json:objects()}
    | kz_datamgr:data_error().
descendant_active_ports(AccountId) ->
    ...
    case
        kz_datamgr:get_results(?KZ_PORT_REQUESTS_DB, ?DESCENDANT_ACTIVE_PORT_LISTING, ViewOptions)
    of
        {'ok', Ports} ->
            {'ok', [
                kz_json:get_value(<<"doc">>, Doc)
             || Doc <- Ports,
...
new(PortReq, Options) ->
    ...
    Metadata = transition_metadata(
        props:get_value('auth_by', Options),
        props:get_value('auth_user_id', Options)
    ),
    Unconf = [
        {?PORT_PVT_TYPE, ?TYPE_PORT_REQUEST},
        {?PORT_PVT_STATE, ?PORT_UNCONFIRMED},
        {?PORT_PVT_TRANSITIONS, [
            transition_metadata_jobj('undefined', ?PORT_UNCONFIRMED, Metadata)
        ]},
        %% makes port listing sane in crossbar
        {<<"pvt_account_name">>, props:get_value('account_name', Options)},
...
transition_to_complete(JObj, Metadata) ->
    case
        transition(
            JObj, Metadata, [?PORT_PENDING, ?PORT_SCHEDULED, ?PORT_REJECTED], ?PORT_COMPLETED
        )
    of
        {'error', _} = E -> E;
...
transition_to_canceled(JObj, Metadata) ->
    transition(
        JObj,
        Metadata,
        [?PORT_UNCONFIRMED, ?PORT_SUBMITTED, ?PORT_PENDING, ?PORT_SCHEDULED, ?PORT_REJECTED],
        ?PORT_CANCELED
    ).
...
%% @end
%%------------------------------------------------------------------------------
-spec attempt_transition(kz_json:object(), transition_metadata(), kz_term:ne_binary()) ->
    transition_response().
attempt_transition(PortReq, Metadata, ToState) ->
    ...
    case is_user_allowed_to_move_state(PortReq, Metadata, ToState, PortAuthority) of
        ...
        'false' -> {'error', 'user_not_allowed'}
    end.

-spec is_user_allowed_to_move_state(
    kz_json:object(), transition_metadata(), kz_term:ne_binary(), kz_term:api_ne_binary()
) ->
    boolean().
is_user_allowed_to_move_state(_, #{}, ToState, _) when
    ToState =:= ?PORT_UNCONFIRMED;
    ToState =:= ?PORT_SUBMITTED
->
    'true';
is_user_allowed_to_move_state(PortReq, #{}, _, 'undefined') ->
    lager:debug("port authority id is missing, disallowing state change for port ~s", [
        kz_doc:id(PortReq)
    ]),
    'false';
is_user_allowed_to_move_state(PortReq, #{auth_account_id := 'undefined'}, _, _) ->
    lager:debug("auth account is is missing, disallowing state change for port ~s", [
        kz_doc:id(PortReq)
    ]),
    'false';
is_user_allowed_to_move_state(
    PortReq, #{auth_account_id := AuthAccountId}, ?PORT_CANCELED, PortAuthority
) ->
    AuthAccountId =:= PortAuthority orelse
        (current_state(PortReq) =:= ?PORT_UNCONFIRMED andalso
            kzd_accounts:is_in_account_hierarchy(AuthAccountId, kz_doc:account_id(PortReq), 'true')) orelse
        %% checks if superduper
        kz_services_reseller:get_id('undefined') =:= AuthAccountId;
is_user_allowed_to_move_state(_, #{auth_account_id := AuthAccountId}, _, PortAuthority) ->
    AuthAccountId =:= PortAuthority orelse
        %% checks if superduper
        kz_services_reseller:get_id('undefined') =:= AuthAccountId.

-spec maybe_transition(kz_json:object(), transition_metadata(), kz_term:ne_binary()) ->
    transition_response().
...
%% @end
%%------------------------------------------------------------------------------
-spec compatibility_transition(knm_number_options:extra_options(), transition_metadata()) ->
    'ok' | {'error', any()}.
...
-spec transition(
    kz_json:object(), transition_metadata(), kz_term:ne_binaries(), kz_term:ne_binary()
) ->
    transition_response().
...
successful_transition(JObj, FromState, ToState, Metadata) ->
    ...
    Setters = [
        {fun kzd_port_requests:set_pvt_port_state/2, ToState},
        {fun kzd_port_requests:set_pvt_tranisitions/2, NewTransitions}
    ],
    kz_doc:setters(kz_json:delete_keys(public_fields(), JObj), Setters).

-spec transition_metadata_jobj(kz_term:api_ne_binary(), kz_term:ne_binary(), transition_metadata()) ->
    kz_json:object().
transition_metadata_jobj(
    FromState,
    ToState,
    #{
...
-spec transition_metadata(kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
    transition_metadata().
...
-spec transition_metadata(
    kz_term:ne_binary(), kz_term:api_ne_binary(), kz_term:api_ne_binary(), kz_term:api_seconds()
) -> transition_metadata().
transition_metadata(?MATCH_ACCOUNT_RAW(AuthAccountId), UserId, Reason, RequestedTimestamp) ->
    ...
    #{
        ...
        user_full_name => kzd_users:full_name(
            UserJObj, kzd_users:username(UserJObj, kzd_users:email(UserJObj))
        ),
        optional_reason => OptionalReason,
        timestamp => Timestamp
    }.
...
assign_to_app(Number, NewApp, JObj) ->
    ...
    lager:debug(
        "assigning number ~s in port request ~s from ~p to ~s",
        [Number, kz_doc:id(JObj), Numbers, NewApp]
    ),
    assign_to_app(Number, NewApp, Numbers, JObj).

-spec assign_to_app(
    kz_term:ne_binary(), kz_term:api_ne_binary(), kz_json:object() | 'undefined', kz_json:object()
) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
...
-spec completed_portin(kz_term:ne_binary(), kz_term:ne_binary(), transition_metadata()) ->
    'ok' | {'error', any()}.
completed_portin(Num, AccountId, #{optional_reason := OptionalReason}) ->
    ...
    Routins = [
        ...
        {fun knm_phone_number:set_ported_in/2, 'true'},
        {
            ...
            kz_json:from_list([{<<"portin_reason">>, OptionalReason}])
        }
    ],
        ...
        #{ko := KOs, ok := _OKs} ->
            ...
            case numbers_not_in_account_nor_in_service(AccountId, NumsKO) of
                ...
                _NumsNotTransitioned ->
                    lager:debug(
                        "failed to transition ~p/~p numbers",
...
reconcile_app_used_by(Numbers, JObj) ->
    ...
    lager:debug(
        "transitioning numbers ~p to active used by apps ~p portin usage ~p",
        [Numbers, AppUsedBy, PortInUsedBy]
    ),
    _ = [
        log_wrong_app_in_portin(proplists:get_value(N, PortInUsedBy), App, N)
     || {N, App} <- AppUsedBy
    ],
...
-spec log_wrong_app_in_portin(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    'ok'.
log_wrong_app_in_portin(App, App, _Num) ->
    'ok';
log_wrong_app_in_portin(PortInApp, App, Num) ->
    lager:debug("port in number ~p has an incorrect app ~p, the correct app is ~p", [
        Num, PortInApp, App
    ]).
...
%% @end
%%------------------------------------------------------------------------------
-spec get_dids_for_app(kz_term:ne_binary(), kz_term:ne_binaries()) ->
...
is_in_account_and_in_service(AccountId, PN) ->
    AccountId =:= knm_phone_number:assigned_to(PN) andalso
...
clear_numbers_from_port(PortReq) ->
    Setters = [
        {fun kzd_port_requests:set_numbers/2, kz_json:new()},
...
%% @end
%%------------------------------------------------------------------------------
-spec send_attachment(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), binary()
) ->
    'error' | 'ok'.
...
set_flag(JObj) ->
    ...
    case save_doc(Doc) of
        ...
        {'error', _R} -> lager:debug("failed to set flag for submitted_port_request: ~p", [_R])
    end.

-spec migrate(any(), pos_integer()) -> 'ok'.
migrate(StartKey, Limit) ->
    ?SUP_LOG_DEBUG(
        ":: getting next batch of ~b port requests starting with key ~1000p",
        [Limit, StartKey]
    ),
...
add_pvt_port_authority(PortRequest) ->
    AccountId = kz_doc:account_id(PortRequest),
    case
        kzd_port_requests:pvt_port_authority(PortRequest) == 'undefined' orelse
            kzd_port_requests:pvt_port_authority_name(PortRequest) == 'undefined'
    of
        'true' ->
            case kzd_port_requests:find_port_authority(AccountId) of
                ...
                AuthorityId ->
                    Setters = [
                        {fun kzd_port_requests:set_pvt_port_authority/2, AuthorityId},
                        {
...
rename_superduper_comment(PortRequest) ->
    ...
    case lists:foldl(fun rename_to_is_private/2, {[], 'false'}, Comments) of
        {_, 'false'} -> 'undefined';
        {Updated, 'true'} -> kzd_port_requests:set_comments(PortRequest, lists:reverse(Updated))
    end.

rename_to_is_private(Comment, {Acc, IsUpdated}) ->
    case kz_json:get_value(<<"superduper_comment">>, Comment) of
        'undefined' ->
            {[Comment | Acc], IsUpdated};
        Boolean ->
            {
                [
                    kzd_comment:set_is_private(
