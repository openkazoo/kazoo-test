...
from_jobjs(JObjs) ->
    ...
    PNs = [
        knm_phone_number:from_json_with_options(Doc, Options)
     || JObj <- JObjs,
    ...
    case take_not_founds(T0) of
        ...
        {T1, NotFounds} ->
            try knm_number:state_for_create(Options) of
                ToState ->
                    lager:debug("picked state ~s for ~s for ~p", [
                        ToState, knm_number_options:assign_to(Options), Nums
                    ]),
                    NewOptions = [{'state', ToState} | Options],
                    ret(
                        pipe(
                            maybe_create(NotFounds, options(NewOptions, T1)),
                            [
                                fun knm_number:new/1,
...
-spec move_with_updates(
    kz_term:ne_binaries(), knm_number_options:options(), knm_phone_number:set_functions()
) -> ret().
move_with_updates(Nums, Options, Updates) ->
...
-spec update(kz_term:ne_binaries(), knm_phone_number:set_functions(), knm_number_options:options()) ->
    ret().
update([?NE_BINARY | _] = Nums, Routines, Options) ->
    %% FIXME: unify to atom OR knm_error.
    Reason = 'not_reconcilable',
    do_update(do_get_pn(Nums, Options, Reason), Routines);
update(Ns, Updates, Options) ->
    Routines = [
        {fun knm_phone_number:set_is_dirty/2, 'false'}
...
-spec assign_to_app(kz_term:ne_binaries(), kz_term:api_ne_binary(), knm_number_options:options()) ->
    ret().
...
emergency_enabled(AccountId = ?MATCH_ACCOUNT_RAW(_)) ->
    ...
    [
        Num
     || {Num, JObj} <- account_listing(AccountDb),
...
new(Options, ToDos, KOs, Reason) ->
    #{
        'todo' => ToDos,
        'ok' => [],
        %%FIXME: find something better than Reason/1.
        'ko' =>
            case is_function(Reason, 1) of
...
merge_okkos([T]) -> T;
merge_okkos([T0 | Ts]) -> lists:foldl(fun merge_okkos/2, T0, Ts).
...
-spec unwrap_phone_numbers(knm_number:knm_numbers()) ->
    {#{num() => knm_number:knm_number()}, knm_phone_number:knm_phone_numbers()}.
...
maybe_create(NotFounds, T) ->
    ...
    Tb = pipe(T, [
        fun knm_number:ensure_can_load_to_create/1,
...
update_for_reconcile(T, Options) ->
    S = [
        {fun knm_phone_number:set_assigned_to/2, knm_number_options:assign_to(Options)},
...
update_services(T = #{todo := Numbers, options := Options}) ->
    case {knm_number_options:batch_run(Options), knm_number_options:dry_run(Options)} of
        {'true', _} ->
            lager:debug("batch_run-ing btw"),
            ok(Numbers, T);
        {_, 'true'} ->
            maybe_dry_run_services(T);
        {_, _} ->
            run_services(T)
    end.
...
run_services([], _Updates, UpdatedServicesAcc) ->
    _ = [
        kz_services:commit(UpdatedServices)
     || UpdatedServices <- lists:reverse(UpdatedServicesAcc)
    ],
    'ok';
run_services([AccountId | AccountIds], Updates, UpdatedServicesAcc) ->
    ...
    UpdatedServices = kz_services:set_updates(
        Services,
        AccountId,
        CurrentJObjs,
        ProposedJObjs
...
-spec check_creditably(
    kz_services:services(), kz_services_invoices:invoices(), boolean() | number()
) -> 'ok'.
...
check_creditably(Services, Quotes, Amount) ->
    Options = #{
        amount => kz_currency:dollars_to_units(Amount),
        quotes => Quotes
    },
...
-spec services_group_number(
    knm_phone_number:knm_phone_number(), kz_term:api_binary(), kz_term:api_binary()
) -> kz_term:proplist().
...
service_additions(Quotes) ->
    kz_services_invoices:foldl(
        fun service_additions_fold/2, [], kz_services_invoices:billable_additions(Quotes)
    ).
...
do_move_not_founds(Nums, Options) ->
    pipe(
        new([{'state', ?NUMBER_STATE_IN_SERVICE} | Options], Nums),
        [
...
fail_if_assign_to_is_not_an_account_id(T = #{'todo' := NsOrPNs, 'options' := Options}) ->
    case knm_number_options:assign_to(Options) of
        ...
        _ ->
            ...
            NsOrNums =
                case knm_phone_number:is_phone_number(hd(NsOrPNs)) of
                    'false' -> NsOrPNs;
...
can_release(T0 = #{'todo' := PNs}) ->
    ...
    F = fun(PN, T) ->
        ...
        case can_release(FromState, knm_phone_number:module_name(PN)) of
            ...
            'false' ->
                {'error', A, B, C} =
                    (catch knm_errors:invalid_state_transition('undefined', FromState, ToState)),
...
reset_features(T) ->
    Routines = [
        fun knm_phone_number:reset_features/1,
