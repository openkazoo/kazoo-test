...
-spec find_numbers_in_account(
    kz_term:ne_binary(), pos_integer(), kz_term:api_ne_binary(), knm_carriers:options()
) ->
    {'ok', knm_number:knm_numbers()}
    | {'error', any()}.
find_numbers_in_account(_Prefix, _Quantity, 'undefined', _Options) ->
    {'ok', []};
find_numbers_in_account(Prefix, Quantity, AccountId, Options) ->
...
-spec do_find_numbers_in_account(
...
do_find_numbers_in_account(Prefix, Quantity, Offset, AccountId, QID) ->
    ViewOptions = [
        {'startkey', [AccountId, ?NUMBER_STATE_AVAILABLE, Prefix]},
...
acquire_number(Number) ->
    ...
    update_doc(Number, [
        {?PVT_STATE, knm_phone_number:state(PhoneNumber)},
...
disconnect_number(Number) ->
    lager:debug(
        "disconnect number ~s in managed provider",
        [knm_phone_number:number(knm_number:phone_number(Number))]
    ),
    update_doc(Number, [
        {?PVT_STATE, ?NUMBER_STATE_RELEASED},
        {?PVT_ASSIGNED_TO, 'undefined'},
        {?PVT_RESERVE_HISTORY, []}
    ]).

-spec generate_numbers(kz_term:ne_binary(), pos_integer(), non_neg_integer()) -> 'ok'.
generate_numbers(AccountId, <<"8835100", _/binary>> = Number, Quantity) when
    byte_size(Number) =:= 15
->
    generate_numbers(AccountId, kz_term:to_integer(Number), kz_term:to_integer(Quantity));
generate_numbers(_AccountId, _Number, 0) ->
    'ok';
generate_numbers(?MATCH_ACCOUNT_RAW(AccountId), Number, Quantity) when
    is_integer(Number),
    is_integer(Quantity),
    Quantity > 0
->
...
-spec save_doc(kz_term:ne_binary(), kz_term:ne_binary()) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
save_doc(AccountId, Number) ->
    JObj = kz_json:from_list([
        {<<"_id">>, knm_converters:normalize(Number)},
...
update_doc(Number, UpdateProps) ->
    ...
    Updates = [
        {?PVT_MODULE_NAME, kz_term:to_binary(?MODULE)}
        | UpdateProps
    ],
    ...
    case kz_datamgr:update_doc(?KZ_INUM_DB, Num, UpdateOptions) of
        ...
        {'error', Reason} -> knm_errors:database_error(Reason, PhoneNumber)
    end.
