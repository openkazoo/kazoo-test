...
-define(ORDER_NUMBER_XPATH,
    "ExistingTelephoneNumberOrderType/TelephoneNumberList/TelephoneNumber/text()"
).
...
find_numbers(<<Prefix:3/binary, _/binary>> = Num, Quantity, Options) when ?IS_US_TOLLFREE(Prefix) ->
    ...
    Params = [
        ...
        binary_to_list(Wildcard),
        "*"
        "&enableTNDetail=true&quantity=",
        quantity_uri_param(Quantity)
    ],
    Result = search(Num, Params),
    process_tollfree_search_response(Result, Options);
find_numbers(<<Prefix:3/binary, _/binary>> = Num, Quantity, Options) when
    ?IS_US_TOLLFREE_WILDCARD(Prefix)
->
    Params = [
        "tollFreeWildCardPattern=",
        binary_to_list(Prefix),
        "&enableTNDetail=false&quantity=",
        quantity_uri_param(Quantity)
    ],
    Result = search(Num, Params),
    process_tollfree_search_response(Result, Options);
...
-else.
api_get(
    "https://dashboard.bandwidth.com/api/accounts/eunit_testing_account/availableNumbers?areaCode=" ++
        _
) ->
    Resp = knm_util:fixture("bandwidth2_find_by_npa_no_detail.xml"),
    handle_response({'ok', 200, [], Resp});
api_get(
    "https://dashboard.bandwidth.com/api/accounts/eunit_testing_account/availableNumbers?tollFreeWildCardPattern=" ++
        _
) ->
...
api_post(Url, Body) ->
    ...
    ?DEBUG_WRITE("Request:~n~s ~s~n~p~n~p~n~p~p~n", [
        'post', Url, Headers, HTTPOptions, Body, UnicodeBody
    ]),
...
handle_response({Result, Code, Props, Response}) when
    is_binary(Response)
->
...
-spec search_response_to_KNM(kz_types:xml_els() | kz_types:xml_el(), kz_term:ne_binary()) ->
    tuple().
...
verify_response(Xml) ->
    ...
    case
        validate_xpath_value(xmerl_xpath:string(NPAPath, Xml)) orelse
            validate_xpath_value(xmerl_xpath:string(TollFreePath, Xml)) orelse
            validate_xpath_value(xmerl_xpath:string(SitesPath, Xml)) orelse
            validate_xpath_value(xmerl_xpath:string(PeersPath, Xml)) orelse
            validate_xpath_value(kz_xml:get_value("//OrderStatus/text()", Xml))
    of
        'true' ->
        ...
        'false' ->
            ...
            Reason =
                case kz_xml:get_value(ErrorPath, Xml) of
