...
-spec find_numbers_in_account(
    kz_term:ne_binary(), pos_integer(), kz_term:api_ne_binary(), knm_search:options()
) ->
    {'ok', knm_number:knm_numbers()}
    | {'error', any()}.
find_numbers_in_account(Prefix, Quantity, AccountId, Options) ->
    case do_find_numbers_in_account(Prefix, Quantity, AccountId, Options) of
        {'error', 'not_available'} = Error ->
            ...
            case
                AccountId =:= 'undefined' orelse
                    AccountId =:= ResellerId
            of
                'true' -> Error;
                'false' -> find_numbers_in_account(Prefix, Quantity, ResellerId, Options)
            end;
        Result ->
            Result
    end.
...
do_find_numbers_in_account(Prefix, Quantity, AccountId, Options) ->
    ...
    case kz_datamgr:get_results(?KZ_MANAGED_DB, <<"numbers_managed/status">>, ViewOptions) of
        {'ok', []} ->
            ...
            {'error', 'not_available'};
        {'ok', JObjs} ->
            lager:debug("found ~B available managed numbers for account ~s", [
                length(JObjs), AccountId
            ]),
...
-spec import_numbers(kz_term:ne_binary(), kz_term:ne_binaries(), kz_json:object()) ->
    kz_json:object().
import_numbers(_AccountId, [], JObj) ->
    JObj;
