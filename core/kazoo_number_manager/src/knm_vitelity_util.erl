...
add_options_fold({K, V}, Options) -> props:insert_value(K, V, Options).
...
xml_resp_tag([_ | Els], Name) -> xml_resp_tag(Els, Name);
xml_resp_tag([], _Name) -> 'undefined'.

-spec xml_el_to_binary('undefined' | kz_types:xml_el()) -> kz_term:api_binary().
xml_el_to_binary('undefined') -> 'undefined';
xml_el_to_binary(#xmlElement{content = Content}) -> kz_xml:texts_to_binary(Content).
...
-spec xml_el_to_kv_pair(kz_types:xml_el()) ->
    {kz_term:ne_binary(), kz_term:api_binary() | kz_json:object()}.
xml_el_to_kv_pair(#xmlElement{
    name = 'did',
    content = Value
}) ->
...
xml_el_to_kv_pair(#xmlElement{
    ...
    case kz_xml:elements(Value) of
        [] ->
            {kz_term:to_binary(Name), kz_xml:texts_to_binary(Value)};
        Els ->
            {kz_term:to_binary(Name), kz_json:from_list(xml_els_to_proplist(Els))}
    end.
