...
-define(DEFAULT_MESSAGE_TIMEOUT,
    kapps_config:get_integer(?CONFIG_CAT, <<"message_timeout">>, 60 * ?MILLISECONDS_IN_SECOND)
).
...
send_sms(EndpointList, Strategy, Im) ->
    send_sms(EndpointList, Strategy, default_message_timeout(), Im).
...
-spec send_and_wait(binary(), kz_term:proplist(), kz_json:objects(), integer()) ->
...
create_sms_endpoints([], Endpoints) ->
    Endpoints;
...
lookup_reg('undefined', _Realm) ->
    {'error', 'invalid_user'};
lookup_reg(_Username, 'undefined') ->
    {'error', 'invalid_realm'};
...
-spec wait_for_correlated_message(
    kz_term:ne_binary() | im(), kz_term:ne_binary(), kz_term:ne_binary(), timeout()
) ->
    kapps_api_std_return().
        ...
        {'ok', JObj} = Ok ->
            case get_correlated_msg_type(JObj) of
                ...
                {_Type, _Event, _MsgId} ->
                    ...
                    wait_for_correlated_message(
                        MsgId, Event, Type, kz_time:decr_timeout(Timeout, Start)
                    )
            end
    end;
