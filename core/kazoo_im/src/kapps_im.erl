...
-define(SPECIAL_VARS, [
    {<<"Account-ID">>, #kapps_im.account_id},
...
put_message_id(#kapps_im{message_id = MsgId}) -> kz_util:put_callid(MsgId).
...
set_from(From, #kapps_im{} = Im) when
    is_binary(From)
->
...
set_application_id(_, #kapps_im{} = Im) ->
    Im.
...
set_authorizing_id(AuthorizingId, #kapps_im{} = Im) when
...
set_authorizing_type(AuthorizingType, #kapps_im{} = Im) when is_binary(AuthorizingType) ->
    set_custom_var(<<"Authorizing-Type">>, AuthorizingType, Im#kapps_im{
...
set_authorization(AuthorizingType, AuthorizingId, #kapps_im{} = Im) when
...
set_direction(Direction, #kapps_im{} = Im) when
    is_binary(Direction)
->
...
set_direction(_Direction, #kapps_im{} = Im) ->
    Im.
...
set_route_type(Type, #kapps_im{} = Im) when
    is_binary(Type)
->
...
set_route_type(_Type, #kapps_im{} = Im) ->
    Im.
...
set_route_id(RouteId, #kapps_im{} = Im) when
    is_binary(RouteId)
->
    Im#kapps_im{route_id = RouteId};
set_route_id(_RouteId, #kapps_im{} = Im) ->
    Im.
...
set_type(Type, #kapps_im{} = Im) when
    is_binary(Type)
->
...
from_payload(JObj) ->
    ...
    Routines = [
        {fun set_message_id/2, kz_im:message_id(JObj, kz_binary:rand_hex(16))},
...
fetch_endpoint(#kapps_im{authorizing_id = 'undefined'} = Im) ->
    Im;
fetch_endpoint(#kapps_im{account_id = 'undefined'} = Im) ->
    Im;
fetch_endpoint(Im) ->
