...
-define(RESPONDERS, [{{?MODULE, 'handle_message'}, [{<<"*">>, <<"inbound">>}]}]).
...
maybe_relay_request(JObj) ->
    ...
    case kz_maps:exec(Routines, Map) of
        #{account_id := AccountId, error := Error} ->
            lager:warning(
                "external ~s request for number ~s validation failed  in account ~s : ~p", [
                    kapps_im:type(IM), Number, AccountId, Error
                ]
            ),
            'nack';
        #{error := Error} ->
            lager:warning("validation failed in external ~s request for number ~s : ~p", [
                kapps_im:type(IM), Number, Error
            ]),
            'nack';
        #{account_id := AccountId, request := Payload, rate := Rate} ->
            lager:info("accepted external ~s request ~s for account ~s", [
                kapps_im:type(IM), Number, AccountId
            ]),
            Values = [
        ...
        _ ->
            lager:debug("unable to determine account for ~s when receiving ~s from ~s", [
                Number, kapps_im:type(IM), kapps_im:from(IM)
            ]),
            'nack'
    end.

lookup_number(#{account_id := _AccountId} = Map) ->
    Map;
            ...
            maps:without(
                ...
                Map#{
                    error => kz_term:to_binary(
                        io_lib:format("number does not have ~s enabled", [kapps_im:type(IM)])
                    )
                }
            )
    end;
number_has_im_enabled(Map) ->
    Map.

account_from_number(#{account_id := _AccountId} = Map) ->
    Map;
...
reseller_enabled(#{reseller := Reseller} = Map) ->
    case kzd_accounts:enabled(Reseller) of
        ...
        'false' ->
            maps:without(
                ['account_id', 'account', 'reseller_id', 'reseller'],
                Map#{error => <<"reseller is disabled">>}
            )
    end;
reseller_enabled(Map) ->
    Map.
    ...
    case kz_services_im:is_enabled(AccountId, kapps_im:type(IM)) of
        ...
        'false' ->
            maps:without(
                ...
                Map#{
                    error => kz_term:to_binary(
                        io_lib:format("account does not have ~s enabled", [kapps_im:type(IM)])
                    )
                }
            )
    end;
account_has_im_enabled(Map) ->
    Map.
    ...
    case kz_services_im:is_enabled(ResellerId, kapps_im:type(IM)) of
        ...
        'false' ->
            maps:without(
                ...
                Map#{
                    error => kz_term:to_binary(
                        io_lib:format("reseller ~s does not have ~s enabled", [
                            ResellerId, kapps_im:type(IM)
                        ])
                    )
                }
            )
    end;
reseller_has_im_enabled(Map) ->
    Map.
