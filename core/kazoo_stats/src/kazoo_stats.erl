...
handle_cast('stop', State) ->
    {'stop', 'ok', State};
handle_cast({Operation, Key, Val}, State) when
    Operation == 'add';
    Operation == 'store'
->
    {'noreply', store_value(Operation, Key, Val, State)};
handle_cast({Operation, Realm, Key, Val}, State) when
    Operation == 'add';
    Operation == 'store'
->
...
store_value(Operation, Realm, Key, Value, State = #state{sip = SipList}) when is_integer(Value) ->
    NewData =
        case props:get_value(Realm, SipList) of
            'undefined' ->
                [{Key, Value}];
            RealmData ->
                NewValue =
...
get_sip_values([]) -> [];
get_sip_values(SipList) -> [{<<"sip">>, SipList}].
...
get_ecallmgr_values2(VarList) ->
    RegFail =
        case
            {
                props:get_value("register-attempt", VarList),
...
recursive_from_proplist(List) when is_list(List) ->
    case lists:all(fun is_integer/1, List) of
        ...
        'false' ->
            kz_json:from_list([
                {kz_term:to_binary(K), recursive_from_proplist(V)}
             || {K, V} <- List
            ])
    end;
