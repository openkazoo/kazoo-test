...
reduce(JObj) ->
    Reducers = [
        {fun reduce_database_jobj/2, JObj},
...
node_version_rollup(Node, Acc) ->
    ...
    Key =
        <<(?TOPO_NODES_KEY)/binary, (format_wg_id([Type, <<"version">>]))/binary, ".",
            (format_wg_id(Version))/binary>>,
...
node_apps_rollup(_Node, Acc, 'false') ->
    Acc;
...
node_apps_rollup_foldl(App, Acc) ->
    Routines = [
        {fun node_apps_rollup_name/2, App},
...
node_kamailio_stats(_Node, Acc, 'false') ->
    Acc;
...
freeswitch_summary(_Nodes, Acc, 'false') ->
    Acc;
...
-spec calculate_aggregate(kz_term:binary(), kz_term:binaries() | any(), kz_json:object()) ->
    kz_json:object().
calculate_aggregate(K, V, Acc) when
    is_list(V) andalso
        length(V) > 0
->
...
dedupe_media_servers([], Acc) ->
    Acc;
dedupe_media_servers([Node | Nodes], Acc) ->
    case kz_json:get_ne_binary_value(<<"type">>, Node) of
        <<"ecallmgr">> ->
            MediaServers = kz_json:get_value(
                [<<"metadata">>, <<"media_servers">>], Node, kz_json:new()
            ),
            dedupe_media_servers(Nodes, kz_json:merge(MediaServers, Acc));
        _ ->
            dedupe_media_servers(Nodes, Acc)
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec node_freeswitch_summary_foldl(
    kz_term:api_binaries(), kz_term:non_neg_integer() | kz_term:binary(), kz_json:object()
) -> kz_json:object().
...
%% @end
%%------------------------------------------------------------------------------
-spec rollup_freeswitch_jobj(
    kz_term:ne_binaries(), non_neg_integer() | kz_term:ne_binary(), kz_json:object()
) -> kz_json:object().
rollup_freeswitch_jobj(_K, V, Acc) when is_binary(V) ->
    Version = binary:replace(V, <<".">>, <<"_">>, ['global']),
    Key =
        <<(?TOPO_NODES_KEY)/binary, (format_wg_id([<<"freeswitch">>, <<"version">>]))/binary, ".",
            Version/binary>>,
    Cnt = kz_json:get_value(Key, Acc, 0),
    kz_json:set_value(Key, Cnt + 1, Acc);
rollup_freeswitch_jobj([<<"startup">>] = _ObjK, V, Acc) when
    is_integer(V)
->
...
reduce_database_jobj(_Obj, Acc, 'false') ->
    Acc;
...
%% @end
%%------------------------------------------------------------------------------
-spec normalize_db_cluster_stats(
    kz_term:binary(), kz_term:binary() | kz_term:binaries() | non_neg_integer(), kz_json:object()
) -> kz_json:object().
...
normalize_db_cluster_stats([Tag, Stat], V, Acc) when is_binary(V) ->
    kz_json:set_value(
        <<(?TOPO_DB_KEY)/binary, (format_wg_id([?ANONYMIZE(kz_term:to_binary(Tag)), Stat]))/binary,
            ".", (format_wg_id(V))/binary>>,
        1,
        Acc
    );
normalize_db_cluster_stats([Tag, Stat], V, Acc) ->
    kz_json:set_value(
        <<(?TOPO_DB_KEY)/binary, (format_wg_id(?ANONYMIZE(kz_term:to_binary(Tag))))/binary, ".",
            Stat/binary>>,
        V,
        Acc
    ).
...
reduce_services_jobj(_Obj, Acc, 'false') ->
    Acc;
...
%% @end
%%------------------------------------------------------------------------------
-spec normalize_services_items_foldl(
    kz_term:binary(), kz_term:ne_binary() | kz_term:binaries() | non_neg_integer(), kz_json:object()
) -> kz_json:object().
normalize_services_items_foldl(K, V, Acc) ->
    [Category | Rest] = K,
    kz_json:set_value(
        <<(?SERVICES_KEY)/binary, Category/binary, ".", (format_wg_id(Rest))/binary, ".count">>,
        V,
        Acc
    ).
...
-spec normalize_summaries_foldl(
    kz_term:ne_binary(),
    kz_term:ne_binary() | kz_term:binaries() | non_neg_integer(),
    kz_json:objects()
) -> kz_json:objects().
...
format_wg_id([A, B]) ->
    <<A/binary, "_", B/binary>>;
format_wg_id([A, B, C]) ->
    <<A/binary, "_", B/binary, "_", C/binary>>.
