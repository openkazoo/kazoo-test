...
%% @end
%%------------------------------------------------------------------------------
-spec open_doc(server_map(), kz_term:ne_binary(), kz_term:api_ne_binary(), kz_data:options()) ->
    doc_resp().
    ...
    case open_doc(Server, DbName, kz_doc:id(Doc), Options) of
        {'ok', JObj} ->
            ...
            case {DocRev, JObjRev} of
                {'undefined', _} ->
                    {'ok', kz_fixturedb_util:update_doc(kz_doc:set_revision(Doc, JObjRev))};
                {_, 'undefined'} ->
                    {'ok', kz_fixturedb_util:update_doc(Doc)};
                {DocRev, JObjRev} ->
                    {'ok', kz_fixturedb_util:update_doc(Doc)};
                {_, _} ->
                    {'error', 'conflict'}
...
save_docs(Server, DbName, Docs, Options) ->
    case kz_fixturedb_db:db_exists(Server, DbName) of
        'false' -> {'error', 'not_found'};
        'true' -> {'ok', [perform_save_doc(Server, DbName, Doc, Options) || Doc <- Docs]}
    end.
    ...
    case save_doc(Server, DbName, Doc, Options) of
        ...
        {'error', 'conflict'} ->
            case lookup_doc_rev(Server, DbName, kz_doc:id(Doc)) of
                {'ok', Rev} ->
                    ensure_saved(Server, DbName, kz_doc:set_revision(Doc, Rev), Options);
                {'error', 'not_found'} ->
                    ensure_saved(Server, DbName, kz_doc:delete_revision(Doc), Options)
            end;
        {'error', _} = Error ->
            Error
    end.
