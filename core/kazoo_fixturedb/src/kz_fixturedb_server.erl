    ...
    {'ok',
        kz_json:from_list(
            [
                ...
                {<<"features">>, [<<"cool">>]},
                {<<"vendor">>,
                    kz_json:from_list([{<<"name">>, <<"the Great FixtureDB Committee">>}])}
            ]
        )}.
...
get_app_connection(#{options := Options} = Server) ->
    case maps:get(test_app, Options, 'undefined') of
        'undefined' -> Server;
        AppName -> set_app_connection(Server, AppName)
    end.
...
maybe_use_app_connection(#{options := Options} = Server, DbName) ->
    case {maps:get(test_app, Options, 'undefined'), maps:get(test_db, Options, 'undefined')} of
        {'undefined', _} ->
            Server;
        {_AppName, 'undefined'} ->
        ...
        {_AppName, _OtherDb} ->
            ?LOG_DEBUG(
                "requested db ~s is not test_db ~s, using kazoo_fixturedb database path...", [
                    DbName, _OtherDb
                ]
            ),
            Server
...
set_app_connection(#{options := Options} = Server, AppName) ->
    Path =
        case maps:get(test_db_subdir, Options, 'undefined') of
            'undefined' -> code:priv_dir(AppName);
            P -> code:lib_dir(AppName, kz_term:to_atom(P, 'true'))
        end,
    case Path of
        {'error', 'bad_name'} ->
            ?LOG_DEBUG("bad_name for path ~p, using default kazoo_fixturedb database path...", [
                AppName
            ]),
            Server;
        _ ->
