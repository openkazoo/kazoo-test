...
measure(ets_tables_sizes) -> [{Tab, ets:info(Tab, size)} || Tab <- ets:all()];
...
split_by_category(Category) ->
    ...
    [
        {binary:replace(NewKey, <<"___">>, <<"__">>, [global]), cleanse_with($,, Value)}
     || Item <- Items,
...
bin_to_integer(<<"true">>) ->
    1;
bin_to_integer(<<"false">>) ->
    0;
bin_to_integer(Value = ?NE_BINARY) ->
    try
        binary_to_integer(Value)
    catch
        error:badarg -> not_an_int
    end.

maybe_from_ref(Tab) when
    is_reference(Tab)
->
