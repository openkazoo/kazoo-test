...
verify_no_doc(Doc) ->
    case
        kz_cache:peek_local(
            ?CACHE_NAME,
            {'couch_util', kz_doc:account_db(Doc), kz_doc:id(Doc)}
        )
    of
...
new_doc(AccountDb, Ref) ->
    BaseDoc = kz_json:from_list([
        ...
        {<<"pvt_type">>, <<"load_test">>}
        | [
            {kz_binary:rand_hex(8), kz_binary:rand_hex(8)}
         || _ <- lists:seq(1, 12)
        ]
    ]),
...
perform_ops(Start, AccountDb, Ops) ->
    do_stuff_to_docs(
        ...
        AccountDb,
        lists:foldl(
            fun(Op, Acc) -> perform_op(Op, Acc, AccountDb) end,
            [],
            Ops
        )
    ).
