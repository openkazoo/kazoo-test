...
fetch(?NE_BINARY = Account, Options) ->
    ...
    case kazoo_modb:get_results(Account, ?LIST_BY_TIMESTAMP, ViewOptions) of
        {'error', _} = Error -> Error;
        {'ok', JObjs} -> {'ok', [kz_transaction:from_json(JObj) || JObj <- JObjs]}
    end.

-spec fetch(kz_term:ne_binary(), kz_time:gregorian_seconds(), kz_time:gregorian_seconds()) ->
    {'ok', transactions()}
    | {'error', any()}.
fetch(?NE_BINARY = Account, CreatedFrom, CreatedTo) when
...
get_ranged(View, Options) ->
    ...
    case MODbs =:= [] of
        ...
        'false' ->
            ...
            lager:debug(
                "getting transactions starting from ~p to ~p from dbs: ~p",
                [
