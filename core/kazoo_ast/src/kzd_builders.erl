...
build_accessors() ->
    ...
    'ok' = filelib:fold_files(
        kz_term:to_list(kz_ast_util:schema_path(<<>>)),
        "^[a-z_]+\\.json$",
        'false',
...
build_accessor(SchemaPath, 'ok') ->
    ...
    case accessors_from_properties(kz_json:get_json_value(<<"properties">>, SchemaJObj)) of
        'undefined' ->
            'ok';
        {Exports, Accessors} ->
            {PatternExports, PatternAccessors} = accessors_from_patterns(
                kz_json:get_json_value(<<"patternProperties">>, SchemaJObj)
            ),

            save_module(SchemaId, [
                BaseModule,
...
accessors_from_patterns(PatternProperties) ->
    {Accessors, _} = kz_json:foldl(
        fun accessor_from_pattern/3,
        {{[], []}, []},
        PatternProperties
    ),
    Accessors.

accessor_from_pattern(<<"^_", _/binary>>, _Properties, Acc) ->
    Acc;
accessor_from_pattern(<<"^_pvt", _/binary>>, _Properties, Acc) ->
    Acc;
...
accessors_from_properties('undefined') ->
    'undefined';
accessors_from_properties(Properties) ->
    kz_json:foldl(
        fun accessor_from_properties/3,
...
add_accessors(Property, Schema, Accessors) ->
    ...
    [
        [
            ...
            Getter,
            "(doc()) -> ",
            default_return_type(ReturnType, Default),
            ".\n",
            Getter,
            "(Doc) ->\n",
            "    ",
...
list_return_subtype(_Schema, <<"string">>) ->
    "kz_term:ne_binaries()";
list_return_subtype(_Schema, <<"integer">>) ->
    "kz_term:integers()";
list_return_subtype(_Schema, <<"number">>) ->
    "[number()]";
list_return_subtype(_Schema, <<"object">>) ->
    "kz_json:objects()";
...
clean_name([]) -> [];
clean_name([_ | _] = Names) -> [clean_name(Name) || Name <- Names];
clean_name(Name) -> binary:replace(Name, <<"-">>, <<"_">>, ['global']).
