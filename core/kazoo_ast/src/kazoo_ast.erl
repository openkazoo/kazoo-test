...
-type function_fun() ::
    fun((function(), accumulator()) -> fun_return())
    | fun((function(), arity(), accumulator()) -> fun_return()).
-type clause_fun() :: fun(
    ([erl_parse:abstract_expr()], [erl_parse:abstract_expr()], accumulator()) -> accumulator()
).
...
process_project(ConfigMap) ->
    try
        lists:foldl(fun process_app/2, ConfigMap, kz_ast_util:project_apps())
    catch
        'throw':{'stop', Acc} -> Acc
    end.
...
process_app_modules(_App, {'skip', Config}) ->
    Config;
process_app_modules(App, Config) ->
    try
        process_modules(kz_ast_util:app_modules(App), Config)
    catch
        'throw':{'stop', Acc} -> Acc
    end.
...
process_modules(Modules, Config) ->
    try
        lists:foldl(fun process_module/2, Config, Modules)
    catch
        'throw':{'stop', Acc} -> Acc
    end.
...
-spec fold_over_module(atom(), config(), [
    {fun((ast_functions(), config()) -> config()), ast_functions()}
...
callback_function(_Function, _Artiy, Config) ->
    {'ok', Config}.
...
callback_expression(_Expression, Config) ->
    Config.
...
callback_module(_Module, Config) ->
    Config.

callback_after_module(
    Module,
    #{
        'after_module' := Fun,
        'accumulator' := Acc0
...
callback_after_module(_Module, Config) ->
    Config.
...
callback_application(_App, Config) ->
    Config.

callback_after_application(
    App,
    #{
        'after_application' := Fun,
        'accumulator' := Acc0
    } = Config0
) ->
    case Fun(App, Acc0) of
...
callback_after_application(_App, Config) ->
    Config.
...
callback_clause(_Args, _Guards, Config) ->
    Config.
