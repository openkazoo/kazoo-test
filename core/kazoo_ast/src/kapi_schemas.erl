...
%% s/kapi_(.+)/\1/
-record(acc, {
    kapi_name = <<"empty">> :: kz_term:ne_binary(),
    %% api function
    api_name = <<"empty">> :: kz_term:api_ne_binary(),
...
maybe_update_schema(PrivDir, GeneratedJObj, ID) ->
    ...
    MergedJObj = kz_json:merge(
        fun kz_json:merge_left/2,
        ExistingJObj,
        GeneratedJObj
    ),
    ...
    case kz_json:are_equal(ExistingJObj, UpdatedSchema) of
        'true' -> 'ok';
        'false' -> 'ok' = file:write_file(Path, kz_json:encode(UpdatedSchema))
    end.
...
expression_to_schema(?MOD_FUN_ARGS('kz_api', 'build_message', [_Prop, Required, Optional]), Acc) ->
    properties_to_schema(
        kz_ast_util:ast_to_list_of_binaries(Required),
        optional_validators(Optional),
        Acc
    );
expression_to_schema(
    ?MOD_FUN_ARGS('kz_api', 'build_message_specific_headers', [_Prop, Required, Optional]), Acc
) ->
    properties_to_schema(
        kz_ast_util:ast_to_list_of_binaries(Required),
        optional_validators(Optional),
...
expression_to_schema(
    ?MOD_FUN_ARGS('kz_api', 'validate_message', [_Prop, _Required, Values, Types]), Acc
) ->
    validators_to_schema(ast_to_proplist(Values), ast_to_proplist(Types), Acc);
...
optional_validators(Optional) -> kz_ast_util:ast_to_list_of_binaries(Optional).
...
set_required(Schema, Required) -> kz_json:set_value(<<"required">>, Required, Schema).
...
base_schema(KAPI, API) ->
    kz_json:from_list([
        {<<"_id">>, <<"kapi.", KAPI/binary, ".", API/binary>>},
...
set_kapi_schema(
    ...
    case kz_json:is_empty(kz_json:get_json_value(<<"properties">>, Schema, kz_json:new())) of
        'true' -> Acc;
        'false' -> Acc#acc{app_schemas = kz_json:set_value([KAPI, API], Schema, Schemas)}
    end.
...
ast_to_kv(?TUPLE([Key, Value])) ->
    {kz_ast_util:binary_match_to_binary(Key), ast_to_value(Value)}.
