...
-type acc() :: #{
    'schema_dir' := 'default' | file:filename_all(),
...
update_schema(Name, AutoGenSchema, PrivDir) ->
    ...
    _ =
        kz_json:new() =/= AccountSchema andalso
            update_schema(Name, AccountSchema, PrivDir, <<"account_config">>),
    update_schema(Name, AutoGenSchema, PrivDir, <<"system_config">>).

-spec update_schema(
    kz_term:ne_binary(), kz_json:object(), file:filename_all(), kz_term:ne_binary()
) -> 'ok'.
...
static_fields(ConfigType, Name, JObj) ->
    ...
    Values = [
        {<<"description">>, <<"Schema for ", Name/binary, " ", ConfigType/binary>>},
...
process_project() ->
    ...
    Options = [
        {'expression', fun expression_to_schema/2},
...
new_acc() ->
    #{
        ...
        %% SchemaName => SchemaProperties
        'app_schemas' => kz_json:new(),
        %% Path => Schemas
        'project_schemas' => kz_json:from_list([
            {kz_ast_util:default_schema_priv_dir(), kz_json:new()}
        ])
    }.
...
expression_to_schema(
    ?MOD_FUN_ARGS(Source = 'kapps_account_config', F = 'get_global', Args), Schemas
) ->
    config_to_schema(Source, F, Args, Schemas);
expression_to_schema(
    ?MOD_FUN_ARGS(Source = 'kapps_account_config', F = 'get_hierarchy', Args), Schemas
) ->
    config_to_schema(Source, F, Args, Schemas);
expression_to_schema(
    ?MOD_FUN_ARGS(Source = 'kapps_account_config', F = 'get_with_strategy', Args), Schemas
) ->
    config_to_schema(Source, F, Args, Schemas);
...
config_to_schema(
    Source,
    F = 'get_with_strategy',
    [?BINARY_MATCH([?BINARY_STRING(Strategy)]), _Account, Cat, K, Default],
    Schemas
) when
    Strategy =:= "hierarchy_merge";
    Strategy =:= "global";
...
category_to_document(Cat) -> kz_ast_util:binary_match_to_binary(Cat).

key_to_key_path(?ATOM(A)) ->
    [kz_term:to_binary(A)];
key_to_key_path(?VAR(_)) ->
    'undefined';
...
guess_type_by_default('undefined') ->
    'undefined';
guess_type_by_default(?ATOM('undefined')) ->
    'undefined';
guess_type_by_default(?ATOM('true')) ->
    <<"boolean">>;
guess_type_by_default(?ATOM('false')) ->
    <<"boolean">>;
guess_type_by_default(?ATOM(_)) ->
    <<"string">>;
guess_type_by_default(?VAR(_V)) ->
    'undefined';
guess_type_by_default(?EMPTY_LIST) ->
    <<"array">>;
guess_type_by_default(?LIST(?BINARY_MATCH(_), {nil, _})) ->
    [<<"string">>];
guess_type_by_default(?LIST(Head, {nil, _})) ->
    [guess_type_by_default(Head)];
...
guess_type_by_default(?BINARY_MATCH(_V)) ->
    <<"string">>;
guess_type_by_default(?INTEGER(_I)) ->
    <<"integer">>;
guess_type_by_default(?FLOAT(_F)) ->
    <<"float">>;
guess_type_by_default(?BINARY_OP(_Op, Arg1, _Arg2)) ->
    guess_type_by_default(Arg1);
guess_type_by_default(?MOD_FUN_ARGS(M, F, [_Cat, _Key])) when
    M =:= 'kapps_config';
    M =:= 'kapps_account_config'
->
    guess_type(F, 'undefined');
guess_type_by_default(?MOD_FUN_ARGS(M, F, [_Cat, _Key, Default | _])) when
    M =:= 'kapps_config';
    M =:= 'kapps_account_config'
->
    guess_type(F, Default);
guess_type_by_default(?MOD_FUN_ARGS('kz_json', 'new', [])) ->
    <<"object">>;
guess_type_by_default(?MOD_FUN_ARGS('kz_json', 'from_list', _Args)) ->
    <<"object">>;
guess_type_by_default(?MOD_FUN_ARGS('kz_json', 'from_list_recursive', _Args)) ->
    <<"object">>;
guess_type_by_default(?MOD_FUN_ARGS('kz_json', 'set_value', [_K, V, _J])) ->
    guess_type_by_default(V);
guess_type_by_default(?MOD_FUN_ARGS('kz_privacy', 'anonymous_caller_id_number', _Args)) ->
    <<"string">>;
guess_type_by_default(?MOD_FUN_ARGS('kz_privacy', 'anonymous_caller_id_name', _Args)) ->
    <<"string">>;
guess_type_by_default(?MOD_FUN_ARGS('kz_account', 'type', [])) ->
    <<"string">>;
guess_type_by_default(?MOD_FUN_ARGS('kz_term', 'to_integer', _Args)) ->
    <<"integer">>;
guess_type_by_default(?MOD_FUN_ARGS('kz_binary', 'rand_hex', _Args)) ->
    <<"string">>;
guess_type_by_default(?MOD_FUN_ARGS(_Mod, 'type', [])) ->
    <<"string">>.
...
guess_properties(Document, Source, [Key], Type, Default) when
    is_binary(Key)
->
...
default_value(?STRING(S)) ->
    kz_term:to_binary(S);
default_value(?INTEGER(I)) ->
    I;
default_value(?FLOAT(F)) ->
    F;
default_value(?BINARY_OP(Op, Arg1, Arg2)) ->
    erlang:Op(default_value(Arg1), default_value(Arg2));
default_value(?BINARY_MATCH(Match)) ->
    kz_ast_util:binary_match_to_binary(Match);
...
default_value(?MOD_FUN_ARGS('kapps_config', 'get_jsons', [_Category, _Key, Default])) ->
    default_value(Default);
default_value(
    ?MOD_FUN_ARGS('kapps_account_config', 'get_global', [_Account, _Category, _Key, Default])
) ->
    default_value(Default);
default_value(?MOD_FUN_ARGS('kapps_account_config', 'get', [_Account, _Category, _Key, Default])) ->
    default_value(Default);
default_value(
    ?MOD_FUN_ARGS('kapps_account_config', 'get_ne_binary', [_Account, _Category, _Key, Default])
) ->
    default_value(Default);
default_value(
    ?MOD_FUN_ARGS('kapps_account_config', 'get_ne_binaries', [_Account, _Category, _Key, Default])
) ->
    default_value(Default);
