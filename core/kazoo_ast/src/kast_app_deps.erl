...
fix_app_deps(App, Missing, Unneeded) ->
    ...
    ?DEBUG(
        "app ~p~n conf: ~p~n missing ~p~n unneeded ~p~n",
...
circles() ->
    ...
    {Graph, Verticies} = lists:foldl(
        fun add_app_to_graph/2,
        {digraph:new(), []},
        kz_ast_util:project_apps()
    ),
    io:format(" done~n"),
    lists:foreach(
        fun(App) -> print_cycle(App, Graph) end,
...
print_cycle(App, Graph) ->
    case digraph:get_short_cycle(Graph, App) of
        'false' -> 'ok';
        Vs -> io:format("cycle through ~p: ~p~n", [App, Vs])
    end.
...
circular_deps(App, RemoteApps) ->
    lists:foldl(
        fun(Dep, Acc) -> circles_fold(App, Dep, Acc) end,
        [],
        RemoteApps
    ).

circles_fold(App, Dep, Acc) ->
    is_kazoo_app(Dep) andalso
        ?DEBUG(
            "dep ~p (of ~p) has remote apps ~p~n",
            [Dep, App, remote_app_list(Dep)]
        ),
    case
        is_kazoo_app(Dep) andalso
            [A || A <- remote_app_list(Dep), A =:= App]
    of
        'false' ->
            ?DEBUG("dep ~p (of ~p) is not a kazoo app (may be a lib)~n", [Dep, App]),
            Acc;
        [] ->
            Acc;
        _ ->
            ?DEBUG("adding dep ~s to list", [Dep]),
            [Dep | Acc]
    end.

-spec is_kazoo_app(atom() | file:filename() | {'error', 'bad_name'}) -> boolean().
is_kazoo_app({'error', 'bad_name'}) ->
    'false';
...
remote_calls(App) ->
    ...
    lists:usort(
        lists:foldl(
            fun remote_calls_from_module/2,
            [],
            AppModules
        )
    ).
...
remote_calls_from_functions(Fs, Acc) ->
    lists:foldl(
        fun remote_calls_from_function/2,
        Acc,
        Fs
    ).
...
remote_calls_from_expression(?GEN_MFA(M, _F, _Arity), Acc) ->
    add_remote_module(M, Acc);
remote_calls_from_expression(?FA(_F, _Arity), Acc) ->
    Acc;
...
remote_calls_from_expression(?ATOM(_), Acc) ->
    Acc;
remote_calls_from_expression(?INTEGER(_), Acc) ->
    Acc;
remote_calls_from_expression(?FLOAT(_), Acc) ->
    Acc;
remote_calls_from_expression(?CHAR(_), Acc) ->
    Acc;
remote_calls_from_expression(?TUPLE(_Elements), Acc) ->
    Acc;
remote_calls_from_expression(?EMPTY_LIST, Acc) ->
    Acc;
