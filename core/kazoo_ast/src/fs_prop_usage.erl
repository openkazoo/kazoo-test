...
write_headers(IO, [First | Sorted]) ->
    ...
    'ok' = file:write(
        IO, io_lib:format("-define(FS_GENERATED_EVENT_FILTERS~n       ,[~p~n", [First])
    ),
    lists:foreach(
        fun(Filter) ->
...
usage_keys({'props', 'delete_keys', Keys, _VarName, _Default}, Acc) ->
    Keys ++ Acc;
usage_keys({'props', 'filter', 'undefined', _VarName, _Default}, Acc) ->
    Acc;
usage_keys({'props', 'insert_values', _Keys, _VarName, _Default}, Acc) ->
    Acc;
usage_keys({'props', 'set_values', _Values, _VarName, 'undefined'}, Acc) ->
    Acc.
...
%% places the Data is accessed
-record(usage, {
    usages = [],
    %% Tracks current var name
    data_var_name = 'Props',
    %% typically when kz_json:set_value is used
    data_var_aliases = [],
    %% what module are we currently in
    current_module,
    %% AST functions loaded
    functions = [],
    %% MFAs visited (to stop recursion)
    visited = []
}).
...
process_action(_M, 'undefined', _As, Acc) ->
    Acc;
process_action(Module, Function, Args, Acc) ->
    #usage{usages = Us} = process_mfa_call(
...
function_args('ecallmgr_channel_move') ->
    {'rebuild_channel', [?VAR(0, 'UUID'), ?VAR(0, 'NewNode'), ?VAR(0, 'Props')]};
function_args('ecallmgr_originate') ->
    {'handle_fs_event', [?VAR(0, 'Props'), ?VAR(0, 'UUID')]};
function_args('ecallmgr_events') ->
    {'event', [?VAR(0, 'EventName'), ?VAR(0, 'UUID'), ?VAR(0, 'Props'), ?VAR(0, 'Node')]};
function_args('ecallmgr_call_events') ->
    {'process_channel_event', [?VAR(0, 'Props')]};
function_args('ecallmgr_fs_config') ->
    {'handle_config_req', [?VAR(0, 'Node'), ?VAR(0, 'Id'), ?VAR(0, 'Conf'), ?VAR(0, 'Props')]};
...
function_args('ecallmgr_fs_authn') ->
    {'handle_directory_lookup', [?VAR(0, 'Id'), ?VAR(0, 'Props'), ?VAR(0, 'Node')]};
function_args('ecallmgr_fs_authz') ->
    {'authorize', [?VAR(0, 'Props'), ?VAR(0, 'CallId'), ?VAR(0, 'Node')]};
function_args('ecallmgr_fs_channel') ->
    {'handle_fs_event', [?VAR(0, 'UUID'), ?VAR(0, 'Props'), ?VAR(0, 'Node'), ?VAR(0, 'Options')]};
function_args('ecallmgr_fs_conference') ->
    {'handle_conf_event', [?VAR(0, 'Props'), ?VAR(0, 'Node')]};
function_args('ecallmgr_fs_conferences') ->
    [
        {'conference_from_props', [?VAR(0, 'Props'), ?VAR(0, 'Node')]},
        {'participant_from_props', [?VAR(0, 'Props'), ?VAR(0, 'Node')]}
    ];
function_args('ecallmgr_fs_loopback') ->
    {'filter', [?VAR(0, 'Node'), ?VAR(0, 'UUID'), ?VAR(0, 'Props')]};
function_args('ecallmgr_fs_msg') ->
    {'process_fs_event', [?VAR(0, 'Node'), ?VAR(0, 'Props')]};
function_args('ecallmgr_fs_recordings') ->
    {'process_event', [?VAR(0, 'UUID'), ?VAR(0, 'Props'), ?VAR(0, 'Node')]};
...
function_args('ecallmgr_call_control') ->
    [
        {'handle_event_info', [?VAR(0, 'CallId'), ?VAR(0, 'Props'), ?VAR(0, 'State')]},
        {'handle_other_event_info', [?VAR(0, 'CallId'), ?VAR(0, 'Props'), ?VAR(0, 'State')]}
    ];
function_args('ecallmgr_fs_presence') ->
    {'maybe_build_presence_event', [?VAR(0, 'Node'), ?VAR(0, 'UUID'), ?VAR(0, 'Props')]};
function_args(_M) ->
    {'undefined', []}.
...
process_expression(Acc, ?RECORD_INDEX(_Name, _Field)) ->
    Acc;
...
process_match_mfa(
    ...
    _M,
    _F,
    [?BINARY_MATCH(_Key), _Value, ?VAR(DataName)]
) ->
...
process_mfa(
    ...
    'props' = M,
    F,
    [Key, ?VAR(DataName), Default]
) ->
    Acc#usage{
        usages = maybe_add_usage(Usages, {M, F, arg_to_key(Key), DataName, arg_to_key(Default)})
    };
process_mfa(
    #usage{
        data_var_name = DataName,
...
arg_list_has_data_var(_DataName, _Aliases, []) ->
    'undefined';
arg_list_has_data_var(DataName, Aliases, [
    ?MOD_FUN_ARGS(
        'props',
        'set_value',
        Args
    ...
    case arg_list_has_data_var(DataName, Aliases, Args) of
        {DataName, _} ->
            ?DEBUG("  sublist had ~p~n", [DataName]),
            {DataName, T};
        'undefined' ->
            arg_list_has_data_var(DataName, Aliases, T);
        {Alias, _} ->
            ?DEBUG("  sublist had alias ~p~n", [Alias]),
            {Alias, T}
    end;
arg_list_has_data_var(DataName, Aliases, [
    ?MOD_FUN_ARGS(
...
arg_list_has_data_var(DataName, Aliases, [?MOD_FUN_ARGS(_M, _F, Args) | T] = As) ->
    case arg_list_has_data_var(DataName, Aliases, Args) of
        {DataName, _} ->
            ?DEBUG("  sub-fun had ~p~n", [DataName]),
            {DataName, As};
        'undefined' ->
            arg_list_has_data_var(DataName, Aliases, T);
        {Alias, _} ->
            ?DEBUG("  sub-fun had alias ~p~n", [Alias]),
            {Alias, As}
    end;
...
arg_list_has_data_var(DataName, Aliases, [?LIST(_H, _T) = H | T] = As) ->
    case arg_list_has_data_var(DataName, Aliases, H) of
        {DataName, _} ->
            ?DEBUG("  sub-list had ~p~n", [DataName]),
            {DataName, As};
        'undefined' ->
            arg_list_has_data_var(DataName, Aliases, T);
        {Alias, _} ->
            ?DEBUG("  sub-list had alias ~p~n", [Alias]),
            {Alias, As}
    end;
...
list_of_keys_to_binary(_Head, _Tail, Path) ->
    Path.
...
process_mf_arity(#usage{usages = Usages} = Acc, M, F, Arity) ->
    case mfa_clauses(Acc, M, F, Arity) of
        ...
        [Clauses] ->
            ...
            Acc#usage{
                usages = lists:usort(ModuleUsages ++ Usages),
                functions = NewFs
            }
    end.
        ...
        [] when ShouldAddAST ->
            case kz_ast_util:module_ast(M) of
                ...
                {M, AST} ->
                    ...
                    #module_ast{functions = NewFs} = kz_ast_util:add_module_ast(
                        #module_ast{functions = Fs}, M, AST
                    ),
                    process_mfa_call(
                        Acc#usage{functions = NewFs},
                        M,
                        F,
                        As,
                        'false'
...
process_mfa_clause(
    #usage{data_var_name = DataName} = Acc,
...
process_mfa_clause(
    ...
    case lists:nth(DataIndex, Args) of
        ...
        ?VAR(DataName) ->
            process_clause_body(Acc, Body);
        ?MOD_FUN_ARGS('props', 'set_value', _Args) ->
            process_clause_body(Acc, Body);
        ?VAR(NewName) ->
            ...
            Acc#usage{
                usages = lists:usort(ClauseUsages),
                functions = ClauseFs,
                visited = Vs
            };
        ?ATOM('undefined') ->
        ...
        _Unexpected ->
            ?DEBUG(
                "unexpected arg(~p) at ~p in ~p, expected ~p~n",
...
data_index(DataName, [?VAR(DataName) | _As], Index) ->
    Index;
data_index(
    DataName,
    [
