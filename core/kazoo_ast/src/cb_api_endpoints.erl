...
api_to_ref_doc([]) -> 'ok';
api_to_ref_doc({Module, Paths}) -> api_to_ref_doc(Module, Paths, module_version(Module)).
...
api_path_to_section(_MOdule, _Paths, Acc) ->
    Acc.
...
sort_methods([], _Ordering, Acc) ->
    lists:reverse(Acc);
sort_methods(Methods, [], Acc) ->
    lists:reverse(Methods ++ Acc);
sort_methods(Methods, [Method | Ordering], Acc) ->
    case lists:member(Method, Methods) of
        'false' ->
            sort_methods(Methods, Ordering, Acc);
        'true' ->
            sort_methods(
                lists:delete(Method, Methods),
                Ordering,
                [Method | Acc]
            )
    end.
...
schema_to_doc(Schema, Doc) ->
    ...
    case
        SchemaJObj =/= 'undefined' andalso
            binary:split(DocContents, <<?SCHEMA_SECTION/binary, "\n">>)
    of
        [Before, After] ->
...
to_swagger_json() ->
    ...
    Swagger = kz_json:set_values(
        [
            {<<"paths">>, to_swagger_paths(Paths, BasePaths)},
        ...
        kz_json:from_list(
            [
             ...
             || {Path, V} = KV <- kz_json:to_proplist(kz_json:flatten(JObj0)),
                not lists:member(<<"patternProperties">>, Path) andalso
                    not is_kazoo_prefixed(Path)
            ]
        )
    ),
...
maybe_fix_ref(RelativePath = ?NE_BINARY) -> <<"#/definitions/", RelativePath/binary>>.
...
to_swagger_paths(Paths, BasePaths) ->
    ...
    kz_json:merge(
        kz_json:set_values(Endpoints, kz_json:new()),
...
add_swagger_path(Method, Acc, Path, SchemaParameter) ->
    ...
    BaseResponse = kz_json:from_list_recursive([
...
-spec make_parameters(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_object()) ->
    kz_term:ne_binaries().
make_parameters(Path, Method, SchemaParameter) ->
    lists:usort(
        fun compare_parameters/2,
        lists:flatten(
            [
                Parameter
             || F <- [
                    fun(P, M) -> maybe_add_schema(P, M, SchemaParameter) end,
...
maybe_add_schema(_Path, Method, Schema) when
    Method =:= <<"put">>;
    Method =:= <<"post">>
->
    Schema;
    ...
    case kz_json:get_ne_binary_value(<<"schema">>, PathMeta) of
        ...
        Schema ->
            kz_json:from_list([
                ...
                {<<"required">>, 'true'},
                {<<"schema">>,
                    kz_json:from_list([{<<"$ref">>, <<"#/definitions/", Schema/binary>>}])}
            ])
    end.
...
format_pc_callback({_Path, []}, Acc, _Module, _ModuleName, _Callback) ->
    io:format(
        "module ~s supported path ~s~nm: ~p c: ~p~n",
...
format_path_tokens(<<"/">>) -> [];
format_path_tokens(<<_/binary>> = Token) -> [format_path_token(Token)];
format_path_tokens(Tokens) -> [format_path_token(Token) || Token <- Tokens, Token =/= <<"/">>].

format_path_token(<<"_", Rest/binary>>) ->
    format_path_token(Rest);
format_path_token(Token = <<Prefix:1/binary, _/binary>>) when
    byte_size(Token) >= 3
->
...
path_name(Module) ->
    case grep_cb_module(Module) of
        ...
        {'match', [<<"schemas">> = Name]} ->
            Name;
        {'match', [<<"shared_auth">> = Name]} ->
            Name;
        {'match', [<<"sup">> = Name]} ->
            Name;
        {'match', [<<"system_configs">> = Name]} ->
            Name;
        {'match', [<<"templates">> = Name]} ->
            Name;
        {'match', [<<"token_auth">> = Name]} ->
            Name;
        {'match', [<<"ubiquiti_auth">> = Name]} ->
            Name;
        {'match', [<<"user_auth">> = Name]} ->
            Name;
        {'match', [<<"quickcall">> = Name]} ->
            <<?ACCOUNTS_PREFIX "/" ?ENDPOINTS_PREFIX "/", Name/binary>>;
        {'match', [Name]} ->
            <<?ACCOUNTS_PREFIX "/", Name/binary>>;
...
process_api_ast_functions(Module, Functions) ->
    {Module, [process_api_ast_function(Module, F, A, Cs) || {F, A, Cs} <- Functions]}.
...
args_list_to_path([]) -> <<"/">>;
args_list_to_path(Args) -> lists:reverse(lists:foldl(fun arg_to_path/2, [], Args)).
...
-spec find_methods_in_clause(erl_parse:abstract_expr(), kz_term:ne_binaries()) ->
    kz_term:ne_binaries().
...
find_methods_in_clause(?EMPTY_LIST, Acc) ->
    Acc;
find_methods_in_clause(
    ?LIST(?BINARY(Method), ?EMPTY_LIST),
    Acc
) ->
    [list_to_binary(Method) | Acc];
find_methods_in_clause(
    ?LIST(?BINARY(Method), Cons),
    Acc
) ->
...
find_methods_in_clause(
    ?LIST(
        ?TUPLE([?ATOM('to_binary'), BinaryList]),
        Rest
    ),
    Acc
) ->
...
find_methods_in_clause(?CASE(_CaseConditional, CaseClauses), Acc0) ->
    lists:foldl(
        fun(?CLAUSE(_Args, _Guards, ClauseBody), Acc1) ->
...
-spec find_content_types_in_clause(erl_parse:abstract_expr(), kz_term:ne_binaries()) ->
    kz_term:ne_binaries().
find_content_types_in_clause(?EMPTY_LIST, Acc) ->
    Acc;
find_content_types_in_clause(
    ?LIST(
        ?TUPLE(?CONTENT_TYPE_VARS(_Type, _SubType)),
        Rest
    ),
    Acc
) ->
...
find_content_types_in_clause(
    ?LIST(
        ?TUPLE(?CONTENT_TYPE_BINS(Type, SubType)),
        Rest
    ),
    Acc
) ->
...
def_path_param(<<"{ADDRESS_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{ALERT_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{APP_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{AUTH_TOKEN}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{BLACKLIST_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{C2C_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{CALLFLOW_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{CARD_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{CCCP_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{CONFERENCE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{CONFIG_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{CONNECTIVITY_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{DEVICE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{DIRECTORY_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{FAXBOX_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{FAX_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{GROUP_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{KEY_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{LEDGER_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{LINK_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{LIST_ENTRY_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{LIST_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{MEDIA_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{MENU_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{NOTIFICATION_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{PORT_REQUEST_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{QUEUE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{RATE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{RESOURCE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{RESOURCE_TEMPLATE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{SMS_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{STORAGE_PLAN_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{TEMPLATE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{TEMPORAL_RULE_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{TEMPORAL_RULE_SET}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{USER_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{VM_BOX_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{WEBHOOK_ID}">> = P) ->
    generic_id_path_param(P);
def_path_param(<<"{MIGRATION_ID}">> = P) ->
    generic_id_path_param(P);
%% When param represents an MoDB id (i.e. 32+4+2 bytes of hexa & 1 dash):
def_path_param(<<"{CDR_ID}">> = P) ->
    modb_id_path_param(P);
def_path_param(<<"{RECORDING_ID}">> = P) ->
    modb_id_path_param(P);
%% When you don't know (ideally you do know):
def_path_param(<<"{ARGS}">> = P) ->
    base_path_param(P);
def_path_param(<<"{ATTACHMENT_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{ATTEMPT_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{CALL_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{COMMENT_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{ERROR_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{EXTENSION}">> = P) ->
    base_path_param(P);
def_path_param(<<"{FAX_JOB_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{HANDLER_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{INTERACTION_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{JOB_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{LANGUAGE}">> = P) ->
    base_path_param(P);
def_path_param(<<"{LEDGER_ENTRY_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{PLAN_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{PROMPT_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{PROVIDER_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{SAMPLE_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{SELECTOR_NAME}">> = P) ->
    base_path_param(P);
def_path_param(<<"{SMTP_LOG_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{SOCKET_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{SYSTEM_CONFIG_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{TEMPLATE_NAME}">> = P) ->
    base_path_param(P);
def_path_param(<<"{THING}">> = P) ->
    base_path_param(P);
def_path_param(<<"{TRANSACTION_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{USERNAME}">> = P) ->
    base_path_param(P);
def_path_param(<<"{VM_MSG_ID}">> = P) ->
    base_path_param(P);
def_path_param(<<"{WHITELABEL_DOMAIN}">> = P) ->
    base_path_param(P);
...
handle_filter_clause(
    [?VAR('Doc'), ?BINARY_MATCH([?BINARY_STRING(FilterName)]), ?VAR('Val')], _Guards, Acc
) ->
    kz_json:set_value(kz_term:to_binary(FilterName), <<"{VALUE}">>, Acc);
handle_filter_clause(
    [?VAR('Doc'), ?BINARY_MATCH([?BINARY_STRING(FilterName)]), ?VAR('Key')], _Guards, Acc
) ->
