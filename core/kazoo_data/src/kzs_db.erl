...
db_create(#{} = Map, DbName, Options) ->
    ...
    case do_db_create(Map, DbName, Options) of
        ...
        'true' ->
            kzs_publish:publish_db(DbName, 'created'),
            db_create_others(Map, DbName, Options);
        'false' ->
            'false'
    end.
...
db_delete(#{} = Map, DbName, Options) ->
    do_db_delete(Map, DbName) andalso
        db_delete_others(Map, DbName, Options) andalso
        kzs_publish:publish_db(DbName, 'deleted').
...
-spec db_info_all({atom(), kz_term:ne_binaries()}, [{atom(), map()}]) ->
    {'ok', kz_term:ne_binaries()} | data_error().
db_info_all(DBs, []) -> DBs;
db_info_all({'ok', DBs}, Others) -> lists:foldl(fun db_info_all_fold/2, DBs, Others).

-spec db_info_all_fold({atom(), map()}, kz_term:ne_binaries()) ->
    {'ok', kz_term:ne_binaries()} | data_error().
...
maybe_cache_db_exists('false', _, _) ->
    'false';
...
do_db_view_update(#{server := {App, Conn}} = Server, Db, NewViews, Remove) ->
    case kzs_view:all_design_docs(Server, Db, ['include_docs']) of
        {'ok', JObjs} ->
            CurrentViews = [
                {kz_doc:id(JObj), kz_json:get_value(<<"doc">>, JObj)}
             || JObj <- JObjs
            ],
...
-spec add_update_remove_views(
    map(), kz_term:ne_binary(), views_listing(), views_listing(), boolean()
) -> boolean().
add_update_remove_views(Server, Db, CurrentViews, NewViews, ShouldRemoveDangling) ->
    ...
    _ =
        case ShouldRemoveDangling of
            'true' ->
                lager:debug(
                    "view updates found ~p new, ~p possible updates and ~p potential removals for db ~s",
                    [length(Add), length(Update), length(Delete), Db]
                );
            'false' ->
                lager:debug(
                    "view updates found ~p new, ~p possible updates for db ~s",
    ...
    _ =
        ShouldRemoveDangling andalso
            delete_views(Server, Db, Delete, CurrentViews),
    Corrected > 0 orelse
        Changed > 0 orelse
        (length(Add) > 0 andalso
            length(Conflicts) < length(Add)).

-spec add_views(map(), kz_term:ne_binary(), kz_term:ne_binaries(), views_listing()) ->
...
-spec update_views(
    map(), kz_term:ne_binary(), kz_term:ne_binaries(), views_listing(), views_listing()
) -> {integer(), kz_term:api_ne_binaries()}.
...
-spec log_save_view_error(kz_term:ne_binary(), kz_term:api_ne_binary()) ->
...
-spec should_update(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    boolean().
...
-spec correct_view_errors(map(), kz_term:ne_binary(), kz_term:ne_binaries(), views_listing()) ->
    integer().
