...
-define(CACHED_SYSTEM_DATAPLAN,
    fetch_cached_dataplan(?SYSTEM_DATAPLAN, fun fetch_simple_dataplan/1)
).
...
-spec plan(kz_term:ne_binary(), atom() | kz_term:ne_binary() | view_options() | kz_json:object()) ->
    map().
...
maybe_override_plan(Plan, #{} = Map) -> maps:merge(Plan, Map).
...
system_dataplan(DBName, _Classification) when
    DBName == ?KZ_CONFIG_DB;
    DBName == ?KZ_DATA_DB;
...
dataplan_type_match(Classification, DocType, Plan, AccountId) ->
    ...
    ClassificationTag = kz_term:to_atom(
        maps:get(<<"connection">>, TypeMap, ClassificationConnection), 'true'
    ),
...
cache_dataplan({_AccountId, StorageId} = Key, PlanJObj) ->
    ...
    Plan2 = maps:merge(Plan, #{<<"connections_map">> => maps:from_list(Connections)}),
    CacheProps = [
        {'origin', [{'db', ?KZ_DATA_DB, StorageId}]},
        {'expires', 'infinity'},
...
cache_system_dataplan() ->
    ...
    CacheProps = [
        {'origin', [{'db', ?KZ_DATA_DB, ?SYSTEM_DATAPLAN}]},
        {'expires', 'infinity'},
        {'callback', fun cache_callback/3}
    ],
...
-spec account_dataplan_merged(kz_json:object(), kz_term:api_object(), kz_json:object()) ->
    kz_json:object().
...
fetch_storage_dataplan_merged(AccountId, StoragePlan) ->
    AccountPlan =
        case fetch_account_dataplan(AccountId) of
...
maybe_start_connection(Connection, Params) when
    is_binary(Connection)
->
...
start_connection(Tag, Params) ->
    ...
    case kz_dataconnections:wait_for_connection(Tag, ?NEW_CONNECTION_TIMEOUT) of
        ...
        'ok' ->
            {Tag, kz_dataconnections:get_server(Tag)}
    end.
...
bind() ->
    Bindings = [
        {<<"doc_created">>, fun handle_created/1},
...
reload(AccountId, StorageId) ->
    case kz_cache:peek_local(?KAZOO_DATA_PLAN_CACHE, {'plan', {AccountId, StorageId}}) of
        {'ok', _Plan} ->
            kz_cache:erase_local(?KAZOO_DATA_PLAN_CACHE, {'plan', {AccountId, StorageId}});
        _Else ->
            load_storage(AccountId, StorageId)
    end.

-spec load_accounts(kz_term:ne_binaries()) -> 'ok'.
load_accounts(Accounts) when
    length(Accounts) > ?KZS_PLAN_INIT_SLICE
->
