...
db_classification(<<"_users">>) ->
    'external';
db_classification(<<"_dbs">>) ->
    'external';
db_classification(<<"users">>) ->
    'external';
db_classification(<<"dbs">>) ->
    'external';
db_classification(<<"_metadata">>) ->
    'external';
db_classification(<<"_nodes">>) ->
    'external';
db_classification(<<"nodes">>) ->
    'external';
db_classification(<<"_replicator">>) ->
    'external';
db_classification(<<"_global_changes">>) ->
    'external';
db_classification(<<"ts">>) ->
    'deprecated';
db_classification(<<"crossbar_schemas">>) ->
    'deprecated';
db_classification(<<"registrations">>) ->
    'deprecated';
db_classification(<<"crossbar%2Fsessions">>) ->
    'deprecated';
db_classification(<<"sms">>) ->
    'deprecated';
db_classification(<<"cccps">>) ->
    'system';
db_classification(<<"signups">>) ->
    'deprecated';
db_classification(?KZ_RATES_DB) ->
    'ratedeck';
db_classification(?MATCH_RATEDECK_DB_ENCODED(_)) ->
    'ratedeck';
db_classification(?MATCH_RATEDECK_DB_encoded(_)) ->
    'ratedeck';
db_classification(?MATCH_RATEDECK_DB_UNENCODED(_)) ->
    'ratedeck';
db_classification(?KZ_ALERTS_DB) ->
    'system';
db_classification(?KZ_OFFNET_DB) ->
    'system';
db_classification(?KZ_ANONYMOUS_CDR_DB) ->
    'system';
db_classification(?KZ_DEDICATED_IP_DB) ->
    'system';
db_classification(?KZ_CONFIG_DB) ->
    'system';
db_classification(?KZ_MEDIA_DB) ->
    'system';
db_classification(?KZ_SCHEMA_DB) ->
    'system';
db_classification(?KZ_OAUTH_DB) ->
    'system';
db_classification(?KZ_AUTH_DB) ->
    'system';
db_classification(?KZ_DATA_DB) ->
    'system';
db_classification(?KZ_TASKS_DB) ->
    'system';
db_classification(?KZ_PENDING_NOTIFY_DB) ->
    'system';
%% Soon to be deprecated
db_classification(?KZ_PROVISIONER_DB) ->
    'system';
db_classification(?KZ_ACCOUNTS_DB) ->
    'aggregate';
db_classification(?KZ_TOKEN_DB) ->
    'aggregate';
db_classification(?KZ_SIP_DB) ->
    'aggregate';
db_classification(?KZ_FAXES_DB) ->
    'aggregate';
db_classification(?KZ_ACDC_DB) ->
    'aggregate';
db_classification(?KZ_SERVICES_DB) ->
    'aggregate';
db_classification(?KZ_PORT_REQUESTS_DB) ->
    'aggregate';
db_classification(?KZ_WEBHOOKS_DB) ->
    'aggregate';
db_classification(<<?KNM_DB_PREFIX, _/binary>>) ->
    'numbers';
db_classification(<<?KNM_DB_PREFIX_ENCODED, _/binary>>) ->
    'numbers';
db_classification(<<?KNM_DB_PREFIX_encoded, _/binary>>) ->
    'numbers';
db_classification(?MATCH_RESOURCE_SELECTORS_UNENCODED(_AccountId)) ->
    'resource_selectors';
db_classification(?MATCH_RESOURCE_SELECTORS_encoded(_AccountId)) ->
    'resource_selectors';
db_classification(?MATCH_RESOURCE_SELECTORS_ENCODED(_AccountId)) ->
    'resource_selectors';
db_classification(?MATCH_RESOURCE_SELECTORS_RAW(_AccountId)) ->
    'resource_selectors';
% these only need to match
db_classification(?MATCH_MODB_SUFFIX_UNENCODED(_A, _B, _Rest, _Year, _Month)) ->
    'modb';
%   "account..." then the
db_classification(?MATCH_MODB_SUFFIX_ENCODED(_A, _B, _Rest, _Year, _Month)) ->
    'modb';
%   right size.
db_classification(?MATCH_MODB_SUFFIX_encoded(_A, _B, _Rest, _Year, _Month)) ->
    'modb';
%   right size.
db_classification(?MATCH_MODB_SUFFIX_RAW(_Account, _Year, _Month)) ->
    'modb';
db_classification(?MATCH_ACCOUNT_UNENCODED(_AccountId)) ->
    'account';
db_classification(?MATCH_ACCOUNT_encoded(_AccountId)) ->
    'account';
db_classification(?MATCH_ACCOUNT_ENCODED(_AccountId)) ->
    'account';
db_classification(?MATCH_PROVISIONER_RAW(_AccountId)) ->
    'provisioner';
db_classification(?MATCH_PROVISIONER_ENCODED(_AccountId)) ->
    'provisioner';
db_classification(?MATCH_PROVISIONER_encoded(_AccountId)) ->
    'provisioner';
...
sort_by_priority(Databases) ->
    ...
    lists:sort(
        fun(Db1, Db2) -> sort_by_priority(Db1, Db2, MODBPriority) end, lists:usort(Databases)
    ).
...
db_priority(?KZ_CONFIG_DB, _) ->
    0;
db_priority(?KZ_DATA_DB, _) ->
    1;
db_priority(?KZ_OFFNET_DB, _) ->
    2;
db_priority(?KZ_ACCOUNTS_DB, _) ->
    3;
db_priority(?KZ_SIP_DB, _) ->
    4;
db_priority(?KZ_AUTH_DB, _) ->
    5;
db_priority(?KZ_WEBHOOKS_DB, _) ->
    6;
db_priority(?KZ_RATES_DB, _) ->
    7;
db_priority(?KZ_ACDC_DB, _) ->
    8;
db_priority(?KZ_FAXES_DB, _) ->
    9;
db_priority(?KZ_SCHEMA_DB, _) ->
    10;
db_priority(?KZ_SERVICES_DB, _) ->
    11;
db_priority(?KZ_PORT_REQUESTS_DB, _) ->
    12;
db_priority(?KZ_TASKS_DB, _) ->
    13;
db_priority(?KZ_PENDING_NOTIFY_DB, _) ->
    13;
db_priority(?KZ_DEDICATED_IP_DB, _) ->
    14;
db_priority(?KZ_ALERTS_DB, _) ->
    15;
db_priority(?KZ_MEDIA_DB, _) ->
    16;
db_priority(?KZ_OAUTH_DB, _) ->
    17;
db_priority(?KZ_TOKEN_DB, _) ->
    18;
db_priority(<<?KNM_DB_PREFIX, _/binary>>, _) ->
    19;
db_priority(<<?KNM_DB_PREFIX_ENCODED, _/binary>>, _) ->
    19;
db_priority(<<?KNM_DB_PREFIX_encoded, _/binary>>, _) ->
    19;
db_priority(?MATCH_ACCOUNT_UNENCODED(_AccountId), _) ->
    21;
db_priority(?MATCH_ACCOUNT_encoded(_AccountId), _) ->
    21;
db_priority(?MATCH_ACCOUNT_ENCODED(_AccountId), _) ->
    21;
...
db_priority(?MATCH_MODB_SUFFIX_RAW(_Account, _Year, _Month) = Db, MODBPriority) ->
    db_priority_modb(kz_util:format_account_modb(Db, 'raw'), MODBPriority);
db_priority(?MATCH_RESOURCE_SELECTORS_UNENCODED(_AccountId), _) ->
    26;
db_priority(?MATCH_RESOURCE_SELECTORS_encoded(_AccountId), _) ->
    26;
db_priority(?MATCH_RESOURCE_SELECTORS_ENCODED(_AccountId), _) ->
    26;
db_priority(?MATCH_RESOURCE_SELECTORS_RAW(_AccountId), _) ->
    26;
db_priority(?MATCH_PROVISIONER_ENCODED(_AccountId), _) ->
    27;
db_priority(?MATCH_PROVISIONER_encoded(_AccountId), _) ->
    27;
db_priority(_Database, _) ->
    27.

db_priority_modb(
    ?MATCH_MODB_SUFFIX_RAW(_Account, Year, Month),
    #{current := {Year, Month}}
) ->
    22;
db_priority_modb(
