...
-define(DEFAULT_TRACE_OUTPUT_FORMAT, [
    'time',
    " [",
    'severity',
    "] |",
...
handle_call({'trace_file', TraceOptions}, _From, #state{traces = Traces} = State) ->
    case start_trace(TraceOptions) of
        {'ok', TraceResult} ->
            ...
            {'reply', {'ok', Ref}, State#state{
                traces = [trace_ref(Ref, TraceOptions, TraceResult) | Traces]
            }};
        Result ->
            {'reply', Result, State}
    end;
...
handle_call(
    ...
    case lists:keytake(TraceRef, 1, Traces) of
        'false' ->
            {'reply', {'error', 'invalid_trace'}, State};
