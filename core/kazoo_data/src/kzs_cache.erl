...
%% Doc related
-export([
    open_cache_doc/3, open_cache_doc/4,
    open_cache_docs/3,
...
-define(DEFAULT_NO_CACHING_TYPES, [
    <<"media">>,
    <<"private_media">>,
    <<"call_recording">>,
    <<"fax">>,
...
% see kz_cache_stampede_tests
-define(STAMPEDE_WAIT_MS, 1500).
...
open_cache_doc(DbName, DocId, Options) ->
    ...
    case kz_cache:fetch_local(?CACHE_NAME, ?CACHE_KEY(DbName, DocId)) of
        {MitigationKey, _Pid} when CacheStrategy =:= 'stampede' ->
            kz_cache:wait_for_stampede_local(
                ?CACHE_NAME, ?CACHE_KEY(DbName, DocId), ?STAMPEDE_WAIT_MS
            );
        {MitigationKey, _Pid} ->
...
open_cache_docs(DbName, DocIds, Options) ->
    ...
    case
        MissedDocIds =/= [] andalso
            kz_datamgr:open_docs(DbName, MissedDocIds, remove_cache_options(Options))
    of
        {'error', _} = E -> E;
        Other -> prepare_jobjs(DbName, DocIds, Options, Cached, Other)
    end.
...
fetch_locals(DbName, DocIds) ->
    F = fun(DocId, {Cached, Missed}) ->
        case kz_cache:fetch_local(?CACHE_NAME, ?CACHE_KEY(DbName, DocId)) of
            ...
            {'ok', {'error', Reason}} -> {[{DocId, 'error', Reason} | Cached], Missed};
            {'ok', Doc} -> {[{DocId, 'ok', Doc} | Cached], Missed}
        end
    end,
...
-spec disassemble_jobjs(kz_term:ne_binary(), kz_term:proplist(), kz_json:objects()) ->
    docs_returned().
    ...
    [
        case kz_json:get_json_value(<<"doc">>, JObj) of
            'undefined' ->
                ...
                _ = maybe_cache_failure(
                    DbName, DocId, Options, {'error', kz_term:to_atom(Reason)}, cache_strategy()
                ),
                {DocId, 'error', Reason};
            Doc ->
...
-spec maybe_cache_failure(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist(), data_error(), cache_strategy()
) -> 'ok'.
...
maybe_cache_failure(
    DbName, DocId, _Options, {'error', ErrorCode} = Error, ErrorCodes, CacheStrategy
) ->
    case lists:member(ErrorCode, ErrorCodes) of
        'true' -> add_to_doc_cache(DbName, DocId, Error, CacheStrategy);
        'false' -> kz_cache:erase_local(?CACHE_NAME, ?CACHE_KEY(DbName, DocId))
    end.

-spec add_to_doc_cache(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object() | data_error()) ->
    'ok'.
add_to_doc_cache(DbName, DocId, CacheValue) ->
    add_to_doc_cache(DbName, DocId, CacheValue, 'none').

-spec add_to_doc_cache(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object() | data_error(), cache_strategy()
) -> 'ok'.
add_to_doc_cache(DbName, DocId, CacheValue, CacheStrategy) ->
    ...
    case kz_json:is_json_object(CacheValue) of
        ...
        'false' when CacheStrategy =:= 'async' ->
            kz_cache:store_local_async(
                ?CACHE_NAME, ?CACHE_KEY(DbName, DocId), CacheValue, CacheProps
            );
...
-spec maybe_cache(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_json:object() | data_error(),
    kz_cache:store_options()
) -> 'ok'.
maybe_cache(DbName, DocId, CacheValue, CacheProps) ->
...
-spec cache_if_not_media(
    kz_term:proplist(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), cache_strategy()
) -> 'ok'.
cache_if_not_media(CacheProps, DbName, DocId, CacheValue, CacheStrategy) ->
    ...
    case
        kzs_util:db_classification(DbName) =/= 'system' andalso
            lists:member(kz_doc:type(CacheValue), ?NO_CACHING_TYPES)
    of
        'true' ->
            kz_cache:erase_local(?CACHE_NAME, ?CACHE_KEY(DbName, DocId));
        'false' when
...
-spec check_if_newer_revision(
    kz_term:proplist(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()
) -> 'ok'.
check_if_newer_revision(CacheProps, DbName, DocId, CacheValue) ->
...
-spec cache_it(kz_term:proplist(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    'ok'.
cache_it(CacheProps, DbName, DocId, CacheValue) ->
...
-spec expires_policy_value(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    timeout().
expires_policy_value(<<"system_config">>, _, _) ->
    'infinity';
expires_policy_value(<<"system_data">>, _, _) ->
    'infinity';
expires_policy_value(DbName, Classification, Type) ->
...
-spec flush_cache_doc(
    kz_term:ne_binary() | db(), kz_term:ne_binary() | kz_json:object(), kz_term:proplist()
) -> 'ok'.
...
flush_cache_docs(DbName) ->
    Filter = fun
        (?CACHE_KEY(DbName1, _DocId) = Key, _) when DbName1 =:= DbName ->
...
-spec flush_cache_docs(kz_term:ne_binary() | db(), kz_term:ne_binaries() | kz_json:objects()) ->
    'ok'.
flush_cache_docs(Db, Docs) ->
    flush_cache_docs(Db, Docs, []).

-spec flush_cache_docs(
    kz_term:ne_binary() | db(), kz_term:ne_binaries() | kz_json:objects(), kz_term:proplist()
) -> 'ok'.
flush_cache_docs(Db, Docs, Options) ->
    _ = [
        flush_cache_doc(Db, Doc, Options)
     || Doc <- Docs
    ],
    'ok'.
...
set_cache_strategy(Strategy) when
    Strategy =:= 'none';
    Strategy =:= 'stampede';
    Strategy =:= 'async';
