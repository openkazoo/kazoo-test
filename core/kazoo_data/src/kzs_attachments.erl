...
%% Attachment-related
-export([
    fetch_attachment/4, fetch_attachment/5,
    stream_attachment/5,
    put_attachment/6,
    delete_attachment/5,
    attachment_url/6
...
-spec fetch_attachment(
    map(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_data:options()
) ->
    {'ok', binary()}
    | data_error()
    | kz_att_error:error().
...
do_fetch_attachment_from_handler(
    [{Handler, HandlerProps}], {Module, ModuleProps}, DbName, DocId, AName
) ->
...
-spec stream_attachment(
    map(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), pid()
) ->
    {'ok', reference()}
    | data_error().
...
do_stream_attachment_from_handler([{Handler, Props}], 'undefined', DbName, DocId, AName, Caller) ->
    Module = kz_term:to_atom(Handler, 'true'),
    Ref = make_ref(),
    _Pid = kz_util:spawn(fun relay_stream_attachment/7, [
        Caller, Ref, Module, Props, DbName, DocId, AName
    ]),
    lager:debug("relaying streamed attachment via ~p(~p) in ~s", [_Pid, Ref, Module]),
    {'ok', Ref};
do_stream_attachment_from_handler(
    [{Handler, HandlerProps}], {Module, ModuleProps}, DbName, DocId, AName, Caller
) ->
    Props = kz_json:set_value(<<"handler_props">>, ModuleProps, HandlerProps),
    ...
    Ref = make_ref(),
    _Pid = kz_util:spawn(fun relay_stream_attachment/7, [
        Caller, Ref, FinalModule, FinalProps, DbName, DocId, AName
    ]),
...
put_attachment(
    #{att_handler := {Handler, Params}} = Map,
    DbName,
    DocId,
    AName,
    Contents,
    ...
    case Handler:put_attachment(Params, DbName, DocId, AName, Contents, Options) of
        ...
        {'error', _Reason, _ExtendedError} = AttHandlerError ->
            case props:get_value('save_error', Options, 'true') of
                'true' ->
                    _Pid = kz_util:spawn(
                        fun save_attachment_handler_error/3,
...
attachment_from_handler(AName, AttHandler, Size, CT) ->
    Props = [
        {<<"content_type">>, kz_term:to_binary(CT)},
        {<<"length">>, Size},
        {<<"stub">>, 'false'},
        {<<"handler">>, AttHandler}
    ],
...
handle_put_attachment(
    #{
        att_post_handler := 'stub',
        server := {App, Conn}
    },
    _Att,
    DbName,
    ...
    App:put_attachment(Conn, DbName, DocId, AName, Contents, Options);
handle_put_attachment(
    #{att_post_handler := 'external'} = Map, Att, DbName, DocId, _AName, _Contents, _Options, Props
) ->
...
-spec external_attachment(
    att_map(), kz_term:ne_binary(), kz_json:object(), kz_json:object(), Props
) ->
...
-spec delete_attachment(
    map(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()
) ->
    {'ok', kz_json:object()}
    | data_error().
...
-spec save_attachment_handler_error(
    att_map(),
    kz_term:ne_binary(),
    kz_att_error:error()
) -> 'ok'.
save_attachment_handler_error(
    Map,
    DbName,
