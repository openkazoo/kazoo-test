...
get_mapped_voice(Voice) ->
    case lists:keyfind(Voice, #voiceDesc.name, ?GOOGLE_TTS_VOICE_MAPPINGS) of
        ...
        'false' -> get_mapped_voice(Voice, binary:match(Voice, <<"/">>))
    end.

-spec get_mapped_voice(kz_term:ne_binary(), 'nomatch' | tuple()) ->
    mapped_voice().
get_mapped_voice(_Voice, 'nomatch') ->
    'undefined';
...
get_matched_voice(Voice) ->
    ...
    lists:filter(
        fun(VoiceDesc) -> should_filter_voice(VoiceDesc, Gender, Language) end,
...
should_filter_language(#voiceDesc{languageCodes = LangCodes}, Language) ->
    lists:any(
        fun(LangCode) -> Language =:= kz_term:to_lower_binary(LangCode) end,
        LangCodes
    ).

-spec make_request(
    kz_term:ne_binary(), mapped_voice(), audioEncoding(), kz_term:proplist(), kz_json:object()
) -> create_resp().
...
-spec build_request_body(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), audioEncoding()
) ->
    kz_term:ne_binary().
build_request_body(Text, Voice, Language, GoogleFormat) ->
    Req = kz_json:set_values(
        [
            {<<"input">>, kazoo_tts_google_utils:getJson(#synthesisInput{text = Text})},
            {<<"voice">>,
                kazoo_tts_google_utils:getJson(#voiceSelectionParams{
                    languageCode = Language, name = Voice
                })},
            {<<"audioConfig">>,
                kazoo_tts_google_utils:getJson(#audioConfig{audioEncoding = GoogleFormat})}
        ],
        kz_json:new()
    ),
    kz_json:encode(Req).

-spec decode(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
...
create_response({'ok', _Code, _RespHeaders, Content}, _EngineData) ->
    ...
    {'error', 'tts_provider_failure',
        kz_json:get_value([<<"error">>, <<"message">>], kz_json:decode(Content))}.
