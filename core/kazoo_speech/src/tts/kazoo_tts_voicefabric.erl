...
-define(VOICEFABRIC_TTS_URL,
    kapps_config:get_string(
        ?MOD_CONFIG_CAT, <<"tts_url_voicefabric">>, <<"https://voicefabric.ru/WSServer/ws/tts">>
    )
).
...
create(Text, Voice, _Format, Options) ->
    ...
    case props:get_value(kz_term:to_lower_binary(Voice), VoiceMappings) of
        ...
        VFabricVoice ->
            ...
            ArgsEncode = kapps_config:get_binary(
                ?MOD_CONFIG_CAT, <<"tts_args_encode">>, <<"multipart">>
            ),
                ...
                {'ok', Headers, Body} ->
                    ...
                    case props:get_value('receiver', Options) of
                        Pid when is_pid(Pid) ->
                            ...
                            Response = kz_http:async_req(
                                Pid, 'post', BaseUrl, Headers, Body, HTTPOptions
                            ),
...
create_response({'ok', 200, Headers, Content}) ->
    ...
    To = "wav",
    Cmd = binary_to_list(
        iolist_to_binary(["sox -t ", From, " ", RawFile, " -t ", To, " ", WavFile])
    ),
    lager:debug("os cmd: ~ts", [Cmd]),
    CmdOut = os:cmd(Cmd),
    CmdOut =:= [] orelse
        lager:debug("cmd out: ~ts", [CmdOut]),
...
voicefabric_get_media_rate(Headers1) ->
    Headers = [
        {kz_term:to_lower_binary(X), kz_term:to_binary(Y)}
     || {X, Y} <- Headers1
    ],
