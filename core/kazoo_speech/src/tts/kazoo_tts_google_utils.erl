...
-spec getJson(synthesisInput() | voiceSelectionParams() | audioConfig()) ->
    {'ok', kz_json:object()}
...
validate_voice_selection(#voiceSelectionParams{}) ->
    'ok'.
...
validate_synthesis_input(#synthesisInput{text = 'undefined'}) ->
    'ok';
validate_synthesis_input(#synthesisInput{ssml = 'undefined'}) ->
    'ok';
...
validate_audio_config(#audioConfig{
    speakingRate = SpeakingRate,
    pitch = Pitch,
    volumeGainDb = VolumeGainDb
}) ->
    Fs = [
        {fun is_speakingRate/1, SpeakingRate, <<"SpeakingRate">>},
...
synthesis_input_to_json(_SynthesisInput, {'error', _Message} = Error) ->
    Error;
synthesis_input_to_json(
    #synthesisInput{
        text = Text,
        ssml = SSML
    },
...
voice_selection_to_json(_VoiceSelection, {'error', _Message} = Error) ->
    Error;
voice_selection_to_json(
...
audio_config_to_json(_AudioConfig, {'error', _Message} = Error) ->
    Error;
audio_config_to_json(
    #audioConfig{
