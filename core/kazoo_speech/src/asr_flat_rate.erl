...
authorize(
    #asr_req{account_id = AccountId, asr_provider = Provider, impact_reseller = 'false'} = Request
) ->
...
authorize(
    ...
    case {maybe_consume_flat_rate(Account, Amount), maybe_consume_flat_rate(Reseller, Amount)} of
        {{'true', _}, {'true', _}} ->
            Request#asr_req{
                account_authorized = 'true', reseller_authorized = 'true', amount = Amount
            };
        {{'true', _}, {'false', _}} ->
            Request#asr_req{
                account_authorized = 'true', reseller_authorized = 'false', amount = Amount
            };
        {{'false', _}, {'true', _}} ->
            Request#asr_req{
                account_authorized = 'false', reseller_authorized = 'true', amount = Amount
            };
        {_, _} ->
            Request#asr_req{
                account_authorized = 'false', reseller_authorized = 'false', amount = Amount
            }
    end.
...
create_ledger_entry(
    #asr_req{account_id = AccountId, reseller_id = ResellerId, impact_reseller = 'true'} = Request
) ->
...
create_ledger_entry(AccountId, Request) ->
    ...
    case kz_ledger:debit(kz_ledger:setters(Setters), AccountId) of
        {'ok', _} ->
            Request;
        {'error', Msg} ->
            asr_request:add_error(
                Request, {'error', 'asr_provider_failure', kz_term:to_binary(Msg)}
            )
    end.
...
debit(#asr_req{account_authorized = 'true', impact_reseller = 'false'} = Request) ->
    maybe_debit(Request);
debit(
    #asr_req{account_authorized = 'true', reseller_authorized = 'true', impact_reseller = 'true'} =
        Request
) ->
    maybe_debit(Request);
debit(Request) ->
...
%%% @end
%%%------------------------------------------------------------------------------
-spec maybe_consume_flat_rate(kz_services:services(), kz_currency:dollars()) ->
    kz_services_standing:acceptable_return().
...
metadata(Request) ->
    kz_json:from_list([
        {<<"account_id">>, asr_request:account_id(Request)},
