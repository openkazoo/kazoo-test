        ...
        {'error', _, _ReasonIP} ->
            case detect_ip_is_bindable(DefaultIP) of
                {'ok', _, DefaultIPAddress} ->
                    ?SUP_LOG_WARNING(
                        "can not bind to address '~s' (~s), using default binding ip '~s'",
                ...
                {'error', _, _ReasonDefault} ->
                    Msg = kz_term:to_binary(
                        io_lib:format(
                            "can not bind to ip '~s' (~s) and default binding ip '~s' (~s)",
...
%% @end
%%------------------------------------------------------------------------------
-spec detect_ip_family(kz_term:ne_binary() | string()) ->
    {inet:address_family(), inet:ip_address()} | {'error', 'einval'}.
...
%% @end
%%------------------------------------------------------------------------------
-spec detect_ip_is_bindable(
    'inet' | 'inet6' | kz_term:text() | {'inet' | 'inet6', inet:ip_address()} | {'error', 'einval'}
) ->
...
detect_ip_is_bindable({Family, IPAdress}) ->
    ...
    try
        ...
        _E:_T ->
            {'error',
                io_lib:format(
                    "exception occurred when detecting ip family supportability/bindability: ~p:~p",
                    [_E, _T]
                )}
    end.
...
is_rfc1918_ip(IP) ->
    verify_cidr(IP, "192.168.0.0/16") orelse
        verify_cidr(IP, "10.0.0.0/8") orelse
        verify_cidr(IP, "172.16.0.0/12").
...
-spec maybe_resolve_fold(kz_term:ne_binary(), kz_term:ne_binaries(), options()) ->
    kz_term:ne_binaries().
maybe_resolve_fold(Domain, IPs, Options) ->
...
-spec maybe_resolve_nameservers([nameserver() | string()], [nameserver()]) ->
    [nameserver()].
maybe_resolve_nameservers([], Nameservers) ->
    Nameservers;
