...
redact_json_password(Line) ->
    re:replace(
        Line,
        <<"(([\'\"])password\\2\\s*:\\s*\\2)((?:.(?!(?<![\\\\])\\2))*.?)\\2">>,
        "\\1" ++ ?REDACTED_REPLACEMENT ++ "\\2",
        [{'return', 'binary'}]
    ).
...
redact_json_credit_card_number(Line) ->
    re:replace(
        Line,
        <<"(([\'\"])credit_card\\2\\s*:\\s*{\\s*\\2number\\2\\s*:\\s*\\2)((?:.(?!(?<![\\\\])\\2))*.?)\\2">>,
        "\\1" ++ ?REDACTED_REPLACEMENT ++ "\\2",
        [{'return', 'binary'}]
    ).
