...
suffix(<<_/binary>> = Suffix, <<_/binary>> = Bin) ->
    try
        truncate_left(Bin, byte_size(Suffix)) =:= Suffix
    catch
        _:_ -> 'false'
    end.
...
pos(Char, <<_:8, Bin/binary>>, N) -> pos(Char, Bin, N + 1).
