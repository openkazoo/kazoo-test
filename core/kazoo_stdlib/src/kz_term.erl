...
to_integer(X, notstrict) when is_binary(X) ->
    try
        ...
        error:badarg -> round(binary_to_float(X))
    end;
...
to_float(X, notstrict) when is_binary(X) ->
    try
        ...
        error:badarg -> binary_to_integer(X) * 1.0
    end;
...
to_number(X) when is_binary(X) ->
    try
        ...
        error:badarg -> binary_to_float(X)
    end;
...
-spec to_binary(
    kz_json:object() | atom() | string() | binary() | integer() | float() | pid() | iolist()
) -> binary().
...
-spec to_api_binary(atom() | string() | binary() | integer() | float() | pid() | iolist()) ->
    api_binary().
...
is_empty(MaybeJObj) ->
    case kz_json:is_json_object(MaybeJObj) of
        %% if not a json object, it's not empty
        'false' -> 'false';
