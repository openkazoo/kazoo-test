...
prop_test_object_gen() ->
    ?FORALL(
        ...
        collect(
            to_range(2, kz_json_generators:max_depth(JObj)),
...
prop_lift_common() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                ?debugFmt(
                    "~nfailed to lift out common ~p~nunique: ~p~n",
                ...
                begin
                    ...
                    {CommonProperties, [UpCommon, UpUnique, UpCommon, UpUnique]} = kz_json:lift_common_properties(
                        [CommonJObj, Merged, CommonJObj, Merged]
                    ),
                    kz_json:are_equal(CommonJObj, CommonProperties) andalso
                        kz_json:are_equal(kz_json:new(), UpCommon) andalso
                        kz_json:are_equal(UpUnique, UniqueJObj)
                end
            )
        end
    ).
...
prop_nothing_in_common() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                ?debugFmt(
                    "~nfailed nothing_in_common:~ncommon ~p~nunique: ~p~n",
                    [CommonJObj, UniqueJObj]
                ),
                begin
                    {CommonProperties, [UpCommon, UpUnique]} = kz_json:lift_common_properties([
                        CommonJObj, UniqueJObj
                    ]),
                    kz_json:are_equal(kz_json:new(), CommonProperties) andalso
                        kz_json:are_equal(CommonJObj, UpCommon) andalso
                        kz_json:are_equal(UpUnique, UniqueJObj)
                end
            )
        end
...
prop_lift_common_blacklist() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                ?debugFmt(
                    "~nfailed to lift out common ~p~nunique: ~p~nmerged: ~p~nblacklisted: ~p~n",
                    [
                        CommonJObj,
                        UniqueJObj,
                ...
                begin
                    ...
                    {CommonProperties, [UpCommon, UpUnique]} = kz_json:lift_common_properties(
                        [CommonJObj, Merged], Blacklist
                    ),

                    kz_json:are_equal(kz_json:delete_keys(Blacklist, CommonJObj), CommonProperties) andalso
                        kz_json:get_value(Blacklist, UpCommon) =:=
                            kz_json:get_value(Blacklist, CommonJObj) andalso
                        kz_json:get_value(Blacklist, UpUnique) =:=
                            kz_json:get_value(Blacklist, Merged)
                end
            )
        end
    ).
...
prop_is_object() ->
    ?FORALL(
        ...
        resize(?MAX_OBJECT_DEPTH, kz_json_generators:deep_object()),
        ?WHENFAIL(
            ?debugFmt("Failed is_valid_json_object with ~p~n", [JObj]),
            kz_json:is_valid_json_object(JObj)
        )
    ).
...
prop_from_list() ->
    ?FORALL(
        ...
        flat_proplist(),
        ?WHENFAIL(
            ?debugFmt("Failed prop_from_list with ~p~n", [Prop]),
...
prop_get_value() ->
    ?FORALL(
        ...
        resize(?MAX_OBJECT_DEPTH, kz_json_generators:deep_object()),
        ?WHENFAIL(
            ?debugFmt("Failed prop_get_value with ~p~n", [JObj]),
            begin
...
prop_set_value() ->
    ?FORALL(
        ...
        ?TRAPEXIT(
            ?WHENFAIL(
                ?debugFmt(
                    "Failed prop_set_value with ~w:~w -> ~p~n",
                    [Key, Value, JObj]
                ),
                begin
                    JObj1 = kz_json:set_value(Key, Value, JObj),

                    'true' =:= kz_json:is_defined(Key, JObj1) andalso
...
prop_delete_key() ->
    ?FORALL(
        ...
        ?TRAPEXIT(
            ?WHENFAIL(
                ?debugFmt(
                    "Failed kz_json:set_value(~w, ~w, ~w)~n",
                    [Key, Value, JObj]
                ),
                begin
                    JObj1 = kz_json:set_value(Key, Value, JObj),
                    ...
                    'true' =:= kz_json:is_defined(Key, JObj1) andalso
                        'false' =:= kz_json:is_defined(Key, JObj2)
                end
            )
        )
    ).
...
prop_flatten_expand() ->
    ?FORALL(
        ...
        resize(?MAX_OBJECT_DEPTH, kz_json_generators:deep_object()),
        ?WHENFAIL(
            ?debugFmt("Failed to flatten/expand: ~p~n", [JObj]),
...
prop_merge_right() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                ?debugFmt(
                    "Failed to merge_right (~p, ~p)~nmerge-r/2: ~p~n",
...
prop_merge_left() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                ?debugFmt(
                    "Failed to merge_left (~p, ~p)~nmerge-l/2: ~p~n",
                    [LeftJObj, RightJObj, MergedJObj]
                ),
...
prop_key_with_null() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                ?debugFmt(
                    "setting nulls failed on ~p:~nnull: ~p~nnot: ~p~n",
                    [Path, WithNull, WithoutNull]
                ),
                kz_json:get_value(Path, WithNull) =:= 'null' andalso
                    kz_json:get_value(Path, WithoutNull) =:= 'undefined'
            )
        end
    ).
...
prop_merge_with_null() ->
    ?FORALL(
        ...
        begin
            ...
            Merged = kz_json:merge(fun kz_json:merge_left/3, WithNull, RightJObj, #{
                'keep_null' => 'true'
            }),
            ...
            ?WHENFAIL(
                begin
                    ?debugFmt(
                        "merging nulls failed on ~p:~nnull: ~p~nright: ~p~nmerged: ~p~n",
                        [Path, WithNull, RightJObj, Merged]
                    ),
                    start_debug(kz_json),
                    kz_json:merge(fun kz_json:merge_left/3, WithNull, RightJObj, #{
                        'keep_null' => 'true'
                    }),
                    stop_debug()
                end,
                kz_json:get_value(Path, WithNull) =:= 'null' andalso
                    kz_json:get_value(Path, Merged) =:= 'null'
            )
        end
    ).
...
merge_left_test_() ->
    JObjs = [
        {
            kz_json:from_list([{<<"foo">>, kz_json:new()}]),
    ...
    [
        [
            ?_assert(
                are_all_properties_found(
                    kz_json:merge(fun kz_json:merge_left/2, Left, Right),
                    Left
                )
            ),
            ?_assert(
                are_all_properties_found(
                    kz_json:merge(fun kz_json:merge_left/2, Right, Left),
                    Right
                )
            )
        ]
...
prop_to_map() ->
    ?FORALL(
        ...
        resize(?MAX_OBJECT_DEPTH, kz_json_generators:deep_object()),
        ?WHENFAIL(
            ?debugFmt("failed json->map->json on ~p~n", [JObj]),
...
is_property_found(Key, ?JSON_WRAPPER(_) = Value, Merged) ->
    case kz_json:get_value(Key, Merged) of
        ?JSON_WRAPPER(_) = MergedV ->
            are_all_properties_found(Value, MergedV);
        Missing ->
...
lift_common_properties_test_() ->
    CommonProperties = kz_json:from_list([
        {<<"foo">>, <<"bar">>},
        {<<"key">>, <<"value">>}
    ]),
    ...
    ?_assertEqual(
        {CommonProperties, [kz_json:new(), UniqueProperties]},
...
lift_common_properties_nested_blacklist_test_() ->
    CommonProperties = kz_json:set_values(
        [
            {[<<"foo">>, <<"fang">>], <<"bar">>},
            {[<<"foo">>, <<"fong">>], <<"bor">>}
        ],
    ...
    {Common, Endpoints} = kz_json:lift_common_properties(
    ...
    [
        ?_assert(
            kz_json:are_equal(
                Common, kz_json:set_values([{[<<"foo">>, <<"fong">>], <<"bor">>}], kz_json:new())
            )
        ),
        ?_assertEqual(
            Endpoints,
...
-define(SP,
    kz_json:decode(
        <<"{\"plan\":{\"phone_numbers\":{\"did_us\":{\"discounts\":{\"cumulative\":{\"rate\":1}}}}}}">>
    )
).
-define(O,
    kz_json:decode(
        <<"{\"phone_numbers\":{\"did_us\":{\"discounts\":{\"cumulative\":{\"rate\":20}}}}}">>
    )
).
...
merge_recursive_overrides_test_() ->
    Key = [
        <<"plan">>, <<"phone_numbers">>, <<"did_us">>, <<"discounts">>, <<"cumulative">>, <<"rate">>
    ],
...
merge_with_null_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            ...
            kz_json:all(
                fun({Key, Value}) ->
                    (not lists:member(Key, NullKeys)) orelse
                        Value =:= 'null'
                end,
...
get_binary_boolean_test_() ->
    [
        ...
        ?_assertEqual(
            <<"false">>,
            kz_json:get_binary_boolean(<<"a_key">>, ?JSON_WRAPPER([{<<"a_key">>, 'false'}]))
        ),
        ?_assertEqual(
            <<"true">>,
            kz_json:get_binary_boolean(<<"a_key">>, ?JSON_WRAPPER([{<<"a_key">>, 'true'}]))
        )
    ].
...
is_false_test_() ->
    [
        ...
        ?_assertEqual(
            'true', kz_json:is_false(<<"a_key">>, ?JSON_WRAPPER([{<<"a_key">>, 'false'}]))
        )
    ].
...
-define(D1_FILTERED,
    ?JSON_WRAPPER([
        {<<"d1k2">>, <<"d1v2">>}, {<<"d1k3">>, [<<"d1v3.1">>, <<"d1v3.2">>, <<"d1v3.3">>]}
    ])
).
-define(D2_FILTERED, ?JSON_WRAPPER([{<<"sub_d1">>, ?D1}])).
-define(D3_FILTERED,
    ?JSON_WRAPPER([
        {<<"d3k1">>, <<"d3v1">>}, {<<"d3k2">>, []}, {<<"sub_docs">>, [?D1, ?D2_FILTERED]}
    ])
).
filter_test_() ->
    [
...
-define(P3, [
    {<<"d3k1">>, <<"d3v1">>},
    {<<"d3k2">>, []},
    {<<"sub_docs">>, [?JSON_WRAPPER(?P1), ?JSON_WRAPPER(?P2)]}
]).
-define(P4, [?P1, ?P2, ?P3]).
-define(P6, [
    {<<"d2k1">>, 1}, {<<"d2k2">>, 3.14}, {<<"sub_d1">>, ?JSON_WRAPPER([{<<"d1k1">>, <<"d1v1">>}])}
]).
-define(P7, [{<<"d1k1">>, <<"d1v1">>}]).
...
get_keys_test_() ->
    ...
    [
        ?_assertEqual(
            'true', lists:all(fun(K) -> lists:member(K, Keys) end, kz_json:get_keys([], ?D1))
        ),
        ?_assertEqual(
            'true',
            lists:all(
                fun(K) -> lists:member(K, Keys) end, kz_json:get_keys([<<"sub_docs">>, 1], ?D3)
            )
        ),
        ?_assertEqual(
            'true',
            lists:all(
                fun(K) -> lists:member(K, [1, 2, 3]) end, kz_json:get_keys([<<"sub_docs">>], ?D3)
            )
        )
    ].
...
delete_key_test_() ->
    [
        ...
        ?_assertEqual(
            ?EMPTY_JSON_OBJECT, kz_json:delete_key(<<"foo">>, ?EMPTY_JSON_OBJECT, 'prune')
        ),
        ?_assertEqual(?EMPTY_JSON_OBJECT, kz_json:delete_key([<<"foo">>], ?EMPTY_JSON_OBJECT)),
        ?_assertEqual(
            ?EMPTY_JSON_OBJECT, kz_json:delete_key([<<"foo">>], ?EMPTY_JSON_OBJECT, 'prune')
        ...
        ?_assertEqual(
            ?EMPTY_JSON_OBJECT, kz_json:delete_key([<<"foo">>, <<"bar">>], ?EMPTY_JSON_OBJECT)
        ),
        ?_assertEqual(
            ?EMPTY_JSON_OBJECT,
            kz_json:delete_key([<<"foo">>, <<"bar">>], ?EMPTY_JSON_OBJECT, 'prune')
        ),
        ...
        ?_assertEqual(
            ?D6_AFTER_SUB_PRUNE, kz_json:delete_key([<<"sub_d1">>, <<"d1k1">>], ?D6, 'prune')
        ),
        ...
        ?_assertError(
            'badarg',
            kz_json:delete_key([<<"foo">>, <<"bar">>], kz_json:from_list([{<<"foo">>, 5}]))
        ),
        ?_assertError(
            'badarg',
            kz_json:delete_key([<<"foo">>, <<"bar">>], kz_json:from_list([{<<"foo">>, 5}]), 'prune')
        )
    ].
...
get_value_test_() ->
    ...
    [
        ...
        %% Basic nested key
        ?_assertEqual(
            'undefined', kz_json:get_value([<<"sub_d1">>, <<"d1k2">>], ?EMPTY_JSON_OBJECT)
        ),
        ...
        %% an array of objects
        ?_assertEqual(
            'undefined', kz_json:get_value([3, <<"sub_docs">>, 2, <<"d2k2">>], ?EMPTY_JSON_OBJECT)
        ),
        ...
        ?_assertEqual(
            <<"not">>, kz_json:get_value([3, <<"sub_docs">>, <<"2">>, <<"d2k2">>], [], <<"not">>)
        ),
...
take_value_test_() ->
    Tests = [
        {'false', <<"foo">>, kz_json:new()},
        {
            {'value', <<"bar">>, kz_json:new()},
            <<"foo">>,
            kz_json:from_list([{<<"foo">>, <<"bar">>}])
        },
        {'false', <<"fuu">>, kz_json:from_list([{<<"foo">>, <<"bar">>}])},
        {'false', [<<"foo">>, <<"fuu">>],
...
-define(T2R3,
    ?JSON_WRAPPER([
        {<<"d1k1">>, <<"d1v1">>},
        {<<"d1k2">>, ?JSON_WRAPPER([{<<"new_key">>, <<"added_value">>}])},
        {<<"d1k3">>, [<<"d1v3.1">>, <<"d1v3.2">>, <<"d1v3.3">>]}
    ])
).
-define(T2R4,
    ?JSON_WRAPPER([
        ...
        {<<"d1k2">>, <<"d1v2">>},
        {<<"d1k3">>, [<<"d1v3.1">>, <<"d1v3.2">>, <<"d1v3.3">>]},
        {<<"d1k4">>, ?JSON_WRAPPER([{<<"new_key">>, <<"added_value">>}])}
    ])
).
...
set_value_object_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            ?T2R3, kz_json:set_value([<<"d1k2">>, <<"new_key">>], <<"added_value">>, ?D1)
        ),
...
-define(T3R2, [
    ?JSON_WRAPPER([{<<"k1">>, <<"v1">>}, {<<"pi">>, 3.14}]), ?JSON_WRAPPER([{<<"k2">>, <<"v2">>}])
]).
-define(T3R3, [
    ?JSON_WRAPPER([
        {<<"k1">>, <<"v1">>}, {<<"callerid">>, ?JSON_WRAPPER([{<<"name">>, <<"2600Hz">>}])}
    ]),
    ?JSON_WRAPPER([{<<"k2">>, <<"v2">>}])
]).
...
-define(T4R1,
    ?JSON_WRAPPER([{<<"Caller-ID">>, 1234}, {list_to_binary(lists:seq(16#C0, 16#D6)), <<"Smith">>}])
).
%% <<"àáâãäåæçèéêëìíîïðñòóôõö"
-define(T4R1V,
    ?JSON_WRAPPER([{<<"caller_id">>, 1234}, {list_to_binary(lists:seq(16#E0, 16#F6)), <<"Smith">>}])
).
%% <<"ØÙÚÛÜÝÞ"
-define(T5R1,
    ?JSON_WRAPPER([{<<"Caller-ID">>, 1234}, {list_to_binary(lists:seq(16#D8, 16#DE)), <<"Smith">>}])
).
%% <<"øùúûüýþ"
-define(T5R1V,
    ?JSON_WRAPPER([{<<"caller_id">>, 1234}, {list_to_binary(lists:seq(16#F8, 16#FE)), <<"Smith">>}])
).

-define(T4R2,
    ?JSON_WRAPPER([
        {<<"Account-ID">>, <<"45AHGJDF8DFDS2130S">>},
        {<<"TRUNK">>, 'false'},
        {<<"Node1">>, ?T4R1},
        {<<"Node2">>, ?T4R1}
    ])
...
-define(T4R3,
    ?JSON_WRAPPER([
        {<<"Node-1">>, ?JSON_WRAPPER([{<<"Node-2">>, ?T4R2}])}, {<<"Another-Node">>, ?T4R1}
    ])
).
-define(T4R3V,
    ?JSON_WRAPPER([
        {<<"node_1">>, ?JSON_WRAPPER([{<<"node_2">>, ?T4R2V}])}, {<<"another_node">>, ?T4R1V}
    ])
).
-define(T5R3,
    ?JSON_WRAPPER([
        {<<"Node-1">>, ?JSON_WRAPPER([{<<"Node-2">>, null}])}, {<<"Another-Node">>, ?T4R1}
    ])
).
...
codec_test_() ->
    [
        ...
        ?_assertThrow(
            {error, {invalid_ejson, undefined}},
            kz_json:encode(?JSON_WRAPPER(?PROPS_WITH_UNDEFINED))
        ),
        ?_assert(
            kz_json:are_equal(
                kz_json:from_list(?PROPS_WITH_UNDEFINED),
...
find_value_test_() ->
    ...
    [
        ?_assertEqual(
            <<"{\"k1\":\"v1\"}">>, kz_json:encode(kz_json:find_value(<<"k1">>, <<"v1">>, JObjs))
        ),
        ?_assertEqual(
            <<"{\"k1\":\"v2\"}">>, kz_json:encode(kz_json:find_value(<<"k1">>, <<"v2">>, JObjs))
        ),
...
-define(CHARGES_SIMPLE,
    kz_json:decode(
        <<"{\"phone_numbers\":{\"did_us\":{\"category\":\"phone_numbers\",\"item\":\"did_us\",\"name\":\"US DID\",\"quantity\":1,\"rate\":1.0,\"single_discount\":true,\"single_discount_rate\":0.0,\"cumulative_discount\":1,\"cumulative_discount_rate\":0.5,\"activation_charge\":2.0,\"minimum\":0,\"exceptions\":[],\"activation_charges\":2.0}},\"activation_charges\":2.0}">>
    )
).
-define(CHARGES_DOUBLE,
    kz_json:decode(
        <<"{\"phone_numbers\":{\"did_us\":{\"category\":\"phone_numbers\",\"item\":\"did_us\",\"name\":\"US DID\",\"quantity\":2,\"rate\":2.0,\"single_discount\":true,\"single_discount_rate\":0.0,\"cumulative_discount\":2,\"cumulative_discount_rate\":1.0,\"activation_charge\":4.0,\"minimum\":0,\"exceptions\":[],\"activation_charges\":4.0}},\"activation_charges\":4.0}">>
    )
).

-define(AMOUNTS,
    kz_json:decode(
        <<"[{\"mobile_data\":{\"amount\":-10.5,\"usage\":{\"type\":\"debit\",\"unit\":\"MB\",\"quantity\":9482}}},{\"per_minute_voip\":{\"amount\":-155,\"usage\":{\"type\":\"voice\",\"unit\":\"sec\",\"quantity\":2120}}},{\"per_minute_voip\":{\"amount\":37,\"usage\":{\"type\":\"voice\",\"unit\":\"sec\",\"quantity\":480}}}]">>
    )
).

-define(AMOUNT,
    kz_json:decode(
        <<"{\"mobile_data\":{\"amount\":-10.5,\"usage\":{\"type\":\"debit\",\"unit\":\"MB\",\"quantity\":9482}},\"per_minute_voip\":{\"amount\":-118,\"usage\":{\"type\":\"voice\",\"unit\":\"sec\",\"quantity\":2600}}}">>
    )
).
...
from_list_recursive_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            kz_json:get_ne_binary_value(Key2, JObj1), kz_json:get_ne_binary_value(Key2, JObj2)
        ),
...
from_map_test_() ->
    [
        ...
        {"encoding the invalid json generated in from_map with a mixed proplist inside map",
            ?_assertEqual(
                kz_json:encode(?FROM_MAP_JSON_4_RIGHT),
                kz_json:encode(kz_json:from_map(?FROM_MAP_MAP_4))
            )}
    ].
