...
prop_to_integer() ->
    ?FORALL(
        ...
        begin
            ...
            lists:all(
                fun([FN, N]) ->
                    erlang:is_integer(kz_term:to_integer(N)) andalso
                        erlang:is_integer(kz_term:to_integer(FN))
                end,
                Is
            )
        end
...
prop_to_number() ->
    ?FORALL(
        ...
        begin
            ...
            lists:all(
                fun([FN, N]) ->
                    erlang:is_number(kz_term:to_number(N)) andalso
                        erlang:is_number(kz_term:to_number(FN))
                end,
                Is
            )
        end
...
prop_to_float() ->
    ?FORALL(
        ...
        begin
            ...
            lists:all(
                fun([FN, N]) ->
                    erlang:is_float(kz_term:to_float(N)) andalso
                        erlang:is_float(kz_term:to_float(FN))
                end,
                Fs
            )
        end
...
to_x_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            element(1, calendar:gregorian_seconds_to_datetime(TS)),
            kz_term:to_date(kz_term:to_list(TS))
        ),
        ?_assertEqual(
            element(1, calendar:gregorian_seconds_to_datetime(TS)),
            kz_term:to_date(kz_term:to_binary(TS))
        ),
