...
greg_secs_to_unix_secs_test() ->
    ...
    ?assertEqual(
        GregSecs - ?UNIX_EPOCH_IN_GREGORIAN, kz_time:gregorian_seconds_to_unix_seconds(GregSecs)
    ).

unix_secs_to_greg_secs_test() ->
    UnixSecs = 1000000000,
    ?assertEqual(
        UnixSecs + ?UNIX_EPOCH_IN_GREGORIAN, kz_time:unix_seconds_to_gregorian_seconds(UnixSecs)
    ).
...
more_elapsed_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            10 * ?MILLISECONDS_IN_SECOND, kz_time:elapsed_ms(kz_time:now_ms(StartTS), FutureTS)
        ),
        ?_assertEqual(
            10 * ?MILLISECONDS_IN_SECOND, kz_time:elapsed_ms(StartTS, kz_time:now_ms(FutureTS))
        ),

        ?_assertEqual(10 * ?MICROSECONDS_IN_SECOND, kz_time:elapsed_us(StartTS, FutureTS)),
        ?_assertEqual(
            10 * ?MICROSECONDS_IN_SECOND, kz_time:elapsed_us(kz_time:now_us(StartTS), FutureTS)
        ),
        ?_assertEqual(
            10 * ?MICROSECONDS_IN_SECOND, kz_time:elapsed_us(StartTS, kz_time:now_us(FutureTS))
        )
    ].
...
rfc1036_test_() ->
    Tests = [
        {{{2015, 4, 7}, {1, 3, 2}}, <<"Tue, 07 Apr 2015 01:03:02 GMT">>},
...
rfc2822_test_() ->
    Tests = [
        {{{2015, 4, 7}, {1, 3, 2}}, <<"Tue, 07 Apr 2015 01:03:02 +0000">>},
...
iso8601_offset_test_() ->
    Tests = [
        {{{2015, 4, 7}, {0, 0, 0}}, <<"2015-04-07T00:00:00Z">>, <<"UTC">>},
    ...
    [
     ...
     || {Date, Expected, Offset} <- Tests
        ...
        [
            {
                "formatting with offset '" ++ kz_term:to_list(Offset) ++ "' should throw '" ++
                    kz_term:to_list(Expected) ++ "'",
                [?_assertThrow({'error', Expected}, kz_time:iso8601(Date, Offset))]
            }
         || {Date, Offset, Expected} <- ThrowsTests
        ].

-define(ISO8601_EXAMPLES, [
    {<<"2015-04-07T00:00:00">>, {{2015, 4, 7}, {0, 0, 0}}},
...
-define(INVALID_ISO8601_EXAMPLES, [
    {<<"2015/04/07T00.00.00">>, 'invalid_date'},
...
from_iso8601_test_() ->
    [
     ...
     || {Date, Expected} <- ?ISO8601_EXAMPLES
        ...
        [
            {
                "parser should throws '" ++ kz_term:to_list(Expected) ++ "' for '" ++
                    kz_term:to_list(Date) ++ "'",
...
-define(ISO8601_MS_EXAMPLES, [
    {<<"2019-10-09T18:36:13.000Z">>, <<"2019-10-09T18:36:13Z">>},
...
maybe_add_iso8601_ms_suffix_test_() ->
    lists:foldl(
        fun({ISO8601WithMS, ISO8601WithoutMS}, Tests) ->
            [
