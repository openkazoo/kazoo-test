...
prop_normalize() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                io:format(
                    "failed to normalize ~w to ~w: got ~w(~w)~n",
...
prop_iso_week() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                io:format(
                    "failed to convert to/from iso week for ~p <=> ~p~n",
                    [ISOWeek, RoundTrip]
                ),
...
prop_iso8601() ->
    ?FORALL(
        ...
        ?WHENFAIL(
            io:format("failed to roundtrip ~p with ~p~n", [Date, ToFun]),
...
prop_relative_diff() ->
    ?FORALL(
        {GregorianSeconds, Offset},
        %% 1/1/1 00:00:00 - 2600/1/1 00:00:00
        {
            range(31640000, 82048118400),
            %% seconds in year
            range(1, 31540000)
        },
        begin
            ...
            lists:all(
                fun({Diff, A, B}) ->
                    case kz_date:relative_difference(A, B) of
                        ...
                        _Result ->
                            io:format(
                                "failed to calc relative diff of ~p and ~p~n"
                                "expected ~p got ~p~n",
                                [A, B, Diff, _Result]
                            ),
                            'false'
