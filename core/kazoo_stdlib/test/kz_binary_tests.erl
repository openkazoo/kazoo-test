...
prop_bin_reverse() ->
    ?FORALL(
        ...
        binary(),
        ?WHENFAIL(
            ?debugFmt("failed to reverse binary ~p~n", [Bin]),
...
join_binary_test_() ->
    [
        ...
        ?_assertEqual(
            <<"foo, bar, baz">>, kz_binary:join([<<"foo">>, <<"bar">>, <<"baz">>], <<", ">>)
        )
    ].
...
-define(BIN,
    <<"75d46db34465e620c214bdeaf214eac1c87833851166fbbd59aeb0a35aa1bd901abac59eb53fe4c076261537632aefba8ab16889550f84bb51c6934a97ba813c">>
).

-spec rev1(binary()) -> binary().
rev1(Bin) -> rev1(Bin, <<>>).
rev1(<<>>, Acc) -> Acc;
rev1(<<H:1/binary, Rest/binary>>, Acc) -> rev1(Rest, <<H/binary, Acc/binary>>).
