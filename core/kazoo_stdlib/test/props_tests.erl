...
filter_empty_test_() ->
    [
        ...
        ?_assertEqual(
            [{'a', 10}, {'b', 8}, {'c', 6}], props:filter_empty([{'a', 10}, {'b', 8}, {'c', 6}])
        ),
        ?_assertEqual(
            [], props:filter_empty([{'a', 0}, {'b', []}, {'c', <<>>}, {'z', 'undefined'}])
        ),
...
filter_empty_strings_test_() ->
    [
        ...
        ?_assertEqual(
            [{'a', 10}, {'b', 8}, {'c', 6}],
            props:filter_empty_strings([{'a', 10}, {'b', 8}, {'c', 6}])
        ),
        ?_assertEqual(
            [{'a', 0}, {'b', []}, {'z', 'undefined'}],
            props:filter_empty_strings([{'a', 0}, {'b', []}, {'c', <<>>}, {'z', 'undefined'}])
        ),
        ?_assertEqual(['a'], props:filter_empty_strings(['a'])),
        ?_assertEqual(['a', {'b', 0}], props:filter_empty_strings(['a', {'b', 0}])),
        ?_assertEqual(
            [{<<"a">>, 'undefined'}], props:filter_empty_strings([{<<"a">>, 'undefined'}])
        ),
...
filter_undefined_test_() ->
    [
        ...
        ?_assertEqual(
            [{'a', 10}, {'b', 8}, {'c', 6}], props:filter_undefined([{'a', 10}, {'b', 8}, {'c', 6}])
        ),
        ?_assertEqual(
            [{'a', 0}, {'b', []}, {'c', <<>>}],
            props:filter_undefined([{'a', 0}, {'b', []}, {'c', <<>>}, {'z', 'undefined'}])
        ),
...
prop_set_value() ->
    ?FORALL(
        ...
        ?WHENFAIL(
            ?debugFmt("failed: props:is_defined(~p, ~p ++ props:set_value(~p, ~p)).~n", [
                KV, Before, KV, After
            ]),
...
prop_set_values() ->
    ?FORALL(
        ...
        ?WHENFAIL(
            ?debugFmt("Props = ~p ++ props:set_values(~p, ~p)~n", [Before, KVs, After]),
            begin
                Props = Before ++ props:set_values(KVs, After),
                lists:all(
                    fun(KV) -> props:is_defined(KV, Props) end,
                    KVs
                )
            end
        )
...
prop_is_defined() ->
    ?FORALL(
        {Props, Existing, NonExisting},
        test_proplist_and_keys(),
        ?WHENFAIL(
            ?debugFmt(
                "exists props:is_defined(~p, ~p)~nnot props:is_defined(~p, ~p)~n",
                [Existing, Props, NonExisting, Props]
            ),
            props:is_defined(Existing, Props) andalso
                'false' =:= props:is_defined(NonExisting, Props)
        )
    ).
...
not_oneof(Props) ->
    ?LET(
        K,
        test_key(),
        (not lists:member(K, Props)) andalso
