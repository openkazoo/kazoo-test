    ...
    _ =
        case kz_ledgers:get_monthly_rollover(Account, Year, Month) of
            {'ok', Ledger} ->
                ...
                io:format(
                    "[~p] account ~s has rollover (created ~p/~p/~p ~p:~p:~p) for ~p-~p with balance ~p~n",
                    [
                        Remaining,
                        AccountId,
                        _Y,
                        ...
                        Year,
                        Month,
                        kz_ledger:unit_amount(Ledger)
                    ]
                );
            {'error', 'not_found'} ->
                io:format("[~p] account ~s has no rollover for ~p-~p~n", [
                    Remaining, AccountId, Year, Month
                ]);
            Else ->
                io:format("[~p] account ~s error getting monthly rollover ~p~n", [
                    Remaining, AccountId, Else
                ])
        end,
    current_rollovers(Accounts).
...
verify_db_rollover(AccountDb, {Current, Total, Options}) ->
    io:format(
        "verify rollover accounts (~p/~p) '~s'~n",
...
verify_rollovers(AccountDb, Year, Month) ->
    ...
    case kz_ledgers:get_monthly_rollover(AccountDb, Year, Month) of
        ...
        Else ->
            io:format(
                "    account ~s ~p-~p error getting monthly rollover ~p~n",
                [AccountId, Year, Month, Else]
            )
    end.

-spec verify_monthly_rollover(
    kz_term:ne_binary(), kz_time:year(), kz_time:month(), kz_ledger:ledger()
) -> 'ok'.
verify_monthly_rollover(AccountDb, Year, Month, Ledger) ->
    ...
    case kz_currency:past_available_units(AccountDb, PreviousYear, PreviousMonth) of
        {'error', _Reason} ->
            AccountId = kz_util:format_account_id(AccountDb, 'raw'),
            io:format(
                "    account ~s ~p-~p : error getting balance for ~p-~p: ~p~n",
...
-spec verify_rollovers(
    kz_term:ne_binary(), kz_time:year(), kz_time:month(), kz_ledger:ledger(), kz_currency:units()
) -> 'ok'.
verify_rollovers(AccountDb, Year, Month, Ledger, AvailableUnits) ->
    AccountId = kz_util:format_account_id(AccountDb, 'raw'),
    case kz_ledger:amount(Ledger) of
        ...
        _RolloverUnits ->
            io:format(
                "    account ~s ~p-~p rollover balance is ~p but previous month balance was ~p~n",
...
fix_rollover(Account, Year, Month) ->
    ...
    case kz_currency:past_available_units(AccountId, PreviousYear, PreviousMonth) of
        {'error', _R} ->
            io:format(
                "account ~s : error getting balance for ~p-~p: ~p~n",
...
-spec maybe_replace_rollover(
    kz_term:ne_binary(), kz_time:year(), kz_time:month(), kz_currency:units()
) ->
    {'ok', kz_ledger:ledger()}
    | {'error', any()}.
maybe_replace_rollover(Account, Year, Month, AvailableUnits) ->
    case kz_ledgers:get_monthly_rollover(Account, Year, Month) of
        {'error', _} ->
            io:format(
                "account ~s ~p-~p rollover missing, creating for ~p units~n",
        ...
        {'ok', Ledger} = Ok ->
            case kz_ledger:amount(Ledger) =:= AvailableUnits of
                ...
                'false' ->
                    io:format(
                        "account ~s ~p-~p incorrect, updating with ~p units~n",
                        [Account, Year, Month, AvailableUnits]
                    ),
                    Setters = [
                        {fun kz_ledger:set_unit_amount/2, AvailableUnits},
