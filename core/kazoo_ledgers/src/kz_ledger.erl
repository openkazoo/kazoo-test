...
-spec set_ledger_type(ledger(), kz_currency:units() | kz_currency:dollars() | kz_term:ne_binary()) ->
    ledger().
set_ledger_type(Ledger, Amount) when
...
maybe_prefix_id(LedgerJObj) ->
    case kz_doc:id(LedgerJObj) of
        ?MATCH_MODB_PREFIX(_Year, _Month, _Id) ->
            LedgerJObj;
        Id ->
...
to_json(#ledger{private_fields = PrivateFields} = Ledger) ->
    Setters = [
        {fun kzd_ledgers:set_account_id/2, account_id(Ledger)},
    ...
    Props = [
        {kz_doc:path_type(), kzd_ledgers:type()},
...
create_hash(LedgerJObj) ->
    Props = [
        {<<"source">>, kzd_ledgers:source(LedgerJObj)},
...
from_json(JObj) ->
    Setters = [
        {fun set_account_id/2, kzd_ledgers:account_id(JObj)},
...
save(Ledger) ->
    ...
    case kazoo_modb:save_doc(MODb, LedgerJObj) of
        ...
        {'error', _Reason} = Error ->
            lager:info(
                "unable to create ~s ~s in ~s ~p-~p: ~p",
                [
                    source_service(Ledger),
                    ledger_type(Ledger),
                    AccountId,
