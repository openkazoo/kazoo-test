...
-export([
    verify_monthly_rollover_exists/1,
...
total_sources(Account, Options) ->
    case get_sources_total(Account, Options) of
        {'ok', Total} -> {'ok', Total};
        {'error', 'missing_ledgers'} -> maybe_migrate_legacy_rollover(Account, Options);
        {'error', 'missing_rollover'} -> maybe_migrate_legacy_rollover(Account, Options);
...
get_sources_total(Account, Options) ->
    ...
    case kazoo_modb:get_results(Account, View, ViewOptions) of
        {'ok', []} ->
            lager:info(
                "missing ledgers from ~s: ~p/~p",
                [
                    Account,
                    props:get_value('year', ViewOptions),
                    props:get_value('month', ViewOptions)
                ]
            ),
        ...
        {'error', _Reason} = Error ->
            ...
            lager:warning(
                "unable to get balance for ~s ~p-~p: ~p",
...
-spec list_source(kz_term:ne_binary(), kz_term:ne_binary(), kz_time:seconds(), kz_time:seconds()) ->
    {'ok', ledgers()}
    | {'error', any()}.
list_source(Account, Source, CreatedFrom, CreatedTo) when
    is_integer(CreatedFrom),
    CreatedFrom > 0,
    is_integer(CreatedTo),
    CreatedTo > 0
->
...
-spec total_source(kz_term:ne_binary(), kz_term:ne_binary(), kz_time:seconds(), kz_time:seconds()) ->
    kz_currency:available_units_return().
total_source(Account, Source, CreatedFrom, CreatedTo) when
    is_integer(CreatedFrom),
    CreatedFrom > 0,
    is_integer(CreatedTo),
    CreatedTo > 0
...
get_ranged(View, Options) ->
    ...
    case MODbs =:= [] of
        ...
        'false' ->
            ...
            lager:debug(
                "getting ledgers starting from ~p to ~p from dbs: ~p",
                [
...
-spec get_ranged(kz_term:ne_binary(), kz_term:proplist(), kz_term:ne_binaries(), kz_json:objects()) ->
    {'ok', kz_json:objects() | ledgers()}
    | {'error', any()}.
get_ranged(_View, _Options, [], Results) ->
    {'ok', Results};
get_ranged(View, Options, [MODb | MODbs], Results) ->
    ...
    case kazoo_modb:get_results(MODb, View, Options) of
        ...
        {'ok', JObjs} when HasDoc ->
            Ledgers = [
                kz_ledger:set_modb(kz_ledger:from_json(JObj), MODb)
             || JObj <- JObjs
            ],
...
available_ledgers(AccountId) ->
    kapps_account_config:get_global(
        AccountId, <<"ledgers">>, <<"registered_ledgers">>, ?DEFAULT_AVIALABLE_LEDGERS
    ).
...
-spec get_monthly_rollover(kz_term:ne_binary()) ->
    {'ok', kz_ledger:ledger()}
    | {'error', any()}.
...
get_monthly_rollover(Account, Year, Month) ->
    case kazoo_modb:open_doc(Account, ?ROLLOVER_ID(Year, Month), Year, Month) of
        {'ok', LedgerJObj} ->
            Ledger = kz_ledger:set_modb(
                kz_ledger:from_json(LedgerJObj),
                Account,
                Year,
                Month
            ),
        ...
        {'error', _Reason} = Error ->
            lager:debug(
                "unable to get monthly rollover for ~s ~p-~p: ~p",
                [Account, Year, Month, _Reason]
            ),
            Error
    end.
...
maybe_migrate_legacy_rollover(Account, Options) ->
    ...
    lager:warning(
        "ledger rollover not found in ~s ~p-~p",
        [Account, Year, Month]
    ),
    ...
    case should_rollover_monthly_balance() of
        ...
        'false' ->
            lager:debug(
                "monthly balance rollover is disabled, assuming previous balance was 0", []
            ),
            _ = rollover(Account, Year, Month, 0),
            get_sources_total(Account, Options)
    end.

-spec migrate_legacy_rollover(
    kz_tern:ne_binary(), kazoo_modb:view_options(), kz_time:year(), kz_time:month()
) ->
    kz_currency:available_units_return().
migrate_legacy_rollover(Account, Options, Year, Month) ->
    case kz_transactions:legacy_total(Account, Year, Month) of
        {'ok', Amount} ->
            lager:debug(
                "found transaction rollover, migrating $~p to ledger balance",
...
rollover(Account, Year, Month, Total) ->
    ...
    Id = <<(kz_term:to_binary(Year))/binary, (kz_date:pad_month(Month))/binary>>,
    Setters =
        props:filter_empty(
            [
    ...
    case kazoo_modb:save_doc(Account, LedgerJObj, Year, Month) of
        {'ok', _SavedJObj} ->
            lager:info(
                "created ledger rollover for ~s ~p-~p",
                [Account, Year, Month]
            ),
            ViewOptions = [
                {'year', Year},
        ...
        {'error', 'conflict'} ->
            lager:info(
                "ledger rollover ~s for ~s ~p-~p exists already",
        ...
        {'error', _Reason} = Error ->
            lager:warning(
                "unable to save ledger rollover for ~s ~p-~p: ~p",
                [Account, Year, Month, _Reason]
            ),
            Error
    end.
