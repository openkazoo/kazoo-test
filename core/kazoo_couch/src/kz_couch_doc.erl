...
-type copy_function() :: fun(
    (server(), kz_term:ne_binary(), kz_json:object(), kz_term:proplist()) -> doc_return()
).
...
do_ensure_saved(#db{} = Db, Doc, Opts) ->
    case do_save_doc(Db, Doc, Opts) of
        {'ok', _} = Ok -> Ok;
        {'error', 'conflict'} -> handle_conflict(Db, Doc, Opts);
        {'error', _} = E -> E
    end.
...
-spec handle_conflict(couchbeam_db(), kz_json:object(), kz_term:proplist(), doc_return()) ->
    doc_return().
handle_conflict(_Db, _Doc, _Opts, {'error', _} = Error) ->
    Error;
handle_conflict(Db, Doc, Opts, {'ok', CurrentDoc}) ->
    % should have Doc's changes
    Diff = kz_json:diff(Doc, CurrentDoc),

    lager:debug("conflict saving ~s: diff: ~p", [kz_doc:id(Doc), Diff]),

    NewDoc = kz_json:merge(
        ...
        {Save, Cont} ->
            case perform_save_docs(Db, Save, Options) of
                {'error', _} = E ->
                    E;
                {'ok', JObjs} ->
                    %% let slow dbs catch up
                    timer:sleep(?COUCH_MAX_BULK_INSERT),
...
copy_doc(
    ...
    copy_doc(
        Conn,
        CopySpec#kz_copy_doc{
            dest_dbname = SourceDb,
...
copy_doc(#server{} = Conn, CopySpec, CopyFun, Options) ->
    ...
    case open_doc(Conn, SourceDbName, SourceDocId, Options) of
        {'ok', SourceDoc} ->
            ...
            DestinationDoc = kz_json:set_values(
                Props, kz_json:delete_keys(?DELETE_KEYS, SourceDoc)
            ),
...
copy_attachments(#server{} = Conn, CopySpec, {[JObj | JObjs], [Key | Keys]}) ->
    ...
    case kz_couch_attachments:fetch_attachment(Conn, SourceDbName, SourceDocId, Key) of
        {'ok', Contents} ->
            ...
            case
                kz_couch_attachments:put_attachment(
                    Conn, DestDbName, DestDocId, Key, Contents, Opts
                )
            of
                {'ok', _AttachmentDoc} ->
...
-spec maybe_set_account_db(kz_term:api_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:proplist().
...
-spec maybe_remove_doc(server(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()) ->
    'ok'.
maybe_remove_doc(Conn, SourceDbName, SourceDocId, Options) ->
    case open_doc(Conn, SourceDbName, SourceDocId, Options) of
        {'ok', SourceDoc} ->
