...
get_admin_dbs('bigcouch') ->
    <<"dbs">>;
get_admin_dbs(_Driver) ->
    <<"_dbs">>.
...
get_admin_nodes('bigcouch') ->
    <<"nodes">>;
get_admin_nodes(_Driver) ->
    <<"_nodes">>.
...
-spec ensure_saved(
    kz_data:connection(), kz_term:ne_binary(), kz_data:document(), kz_data:options()
) -> any().
...
-spec fetch_attachment(
    kz_data:connection(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()
) -> any().
...
-spec stream_attachment(
    kz_data:connection(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), pid()
) ->
    {'ok', doc()}
    | {'error', any()}.
...
-spec get_results_count(
    server(), 'all_docs' | kz_term:ne_binary(), kz_term:ne_binary(), view_options()
) ->
...
db_local_filter_option({'endkey', Value}, DB) ->
    DB =< Value;
db_local_filter_option(_Option, _DB) ->
    'true'.
