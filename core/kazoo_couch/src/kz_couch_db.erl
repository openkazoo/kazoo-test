...
check_db_create_error(Server, DbName, <<"conflict">>) ->
    lager:warning(
        "db ~s creation failed with HTTP error 500 and conflict reason, checking db exists", [
            DbName
        ]
    ),
...
-spec archive(
    db(), kz_term:ne_binary(), file:io_device(), pos_integer(), non_neg_integer(), non_neg_integer()
) -> 'ok'.
...
archive(#db{} = Db, DbName, File, MaxDocs, N, Pos) when N =< MaxDocs ->
    ...
    case kz_couch_view:all_docs(Db, ViewOptions) of
        {'ok', []} ->
            io:format("    no docs left after pos ~p~n", [Pos]);
        {'ok', Docs} ->
...
maybe_add_comma(File, _Pos) -> 'ok' = file:write(File, [$,]).
