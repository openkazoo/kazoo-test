...
design_compact(#server{} = Conn, DbName, Design) ->
    case couchbeam:compact(kz_couch_util:get_db(Conn, DbName), Design) of
        {'error', _E} ->
            lager:info("failed to compact design doc ~s/~s: ~p", [
                DbName, Design, kz_couch_util:format_error(_E)
            ]),
            'false';
        'ok' ->
            'true'
...
do_fetch_results(Db, DesignDoc, Options) when
    DesignDoc =:= 'all_docs' orelse
        is_tuple(DesignDoc)
->
    ?RETRY_504(
        case couchbeam_view:fetch(Db, DesignDoc, Options) of
            {'ok', JObj} ->
                {'ok', kz_json:get_value(<<"rows">>, JObj, JObj)};
            {'error', Error, []} ->
                {'error', Error};
            {'error', Error, Rows} ->
...
-spec do_fetch_results_count(couchbeam_db(), kz_term:ne_binary() | ddoc(), view_options()) ->
    {'ok', kz_term:api_integer()}
    | couchbeam_error().
do_fetch_results_count(Db, DesignDoc, Options) when
    is_binary(DesignDoc)
->
    [DesignName, ViewName | _] = binary:split(DesignDoc, <<"/">>, ['global']),
