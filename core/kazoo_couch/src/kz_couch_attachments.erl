...
-spec stream_attachment(
    server(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), pid()
) ->
    {'ok', reference()}
    | couchbeam_error().
...
-spec put_attachment(
    server(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()
) ->
    {'ok', kz_json:object()}
    | couchbeam_error().
...
-spec delete_attachment(
    server(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()
) ->
    {'ok', kz_json:object()}
    | couchbeam_error().
...
attachment_url(#server{} = Conn, DbName, DocId, AName, Options) ->
    case kapps_config:get_is_true(?CONFIG_CAT, <<"use_bigcouch_direct">>, 'true') of
        'true' ->
            list_to_binary([
                kz_couch_util:db_url(Conn, DbName),
                "/",
...
-spec do_del_attachment(
    couchbeam_db(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()
) ->
    {'ok', kz_json:object()}
    | couchbeam_error().
