...
test_custom_sip_headers() ->
    ...
    [
        ?_assertEqual(
            <<"foo">>, kzd_devices:custom_sip_header_inbound(TestDevice, <<"x-device-header">>)
        ),
        ?_assertEqual(
            <<"bar">>, kzd_devices:custom_sip_header_outbound(TestDevice, <<"x-outbound-header">>)
        ),
        ?_assertEqual(
            <<"Hz">>, kzd_devices:custom_sip_header_inbound(TestDevice, <<"x-legacy-header">>)
        ),
        ?_assertEqual(
            'undefined', kzd_devices:custom_sip_header_inbound(TestDevice, <<"x-missing-header">>)
        ),
        ?_assertEqual(
            'undefined', kzd_devices:custom_sip_header_outbound(TestDevice, <<"x-missing-header">>)
        )
    ].
...
test_no_legacy_sip_headers() ->
    ...
    [
        ...
        ?_assertEqual(
            'undefined', kzd_devices:custom_sip_header_inbound(TestDevice, <<"x-legacy-header">>)
        ),
        ?_assertEqual(
            'undefined', kzd_devices:custom_sip_header_inbound(TestDevice, <<"x-missing-header">>)
        ),
...
test_no_inbound_sip_headers() ->
    ...
    [
        ?_assertEqual(
            <<"baz">>, kzd_devices:custom_sip_header_inbound(TestDevice, <<"x-device-header">>)
        ),
        ?_assertEqual(
            <<"bar">>, kzd_devices:custom_sip_header_outbound(TestDevice, <<"x-outbound-header">>)
        ),
...
test_no_outbound_custom_sip_headers() ->
    ...
    [
        ...
        ?_assertEqual(
            'undefined', kzd_devices:custom_sip_header_outbound(TestDevice, <<"x-outbound-header">>)
        ),
        ?_assertEqual(
            <<"Hz">>, kzd_devices:custom_sip_header_inbound(TestDevice, <<"x-legacy-header">>)
        ),
        ...
        {"invalid header value check",
            ?_assertMatch(
                ...
                validate(
                    Schema,
                    kz_json:set_value(
                        [<<"sip">>, <<"custom_sip_headers">>, <<"out">>, <<"bad">>],
                        kz_json:new(),
                        TestDevice
                    )
                )
        ...
        {"invalid header check",
            ?_assertMatch(
                ...
                validate(
                    Schema,
                    kz_json:set_value(
                        [<<"sip">>, <<"custom_sip_headers">>, <<"out">>, <<"bad@header">>],
                        <<"ok">>,
                        TestDevice
                    )
                )
...
test_outbound_flags() ->
    ...
    ExpectedNewData = kz_json:decode(
        "{\"dynamic\": [\"not_exported\", \"from_realm\"], \"static\": [\"device_new_static_flag\"]}"
    ),
    ...
    Update = kz_json:from_list([
        {<<"dynamic">>, [<<"not_exported">>, <<"from_realm">>]},
        {<<"static">>, [<<"device_new_static_flag">>]}
    ]),
    ...
    [
        ...
        {"verify both update with undefined static",
            ?_assertEqual(
                [],
                kzd_devices:outbound_static_flags(
                    kzd_devices:set_outbound_flags(NewData, 'undefined', [<<"unrelated_flag">>])
                )
            )},
        {"verify both update with undefined dynamic",
            ?_assertEqual(
                [],
                kzd_devices:outbound_dynamic_flags(
                    kzd_devices:set_outbound_flags(NewData, [<<"unrelated_flag">>], 'undefined')
                )
            )}
    ].
...
test_outbound_flags_static() ->
    {'ok', OldData} = kzd_devices:fetch(?FIXTURE_MASTER_ACCOUNT_ID, ?DEVICE_1_ID),
    UpdatedOldData = kz_json:get_value(
        <<"outbound_flags">>, kzd_devices:set_outbound_static_flags(OldData, [<<"updated_flag">>])
    ),
    ExpectedOldUpdate = kz_json:decode("{\"static\": [\"updated_flag\"]}"),

    {'ok', NewData} = kzd_devices:fetch(?FIXTURE_RESELLER_ACCOUNT_ID, ?DEVICE_2_ID),
    UpdatedNewData = kz_json:get_value(
        <<"outbound_flags">>, kzd_devices:set_outbound_static_flags(NewData, [<<"updated_flag">>])
    ),
    ExpectedNewUpdate = kz_json:decode(
        "{\"dynamic\": [\"not_exported\", \"from_realm\"], \"static\": [\"updated_flag\"]}"
    ),

    [
        {"verify get for deprecated format",
            ?_assertEqual(
                [<<"device_old_static_flag">>], kzd_devices:outbound_static_flags(OldData)
            )},
        {"verify get for new format",
            ?_assertEqual(
                [<<"device_new_static_flag">>], kzd_devices:outbound_static_flags(NewData)
            )},
        {"verify set with old format converts to new",
            ?_assertEqual(ExpectedOldUpdate, UpdatedOldData)},
        {"verify set with new format", ?_assertEqual(ExpectedNewUpdate, UpdatedNewData)}
    ].

test_outbound_dynamic_flags() ->
    {'ok', OldData} = kzd_devices:fetch(?FIXTURE_MASTER_ACCOUNT_ID, ?DEVICE_1_ID),
    UpdatedOldData = kz_json:get_value(
        <<"outbound_flags">>, kzd_devices:set_outbound_dynamic_flags(OldData, [<<"updated_flag">>])
    ),
    ExpectedOldUpdate = kz_json:decode(
        "{\"static\": [\"device_old_static_flag\"], \"dynamic\": [\"updated_flag\"]}"
    ),

    {'ok', NewData} = kzd_devices:fetch(?FIXTURE_RESELLER_ACCOUNT_ID, ?DEVICE_2_ID),
    UpdatedNewData = kz_json:get_value(
        <<"outbound_flags">>, kzd_devices:set_outbound_dynamic_flags(NewData, [<<"updated_flag">>])
    ),
    ExpectedNewUpdate = kz_json:decode(
        "{\"dynamic\": [\"updated_flag\"], \"static\": [\"device_new_static_flag\"]}"
    ),
    [
        {"verify get for deprecated format",
        ...
        {"verify get for new format",
            ?_assertEqual(
                [<<"not_exported">>, <<"from_realm">>], kzd_devices:outbound_dynamic_flags(NewData)
            )},
        {"verify set with old format converts to new",
            ?_assertEqual(ExpectedOldUpdate, UpdatedOldData)},
        {"verify set with new format", ?_assertEqual(ExpectedNewUpdate, UpdatedNewData)}
...
test_device_param_setting() ->
    ...
    Setup = [
        {<<"rick_and_morty">>, fun kzd_devices:set_sip_username/2, fun kzd_devices:sip_username/1},
        ...
        {<<"rick-l@4a6812.sip.2600hz.local">>, fun kzd_devices:set_presence_id/2,
            fun kzd_devices:presence_id/1},
        ...
        {<<"user0000000000000000000000000002">>, fun kzd_devices:set_owner_id/2,
            fun kzd_devices:owner_id/1},
        {'false', fun kzd_devices:set_enabled/2, fun kzd_devices:enabled/1},
        {'false', fun kzd_devices:set_mwi_unsolicited_updates/2,
            fun kzd_devices:mwi_unsolicited_updates/1}
    ],
...
test_calculating_presence_id() ->
    ...
    BlankDevice = kz_json:delete_keys(
        [
            <<"presence_id">>,
            <<"owner_id">>,
            <<"call_forward">>,
    ...
    HotdeskedDevice = kzd_devices:set_hotdesk(
        JustUsername,
        kz_json:from_list([{<<"users">>, kz_json:from_list([{OwnerId, kz_binary:rand_hex(16)}])}])
    ),
    HotdeskedAndOwnedDevice = kzd_devices:set_owner_id(HotdeskedDevice, OwnerId),

    Tests = [
        {"SIP username", <<SIPUsername/binary, "@", AccountRealm/binary>>, JustUsername},
        {"Device presence_id", <<DevicePresenceId/binary, "@", AccountRealm/binary>>,
            WithDevicePresenceId},
