...
-export([
    account_id/1,
    account_billing/1,
    account_trunk_usage/1,
    application_name/1,
    raw_application_name/1,
    authorizing_id/1,
    authorizing_type/1,
    call_direction/1,
    original_call_direction/1,
    call_id/1,
    ...
    outbound_flags/1,
    presence_id/1,
    presence_direction/1,
    request_realm/1,
    reseller_id/1,
    reseller_billing/1,
    reseller_trunk_usage/1,
    resource_id/1,
...
-spec ccv(data(), kz_term:ne_binary(), Default) ->
...
custom_channel_vars_fold(_, Acc) ->
    Acc.
...
switch_uri(Props) ->
    case props:get_first_defined([<<"Switch-URI">>, <<"variable_Switch-URI">>], Props) of
        'undefined' ->
            case switch_url(Props) of
                'undefined' ->
                    'undefined';
                SwitchURL ->
                    case binary:split(SwitchURL, <<"@">>) of
