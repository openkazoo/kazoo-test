...
-type task() :: #{
    ...
    %% Time of task creation (PUT)
    created => kz_time:gregorian_seconds(),
    %% Time of task start (PATCH)
    started => kz_time:api_seconds(),
    %% Time of task finish (> started)
    finished => kz_time:api_seconds(),
    %% CSV rows (undefined for a noinput task)
    total_rows => kz_term:api_pos_integer(),
    %% Rows that crashed or didn't return ok
    total_rows_failed => kz_term:api_non_neg_integer(),
    %% Rows that returned 'ok'
    total_rows_succeeded => kz_term:api_non_neg_integer(),
    %% true if stopped, undefined of false otherwise
    was_stopped => kz_term:api_boolean()
}.
...
new(
    ?MATCH_ACCOUNT_RAW(AuthAccountId),
    ...
    case help(Category, Action) of
        ...
        API ->
            ...
            case find_input_errors(API, Input) of
                ...
                _ ->
                    InputName =
                        case kz_term:is_empty(CSVName) of
                            'true' -> 'undefined';
...
-spec are_mandatories_unset(nonempty_list(boolean()), nonempty_list(kz_term:ne_binary())) ->
    boolean().
...
status(#{
    ...
    Finished =/= 'undefined',
    is_integer(TotalRows),
    is_integer(TotalFailed),
    is_integer(TotalSucceeded),
    TotalRows > TotalFailed,
