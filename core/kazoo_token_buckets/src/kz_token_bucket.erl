...
%% API
-export([
    start_link/2, start_link/3, start_link/4, start_link/5,
    stop/1,
    consume/2,
    consume_until/2,
    credit/2,
...
-ifdef(TEST).
-define(FILL_TIME(App),
    is_binary(App) andalso
        ?FILL_TIME
).
-else.
-define(FILL_TIME(App),
    kapps_config:get_ne_binary(?APP_NAME, [App, <<"tokens_fill_time">>], ?FILL_TIME)
).
-endif.
...
% max size of bucket
-record(state, {
    max_tokens = 100 :: pos_integer(),
    % current count
    tokens = 0 :: non_neg_integer(),
    % tokens/fill_rate_time() added
    fill_rate = 1 :: pos_integer(),
...
-spec start_link(pos_integer(), pos_integer(), boolean(), fill_rate_time()) ->
    kz_types:startlink_ret().
start_link(Max, FillRate, FillAsBlock, FillTime) when
    is_integer(FillRate),
    FillRate > 0,
    is_integer(Max),
    Max > 0,
    is_boolean(FillAsBlock),
    (FillTime =:= 'second' orelse
        FillTime =:= 'minute' orelse
        FillTime =:= 'hour' orelse
        FillTime =:= 'day')
->
    gen_server:start_link(?SERVER, [Max, FillRate, FillAsBlock, FillTime], []).

-spec start_link(atom(), pos_integer(), pos_integer(), boolean(), fill_rate_time()) ->
    kz_types:startlink_ret().
start_link(Name, Max, FillRate, FillAsBlock, FillTime) when
    is_integer(FillRate),
    FillRate > 0,
    is_integer(Max),
    Max > 0,
...
consume(_Srv, Tokens) when
    is_integer(Tokens) andalso
        Tokens =< 0
->
    'true';
consume(Srv, Tokens) when
    is_integer(Tokens) andalso
        Tokens > 0
->
...
consume_until(_Srv, Tokens) when
    is_integer(Tokens) andalso
        Tokens =< 0
->
    'true';
consume_until(Srv, Tokens) when
    is_integer(Tokens) andalso
        Tokens > 0
->
...
init([Max, FillRate, FillAsBlock, FillTime]) ->
    ...
    lager:debug(
        "starting token bucket with ~b max, filling at ~b/~s, in a block: ~s",
...
handle_info(
    ...
    {'noreply', State#state{
        tokens = add_tokens(Max, Current, FillRate, FillAsBlock, FillTime),
        fill_ref = start_fill_timer(FillRate, FillAsBlock, FillTime)
    }};
...
-spec add_tokens(
    pos_integer(), non_neg_integer(), pos_integer(), boolean(), fill_rate_time() | pos_integer()
) ->
    non_neg_integer().
