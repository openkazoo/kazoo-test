...
-define(MAX_TOKENS,
    kapps_config:get_integer(?APP_NAME, [?DEFAULT_APP, <<"max_bucket_tokens">>], 100)
).
...
-record(bucket, {
    key :: {kz_term:ne_binary(), kz_term:ne_binary()} | '_',
    srv :: pid() | '$1' | '$2' | '_',
    ref :: reference() | '$2' | '_',
...
consume_tokens(<<_/binary>> = Key, Count, StartIfMissing) when
    is_integer(Count),
...
-spec consume_tokens_until(
    kz_term:ne_binary(), kz_term:ne_binary() | pos_integer(), pos_integer() | boolean()
) -> boolean().
consume_tokens_until(App = ?NE_BINARY, Key = ?NE_BINARY, Count) when
    is_integer(Count)
->
    consume_tokens_until(App, Key, Count, 'true');
consume_tokens_until(Key = ?NE_BINARY, Count, StartIfMissing) when
    is_integer(Count),
    is_boolean(StartIfMissing)
->
    consume_tokens(?DEFAULT_APP, Key, Count, StartIfMissing).

-spec consume_tokens_until(kz_term:ne_binary(), kz_term:ne_binary(), pos_integer(), boolean()) ->
    boolean().
consume_tokens_until(App = ?NE_BINARY, Key = ?NE_BINARY, Count, StartIfMissing) when
    is_integer(Count),
    is_boolean(StartIfMissing)
->
    consume_tokens(App, Key, Count, StartIfMissing, fun kz_token_bucket:consume_until/2).

-spec consume_tokens(kz_term:ne_binary(), kz_term:ne_binary(), integer(), boolean(), fun()) ->
    boolean().
    ...
    case get_bucket(App, Key) of
        ...
        Srv ->
            case is_process_alive(Srv) of
                ...
                'false' -> maybe_start_bucket(App, Key, Count, StartIfMissing, BucketFun)
            end
    end.

-spec maybe_start_bucket(kz_term:ne_binary(), kz_term:ne_binary(), integer(), boolean(), fun()) ->
    boolean().
maybe_start_bucket(_App, _Key, _Count, 'false', _BucketFun) ->
    'false';
...
-spec get_bucket(kz_term:ne_binary(), kz_term:ne_binary(), 'record' | 'server') ->
    kz_term:api_pid() | bucket().
...
-spec start_bucket(
    ...
    pos_integer(),
    pos_integer(),
    kz_token_bucket:fill_rate_time()
) ->
    'ok' | 'error' | 'exists'.
...
handle_call(
    {'start', _App, _Name, _MaxTokens, _FillRate, _FillTime},
    _From,
    #state{table_id = 'undefined'} = State
) ->
    lager:debug(
        "not starting token bucket for ~p ~p, table not ready",
        [_App, _Name]
    ),
    {'reply', 'error', State};
handle_call(
    {'start', App, Name, MaxTokens, FillRate, FillTime}, _From, #state{table_id = Tbl} = State
) ->
    lager:debug(
        "maybe starting token bucket for ~s, ~s (~b at ~b/~s)",
        [App, Name, MaxTokens, FillRate, FillTime]
    ),
    case
        not exists(App, Name) andalso
            kz_buckets_sup:start_bucket(MaxTokens, FillRate, FillTime)
    of
        {'ok', Pid} when is_pid(Pid) ->
            T = {App, Name},
            case ets:insert_new(Tbl, new_bucket(Pid, T)) of
                'true' ->
                    lager:debug("new bucket for ~s, ~s: ~p", [App, Name, Pid]);
                'false' ->
                    lager:debug("hmm, bucket appears to exist for ~s, ~s, stopping ~p", [
                        App, Name, Pid
                    ]),
