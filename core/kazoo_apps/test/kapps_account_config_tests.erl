...
kapps_accuont_config_test_() ->
    {'setup', fun kazoo_apps_test_util:setup/0, fun kazoo_apps_test_util:cleanup/1, fun(Map) ->
        [
        ...
        {"Testing get_ne_binary account config", [
            {"get a key with binary value",
                ?_assertEqual(
                    true,
                    is_ne_binary(
                        kapps_account_config:get_ne_binary(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                            <<"root_obj_key">>, <<"b_key">>
                        ])
                    )
                )},
            ...
            {"get an empty value should return default",
                ?_assertEqual(
                    undefined,
                    kapps_account_config:get_ne_binary(
                        ?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, <<"not_exists">>
                    )
                )},
            {"get a list of binary value should return list of binary",
                ?_assertEqual(
                    true,
                    is_ne_binaries(
                        kapps_account_config:get_ne_binaries(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                            <<"root_obj_key">>, <<"b_keys">>
                        ])
                    )
                )},
    ...
    [
        {"Testing account get config", [
            {"exists config doc in account should result in account",
                ?_assertEqual(
                    <<"sub_account">>,
                    kapps_account_config:get(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                        <<"root_obj_key">>, <<"b_key">>
                    ])
                )},
            {"undefined property in account should result in Default",
                ?_assertEqual(
                    ...
                    kapps_account_config:get(
                        ?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, <<"udon_me">>, <<"me_don_you">>
                    )
                )}
        ]}
    ].
    ...
    [
        {"Testing get global account config", [
            {"exists config doc in sub-account on get_global/2 should result in account's config doc",
                ?_assertEqual(
                    SubAccountConfig,
                    kapps_account_config:get_global(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT)
                )},
            ...
            {"not exists sub-account config doc and exists config doc for reseller on get_global/2 should result in reseller config doc",
                ?_assertEqual(
                    ResellerOnly,
                    kapps_account_config:get_global(?FIXTURE_CHILD_ACCOUNT_ID, ?RESELLER_ONLY)
                )},
            {"not exists config doc for sub-account and reseller on get_global/2 should result in system_config default value object",
                ?_assertEqual(
                    SystemOnly,
                    kapps_account_config:get_global(?FIXTURE_CHILD_ACCOUNT_ID, ?SYSTEM_ONLY)
                )},
            {"not exists config doc for sub-account and reseller and an empty default system_config on get_global/2 should result in empty doc",
                ?_assertEqual(
                    kz_json:new(),
                    kapps_account_config:get_global(?FIXTURE_PARENT_ACCOUNT_ID, ?SYSTEM_EMPTY)
                )},
            {"not exists config doc for category on get_global/2 should result in an empty object with category as id",
                ?_assertEqual(
                    kz_doc:set_id(kz_json:new(), <<"no_cat_please">>),
                    kapps_account_config:get_global(?FIXTURE_PARENT_ACCOUNT_ID, <<"no_cat_please">>)
                )},
    ...
    [
        {"Common get global account config", [
            ...
            {"undefined property in sub-account config doc should result in system_config value",
                ?_assertEqual(
                    <<"system_only">>,
                    Fun(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [<<"root_obj_key">>, <<"system_key">>])
                )},
            {"defined key in sub-account config doc should result in account value",
                ?_assertEqual(
                    <<"sub_account">>, Fun(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, <<"one_root_key">>)
                )}
        ]}
    ].
...
test_get_from_reseller(_) ->
    ...
    [{"Testing get config from reseller", common_tests_for_get_from_reseller(FunToTest)}].
    ...
    [
        {"Testing strategy with no account id", [
            {"undefined account id should result in system_config value",
                ?_assertEqual(
                    <<"system_only">>,
                    kapps_account_config:get_with_strategy(<<"global">>, undefined, ?TEST_CAT, [
                        <<"root_obj_key">>, <<"system_key">>
                    ])
                )},
            {"empty call object (no account id) should result in system_config value",
                ?_assertEqual(
                    <<"system_only">>,
                    kapps_account_config:get_with_strategy(
                        <<"global">>, kapps_call:new(), ?TEST_CAT, [
                            <<"root_obj_key">>, <<"system_key">>
                        ]
                    )
                )},
            {"empty jobj object (no account id) should result in system_config value",
                ?_assertEqual(
                    <<"system_only">>,
                    kapps_account_config:get_with_strategy(
                        <<"global">>, kz_json:new(), ?SYSTEM_ONLY, <<"key">>
                    )
                )},
            {"unknown object (no account id) should result in system_config value",
                ?_assertEqual(
                    <<"system_only">>,
                    kapps_account_config:get_with_strategy(
                        <<"global">>, maps:new(), ?SYSTEM_ONLY, <<"key">>
                    )
                )},
            ...
            {"passing non raw account id in jobj where account & reseller do not have config doc should result in system_config value",
                ?_assertEqual(
                    ...
                    kapps_account_config:get_with_strategy(
                        <<"global">>,
                        kz_json:from_list([{<<"account_id">>, Db}]),
                        ?SYSTEM_ONLY,
                        <<"key">>
                    )
                )}
...
get_strategy_global() ->
    ...
    [{"Testing get config global strategy", common_tests_for_get_global(FunToTest)}].
...
get_strategy_reseller() ->
    ...
    [{"Testing get config reseller strategy", common_tests_for_get_from_reseller(FunToTest)}].

get_strategy_hierarchy_merge() ->
    SysRootObjKey = get_fixture_value(
        [<<"default">>, <<"root_obj_key">>], ?KZ_CONFIG_DB, ?TEST_CAT
    ),
    ResellerRootJObjKey = get_fixture_value(
        <<"root_obj_key">>, ?FIXTURE_RESELLER_ACCOUNT_ID, ?TEST_CAT
    ),
    ...
    SometimesEmptyValue_System = kz_json:get_value(
        [<<"obj_empty_test">>, <<"obj_empty_sometimes">>], SysRootObjKey
    ),
    SometimesEmptyValue_Reseller = kz_json:get_value(
        [<<"obj_empty_test">>, <<"obj_empty_sometimes">>], ResellerRootJObjKey
    ),
    SometimesEmptyValue_Sub1 = kz_json:get_value(
        [<<"obj_empty_test">>, <<"obj_empty_sometimes">>], ParentRootObjKey
    ),
    ...
    [
        {"Testing get config hierarchy_merge strategy", [
            {"defined value in account where account is reseller itself should result in merged value of reseller and system",
                ?_assertEqual(
                    kz_json:merge_recursive([SysRootObjKey, ResellerRootJObjKey]),
                    kapps_account_config:get_with_strategy(
                        <<"hierarchy_merge">>,
                        ?FIXTURE_RESELLER_ACCOUNT_ID,
                        ?TEST_CAT,
                        <<"root_obj_key">>
                    )
                )},
            {"defined value in parent-account and reseller should result in merged value of parent-account, reseller and system",
                ?_assertEqual(
                    kz_json:merge_recursive([SysRootObjKey, ResellerRootJObjKey, ParentRootObjKey]),
                    kapps_account_config:get_hierarchy(
                        ?FIXTURE_PARENT_ACCOUNT_ID, ?TEST_CAT, <<"root_obj_key">>
                    )
                )},
            {"defined value in account & system and undefined property in parent & reseller should result in merged value of account and system",
                ?_assertEqual(
                    get_key_and_merge_configs(<<"sub_account+system">>, [
                        SysRootObjKey, SubRootObjKey
                    ]),
                    kapps_account_config:get_hierarchy(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                        <<"root_obj_key">>, <<"sub_account+system">>
                    ])
                )},
            {"customized account, reseller, system and empty/not_exists parent should result in merged value of all customized and system",
                ?_assertEqual(
                    get_key_and_merge_configs(<<"sub+reseller+system">>, [
                        SysRootObjKey, ResellerRootJObjKey, SubRootObjKey
                    ]),
                    kapps_account_config:get_hierarchy(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                        <<"root_obj_key">>, <<"sub+reseller+system">>
                    ])
                )},
            {"customized account, parent, system and empty reseller should result in merged value of all customized and system",
                ?_assertEqual(
                    get_key_and_merge_configs(<<"sub+parent+empty_reseller+system">>, [
                        SysRootObjKey, ParentRootObjKey, SubRootObjKey
                    ]),
                    kapps_account_config:get_hierarchy(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                        <<"root_obj_key">>, <<"sub+parent+empty_reseller+system">>
                    ])
                )},
            {"customized account, parent, system and not customized reseller should result in merged value of all customized and system",
                ?_assertEqual(
                    get_key_and_merge_configs(<<"sub+parent+system">>, [
                        SysRootObjKey, ParentRootObjKey, SubRootObjKey
                    ]),
                    kapps_account_config:get_hierarchy(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                        <<"root_obj_key">>, <<"sub+parent+system">>
                    ])
                )},
            {"customized account, parents and system should result in merged value of all",
                ?_assertEqual(
                    kz_json:merge_recursive([
                        SysRootObjKey, ResellerRootJObjKey, ParentRootObjKey, SubRootObjKey
                    ]),
                    kapps_account_config:get_hierarchy(
                        ?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, <<"root_obj_key">>
                    )
                )},
            {"not customized account with undefined parent account id and no reseller should result in system_config",
                ?_assertEqual(
                    SysRootObjKey,
                    kapps_account_config:get_hierarchy(
                        kz_binary:rand_hex(16), ?TEST_CAT, <<"root_obj_key">>
                    )
                )},
            {"check if the account set an empty jobj, the content of the hierarchy is merged into the jobj properly",
                ?_assertEqual(
                    kz_json:merge_recursive([
                        SometimesEmptyValue_System,
                        SometimesEmptyValue_Reseller,
                        SometimesEmptyValue_Sub1
                    ]),
                    kapps_account_config:get_hierarchy(?FIXTURE_CHILD_ACCOUNT_ID, ?TEST_CAT, [
                        <<"root_obj_key">>, <<"obj_empty_test">>, <<"obj_empty_sometimes">>
                    ])
                )}
        ]}
    ].
