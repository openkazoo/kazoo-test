...
-spec system_alert(
    string() | kz_term:ne_binary(), string() | kz_term:ne_binary(), kz_term:proplist()
) -> 'ok'.
system_alert(Subject, Msg, Headers) when
    not is_binary(Subject);
    not is_binary(Msg)
->
...
-spec system_alert(
    kz_term:ne_binary(), string() | kz_term:ne_binary(), [any()], kz_term:proplist()
) -> 'ok'.
...
detailed_alert(Subject, Msg, Props, Headers) when
    not is_binary(Subject);
    not is_binary(Msg)
->
