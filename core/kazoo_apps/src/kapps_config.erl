...
-export([
    get_is_true/2, get_is_true/3, get_is_true/4,
    is_true/2, is_true/3
]).
...
-export([
    get_ne_binary_or_ne_binaries/2, get_ne_binary_or_ne_binaries/3, get_ne_binary_or_ne_binaries/4
]).
...
as_json_value('undefined', Default) ->
    Default;
...
-spec get_pos_integer(config_category(), config_key(), Default, config_node()) ->
    pos_integer() | Default.
...
-spec get_non_neg_integer(config_category(), config_key(), Default, config_node()) ->
...
-spec get_ne_binary_or_ne_binaries(config_category(), config_key()) ->
...
-spec get_ne_binary_or_ne_binaries(config_category(), config_key(), Default) ->
    kz_term:ne_binary() | kz_term:ne_binaries() | Default.
...
-spec get_ne_binary_or_ne_binaries(config_category(), config_key(), Default, config_node()) ->
    kz_term:ne_binary() | kz_term:ne_binaries() | Default.
...
value_or_values('undefined', Default) ->
    Default;
value_or_values(<<>>, Default) ->
    Default;
value_or_values([], Default) ->
    Default;
value_or_values(<<Value/binary>>, _Default) ->
    Value;
...
-spec get_ne_binary(config_category(), config_key(), Default, config_node()) ->
...
-spec get_ne_binaries(config_category(), config_key(), Default, config_node()) ->
...
get(Category, Keys, Default, Node) ->
    case get_category(Category) of
        ...
        {'error', 'not_found'} ->
            ...
            Default;
        {'error', Error} ->
            lager:debug("error ~p getting  category ~s(default) ~p: ~p", [
                Error, Category, Keys, Default
            ]),
            Default
    end.
...
-else.
update_category('undefined', _, _, _, _) ->
    'ok';
update_category(_, 'undefined', _, _, _) ->
    'ok';
update_category(_, _, 'undefined', _, _) ->
    'ok';
...
update_category(Category, Keys, Value, Node, Options) ->
    case kz_datamgr:open_cache_doc(?KZ_CONFIG_DB, Category) of
        {'ok', JObj} ->
            lager:debug("updating category ~s(~s).~s to ~p", [
                Category,
                Node,
                kz_binary:join(Keys),
                Value
        ...
        {'error', _Reason} = E ->
            lager:debug("failed to update category ~s: ~p", [
                Category, kz_datamgr:format_error(_Reason)
            ]),
            E
    end.

-spec update_category(
    config_category(), config_key(), any(), kz_term:ne_binary(), update_options(), kz_json:object()
) ->
    {'ok', kz_json:object()}.
update_category(Category, Keys, Value, Node, Options, JObj) ->
    ...
    NodePath = [Node | Keys],
    case
        kz_json:get_value(NodePath, JObj) =/= 'undefined' orelse
            props:is_true('node_specific', Options, 'false')
    of
        'true' ->
...
-spec update_category(
    config_category(), kz_json:object(), kz_datamgr:update_options(), kz_term:api_object()
) ->
    {'ok', kz_json:object()}
...
-spec maybe_save_category(
    kz_term:ne_binary(), kz_json:object(), kz_datamgr:update_options(), kz_term:api_object()
) ->
    {'ok', kz_json:object()}
    | kz_datamgr:data_error().
maybe_save_category(Category, JObj, Updates, PvtFields) ->
...
maybe_save_category(Category, JObj, Updates, PvtFields, Looped, _) ->
    lager:debug(
        "updating configuration category ~s(~s)",
    ...
    case kz_datamgr:update_doc(?KZ_CONFIG_DB, Category, WithPvtUpdates) of
        {'ok', SavedJObj} = Ok ->
            lager:info("saved cat ~s to db ~s (~s)", [
                Category, ?KZ_CONFIG_DB, kz_doc:revision(SavedJObj)
            ]),
...
-spec flush(
    kz_term:ne_binary(), kz_term:ne_binary() | kz_term:ne_binaries(), kz_term:api_ne_binary()
) -> 'ok'.
...
fetch_category(Category, _) when
    Category =:= <<"test_account_config">>;
...
%% @end
%%------------------------------------------------------------------------------
-type migrate_fun() :: fun(
    (kz_term:ne_binary(), kz_term:ne_binary(), config_key(), any()) -> any()
).
-type migrate_setting() ::
    {kz_term:ne_binary(), config_key()}
...
-define(CONFIG_MIGRATIONS, [
    ...
    {{<<"callflow">>, <<"sip_transport">>}, {<<"kazoo_endpoint">>, <<"sip_transport">>}},
    {
    ...
    {{<<"callflow.mobile">>, <<"codecs">>}, {<<"kazoo_endpoint.mobile">>, <<"codecs">>}},
    {
        {<<"callflow.mobile">>, <<"custom_sip_interface">>},
        {<<"kazoo_endpoint.mobile">>, <<"custom_sip_interface">>}
    },
    {{<<"callflow.mobile">>, <<"formatter">>}, {<<"kazoo_endpoint.mobile">>, <<"formatter">>}},
    {{<<"callflow.mobile">>, <<"prefix">>}, {<<"kazoo_endpoint.mobile">>, <<"prefix">>}},
    {{<<"callflow.mobile">>, <<"suffix">>}, {<<"kazoo_endpoint.mobile">>, <<"suffix">>}},
    {{<<"callflow.mobile">>, <<"realm">>}, {<<"kazoo_endpoint.mobile">>, <<"realm">>}},
    {{<<"callflow.mobile">>, <<"path">>}, {<<"kazoo_endpoint.mobile">>, <<"path">>}},
    {
    ...
    {{<<"number_manager">>, <<"aging_expiry_d">>}, {<<"tasks">>, <<"aging_expiry_d">>}},
    {{<<"number_manager">>, <<"discovery_expiry_d">>}, {<<"tasks">>, <<"discovery_expiry_d">>}},
    {{<<"number_manager">>, <<"aging_expiry_d">>}, {<<"tasks">>, <<"aging_expiry_d">>}},
    {{<<"number_manager">>, <<"crawler_timer_ms">>}, {<<"tasks">>, <<"crawler_timer_ms">>}},
    {
    ...
    {{<<"crossbar">>, <<"cleanup_timer">>}, {<<"tasks">>, <<"browse_dbs_interval_s">>}},
    {{<<"crossbar">>, <<"soft_delete_pause_ms">>}, {<<"tasks">>, <<"soft_delete_pause_ms">>}},
    {{<<"crossbar">>, <<"token_auth_expiry">>}, {<<"crossbar.auth">>, <<"token_auth_expiry_s">>}},
    {{<<"cb_modb">>, <<"maybe_archive_modbs">>}, {<<"tasks">>, <<"should_archive_modbs">>}},
    {
        {<<"cb_port_requests">>, <<"unfinished_port_request_lifetime_s">>},
        {<<"tasks">>, <<"unfinished_port_request_lifetime_s">>}
    },

    {{<<"callflow">>, <<"privacy_name">>}, {<<"privacy">>, <<"privacy_name">>}},
    {{<<"callflow">>, <<"privacy_number">>}, {<<"privacy">>, <<"privacy_number">>}},
    {
        {<<"stepswitch">>, <<"block_anonymous_caller_id">>},
        {<<"privacy">>, <<"block_anonymous_caller_id">>}
    },

    {{<<"fax">>, <<"conversion_command">>}, {<<"fax">>, <<"conversion_pdf_command">>}},
    {{<<"fax">>, <<"file_cache_path">>}, {<<"kazoo_convert">>, <<"file_cache_path">>}},

    {{<<"media">>, <<"tts_cache">>}, {<<"speech">>, <<"tts_cache">>}},
    {
    ...
    {{<<"sysconf">>, <<"acl_request_timeout_ms">>}, {<<"ecallmgr">>, <<"acl_request_timeout_ms">>}},
    {
        {<<"sysconf">>, <<"acl_request_timeout_fudge_ms">>},
        {<<"ecallmgr">>, <<"acl_request_timeout_fudge_ms">>}
    },

    {{<<"jonny5">>, <<"default_enabled">>}, {<<"limits">>, <<"enabled">>}},
    {{<<"jonny5">>, <<"default_calls">>}, {<<"limits">>, <<"calls">>}},
    {
        {<<"jonny5">>, <<"default_resource_consuming_calls">>},
        {<<"limits">>, <<"resource_consuming_calls">>}
    },
    {{<<"jonny5">>, <<"default_burst_trunks">>}, {<<"limits">>, <<"burst_trunks">>}},
    {{<<"jonny5">>, <<"default_inbound_trunks">>}, {<<"limits">>, <<"inbound_trunks">>}},
    {{<<"jonny5">>, <<"default_outbound_trunks">>}, {<<"limits">>, <<"outbound_trunks">>}},
    {{<<"jonny5">>, <<"default_twoway_trunks">>}, {<<"limits">>, <<"twoway_trunks">>}},
    {{<<"jonny5">>, <<"default_max_postpay_amount">>}, {<<"limits">>, <<"max_postpay_amount">>}},
    {{<<"jonny5">>, <<"default_reserve_amount">>}, {<<"limits">>, <<"reserve_amount">>}},
    {{<<"jonny5">>, <<"default_allow_prepay">>}, {<<"limits">>, <<"allow_prepay">>}},
    {{<<"jonny5">>, <<"default_allow_postpay">>}, {<<"limits">>, <<"allow_postpay">>}},
    {{<<"jonny5">>, <<"default_soft_limit_outbound">>}, {<<"limits">>, <<"soft_limit_outbound">>}},
    {{<<"jonny5">>, <<"default_soft_limit_inbound">>}, {<<"limits">>, <<"soft_limit_inbound">>}},
    {
        {<<"jonny5">>, <<"default_authz_resource_types">>},
        {<<"limits">>, <<"authz_resource_types">>}
    },
    {{<<"provisioner">>, <<"cluster_id">>}, {<<"cluster">>, <<"cluster_id">>}},
    {{<<"services">>, <<"master_account_bookkeeper">>}, fun
        (_FromId, _Node, _FromSetting, <<"kz_bookkeeper_braintree">>) ->
            {<<"services">>, <<"master_account_bookkeeper">>, <<"braintree">>};
    ...
    {{<<"services">>, <<"modules">>}, 'undefined'},
    {{<<"services">>, <<"support_billing_id">>}, 'undefined'}
]).
...
migrate_config_setting({From, To}) ->
    ...
    case remove_config_setting(From) of
        {'ok', _, []} -> 'ok';
        {'ok', JObj, Removed} -> migrate_config_setting(JObj, Removed, To);
        {'error', 'not_found'} -> 'ok';
...
-spec migrate_config_setting(kz_json:object(), migrate_values(), migrate_setting()) ->
    'ok'
    | {'error', any()}.
migrate_config_setting(UpdatedFrom, Removed, Fun) when
    is_function(Fun)
->
...
-spec migrate_config_setting_fun(
    migrate_fun(), migrate_values(), 'undefined' | migrate_setting(), migrate_values()
) ->
    {migrate_values(), migrate_setting()}.
migrate_config_setting_fun(_Fun, [], To, Removed) ->
    {Removed, To};
migrate_config_setting_fun(
    Fun, [{FromId, Node, FromSetting, Value} = Remove | Removals], _To, Removed
) ->
...
add_config_setting(Id, Setting, Values) when is_binary(Id) ->
    case kz_datamgr:open_doc(?KZ_CONFIG_DB, Id) of
        {'ok', JObj} ->
            add_config_setting(JObj, Setting, Values);
        {'error', 'not_found'} ->
            New = kz_doc:update_pvt_parameters(
...
add_config_setting(JObj, ToSetting, [{FromId, Node, FromSetting, Value} | Values]) ->
    ...
    case kz_json:get_value(Key, JObj) of
        ...
        Value ->
            add_config_setting(JObj, ToSetting, Values);
        _Else ->
            io:format(
                "the system tried to move the parameter listed below"
...
-spec remove_config_setting(
    [{kz_term:ne_binary(), kz_term:ne_binary(), config_key()}], kz_json:object(), migrate_values()
) ->
    {'ok', kz_json:object(), migrate_values()}.
...
remove_config_setting([{Id, Node, Setting} | Keys], JObj, Removed) ->
    ...
    case kz_json:get_value(Key, JObj) of
        'undefined' ->
            remove_config_setting(Keys, JObj, Removed);
        Value ->
    ...
    case kz_datamgr:open_doc(?KZ_CONFIG_DB, ToId) of
        ...
        {'error', 'not_found'} ->
            case kz_datamgr:open_doc(?KZ_CONFIG_DB, FromId) of
                ...
                {'ok', FromJObj} -> migrate_from_doc_to_doc(FromJObj, base_to_doc(ToId))
            end
    end.
...
migrate_from_doc_to_doc(FromJObj, ToJObj) ->
    ...
    {'ok', _ConfigDoc} = maybe_save_category(
        kz_doc:id(ToJObj),
        MigratedJObj,
...
-spec migrate_config_doc_node(kz_term:ne_binary(), kz_json:object(), kz_json:object()) ->
    kz_json:object().
migrate_config_doc_node(FromNodeBefore, FromConfig, ToJObj) ->
    FromNode = maybe_fix_nodename(FromNodeBefore),
    kz_json:foldl(
        fun(ConfigKey, ConfigValue, Acc) ->
...
-spec fetch_current(config_category(), config_key()) ->
    kz_json:json_term() | 'undefined' | fetch_ret().
fetch_current(Category, Key) ->
    fetch_current(Category, Key, 'undefined', node()).

-spec fetch_current(config_category(), config_key(), Default) ->
    kz_json:json_term() | Default | fetch_ret().
fetch_current(Category, Key, Default) ->
    fetch_current(Category, Key, Default, node()).

-spec fetch_current(config_category(), config_key(), Default, config_node()) ->
    kz_json:json_term() | Default | fetch_ret().
