...
-spec store_file(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary() | function()) ->
    'ok' | {'error', any()}.
store_file(Node, Filename, Url) ->
    store_file(Node, Filename, Url, #{}).

-spec store_file(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary() | function(), map()) ->
    'ok' | {'error', any()}.
...
-spec store_file(
    kz_term:ne_binary(), kz_term:ne_binary() | function(), pos_integer(), timeout(), map()
) ->
    'ok' | {'error', any()}.
store_file(Filename, Url, Tries, Timeout, #{media_server := Node} = Map) ->
    Msg =
        case kz_maps:get('alert_msg', Map) of
            'undefined' ->
    ...
    of
        {'ok', JObj} ->
            case kz_json:get_ne_binary_value(<<"Result">>, JObj) of
                ...
                <<"error">> ->
                    ...
                    retry_store_file(
                        Tries - 1, Timeout, API, Msg, Error, maybe_add_debug_data(JObj, Map)
                    );
                _Other ->
        ...
        {'returned', _JObj, Basic} ->
            ...
            retry_store_file(Tries - 1, Timeout, API, Msg, kz_term:to_binary(Error), Map#{
                basic_return => kz_json:to_proplist(Basic)
            });
        {'timeout', _JObj} ->
