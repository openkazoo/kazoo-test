...
-define(NOTIFY_TYPE_EXCEPTION(AccountId),
    kapps_account_config:get_global(
        AccountId, ?NOTIFY_CAT, <<"notify_persist_exceptions">>, ?DEFAULT_TYPE_EXCEPTION
    )
).

-define(DEFAULT_RETRY_PERIOD,
    kapps_config:get_integer(
        <<"tasks.notify_resend">>, <<"retry_after_fudge_s">>, 10 * ?SECONDS_IN_MINUTE
    )
).
...
%% @end
%%------------------------------------------------------------------------------
-spec call_collect(kz_term:api_terms(), kz_amqp_worker:publish_fun()) ->
...
cast(Req, PublishFun) ->
    ...
    Fun = fun() ->
        _ =
            case kz_term:is_ne_binary(CallId) of
...
%% @end
%%------------------------------------------------------------------------------
-spec handle_resp(kz_term:api_ne_binary(), kz_term:api_terms(), kz_amqp_worker:request_return()) ->
    'ok'.
...
%% @end
%%------------------------------------------------------------------------------
-spec check_for_failure(kz_term:api_ne_binary(), kz_term:api_terms(), {
    'ok' | 'returned' | 'timeout', kz_json:object() | kz_json:objects()
}) -> 'ok'.
...
maybe_log_metadata('undefined', _) ->
    'ok';
maybe_log_metadata(_, {<<"completed">>, 'undefined'}) ->
    'ok';
...
maybe_handle_error(NotifyType, Req, Reason) ->
    ...
    should_persist_notify(AccountId) andalso
        should_handle_notify_type(NotifyType, AccountId) andalso
        handle_error(NotifyType, Req, Reason).
...
handle_error(NotifyType, Req, {Reason, Metadata}) ->
    lager:warning("failed to publish notification ~s: ~p  , saving the payload...", [
        NotifyType, Reason
    ]),
    ...
    JObj = kz_doc:update_pvt_parameters(
        kz_json:from_list_recursive(Props), 'undefined', PvtOptions
    ),
...
save_pending_notification(NotifyType, JObj, Loop) ->
    case kz_datamgr:save_doc(?KZ_PENDING_NOTIFY_DB, JObj) of
        {'ok', _SavedJObj} ->
            lager:warning("payload for notification ~s is saved to ~s", [
                NotifyType, kz_doc:id(_SavedJObj)
            ]);
        {'error', 'timeout'} ->
...
collecting([JObj | _]) ->
    case
        kapi_notifications:notify_update_v(JObj) andalso
...
%% rendering problems
should_ignore_failure(<<"failed_template:", _/binary>>) -> 'true';
%% rendering problems
should_ignore_failure(<<"template_error:", _/binary>>) -> 'true';
%% no module is bound?
should_ignore_failure(<<"no teletype template modules responded">>) -> 'true';
should_ignore_failure(<<"unknown error throw-ed">>) -> 'true';
%% explicitly not ignoring these below:

%% maybe something went wrong with template, trying later?
should_ignore_failure(<<"unknown_template_error">>) -> 'false';
%% probably fax or voicemail is not stored in storage yet, retry later
should_ignore_failure(<<"no_attachment">>) -> 'false';
%% not ignoring it yet (voicemail_new)
should_ignore_failure(<<"badmatch">>) -> 'false';
...
handle_amqp_worker_error(Error) ->
    case kz_json:is_json_object(Error) of
        'true' ->
            json_to_reason(Error);
        'false' ->
            {
                <<"sending the amqp resulted in failure: ", (cast_to_binary(Error))/binary>>,
                'undefined'
            }
    end.
...
json_to_reason({ErrorType, JObjs}) when is_list(JObjs) ->
    ...
    case lists:foldl(Fun, maps:new(), Reasons) of
        ...
        #{<<"failed">> := [JObj | _]} ->
            FailureMsg = kz_json:get_ne_binary_value(
                <<"Failure-Message">>, JObj, <<"unknown_reason">>
            ),
            {
                <<"teletype failed with reason ", FailureMsg/binary>>,
                kz_json:get_value(<<"Metadata">>, JObj)
            };
        #{<<"pending">> := [JObj | _]} ->
        ...
        _ ->
            {
                <<"received ", (cast_to_binary(ErrorType))/binary,
                    " without any response from teletype">>,
                'undefined'
            }
    end;
...
notify_type(PublishFun) ->
    case catch erlang:fun_info_mfa(PublishFun) of
        {'kapi_notifications', Fun, 1} ->
            notify_type(cast_to_binary(Fun));
        _Other ->
...
should_persist_notify(AccountId) ->
    ?DEFAULT_PUBLISHER_ENABLED andalso
        ?ACCOUNT_SHOULD_PERSIST(AccountId).
...
should_handle_notify_type(NotifyType, AccountId) ->
    not lists:member(NotifyType, ?GLOBAL_FORCE_NOTIFY_TYPE_EXCEPTION) andalso
        not lists:member(NotifyType, ?NOTIFY_TYPE_EXCEPTION(AccountId)).
