...
-export([
    parallel_migrate_modbs/1,
    parallel_migrate_modbs/2
]).
-export([
    migrate_modbs_ranged/1,
    migrate_modbs_ranged/2,
    migrate_modbs_ranged/3
]).
-export([
    parallel_migrate_modbs_ranged/2,
...
binding('refresh') ->
    <<"maintenance.refresh">>;
binding('refresh_account') ->
    <<"maintenance.refresh.account">>;
binding('register_views') ->
    <<"maintenance.register_views">>;
...
migrate(Pause, Databases) ->
    Accounts = [
        kz_util:format_account_id(Db, 'encoded')
     || Db <- Databases,
...
parallel_migrate(_, [], Refs) ->
    wait_for_parallel_migrate(Refs);
...
parallel_migrate_worker(Ref, Pause, Databases, Parent) ->
    io:format(
        "~p parallel migrate worker ~p with ~p databases~n",
...
-spec split_parallel_migrate(integer(), kz_term:ne_binaries(), dict:dict(), integer()) ->
    kz_term:ne_binaries().
...
-spec parallel_migrate_modbs_ranged(text_or_integer(), text_or_integer(), text_or_integer()) ->
    'no_return'.
...
-spec parallel_migrate_modbs_ranged(
    text_or_integer(), text_or_integer(), text_or_integer(), text_or_integer()
) -> 'no_return'.
...
modb_in_range(Database, StartDate, EndDate) ->
    {_, MODbYear, MODbMonth} = kazoo_modb_util:split_account_mod(Database),
    modb_before_start(StartDate, MODbYear, MODbMonth) andalso
...
modb_before_start(StartDate, MODbYear, MODbMonth) ->
    {StartYear, StartMonth} =
        case kz_term:floor(math:log10(StartDate)) + 1 of
            4 -> {StartDate, 1};
            6 -> {kz_term:to_integer(StartDate / 100), StartDate rem 100}
        end,
    MODbYear >= StartYear andalso
        MODbMonth >= StartMonth.
...
modb_after_end(EndDate, MODbYear, MODbMonth) ->
    {EndYear, EndMonth} =
        case kz_term:floor(math:log10(EndDate)) + 1 of
            4 -> {EndDate, 12};
            6 -> {kz_term:to_integer(EndDate / 100), EndDate rem 100}
        end,
    MODbYear =< EndYear andalso
        MODbMonth =< EndMonth.
...
-spec refresh(kz_term:ne_binaries(), non_neg_integer(), non_neg_integer(), [string()]) ->
    'no_return'.
refresh([], _, _, []) ->
    'no_return';
...
-spec refresh(kz_term:ne_binary()) ->
    'ok'
    | boolean()
...
ensure_aggregates([], _Total) ->
    'ok';
ensure_aggregates([Account | Accounts], Total) ->
    io:format(
        "~p (~p/~p) aggregating database '~s'~n",
...
ensure_aggregate(Account) ->
    ensure_aggregate(
        Account,
        kz_datamgr:open_doc(
            kz_util:format_account_db(Account),
...
ensure_aggregate(Account, _) ->
    io:format(
        "ensuring necessary documents from account '~s' are in aggregate dbs~n",
        [Account]
    ),
...
ensure_aggregate_faxboxes([], _Total) ->
    'ok';
ensure_aggregate_faxboxes([Account | Accounts], Total) ->
    io:format(
        "~p (~p/~p) aggregating faxboxes account '~s'~n",
        [self(), length(Accounts) + 1, Total, Account]
    ),
    _ = ensure_aggregate_faxbox(Account),
...
update_or_add_to_faxes_db(Faxboxes) ->
    case kz_datamgr:all_docs(?KZ_FAXES_DB, [{'keys', [kz_doc:id(Doc) || Doc <- Faxboxes]}]) of
        {'ok', Docs} ->
            Revs = maps:from_list([
                {kz_json:get_value(<<"key">>, D), kz_json:get_value([<<"value">>, <<"rev">>], D)}
             || D <- Docs
            ]),
...
ensure_aggregate_accounts([], _Total) ->
    'ok';
ensure_aggregate_accounts([Account | Accounts], Total) ->
    io:format(
        "~p (~p/~p) aggregating accounts '~s'~n",
        [self(), length(Accounts) + 1, Total, Account]
    ),
    _ = ensure_aggregate_account(Account),
...
ensure_aggregate_account(Account) ->
    ...
    case kz_datamgr:open_doc(AccountDb, AccountId) of
        {'error', _} -> 'ok';
        {'ok', JObj} -> update_or_add_to_accounts_db(AccountId, JObj)
    end.
...
ensure_aggregate_devices([], _Total) ->
    'ok';
ensure_aggregate_devices([Account | Accounts], Total) ->
    io:format(
        "~p (~p/~p) aggregating devices account '~s'~n",
        [self(), length(Accounts) + 1, Total, Account]
    ),
    _ = ensure_aggregate_device(Account),
...
refresh_account_devices(AccountDb, AccountRealm, Devices) ->
    _ = [
        kapps_util:add_aggregate_device(AccountDb, kz_json:get_value(<<"doc">>, Device))
     || Device <- Devices,
...
should_aggregate_device(AccountRealm, Device) ->
    kzd_devices:sip_realm(Device, AccountRealm) =/= AccountRealm orelse
        kzd_devices:sip_ip(Device) =/= 'undefined'.
...
remove_aggregate_devices(AccountDb, AccountRealm, Devices) ->
    _ = [
        kapps_util:rm_aggregate_device(AccountDb, kz_json:get_value(<<"doc">>, Device))
     || Device <- Devices,
...
should_remove_aggregate(AccountRealm, Device) ->
    kzd_devices:sip_realm(Device, AccountRealm) =:= AccountRealm andalso
        kzd_devices:sip_ip(Device) =:= 'undefined'.
...
verify_aggregated_device(AccountDb, AccountId, JObj) ->
    case kz_datamgr:open_doc(AccountDb, AccountId) of
        {'error', 'not_found'} ->
            io:format(
                "    removing ~s doc ~s referencing missing db ~s~n",
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_log_doc_update(
    {'ok', kz_json:object() | kz_json:objects()} | kazoo_data:data_error(),
    kz_term:api_ne_binary(),
    kz_term:ne_binary()
) -> 'ok'.
maybe_log_doc_update({'ok', _}, 'undefined', _) -> 'ok';
maybe_log_doc_update({'ok', _}, Success, _) -> io:format("~s~n", [Success]);
maybe_log_doc_update({'error', _Reason}, _, Failed) -> io:format("~s: ~p~n", [Failed, _Reason]).
...
-type ensure_state() :: #{
    master := kz_term:ne_binary(),
    real_ids := gb_sets:set(),
    fixed_trees := map()
}.

-spec get_accounts_tree() ->
    {'ok', kz_json:objects()}
...
-spec get_accounts_tree(kz_term:ne_binaries()) ->
    {'ok', kz_json:objects()}
    | kz_datamgr:data_error().
get_accounts_tree(Accounts) ->
    ViewOptions = [
        {'keys', [kz_util:format_account_id(A) || A <- Accounts]},
        'include_docs'
    ],
...
%% actually ensuring
-spec actually_ensure_tree_accounts(
    kazoo_data:get_results_return(), boolean() | kz_term:ne_binary()
) -> 'ok'.
...
maybe_fix_accounts_tree(#{fixed_trees := Fixed} = State, 'false') ->
    ...
    _ = maps:fold(
        fun(Depth, AccountsMap, Curr) ->
            io:format(
                "--> (~b/~b) fixing pvt_tree for depth ~s~n",
...
fix_accounts_tree(AccountsMap) ->
    ...
    maps:fold(
        fun(AccountId, Tree, Curr) ->
            io:format(
                "         - (~b/~b) fixing pvt_tree for '~s'~n",
...
fix_account_tree(AccountId, Tree) ->
    ...
    maybe_log_doc_update(
        kzd_accounts:update(AccountId, Updates),
        'undefined',
...
fix_port_requests_tree(AccountId, Tree) ->
    ...
    case kz_datamgr:get_results(?KZ_PORT_REQUESTS_DB, ViewName, ViewOptions) of
        ...
        {'ok', JObjs} ->
            UpdatedJObjs = [
                kzd_port_requests:set_pvt_tree(JObj, Tree)
             || J <- JObjs,
                JObj <- [kz_json:get_value(<<"doc">>, J)],
                Tree =/= kzd_port_requests:pvt_tree(JObj)
            ],
            maybe_log_doc_update(
                kz_datamgr:save_docs(?KZ_PORT_REQUESTS_DB, UpdatedJObjs),
                <<"           --> fixed pvt_tree for",
...
-spec ensure_tree_is_tree_fold(ensure_state(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    ensure_state().
ensure_tree_is_tree_fold(
    #{fixed_trees := FixedTrees} = State,
    AccountId,
    Tree
) ->
...
-spec ensure_tree_is_tree_fold(
    ensure_state(), kz_term:ne_binary(), kz_term:ne_binaries(), kz_term:ne_binaries()
) ->
    kz_term:ne_binaries().
...
ensure_tree_is_tree_fold(
    #{master := MasterAccountId} = State, AccountId, [MasterAccountId | Tree], []
) ->
    ensure_tree_is_tree_fold(State, AccountId, Tree, [MasterAccountId]);
ensure_tree_is_tree_fold(
    #{master := MasterAccountId} = State, AccountId, [_BadMasterId | Tree], []
) ->
    ensure_tree_is_tree_fold(State, AccountId, Tree, [MasterAccountId]);
ensure_tree_is_tree_fold(
    #{
    ...
    case
        Id =/= MasterAccountId andalso
            gb_sets:is_member(Id, RealIds)
    of
        'true' ->
...
import_account(_AccountId, _ParentId, {'error', _Reason1}, {'error', _Reason2}) ->
    io:format(
        "can not open account '~s' (~p) and parent '~s' (~p)~n",
...
import_account(AccountId, ParentId, {'ok', AccountJObj}, {'ok', ParentJObj}) ->
    ...
    Setters = [
        {fun kzd_accounts:set_tree/2, AccountTree},
    ...
    case kz_datamgr:save_doc(kz_util:format_account_db(AccountId), NewAccountJObj) of
        {'ok', SavedJObj} ->
            io:format(
                "account saved, updating services and import account's numbers to number dbs~n"
            ),
            ...
            'ok';
        {'error', _Reason} ->
            io:format("failed to update account '~s' definition in accountdb: ~p~n", [
                AccountId, _Reason
            ])
    end.
...
ensure_reseller_id_accounts([], _) ->
    'ok';
ensure_reseller_id_accounts([Account | Accounts], Total) ->
    io:format(
        "(~p/~p) ensuring reseller id for account '~s'~n",
...
-spec extract_messages(kz_json:objects() | kz_json:object(), kz_term:ne_binaries()) ->
    kz_term:ne_binaries().
extract_messages([], CurMessages) ->
    CurMessages;
...
migrate_limits(Account) ->
    ...
    AccountDb =
        case kz_datamgr:db_exists(Account) of
            'true' -> Account;
...
clean_trunkstore_docs(AccountDb, [JObj | JObjs], Trunks, InboundTrunks) ->
    ...
    _ =
        case
            kz_json:get_ne_value(<<"servers">>, Doc) =:= 'undefined' andalso
                kz_json:get_ne_value(<<"pvt_created_by">>, Doc) =:= <<"jonny5">>
        of
            'true' -> kz_datamgr:save_doc(AccountDb, kz_doc:set_soft_deleted(Doc, 'true'));
            'false' -> 'ok'
        end,
    NewTrunks =
        case kz_json:get_integer_value([<<"account">>, <<"trunks">>], Doc, 0) of
...
migrate_media(Account) ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"media/listing_by_name">>, []) of
        {'ok', []} ->
            io:format("no public media files in db ~s~n", [AccountDb]);
        {'ok', [_ | _] = JObjs1} ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"media/listing_private_media">>, []) of
        {'ok', []} ->
            io:format("no private media files in db ~s~n", [AccountDb]);
        {'ok', JObjs2} ->
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_migrate_attachments(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    any().
maybe_migrate_attachments(AccountDb, Id, JObj) ->
    case kz_doc:attachments(JObj) of
        ...
        Attachments ->
            _ = [
                catch migrate_attachment(AccountDb, JObj, K, V)
...
migrate_attachment(AccountDb, ViewJObj) ->
    ...
    _ =
        case kz_datamgr:open_doc(AccountDb, Id) of
            {'error', _} = E1 ->
                io:format("unable to open media for attachment migration ~s/~s: ~p~n", [
                    AccountDb, Id, E1
                ]);
            {'ok', JObj1} ->
    ...
    case kz_datamgr:open_doc(AccountDb, Id) of
        {'error', _} = E2 ->
            io:format("unable to open media for depreciated field removal ~s/~s: ~p~n", [
                AccountDb, Id, E2
            ]);
        {'ok', JObj2} ->
...
remove_deprecated_attachment_properties(AccountDb, Id, JObj) ->
    ...
    Result =
        case
            (J =/= JObj) andalso
                kz_json:get_value(<<"source_id">>, J)
        of
            'false' ->
                'ignore';
            'undefined' ->
                kz_datamgr:save_doc(
                    AccountDb, kz_json:set_value(<<"media_source">>, <<"upload">>, J)
                );
            _Else ->
                kz_datamgr:save_doc(
                    AccountDb, kz_json:set_value(<<"media_source">>, <<"recording">>, J)
                )
        end,
    case Result of
        'ignore' ->
        ...
        {'error', _} = E3 ->
            io:format("removal of depreciated properties from ~s/~s failed: ~p~n", [
                AccountDb, Id, E3
            ])
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec migrate_attachment(
    kz_term:ne_binary(), kz_json:object(), kz_term:ne_binary(), kz_json:object()
) -> 'ok'.
migrate_attachment(AccountDb, JObj, Attachment, MetaData) ->
    ...
    Migrations = [
        fun({A, MCT}) -> maybe_update_attachment_content_type(A, MCT, DocCT) end,
...
-spec maybe_add_extension({kz_term:ne_binary(), kz_term:ne_binary()}) ->
...
-spec maybe_update_attachment(
    kz_term:ne_binary(), kz_term:ne_binary(), attachment_and_content(), attachment_and_content()
) -> 'ok'.
...
maybe_update_attachment(AccountDb, Id, {OrigAttach, _CT1}, {NewAttach, CT}) ->
    ...
    Updaters = [
        fun(_) -> try_load_attachment(AccountDb, Id, OrigAttach) end,
        fun(Content1) ->
...
maybe_resave_attachment(Content1, AccountDb, Id, OrigAttach, NewAttach, CT) ->
    ...
    case
        kz_doc:attachment_length(JObj, OrigAttach) =:=
            kz_doc:attachment_length(JObj, NewAttach)
    of
        'false' ->
            io:format("unable to put new attachment ~s/~s/~s: ~p~n", [
                AccountDb, Id, NewAttach, Result
            ]),
        ...
        'true' ->
            ...
            case file:write_file(Filename, Content1) of
                'ok' ->
                    'ok';
                {'error', _R} = E2 ->
                    io:format("unable to backup attachment ~s/~s/~s: ~p~n", [
                        AccountDb, Id, NewAttach, _R
                    ]),
                    throw(E2)
            end
    end.

-spec maybe_cleanup_old_attachment(
...
maybe_cleanup_old_attachment(AccountDb, Id, OrigAttach, NewAttach) ->
    case kz_datamgr:delete_attachment(AccountDb, Id, OrigAttach) of
        {'ok', _} ->
            ...
            'ok';
        {'error', _R} = E ->
            io:format("unable to remove original attachment ~s/~s/~s: ~p~n", [
                AccountDb, Id, OrigAttach, _R
            ]),
            throw(E)
    end.
...
-spec purge_doc_type(kz_term:ne_binaries() | kz_term:ne_binary(), kz_term:ne_binary()) ->
    ...
    | {'error', _}
    | 'ok'.
purge_doc_type(Type, Account) when
    is_binary(Type);
    is_binary(Account)
->
    purge_doc_type(
        ...
        kapps_config:get_integer(
            ?SYSCONFIG_COUCH, <<"default_chunk_size">>, ?MILLISECONDS_IN_SECOND
        )
    );
purge_doc_type([], _Account) ->
    'ok';
...
migrate_system(PreviousVersion, ThisVersion) ->
    ...
    Result = lists:foldl(
        fun
            (Fun, Acc) when is_function(Fun, 2) ->
    ...
    case lists:all(fun kz_term:is_true/1, Result) of
        'true' ->
            _ = set_last_migrate_version(ThisVersion),
            'ok';
        'false' ->
            'ok'
    end.
...
cleanup_invalid_notify_docs() ->
    ...
    case kz_datamgr:all_docs(?KZ_CONFIG_DB, ['include_docs']) of
        ...
        {'error', _R} -> lager:warning("unable to fetch all system config docs: ~p", [_R])
    end.
...
-spec maybe_remove_invalid_notify_doc(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    'ok'.
maybe_remove_invalid_notify_doc(<<"notification">>, <<"notification", _/binary>>, _) ->
    'ok';
maybe_remove_invalid_notify_doc(<<"notification">>, _, JObj) ->
    _ = kz_datamgr:del_doc(?KZ_CONFIG_DB, JObj),
    'ok';
maybe_remove_invalid_notify_doc(_Type, _Id, _Doc) ->
    'ok'.
...
remove_system_media_refs(HangupCause, Config) ->
    case kz_json:is_json_object(Config) of
        ...
        'true' -> {HangupCause, kz_json:foldl(fun remove_system_media_ref/3, kz_json:new(), Config)}
    end.

-spec remove_system_media_ref(kz_json:path(), kz_json:json_term(), kz_json:object()) ->
    kz_json:object().
remove_system_media_ref(Key, <<"/system_media/", Value/binary>>, Acc) ->
    kz_json:set_value(Key, Value, Acc);
remove_system_media_ref(Key, Value, Acc) ->
    kz_json:set_value(Key, Value, Acc).
...
-spec deprecate_timezone_for_node(
    kz_json:key(), kz_json:object(), kz_term:api_ne_binary(), kz_term:api_ne_binary()
) ->
    kz_json:object().
...
check_system_configs() ->
    ...
    _ = [
        log_system_config_errors(Config, Errors)
...
register_system_dbs_views() ->
    Views = [
        kapps_util:get_view_json(?APP, ?ACCOUNTS_AGG_VIEW_FILE),
...
wait_for_check(Pid, Ref, StartTimeMs) ->
    receive
        ...
        {'DOWN', Ref, 'process', Pid, Reason} ->
            lager:error("check in ~p failed to run after ~pms: ~p", [
                Pid, kz_time:elapsed_ms(StartTimeMs), Reason
            ]),
            throw(Reason)
...
maybe_log_telemetry_warning(Days) when
    is_integer(Days) andalso
        Days > 0
->
...
print_header() ->
    ...
    io:format(
        "       - https://derflounder.wordpress.com/2019/07/23/suppressing-microsoft-autoupdates-required-data-notice-screen/~n"
    ),
    'no_return'.
