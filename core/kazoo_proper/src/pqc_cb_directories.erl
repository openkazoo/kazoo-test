...
-spec create(pqc_cb_api:state(), kz_term:ne_binary(), kzd_directories:doc()) ->
    pqc_cb_api:response().
...
-spec fetch(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:api_proplist()) ->
    pqc_cb_api:response().
...
seq(API) ->
    ...
    FetchedUsers = kzd_directories:users(
        kz_json:get_json_value(<<"data">>, kz_json:decode(FetchResp))
    ),
    ...
    'true' = lists:all(
        fun(FetchedUser) ->
            lists:member(
                kz_json:get_ne_binary_value(<<"user_id">>, FetchedUser),
                UserIds
            )
        end,
        FetchedUsers
...
-spec create_users(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    [kzd_users:doc()].
create_users(API, AccountId, DirectoryId) ->
    [create_user(API, AccountId, DirectoryId, N) || N <- lists:seq(1, 100)].

-spec create_user(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary(), 1..100) ->
    kzd_users:doc().
