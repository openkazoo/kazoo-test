...
-spec new_message(
    pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object(), binary()
) ->
    pqc_cb_api:response().
new_message(API, AccountId, BoxId, MessageJObj, MessageBin) ->
    ...
    Body = kz_http_util:encode_multipart(
        [
            {kz_json:encode(pqc_cb_api:create_envelope(MessageJObj)), [
                {<<"content-type">>, <<"application/json">>}
            ]},
            {MessageBin, [{<<"content-type">>, <<"audio/mp3">>}]}
        ],
        Boundary
    ),

    RequestHeaders = pqc_cb_api:request_headers(
        API,
        [
...
-spec list_messages(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
list_messages(API, AccountId, BoxId) ->
    list_messages(API, AccountId, BoxId, []).

-spec list_messages(
    pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:proplist()
) -> pqc_cb_api:response().
...
-spec create_box(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
...
-spec delete_box(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
...
messages_url(AccountId, BoxId, QuerystringParams) ->
    [
        string:join(
            [pqc_cb_accounts:account_url(AccountId), "vmboxes", kz_term:to_list(BoxId), "messages"],
            "/"
        ),
        "?",
...
seq_kcro_24() ->
    ...
    AllNewOnePageResp = list_messages(
        API,
        AccountId,
        BoxId,
        [{<<"filter_not_folder">>, <<"saved">>}]
    ...
    NoPagingFilterResp = list_messages(API, AccountId, BoxId, [
        {<<"paginate">>, 'false'},
        {<<"filter_not_folder">>, <<"saved">>}
    ]),
...
remove_expected_messages(CDRIds, []) ->
    CDRIds;
...
create_messages(API, AccountId, BoxId, {{Year, Month, Day}, Folders}) ->
    ...
    {_StartTime, Messages} = lists:foldl(
        fun create_voicemail/2,
        {StartTime, []},
        Folders
    ),
    lists:foreach(
