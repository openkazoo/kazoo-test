...
-define(NUMBERS,
    (maps:fold(
        fun(_, #{<<"numbers">> := Nums}, Acc) -> Acc ++ Nums end,
        [],
        ?ACCOUNTS_SETTINGS
    ))
).
...
seq() ->
    _ = init_system(),
    'ok' = eunit:test(
        [{'setup', fun initial_state/0, fun cleanup/1, fun all_port_seq/1}],
        [verbose]
    ).
...
initial_state() ->
    ...
    try
        lists:foldl(
            fun(Name, Acc) -> initialize_account(API, Name, Acc) end,
            State,
...
initialize_account(API, AccountName, StateAcc) ->
    ...
    AccountApi = pqc_cb_api:authenticate(
    ...
    IsAuthority = maps:get(
        <<"is_port_authority">>, maps:get(AccountName, ?ACCOUNTS_SETTINGS), false
    ),
    ...
    StateAcc#{
        AccountName => #{
            model => pqc_kazoo_model:new(AccountApi),
...
-spec create_admin_user(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
create_admin_user(API, AccountId, AccountName) ->
    Email = <<"admin@", AccountName/binary, ".com">>,
    Setters = [
        {fun kzd_users:set_first_name/2, <<AccountName/binary, "-admin">>},
...
create_whitelabel(API, AccountId, AccountName, 'true') ->
    Setters = [
        {fun kzd_whitelabel:set_port_authority/2, AccountId},
...
cleanup_port_requests_db() ->
    ...
    case
        kz_datamgr:get_results(
            ?KZ_PORT_REQUESTS_DB, <<"port_requests/listing_by_number">>, ViewOptions
        )
    of
        {'ok', []} ->
            ok;
        {'ok', Ports} ->
            ...
            _ = [
                ?debugFmt("unabled to delete ~s: ~p", [
                    kz_doc:id(Res), kz_json:get_value(<<"error">>, Res)
                ])
             || Res <- Result,
    ...
    [
        {
            ...
            ?IF(
                kz_term:is_ne_binary(Created),
                create_ports_seq(State, AccountName, Created),
                ?_assert(false)
            )
        }
    ].
...
create_ports_seq(State, AccountName, Resp) ->
    ...
    [
        {"port id '" ++ kz_term:to_list(PortId) ++ "'", ?_assert(kz_term:is_ne_binary(PortId))},
        {"checking state",
            ...
            ?_assertEqual(AuthorityId, kz_json:get_ne_binary_value(<<"port_authority">>, ReadOnly))},
        {
            ...
            ?_assertEqual(
                AuthorityName, kz_json:get_ne_binary_value(<<"port_authority_name">>, ReadOnly)
            )
        }
    ].
        ...
        [
            {
                ...
                ?_assert(
                    kz_term:is_not_empty(
                        kz_json:get_list_value(<<"port_requests">>, hd(AccountList), [])
                    )
                )
            }
            | [
            ...
            [
                {
                    ...
                    ?_assertEqual(
                        AuthorityId,
                        kz_json:get_value([<<"_read_only">>, <<"port_authority">>], Port)
                    )
                },
                {
                    "authority name '" ++ kz_term:to_list(AuthorityName) ++ "'",
                    ?_assertEqual(
                        AuthorityName,
                        kz_json:get_value([<<"_read_only">>, <<"port_authority_name">>], Port)
                    )
                }
            ]
         || AccountPorts <- JObj,
...
port_account_seq(#{master := #{model := Model}}) ->
    API = pqc_kazoo_model:api(Model),
    [{"get master's account ports", ?_assertEqual(true, have_ports(list_account_ports(API)))}].
...
have_ports(Resp) ->
    ...
    kz_json:get_ne_binary_value(<<"status">>, JObj, <<"error">>) =/= <<"error">> andalso
        is_list(kz_json:get_list_value(<<"data">>, JObj)).
...
seed_ports(AccountName) ->
    ...
    Numbers = kz_json:from_list([
        {Num, kz_json:new()}
     || Num <- maps:get(<<"numbers">>, maps:get(AccountName, ?ACCOUNTS_SETTINGS))
    ]),

    Setters = [
        {fun kzd_port_requests:set_bill_account_number/2, <<"Seed">>},
