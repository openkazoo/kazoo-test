...
upload_csv(API, CSV, _RatedeckId) ->
    ...
    TaskId = kz_json:get_ne_binary_value(
        [<<"data">>, <<"_read_only">>, <<"id">>],
...
-spec assign_service_plan(
    pqc_cb_api:state(), kz_term:ne_binary() | proper_types:type(), kz_term:ne_binary()
) ->
    pqc_cb_api:response().
...
rate_account_did(API, AccountId, DID) ->
    ...
    URL = string:join(
        [pqc_cb_accounts:account_url(AccountId), "rates", "number", kz_term:to_list(DID)], "/"
    ),
...
ratedeck_service_plan(<<_/binary>> = RatedeckId) ->
    Plan = kz_json:from_list([{<<"ratedeck">>, kz_json:from_list([{RatedeckId, kz_json:new()}])}]),
    Funs = [{fun kzd_service_plan:set_plan/2, Plan}],

    lists:foldl(
        fun({F, V}, Acc) -> F(Acc, V) end,
        kz_json:from_list([
            {<<"_id">>, service_plan_id(RatedeckId)},
    ...
    of
        <<"success">> ->
            ...
            get_csvs(
                API,
                TaskId,
                kz_json:get_list_value([<<"data">>, <<"_read_only">>, <<"csvs">>], GetJObj, [])
            ),
...
-spec delete_rate(pqc_cb_api:state(), kz_term:ne_binary() | kzd_rates:doc()) ->
    pqc_cb_api:response().
...
-spec delete_rate(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
...
-spec rate_did(pqc_cb_api:state(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    kz_term:api_integer().
...
cleanup(API) ->
    ...
    _DelSPs = [
        pqc_cb_services:delete_service_plan(API, service_plan_id(RatedeckId))
...
seq() ->
    ...
    RateCost = kz_currency:units_to_dollars(
        kapps_call_util:base_call_cost(
            kzd_rates:rate_cost(RateDoc),
    ...
    _ =
        case is_binary(AccountId) of
            'true' ->
                ?INFO("created account ~s~n", [AccountId]);
            'false' ->
    ...
    _ =
        case
            kz_json:get_value(
                [<<"data">>, ServicePlanId, <<"vendor_id">>],
                kz_json:decode(_Assigned)
            )
        of
            'undefined' ->
                ?ERROR("failed to assign service plan for '~s' to account ~s", [
                    RatedeckId, AccountId
                ]),
                lager:error("failed to assign service plan for ~s to account ~s: ~s", [
                    RatedeckId, AccountId, _Assigned
                ]),
                throw('no_plan');
            _MasterAccountId ->
...
command(Model) ->
    ...
    oneof([
        {'call', ?MODULE, 'upload_rate', [API, RateDoc]},
...
postcondition1(
    ...
    case pqc_kazoo_model:does_rate_exist(Model, RatedeckId, RateDoc) of
        'true' ->
            ...
            <<"success">> =:= kz_json:get_ne_binary_value(<<"status">>, Resp) andalso
                kz_json:is_true([<<"data">>, <<"_read_only">>, <<"deleted">>], Resp);
        'false' ->
...
postcondition1(
    ...
    case pqc_kazoo_model:does_ratedeck_exist(Model, RatedeckId) of
        'true' ->
            ?INFO(
                "ratedeck ~s exists, creating service plan should succeed: ~p",
                [RatedeckId, APIResult]
            ),
            'ok' =:= APIResult;
        'false' ->
            ?INFO(
                "ratedeck ~s does not exist, creating service plan should fail: ~p",
                [RatedeckId, APIResult]
            ),
...
postcondition1(
    ...
    case pqc_kazoo_model:does_service_plan_exist(Model, PlanId) of
        'true' ->
            ?INFO("model has service plan ~s, is assigned to account ~s: ~s", [
                PlanId, _AccountId, APIResult
            ]),
            'undefined' =/=
                kz_json:get_value(
                    [<<"data">>, <<"plan">>, <<"ratedeck">>, RatedeckId],
                    kz_json:decode(APIResult)
                );
        'false' ->
            ?INFO("model does not have service plan ~s, API should not have it listed: ~s", [
                PlanId, APIResult
            ]),
            'undefined' =:=
                kz_json:get_value(
...
matches_service_plan_cost(Model, AccountId, DID, APIResult) ->
    case pqc_kazoo_model:has_service_plan_rate_matching(Model, AccountId, DID) of
        {'true', Cost} when is_number(APIResult) ->
            ?INFO(
                "model rates ~s against account ~s as ~p, got ~p in API",
                [DID, AccountId, Cost, kz_currency:dollars_to_units(APIResult)]
            ),
            Cost =:= kz_currency:dollars_to_units(APIResult);
        {'true', _Cost} ->
            ?INFO(
                "model rates ~s against account ~s as ~p, but got ~p in API",
                [DID, AccountId, _Cost, APIResult]
            ),
            'false';
        'false' ->
            ?INFO(
                "model has no rate for ~s against ~s, got ~p from API",
...
matches_cost(Model, RatedeckId, DID, APIResult) ->
    case pqc_kazoo_model:has_rate_matching(Model, RatedeckId, DID) of
        {'true', Cost} when is_number(APIResult) ->
            ?INFO(
                "model rates ~s as ~p, got ~p in API",
                [DID, Cost, kz_currency:dollars_to_units(APIResult)]
            ),
            Cost =:= kz_currency:dollars_to_units(APIResult);
        {'true', _Cost} ->
            ?INFO(
                "model rates ~s as ~p, but got ~p in API",
                [DID, _Cost, APIResult]
            ),
            'false';
        'false' ->
            ?INFO(
                "model has no rate for ~s, got ~p from API",
                [DID, APIResult]
            ),
            'undefined' =:= APIResult
    end.
