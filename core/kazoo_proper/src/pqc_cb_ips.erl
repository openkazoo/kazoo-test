...
-record('dedicated', {
    ip :: kz_term:api_ne_binary(),
    host :: kz_term:api_ne_binary(),
    zone :: kz_term:api_ne_binary()
}).
-define(DEDICATED(IP, Host, Zone), #dedicated{ip = IP, host = Host, zone = Zone}).
...
next_state(
    Model, _APIResp, {'call', ?MODULE, 'remove_ip', [_API, AccountId, ?DEDICATED(IP, _, _)]}
) ->
    pqc_util:transition_if(
        Model,
        [
...
next_state(Model, _APIResp, {'call', ?MODULE, 'fetch_ip', [_API, _AccountId, _Dedicated]}) ->
    Model;
next_state(
    Model, _APIResp, {'call', ?MODULE, 'assign_ip', [_API, AccountId, ?DEDICATED(IP, _, _)]}
) ->
    pqc_util:transition_if(
        Model,
        [
...
postcondition(Model, {'call', ?MODULE, 'list_ips', [_API]}, {'error', 'not_found'}) ->
    [] =:= pqc_kazoo_model:dedicated_ips(Model);
postcondition(
    Model, {'call', ?MODULE, 'assign_ips', [_API, AccountId, Dedicateds]}, {'ok', ListedIPs}
) ->
    lists:all(
        fun({IP, IPInfo}) ->
...
postcondition(
    _Model, {'call', ?MODULE, 'assign_ips', [_API, _AccountId, _Dedicateds]}, {'error', 'not_found'}
) ->
    'true';
postcondition(
    Model,
    ...
    pqc_kazoo_model:is_ip_assigned(Model, AccountId, IP) andalso
        IP =:= kz_json:get_ne_binary_value(<<"ip">>, RemovedIP) andalso
        Host =:= kz_json:get_ne_binary_value(<<"host">>, RemovedIP) andalso
        Zone =:= kz_json:get_ne_binary_value(<<"zone">>, RemovedIP) andalso
        'true' =:= kz_json:is_true([<<"_read_only">>, <<"deleted">>], RemovedIP);
postcondition(
    Model,
...
postcondition(
    Model,
    {'call', ?MODULE, 'fetch_ip', [_API, AccountId, ?DEDICATED(IP, _Host, _Zone) = Dedicated]},
    {'ok', FetchedIP}
) ->
    pqc_kazoo_model:is_ip_assigned(Model, AccountId, IP) andalso
        is_assigned(AccountId, Dedicated, FetchedIP);
postcondition(
    Model,
    {'call', ?MODULE, 'fetch_ip', [_API, AccountId, ?DEDICATED(IP, _Host, _Zone)]},
    {'error', 'not_found'}
) ->
    not pqc_kazoo_model:is_ip_assigned(Model, AccountId, IP);
postcondition(
    Model,
    {'call', ?MODULE, 'assign_ip', [_API, AccountId, ?DEDICATED(_, _, _) = Dedicated]},
    {'ok', AssignedIP}
) ->
    lists:all(
        fun({IP, IPInfo}) ->
        ...
        all_requested_are_listed(AccountId, [Dedicated], [AssignedIP]);
postcondition(
    _Model, {'call', ?MODULE, 'assign_ip', [_API, _AccountId, _Dedicated]}, {'error', 'not_found'}
) ->
    'true';
...
postcondition(
    _Model, {'call', ?MODULE, 'fetch_assigned', [_API, 'undefined']}, {'error', 'not_found'}
) ->
    'true';
...
postcondition(
    Model, {'call', ?MODULE, 'fetch_assigned', [_API, AccountId]}, {'error', 'not_found'}
) ->
    [] =:= pqc_kazoo_model:account_ips(Model, AccountId);
postcondition(
    Model, {'call', ?MODULE, 'create_ip', [_API, ?DEDICATED(IP, _, _)]}, {'ok', _CreatedIP}
) ->
    'undefined' =:= pqc_kazoo_model:dedicated_ip(Model, IP);
postcondition(
    Model, {'call', ?MODULE, 'create_ip', [_API, ?DEDICATED(IP, _, _)]}, {'error', 'conflict'}
) ->
    'undefined' =/= pqc_kazoo_model:dedicated_ip(Model, IP);
postcondition(
    Model, {'call', ?MODULE, 'delete_ip', [_API, ?DEDICATED(IP, _, _)]}, {'ok', _Deleted}
) ->
    'undefined' =/= pqc_kazoo_model:dedicated_ip(Model, IP);
postcondition(
    Model, {'call', ?MODULE, 'delete_ip', [_API, ?DEDICATED(IP, _, _)]}, {'error', 'not_found'}
) ->
    'undefined' =:= pqc_kazoo_model:dedicated_ip(Model, IP).
    ...
    ?FORALL(
        ...
        ?TRAPEXIT(
            begin
                ...
                ?WHENFAIL(
                    io:format("S: ~p~nP: ~p~n", [Sequential, Parallel]),
                    aggregate(command_names(Cmds), Result =:= 'ok')
                )
            end
        )
...
do_dedicated_ips_exist(Model, Dedicateds) ->
    lists:all(
        fun(?DEDICATED(IP, _, _)) -> pqc_kazoo_model:does_ip_exist(Model, IP) end,
        Dedicateds
    ).
...
are_dedicated_ips_unassigned(Model, Dedicateds) ->
    lists:all(
        fun(?DEDICATED(IP, _, _)) -> pqc_kazoo_model:is_ip_unassigned(Model, IP) end,
        Dedicateds
    ).
...
-spec are_all_ips_listed(
    [{kz_term:ne_binary(), pqc_kazoo_model:dedicated_ip()}], kz_json:objects(), boolean()
) ->
    boolean().
are_all_ips_listed([], [], _CheckHost) ->
    'true';
are_all_ips_listed(_ModelIPs, [], _CheckHost) ->
    'false';
are_all_ips_listed([], _ListedIPs, _CheckHost) ->
    'false';
...
is_ip_listed(IP, IPInfo, ListedIPs, CheckHost) ->
    ...
    lists:any(
        fun(ListedIP) ->
            IP =:= kz_json:get_ne_binary_value(<<"ip">>, ListedIP) andalso
                Zone =:= kz_json:get_ne_binary_value(<<"zone">>, ListedIP) andalso
                (CheckHost =:= 'false' orelse
...
is_assigned(AccountId, ?DEDICATED(DIP, DHost, DZone), ListedIP) ->
    ...
    AccountId =:= AssignedTo andalso
        <<"assigned">> =:= Status andalso
        IP =:= DIP andalso
        Host =:= DHost andalso
        Zone =:= DZone.
