...
-spec list_number(pqc_cb_api:state(), kz_term:api_ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
list_number(_API, 'undefined', _Number) ->
    ?FAILED_RESPONSE;
...
add_number(API, AccountId, Number) -> add_number(API, AccountId, Number, kz_json:new()).

-spec add_number(
    pqc_cb_api:state(), kz_term:api_ne_binary(), kz_term:ne_binary(), kz_json:object()
) ->
    pqc_cb_api:response().
add_number(API, AccountId, Number, RequestData) ->
    URL = number_url(AccountId, Number),
    RequestHeaders = pqc_cb_api:request_headers(API),
    RequestEnvelope = pqc_cb_api:create_envelope(
        RequestData,
...
-spec remove_number(pqc_cb_api:state(), kz_term:api_ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
remove_number(_API, 'undefined', _Number) ->
    ?FAILED_RESPONSE;
...
-spec activate_number(pqc_cb_api:state(), kz_term:api_ne_binary(), kz_term:ne_binary()) ->
    pqc_cb_api:response().
activate_number(_API, 'undefined', _Number) ->
    ?FAILED_RESPONSE;
activate_number(API, AccountId, Number) ->
    URL = number_url(AccountId, Number, "activate"),
    RequestHeaders = pqc_cb_api:request_headers(API),
    RequestEnvelope = pqc_cb_api:create_envelope(
        kz_json:new(), kz_json:from_list([{<<"accept_charges">>, 'true'}])
    ),
    pqc_cb_api:make_request(
        [201, 404, 500],
        fun kz_http:put/3,
...
number_url(AccountId, Number) ->
    string:join(
        [
            ...
            "phone_numbers",
            kz_term:to_list(kz_http_util:urlencode(Number))
        ],
        "/"
    ).
...
numbers_url(AccountId) ->
    string:join(
        [pqc_cb_accounts:account_url(AccountId), "phone_numbers"],
        "/"
    ).
    ...
    ?FORALL(
        ...
        ?TRAPEXIT(
            begin
                ...
                ?WHENFAIL(
                    io:format(
                        "Final Model : ~p~nFailing Cmds: ~p~n",
...
command(Model, 'true') ->
    ...
    oneof([
        {'call', ?MODULE, 'list_number', [API, AccountId, phone_number()]},
        ...
        'true' ->
            case pqc_kazoo_model:number_data(Model, Number) of
                #{
                    ...
                    409 =:= pqc_cb_response:error_code(APIResult) andalso
                        <<"number_exists">> =:= pqc_cb_response:message(APIResult);
                #{} ->
...
postcondition(
    ...
    case pqc_kazoo_model:does_account_exist(Model, AccountId) of
        ...
        'true' ->
            case pqc_kazoo_model:number_data(Model, Number) of
                #{'account_id' := AccountId} ->
                    <<"success">> =:= pqc_cb_response:status(APIResult) andalso
                        'deleted' =:= pqc_cb_response:number_state(APIResult);
                #{} ->
                    404 =:= pqc_cb_response:error_code(APIResult);
                'undefined' ->
                    404 =:= pqc_cb_response:error_code(APIResult)
        ...
        'true' ->
            case pqc_kazoo_model:number_data(Model, Number) of
                #{
                    ...
                    'reserved' =:= NumberState orelse
                        'in_service' =:= NumberState
                ->
...
seq_kzoo_41() ->
    ...
    'true' = kz_json:is_empty(
        kz_json:get_json_value([<<"data">>, <<"numbers">>], kz_json:decode(EmptySummaryResp))
    ),

    PhoneNumber = hd(?PHONE_NUMBERS),

    CreateResp = add_number(
        API, AccountId, PhoneNumber, kz_json:from_list([{<<"carrier_name">>, <<"knm_other">>}])
    ),
    ...
    SummaryJObj = kz_json:get_json_value(
        [<<"data">>, <<"numbers">>, PhoneNumber], kz_json:decode(SummaryResp)
    ),
