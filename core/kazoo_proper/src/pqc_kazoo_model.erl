...
-export([
    ...
    has_rate_matching/3,
    has_service_plan_rate_matching/3,
    does_ip_exist/2,
    is_ip_missing/2,
    is_ip_unassigned/2,
    is_ip_assigned/3,
    ...
    add_rate_to_ratedeck/3,
    remove_rate_from_ratedeck/3,
...
dedicated_zones(#kazoo_model{'dedicated_ips' = IPs}) ->
    maps:fold(
        fun(_IP, #{'zone' := Zone}, Zones) -> [Zone | Zones] end,
        [],
        IPs
    ).
...
dedicated_hosts(#kazoo_model{'dedicated_ips' = IPs}) ->
    maps:fold(
        fun(_IP, #{'host' := Host}, Hosts) -> [Host | Hosts] end,
        [],
        IPs
    ).
...
-spec has_service_plan_rate_matching(model(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    boolean().
has_service_plan_rate_matching(
    #kazoo_model{
        'accounts' = Accounts,
        'service_plans' = SPs
    } = Model,
        ...
        [{SPId, SP}] ->
            case lists:member({SPId, SP}, SPs) of
                ...
                'true' ->
                    ...
                    Ratedeck = ratedeck(Model, RatedeckId),
                    _ = ?DEBUG("from service plan ~s, got ratedeck ~s: ~p", [
                        SPId, RatedeckId, Ratedeck
                    ]),
                    has_rate_matching(Ratedeck, DID)
            end
    end.
...
has_rate_matching(Ratedeck, Number) ->
    case
        [
            Cost
         || {Prefix, Cost} <- maps:to_list(Ratedeck),
...
does_ratedeck_exist(#kazoo_model{} = Model, RatedeckId) ->
    Ratedeck = ratedeck(Model, RatedeckId),
    is_map(Ratedeck) andalso
        0 < maps:size(Ratedeck).
...
-spec add_number_to_account(
    model(), kz_term:ne_binary(), kz_term:ne_binary(), pqc_cb_api:response()
) -> model().
...
-spec add_service_plan(model() | service_plans(), kzd_service_plan:doc()) ->
    model() | service_plans().
...
add_service_plan(Model, AccountId, ServicePlan) ->
    add_service_plan_to_account(
        add_service_plan(Model, ServicePlan),
        AccountId,
        ServicePlan
    ).
...
remove_number_from_account(
    #kazoo_model{'numbers' = Numbers} = Model,
    Number
) ->
