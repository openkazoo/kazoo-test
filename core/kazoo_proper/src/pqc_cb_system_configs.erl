...
-spec get_node_config(pqc_cb_api:api(), kz_term:ne_binary(), kz_term:ne_binary()) ->
...
-spec patch_default_config(pqc_cb_api:api(), kz_tern:ne_binary(), kz_json:object()) ->
    kzd_system_configs:doc().
...
seq() ->
    ...
    <<"default_knee">> = kz_json:get_value(
        [<<"default">>, <<"nested">>, <<"knee">>], GetSchemaDefaults
    ),

    Section = kz_json:from_list([
        {<<"key">>, <<"value">>},
        {<<"nested">>, kz_json:from_list([{<<"knee">>, <<"nalue">>}])}
    ]),
    Defaults = kz_json:from_list([
        {<<"default">>, Section},
        {<<"id">>, ?CATEGORY_ID}
    ]),
    ...
    <<"nalue">> = kapps_config:get(
        ?CATEGORY_ID, [<<"nested">>, <<"knee">>], 'undefined', <<"default">>
    ),
    <<"value">> = kapps_config:get(?CATEGORY_ID, <<"key">>, 'undefined', <<"default">>),

    NodeSection = kz_json:from_list([
        {<<"key">>, <<"node">>},
